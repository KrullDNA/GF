...
class KDNASelectColumns {
	 ...
	 * @uses KDNAFormsModel::get_grid_columns()
	 * @uses KDNASelectColumns::get_selectable_entry_meta()
	...
	public static function select_columns_page() {
		...
		if ( empty( $form_id ) ) {
			echo esc_html__( 'Oops! We could not locate your form. Please try again.', 'kdnaforms' );
			exit;
		}
		?>
		<html>
		...
		?>
			...
			<style type="text/css">
				...
				.kdnaform_select_column_heading {
					padding-bottom: 7px;
					font-size: 1.125rem;
				}
		...
		<?php
		$columns = KDNAFormsModel::get_grid_columns( $form_id );
		...
		array_push( $form['fields'], array( 'id' => 'created_by', 'label' => __( 'User', 'kdnaforms' ) ) );
		...
		<div class="gcolumn_wrapper">
			<div class="gcolumn_container_left">
				<div class="kdnaform_select_column_heading"><?php esc_html_e( 'Active Columns', 'kdnaforms' ); ?></div>
			...
			<div class="gcolumn_container_right" id="available_column">
				<div class="kdnaform_select_column_heading"> <?php esc_html_e( 'Inactive Columns', 'kdnaforms' ); ?></div>
				<ul id="sortable_available" class="sortable_connected">
				<?php
				foreach ( $form['fields'] as $field ) {
					/* @var KDNA_Field $field */
					if ( KDNAFormsModel::get_input_type( $field ) == 'checkbox' && ! in_array( $field->id, $field_ids ) ) {
						?>
						<li id="<?php echo esc_attr( $field->id ); ?>"><?php echo esc_html( KDNACommon::get_label( $field ) ) ?></li>
					<?php
					}
					...
					if ( is_array( $inputs ) ) {
						foreach ( $inputs as $input ) {
							...
							if ( ! in_array( $input['id'], $field_ids ) && ! ( $field->type == 'creditcard' && in_array( $input['id'], array( floatval( "{$field->id}.2" ), floatval( "{$field->id}.3" ), floatval( "{$field->id}.5" ) ) ) ) ) {
								?>
								<li id="<?php echo esc_attr( $input['id'] ); ?>"><?php echo esc_html( KDNACommon::get_label( $field, $input['id'] ) ); ?></li>
							<?php
							}
						}
		...
		<div class="panel-buttons">
			<input type="button" value="  <?php esc_attr_e( 'Save', 'kdnaforms' ); ?>  " class="button-primary" onclick="SelectColumns();" onkeypress="SelectColumns();" />&nbsp;
			<input type="button" value="<?php esc_attr_e( 'Cancel', 'kdnaforms' ); ?>" class="button" onclick="self.parent.tb_remove();" onkeypress="self.parent.tb_remove();" />
		</div>
...
KDNASelectColumns::select_columns_page();
