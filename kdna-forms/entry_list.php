...
class KDNAEntryList {
	...
	public static function all_entries_page() {
		...
		if ( rgget( 'restored' ) ) {
			...
			KDNACommon::add_dismissible_message(
				sprintf(
					esc_html__( '%s restored from the Trash.', 'kdnaforms' ),
					esc_html__( '1 entry', 'kdnaforms' )
				),
				'success'
			);
		...
		if ( rgget( 'deleted' ) ) {
			...
			KDNACommon::add_dismissible_message(
				sprintf(
					esc_html__( '%s permanently deleted.', 'kdnaforms' ),
					esc_html__( '1 entry', 'kdnaforms' )
				),
				'success'
			);
		...
		if ( rgget( 'trashed_entry' ) ) {
			...
			KDNACommon::add_dismissible_message(
				sprintf(
					esc_html__( '1 entry moved to the Trash. %sUndo%s', 'kdnaforms' ),
		...
		if ( sizeof( $forms ) == 0 ) {
			...
			<div style="margin:50px 0 0 10px;">
				<?php echo sprintf( esc_html__( "You don't have any active forms. Let's go %screate one%s", 'kdnaforms' ), '<a href="?page=gf_new_form">', '</a>' ); ?>
			</div>
			<?php
		} else {
			if ( empty( $form_id ) ) {
			 ...
			 */
			do_action( 'kdnaform_pre_entry_list', $form_id );
			 ...
			 * @param int $form_id The ID of the form that the entry list is being displayed for.
			 */
			do_action( 'kdnaform_post_entry_list', $form_id );
		}
	...
	public static function get_screen_options_markup( $status, $args ) {
		...
		$return = $status;
		if ( ! KDNAForms::get_page() == 'entry_list' ) {
			return $return;
		}
		...
		$filter_title         = esc_html__( 'Default Filter', 'kdnaforms' );
		$pagination_title     = esc_html__( 'Pagination', 'kdnaforms' );
		$entries_label        = esc_html__( 'Number of entries per page:', 'kdnaforms' );
		$display_mode_title   = esc_html__( 'Display Mode', 'kdnaforms' );
		$display_modes_markup = join( "\n", self::get_display_modes_markup() );

		$button  = get_submit_button( esc_html__( 'Apply', 'kdnaforms' ), 'button button-primary', 'screen-options-apply', false );
		$return .= "
			<fieldset class='screen-options'>
            <legend>{$filter_title}</legend>
            <div class='kdnaform-screen-options-filters-container'>
				{$radios_str}
            </div>
            </fieldset>
            <fieldset class='screen-options'>
	...
	private static function get_display_modes_markup() {
		...
		$display_modes = array(
			array(
				'id'    => 'standard',
				'label' => esc_html__( 'Standard', 'kdnaforms' ),
			),
			array(
				'id'    => 'full_width',
				'label' => esc_html__( 'Full Width', 'kdnaforms' ),
			),
		);
	...
	public static function get_screen_options_values() {
		...
		$option_values = get_user_option( 'kdnaform_entries_screen_options' );
	...
	public static function leads_page( $form_id ) {
		...
		if ( empty( $form['id'] ) ) {
			KDNACommon::add_error_message( esc_html__( "Oops! We couldn't find your form. Please try again.", 'kdnaforms' ) );
			KDNAForms::admin_header();

			return;
		}

		$table = new KDNA_Entry_List_Table( array( 'form_id' => $form_id, 'form' => $form ) );
		...
		?>
			<form id="entry_list_form" method="post" class="kdnaform-settings-panel__content kdnaform-settings-panel__content--entry-list">
				<?php
				$table->views();
                ?>
                <div id="entry_search_container">
                    ...
                    <a style="" class="button" id="entry_search_button"
                       href="javascript:Search('<?php echo esc_js( $table->get_orderby() ); ?>', '<?php echo esc_js( $table->get_order() ) ?>', <?php echo absint( $form_id ); ?>, jQuery('.kdnaform-filter-value').val(), '<?php echo esc_js( $table->get_filter() ) ?>', jQuery('.kdnaform-filter-field').val(), jQuery('.kdnaform-filter-operator').val());"><?php esc_html_e( 'Search', 'kdnaforms' ) ?></a>
	...
	public static function get_icon_url( $path ) {
		...
		return KDNACommon::get_base_url() . "/images/doctypes/$file_name";
	}
	...
	public static function get_filter_links( $form, $include_counts = true ) {
		...
		$counts       = $include_counts ? KDNAFormsModel::get_form_counts( $form_id ) : array();
		$filter_links = array(
			array(
				'id'            => 'all',
				'field_filters' => array(),
				'count'         => rgar( $counts, 'total', 0 ),
				'label'         => esc_html_x( 'All', 'Entry List', 'kdnaforms' ),
			),
			array(
				'id'            => 'unread',
				'field_filters' => array(
				...
				'label'         => esc_html_x( 'Unread', 'Entry List', 'kdnaforms' ),
			),
			array(
				'id'            => 'star',
				'field_filters' => array(
				...
				'label'         => esc_html_x( 'Starred', 'Entry List', 'kdnaforms' ),
			),
			array(
				'id'            => 'spam',
				'field_filters' => array(),
				'count'         => rgar( $counts, 'spam', 0 ),
				'label'         => esc_html__( 'Spam', 'kdnaforms' ),
			),
			array(
				'id'            => 'trash',
				'field_filters' => array(),
				'count'         => rgar( $counts, 'trash', 0 ),
				'label'         => esc_html__( 'Trash', 'kdnaforms' ),
			),
		);
		 ...
		 */
		return apply_filters( 'kdnaform_filter_links_entry_list', $filter_links, $form, $include_counts, $counts );
	}
...
/**
 * Class KDNA_Entry_List_Table
 *
 * @since 2.0
 */
final class KDNA_Entry_List_Table extends WP_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		if ( ! isset( $this->_form ) ) {
			...
			$this->_form = KDNAFormsModel::get_form_meta( $form_id );
		}
	...
	public function get_filter_links( $include_counts = true ) {
		...
		return KDNAEntryList::get_filter_links( $form, $include_counts );
	}
	...
	function prepare_items() {
		...
		if ( ! empty( $sort_field ) ) {
			...
			$sort_field_meta = KDNAAPI::get_field( $form_id, $sort_field );

			if ( $sort_field_meta instanceof KDNA_Field ) {
		 ...
		 * @param array $args {
		 *
		 *     Array of arguments that will be passed to KDNAAPI::get_entries() to fetch the entries to be displayed.
		 *
	...
	function get_columns() {
		...
		if ( empty( $columns ) ) {
			$table_columns['field_id-id'] = esc_html__( 'Entry Id', 'kdnaforms' );
		}
		...
		$table_columns['column_selector'] = '<a name="<div class=\'tb-title\'><div class=\'tb-title__logo\'></div><div class=\'tb-title__text\'><div class=\'tb-title__main\'>' . esc_attr__( 'Select Entry Table Columns', 'kdnaforms' ) . '</div><div class=\'tb-title__sub\'>' . esc_attr( 'Drag & drop to order and select which columns are displayed in the entries table.', 'kdnaforms' ) . '</div></div></div>" aria-label="' . esc_attr( $title ) . '" href="' . esc_url( $column_selector_url ) . '" class="thickbox entries_edit_icon"><i title="' . esc_attr( $title ) . '" class="kdnaform-icon kdnaform-icon--cog kdnaform-icon--entries-edit"></i></a>';
		 ...
		 */
		$table_columns = apply_filters( 'kdnaform_entry_list_columns', $table_columns, $form_id );

		return apply_filters( 'kdnaform_entry_list_columns_' . $form_id, $table_columns, $form_id );
	}
	...
	function _column_is_starred( $entry, $classes, $data, $primary ) {
		...
		if ( $this->filter !== 'trash' ) {
			$action = KDNACommon::current_user_can_any( 'kdnaforms_edit_entries' ) ? "ToggleStar(this, '" . intval( $entry['id'] ) . "','" . esc_attr( $this->filter ) . "');" : 'return false;';
			?>
			<img role="presentation" id="star_image_<?php echo esc_attr( $entry['id'] ) ?>" src="<?php echo esc_url( KDNACommon::get_base_url() ); ?>/images/star<?php echo intval( $entry['is_starred'] ) ?>.svg" onclick="<?php echo $action; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" />
			<?php
		}
		echo '</td>';
	}
	...
	function column_default( $entry, $column_id ) {
		...
		if ( ! empty( $field ) && $field->type == 'post_category' ) {
			$value = KDNACommon::prepare_post_category_value( $value, $field, 'entry_list' );
		}

		// Filtering lead value
		$value = apply_filters( 'kdnaform_get_field_value', $value, $entry, $field );

		switch ( $field_id ) {

			case 'source_url' :
				$value = "<a href='" . esc_attr( $entry['source_url'] ) . "' target='_blank' alt='" . esc_attr( $entry['source_url'] ) . "'>.../" . esc_attr( KDNACommon::truncate_url( $entry['source_url'] ) ) . '</a>';
				break;

			case 'date_created' :
			case 'payment_date' :
				$value = KDNACommon::format_date( $value, false );
				break;

			case 'payment_amount' :
				$value = KDNACommon::to_money( $value, $entry['currency'] );
				break;

			case 'payment_status' :
				$value = KDNACommon::get_entry_payment_status_text( $entry['payment_status'] );
				break;
		...
		$value = apply_filters( 'kdnaform_entries_field_value', $value, $form_id, $field_id, $entry );
		...
		if ( $column_id == $primary ) {
			...
			$aria_label = sprintf( esc_html__( 'View entry number %s', 'kdnaforms' ), $entry['id'] );
			 ...
			 */
			$column_value = apply_filters( 'kdnaform_entries_primary_column_filter', $column_value, $form_id, $field_id, $entry, $query_string, $edit_url, $value );
			 ...
			 * @param string $query_string The current page's query string
			 */
			echo apply_filters( 'kdnaform_entries_column_filter', $value, $form_id, $field_id, $entry, $query_string ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

			// Maintains gap between value and content from kdnaform_entries_column which existed when using 1.9 and earlier.
			echo '&nbsp; ';
			 ...
			 * @param string $query_string The current page's query string
			 */
			do_action( 'kdnaform_entries_column', $form_id, $field_id, $value, $entry, $query_string );
		}
	...
	function no_items() {
		...
		switch ( $this->filter ) {
			case 'unread' :
				$message = isset( $_GET['field_id'] ) ? esc_html__( 'This form does not have any unread entries matching the search criteria.', 'kdnaforms' ) : esc_html__( 'This form does not have any unread entries.', 'kdnaforms' ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				break;

			case 'star' :
				$message = isset( $_GET['field_id'] ) ? esc_html__( 'This form does not have any starred entries matching the search criteria.', 'kdnaforms' ) : esc_html__( 'This form does not have any starred entries.', 'kdnaforms' ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				break;

			case 'spam' :
				$message = esc_html__( 'This form does not have any spam.', 'kdnaforms' );
				break;

			case 'trash' :
				$message = isset( $_GET['field_id'] ) ? esc_html__( 'This form does not have any entries in the trash matching the search criteria.', 'kdnaforms' ) : esc_html__( 'This form does not have any entries in the trash.', 'kdnaforms' ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				break;

			default :
				if ( isset( $_GET['field_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
					$message = esc_html__( 'This form does not have any entries matching the search criteria.', 'kdnaforms' );
				} elseif ( $this->filter ) {
					$message = esc_html__( 'This form does not have any entries matching the selected filter.', 'kdnaforms' );
				} else {
					$message = esc_html__( 'This form does not have any entries yet.', 'kdnaforms' );
				}
		}
	...
	protected function handle_row_actions( $entry, $column_name, $primary ) {
		...
		switch ( $this->filter ) {
			case 'trash':
				$actions['view'] = array(
					'class' => 'edit',
					'link'  => '<a href="' . esc_url( $detail_url ) . '">' . esc_html__( 'View', 'kdnaforms' ) . '</a>',
				);
				if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
					$actions['restore'] = array(
						'class' => 'edit',
						'link'  => "<a data-wp-lists='delete:the-list:entry_row_" . esc_attr( $entry['id'] ) . '::status=restore&entry=' . esc_attr( $entry['id'] ) . "' href=\"" . wp_nonce_url( '?page=gf_entries', 'gf_delete_entry' ) . '">' . esc_html__( 'Restore', 'kdnaforms' ) . '</a>',
					);
					$delete_link        = '<a data-wp-lists="delete:the-list:entry_row_' . esc_attr( $entry['id'] ) . '::status=delete&entry=' . esc_attr( $entry['id'] ) . '" href="' . wp_nonce_url( '?page=gf_entries', 'gf_delete_entry' ) . '">' . esc_html__( 'Delete Permanently', 'kdnaforms' ) . '</a>';
					...
					$actions['delete'] = array(
						'class' => 'delete',
						'link'  => apply_filters( 'kdnaform_delete_entry_link', $delete_link ),
					);
				}
				break;
			case 'spam':
				...
				if ( KDNACommon::current_user_can_any( 'kdnaforms_edit_entries' ) ) {
					$actions['unspam'] = array(
						'class' => 'edit',
						'link' => "<a data-wp-lists='delete:the-list:entry_row_" . esc_attr($entry['id']) . "::status=unspam&entry=" . esc_attr($entry['id']) . "' aria-label=\"" . esc_attr__('Mark this entry as not spam', 'kdnaforms') . "\" href=\"" . wp_nonce_url('?page=gf_entries', 'gf_delete_entry') . "\">" . esc_html__('Not Spam', 'kdnaforms') . '</a>',
					);
				}
				if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
					$delete_link = '<a data-wp-lists="delete:the-list:entry_row_' . esc_attr( $entry['id'] ) . '::status=delete&entry=' . esc_attr( $entry['id'] ) . '" href="' . wp_nonce_url( '?page=gf_entries', 'gf_delete_entry' ) . '">' . esc_html__( 'Delete Permanently', 'kdnaforms' ) . '</a>';
			...
			default:
				...
				if ( KDNACommon::current_user_can_any( 'kdnaforms_edit_entries' ) ) {
					$actions['mark_read'] = array(
						'class' => 'edit',
						'link'  => '<a id="mark_read_' . esc_attr( $entry['id'] ) . '" aria-label="Mark this entry as read" href="javascript:ToggleRead(\'' . esc_js( $entry['id'] ) . '\', \'' . esc_js( $this->filter ) . '\');" style="display:' . ( $entry['is_read'] ? 'none' : 'inline' ) . '">' . esc_html__( 'Mark read', 'kdnaforms' ) . '</a><a id="mark_unread_' . absint( $entry['id'] ) . '" aria-label="' . esc_attr__( 'Mark this entry as unread', 'kdnaforms' ) . '" href="javascript:ToggleRead(\'' . esc_js( $entry['id'] ) . '\', \'' . esc_js( $this->filter ) . '\');" style="display:' . ( $entry['is_read'] ? 'inline' : 'none' ) . '">' . esc_html__( 'Mark unread', 'kdnaforms' ) . '</a>',
					);
				}
				if ( KDNACommon::spam_enabled( $form_id ) && KDNACommon::current_user_can_any( 'kdnaforms_edit_entries' ) ) {
					$actions['spam'] = array(
						'class' => 'spam',
						'link'  => '<a data-wp-lists="delete:the-list:entry_row_' . esc_attr( $entry['id'] ) . '::status=spam&entry=' . esc_attr( $entry['id'] ) . '" href="' . wp_nonce_url( '?page=gf_entries', 'gf_delete_entry' ) . '">' . esc_html__( 'Mark as Spam', 'kdnaforms' ) . '</a>',
					);
				}
				if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
					$actions['delete'] = array(
						'class' => 'delete',
						'link'  => '<a data-wp-lists="delete:the-list:entry_row_' . esc_attr( $entry['id'] ) . '::status=trash&entry=' . esc_attr( $entry['id'] ) . '" href="' . wp_nonce_url( '?page=gf_entries', 'gf_delete_entry' ) . '">' . esc_html__( 'Trash', 'kdnaforms' ) . '</a>',
					);
				}
				break;
		}
		...
		<div class="row-actions">
			 ...
			 */
			$actions = apply_filters( 'kdnaform_entries_action_links', $actions, $this->filter, $entry, $form_id );
			...
			do_action( 'kdnaform_entries_first_column_actions', $form_id, $field_id, $value, $entry, $query_string );
		 ...
		 * @param string $query_string The current page's query string
		 */
		do_action( 'kdnaform_entries_first_column', $form_id, $field_id, $value, $entry, $query_string );
	...
	function get_bulk_actions() {
		...
		switch ( $this->filter ) {
			case 'trash' :
				if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
					$actions['restore'] = esc_html__( 'Restore', 'kdnaforms' );
					$actions['delete']  = esc_html__( 'Delete Permanently', 'kdnaforms' );
				}
				break;
			case 'spam' :
				if ( KDNACommon::current_user_can_any( 'kdnaforms_edit_entries' ) ) {
					$actions['unspam'] = esc_html__( 'Not Spam', 'kdnaforms' );
				}
				if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
					$actions['delete'] = esc_html__( 'Delete Permanently', 'kdnaforms' );
				}
			...
			default:
				if ( KDNACommon::current_user_can_any( 'kdnaforms_edit_entries' ) ) {
					$actions['mark_read']            = esc_html__( 'Mark as Read', 'kdnaforms' );
					$actions['mark_unread']          = esc_html__( 'Mark as Unread', 'kdnaforms' );
					$actions['add_star']             = esc_html__( 'Add Star', 'kdnaforms' );
					$actions['remove_star']          = esc_html__( 'Remove Star', 'kdnaforms' );
				}
				$actions['resend_notifications'] = esc_html__( 'Resend Notifications', 'kdnaforms' );
				$actions['print']                = esc_html__( 'Print', 'kdnaforms' );

				if ( KDNACommon::spam_enabled( $this->get_form_id() ) && KDNACommon::current_user_can_any( 'kdnaforms_edit_entries' ) ) {
					$actions['spam'] = esc_html__( 'Spam', 'kdnaforms' );
				}
				if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
					$actions['trash'] = esc_html__( 'Trash', 'kdnaforms' );
				}
		}
	...
	function bulk_actions( $which = '' ) {
		...
		if ( ! in_array( $filter, array( 'trash', 'spam' ) ) || ! KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
			return;
		}

		$message      = $filter == 'trash' ? esc_html__( "WARNING! This operation cannot be undone. Empty trash? 'Ok' to empty trash. 'Cancel' to abort.", 'kdnaforms' ) : esc_html__( "WARNING! This operation cannot be undone. Permanently delete all spam? 'Ok' to delete. 'Cancel' to abort.", 'kdnaforms' );
		$button_label = $filter == 'trash' ? __( 'Empty Trash', 'kdnaforms' ) : __( 'Delete All Spam', 'kdnaforms' );
		?>
	...
	function process_action() {
		...
		if ( $delete_permanently ) {
			if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
				KDNAFormsModel::delete_leads_by_form( $form_id, $this->filter );
			}
			return;
		}
		...
		if ( $single_action ) {
			...
			switch ( $single_action ) {
				case 'delete' :
					if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
						...
						$message = esc_html__( 'Entry deleted.', 'kdnaforms' );
					} else {
						$message = esc_html__( "You don't have adequate permission to delete entries.", 'kdnaforms' );
					}

					break;
				case 'change_columns':
					$columns = KDNACommon::json_decode( rgpost( 'grid_columns' ), true );
					KDNAFormsModel::update_grid_column_meta( $form_id, $columns );
			...
			$entries = empty( $select_all ) ? rgpost( 'entry' ) : KDNAAPI::get_entry_ids( $form_id, $search_criteria );

			$entry_count = count( $entries ) > 1 ? sprintf( esc_html__( '%d entries', 'kdnaforms' ), count( $entries ) ) : esc_html__( '1 entry', 'kdnaforms' );
			...
			switch ( $bulk_action ) {
				case 'delete':
					if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
						KDNAFormsModel::delete_entries( $entries );
						$message = sprintf( esc_html__( '%s deleted.', 'kdnaforms' ), $entry_count );
					} else {
						$message       = esc_html__( "You don't have adequate permission to delete entries.", 'kdnaforms' );
						$message_class = 'error';
					}
				...
				case 'trash':
					if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
						KDNAFormsModel::change_entries_status( $entries, 'trash' );
						$message = sprintf( esc_html__( '%s moved to Trash.', 'kdnaforms' ), $entry_count );
					} else {
						$message       = esc_html__( "You don't have adequate permissions to trash entries.", 'kdnaforms' );
						$message_class = 'error';
					}
					break;
				...
				case 'restore':
					if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
						KDNAFormsModel::restore_entries_status( $entries );
						$message = sprintf( esc_html__( '%s restored from the Trash.', 'kdnaforms' ), $entry_count );
					} else {
						$message       = esc_html__( "You don't have adequate permissions to restore entries.", 'kdnaforms' );
						$message_class = 'error';
					}
					break;
				...
				case 'unspam':
					KDNAFormsModel::restore_entries_status( $entries );
					$message = sprintf( esc_html__( '%s restored from the spam.', 'kdnaforms' ), $entry_count );
					break;

				case 'spam':
					KDNAFormsModel::change_entries_status( $entries, 'spam' );
					$message = sprintf( esc_html__( '%s marked as spam.', 'kdnaforms' ), $entry_count );
					break;

				case 'mark_read':
					KDNAFormsModel::update_entries_property( $entries, 'is_read', 1 );
					$message = sprintf( esc_html__( '%s marked as read.', 'kdnaforms' ), $entry_count );
					break;

				case 'mark_unread':
					KDNAFormsModel::update_entries_property( $entries, 'is_read', 0 );
					$message = sprintf( esc_html__( '%s marked as unread.', 'kdnaforms' ), $entry_count );
					break;

				case 'add_star':
					KDNAFormsModel::update_entries_property( $entries, 'is_starred', 1 );
					$message = sprintf( esc_html__( '%s starred.', 'kdnaforms' ), $entry_count );
					break;

				case 'remove_star':
					KDNAFormsModel::update_entries_property( $entries, 'is_starred', 0 );
					$message = sprintf( esc_html__( '%s unstarred.', 'kdnaforms' ), $entry_count );
					break;
	...
	function output_scripts() {
		...
		$field_filters = KDNACommon::get_field_filter_settings( $form );
		...
		<script type="text/javascript">
			...
			function UpdateEntryProperty(entry_id, name, value) {
				...
				mysack.onError = function () {
					alert(<?php echo json_encode( __( 'Ajax error while setting entry property', 'kdnaforms' ) ); ?>)
				};
				mysack.runAJAX();
			...
			function handleBulkApply(actionElement) {
				...
				if (leadIds.length == 0) {
					alert(<?php echo json_encode( __( 'Please select at least one entry.', 'kdnaforms' ) ); ?>);
					return false;
				}
				...
				switch (action) {
					...
					case 'resend_notifications':
						...
						tb_show(<?php echo json_encode( esc_html__( 'Resend Notifications', 'kdnaforms' ) ); ?>, '#TB_inline?width=350&amp;inlineId=notifications_modal_container', '');
						return false;
						break;

					case 'print':
						resetPrintUI();
						tb_show(<?php echo json_encode( esc_html__( 'Print Entries', 'kdnaforms' ) ); ?>, '#TB_inline?width=350&amp;height=250&amp;inlineId=print_modal_container', '');
						return false;
						break;
			...
			function BulkResendNotifications() {
				...
				jQuery(".kdnaform_notifications:checked").each(function () {
				...
				if (selectedNotifications.length <= 0) {
					displayMessage(<?php echo json_encode( esc_html__( 'You must select at least one type of notification to resend.', 'kdnaforms' ) ); ?>, "error", "#notifications_container");
					return;
				}
			...
			function resetResendNotificationsUI() {
				...
				jQuery( '.kdnaform_notifications' ).prop( 'checked' , false );
			...
			function toggleNotificationOverride(isInit) {
				...
				if (jQuery(".kdnaform_notifications:checked").length > 0) {
			...
			function setSelectAllText() {
				...
				jQuery("#kdnaform-select-all-message td").html(tr);
			}

			function getSelectAllText() {
				var count;
				count = jQuery("#the-list tr.entry_row:visible:not('#kdnaform-select-all-message')").length;
			...
			function getSelectAllTr() {
				...
				return "<tr id='kdnaform-select-all-message' class='no-items' style='display:none;background-color:lightyellow;text-align:center;'><td colspan='{0}'>{1}</td></tr>".gformFormat(colspan, t);
			}
			function toggleSelectAll(visible) {
				if (gformVars.countAllEntries <= gformVars.perPage) {
					jQuery('#kdnaform-select-all-message').hide();
					return;
				}

				if (visible)
					setSelectAllText();
				jQuery('#kdnaform-select-all-message').toggle(visible);
			}
			...
			function clearSelectAllMessage() {
				...
				jQuery("#kdnaform-select-all-message").hide();
				jQuery("#kdnaform-select-all-message td").html('');
			}
			...
			function selectAllEntriesOnAllPages() {
				...
				jQuery("#kdnaform-select-all-message td").html(trHtmlClearSelection);
			}
			...
			jQuery(document).ready(function () {
				...
				var list = jQuery("#the-list").wpList({ delAfter: afterAjaxDelete, alt: <?php echo json_encode( esc_html__( 'Entry List', 'kdnaforms' ) ) ?>});
				...
				jQuery("#entry_filters").on("keypress", ".kdnaform-filter-value", (function (event) {
					if (event.keyCode == 13) {
						var urlParams = new URLSearchParams(window.location.search);
						var currentFilter = urlParams.get('filter') || '';
						Search(<?php echo json_encode( $orderby ); ?>, <?php echo json_encode( $order ); ?>, <?php echo absint( $form_id ) ?>, jQuery('.kdnaform-filter-value').val(), currentFilter, jQuery('.kdnaform-filter-field').val(), jQuery('.kdnaform-filter-operator').val());
			...
			<div id="notifications_container">
				...
				<div id="post_tag" class="tagsdiv">
					<div id="resend_notifications_options">
						...
						$notifications = KDNACommon::get_notifications( 'resend_notifications', $form );

						if ( ! is_array( $notifications ) || count( $form['notifications'] ) <= 0 ) {
							?>
							<p class="description"><?php esc_html_e( 'You cannot resend notifications for these entries because this form does not currently have any notifications configured.', 'kdnaforms' ); ?></p>

							<a href="<?php echo esc_url( admin_url( "admin.php?page=gf_edit_forms&view=settings&subview=notification&id={$form['id']}" ) ); ?>" class="button"><?php esc_html_e( 'Configure Notifications', 'kdnaforms' ) ?></a>
							<?php
						} else {
							?>
							<p class="description"><?php esc_html_e( 'Specify which notifications you would like to resend for the selected entries.', 'kdnaforms' ); ?></p>
							<?php
							foreach ( $notifications as $notification ) {
								?>
								<input type="checkbox" class="kdnaform_notifications" value="<?php echo esc_attr( $notification['id'] ); ?>" id="notification_<?php echo esc_attr( $notification['id'] ); ?>" onclick="toggleNotificationOverride();" />
							...
							<div id="notifications_override_settings" style="display:none;">
								...
								<p class="description" style="padding-top:0; margin-top:0;">
									<?php esc_html_e( 'You may override the default notification settings by entering a comma delimited list of emails to which the selected notifications should be sent.', 'kdnaforms' ); ?>
								</p>
								<label for="notification_override_email"><?php esc_html_e( 'Send To', 'kdnaforms' ); ?> <?php kdnaform_tooltip( 'notification_override_email' ) ?></label><br />
					...
					<div id="resend_notifications_close" style="display:none;margin:10px 0 0;">
						<input type="button" name="resend_notifications_close_button" value="<?php esc_attr_e( 'Close Window', 'kdnaforms' ) ?>" class="button" style="" onclick="closeModal(true);" />
					</div>
			...
			<div id="print_container">
				...
				<div class="tagsdiv">
					<div id="print_options">
						...
						<input type="button" value="<?php esc_attr_e( 'Print', 'kdnaforms' ); ?>" class="button" onclick="BulkPrint();" />
	...
	function get_grid_columns() {
		if ( ! isset( $this->_grid_columns ) ) {
			$this->_grid_columns = KDNAFormsModel::get_grid_columns( $this->get_form_id(), true );
