...
jQuery( document ).on( 'kdnaform_post_render', gformBindFormatPricingFields );
...
function gformDeleteUploadedFile(formId, fieldId, deleteButton){
    ...
    //removing the kdnaform_hidden class
    parent.find('.validation_message,#extensions_message_' + formId + '_' + fieldId).removeClass("kdnaform_hidden");
    ...
    var filesJson = jQuery('#kdnaform_uploaded_files_' + formId).val();

    if(filesJson){
        var files = jQuery.secureEvalJSON(filesJson);
        if(files) {
            var inputName = "input_" + fieldId;
            var $multfile = parent.find("#kdnaform_multifile_upload_" + formId + "_" + fieldId );
            ...
            jQuery('#kdnaform_uploaded_files_' + formId).val(jQuery.toJSON(files));
        }
    }
}
...
function gformIsHidden(element){
	 ...
	 */
	return gform.applyFilters('kdnaform_is_hidden', isHidden, element);
...
var gformCalculateTotalPrice =  gform.tools.debounce(function(formId){
	...
	//kdnaform_product_total filter. Allows users to perform custom price calculation
	if(window["kdnaform_product_total"])
		price = window["kdnaform_product_total"](formId, price);

	price = gform.applyFilters('kdnaform_product_total', price, formId);
...
function gformUpdateTotalFieldPrice( formId, price ) {
	...
	var isLegacy = document.querySelector( '#kdnaform_wrapper_' + formId + '.kdnaform_legacy_markup_wrapper' );
...
function gformGetBasePrice(formId, productFieldId){
    ...
    var c = new gform.Currency(kdna_global.gf_currency_config);
...
function gformFormatMoney(text, isNumeric){
    if(!kdna_global.gf_currency_config)
        return text;

    var currency = new gform.Currency(kdna_global.gf_currency_config);
...
function gformGetOptionLabel(element, selected_value, current_price, form_id, field_id){
    ...
    if(window["kdnaform_format_option_label"])
        label = kdnaform_format_option_label(label, original_label, price_label, current_price, price, form_id, field_id);
...
function bindProductChangeEvent() {
	// For backwards compatibility, fire jQuery kdnaform_price_change event.
	document.addEventListener( 'gform/products/product_field_changed', function( event ) {
		...
		jQuery( document ).trigger( 'kdnaform_price_change', [ productIds, event.detail.htmlInput, this ] );
	} );
}
...
function gformAddListItem( addButton, max ) {
    ...
    $clone = gform.applyFilters( 'kdnaform_list_item_pre_add', $clone, $group );
    ...
    gform.doAction( 'kdnaform_list_post_item_add', $clone, $container );
...
function gformDeleteListItem( deleteButton, max ) {
    ...
    gform.doAction( 'kdnaform_list_post_item_delete', $container );
...
function gformAdjustRowAttributes( $container ) {
    ...
    if( $container.parents( '.kdnaform_wrapper' ).hasClass( 'kdnaform_legacy_markup_wrapper' ) ) {
        return;
    }
...
function gformAddRepeaterItem( addButton, max ) {
	...
	gform.doAction('kdnaform_repeater_post_item_add', $clone, $container);
...
function gformDeleteRepeaterItem(deleteButton, max) {
	...
	gform.doAction('kdnaform_repeater_post_item_delete', $container);
...
function gformMatchCard(id) {
    ...
    var cardContainer = jQuery('#' + id).parents('.gfield').find('.kdnaform_card_icon_container');

    if(!cardType) {

        jQuery(cardContainer).find('.kdnaform_card_icon').removeClass('kdnaform_card_icon_selected kdnaform_card_icon_inactive');

    } else {

        jQuery(cardContainer).find('.kdnaform_card_icon').removeClass('kdnaform_card_icon_selected').addClass('kdnaform_card_icon_inactive');
        jQuery(cardContainer).find('.kdnaform_card_icon_' + cardType).removeClass('kdnaform_card_icon_inactive').addClass('kdnaform_card_icon_selected');
    }
}
...
function gformToggleCreditCard(){
    if(jQuery("#kdnaform_payment_method_creditcard").is(":checked"))
        jQuery(".kdnaform_card_fields_container").slideDown();
    else
        jQuery(".kdnaform_card_fields_container").slideUp();
}
...
function gformInitChosenFields( fieldList, noResultsText ) {
    return jQuery( fieldList ).each( function(){
        ...
        if( ( element.is( ':visible' ) || isConvoForm ) && element.siblings( '.chosen-container' ).length == 0 ) {
            ...
            var options = gform.applyFilters( 'kdnaform_chosen_options', chosenOptions, element );
...
var GFMergeTag = function() {
	...
	GFMergeTag.getMergeTagValue = function( formId, inputId, modifier ) {
		...
		let value = window.gform.applyFilters( 'kdnaform_value_merge_tag_' + formId + '_' + mergeTagInfo.fieldId, false, inputForFilter, mergeTagInfo.modifier );
		if ( value !== false ) {
			return value;
		}
...
var GFCalc = function(formId, formulaFields){
    ...
    this.init = function(formId, formulaFields) {
	    ...
	    jQuery(document)
		    .off("kdnaform_post_conditional_logic.gfCalc_{0}".gformFormat(formId))
		    .on("kdnaform_post_conditional_logic.gfCalc_{0}".gformFormat(formId), function(){
    ...
    this.runCalc = function(formulaField, formId) {
        var calcObj      = this,
            ...
            formula      = gform.applyFilters( 'kdnaform_calculation_formula', formulaField.formula, formulaField, formId, calcObj ),
        ...
        if( window["kdnaform_calculation_result"] ) {
            result = window["kdnaform_calculation_result"](result, formulaField, formId, calcObj);
            if( window.console )
                console.log( '"kdnaform_calculation_result" function is deprecated since version 1.8! Use "kdnaform_calculation_result" JS hook instead.' );
        }

        // allow users to modify result with their own function
        result = gform.applyFilters( 'kdnaform_calculation_result', result, formulaField, formId, calcObj );

        // allow result to be custom formatted
        var formattedResult = gform.applyFilters( 'kdnaform_calculation_format_result', false, result, formulaField, formId, calcObj );
    ...
    this.bindCalcEvents = function(formulaField, formId) {
        ...
        for(var i in matches) {
            ...
            gform.doAction( 'kdnaform_post_calculation_events', matches[i], formulaField, formId, calcObj );
    ...
    this.replaceFieldTags = function( formId, expr, formulaField ) {
        ...
        for(i in matches) {
            ...
            value = gform.applyFilters( 'kdnaform_merge_tag_value_pre_calculation', value, matches[i], isVisible, formulaField, formId );
...
gform.recaptcha = {
	...
	renderRecaptcha: function() {
		jQuery( '.ginput_recaptcha:not(.gform-initialized)' ).each( function() {
			 ...
			 */
			const callback = gform.applyFilters( 'kdnaform_recaptcha_callback', false, $elem );
			if ( callback ) {
...
jQuery( document ).on( 'kdnaform_post_render', gform.recaptcha.renderOnRecaptchaLoaded );
...
function gformValidateFileSize( field, max_file_size ) {
	...
	if ( file && file.size > max_file_size ) {
		...
		validation_element.text(file.name + " - " + kdnaform_kdnaforms.strings.file_exceeds_limit);
		// Announce error.
		wp.a11y.speak( file.name + " - " + kdnaform_kdnaforms.strings.file_exceeds_limit );
...
(function (gfMultiFileUploader, $) {
    ...
    var strings = typeof kdnaform_kdnaforms != 'undefined' ? kdnaform_kdnaforms.strings : {};
    var imagesUrl = typeof kdnaform_kdnaforms != 'undefined' ? kdnaform_kdnaforms.vars.images_url : "";

	$(document).on('kdnaform_post_render', function(e, formID){
		$( "form#kdnaform_" + formID + " .kdnaform_fileupload_multifile" ).each( function(){
			setup( this );
		} );
	...
	$(document).on("kdnaform_post_conditional_logic", function(e,formID, fields, isInit){
		if(!isInit){
    ...
    $(document).ready(function () {
        if((typeof adminpage !== 'undefined' && adminpage === 'toplevel_page_gf_edit_forms')|| typeof plupload == 'undefined'){
            $(".kdnaform_button_select_files").prop("disabled", true);
        } else if (typeof adminpage !== 'undefined' && adminpage.indexOf('_page_gf_entries') > -1) {
            $(".kdnaform_fileupload_multifile").each(function(){
                setup(this);
            });
        }
    });
    ...
    function setup(uploadElement){
	    ...
	    uploader.bind( 'Init', function( up, params ) {
		    if ( ! up.features.dragdrop ) {
			    $( ".kdnaform_drop_instructions" ).hide();
		    }
	    ...
	    function setFieldAccessibility( container ) {
		    ...
		    var button = container.querySelectorAll( '.kdnaform_button_select_files' )[ 0 ];
        ...
        uploader.bind('FilesAdded', function(up, files) {
            ...
            $.each(files, function(i, file) {
                 ...
                 */
                statusMarkup = gform.applyFilters( 'kdnaform_file_upload_status_markup', statusMarkup, file, size, strings, removeFileJs, up )
            ...
            var formElementID = "form#kdnaform_" + formID;
            var uidElementID = "input:hidden[name='kdnaform_unique_id']";
            ...
            up.settings.multipart_params.kdnaform_unique_id = uniqueID;
            up.start();
		...
		uploader.bind('FileUploaded', function(up, file, result) {
			 ...
			 */
			html = gform.applyFilters('kdnaform_file_upload_markup', html, file, up, strings, imagesUrl, response);
		...
		function getAllFiles(){
			var selector = '#kdnaform_uploaded_files_' + formID,
        ...
        function setUploadedFiles(fieldID, files){
            var allFiles = getAllFiles();
            var $uploadedFiles = $('#kdnaform_uploaded_files_' + formID);
            var inputName = getInputName(fieldID);
...
function gformInitSpinner(formId, spinnerUrl, isLegacy = true) {
	...
	document.addEventListener( 'gform/submission/submission_aborted', function( event ) {
		...
		jQuery( '#kdnaform_ajax_spinner_' + event.detail.form.dataset.formid ).remove();
	} );
}
...
function gformShowSpinner( formId, spinnerUrl ) {
	...
	let filteredSpinner = gform.applyFilters('kdnaform_spinner_url', spinnerUrl, formId);
	let defaultSpinner = gform.applyFilters('kdnaform_spinner_url', kdna_global.spinnerUrl, formId);
	...
	let $spinnerTarget = gform.applyFilters('kdnaform_spinner_target_elem', jQuery('#kdnaform_submit_button_' + formId + ', #kdnaform_wrapper_' + formId + ' .kdnaform_next_button, #kdnaform_send_resume_link_button_' + formId), formId);
...
function gformAddSpinner(formId, spinnerUrl) {
	...
	if (typeof spinnerUrl == 'undefined' || !spinnerUrl) {
		spinnerUrl = gform.applyFilters('kdnaform_spinner_url', kdna_global.spinnerUrl, formId);
	}
	...
	if (jQuery('#kdnaform_ajax_spinner_' + formId).length == 0) {
		 ...
		 */
		var $spinnerTarget = gform.applyFilters('kdnaform_spinner_target_elem', jQuery('#kdnaform_submit_button_' + formId + ', #kdnaform_wrapper_' + formId + ' .kdnaform_next_button, #kdnaform_send_resume_link_button_' + formId), formId);
		$spinnerTarget.after('<img id="kdnaform_ajax_spinner_' + formId + '"  class="kdnaform_ajax_spinner" src="' + spinnerUrl + '" alt="" />');
	}
...
function gf_raw_input_change( event, elem ) {
    ...
    var $input    = jQuery( elem ),
	     ...
	     */
        fieldMeta = gform.applyFilters( 'kdnaform_field_meta_raw_input_change', { fieldId: fieldId, formId: formId }, $input, event );
...
function gf_input_change( elem, formId, fieldId ) {
    gform.doAction( 'kdnaform_input_change', elem, formId, fieldId );
}
...
jQuery( function() {
	gform.tools.trigger( 'kdnaform_main_scripts_loaded' );
} );
