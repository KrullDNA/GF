...
if ( ! class_exists( 'KDNAWidget' ) ) {
	/**
	 * Class KDNAWidget
	 *
	 * Facilitates the creation of the KDNA Forms widget
	 *
	 * @see WP_Widget
	 */
	class KDNAWidget extends WP_Widget {

		/**
		 * KDNAWidget constructor.
		...
		function __construct() {
			...
			WP_Widget::__construct(
				'kdnaform_widget',
				__( 'Form', 'kdnaforms' ),
				array( 'classname' => 'kdnaform_widget', 'description' => $description ),
				array( 'width' => 200, 'height' => 250, 'id_base' => 'kdnaform_widget' )
			);
		 ...
		 * @see KDNAFormsModel::get_form_meta
		 * @see KDNAForms::print_form_scripts
		 * @see KDNAForms::get_form
		 *
		...
		function widget( $args, $instance ) {
			...
			if ( empty( $instance ) ) {
				$forms = KDNAFormsModel::get_forms( 1, 'title' );
				if ( empty( $forms ) ) {
					return '';
				}
				$form                        = KDNAAPI::get_form( $forms[0]->id );
			...
			// Creating form
			if ( empty( $form ) ) {
				$form = KDNAFormsModel::get_form_meta( $instance['form_id'] );
			}
			if ( empty( $instance['disable_scripts'] ) && ! is_admin() ) {
				KDNAForms::print_form_scripts( $form, $instance['ajax'] );
			}

			$form_markup = KDNAForms::get_form( $instance['form_id'], $instance['showtitle'], $instance['showdescription'], false, null, $instance['ajax'], $tabindex );
		...
		function form( $instance ) {
			...
			$instance = wp_parse_args( (array) $instance, array( 'title' => __( 'Contact Us', 'kdnaforms' ), 'tabindex' => '0' ) );
			?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title', 'kdnaforms' ); ?>:</label>
			...
			<p>
				<label for="<?php echo absint( $this->get_field_id( 'form_id' ) ); ?>"><?php esc_html_e( 'Select a Form', 'kdnaforms' ); ?>:</label>
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'showtitle' ) ); ?>"><?php esc_html_e( 'Display form title', 'kdnaforms' ); ?></label><br />
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'showdescription' ) ); ?>"><?php esc_html_e( 'Display form description', 'kdnaforms' ); ?></label><br />
			</p>
			<p>
