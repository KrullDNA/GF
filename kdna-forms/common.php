 ...
 * Class KDNACommon
 *
 * Includes common methods accessed throughout KDNA Forms and add-ons.
 */
class KDNACommon {
	...
	/**
	 * An array of event start times set by KDNACommon::timer_start().
	 *
	 * @since 2.7.1
	 *
	 * @var float[]
	...
	// deprecated; set to KDNAForms::$version in KDNAForms::init() for backwards compat
	...
	public static function get_selection_fields( $form, $selected_field_id ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			$field_label = KDNAFormsModel::get_label( $field );
	...
	public static function remove_currency_symbol( $value, $currency = null ) {
		...
		if ( ! is_array( $currency ) ) {
			$code = empty( $currency ) ? KDNACommon::get_currency() : $currency;
			if ( empty( $code ) ) {
				$code = 'USD';
			}
	...
	public static function format_number( $number, $number_format, $currency_code_or_entry = '', $include_thousands_sep = false ) {
		...
		if ( $number_format === 'currency' ) {
			...
			if ( empty( $currency_code ) ) {
				$currency_code = KDNACommon::get_submission_currency();
			}
	...
	public static function recursive_add_index_file( $dir ) {
		...
		if ( ! is_dir( $dir ) || ! wp_is_writable( $dir ) || is_link( $dir ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Path ' . $dir . ' is not a valid path or is not writable' );

			return;
		}

		if ( ! ( $dp = opendir( $dir ) ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Unable to open directory: ' . $dir );
		...
		KDNACommon::log_debug( __METHOD__ . '(): Adding file: ' . $index_file_path );
		 ...
		 */
		if ( true === apply_filters( 'kdnaform_check_empty_index_file_exists', false, $dir ) ) {
	...
	public static function add_htaccess_file() {
		...
		$txt   = '# Disable parsing of PHP for some server configurations. This file may be removed or modified on certain server configurations by using by the kdnaform_upload_root_htaccess_rules filter. Please consult your system administrator before removing this file.
<Files *>
  SetHandler none
...
<IfModule headers_module>
  Header set X-Robots-Tag "noindex"
		...
		/**
		 * A filter to allow the modification/disabling of parsing certain PHP within KDNA Forms
		 *
		 * @since 1.9.2
		 *
		 * @param mixed $rules The Rules of what to parse or not to parse
		 */
		$rules = apply_filters( 'kdnaform_upload_root_htaccess_rules', $rules );
		if ( ! empty( $rules ) ) {
	 ...
	 *
	 * @since unknown
	 * @since 2.6.2 Updated to use KDNA_PLUGIN_DIR_PATH.
	 *
	 * @return string
	 */
	public static function get_base_path() {
		return untrailingslashit( KDNA_PLUGIN_DIR_PATH );
	}
	 ...
	 * @access  public
	 *
	 * @used-by KDNAFormSettings::handle_confirmation_edit_submission()
	 * @used-by KDNA_Field_Post_Image::get_value_save_entry()
	 * @used-by KDNA_Field_Website::get_value_entry_detail()
	 * @used-by KDNA_Field_Website::validate()
	 *
	...
	public static function is_valid_url( $url ) {
		 ...
		 *
		 * @since 2.0.7.12
		 * @see   https://docs.kdnaforms.com/kdnaform_rfc_url_validation/
		 *
		 * @param bool true If RFC validation should be enabled. Defaults to true. Set to false to disable RFC validation.
		 */
		$use_rfc = apply_filters( 'kdnaform_rfc_url_validation', true );
		 ...
		 *
		 * @since 2.0.7.12
		 * @see   https://docs.kdnaforms.com/kdnaform_is_valid_url/
		 *
		 * @param bool   $is_valid True if valid. False otherwise.
		 * @param string $url      The URL being validated.
		 */
		$is_valid = apply_filters( 'kdnaform_is_valid_url', $is_valid, $url );
	...
	public static function get_label( $field, $input_id = 0, $input_only = false, $allow_admin_label = true ) {
		return KDNAFormsModel::get_label( $field, $input_id, $input_only, $allow_admin_label );
	}

	public static function get_input( $field, $id ) {
		return KDNAFormsModel::get_input( $field, $id );
	}
	...
	public static function insert_variables( $fields, $element_id, $hide_all_fields = false, $callback = '', $onchange = '', $max_label_size = 40, $exclude = null, $args = '', $class_name = '' ) {
		...
		$exclude    = apply_filters( 'kdnaform_merge_tag_list_exclude', $exclude, $element_id, $fields );
		...
		$class    = trim( $class_name . ' kdnaform_merge_tags' );
	...
	public static function get_merge_tags( $fields, $element_id, $hide_all_fields = false, $exclude_field_types = array(), $option = '' ) {
		...
		if ( ! $hide_all_fields ) {
			$ungrouped[] = array(
				'tag'   => '{all_fields}',
				'label' => esc_html__( 'All Submitted Fields', 'kdnaforms' )
			);
		}
				...
				switch ( $input_type ) {
					...
					case 'name' :
						...
						if ( $field->nameFormat == 'extended' ) {
							...
							$prefix                   = KDNACommon::get_input( $field, $field->id . '.2' );
							$suffix                   = KDNACommon::get_input( $field, $field->id . '.8' );
		...
		if ( ! empty( $pricing_fields ) ) {
			...
			if ( ! $hide_all_fields ) {
				$pricing_group[] = array(
					'tag'   => '{pricing_fields}',
					'label' => esc_html__( 'All Pricing Fields', 'kdnaforms' )
				);
			}
		...
		$other_group[] = array( 'tag' => '{ip}', 'label' => esc_html__( 'User IP Address', 'kdnaforms' ) );
		$other_group[] = array(
			'tag'   => '{date_mdy}',
			'label' => esc_html__( 'Date', 'kdnaforms' ) . ' (mm/dd/yyyy)'
		);
		$other_group[] = array(
			'tag'   => '{date_dmy}',
			'label' => esc_html__( 'Date', 'kdnaforms' ) . ' (dd/mm/yyyy)'
		);
		$other_group[] = array(
			'tag'   => '{embed_post:ID}',
			'label' => esc_html__( 'Embed Post/Page Id', 'kdnaforms' )
		);
		$other_group[] = array(
			'tag'   => '{embed_post:post_title}',
			'label' => esc_html__( 'Embed Post/Page Title', 'kdnaforms' )
		);
		$other_group[] = array( 'tag' => '{embed_url}', 'label' => esc_html__( 'Embed URL', 'kdnaforms' ) );
		$other_group[] = array( 'tag' => '{entry_id}', 'label' => esc_html__( 'Entry Id', 'kdnaforms' ) );
		$other_group[] = array( 'tag' => '{entry_url}', 'label' => esc_html__( 'Entry URL', 'kdnaforms' ) );
		$other_group[] = array( 'tag' => '{form_id}', 'label' => esc_html__( 'Form Id', 'kdnaforms' ) );
		$other_group[] = array( 'tag' => '{form_title}', 'label' => esc_html__( 'Form Title', 'kdnaforms' ) );
		$other_group[] = array( 'tag' => '{user_agent}', 'label' => esc_html__( 'HTTP User Agent', 'kdnaforms' ) );
		$other_group[] = array( 'tag' => '{referer}', 'label' => esc_html__( 'HTTP Referer URL', 'kdnaforms' ) );

		if ( self::has_post_field( $fields ) ) {
			$other_group[] = array( 'tag' => '{post_id}', 'label' => esc_html__( 'Post Id', 'kdnaforms' ) );
			$other_group[] = array(
				'tag'   => '{post_edit_url}',
				'label' => esc_html__( 'Post Edit URL', 'kdnaforms' )
			);
		}

		$other_group[] = array(
			'tag'   => '{user:display_name}',
			'label' => esc_html__( 'User Display Name', 'kdnaforms' )
		);
		$other_group[] = array( 'tag' => '{user:user_email}', 'label' => esc_html__( 'User Email', 'kdnaforms' ) );
		$other_group[] = array( 'tag' => '{user:user_login}', 'label' => esc_html__( 'User Login', 'kdnaforms' ) );

		$form_id = isset( $fields[0] ) ? $fields[0]->formId : rgget( 'id' );
		$form_id = absint( $form_id );

		$custom_group = apply_filters( 'kdnaform_custom_merge_tags', array(), $form_id, $fields, $element_id );
		...
		$merge_tags = array(
			...
			'required'  => array(
				'label' => esc_html__( 'Required form fields', 'kdnaforms' ),
				'tags'  => $required_group,
			),
			'optional'  => array(
				'label' => esc_html__( 'Optional form fields', 'kdnaforms' ),
				'tags'  => $optional_group,
			),
			'pricing'   => array(
				'label' => esc_html__( 'Pricing form fields', 'kdnaforms' ),
				'tags'  => $pricing_group,
			),
			'other'     => array(
				'label' => esc_html__( 'Other', 'kdnaforms' ),
				'tags'  => $other_group,
			),
			'custom'    => array(
				'label' => esc_html__( 'Custom', 'kdnaforms' ),
				'tags'  => $custom_group,
			)
		);
	...
	public static function get_field_merge_tags( $field, $option = '' ) {
		...
		$merge_tags = array();
		$tag_args   = KDNAFormsModel::get_input_type( $field ) == 'list' ? ":{$option}" : ''; //args currently only supported by list field

		$inputs = $field->get_entry_inputs();

		if ( is_array( $inputs ) ) {

			if ( KDNAFormsModel::get_input_type( $field ) == 'checkbox' ) {
				$value        = '{' . esc_html( KDNACommon::get_label( $field, $field->id ) ) . ':' . $field->id . "{$tag_args}}";
				$merge_tags[] = array(
					'tag'   => $value,
					'label' => esc_html( KDNACommon::get_label( $field, $field->id ) )
				);
			}

			foreach ( $field->inputs as $input ) {
				if ( KDNAFormsModel::get_input_type( $field ) == 'creditcard' ) {
					//only include the credit card type (field_id.4) and number (field_id.1)
					if ( $input['id'] == $field->id . '.1' || $input['id'] == $field->id . '.4' ) {
						$value        = '{' . esc_html( KDNACommon::get_label( $field, $input['id'] ) ) . ':' . $input['id'] . "{$tag_args}}";
						$merge_tags[] = array(
							'tag'   => $value,
							'label' => esc_html( KDNACommon::get_label( $field, $input['id'] ) )
						);
					}
				} else {
					$value        = '{' . esc_html( KDNACommon::get_label( $field, $input['id'] ) ) . ':' . $input['id'] . "{$tag_args}}";
			...
			$value        = '{' . esc_html( KDNACommon::get_label( $field ) ) . ':' . $field->id . "{$tag_args}}";
			$merge_tags[] = array(
				'tag'   => $value,
				'label' => esc_html( KDNACommon::get_label( $field ) )
			);
		}
	...
	public static function insert_field_variable( $field, $max_label_size = 40, $args = '' ) {
		...
		$tag_args = KDNAFormsModel::get_input_type( $field ) == 'list' ? ":{$args}" : ''; //args currently only supported by list field

		if ( is_array( $field->inputs ) ) {
			if ( KDNAFormsModel::get_input_type( $field ) == 'checkbox' ) {
				?>
				<option value='<?php echo '{' . esc_html( KDNACommon::get_label( $field, $field->id ) ) . ':' . esc_html( $field->id . "{$tag_args}}" ); ?>'><?php echo esc_html( KDNACommon::get_label( $field, $field->id ) ) ?></option>
				<?php
			}

			foreach ( $field->inputs as $input ) {
				?>
				<option value='<?php echo '{' . esc_html( KDNACommon::get_label( $field, $input['id'] ) ) . ':' . esc_html( $input['id'] . "{$tag_args}}" ); ?>'><?php echo esc_html( KDNACommon::get_label( $field, $input['id'] ) ) ?></option>
				<?php
			}
		} else {
			?>
			<option value='<?php echo '{' . esc_html( KDNACommon::get_label( $field ) ) . ':' . esc_attr( $field->id . "{$tag_args}}" ) ?>'><?php echo esc_html( KDNACommon::get_label( $field ) ) ?></option>
			<?php
		}
	}
	...
	public static function insert_post_content_variables( $fields, $element_id, $callback, $max_label_size = 25 ) {
		...
		self::insert_variables( $fields, $element_id, true, '', $insert_variables_onchange, $max_label_size, null, '', 'kdnaform_content_template_merge_tags' );
		?>
		&nbsp;&nbsp;
		<select id="<?php echo esc_attr( $element_id ); ?>_image_size_select" onchange="InsertPostImageVariable('<?php echo esc_js( $element_id ); ?>', '<?php echo esc_js( $element_id ); ?>'); SetCustomFieldTemplate();" style="display:none;">
			<option value=""><?php esc_html_e( 'Select image size', 'kdnaforms' ) ?></option>
			<option value="thumbnail"><?php esc_html_e( 'Thumbnail', 'kdnaforms' ) ?></option>
			<option value="thumbnail:left"><?php esc_html_e( 'Thumbnail - Left Aligned', 'kdnaforms' ) ?></option>
			<option value="thumbnail:center"><?php esc_html_e( 'Thumbnail - Centered', 'kdnaforms' ) ?></option>
			<option value="thumbnail:right"><?php esc_html_e( 'Thumbnail - Right Aligned', 'kdnaforms' ) ?></option>

			<option value="medium"><?php esc_html_e( 'Medium', 'kdnaforms' ) ?></option>
			<option value="medium:left"><?php esc_html_e( 'Medium - Left Aligned', 'kdnaforms' ) ?></option>
			<option value="medium:center"><?php esc_html_e( 'Medium - Centered', 'kdnaforms' ) ?></option>
			<option value="medium:right"><?php esc_html_e( 'Medium - Right Aligned', 'kdnaforms' ) ?></option>

			<option value="large"><?php esc_html_e( 'Large', 'kdnaforms' ) ?></option>
			<option value="large:left"><?php esc_html_e( 'Large - Left Aligned', 'kdnaforms' ) ?></option>
			<option value="large:center"><?php esc_html_e( 'Large - Centered', 'kdnaforms' ) ?></option>
			<option value="large:right"><?php esc_html_e( 'Large - Right Aligned', 'kdnaforms' ) ?></option>

			<option value="full"><?php esc_html_e( 'Full Size', 'kdnaforms' ) ?></option>
			<option value="full:left"><?php esc_html_e( 'Full Size - Left Aligned', 'kdnaforms' ) ?></option>
			<option value="full:center"><?php esc_html_e( 'Full Size - Centered', 'kdnaforms' ) ?></option>
			<option value="full:right"><?php esc_html_e( 'Full Size - Right Aligned', 'kdnaforms' ) ?></option>
		</select>
		<?php
	}
	...
	public static function insert_calculation_variables( $fields, $element_id, $onchange = '', $callback = '', $max_label_size = 40 ) {
		...
		<select id="<?php echo esc_attr( $element_id ); ?>_variable_select" class="<?php echo esc_attr( $class ); ?>" onchange="<?php echo esc_attr( $onchange ); ?>">
			<option value=''><?php esc_html_e( 'Insert Merge Tag', 'kdnaforms' ); ?></option>
			<optgroup label="<?php esc_attr_e( 'Allowable form fields', 'kdnaforms' ); ?>">
				<?php foreach ( $fields as $field ) {
					...
					if ( KDNAFormsModel::get_input_type( $field ) == 'checkbox' ) {
						foreach ( $field->inputs as $input ) { ?>
							<option value='<?php echo esc_attr( '{' . esc_html( KDNACommon::get_label( $field, $input['id'] ) ) . ':' . $input['id'] . '}' ); ?>'><?php echo esc_html( KDNACommon::get_label( $field, $input['id'] ) ); ?></option>
						<?php }
					} else {
			...
			$custom_merge_tags = apply_filters( 'kdnaform_custom_merge_tags', array(), $form_id, $fields, $element_id );

			if ( is_array( $custom_merge_tags ) && ! empty( $custom_merge_tags ) ) { ?>

				<optgroup label="<?php esc_attr_e( 'Custom', 'kdnaforms' ); ?>">
	...
	public static function replace_variables( $text, $form, $lead, $url_encode = false, $esc_html = true, $nl2br = true, $format = 'html', $aux_data = array() ) {
		 ...
		 *
		 * @see   https://docs.kdnaforms.com/kdnaform_merge_tag_data/
		 */
		$data = apply_filters( 'kdnaform_merge_tag_data', $data, $text, $form, $lead );

		$lead = $data['entry'];

		$text = $format == 'html' && $nl2br ? nl2br( $text ) : $text;
		$text = apply_filters( 'kdnaform_pre_replace_merge_tags', $text, $form, $lead, $url_encode, $esc_html, $nl2br, $format );
		...
		if ( false !== strpos( $text, 'merge_tag' ) ) {
			// Replacing conditional merge tag variables: [kdnaforms action="conditional" merge_tag="{Other Services:4}" ....
		...
		// TODO: Deprecate the 'kdnaform_replace_merge_tags' and replace it with a call to the 'kdnaform_merge_tag_filter'
		//$text = apply_filters('kdnaform_merge_tag_filter', $text, false, false, false );
	...
	public static function get_embed_post() {
		...
		if ( ! rgempty( 'kdnaform_embed_post' ) ) {
			$post_id    = absint( rgpost( 'kdnaform_embed_post' ) );
	...
	public static function replace_variables_prepopulate( $text, $url_encode = false, $entry = false, $esc_html = false, $form = false, $nl2br = false, $format = 'html' ) {
		...
		if ( is_string( $text ) && strpos( $text, '{' ) !== false ) {
			...
			//embed url
			$current_page_url = empty( $entry ) ? KDNAFormsModel::get_current_page_url() : rgar( $entry, 'source_url' );
				...
				foreach ( $matches as $match ) {
					...
					if( ! empty( $date_string ) ) {
						...
						if ( empty( $date_format ) ) {
							...
							$date_format = $is_diff ? esc_html__( '%s ago', 'kdnaforms' ) : get_option( 'date_format' );
						}
			...
			// ip
			$request_ip = rgars( $form, 'personalData/preventIP' ) ? '' : KDNAFormsModel::get_ip();
		 ...
		 */
		$text = apply_filters( 'kdnaform_replace_merge_tags', $text, $form, $entry, $url_encode, $esc_html, $nl2br, $format );
		...
		foreach ( $form['fields'] as $field ) {
			...
			switch ( $field->type ) {
				...
				case 'section' :
					...
					if ( KDNAFormsModel::is_field_hidden( $form, $field, array(), $lead ) ) {
						break;
					}

					if ( ( ! KDNACommon::is_section_empty( $field, $form, $lead ) || $display_empty ) && ! $field->is_administrative() ) {
					...
					$field_value = apply_filters( 'kdnaform_merge_tag_filter', $field_value, $merge_tag, $options, $field, $field_label, $format );
				...
				default :
					...
					if ( self::is_product_field( $field->type ) ) {
						...
						$display_product_summary = apply_filters( 'kdnaform_display_product_summary', true, $field, $form, $lead );
						if ( $display_product_summary ) {
							break;
						}
					} else if ( KDNAFormsModel::is_field_hidden( $form, $field, array(), $lead ) ) {
						// ignore fields hidden by conditional logic
						break;
					}

					$field->set_modifiers( $options_array );
					$raw_field_value = KDNAFormsModel::get_lead_field_value( $lead, $field );
					$field_value     = KDNACommon::get_lead_field_display( $field, $raw_field_value, $lead, $use_text, $format, 'email' );

					$display_field = true;
					//depending on parameters, don't display adminOnly or hidden fields
					if ( $no_admin && $field->is_administrative() ) {
						$display_field = false;
					} else if ( $no_hidden && KDNAFormsModel::get_input_type( $field ) == 'hidden' ) {
						$display_field = false;
					}
					...
					$field_value = apply_filters( 'kdnaform_merge_tag_filter', $field_value, $merge_tag, $options, $field, $raw_field_value, $format );
	...
	public static function get_submitted_pricing_fields( $form, $lead, $format, $use_text = true, $use_admin_label = false ) {
		$form_id     = $form['id'];
		$products    = KDNACommon::get_product_fields( $form, $lead, $use_text, $use_admin_label );
		$field_data  = '';
		if ( ! empty( $products['products'] ) ) {
			switch ( $format ) {
				case 'text':
					$field_data = KDNA_Order_Summary::render( $form, $lead, 'pricing-fields-text', $use_text, $use_admin_label );
					break;

				default :
					$field_data = KDNA_Order_Summary::render( $form, $lead, 'pricing-fields-html', $use_text, $use_admin_label );
					break;
			}
			 ...
			 *
			 * @since 2.1.2.5
			 * @see   https://docs.kdnaforms.com/kdnaform_order_summary/
			 *
	...
	private static function prepare_user_notification( $form, $lead, $override_options = false ) {
		...
		$message        = KDNACommon::replace_variables( rgget( 'message', $form['autoResponder'] ), $form, $lead, false, false, ! rgget( 'disableAutoformat', $form['autoResponder'] ), $message_format );
		 ...
		 */
		if ( apply_filters( 'kdnaform_enable_shortcode_notification_message', true, $form, $lead ) ) {
			$message = do_shortcode( $message );
		}

		//Running trough variable replacement
		$to        = KDNACommon::replace_variables( $to, $form, $lead, false, false );
		$from      = KDNACommon::replace_variables( rgget( 'from', $form['autoResponder'] ), $form, $lead, false, false );
		$bcc       = KDNACommon::replace_variables( rgget( 'bcc', $form['autoResponder'] ), $form, $lead, false, false );
		$reply_to  = KDNACommon::replace_variables( rgget( 'replyTo', $form['autoResponder'] ), $form, $lead, false, false );
		$from_name = KDNACommon::replace_variables( rgget( 'fromName', $form['autoResponder'] ), $form, $lead, false, false );
	...
	private static function prepare_admin_notification( $form, $lead, $override_options = false ) {
		...
		$message        = KDNACommon::replace_variables( rgget( 'message', $form['notification'] ), $form, $lead, false, false, ! rgget( 'disableAutoformat', $form['notification'] ), $message_format );

		if ( apply_filters( 'kdnaform_enable_shortcode_notification_message', true, $form, $lead ) ) {
			$message = do_shortcode( $message );
		}

		// License expired message removed - plugin is now free.
		...
		if ( rgempty( 'fromNameField', $form['notification'] ) ) {
			...
			$field     = KDNAFormsModel::get_field( $form, rgget( 'fromNameField', $form['notification'] ) );
			$value     = KDNAFormsModel::get_lead_field_value( $lead, $field );
			$from_name = KDNACommon::get_lead_field_display( $field, $value );
		}
		...
		if ( rgempty( 'routing', $form['notification'] ) ) {
			...
			foreach ( $form['notification']['routing'] as $routing ) {
				...
				$source_field   = KDNAFormsModel::get_field( $form, $routing['fieldId'] );
				$field_value    = KDNAFormsModel::get_lead_field_value( $lead, $source_field );
				$is_value_match = KDNAFormsModel::is_value_match( $field_value, $routing['value'], $routing['operator'], $source_field, $routing, $form ) && ! KDNAFormsModel::is_field_hidden( $form, $source_field, array(), $lead );
		...
		$email_to  = KDNACommon::replace_variables( $email_to, $form, $lead, false, false );
		$from      = KDNACommon::replace_variables( $from, $form, $lead, false, false );
		$bcc       = KDNACommon::replace_variables( rgget( 'bcc', $form['notification'] ), $form, $lead, false, false );
		$reply_to  = KDNACommon::replace_variables( $replyTo, $form, $lead, false, false );
	...
	public static function send_notification( $notification, $form, $lead, $data = array() ) {
		...
		if ( empty( $email_to ) && rgar( $notification, 'toType' ) == 'routing' && ! empty( $notification['routing'] ) ) {
			...
			foreach ( $notification['routing'] as $routing ) {
				...
				KDNACommon::log_debug( __METHOD__ . '(): Evaluating Routing - rule => ' . print_r( $routing, 1 ) );

				$source_field   = KDNAFormsModel::get_field( $form, rgar( $routing, 'fieldId' ) );
				$field_value    = KDNAFormsModel::get_lead_field_value( $lead, $source_field );
				$is_value_match = KDNAFormsModel::is_value_match( $field_value, rgar( $routing, 'value', '' ), rgar( $routing, 'operator', 'is' ), $source_field, $routing, $form ) && ! KDNAFormsModel::is_field_hidden( $form, $source_field, array(), $lead );

				if ( $is_value_match ) {
					$email_to[] = $routing['email'];
				}

				KDNACommon::log_debug( __METHOD__ . '(): Evaluating Routing - field value => ' . print_r( $field_value, 1 ) );
				$is_value_match = $is_value_match ? 'Yes' : 'No';
				KDNACommon::log_debug( __METHOD__ . '(): Evaluating Routing - is value match? ' . $is_value_match );
			}

			$email_to = join( ',', $email_to );
		} elseif ( ! empty( $to_field ) ) {
			$source_field = KDNAFormsModel::get_field( $form, $to_field );
			$email_to     = KDNAFormsModel::get_lead_field_value( $lead, $source_field );
		}

		// Running through variable replacement
		$to        = KDNACommon::remove_extra_commas( KDNACommon::replace_variables( $email_to, $form, $lead, false, false, false, 'text', $data ) );
		$subject   = KDNACommon::replace_variables( rgar( $notification, 'subject' ), $form, $lead, false, false, false, 'text', $data );
		$from      = KDNACommon::replace_variables( rgar( $notification, 'from' ), $form, $lead, false, false, false, 'text', $data );
		$from_name = KDNACommon::replace_variables( rgar( $notification, 'fromName' ), $form, $lead, false, false, false, 'text', $data );
		$bcc       = KDNACommon::remove_extra_commas( KDNACommon::replace_variables( rgar( $notification, 'bcc' ), $form, $lead, false, false, false, 'text', $data ) );
		$replyTo   = KDNACommon::remove_extra_commas( KDNACommon::replace_variables( rgar( $notification, 'replyTo' ), $form, $lead, false, false, false, 'text', $data ) );
		...
		$message = KDNACommon::replace_variables( rgar( $notification, 'message' ), $form, $lead, false, false, ! rgar( $notification, 'disableAutoformat' ), $merge_tag_format, $data );
		...
		if ( rgar( $notification, 'enableAttachments', false ) ) {
			...
			$upload_fields = KDNACommon::get_fields_by_type( $form, array( 'fileupload' ) );
			...
			foreach ( $upload_fields as $upload_field ) {
				...
				foreach ( $files as $file ) {
					if ( is_string( $file ) ) {
						$root_url = rgar( KDNA_Field_FileUpload::get_file_upload_path_info( $file, $entry_id ), 'url' );
						if ( ! str_starts_with( $file, $root_url ) ) {
							self::log_debug( __METHOD__ . sprintf( '(): Not attaching file from URL: %s', $file ) );
							continue;
						}

						$file_path = KDNAFormsModel::get_physical_file_path( $file, rgar( $lead, 'id' ) );
		...
		if ( $message_format === 'multipart' ) {
			...
			$text_message = KDNACommon::replace_variables( rgar( $notification, 'message' ), $form, $lead, false, false, ! rgar( $notification, 'disableAutoformat' ), 'text', $data );
	...
	public static function send_notifications( $notification_ids, $form, $lead, $do_conditional_logic = true, $event = 'form_submission', $data = array() ) {
		...
		if ( ! is_array( $notification_ids ) || empty( $notification_ids ) ) {
			KDNACommon::log_debug( __METHOD__ . "(): Aborting. No notifications to process for {$event} event for entry #{$entry_id}." );

			return;
		}

		KDNACommon::timer_start( __METHOD__ );
		KDNACommon::log_debug( __METHOD__ . "(): Processing notifications for {$event} event for entry #{$entry_id}: " . print_r( $notification_ids, true ) . "\n(only active/applicable notifications are sent)" );
		...
		foreach ( $notification_ids as $notification_id ) {
			...
			if ( isset( $form['notifications'][ $notification_id ]['isActive'] ) && ! $form['notifications'][ $notification_id ]['isActive'] ) {
				KDNACommon::log_debug( __METHOD__ . "(): Notification is inactive, not processing notification (#{$notification_id} - {$form['notifications'][$notification_id]['name']}) for entry #{$entry_id}." );
				continue;
			}
			...
			if ( $do_conditional_logic && ! KDNACommon::evaluate_conditional_logic( rgar( $notification, 'conditionalLogic' ), $form, $lead ) ) {
				KDNACommon::log_debug( __METHOD__ . "(): Notification conditional logic not met, not processing notification (#{$notification_id} - {$notification['name']}) for entry #{$entry_id}." );
				continue;
			}
			...
			if ( rgar( $notification, 'type' ) == 'user' ) {
				...
				$legacy_notification = KDNACommon::prepare_user_notification( $form, $lead );
				...
				$legacy_notification = KDNACommon::prepare_admin_notification( $form, $lead );
				$notification        = self::merge_legacy_notification( $notification, $legacy_notification );
			}
		...
		KDNACommon::log_debug( __METHOD__ . sprintf( '(): Sending notifications for entry (#%d) completed in %F seconds.', $entry_id, KDNACommon::timer_end( __METHOD__ ) ) );
	}

	public static function send_form_submission_notifications( $form, $lead ) {
		KDNAAPI::send_notifications( $form, $lead );
	}
	...
	public static function get_notifications_to_send( $event, $form, $lead ) {
		...
		foreach ( $notifications as $notification ) {
			if ( KDNACommon::evaluate_conditional_logic( rgar( $notification, 'conditionalLogic' ), $form, $lead ) ) {
	...
	public static function send_email( $from, $to, $bcc, $reply_to, $subject, $message, $from_name = '', $message_format = 'html', $attachments = '', $entry = false, $notification = false, $cc = null ) {
		...
		if ( ! KDNACommon::is_valid_email( $from ) ) {
		...
		$error = false;
		if ( ! KDNACommon::is_valid_email_list( $to ) ) {

			$error_info = esc_html__( 'Cannot send email because the TO address is invalid.', 'kdnaforms' );
			KDNAFormsModel::add_notification_note( $entry_id, false, $notification, $error_info, $email );

			$error = new WP_Error( 'invalid_to', 'Cannot send email because the TO address is invalid.' );

		} elseif ( empty( $subject ) && empty( $message ) ) {

			$error_info = esc_html__( 'Cannot send email because there is no SUBJECT and no MESSAGE.', 'kdnaforms' );
			KDNAFormsModel::add_notification_note( $entry_id, false, $notification, $error_info, $email );

			$error = new WP_Error( 'missing_subject_and_message', 'Cannot send email because there is no SUBJECT and no MESSAGE.' );

		} elseif ( ! KDNACommon::is_valid_email( $from ) ) {

			$error_info = esc_html__( 'Cannot send email because the FROM address is invalid.', 'kdnaforms' );
			KDNAFormsModel::add_notification_note( $entry_id, false, $notification, $error_info, $email );
		...
		if ( is_wp_error( $error ) ) {
			KDNACommon::log_error( __METHOD__ . '(): ' . $error->get_error_message() );
			KDNACommon::log_error( print_r( compact( 'to', 'subject', 'message' ), true ) );

			/**
			 * Fires when an email from KDNA Forms has failed to send
			 *
			 * @since 1.8.10
			 *
			 ...
			 *
			 */
			do_action( 'kdnaform_send_email_failed', $error, $email, $entry );
		 ...
		 * @since 2.2.0.3
		 */
		if ( apply_filters( 'kdnaform_format_email_to', false ) ) {
		...
		if ( KDNACommon::is_valid_email_list( $reply_to ) ) {
			$headers['Reply-To'] = "Reply-To: {$reply_to}";
		}

		if ( KDNACommon::is_valid_email_list( $bcc ) ) {
			$headers['Bcc'] = "Bcc: $bcc";
		}

		if ( KDNACommon::is_valid_email_list( $cc ) ) {
			$headers['Cc'] = "Cc: $cc";
		}

		$headers['Content-type'] = "Content-type: {$content_type}; charset=" . get_option( 'blog_charset' );

		$source_header_enabled = defined( 'KDNA_ENABLE_NOTIFICATION_EMAIL_HEADER' ) && KDNA_ENABLE_NOTIFICATION_EMAIL_HEADER;
		 ...
		 * @since 2.9.14
		 *
		 * @param string $header       The source header value. Defaults to `site={site_url}`, if the `KDNA_ENABLE_NOTIFICATION_EMAIL_HEADER` constant is used.
		 * @param array  $notification The current notification object.
		 ...
		 * @param array  $entry          The current Entry object.
		 */
		extract( apply_filters( 'kdnaform_pre_send_email', compact( 'to', 'subject', 'message', 'headers', 'attachments', 'abort_email' ), $message_format, $notification, $entry ) );
		...
		if ( ! $abort_email ) {
			...
			KDNACommon::log_debug( __METHOD__ . '(): Sending email via wp_mail().' );
			KDNACommon::log_debug( print_r( compact( 'to', 'subject', 'message', 'headers', 'attachments', 'abort_email' ), true ) );

			// Content type filter is needed to get around a bug in WordPress that ignores the boundary attribute and character set.
			add_filter( 'wp_mail_content_type', array( 'KDNACommon', 'set_content_type_boundary' ) );
			add_filter( 'wp_mail_charset', array( 'KDNACommon', 'set_mail_charset' ) );

			// Sending email.
			$is_success = wp_mail( $to, $subject, $message, $headers, $attachments );

			// Removing filter. It is only needed when sending GF notifications.
			remove_filter( 'wp_mail_content_type', array( 'KDNACommon', 'set_content_type_boundary' ) );
			remove_filter( 'wp_mail_charset', array( 'KDNACommon', 'set_mail_charset' ) );
			...
			KDNAFormsModel::add_notification_note( $entry_id, $result, $notification, $error_info, $email );

			KDNACommon::log_debug( __METHOD__ . "(): Result from wp_mail(): {$result}" );

			if ( ! is_wp_error( $is_success ) && $is_success ) {
				KDNACommon::log_debug( sprintf( '%s(): WordPress successfully passed the notification email (#%s - %s)%s to the sending server.', __METHOD__, $notification['id'], $notification['name'], $entry_info ) );
			} else {
				KDNACommon::log_error( sprintf( '%s(): WordPress was unable to send the notification email (#%s - %s)%s to the sending server.', __METHOD__, $notification['id'], $notification['name'], $entry_info ) );
			}

			if ( has_filter( 'phpmailer_init' ) ) {
				KDNACommon::log_debug( __METHOD__ . '(): The WordPress phpmailer_init hook has been detected, usually used by SMTP plugins. It can alter the email setup/content or sending server, and impact the notification deliverability.' );
			}

			if ( ! empty( $error_info ) ) {
				KDNACommon::log_debug( __METHOD__ . '(): PHPMailer class returned an error message: ' . $error_info );
			}
		} else {
			KDNACommon::log_debug( sprintf( '%s(): Aborting notification (#%s - %s)%s. The kdnaform_pre_send_email hook was used to set the abort_email parameter to true.', __METHOD__, $notification['id'], $notification['name'], $entry_info ) );
		}
		 ...
		 *
		 */
		do_action( 'kdnaform_after_email', $is_success, $to, $subject, $message, $headers, $attachments, $message_format, $from, $from_name, $bcc, $reply_to, $entry, $cc );
	}
	...
	public static function set_content_type_boundary( $content_type ) {
		...
		if ( $content_type === 'multipart/alternative' ) {
			$boundary     = KDNACommon::$email_boundary;
...
--{$boundary}--";
	...
	public static function add_emails_sent() {
		...
		update_option( 'kdnaform_email_count', ++ $count );

	}

	public static function get_emails_sent() {
		$count = get_option( 'kdnaform_email_count' );
	...
	public static function get_api_calls() {
		$count = get_option( 'kdnaform_api_count' );
	...
	public static function add_api_call() {
		...
		update_option( 'kdnaform_api_count', ++ $count );
	...
	public static function has_repeater_field( $form ) {
		if ( is_array( $form['fields'] ) ) {
			foreach ( $form['fields'] as $field ) {
				if ( $field instanceof KDNA_Field_Repeater ) {
					return true;
				}
			}
		}
	...
	private static function has_field_by_type( $form, $type ) {
		if ( is_array( $form['fields'] ) ) {
			foreach ( $form['fields'] as $field ) {
				...
				if ( KDNAFormsModel::get_input_type( $field ) == $type ) {
					return true;
				}
			}
		}
	 ...
	 * @param string $plugin_path Path of the plugin to be checked, relative to the plugins folder. i.e. "kdnaforms/kdnaforms.php"
	 *
	 * @return bool True if current user can uninstall the plugin. False otherwise
	 */
	public static function current_user_can_uninstall( $caps = 'kdnaforms_uninstall', $plugin_path = KDNA_PLUGIN_BASENAME ) {
	...
	public static function current_user_can_any( $caps ) {
		...
		if ( ! is_array( $caps ) ) {
			$has_cap = current_user_can( $caps ) || current_user_can( 'kdnaform_full_access' );
	...
	public static function is_product_field( $field_type ) {
		 ...
		 */
		$product_fields = apply_filters( 'kdnaform_product_field_types', array(
			'option',
			'quantity',
			'product',
			'total',
	 ...
	 *
	 * @since 2.4.18 Added kdnaforms_logging and kdnaforms_api_settings.
	 * @since 2.2.1.12 Added kdnaforms_system_status.
	 * @since unknown
	 *
	 * @return array
	 */
	public static function all_caps() {
		return array(
			...
			'kdnaforms_delete_forms',
			'kdnaforms_create_form',
			'kdnaforms_view_entries',
			'kdnaforms_edit_entries',
			'kdnaforms_delete_entries',
			'kdnaforms_view_settings',
			'kdnaforms_edit_settings',
			'kdnaforms_export_entries',
			'kdnaforms_uninstall',
			'kdnaforms_view_entry_notes',
			'kdnaforms_edit_entry_notes',
			'kdnaforms_view_updates',
			'kdnaforms_view_addons',
			'kdnaforms_preview_forms',
			'kdnaforms_system_status',
			'kdnaforms_logging',
			'kdnaforms_api_settings',
		);
	}
	...
	public static function get_remote_message() {
		return stripslashes( get_option( 'kdna_forms_message' ) );
	}
	...
	public static function has_update( $use_cache = true ) {
		$version_info = KDNACommon::get_version_info( $use_cache );
		$version      = rgar( $version_info, 'version' );

		return empty( $version ) ? false : version_compare( KDNACommon::$version, $version, '<' );
	}
	 ...
	 *
	 * @since unknown
	 * @since 2.5     Deprecated the kdnaform_version_info option. Get the license and plugins data from their own methods.
	 *
	...
	public static function legacy_get_version_info( $cache = true ) {
		...
		$version_info = get_option( 'kdnaform_version_info' );
		if ( ! $cache ) {
			$version_info = null;
		} else {
		...
		if ( ! $version_info ) {
			...
			// Caching response.
			update_option( 'kdnaform_version_info', $version_info, false ); //caching version info
		}
	...
	public static function get_remote_request_params() {
		...
		return sprintf( 'of=KDNAForms&key=%s&v=%s&wp=%s&php=%s&mysql=%s&version=2', urlencode( self::get_key() ), urlencode( self::$version ), urlencode( get_bloginfo( 'version' ) ), urlencode( phpversion() ), urlencode( KDNACommon::get_db_version() ) );
	}
	...
	public static function get_remote_post_params() {
		...
		$form_counts    = KDNAFormsModel::get_form_count();
		$active_count   = $form_counts['active'];
		$inactive_count = $form_counts['inactive'];
		$fc             = abs( $active_count ) + abs( $inactive_count );
		$entry_count    = KDNAFormsModel::get_entry_count_all_forms( 'active' );
		$meta_counts    = KDNAFormsModel::get_entry_meta_counts();
		$im             = is_multisite();
		$lang           = get_locale();
		...
		$post = array(
			...
			'php'     => phpversion(),
			'mysql'   => KDNACommon::get_db_version(),
			'version' => '2',
			'plugins' => $plugins,
			'tn'      => $theme_name,
			'tu'      => $theme_uri,
	...
	public static function ensure_wp_version() {
		if ( ! KDNA_SUPPORTED_WP_VERSION ) {
			echo "<div class='error' style='padding:10px;'>" . sprintf( esc_html__( 'KDNA Forms requires WordPress %s or greater. You must upgrade WordPress in order to use KDNA Forms', 'kdnaforms' ), esc_html( KDNA_MIN_WP_VERSION ) ) . '</div>';
	...
	public static function check_update( $option, $cache = true ) {
		...
		$plugin = array(
			'url'         => 'https://kdnaforms.com',
			'slug'        => 'kdnaforms',
			'plugin'      => $plugin_path,
			'package'     => str_replace( '{KEY}', KDNACommon::get_key(), $url ),
			'new_version' => $version,
			'id'          => '0',
		);
		// Empty response means that the key is invalid. Do not queue for upgrade.
		if ( ! rgar( $version_info, 'is_valid_key' ) || version_compare( KDNACommon::$version, $version, '>=' ) ) {
	...
	public static function cache_remote_message() {
		...
		$raw_response = self::post_to_manager( 'message.php', KDNACommon::get_remote_request_params(), $options );
		...
		update_option( 'kdna_forms_message', $message );
	}
	...
	public static function post_to_manager( $file, $query, $options ) {
		...
		if ( empty( $manager_url ) ) {
			return new WP_Error( 'no_manager_url', 'KDNA_MANAGER_URL is not defined. License functionality has been removed.' );
		}
		$request_url = $manager_url . '/' . $file . '?' . $query;
	...
	public static function format_date( $gmt_datetime, $is_human = true, $date_format = '', $include_time = true ) {
		...
		if ( $is_human ) {
			...
			if ( $time_diff > 0 && $time_diff < DAY_IN_SECONDS ) {
				return sprintf( esc_html__( '%s ago', 'kdnaforms' ), human_time_diff( $gmt_time ) );
			}
		}
		...
		if ( $include_time ) {
			...
			return sprintf( esc_html__( '%1$s at %2$s', 'kdnaforms' ), date_i18n( $date_format, $local_time, true ), date_i18n( $time_format, $local_time, true ) );
		}
	...
	public static function selection_display( $value, $field, $currency = '', $use_text = false ) {
		...
		if ( $use_text ) {
			$val = KDNAFormsModel::get_choice_text( $field, $val );
		}
	...
	public static function get_field_placeholder_attribute( $field ) {
		...
		$placeholder_value = KDNACommon::replace_variables_prepopulate( $field->placeholder );
	...
	public static function get_input_placeholder_value( $input ) {
		...
		return empty( $placeholder ) ? '' : KDNACommon::replace_variables_prepopulate( $placeholder );
	}

	public static function get_tabindex() {
		return KDNACommon::$tab_index > 0 ? "tabindex='" . KDNACommon::$tab_index ++ . "'" : '';
	}

	/**
	 * @deprecated
	 * @remove-in 3.0
	 * @param KDNA_Field_Checkbox $field
	 * @param                   $value
	 * @param                   $disabled_text
	 *
	 * @return mixed
	...
	public static function get_checkbox_choices( $field, $value, $disabled_text ) {
		_deprecated_function( 'get_checkbox_choices', '1.9', 'KDNA_Field_Checkbox::get_checkbox_choices' );

		return $field->get_checkbox_choices( $value, $disabled_text );
	}

	/**
	 * @deprecated Deprecated since 1.9. Use KDNA_Field_Checkbox::get_radio_choices() instead.
	 * @remove-in 3.0
	 * @param KDNA_Field_Radio $field
	 * @param string         $value
	 * @param                $disabled_text
	 *
	 * @return mixed
	...
	public static function get_radio_choices( $field, $value, $disabled_text ) {
		...
		_deprecated_function( 'get_radio_choices', '1.9', 'KDNA_Field_Checkbox::get_radio_choices' );
				...
				if ( ! isset( $_GET['gf_token'] ) && empty( $_POST ) && self::is_empty_array( $value ) && rgget('view') != 'entry' ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.NonceVerification.Recommended
					...
					if ( is_array( $value ) ) {
						...
						foreach ( $value as $item ) {
							if ( KDNAFormsModel::choice_value_match( $field, $choice, $item ) ) {
								$is_match = true;
								break;
							}
						}
						...
						$selected = KDNAFormsModel::choice_value_match( $field, $choice, $value ) ? "selected='selected'" : '';
					}
				}
	...
	public static function is_section_empty( $section_field, $form, $entry ) {
		...
		$cache_key = "KDNACommon::is_section_empty_{$form['id']}_{$section_field->id}";
		$value     = KDNACache::get( $cache_key, $is_hit, false );
		...
		foreach ( $fields as $field ) {
			...
			$value = KDNAFormsModel::get_lead_field_value( $entry, $field );
			$value = KDNACommon::get_lead_field_display( $field, $value, $entry );
			...
			if ( self::is_product_field( $field->type ) ) {
				 ...
				 *
				 */
				$display_product_summary = apply_filters( 'kdnaform_display_product_summary', true, $field, $form, $entry );

				$is_field_displayed_in_section = ! $display_product_summary;
			}

			if ( $is_field_displayed_in_section ) {
				KDNACache::set( $cache_key, 0 );
	...
	public static function get_us_state_code( $state_name ) {
		return KDNA_Fields::get( 'address' )->get_us_state_code( $state_name );
	}

	public static function get_country_code( $country_name ) {
		return KDNA_Fields::get( 'address' )->get_country_code( $country_name );
	}

	public static function get_us_states() {
		return KDNA_Fields::get( 'address' )->get_us_states();
	}

	public static function get_canadian_provinces() {
		return KDNA_Fields::get( 'address' )->get_canadian_provinces();
	}
	...
	public static function get_fields_by_type( $form, $types ) {
		return KDNAAPI::get_fields_by_type( $form, $types );
	}

	public static function has_pages( $form ) {
		return sizeof( KDNAAPI::get_fields_by_type( $form, array( 'page' ) ) ) > 0;
	}
	...
	public static function form_page_title( $form ) {
		$editable_class = KDNACommon::current_user_can_any( 'kdnaforms_edit_forms' ) ? ' kdnaform_settings_page_title_editable' : '';
	...
	public static function has_field_calculation( $field ) {
		_deprecated_function( 'has_field_calculation', '1.7', 'KDNA_Field::has_calculation' );
	...
	public static function get_field_input( $field, $value = '', $lead_id = 0, $form_id = 0, $form = null ) {
		...
		if ( ! $field instanceof KDNA_Field ) {
			$field = KDNA_Fields::create( $field );
		}
		...
		if ( rgget('view') == 'entry' && ! empty( $lead_id ) ) {
			...
			if ( is_numeric( $post_id ) && self::is_post_field( $field ) ) {
				...
				$post_link = '<div>' . sprintf( __( 'You can <a href="%s">edit this post</a> from the post page.', 'kdnaforms' ), 'post.php?action=edit&post=' . $post_id ) . '</div>';
			}
		}
		...
		$type = KDNAFormsModel::get_input_type( $field );
		switch ( $type ) {
	...
	public static function is_ssl() {
		...
		return apply_filters( 'kdnaform_is_ssl', $is_ssl );
	}
	...
	private static function get_preview_link_args( $args ) {
		$options = wp_parse_args( $args, array(
			'aria-label'   => esc_html__( 'Preview this form', 'kdnaforms' ),
			'capabilities' => array(
				'kdnaforms_edit_forms',
				'kdnaforms_create_form',
				'kdnaforms_preview_forms'
	...
	public static function get_preview_link( $args = array() ) {
		...
		if ( ! KDNACommon::current_user_can_any( $options[ 'capabilities' ] ) ) {
			return '';
		}
		...
		$preview_link = sprintf(
			...
			esc_html__( 'Preview this form', 'kdnaforms' ),
		 ...
		 */
		$preview_link = apply_filters( 'kdnaform_preview_form_link', $preview_link );
	...
	public static function get_disallowed_file_extensions() {
		...
		$extensions = apply_filters( 'kdnaform_disallowed_file_extensions', $extensions );
	...
	public static function check_type_and_ext( $file, $file_name = '' ) {
		...
		if ( $proper_filename && strtolower( $proper_filename ) !== strtolower( $file_name ) ) {
			return new WP_Error( 'invalid_file', esc_html__( 'There was an problem while verifying your file.', 'kdnaforms' ) );
		}

		// If either $ext or $type are empty, WordPress doesn't like this file and we should bail.
		if ( ! $ext ) {
			return new WP_Error( 'illegal_extension', esc_html__( 'Sorry, this file extension is not permitted for security reasons.', 'kdnaforms' ) );
		}
		if ( ! $type ) {
			return new WP_Error( 'illegal_type', esc_html__( 'Sorry, this file type is not permitted for security reasons.', 'kdnaforms' ) );
		}
	 ...
	 * Falls back to KDNACommon::get_currency() if verification fails or posted currency is missing.
	 *
	 * @since 2.9.26
	 *
	...
	public static function get_submission_currency() {
		...
		$decrypted_currency = KDNACommon::openssl_decrypt( $posted_currency );
	 ...
	 *
	 * @param KDNA_Field     $field    The field.
	...
	public static function get_lead_field_display( $field, $value, $entry = array(), $use_text = false, $format = 'html', $media = 'screen' ) {
		...
		if ( ! is_array( $entry ) ) {
			trigger_error( 'Since version 2.9.29 KDNACommon::get_lead_field_display() expects the entry array as the third parameter. Trace: ' . esc_html( wp_debug_backtrace_summary( null, 1 ) ), E_USER_WARNING );
	...
	public static function get_product_fields( $form, $lead, $use_choice_text = false, $use_admin_label = false ) {
		...
		if ( ! rgempty( 'id', $lead ) ) {
			$product_info = kdnaform_get_meta( rgar( $lead, 'id' ), "kdnaform_product_info_{$use_choice_text}_{$use_admin_label}" );
		}
		...
		if ( ! $product_info ) {
			...
			foreach ( $form['fields'] as $field ) {
				$id         = $field->id;
				$lead_value = KDNAFormsModel::get_lead_field_value( $lead, $field );

				$quantity_field = self::get_product_fields_by_type( $form, array( 'quantity' ), $id );
				$quantity       = sizeof( $quantity_field ) > 0 && ! KDNAFormsModel::is_field_hidden( $form, $quantity_field[0], array(), $lead ) ? KDNAFormsModel::get_lead_field_value( $lead, $quantity_field[0] ) : 1;
				...
				switch ( $field->type ) {
					...
					case 'product' :
						...
						$is_hidden = KDNAFormsModel::is_field_hidden( $form, $field, array(), $lead );
						if ( $is_hidden ) {
							break;
						}
						...
						if ( is_array( $lead_value ) ) {
							...
							if ( $field->inputType == 'price' ) {
								...
								if ( $use_choice_text ) {
									$name = KDNAFormsModel::get_choice_text( $field, $name );
								}
								 ...
								 */
								$include_field_label = apply_filters( 'kdnaform_product_info_name_include_field_label', false );
						...
						if ( isset( $products[ $id ] ) ) {
							...
							foreach ( $option_fields as $option_field ) {
								$option_value = KDNAFormsModel::get_lead_field_value( $lead, $option_field );
			...
			if ( ! empty( $shipping_fields ) && ! KDNAFormsModel::is_field_hidden( $form, $shipping_fields[0], array(), $lead ) ) {
				$shipping_price    = KDNAFormsModel::get_lead_field_value( $lead, $shipping_fields[0] );
				...
				if ( $shipping_fields[0]->inputType != 'singleshipping' && ! empty( $shipping_price ) ) {
					...
					if ( $use_choice_text ) {
						$shipping_method = KDNAFormsModel::get_choice_text( $shipping_fields[0], $shipping_method );
					}
			...
			if ( ! rgempty( 'id', $lead ) && ! empty( $product_info['products'] ) ) {
				kdnaform_update_meta( $lead['id'], "kdnaform_product_info_{$use_choice_text}_{$use_admin_label}", $product_info, $form['id'] );
			}
		}
	...
	public static function get_total( $products ) {
		...
		foreach ( $products['products'] as $product ) {
			...
			$quantity = self::to_number( $product['quantity'], KDNACommon::get_submission_currency() );
			if ( $quantity !== 0 ) {
				$has_product = true;
			}
	...
	public static function get_option_info( $value, $option, $use_choice_text ) {
		...
		if ( $use_choice_text ) {
			$name = KDNAFormsModel::get_choice_text( $option, $name );
		}
	 ...
	 * @return string
	 */
	public static function kdnaform_do_shortcode( $content ) {
		require_once self::get_base_path() . '/form_display.php';

		$is_ajax = false;
		$forms   = KDNAFormDisplay::get_embedded_forms( $content, $is_ajax );
		...
		foreach ( $forms as $form ) {
			 ...
			 */
			$disable_print_form_script = apply_filters( 'kdnaform_disable_print_form_scripts', false, $form, $is_ajax );

			if ( headers_sent() && ! $disable_print_form_script ) {
				KDNAFormDisplay::print_form_scripts( $form, $is_ajax );
			} else {
	...
	public static function is_spam_entry( $entry, $form ) {
		...
		$use_cache = class_exists( 'KDNAFormDisplay' );

		if ( $use_cache ) {
			$is_spam = rgars( KDNAFormDisplay::$submission, $form_id . '/is_spam' );
		...
		if ( has_filter( 'kdnaform_entry_is_spam', $akismet_callback ) === false ) {
			add_filter( 'kdnaform_entry_is_spam', $akismet_callback, 90, 3 );
		}

		KDNACommon::log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_entry_is_spam.' );

		/**
		 * Allows submissions to be flagged as spam by custom methods.
		 *
		 * @since 1.8.17
		 * @since 2.4.17 Moved from KDNAFormDisplay::handle_submission().
		 *
		...
		if ( $use_cache ) {
			KDNAFormDisplay::$submission[ $form_id ]['is_spam'] = $is_spam;
		}

		KDNACommon::log_debug( __METHOD__ . sprintf( '(): Spam checks completed in %F seconds. Is submission considered spam? %s.', KDNACommon::timer_end( __METHOD__ ), $is_spam ? 'Yes' : 'No' ) );
	...
	public static function set_spam_filter( $form_id, $filter, $reason ) {
		...
		if ( ! isset( KDNAFormDisplay::$submission[ $form_id ] ) ) {
			KDNAFormDisplay::$submission[ $form_id ] = array();
		}

		// Only save the first spam reason.
		if ( ! isset( KDNAFormDisplay::$submission[ $form_id ]['spam_filter'] ) ) {
			KDNAFormDisplay::$submission[ $form_id ]['spam_filter'] = array( 'filter' => $filter, 'reason' => $reason );
		}
	}


	public static function spam_enabled( $form_id ) {
		$spam_enabled = self::akismet_enabled( $form_id ) || has_filter( 'kdnaform_entry_is_spam' ) || has_filter( "kdnaform_entry_is_spam_{$form_id}" );

		return $spam_enabled;
	}

	/**
	 * Callback for kdnaform_entry_is_spam; performs the Akimset spam check.
	 *
	 * @since 2.9.12 Moved to a filter callback from KDNACommon::is_spam_entry().
	 *
	 * @param bool  $is_spam Indicates if the submission has been flagged as spam.
	 * @param array $form    The form currently being processed.
	 * @param array $entry   The entry currently being processed.
	...
	public static function entry_is_spam_akismet( $is_spam, $form, $entry ) {
		...
		if ( $is_spam ) {
			self::set_spam_filter( $form_id, __( 'Akismet Spam Filter', 'kdnaforms' ), '' );
		}
	...
	private static function get_akismet_field( $field_type, $form, $lead ) {
		$fields = KDNAAPI::get_fields_by_type( $form, array( $field_type ) );
		if ( empty( $fields ) ) {
			return '';
		}

		$value = KDNAFormsModel::get_lead_field_value( $lead, $fields[0] );
		switch ( $field_type ) {
			case 'name' :
				$value = KDNACommon::get_lead_field_display( $fields[0], $value );
				break;
		}
	 ...
	 *
	 * @param null|KDNA_Field_Radio $field Null or the Field currently being prepared for display or being validated.
	 *
	 * @return string
	 */
	public static function get_other_choice_value( $field = null ) {
		$placeholder = esc_html__( 'Other', 'kdnaforms' );
		 ...
		 * @param null|KDNA_Field_Radio $field       Null or the Field currently being prepared for display or being validated.
		 */
		$placeholder = apply_filters( 'kdnaform_other_choice_value', $placeholder, $field );
	...
	public static function create_post( $form, &$lead ) {
		...
		if ( ! $disable_post ) {
			...
			$post_id = KDNAFormsModel::create_post( $form, $lead );
		}
	 ...
	 * NOTE: There is a future refactoring opportunity to reduce code duplication by merging this method with KDNAFormsModel::evaluate_conditional_logic(), which currently handles field conditional logic.
	 *
	...
	public static function evaluate_conditional_logic( $logic, $form, $entry ) {
		 ...
		 * @param array $form       The form currently being processed.
		 */
		$entry_meta_keys = array_keys( apply_filters( 'kdnaform_entry_meta_pre_evaluate_conditional_logic', $entry_meta, $form ) );
		$match_count     = 0;
		...
		foreach ( $rules as $rule ) {
			...
			try {
				 ...
				 * @param array $entry        The current entry object.
				 */
				$rule = apply_filters( 'kdnaform_rule_pre_evaluation', $rule, $form, $logic, array(), $entry );
			} catch ( Error $e ) {
				self::log_error( __METHOD__ . '(): Error from function hooked to kdnaform_rule_pre_evaluation. ' . $e->getMessage() );
			}
			...
			if ( in_array( $rule_field_id, $entry_meta_keys ) ) {
				...
				$source_field = KDNAFormsModel::get_field( $form, $rule_field_id );
				$source_value = empty( $entry ) ? KDNAFormsModel::get_field_value( $source_field, array() ) : KDNAFormsModel::get_lead_field_value( $entry, $source_field );
			}
			 ...
			 */
			$source_value = apply_filters( 'kdnaform_rule_source_value', $source_value, $rule, $form, $logic, $entry );

			$is_value_match = KDNAFormsModel::is_value_match( $source_value, rgar( $rule, 'value' ), rgar( $rule, 'operator' ), $source_field, $rule, $form );
	...
	public static function get_card_types() {
		...
		$cards = apply_filters( 'kdnaform_creditcard_types', $cards );
	...
	public static function is_logging_plugin_active() {
		...
		return is_plugin_active( 'kdnaformslogging/logging.php' ) && file_exists( trailingslashit( WP_PLUGIN_DIR ) . 'kdnaformslogging/logging.php' );
			...
			if ( $display_all ) {
				$selected  = $value == $category->term_id ||
				             (
					             ...
					             KDNAFormsModel::get_input_type( $field ) == 'select' && // only preselect default category on select fields
					             ! $is_post &&
					             ! $has_placeholder
				             );
				$choices[] = array(
	...
	public static function prepare_post_category_value( $value, $field, $mode = 'entry_detail' ) {
		...
		if ( $field instanceof KDNA_Field_MultiSelect ) {
		...
		switch ( $mode ) {
			case 'entry_list':
				...
				break;
			case 'entry_detail':
				$value = KDNAFormsModel::get_input_type( $field ) == 'checkbox' ? $cat_names : self::implode_non_blank( ', ', $cat_names );
				break;
	...
	public static function calculate( $field, $form, $lead ) {
		...
		$formula = (string) apply_filters( 'kdnaform_calculation_formula', $field->calculationFormula, $field, $form, $lead );
		...
		if ( is_array( $matches ) ) {
			foreach ( $matches as $match ) {
				...
				$value   = apply_filters( 'kdnaform_merge_tag_value_pre_calculation', $value, $input_id, rgar( $match, 4 ), $field, $form, $lead );
		...
		if ( preg_match( '/^[0-9 -\/*\(\)]+$/', $formula ) ) {
			...
			try {
				...
				KDNACommon::log_debug( __METHOD__ . sprintf( '(): Formula tried dividing by zero: "%s".', $e->getMessage() ) );
				$result = 0;
			} catch ( ParseError $e ) {
				KDNACommon::log_debug( __METHOD__ . sprintf( '(): Formula could not be parsed: "%s".', $e->getMessage() ) );
				$result = 0;
			} catch ( ErrorException $e ) {
				KDNACommon::log_debug( __METHOD__ . sprintf( '(): Formula caused an exception: "%s".', $e->getMessage() ) );
				$result = 0;
			}
			error_reporting( $prev_reporting_level ); // phpcs:ignore QITStandard.PHP.DebugCode.ErrorReportingModified
		}

		$result = apply_filters( 'kdnaform_calculation_result', $result, $formula, $field, $form, $lead );

		if ( ! $result || ! is_numeric( $result ) || ! is_finite( $result ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): No result or non-numeric result. Returning zero instead.' );
			$result = 0;
		}
	...
	public static function get_calculation_value( $field_id, $form, $lead, $number_format = '', $modifier = '' ) {
		...
		foreach ( $filters as $filter ) {
			...
			$replaced_value = KDNACommon::replace_variables( "{:{$field_id}:$filter}", $form, $lead );
		...
		if ( ! $value || ! is_numeric( $value ) ) {
			KDNACommon::log_debug( "KDNACommon::get_calculation_value(): No value or non-numeric value available for field #{$field_id}. Returning zero instead." );
			$value = 0;
		}
		...
		extract( // nosemgrep audit.php.wp.security.extract.shortcode-attr
			shortcode_atts(
				array(
					...
					'condition' => '',
					'value'     => '',
				), $attributes, 'kdnaforms_conditional'
			)
		);

		$merge_tag = self::maybe_format_numeric( $merge_tag, $condition );

		return KDNAFormsModel::matches_conditional_operation( $merge_tag, $value, $condition ) ? do_shortcode( $content ) : '';
	...
	public static function maybe_format_numeric( $text, $operation, $number_format = '' ) {
		...
		$text = KDNACommon::maybe_add_leading_zero( $text );
		...
		if ( ! $number_format ) {
			...
			$number_format = KDNACommon::is_numeric( $text, 'currency' ) ? 'currency' : 'decimal_dot';
		}

		// If the number is numeric for the specified number_format, return the formatted number.
		if ( KDNACommon::is_numeric( $text, $number_format ) ) {
			return KDNACommon::clean_number( $text, $number_format );
		}

		// If the number is formatted as date or time, return it as is.
		if ( KDNACommon::is_date_time_formatted( $text ) ) {
			return $text;
		}
	...
	public static function log_remote_response( $response ) {
		if ( is_wp_error( $response ) || isset( $_GET['kdnaform_debug'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public static function echo_if( $condition, $text ) {
		_deprecated_function( 'KDNACommon::echo_if() is deprecated', '1.9.9', 'Use checked() or selected() instead.' );
	...
	public static function gf_vars( $echo = true ) {
		 ...
		 */
		$prefixes_string = __( 'Mr., Mrs., Miss, Ms., Mx., Dr., Prof., Rev.', 'kdnaforms' );
		...
		if ( ( is_admin() && rgget( 'id' ) ) || ( self::is_form_editor() && rgpost( 'form_id' ) ) ) {
			$gf_vars['conditionalLogic'] = array(
				'views' => array(
					...
					'logicDescription' => file_get_contents( KDNACommon::get_base_path() . '/js/components/form_editor/conditional_flyout/views/logic_description.html' ),
					'main'             => file_get_contents( KDNACommon::get_base_path() . '/js/components/form_editor/conditional_flyout/views/main_control.html' ),
					'rule'             => file_get_contents( KDNACommon::get_base_path() . '/js/components/form_editor/conditional_flyout/views/rule.html' ),
					'option'           => file_get_contents( KDNACommon::get_base_path() . '/js/components/form_editor/conditional_flyout/views/option.html' ),
					'input'            => file_get_contents( KDNACommon::get_base_path() . '/js/components/form_editor/conditional_flyout/views/input.html' ),
					'select'           => file_get_contents( KDNACommon::get_base_path() . '/js/components/form_editor/conditional_flyout/views/select.html' ),
					'a11yWarning'      => file_get_contents( KDNACommon::get_base_path() . '/js/components/form_editor/conditional_flyout/views/a11y_warning.html' ),
				),
				'conditionalLogicHelperText' => __( 'To use conditional logic, please create a field that supports conditional logic.', 'kdnaforms' ),
				'categories'                 => KDNAForms::get_post_category_options(),
				'addressOptions'             => KDNAForms::get_address_rule_value_options( rgget( 'id' ) ),
				'addRuleText'                => __( 'add another rule', 'kdnaforms' ),
				'removeRuleText'             => __( 'remove this rule', 'kdnaforms' ),
			);
		}
	...
	public static function display_admin_message( $errors = false, $messages = false ) {
		...
		$errors   = apply_filters( 'kdnaform_admin_error_messages', $errors );
		$messages = apply_filters( 'kdnaform_admin_messages', $messages );
	 ...
	 *
	 * @param bool        $messages
	 * @param string|null $page Defaults to current KDNA Forms page from KDNAForms::get_page().
	 *
	 * @since 2.0
	 */
	...
	public static function requires_gf_hooks_javascript() {
		...
		return ! KDNAFormDisplay::$hooks_js_printed;
	}
	...
	public static function get_inline_script_tag( $scripts = '', $cdata = true ) {
		$script_body = $cdata ? sprintf(
			 ...
			 * @since 1.6.3
			 */
			apply_filters( 'kdnaform_cdata_open', '' ),
			$scripts,
			/**
			 ...
			 * @since 1.6.3
			 */
			apply_filters( 'kdnaform_cdata_close', '' )
		) : $scripts;
	...
	public static function admin_screen_reader_title(){
		...
		$page_title = KDNAForms::modify_admin_title( $admin_title, '' );
	...
	public static function check_for_gf_widgets( $sidebar_index ) {
		...
		foreach( $sidebars as $sidebar => $widgets ) {
			...
			foreach( $widgets as $widget ) {
				if ( strpos( $widget, 'kdnaform_widget' ) !== false ) {
					KDNAFormDisplay::$sidebar_has_widget = true;
				}
			}
		}
	}
	 ...
	 *
	 * @param bool $set_printed_prop Optional. Indicates if KDNAFormDisplay::$hooks_js_printed should be set to true.
	 *
	 * @return false|string
	 */
	public static function get_hooks_javascript_code( $set_printed_prop = true ) {
		if ( $set_printed_prop ) {
			require_once self::get_base_path() . '/form_display.php';
			KDNAFormDisplay::$hooks_js_printed = true;
		}
	...
	public static function maybe_add_leading_zero( $value ) {
		$value      = trim( $value );
		$first_char = KDNACommon::safe_substr( $value, 0, 1 );
	...
	public static function get_field_filter_settings( $form ) {
		...
		$field_filters = array(
			array(
				'key'             => '0',
				'text'            => esc_html__( 'Any form field', 'kdnaforms' ),
		...
		foreach ( $form['fields'] as $field ) {
			...
			// Start backwards compatibility. Adds missing settings. Required until the field add-ons implement the KDNA_Field helpers.
		 ...
		 */
		return apply_filters( 'kdnaform_field_filters', $field_filters, $form );
	}
	...
	public static function get_entry_info_filter_columns( $get_users = true ) {
		...
		if ( $get_users ) {
			$args            = apply_filters( 'kdnaform_filters_get_users', array(
				'number' => 200,
		...
		return array(
			'entry_id'       => array(
				'text'      => esc_html__( 'Entry ID', 'kdnaforms' ),
				'operators' => array( 'is', 'isnot', '>', '<' )
			),
			'date_created'   => array(
			...
			'ip'             => array(
				'text'      => esc_html__( 'IP Address', 'kdnaforms' ),
				'operators' => array( 'is', 'isnot', '>', '<', 'contains' ),
			),
			'source_url'     => array(
				'text'      => esc_html__( 'Source URL', 'kdnaforms' ),
				'operators' => array( 'is', 'isnot', '>', '<', 'contains' ),
			),
			'payment_status' => array(
				'text'      => esc_html__( 'Payment Status', 'kdnaforms' ),
				'operators' => array( 'is', 'isnot' ),
			...
			'transaction_id' => array(
				'text'      => esc_html__( 'Transaction ID', 'kdnaforms' ),
				'operators' => array( 'is', 'isnot', '>', '<', 'contains' ),
			),
			'created_by'     => array(
				'text'      => esc_html__( 'User', 'kdnaforms' ),
				'operators' => array( 'is', 'isnot' ),
	...
	public static function get_entry_payment_statuses() {
		$payment_statuses = array(
			'Authorized' => esc_html__( 'Authorized', 'kdnaforms' ),
			'Paid'       => esc_html__( 'Paid', 'kdnaforms' ),
			'Processing' => esc_html__( 'Processing', 'kdnaforms' ),
			'Failed'     => esc_html__( 'Failed', 'kdnaforms' ),
			'Active'     => esc_html__( 'Active', 'kdnaforms' ),
			'Cancelled'  => esc_html__( 'Cancelled', 'kdnaforms' ),
			'Pending'    => esc_html__( 'Pending', 'kdnaforms' ),
			'Refunded'   => esc_html__( 'Refunded', 'kdnaforms' ),
			'Voided'     => esc_html__( 'Voided', 'kdnaforms' ),
		);
		 ...
		 */
		$payment_statuses = apply_filters( 'kdnaform_payment_statuses', $payment_statuses );
	...
	public static function get_entry_meta_filter_settings( $form_id ) {
		$filters    = array();
		$entry_meta = KDNAFormsModel::get_entry_meta( $form_id );
		if ( empty( $entry_meta ) ) {
			return $filters;
		}
	...
	public static function get_field_filters_from_post( $form ) {
		...
		if ( is_array( $filter_fields ) ) {
			...
			for ( $i = 0; $i < count( $filter_fields ); $i ++ ) {
				...
				$field = KDNAFormsModel::get_field( $form, $key );
				if ( $field ) {
					$input_type = KDNAFormsModel::get_input_type( $field );
				 ...
				 * @param array    $form         The form object the filter settings have been prepared for.
				 * @param KDNA_Field $field        The current field being evaluated.
				 */
				$field_filter = apply_filters( 'kdnaform_field_filter_from_post', $field_filter, $form, $field );
	...
	public static function has_multifile_fileupload_field( $form ) {
		$fileupload_fields = KDNAAPI::get_fields_by_type( $form, array( 'fileupload', 'post_custom_field' ) );
	 ...
	 */
	public static function localize_kdnaform_i18n() {
	 ...
	 * @see        class-kdna-config-service-provider.php::register_config_items()
	 */
	public static function  localize_kdnaform_kdnaforms_multifile() {
		return; // as of 2.6, we no longer directly localize our data.
	}
	...
	public static function trim_deep( $array ) {
		...
		return array_map( array( 'KDNACommon', 'trim_deep' ), $array );
	}
	 ...
	 *
	 * mcrypt_encrypt is deprecated in PHP 7.1, use KDNACommon::openssl_encrypt() instead.
	 *
	 * @deprecated 2.3
	 * @remove-in 3.0
	 *
	...
	public static function encrypt( $text, $key = null, $mcrypt_cipher_name = false ) {
		...
		_deprecated_function( 'KDNACommon::encrypt()', '2.3', 'KDNACommon::openssl_encrypt()' );

		$use_mcrypt = apply_filters( 'kdnaform_use_mcrypt', function_exists( 'mcrypt_encrypt' ) );
	 ...
	 *
	 * mcrypt_decrypt is deprecated in PHP 7.1, use KDNACommon::openssl_decrypt() instead.
	 *
	 * @deprecated 2.3
	 * @remove-in 3.0
	 *
	...
	public static function decrypt( $text, $key = null, $mcrypt_cipher_name = false ) {
		...
		_deprecated_function( 'KDNACommon::decrypt()', '2.3', 'KDNACommon::openssl_decrypt()' );

		$use_mcrypt = apply_filters( 'kdnaform_use_mcrypt', function_exists( 'mcrypt_decrypt' ) );
	...
	public static function openssl_encrypt( $text, $encryption_key = null, $cipher_name = 'aes-256-ctr', $mac_key = null ) {
		...
		if ( function_exists( 'openssl_encrypt' ) ) {
			...
			if ( empty( $encryption_key ) ) {
				$encryption_key = 'kdnaforms_encryption_key' . wp_salt( 'nonce' );
			}
			...
			if ( empty( $mac_key ) ) {
				$mac_key = 'kdnaforms_encryption_mac' . wp_salt( 'nonce' );
			}
	...
	public static function is_form_editor() {
		$is_form_editor = KDNAForms::get_page() == 'form_editor' || ( defined( 'DOING_AJAX' ) && DOING_AJAX && in_array( rgpost( 'action' ), array(
					'rg_add_field',
		...
		return apply_filters( 'kdnaform_is_form_editor', $is_form_editor );
	}

	public static function is_entry_detail() {
		$is_entry_detail = KDNAForms::get_page() == 'entry_detail_edit' || KDNAForms::get_page() == 'entry_detail';

		return apply_filters( 'kdnaform_is_entry_detail', $is_entry_detail );
	}

	public static function is_entry_detail_view() {
		$is_entry_detail_view = KDNAForms::get_page() == 'entry_detail';

		return apply_filters( 'kdnaform_is_entry_detail_view', $is_entry_detail_view );
	}

	public static function is_entry_detail_edit() {
		$is_entry_detail_edit = KDNAForms::get_page() == 'entry_detail_edit';

		return apply_filters( 'kdnaform_is_entry_detail_edit', $is_entry_detail_edit );
	}
	...
	public static function get_upload_page_slug() {
		$slug = get_option( 'kdnaform_upload_page_slug' );
		if ( empty( $slug ) ) {
			$slug = substr( str_shuffle( wp_hash( microtime() ) ), 0, 15 );
			update_option( 'kdnaform_upload_page_slug', $slug );
		}
	...
	public static function table_exists( $table_name, $bypass_cache = false ) {
		$found  = false;
		$result = ! $bypass_cache && (bool) KDNACache::get( 'table_exists_' . $table_name, $found, false );
		...
		if ( ! $found ) {
			...
			$result = ! empty( $count );
			KDNACache::set( 'table_exists_' . $table_name, $result );
		}
	 ...
	 *
	 * @param string $domain   The plugin text domain. Default is kdnaforms.
	 * @param string $basename The plugin basename. plugin_basename() will be used to get the KDNA Forms basename when not provided.
	 *
	 * @return string
	 */
	public static function load_gf_text_domain( $domain = 'kdnaforms', $basename = '' ) {
		...
		if ( ! empty( $domain ) && $locale != 'en_US' && ! is_textdomain_loaded( $domain ) ) {
			...
			load_textdomain( $domain, sprintf( '%s/kdnaforms/%s-%s.mo', WP_LANG_DIR, $domain, $locale ) );
	 ...
	 * @return array
	 */
	public static function get_installed_translations( $domain = 'kdnaforms', $return_files = false ) {
	...
	public static function replace_field_variable( $text, $form, $lead, $url_encode, $esc_html, $nl2br, $format, $input_id, $match, $esc_attr = false ) {
		$field = KDNAFormsModel::get_field( $form, $input_id );
		...
		$value = apply_filters( 'kdnaform_merge_tag_filter', $value, $input_id, $modifier, $field, $raw_value, $format );
		if ( $value === false ) {
			$value = '';
		}
	...
	public static function maybe_sanitize_confirmation_message( $confirmation_message ) {
		 ...
		 */
		$sanitize_confirmation_nessage = apply_filters( 'kdnaform_sanitize_confirmation_message', $sanitize_confirmation_nessage );
	...
	/**
	 * Generates a hash for a KDNA Forms download.
	 *
	...
	public static function generate_download_hash( $form_id, $field_id, $file, $entry_id = 0 ) {
		 ...
		 */
		$algo = apply_filters( 'kdnaform_download_hash_algorithm', $algo );

		$hash = hash_hmac( $algo, $key, 'kdnaform_download' . wp_salt() );
		/**
		 ...
		 */
		$hash = apply_filters( 'kdnaform_download_hash', $hash, $form_id, $file );
	...
	public static function get_visibility_options() {
		...
		$options = array(
			array(
				'label'       => __( 'Visible', 'kdnaforms' ),
				'value'       => 'visible',
				'description' => __( 'Default option. The field is visible when viewing the form.', 'kdnaforms' )
			),
			array(
				'label'       => __( 'Hidden', 'kdnaforms' ),
				'value'       => 'hidden',
				'description' => __( 'The field is hidden when viewing the form. Useful when you require the functionality of this field but do not want the user to be able to see this field.', 'kdnaforms' )
			),
			array(
				'label'       => __( 'Administrative', 'kdnaforms' ),
				'value'       => 'administrative',
				'description' => __( 'The field is only visible when administering submitted entries. The field is not visible or functional when viewing the form.', 'kdnaforms' )
			),
		);
		 ...
		 * }
		 */
		return (array) apply_filters( 'kdnaform_visibility_options', $options );
	}
	...
	public static function get_visibility_tooltip() {
		...
		return sprintf( '<strong>%s</strong> %s<br><br>%s', __( 'Visibility', 'kdnaforms' ), __( 'Select the visibility for this field.', 'kdnaforms' ), $markup );
	}
	...
	<body>
		{message}
			 ...
			 */
			$template = apply_filters( 'kdnaform_html_message_template_pre_send_email', $template );
	 ...
	 * @since 2.3
	 *
	 * @param $new_key string Unhashed KDNA Forms license key
	...
	public static function email_domain_matches( $email_address, $domain = '' ) {
		...
		KDNACommon::log_debug( __METHOD__ . '(): Email address: ' . $email_address );

		if ( ! is_email( $email_address ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Email address failed is_email() validation.' );
			return false;
		}
		...
		KDNACommon::log_debug( __METHOD__ . '(): Domain or URL: ' . $domain );

		$email_domain = explode( '@', $email_address );

		$domain_matches = ( strpos( $domain, array_pop( $email_domain ) ) !== false ) ? true : false;
		KDNACommon::log_debug( __METHOD__ . '(): Domain matches? '. var_export( $domain_matches, true ) ); // phpcs:ignore QITStandard.PHP.DebugCode.DebugFunctionFound
    ...
    public static function is_legacy_markup_enabled_og( $form_or_id ) {
	    ...
	    $is_enabled = KDNACache::get( $key, $found );
	...
	public static function is_legacy_markup_enabled( $form_or_id ) {
		...
		return KDNACommon::is_legacy_markup_enabled_og( $form_or_id );
	}
	...
	public static function form_requires_login( $form ) {
		...
		$key           = __METHOD__ . $form_id;
		$require_login = KDNACache::get( $key, $found );
		...
		KDNACache::set( $key, $require_login );
	...
	public static function output_svg( $key ) {
		$svgs = KDNAForms::get_service_container()->get( \KDNA_Forms\KDNA_Forms\Assets\KDNA_Asset_Service_Provider::SVG_OPTIONS );
	...
	public static function record_cron_event( $hook ) {
		...
		KDNACache::set( KDNACache::KEY_CRON_EVENTS, $events, true );
	}
	 ...
	 * @return bool
	 */
	public static function is_network_active( $plugin = KDNA_PLUGIN_BASENAME ) {
		if ( ! is_multisite() ) {
			return false;
		}
	...
	/**
	 * Passes the given form through the kdnaform_admin_pre_render filter.
	 *
	 * @since 2.9.1
	 *
	 ...
	 * @return array|null
	 */
	public static function kdnaform_admin_pre_render( $form ) {
	...
	/**
	 * Applies the kdnaform_disable_css filter and checks 'disable css global' setting to decide if default css should be output or not.
	 *
	 * @since 2.9.1
	 *
	 * @return bool|null
	...
	public static function output_default_css() {
		return (bool) ( ! KDNACommon::is_frontend_default_css_disabled() || KDNACommon::is_form_editor() || KDNACommon::is_entry_detail() );
	}
	...
	public static function log_once_per_request( $message ) {
		...
		KDNACommon::log_debug( "KDNACommon::evaluate_minimum_requirements(): {$message}" );
	...
	public static function evaluate_minimum_requirements( array $minimum_requirements ) {
		...
		$installed = KDNAForms::get_installed_plugins();
		...
		foreach ( $minimum_requirements as $requirements ) {
			...
			if ( ! isset( $installed[ $addon_slug ] ) ) {
				...
				$message = sprintf(
					esc_html__( 'The KDNA Forms %1$s requires %2$s to be installed. Please install %2$s %3$s.', 'kdnaforms' ),
						$parent_title,
						$addon_name,
						$addon_version
				);
			...
			if ( ! $installed[ $addon_slug ]['is_active'] ) {
				// translators: 1: current add-on name, 2: required add-on name, 3: required add-on version..
				$message = sprintf(
					esc_html__( 'The KDNA Forms %1$s requires %2$s to be active. Please activate %2$s %3$s.', 'kdnaforms' ),
				$parent_title,
			...
			if ( version_compare( $installed[ $addon_slug ]['version'], $addon_version, '<' ) ) {
				...
				$message = sprintf(
					esc_html__( 'The KDNA Forms %1$s requires %2$s version %3$s or higher for full compatibility. You are currently using version %4$s. Please update %2$s to %5$s version.', 'kdnaforms' ),
						$parent_title,
...
class KDNACategoryWalker extends Walker {
	/**
...
class KDNACache {
	...
	private static $_transient_prefix = 'KDNACache_';
	...
	public static function flush( $flush_persistent = false ) {
		...
		if ( is_multisite() ) {
			$sql = $wpdb->prepare( "
                 ...
                 meta_key LIKE %s
                ",
				'\_site\_transient\_timeout\_KDNACache\_%',
				'_site_transient_KDNACache_%'
			);
		} else {
			$sql = $wpdb->prepare( "
                 ...
                 option_name LIKE %s
                ",
				'\_transient\_timeout\_KDNACache\_%',
				'_transient_KDNACache_%'
			);
		}
...
class GF_Cache {
	...
	public function set( $key, $data, $is_persistent = false, $expiration_seconds = 0 ) {
		return KDNACache::set( $key, $data, $is_persistent, $expiration_seconds );
	}

	public function delete( $key ) {
		return KDNACache::delete( $key );
	}

	public function flush( $flush_persistent = false ) {
		return KDNACache::flush( $flush_persistent );
	}
...
class GF_Late_Static_Binding {
	...
	/**
	 * Binding for KDNAFormDisplay::footer_init_scripts
	 */
	public function KDNAFormDisplay_footer_init_scripts() {
		return KDNAFormDisplay::footer_init_scripts( $this->args['form_id'] );
	}
}
