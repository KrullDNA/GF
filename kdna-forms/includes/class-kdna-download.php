...
class KDNA_Download {
	...
	/**
	 * If the request is for a KDNA Forms file download then validate and deliver.
	 *
	 * @since 2.0
	 */
	public static function maybe_process() {
		if ( isset( $_GET['gf-download'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			KDNACommon::log_debug( __METHOD__ . "(): Starting file download process. file: {$file}, hash: {$hash}." );

			$permission_granted = self::validate_download( $form_id, $field_id, $file, $hash, $entry_id );

			if ( has_filter( 'kdnaform_permission_granted_pre_download' ) ) {
				KDNACommon::log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_permission_granted_pre_download.' );
			}
			 ...
			 */
			$permission_granted = apply_filters( 'kdnaform_permission_granted_pre_download', $permission_granted, $form_id, $field_id );

			if ( $permission_granted ) {
				KDNACommon::log_debug( __METHOD__ . '(): Download validated. Proceeding.' );
				self::deliver( $form_id, $file, $entry_id );
			} else {
				KDNACommon::log_debug( __METHOD__ . '(): Download validation failed. Aborting with 401.' );
				self::die_401();
			}
		}
	}
	...
	private static function validate_download( $form_id, $field_id, $file, $hash, $entry_id ) {
		...
		if ( has_filter( 'kdnaform_require_login_pre_download' ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_require_login_pre_download.' );
		}
		 ...
		 * @param string $field_id      The ID of the field used to upload the requested file.
		 */
		$require_login = apply_filters( 'kdnaform_require_login_pre_download', false, $form_id, $field_id );

		if ( $require_login && ! is_user_logged_in() ) {
			KDNACommon::log_debug( __METHOD__ . '(): Login required to access file.' );

			return false;
		}

		$hash_check = KDNACommon::generate_download_hash( $form_id, $field_id, $file, $entry_id );
		$valid      = hash_equals( $hash, $hash_check );

		if ( ! $valid ) {
			KDNACommon::log_debug( __METHOD__ . '(): Hash failed validation.' );
	...
	private static function deliver( $form_id, $file, $entry_id ) {
		if ( $entry_id && ! KDNAAPI::entry_exists( $entry_id ) ) {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): Entry ID %d has been deleted. Aborting with 404', $entry_id ) );
			self::die_404();
		}

		$path      = KDNAFormsModel::get_upload_path( $form_id );
		$file_path = trailingslashit( $path ) . $file;

		KDNACommon::log_debug( __METHOD__ . "(): Checking if file exists: {$file_path}." );

		if ( file_exists( $file_path ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): File exists. Starting delivery.' );
			 ...
			 */
			if ( apply_filters( 'kdnaform_enable_download_nocache_headers', true, $form_id, $file ) ) {
				nocache_headers();
			}
			...
			KDNACommon::log_debug( __METHOD__ . '(): File does not exist. Aborting with 404.' );
			self::die_404();
		}
	}
