...
abstract class KDNA_Telemetry_Data {
	...
	public function is_data_collection_allowed() {
		return get_option( 'kdna_forms_dataCollection', false );
	}
	 ...
	 * @since 2.8
	 *
	 * @param KDNA_Telemetry_Data $data The data to save.
	 *
	 * @return void
	 */
	public static function save_data( KDNA_Telemetry_Data $data ) {
	...
	public static function take_snapshot() {
		$snapshot = new KDNA_Telemetry_Snapshot_Data();
	...
	public static function send_data( $entries ) {
		...
		$endpoint = defined( 'KDNA_TELEMETRY_ENDPOINT' ) ? KDNA_TELEMETRY_ENDPOINT : self::TELEMETRY_ENDPOINT;
		$site_url = get_site_url();
		$data = array(
			'license_key_md5' => KDNACommon::get_key(),
			'site_url'        => $site_url,
			'product'         => 'kdnaforms',
			'tag'             => 'system_report',
			'data'            => $entries,
