...
class KDNA_Telemetry_Snapshot_Data extends KDNA_Telemetry_Data {
	...
	public function __construct() {
		...
		// Merges the default callbacks with any additional callbacks added via the kdnaform_telemetry_snapshot_data filter. Default callbacks are added last so they can't be overridden.
	...
	public function get_callbacks() {
		 ...
		 */
		return apply_filters( 'kdnaform_telemetry_snapshot_data', array() );
	}
	...
	public function get_site_basic_info() {
		...
		$im             = is_multisite();
		$lang           = get_locale();
		$db             = KDNACommon::get_dbms_type();

		$post = array(
			'key'                 => KDNACommon::get_key(),
			'wp_version'          => get_bloginfo( 'version' ),
			'php_version'         => phpversion(),
			'mysql_version'       => KDNACommon::get_db_version(),
			'plugins'             => $plugins,
			...
			'emails_sent'         => KDNACommon::get_emails_sent(),
			'api_calls'           => KDNACommon::get_api_calls(),
	...
	/**
	 * Collect the data from the KDNA Forms settings page
	 *
	 * @since 2.8.3
	 *
	 * @return array
	...
	public function get_gf_settings() {
		...
		$kdnaform_settings = array();

		$settings_keys = array(
			'kdnaform_enable_logging',
			'kdna_forms_default_theme',
			'kdnaform_enable_toolbar_menu',
			'kdnaform_enable_noconflict',
		);

		foreach ( $settings_keys as $key ) {
			$kdnaform_settings[ $key ] = get_option( $key );
		}

		return $kdnaform_settings;
	}
	...
	public function get_legacy_forms() {
		...
		foreach ( $forms as $form ) {
			...
			if ( KDNACommon::is_legacy_markup_enabled( $form->id ) ) {
				$legacy_forms++;
			}
		}
	...
	public static function data_sent( $response ) {
		...
		update_option( 'kdnaform_version_info', $decoded, false );

		\KDNACommon::log_debug( __METHOD__ . sprintf( '(): Version info cached.' ) );
	}
}
