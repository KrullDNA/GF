...
if ( ! class_exists( 'KDNA_Query_Call' ) ) {
	require_once( 'class-kdna-query-call.php' );
}

if ( ! class_exists( 'KDNA_Query_Column' ) ) {
	require_once( 'class-kdna-query-column.php' );
}

if ( ! class_exists( 'KDNA_Query_Condition' ) ) {
	require_once( 'class-kdna-query-condition.php' );
}

if ( ! class_exists( 'KDNA_Query_Literal' ) ) {
	require_once( 'class-kdna-query-literal.php' );
}

if ( ! class_exists( 'KDNA_Query_JSON_Literal' ) ) {
	require_once( 'class-kdna-query-json-literal.php' );
}

if ( ! class_exists( 'KDNA_Query_Series' ) ) {
	require_once( 'class-kdna-query-series.php' );
}

/**
 * The KDNA Forms Query Builder class.
 *
 * @internal
 *
 * @since 2.3
...
class KDNA_Query {
	...
	public function parse( $form_id, $search_criteria = array(), $sorting = null, $paging = null ) {
		...
		$order = new KDNA_Query_Column( $sort_field, $form_id );
		...
		if ( ! $order->is_entry_column() ) {
			if ( isset( $sorting['is_numeric'] ) ) {
				...
				$field = KDNAAPI::get_field( $form_id, $sort_field );
		...
		if ( $force_order_numeric ) {
			$order = KDNA_Query_Call::CAST( $order, KDNA_Query::TYPE_DECIMAL );
		}

		$this->from( $from );

		if ( $sort_dir_query == 'RAND' ) {
			$this->order( KDNA_Query_Call::RAND() );
		} else {
		...
		if ( isset( $search_criteria['status'] ) ) {
			...
			if ( is_array( $search_criteria['status'] ) ) {
				...
				foreach ( $search_criteria['status'] as $status ) {
					$statuses[] = new KDNA_Query_Literal( $status );
				}
				$condition_value    = new KDNA_Query_Series( $statuses );
				$condition_operator = KDNA_Query_Condition::IN;
			} else {
				$condition_value    = new KDNA_Query_Literal( $search_criteria['status'] );
				$condition_operator = KDNA_Query_Condition::EQ;
			}

			$property_conditions[] = new KDNA_Query_Condition(
				new KDNA_Query_Column( 'status' ),
				$condition_operator,
				$condition_value
			);
		}
		...
		$column = count( $from ) > 1 ? new KDNA_Query_Column( 'date_created' ) : new KDNA_Query_Column( 'date_created', $form_id );
		...
		if ( ! empty( $start_date ) ) {
			...
			try {
				...
				$property_conditions[] = new KDNA_Query_Condition(
					$column,
					KDNA_Query_Condition::GTE,
					new KDNA_Query_Literal( $start_date_str_utc )
				);
			} catch ( Exception $e ) {
				KDNAAPI::log_error( __METHOD__ . '(): Invalid start_date; ' . $e->getMessage() );
			}
		...
		if ( ! empty( $end_date ) ) {
			...
			try {
				...
				if ( ! empty( $end_date ) ) {
					$property_conditions[] = new KDNA_Query_Condition(
						$column,
						KDNA_Query_Condition::LTE,
						new KDNA_Query_Literal( $end_date_str_utc )
					);
				}
			} catch ( Exception $e ) {
				KDNAAPI::log_error( __METHOD__ . '(): Invalid end_date; ' . $e->getMessage() );
			}
		...
		if ( ! empty( $property_conditions ) ) {
			if ( count( $property_conditions ) > 1 ) {
				$properties_condition = call_user_func_array( array( 'KDNA_Query_Condition', '_and' ), $property_conditions );
			} else {
		...
		if ( ! empty( $search_criteria['field_filters'] ) ) {
			...
			foreach ( $field_filters as $filter ) {
				...
				$operator = isset( $filter['operator'] ) ? $filter['operator'] : KDNA_Query_Condition::EQ;
				$operator = strtoupper( $operator );

				switch ( $operator ) {
					case 'CONTAINS':
						$operator = KDNA_Query_Condition::LIKE;
						$value    = '%' . $value . '%';
						break;
					case 'IS NOT':
					case 'ISNOT':
					case '<>':
						$operator = KDNA_Query_Condition::NEQ;
						break;
					case 'IS':
					case '=':
						$operator = KDNA_Query_Condition::EQ;
						break;
					case 'LIKE':
						$operator = KDNA_Query_Condition::LIKE;
						break;
					case 'NOT IN':
					case 'NOTIN':
						$operator = KDNA_Query_Condition::NIN;
						break;
					case 'IN':
						$operator = KDNA_Query_Condition::IN;
				}
				...
				if ( $operator != KDNA_Query_Condition::LIKE && $is_numeric_filter ) {
					if ( ! is_numeric( $value ) ) {
						$value = floatval( $value );
					}
					$filters[] = new KDNA_Query_Condition(
						KDNA_Query_Call::CAST( new KDNA_Query_Column( $key, $form_id ), self::TYPE_DECIMAL ),
						$operator,
						new KDNA_Query_Literal( $value )
					);
					continue;
				}
				...
				if ( is_array( $value ) ) {
					foreach ( $value as &$v ) {
						$v = $field && $field->storageType == 'json' ? new KDNA_Query_JSON_Literal( (string) $v ) : new KDNA_Query_Literal( $v );
					}
					$value = new KDNA_Query_Series( $value );

					$filters[] = new KDNA_Query_Condition(
						new KDNA_Query_Column( $key, $form_id ),
						$operator,
						$value
					);
				...
				if ( $key == 'date_created' && $operator == KDNA_Query_Condition::EQ ) {
					if ( ! empty( $value ) ) {
						try {
							...
							$column = count( $from ) > 1 ? new KDNA_Query_Column( $key ) : new KDNA_Query_Column( $key, $form_id );

							$filters[] = new KDNA_Query_Condition(
								$column,
								KDNA_Query_Condition::BETWEEN,
								new KDNA_Query_Series( array(
									new KDNA_Query_Literal( $date_create_start_utc ),
									new KDNA_Query_Literal( $date_created_end_utc ),
								) )
							);
						} catch ( Exception $e ) {
							KDNAAPI::log_error( __METHOD__ . '(): Invalid date_created; ' . $e->getMessage() );
						}
					}
				...
				$literal = $field && $field->storageType == 'json' ? new KDNA_Query_JSON_Literal( (string) $value ) : new KDNA_Query_Literal( (string) $value );
			...
			if ( count( $filters ) > 1 ) {
				$filters_condition = call_user_func_array( array( 'KDNA_Query_Condition', $condition_mode ), $filters );
			} elseif ( $filters ) {
		...
		if ( ! empty( $properties_condition ) && ! empty( $filters_condition ) ) {
			$where = call_user_func_array( array( 'KDNA_Query_Condition', '_and' ), array( $properties_condition, $filters_condition ) );
		} else {
	 ...
	 *
	 * @return KDNA_Query_Condition|mixed
	 */
	private function get_global_condition( $filter, $form_ids ) {
		...
		if ( $form_ids == 0 ) {
			...
			$forms[] = KDNAAPI::get_form( $form_ids );
		}
		...
		switch ( $original_operator ) {
			...
			case 'IN':
				...
				break;
			default:
				$operator = empty( $original_operator ) ? KDNA_Query_Condition::EQ : $original_operator;
		}
					...
					if ( is_array( $field->choices ) ) {
						foreach ( $field->choices as $choice ) {
							if ( ( $operator == '=' && strtolower( $choice['text'] ) == strtolower( $val ) ) || ( $operator == 'LIKE' && ! empty( $val ) && strpos( strtolower( $choice['text'] ), strtolower( $val ) ) !== false ) ) {
								...
								$choice_filters[] = new KDNA_Query_Condition(
									new KDNA_Query_Column( $field->id, $form['id'] ),
									$choice_search_operator,
									new KDNA_Query_Literal( $choice_value )
								);
							}
						}
					}
		...
		if ( is_array( $val ) ) {
			foreach ( $val as &$v ) {
				$v = new KDNA_Query_Literal( $v );
			}
			$val = new KDNA_Query_Series( $val );

			$choice_filters[] = new KDNA_Query_Condition(
				new KDNA_Query_Column( KDNA_Query_Column::META, 0 ),
				$operator,
				$val
			);
		} else {
			...
			$choice_filters[] = new KDNA_Query_Condition(
				new KDNA_Query_Column( KDNA_Query_Column::META, $form_ids ),
				$operator,
				new KDNA_Query_Literal( $val )
			);
		}

		if ( count( $choice_filters ) > 1 ) {

			$combine_operator = in_array( $operator, array( KDNA_Query_Condition::NEQ, KDNA_Query_Condition::NIN ) ) ? '_and' : '_or';

			$condition = call_user_func_array( array( 'KDNA_Query_Condition', $combine_operator ), $choice_filters );
		} else {
	 ...
	 *
	 * @return KDNA_Query Chainable $this.
	 */
	 ...
	 *
	 * @param KDNA_Query_Column $on_column The column to join on.
	 * @param KDNA_Query_Column $to_column The column to join the above to.
	 *
	 * @return KDNA_Query Chainable $this.
	 */
	public function join( $on_column, $to_column) {
		if ( $on_column instanceof KDNA_Query_Column && $to_column instanceof KDNA_Query_Column ) {
	 ...
	 *
	 * @param KDNA_Query_Condition $condition A condition.
	 *
	 * @return KDNA_Query Chainable $this.
	 */
	public function where( $condition ) {
		if ( $condition instanceof KDNA_Query_Condition ) {
	 ...
	 * Sets the order.
	 *
	 * @param KDNA_Query_Column|KDNA_Query_Call $column The field, function to order by.
	 * @param string $order The order (one of self::ASC, self::DESC or empty). Default for KDNA_Query_Column: self::ASC Default for KDNA_Query_Call: empty
	 *
	 * @return KDNA_Query Chainable $this.
	 */
	public function order( $column, $order = null ) {
		 ...
		 * This is a function.
		 */
		if ( $column instanceof KDNA_Query_Call ) {
		...
		} elseif ( $column instanceof KDNA_Query_Column ) {
	...
	private function query() {
		if ( count( $this->queries ) ) {
			KDNACommon::log_debug( 'Reusing KDNA_Query is undefined behavior. Create a new instance instead.' );
		}
		 ...
		 */
		$from = sprintf( 'FROM `%s` AS `%s`', KDNAFormsModel::get_entry_table_name(), $this->_alias( null, reset( $this->from ) ) );
		...
		if ( $form_ids ) {
			$this->where( KDNA_Query_Condition::_and(
			/**
			 * Prepend the selected form IDs.
			 */
				new KDNA_Query_Condition(
					new KDNA_Query_Column( 'form_id' ),
					KDNA_Query_Condition::IN,
					new KDNA_Query_Series( $form_ids )
				),
	...
	public function _order_generate( $orders ) {
		...
		foreach ( $orders as $o ) {
			...
			} else if ( $column instanceof KDNA_Query_Column ) {
	...
	private function find_joins( $condition, &$already_joined ) {
		 ...
		 */
		if ( in_array( $condition->operator, array( KDNA_Query_Condition::_AND, KDNA_Query_Condition::_OR ) ) ) {
	...
	public function _select_infer( $joins ) {
		...
		foreach ( $joins as $join ) {
			list( $on, $column ) = $join;
			if ( ! $on instanceof KDNA_Query_Column || ! $column instanceof KDNA_Query_Column ) {
				continue;
			}
	...
	public function _join_generate( $joins ) {
		...
		foreach ( $joins as $join ) {
			...
			if ( $on->is_entry_column() ) {
				$table_on = KDNAFormsModel::get_entry_table_name();
				...
				$table_on = KDNAFormsModel::get_entry_meta_table_name();
			...
			if ( $column->is_entry_column() ) {
				 ...
				 * Make sure a WHERE clause exists on meta fields.
				 */
				$meta_condition = new KDNA_Query_Condition(
					new KDNA_Query_Column( 'meta_key', $column->source, $equals_table ),
					KDNA_Query_Condition::EQ,
					new KDNA_Query_Literal( $column->field_id )
				);

				$this->where(
					KDNA_Query_Condition::_and( $this->where, $meta_condition )
				);
	...
	public function _join_infer( $condition ) {
		...
		foreach ( $condition->columns as $column ) {
			...
			if ( $column->is_entry_column() ) {
				...
				if ( ! $column->alias && ( $this->_alias( null, $column->source ) !== 't1' ) ) {
					$this->_inferred_joins []= sprintf( '`%s` AS `%d` ON `%%s`.`id` = `%%s`.`entry_id`', // the rest is filled in during _priming
						KDNAFormsModel::get_entry_table_name(), $column->source
					);
				}
				continue;
			}
			...
			if ( ! $column->is_meta_column() && $column->field_id != KDNA_Query_Column::META ) {
				if ( $column->field_id == intval( $column->field_id ) && ( $field = KDNAFormsModel::get_field( KDNAAPI::get_form( $column->source ? $column->source : reset( $this->from ) ), $column->field_id ) ) && $field->get_entry_inputs() ) {
					/**
					 * Multi-input across all inputs.
					 */
					$literal = new KDNA_Query_Literal( sprintf( '%d.%%', $column->field_id ) );
					$this->_inferred_joins []= sprintf( '`%s` AS `%s` ON (`%s`.`entry_id` = `%s`.`id` AND `%s`.`meta_key` LIKE %s)',
						KDNAFormsModel::get_entry_meta_table_name(), $alias, $alias, $this->_alias( null, $column->source ), $alias, $literal->sql( $this )
					);
				} else {
					$literal = new KDNA_Query_Literal( $column->field_id );
				...
				$this->_inferred_joins []= sprintf( '`%s` AS `%s` ON `%s`.`entry_id` = `%s`.`id`',
					KDNAFormsModel::get_entry_meta_table_name(), $alias, $alias, $this->_alias( null, $column->source )
				);
			}
		}
		...
		foreach ( $order as $o ) {
			...
			foreach ( $columns as $column ) {
				...
				if ( ! in_array( "{$source_id}_{$field_id}", $already_joined ) ) {
					...
					$literal = new KDNA_Query_Literal( $field_id );
	 ...
	 *
	 * @param KDNA_Query_Condition $condition Either a condition or an array of conditions. Usually from self::$where.
	 *
	 * @internal
	 *
	 * @return string.
	...
	public function get_entries( $entry_ids ) {
		...
		foreach ( $entryset as $entry ) {
			...
			if ( isset( $cache['form_entry_meta'][ $form_id ] ) ) {
				...
				$entry_meta = $cache['form_entry_meta'][ $form_id ] = KDNAFormsModel::get_entry_meta( $form_id );
			}
			if ( ! empty( $entry_meta ) ) {
...
", $placeholders, $meta_clauses_str );
		...
		$entries_with_encrypted_fields = KDNAFormsModel::get_openssl_encrypted_fields_by_entries( $ids );

		foreach ( $entryset as $entry ) {
			if ( isset( $cache['form_meta'][ $entry['form_id'] ] ) ) {
				$form = $cache['form_meta'][ $entry['form_id'] ];
			} else {
				$form = $cache['form_meta'][ $entry['form_id'] ] = KDNAFormsModel::get_form_meta( $entry['form_id'] );
			}
			...
			KDNAFormsModel::hydrate_repeaters( $entries[ $entry['id'] ], $form, true );
		}
