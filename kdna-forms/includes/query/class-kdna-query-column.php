...
/**
 * The KDNA Forms Query Column class.
 */
class KDNA_Query_Column {
	 ...
	 *
	 * @param $query KDNA_Query The query.
	 *
	 * @return string The SQL.
	 */
	public function sql( $query ) {
		if ( ! $query instanceof KDNA_Query ) {
			return '';
		}
	...
	public function is_entry_column() {
		...
		if ( empty( $entry_columns ) ) {
			global $wpdb;
			$entry_columns = wp_list_pluck( $wpdb->get_results( $wpdb->prepare( 'SHOW COLUMNS FROM %i', KDNAFormsModel::get_entry_table_name() ), ARRAY_A ), 'Field' ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		}
	...
	public function is_nullable_entry_column() {
		...
		if ( empty( $nullable_entry_columns ) ) {
			global $wpdb;
			$nullable_entry_columns = wp_list_pluck( $wpdb->get_results( $wpdb->prepare( "SHOW COLUMNS FROM %i WHERE `Null` = 'YES'", KDNAFormsModel::get_entry_table_name() ), ARRAY_A ), 'Field' ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		}
