...
/**
 * The KDNA Forms Query Call class.
 */
class KDNA_Query_Call {
	/**
	 ...
	 * A cast call.
	 *
	 * @param KDNA_Query_Column $column The column to cast.
	 * @param string $type The type to cast to.
	 *
	 * @return KDNA_Query_Call|null instance or null.
	 */
	public static function CAST( $column, $type ) {
		if ( ! $column instanceof KDNA_Query_Column ) {
			return null;
		}
	...
	private function cast_sql( $query ) {
		...
		if ( ! in_array( $type, array( KDNA_Query::TYPE_SIGNED, KDNA_Query::TYPE_UNSIGNED, KDNA_Query::TYPE_DECIMAL ) ) ) {
			return '';
		}
		...
		if ( KDNA_Query::TYPE_DECIMAL === $type ) {
	...
	private function get_columns() {
		...
		foreach ( $this->parameters as $p ) {
			if ( $p instanceof KDNA_Query_Column ) {
				$columns[] = $p;
			}
		}
		return $columns;
