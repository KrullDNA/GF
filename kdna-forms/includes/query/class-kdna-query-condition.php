...
/**
 * The KDNA Forms Query Condition class.
 */
class KDNA_Query_Condition {
	/**
	 ...
	 * A condition.
	 *
	 * @param KDNA_Query_Column|KDNA_Query_Call|KDNA_Query_Literal|null $left The left-hand expression.
	 * @param string|null $operator The operator.
	 * @param KDNA_Query_Column|KDNA_Query_Call|KDNA_Query_Literal|KDNA_Query_Series|null $right The right-hand expression.
	 *
	 * @return KDNA_Query_Condition $this This condition.
	 */
	public function __construct( $left = null, $operator = null, $right = null ) {
		 ...
		 */
		if ( self::is_valid_expression_type( $left ) && ! $left instanceof KDNA_Query_Series ) {
			$this->_left = $left;
		}
	 ...
	 *
	 * Accepts any number of KDNA_Query_Condition objects.
	 *
	 * @return KDNA_Query_Condition The condition.
	 */
	public static function _and() {
		$conditions = array();

		foreach ( func_get_args() as $arg ) {
			if ( $arg instanceof KDNA_Query_Condition ) {
				$conditions[] = $arg;
			}
		}
	...
	public function sql( $query ) {
		...
		if ( $this->operator && $this->left && $this->right ) {
			 ...
			 * Meta field.
			 */
			if ( $this->left instanceof KDNA_Query_Column && ( ! $this->left->is_entry_column() && ! $this->left->is_meta_column() )  && $this->left->field_id ) {
				/**
				...
				if ( $this->left->field_id == KDNA_Query_Column::META ) {

					// Global meta search doesn't require a meta_key clause.
					$compare_condition = new self(
						new KDNA_Query_Column( 'meta_value', $this->left->source, $alias ),
						$this->operator,
						$this->right
					);
				...
				if ( is_numeric( $this->left->field_id ) && intval( $this->left->field_id ) == $this->left->field_id ) {
					if ( ( $field = KDNAFormsModel::get_field( KDNAAPI::get_form( $this->left->source ), $this->left->field_id ) ) && $field->get_entry_inputs() ) {

						/**
						 * EQ and NEQ require an unordered comparison of all the values for the entry.
						 */
						if ( in_array( $this->operator, array( self::EQ, self::NEQ ) ) && $this->right instanceof KDNA_Query_Series ) {
							...
							foreach ( $this->right->values as $literal ) {
								$subquery = $wpdb->prepare( sprintf( "SELECT 1 FROM `%s` WHERE `meta_key` LIKE %%s AND `meta_value` = %s AND `entry_id` = `%s`.`id`", // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
									KDNAFormsModel::get_entry_meta_table_name(), $literal->sql( $query ), $query->_alias( null, $this->left->source ) ), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
									sprintf( '%d.%%', $this->left->field_id ) );
								$compare_condition = new self( new KDNA_Query_Call( sprintf( '%sEXISTS', $this->operator == self::NEQ ? 'NOT ' : '' ), array( $subquery ) ) );
							...
							$subquery = $wpdb->prepare( sprintf( "SELECT COUNT(1) FROM `%s` WHERE `meta_key` LIKE %%s AND `entry_id` = `%s`.`id`", // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
								KDNAFormsModel::get_entry_meta_table_name(), $query->_alias( null, $this->left->source ) ), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
								sprintf( '%d.%%', $this->left->field_id ) );
							...
							$subquery = $wpdb->prepare( sprintf( "SELECT 1 FROM `%s` WHERE `meta_key` LIKE %%s AND `meta_value` = %s AND `entry_id` = `%s`.`id`", // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
								KDNAFormsModel::get_entry_meta_table_name(), $this->right->sql( $query ), $query->_alias( null, $this->left->source ) ), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
								sprintf( '%d.%%', $this->left->field_id ) );
							$compare_condition = new self( new KDNA_Query_Call( sprintf( '%sEXISTS', $this->operator == self::NCONTAINS ? 'NOT ' : '' ), array( $subquery ) ) );
							return $compare_condition->sql( $query );

							/**
							 * One of.
							 */
						} elseif ( in_array( $this->operator, array( self::IN, self::NIN ) ) && $this->right instanceof KDNA_Query_Series ) {
							$subquery = $wpdb->prepare( sprintf( "SELECT 1 FROM `%s` WHERE `meta_key` LIKE %%s AND `meta_value` IN (%s) AND `entry_id` = `%s`.`id`", // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
								KDNAFormsModel::get_entry_meta_table_name(), str_replace( '%', '%%', $this->right->sql( $query, ', ' ) ), $query->_alias( null, $this->left->source ) ), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
								sprintf( '%d.%%', $this->left->field_id ) );
							$compare_condition = new self( new KDNA_Query_Call( sprintf( '%sEXISTS', $this->operator == self::NIN ? 'NOT ' : '' ), array( $subquery ) ) );
							return $compare_condition->sql( $query );
							...
							$subquery = $wpdb->prepare( sprintf( "SELECT 1 FROM `%s` WHERE `meta_key` LIKE %%s AND `meta_value` %s %s AND `entry_id` = `%s`.`id`", // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
								KDNAFormsModel::get_entry_meta_table_name(), $operator, str_replace( '%', '%%', $this->right->sql( $query ) ), $query->_alias( null, $this->left->source ) ), // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
								sprintf( '%d.%%', $this->left->field_id ) );
							$compare_condition = new self( new KDNA_Query_Call( sprintf( '%sEXISTS', $is_negative ? 'NOT ' : '' ), array( $subquery ) ) );
							return $compare_condition->sql( $query );
						}
					}
				}
				...
				$compare_condition = self::_and(
					new self(
						new KDNA_Query_Column( 'meta_key', $this->left->source, $alias ),
						self::EQ,
						new KDNA_Query_Literal( $this->left->field_id )
					),
					new self(
						new KDNA_Query_Column( 'meta_value', $this->left->source, $alias ),
						$this->operator,
				...
				if ( ( in_array( $this->operator, array( self::NIN, self::NBETWEEN ) ) && ! in_array( new KDNA_Query_Literal(''), $this->right->values ) )
					...
					$subquery = $wpdb->prepare( sprintf( "SELECT 1 FROM `%s` WHERE `meta_key` = %%s AND `entry_id` = `%s`.`id`", // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
						KDNAFormsModel::get_entry_meta_table_name(), $query->_alias( null, $this->left->source ) ), $this->left->field_id ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
					$not_exists = new self( new KDNA_Query_Call( 'NOT EXISTS', array( $subquery ) ) );
			...
			if ( ( $left = $this->left_sql( $query ) ) && ( $right = $this->right_sql( $query ) ) ) {
				...
				if ( $this->left instanceof KDNA_Query_Column && $this->left->is_nullable_entry_column() ) {
				...
				if ( $this->left instanceof KDNA_Query_Column && $this->left->is_entry_column() && $this->left->source ) {
		...
		if ( $this->left && ( $this->left instanceof KDNA_Query_Call || $this->left instanceof KDNA_Query_Column ) ) {
	...
	public static function is_valid_expression_type( $expression ) {
		return (
			( $expression instanceof KDNA_Query_Literal ) ||
			( $expression instanceof KDNA_Query_Column ) ||
			( $expression instanceof KDNA_Query_Series ) ||
			( $expression instanceof KDNA_Query_Call ) ||
	...
	private function right_sql( $query ) {
		...
		if ( in_array( $this->operator, array( self::IN, self::NIN ) ) ) {
			if ( ! $this->right instanceof KDNA_Query_Series ) {
				return '';
			}
	...
	private function left_sql( $query ) {
		if ( $this->left instanceof KDNA_Query_Call ) {

			$columns = array();

			foreach ( $this->left->parameters as $c ) {
				if ( $c instanceof KDNA_Query_Column ) {
					$columns[] = $c;
				}
			}
			...
			if ( $columns ) {
				...
				foreach ( $columns as $column ) {
					...
					$condition = new KDNA_Query_Condition(
						new KDNA_Query_Column( 'meta_key', $column->source, $alias ),
	...
	public function get_columns() {
		...
		if ( $this->left instanceof KDNA_Query_Column ) {
			$columns[] = $this->left;
		}

		if ( $this->right instanceof KDNA_Query_Column ) {
			$columns[] = $this->right;
		}
