...
class KDNA_Field_Email extends KDNA_Field {

	public $type = 'email';

	public function get_form_editor_field_title() {
		return esc_attr__( 'Email', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_description() {
		return esc_attr__( 'Allows users to enter a valid email address.', 'kdnaforms' );
	}
	...
	public function get_field_container_tag( $form ) {
		...
		if ( KDNACommon::is_legacy_markup_enabled( $form ) || ! $this->emailConfirmEnabled ) {
	...
	public function validate( $value, $form ) {
		...
		if ( $not_blank && ! KDNACommon::is_valid_email( $email ) ) {
			$this->failed_validation  = true;
			$this->validation_message = $this->errorMessage ?: esc_html__( 'The email address entered is invalid, please check the formatting (e.g. email@domain.com).', 'kdnaforms' );
		} elseif ( $not_blank && $this->is_email_rejected( $email ) ) {
			$this->set_context_property( 'is_value_spam', true );
			$this->failed_validation  = true;
			$this->validation_message = $this->errorMessage ?: esc_html__( 'The email address entered is invalid.', 'kdnaforms' );
		} elseif ( $this->emailConfirmEnabled ) {
			$confirm = is_array( $value ) ? rgar( $value, 1 ) : $this->get_input_value_submission( 'input_' . $this->id . '_2' );
			if ( $confirm !== $email ) {
				$this->failed_validation  = true;
				$this->validation_message = esc_html__( 'Your emails do not match.', 'kdnaforms' );
			}
		}
	}
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		$enter_email_field_input = KDNAFormsModel::get_input( $this, $this->id . '' );
		...
		if ( $is_form_editor ) {
			...
			if ( $is_sub_label_above ) {
				return "<div class='kdnainput_container kdnainput_container_email kdnainput_single_email' {$single_style}>
                        ...
                        <div class='kdnainput_complex kdnainput_container kdnainput_container_email kdnainput_confirm_email kdnaform-grid-row' {$confirm_style} id='{$field_id}_container'>
                            <span id='{$field_id}_1_container' class='kdnainput_left kdnaform-grid-col kdnaform-grid-col--size-auto'>
                                <label for='{$field_id}' class='kdnaform-field-label kdnaform-field-label--type-sub {$sub_label_class}'>{$enter_email_label}</label>
                            ...
                            <span id='{$field_id}_2_container' class='kdnainput_right kdnaform-grid-col kdnaform-grid-col--size-auto'>
                                <label for='{$field_id}_2' class='kdnaform-field-label kdnaform-field-label--type-sub {$sub_label_class}'>{$confirm_email_label}</label>
			...
			if ( $this->emailConfirmEnabled && ! $is_entry_detail ) {
				...
				if ( $is_sub_label_above ) {
					return "<div class='kdnainput_complex kdnainput_container kdnainput_container_email kdnaform-grid-row' id='{$field_id}_container'>
                                <span id='{$field_id}_1_container' class='kdnainput_left kdnaform-grid-col kdnaform-grid-col--size-auto'>
                                    <label for='{$field_id}' class='kdnaform-field-label kdnaform-field-label--type-sub {$sub_label_class}'>" . $enter_email_label . "</label>
				...
				return "<div class='kdnainput_container kdnainput_container_email'>
	...
	public function get_value_entry_detail( $value, $entry = array(), $use_text = false, $format = 'html', $media = 'screen' ) {
		if ( KDNACommon::is_valid_email( $value ) && $format == 'html'  ) {
...
KDNA_Fields::register( new KDNA_Field_Email() );
