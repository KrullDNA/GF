...
class KDNA_Field_CAPTCHA extends KDNA_Field {
	/**
	 * @var string
	 */
	public $type = 'captcha';
	...
	public function __construct( $data = array() ) {
		add_filter( 'kdnaform_ajax_submission_result', array( 'KDNA_Field_CAPTCHA', 'set_recaptcha_response' ) );
		add_filter( 'kdnaform_ajax_validation_result', array( 'KDNA_Field_CAPTCHA', 'set_recaptcha_response' ) );

		parent::__construct( $data );

		if ( ! has_filter( 'kdnaform_pre_render', array( __CLASS__, 'maybe_remove_recaptcha_v2' ) ) ) {
			add_filter( 'kdnaform_pre_render', array( __CLASS__, 'maybe_remove_recaptcha_v2' ), 11 );
		}
	}
    ...
    public static function set_recaptcha_response( $result ) {
        ...
        $recaptcha_field = \KDNAFormsModel::get_fields_by_type( $form, array( 'captcha' ) );
    ...
    public function get_form_editor_field_title() {
		return esc_attr__( 'CAPTCHA', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_description() {
		return esc_attr__( 'Adds a captcha field to your form to help protect your website from spam and bot abuse.', 'kdnaforms' );
	}
	...
	public function get_field_sidebar_messages() {
		...
		if ( ( ! empty( $this->get_site_key() ) && ! empty( $this->get_secret_key() ) ) ) {
			if ( is_plugin_active( 'kdnaformsconversationalforms/conversationalforms.php' ) ) {
				return array(
					'type'             => 'notice',
					'content'          => sprintf(
						'<div class="kform-typography--weight-regular">%s</div>',
		...
		return array(
			'type'             => 'notice',
			'content'          => sprintf(
                    '%s<div class="kform-spacing kform-spacing--top-1">%s</div>',
			...
			case 'simple_captcha' :
				if ( class_exists( 'ReallySimpleCaptcha' ) ) {
					...
					if ( ! $captcha_obj->check( $prefix, str_replace( ' ', '', $value ) ) ) {
						$this->set_failed_validation( esc_html__( "The CAPTCHA wasn't entered correctly. Go back and try it again.", 'kdnaforms' ) );
					}
	 ...
	 *
	 * @see KDNA_Field_CAPTCHA::verify_decoded_response()
	 *
	 * @since unknown
	 *
	 * @param array $form The form data.
	...
	public function validate_recaptcha( $form ) {
		if ( rgpost( 'kdnaform_conversational_form' ) ) {
			$hash = md5( $form['title'] . $form['id'] );
			if ( $hash === rgpost( 'kdnaform_conversational_form' ) && is_plugin_active( 'kdnaformsconversationalforms/conversationalforms.php' ) ) {
		...
		if ( ! ( $this->verify_decoded_response( $form, $response ) || $this->verify_recaptcha_response( $response ) ) ) {
			$this->set_failed_validation( __( 'The reCAPTCHA was invalid. Go back and try it again.', 'kdnaforms' ) );
			return false;
		}
	 ...
	 *
	 * @see KDNA_Field_CAPTCHA::get_encoded_recaptcha_response()
	 *
	 * @since 2.4.24
	 *
	 * @return string
	...
	public function verify_recaptcha_response( $response, $secret_key = null ) {
		...
		if ( ! is_wp_error( $response ) ) {
			...
			KDNACommon::log_debug( __METHOD__ . '(): Validating the reCAPTCHA response has failed due to the following: ' . $response->get_error_message() );
		}
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		switch ( $this->captchaType ) {
			case 'simple_captcha' :
				...
				return "<div class='kfield_captcha_container'><img class='kfield_captcha' src='" . esc_url( rgar( $captcha, 'url' ) ) . "' alt='' {$dimensions} /><div class='kfield_captcha_input_container simple_captcha_{$size}'><input type='text' autocomplete='off' name='input_{$id}' id='{$field_id}' {$tabindex}/><input type='hidden' name='input_captcha_prefix_{$id}' value='" . esc_attr( rgar( $captcha, 'prefix' ) ) . "' /></div></div>";
				break;
			...
			case 'math' :
				...
				return "<div class='kfield_captcha_container'><img class='kfield_captcha' src='" . esc_url( rgar( $captcha_1, 'url' ) ) . "' alt='' {$dimensions} /><img class='kfield_captcha' src='" . esc_url( rgar( $captcha_2, 'url' ) ) . "' alt='' {$dimensions} /><img class='kfield_captcha' src='" . esc_url( rgar( $captcha_3, 'url' ) ) . "' alt='' {$dimensions} /><div class='kfield_captcha_input_container math_{$size}'><input type='text' autocomplete='off' name='input_{$id}' id='{$field_id}' {$tabindex}/><input type='hidden' name='input_captcha_prefix_{$id}' value='" . esc_attr( $prefix_value ) . "' /></div></div>";
				break;
			...
			default:
				...
				if ( $is_entry_detail || $is_form_editor ){
					...
					if ( empty( $this->site_key ) || empty( $this->secret_key ) ) {
						return '<div class="kinput_container kinput_container_addon_message kinput_container_addon_message_captcha">
							<div class="kform-alert kform-alert--info kform-alert--theme-cosmos kform-spacing kform-spacing--bottom-0 kform-theme__disable">
								<span
									class="kform-icon kform-icon--information-simple kform-icon--preset-active kform-icon-preset--status-info kform-alert__icon"
									aria-hidden="true"
								></span>
								<div class="kform-alert__message-wrap">
									<div class="kform-alert__message">
										'. __( 'Configuration Required', 'kdnaforms' ) .'
										<div class="kform-spacing kform-spacing--top-1">'. sprintf(
									'%s %s%s.%s',
					...
					return "<div class='kinput_container'><img class='kfield_captcha' src='" . KDNACommon::get_base_url() . "/images/captcha_{$type_suffix}{$theme}.svg' alt='{$alt}' /></div>";
				}
				...
				$output = "<div id='" . esc_attr( $field_id ) ."' class='kinput_container kinput_recaptcha' data-sitekey='" . esc_attr( $this->site_key ) . "' {$stoken} data-theme='" . esc_attr( $theme ) . "' data-tabindex='{$tabindex}' {$size} data-badge='{$badge}'></div>";
	...
	public function get_encoded_recaptcha_response( $form, $response ) {
		...
		return KDNACommon::openssl_encrypt( base64_encode( json_encode( $this->response ) ), $this->secret_key );
	}
	...
	private function get_decoded_recaptcha_response( $response ) {
		$decoded_response = KDNACommon::openssl_decrypt( $response, $this->get_secret_key() );
	...
	private function requires_encoding( $form, $recaptcha_response ) {
		return $recaptcha_response && ! $this->failed_validation && KDNAFormDisplay::get_current_page( rgar( $form, 'id' ) ) != $this->pageNumber && ! $this->is_on_last_page( $form );
	}
	...
	private function is_on_last_page( $form ) {
	    $pages = KDNAAPI::get_fields_by_type( $form, array( 'page' ) );
	...
	public function create_recaptcha_secure_token( $secret_key ) {
		...
		KDNACommon::log_debug( 'recaptcha token parameters: ' . $plaintext );
		...
		//encrypt as 128
		$encrypted = KDNACommon::openssl_encrypt( $padded, $secret_key, MCRYPT_RIJNDAEL_128 ); // gitleaks:allow

		$token = str_replace( array( '+', '/', '=' ), array( '-', '_', '' ), $encrypted );
		KDNACommon::log_debug( ' token being used is: ' . $token );

		return $token;
	}

	public function use_stoken() {
		// 'kdnaform_recaptcha_keys_status' will be set to true if new keys have been entered
		return ! get_option( 'kdnaform_recaptcha_keys_status', false );
	}

	/**
	 * Returns the kdna-download URL for the given image filename.
	 *
	 * @since 2.9.21
	 *
	 * @param string $filename The filename.
	 *
	 * @return string The kdna-download URL.
	 */
	private function get_image_url( $filename ) {
		return add_query_arg(
			array(
				'kdna-download' => urlencode( $filename ),
				'form-id'     => 'captcha',
				'field-id'    => $this->id,
				'hash'        => KDNACommon::generate_download_hash( 'captcha', $this->id, $filename ),
			),
			site_url( 'index.php', KDNACommon::is_ssl() ? 'https' : 'http' )
		);
	}

}

KDNA_Fields::register( new KDNA_Field_CAPTCHA() );
