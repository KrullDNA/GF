...
require_once( plugin_dir_path( __FILE__ ) . 'class-kdna-field-multiple-choice.php' );

class KDNA_Field_Image_Choice extends KDNA_Field_Multiple_Choice {
	...
	public function __construct( $data = array() ) {
		if ( ! has_action( 'kdnaform_after_save_form', array( __class__, 'resize_images' ) ) ) {
			add_action( 'kdnaform_after_save_form', array( __class__, 'resize_images' ), 10, 2 );
		}

		if ( ! has_action( 'kdnaform_forms_post_import', array( __class__, 'resize_images_after_import' ) ) ) {
			add_action( 'kdnaform_forms_post_import', array( __class__, 'resize_images_after_import' ), 10, 2 );
		}
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'Image Choice', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_description() {
		return esc_attr__( 'Allow users to choose from a list of images.', 'kdnaforms' );
	}
	...
	public static function resize_images( $form, $is_new_form ) {
		...
		remove_filter( 'intermediate_image_sizes_advanced', array( 'KDNAForms', 'remove_image_sizes' ) );

		$image_sizes = KDNAForms::get_image_sizes();
				...
				foreach ( $field->choices as $choice ) {
					if ( rgar( $choice, 'attachment_id' ) ) {
						...
						if ( is_array( $image ) && $image[3] === false ) {
							KDNACommon::log_debug( __METHOD__ . '(): Resizing image attachment id ' . $choice['attachment_id'] . ' for the image choice field in form ' . $form['id'] );
	...
	public function get_form_editor_inline_script_on_page_render() {
		...
		$js .= 'jQuery( document ).on( "kdnaform_load_field_settings", function( event, field, form ) {
			if ( field.type !== "image_choice" ) {
				return;
			}
	...
	public function get_merge_tag_img_url( $value, $input_id, $entry, $form, $field ) {
		...
		foreach ( $field->choices as $choice ) {
			if ( is_array( $value ) && KDNAFormsModel::choice_value_match( $field, $choice, $value[ $input_id ] ) ) {
				return rgar( $choice, 'file_url' ) ? $choice['file_url'] : '';
			} else if ( ! is_array( $value ) && KDNAFormsModel::choice_value_match( $field, $choice, $value ) ) {
				return rgar( $choice, 'file_url' ) ? $choice['file_url'] : '';
			}
		}
...
KDNA_Fields::register( new KDNA_Field_Image_Choice() );
