 ...
 * Class KDNA_Field
 *
 * This class provides the base functionality for developers when creating new fields for KDNA Forms. It facilitates the following:
 ...
 */
class KDNA_Field extends stdClass implements ArrayAccess {
	...
	public function &__get( $key ) {
		...
		switch ( $key ) {
			...
			case 'size':
				...
				if ( isset( $this->size ) ) {
					$value = KDNACommon::whitelist( $this->size, array( 'small', 'medium', 'large' ) );
				}
	...
	public function get_form_editor_field_type_icon() {
		if ( $this->type !== $this->inputType ) {
			$field_class = KDNA_Fields::get( $this->type );
			if ( $field_class ) {
	...
	public function get_form_editor_field_description() {
		return sprintf( esc_attr__( 'Add a %s field to your form.', 'kdnaforms' ), $this->get_form_editor_field_title() );
	}
	 ...
	 * 'text'  => 'Button text'
	 *
	 * Built-in fields don't need to implement this because the buttons are added in sequence in KDNAFormDetail
	 *
	 * @return array
	 */
	 ...
	 *
	 * The {FIELD} placeholder will be replaced in KDNAFormDisplay::get_field_content with the markup returned by KDNA_Field::get_field_input().
	 *
	 * @param string|array $value                The field value. From default/dynamic population, $_POST, or a resumed incomplete submission.
	...
	public function get_field_content( $value, $force_frontend_label, $form ) {
		...
		$validation_message = ( $this->failed_validation && ! empty( $this->validation_message ) ) ? sprintf( "<div id='%s' class='kfield_description validation_message kfield_validation_message'>%s</div>", $validation_message_id, $this->validation_message ) : '';

		$is_form_editor  = $this->is_form_editor();
		$is_entry_detail = $this->is_entry_detail();
		$is_admin        = $is_form_editor || $is_entry_detail;

		$required_div = $this->isRequired ? '<span class="kfield_required">' . $this->get_required_indicator() . '</span>' : '';
		...
		$description = $this->get_description( $this->description, 'kfield_description' );
	...
	public function get_field_container( $atts, $form ) {
		...
		$field_sidebar_messages  = KDNACommon::is_form_editor() ? $this->get_field_sidebar_messages() : '';
		 ...
		 */
		$field_sidebar_messages = $is_form_editor ? apply_filters( 'kdnaform_field_sidebar_messages', $field_sidebar_messages, $this ) : $field_sidebar_messages;
		...
		if ( ! empty( $sidebar_message_content ) ) {
			$atts['class'] .= ' kfield-has-sidebar-message kfield-has-sidebar-message--type-' . ( $sidebar_message_type === 'error' ? 'warning' : $sidebar_message_type );
		...
		return sprintf(
			...
			empty( $sidebar_message_content ) ? '' : '<span class="field-sidebar-message-content field-sidebar-message-content--type-' . $sidebar_message_type . ' hidden">' . \KDNACommon::maybe_wp_kses( $sidebar_message_content ) . '</span>'
		);
	...
	public function get_field_container_tag( $form ) {
		...
		return KDNACommon::is_legacy_markup_enabled( $form ) ? 'li' : 'div';
	 ...
	 *
	 * @since unknown
	 * @since 2.5     Added `screen-reader-text` if the label hasn't been set; added `kfield_label_before_complex` if the field has inputs.
	...
	public function get_field_label_class() {
		...
		// Added `kfield_label_before_complex` if the field has inputs.
		$class .= is_array( $this->inputs ) ? ' kfield_label_before_complex' : '';
	...
	public function is_value_submission_empty( $form_id ) {
		...
		// KDNA_Field_Radio can now have inputs if it's a Choice or Image Choice field
		if ( is_array( $this->inputs ) && ! ( $this instanceof KDNA_Field_Radio ) ) {
			foreach ( $this->inputs as $input ) {
			...
			if ( is_array( $value ) ) {
				...
				foreach ( $value as $input ) {
					$input = KDNACommon::trim_deep( $input );
					if ( KDNACommon::safe_strlen( $input ) <= 0 ) {
						return true;
					}
				}
	...
	public function set_required_error( $value, $require_complex_message = false ) {
		...
		$this->validation_message = empty( $this->errorMessage ) ? __( 'This field is required.', 'kdnaforms' ) : $this->errorMessage;
	...
	public function complex_validation_message( $value, $required_inputs ) {
		...
		return sprintf( __( 'Please complete the following fields: %s.', 'kdnaforms' ), $field_list );
	}
	 ...
	 * @access public
	 *
	 * @used-by KDNA_Field_Name::validate()
	 * @uses    KDNAFormsModel::get_input()
	 *
	...
	public function get_input_property( $input_id, $property_name ) {
		$input = KDNAFormsModel::get_input( $this, $this->id . '.' . (string) $input_id );
	...
	public function get_input_value_submission( $standard_name, $custom_name = '', $field_values = array(), $get_from_post_global_var = true ) {
		...
		if ( ! empty( $_POST[ 'is_submit_' . $form_id ] ) && $get_from_post_global_var ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			...
			return KDNAFormsModel::maybe_trim_input( $value, $form_id, $this );
		} elseif ( $this->allowsPrepopulate ) {
			return KDNAFormsModel::get_parameter_value( $custom_name, $field_values, $this );
		}
	...
	public function get_value_entry_detail( $value, $entry = array(), $use_text = false, $format = 'html', $media = 'screen' ) {
		...
		if ( is_array( $value ) ) {
			...
			if ( $class === KDNA_Field::class ) {
				$error_message = sprintf( esc_html__( 'Field value cannot be displayed. Please activate the add-on that includes the `%s` field type.', 'kdnaforms' ), $this->type );
			} else {
				$error_message = sprintf( esc_html__( 'Field value cannot be displayed. If you are the developer of the `%s` field type, please implement `%s::get_value_entry_detail()` to define how the value is displayed.', 'kdnaforms' ), $this->type, $class );
			}

			return '<div class="error-alert-container alert-container">
						<div class="kform-alert kform-alert--error">
							<span class="kform-alert__icon kform-icon kform-icon--circle-close" aria-hidden="true"></span>
							<div class="kform-alert__message-wrap">
								<p class="kform-alert__message">' . $error_message . '</p>
							</div>
						</div>
					</div>';
		}
	...
	/**
	 * Format the entry value before it is used in entry exports and by framework add-ons using KDNAAddOn::get_field_value().
	 *
	...
	/**
	 * Apply the kdnaform_get_input_value filter to an entry's value.
	 *
	 * @since 2.4.24
	 *
	...
	public function get_selected_choice_output( $value, $currency = '', $use_text = false ) {
		...
		if ( $this->enablePrice ) {
			...
			if ( ! empty( $parts[1] ) ) {
				$price = KDNACommon::to_money( $parts[1], $currency );
			}
		}
	...
	public function get_selected_choice( $value ) {
		...
		foreach ( $this->choices as $choice ) {
			if ( KDNAFormsModel::choice_value_match( $this, $choice, $value ) ) {
				return $choice;
			}
		}
	...
	public function get_placeholder_value( $placeholder ) {
		...
		$placeholder_value = KDNACommon::replace_variables_prepopulate( $placeholder );
	...
	public function is_entry_detail() {
		return isset( $this->_is_entry_detail ) ? (bool) $this->_is_entry_detail : KDNACommon::is_entry_detail();
	}
	...
	public function is_entry_detail_edit() {
		return KDNACommon::is_entry_detail_edit();
	}
	...
	public function has_calculation() {
		...
		if ( $type == 'number' ) {
			if ( $this->calculationFormula ) {
				$ids = KDNACommon::get_field_ids_from_formula_tag( $this->calculationFormula );
	...
	public function get_admin_buttons() {
		...
		if(  ! in_array( $this->type, $duplicate_disabled ) ) {
			...
			$duplicate_field_link = "
				<button
					id='kfield_duplicate_{$this->id}'
					class='kfield-field-action kfield-duplicate'
				...
				>
					...
					<span class='kfield-field-action__description' aria-hidden='true'>" . esc_html__( 'Duplicate', 'kdnaforms' ) . "</span>
				</button>";
		}
		 ...
		 */
		$duplicate_field_link = apply_filters( 'kdnaform_duplicate_field_link', $duplicate_field_link );

		$delete_aria_action = __( 'delete this field', 'kdnaforms' );
		$delete_field_link = "
			<button
				id='kfield_delete_{$this->id}'
				class='kfield-field-action kfield-delete'
			...
			>
				<i class='kform-icon kform-icon--trash'></i>
				<span class='kfield-field-action__description' aria-hidden='true'>" . esc_html__( 'Delete', 'kdnaforms' ) . "</span>
			</button>";
		 ...
		 */
		$delete_field_link = apply_filters( 'kdnaform_delete_field_link', $delete_field_link );

		$edit_aria_action = __( 'jump to this field\'s settings', 'kdnaforms' );
		$edit_field_link = "
			<button
				id='kfield_edit_{$this->id}'
				class='kfield-field-action kfield-edit'
			...
			>
				<i class='kform-icon kform-icon--settings'></i>
				<span class='kfield-field-action__description' aria-hidden='true'>" . esc_html__( 'Settings', 'kdnaforms' ) . "</span>
			</button>";
		...
		$drag_handle = '
			<span class="kfield-field-action kfield-drag">
				<i class="kform-icon kform-icon--drag-indicator"></i>
				<span class="kfield-field-action__description">' . esc_html__( 'Move', 'kdnaforms' ) . '</span>
			</span>';
		...
		$field_icon = '<span class="kfield-field-action kfield-icon" title="' . $this->get_form_editor_field_title() . '">' . KDNACommon::get_icon_markup( array( 'icon' => $this->get_form_editor_field_type_icon() ) ) . '</span>';
		...
		if ( ! empty( $sidebar_message ) ) {
			$sidebar_message_types = array(
				'warning' => array( 'kform-icon--exclamation-simple', 'kform-icon-preset--status-error' ),
				'error'   => array( 'kform-icon--exclamation-simple', 'kform-icon-preset--status-error' ),
				'info'    => array( 'kform-icon--information-simple', 'kform-icon-preset--status-info' ),
				'notice'  => array( 'kform-icon--information-simple', 'kform-icon-preset--status-info' ),
				'success' => array( 'kform-icon--checkmark-simple', 'kform-icon-preset--status-correct' ),
			);
			...
			$compact_view_sidebar_message_icon             = sprintf( '<span class="kfield-sidebar-message-icon kform-icon kform-icon--preset-active %1$s" title="%2$s" aria-label="%2$s"></span>', implode( ' ', $sidebar_message_types[ $compact_view_sidebar_message_icon_type ] ), esc_attr( $compact_view_sidebar_message_icon_helper_text ) );
		}

		$admin_buttons = "
			<div class='kfield-admin-icons kform-theme__disable'>
				{$drag_handle}
				{$duplicate_field_link}
				{$edit_field_link}
				{$delete_field_link}
			...
			<div class='kfield-compact-icons kform-theme__disable'>
				{$field_id}
				{$conditional}
			</div>";
	...
	public function get_required_indicator() {
		return KDNAFormsModel::get_required_indicator( $this->formId );
	}
	...
	public function get_hidden_admin_markup() {
		 ...
		 return '<div class="admin-hidden-markup"><i class="kform-icon kform-icon--hidden" aria-hidden="true" title="'. esc_attr( __( 'This field is hidden when viewing the form', 'kdnaforms' ) ) .'"></i><span>'. esc_attr( __( 'This field is hidden when viewing the form', 'kdnaforms' ) ) .'</span></div>';
	...
	public function get_field_label( $force_frontend_label, $value ) {
		$label = $force_frontend_label ? $this->label : KDNACommon::get_label( $this );
	...
	public function maybe_add_aria_describedby( $input, $field_id, $form_id ) {
		$first_input_for_field = $this->get_first_input_id( KDNAFormsModel::get_form_meta( $form_id ) );
	...
	public function get_description( $description, $css_class ) {
		...
		$is_entry_detail = $this->is_entry_detail();
		$is_admin        = $is_form_editor || $is_entry_detail;
		$id              = "kfield_description_{$this->formId}_{$this->id}";
	...
	public function get_aria_describedby( $extra_ids = array() ) {
		...
		if ( ! empty( $this->description ) ) {
			$describedby_ids[] = "kfield_description_{$this->formId}_{$this->id}";
		}
	...
	public function should_be_validated() {
		...
		if ( KDNAFormDisplay::get_source_page( $this->formId ) != $this->pageNumber ) {
			return false;
		}
	...
	public function get_inputs_describedby_attributes( $required_inputs_ids, $values ) {
		...
		foreach ( $this->inputs as $input ) {
			$input_id = str_replace( $this->id . '.', '', $input['id'] );
			$input_value = KDNAForms::get( $input['id'], $values );
	...
	public function get_value_default() {
		if ( ! is_array( $this->inputs ) ) {
			...
			return $this->is_form_editor() ? $default_value : KDNACommon::replace_variables_prepopulate( $default_value );
		}
		...
		foreach ( $this->inputs as $input ) {
			...
			$value[ strval( $input['id'] ) ] = $this->is_form_editor() ? $default_value : KDNACommon::replace_variables_prepopulate( $default_value );
		}
	...
	public function register_form_init_scripts( $form ) {
		KDNAFormDisplay::add_init_script( $form['id'], $this->type . '_' . $this->id, KDNAFormDisplay::ON_PAGE_RENDER, $this->get_form_inline_script_on_page_render( $form ) );
	}
	 ...
	 *
	 * This base method will only strip HTML tags if the field or the kdnaform_allowable_tags filter allows HTML.
	 *
	...
	public function sanitize_entry_value( $value, $form_id ) {
		...
		if ( $allowable_tags === true ) {
			...
			// Strip all tags except those allowed by the kdnaform_allowable_tags filter.
	...
	public function sanitize_settings() {
		...
		if ( $this->size ) {
			$this->size = KDNACommon::whitelist( $this->size, $this->get_size_choices( true ) );
		}
	...
	public function sanitize_settings_choices( $choices = null ) {
		...
		foreach ( $choices as &$choice ) {
			...
			if ( isset( $choice['price'] ) && ! empty( $choice['price'] ) ) {
				$price_number    = KDNACommon::to_number( $choice['price'] );
				$choice['price'] = KDNACommon::to_money( $price_number );
			}
	...
	public function sanitize_settings_conditional_logic( $logic = null ) {
		...
		$logic = KDNAFormsModel::sanitize_conditional_logic( $logic );
	...
	public function get_allowable_tags( $form_id = null ) {
		 ...
		 * @param KDNA_Field $this
		 * @param int      $form_id
		 */
		$allowable_tags = apply_filters( 'kdnaform_allowable_tags', $allow_html, $this, $form_id );
		$allowable_tags = apply_filters( "kdnaform_allowable_tags_{$form_id}", $allowable_tags, $this, $form_id );
	...
	public function post_convert_field() {
		...
		if (
			...
			&& ( in_array( $this->type, array( 'consent', 'donation' ) ) || KDNACommon::is_product_field( $this->type ) )
		) {
	...
	public function get_size_choices( $values_only = false ) {
		$choices = array(
			array( 'value' => 'small', 'text' => __( 'Small', 'kdnaforms' ) ),
			array( 'value' => 'medium', 'text' => __( 'Medium', 'kdnaforms' ) ),
			array( 'value' => 'large', 'text' => __( 'Large', 'kdnaforms' ) ),
		);
		 ...
		 */
		$choices = apply_filters( 'kdnaform_field_size_choices', $choices );
	...
	public function get_filter_settings() {
		$filter_settings = array(
			'key'  => $this->id,
			'text' => KDNAFormsModel::get_label( $this ),
		);
	...
	public function get_product_quantity_label( $form_id ) {
		...
		return kdna_apply_filters( array(
			...
			$form_id,
			$this->id,
		), esc_html__( 'Quantity', 'kdnaforms' ), $form_id );
	}
