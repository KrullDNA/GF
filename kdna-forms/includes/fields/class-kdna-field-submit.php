...
class KDNA_Field_Submit extends KDNA_Field {
	...
	public function __construct( $data = array() ) {
		add_filter( 'kdnaform_pre_render', array( $this, 'inject_inline_button' ), 100 );
	...
	public function get_field_container( $atts, $form ) {
		...
		$atts['data-field-class']    = 'kdnaform_editor_submit_container';
	...
	/**
	 * Add the submit button as a field if it is inline. Target of the kdnaform_pre_render filter.
	 *
	 * @since 2.6
	 *
	...
	public function inject_inline_button( $form ) {
		...
		$is_injected = (bool) KDNAFormsModel::get_fields_by_type( $form, array( $this->type ) );
		if ( ! $is_injected ) {
			$form['fields'][] = $this;
		}
...
KDNA_Fields::register( new KDNA_Field_Submit() );
