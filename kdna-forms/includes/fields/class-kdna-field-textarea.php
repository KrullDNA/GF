...
class KDNA_Field_Textarea extends KDNA_Field {

	public $type = 'textarea';

	public function get_form_editor_field_title() {
		return esc_attr__( 'Paragraph Text', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_description() {
		return esc_attr__( 'Allows users to submit multiple lines of text.', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_icon() {
		return 'kdnaform-icon--paragraph-text';
	}
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		if ( ! $is_admin && $this->is_rich_edit_enabled() && ( ! $current_screen || ( $current_screen && ! rgobj( $current_screen, 'is_block_editor' ) ) ) ) {
			...
			$tabindex = KDNACommon::$tab_index > 0 ? KDNACommon::$tab_index ++ : '';
			 ...
			 */
			$editor_settings = apply_filters( 'kdnaform_rich_text_editor_options', array(
			...
			$editor_settings = apply_filters( sprintf( 'kdnaform_rich_text_editor_options_%d', $form['id'] ),               $editor_settings, $this, $form, $entry );
			$editor_settings = apply_filters( sprintf( 'kdnaform_rich_text_editor_options_%d_%d', $form['id'], $this->id ), $editor_settings, $this, $form, $entry );
			...
			if ( $this->is_form_editor() ) {
				...
				$input       = "<div id='{$field_id}_rte_preview' class='kdnaform-rte-preview kdnaform-theme__disable {$size}' style='display:{$display}'>
		...
		return sprintf( "<div class='kdnainput_container kdnainput_container_textarea'>%s</div>", $input );
	}
	...
	public function validate( $value, $form ) {
		...
		if ( KDNACommon::safe_strlen( $value ) > $this->maxLength ) {
			$this->failed_validation  = true;
			$this->validation_message = empty( $this->errorMessage ) ? esc_html__( 'The text entered exceeds the maximum number of characters.', 'kdnaforms' ) : $this->errorMessage;
		}
	}
	...
	public static function end_wp_tiny_mce_init_buffer() {
		...
		$height_issue_fix = version_compare( $wp_version, '5.0', '>=' ) && version_compare( $wp_version, '5.2', '<' ) ? ' kdnaform_post_conditional_logic' : '';
...
KDNA_Fields::register( new KDNA_Field_Textarea() );
