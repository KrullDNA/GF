...
class ChoiceDecorator {
	...
	public function get_field_classes( $form_id, $field ) {
		...
		$label_visibility_class  = "kinput_container_image_choice--label-{$choice_label_visibility}";
		...
		$input_visibility_class  = "kinput_container_image_choice--input-{$input_visibility}";
	...
	public function get_image_markup( $choice, $choice_id, $choice_number, $form ) {
		$image_aria_describedby = 'kchoice_image_' . $choice_id;
		...
		if ( ! empty( $choice['attachment_id'] ) ) {
			...
			$image = wp_get_attachment_image(
				...
				'kform-image-choice-' . $image_size,
				false,
				array(
					'class'   => 'kfield-choice-image',
					'alt'     => $image_alt,
			...
			$image = sprintf(
				'<span class="kfield-choice-image-no-image" id="%s"><span>%s</span></span>',
		...
		return sprintf( '<div class="kfield-choice-image-wrapper">%s</div>', $image );
	}
}
