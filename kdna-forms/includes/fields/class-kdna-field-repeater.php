 ...
 * 2.4
 *
 * Class KDNA_Field_Repeater
 */
class KDNA_Field_Repeater extends KDNA_Field {
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'Repeater', 'kdnaforms' );
	}
	...
	public function get_value_submission_recursive( $field_values, $get_from_post_global_var ) {
		...
		if ( isset( $this->fields ) && is_array( $this->fields ) ) {
			...
			foreach ( $this->fields as $sub_field ) {
				/* @var KDNA_Field $sub_field */
				if ( isset( $sub_field->fields ) && is_array( $sub_field->fields ) ) {
					/* @var KDNA_Field_Repeater_Table $sub_field */
					...
					if ( $sub_field instanceof KDNA_Field_List ) {
	...
	public function get_field_input( $form, $values = '', $entry = null ) {
		...
		return sprintf( "<div class='kfield_repeater_wrapper' data-max_items='{$max_items}'>%s</div>", $html );
	}

	/**
	 * Returns the markup for the top of the repeater container.
	 *
	 * This method must return the opening tag for the container and this tag must have the class 'kfield_repeater_container'
	 *
	 * @since 2.4
	 *
	 * @param $values
	...
	public function get_input_top( $values ) {
		$html = "<fieldset class='kfield_repeater kfield_repeater_container'>\n";
		$label = esc_html( $this->label );
		$html .= "<legend class='kfield_label kform-field-label'>{$label}</legend>";
		return $html;
	}
	 ...
	 *
	 * This method must return a single HTML element with the class 'kfield_repeater_items'. This elemment must contain
	 * all the items as direct children and each item must have the class 'kfield_repeater_item'.
	 *
	 * @since 2.4
	 *
	 * @param $values
	...
	public function get_input_items( $values, $entry ) {
		...
		$rows = '<div class="kfield_repeater_items">';

		$i = 0;
		foreach ( $values as $value ) {
			$row = "<div class='kfield_repeater_item'>";
			foreach ( $fields as $field ) {
				...
				$row .= "<div class='kfield_repeater_cell'>" . $field_input . '</div>';

				$field->set_context_property( 'itemIndex', null );
			}
			$buttons = $this->get_buttons( $values );
			$row .= "<div class='kfield_repeater_buttons'>{$buttons}</div>";
			$row .= '</div>';
			$rows .= $row;
			$i++;
		}
	...
	public function get_buttons( $values ) {
		...
		$html = "<button type='button' class='add_repeater_item kform-theme-button kform-theme-button--secondary kform-theme-button--size-sm {$disabled_icon_class} {$add_button_class}' {$add_events}>" . $add_button_text . "</button>" .
		        "<button type='button' class='remove_repeater_item kform-theme-button kform-theme-button--secondary kform-theme-button--size-sm {$remove_button_class}' {$delete_events} style='{$delete_display}'>" . $remove_button_text . "</button>";
	...
	public function get_value_entry_detail( $item_values, $entry = array(), $use_text = false, $format = 'html', $media = 'screen' ) {
		...
		/* @var KDNA_Field[] $fields */
		$fields = $this->fields;
		$html   = "<div class='kfield_repeater' {$repeater_style}>";
		$repeater_label = $this->nestingLevel === 0 ? '' : $this->label;
		$html   .= "<div class='kfield_label kform-field-label' {$label_style}>{$repeater_label}</div>";
		$html   .= '<div class="kfield_repeater_items">';
		foreach ( $item_values as $item_value ) {
			$html .= '<div class="kfield_repeater_item">';
			foreach ( $fields as $sub_field ) {
				...
				$label = empty( $sub_field->fields ) ? "<div class='kfield_repeater_label' {$sub_field_label_style}>{$label}</div>" : '';
				$value = $sub_field->get_value_entry_detail( $sub_field_value, $entry, $use_text, 'html', $media );
				$value = "<div class='kfield_repeater_value' style='color:rgba(117, 117, 117, 1);font-size: 14px'>{$value}</div>";
				$html .= '<div class="kfield_repeater_cell">' . $label . $value . '</div>';
			}
			$html .= '</div>';
		}
		$html .= '</div>';
	...
	public function get_sub_field_input( $field, $form, $field_value, $entry, $index ) {
		$target_page = rgpost( 'kform_target_page_number_' . $this->formId );
		$source_page = rgpost( 'kform_source_page_number_' . $this->formId );
	...
	public function validate_subfield( $field, $field_value, $form ) {
		...
		if ( $field->isRequired && $field->is_value_empty( $field_value ) ) {
			$field->failed_validation  = true;
			$field->validation_message = empty( $field->errorMessage ) ? __( 'This field is required.', 'kdnaforms' ) : $field->errorMessage;
		}
	...
	public function get_sub_field_content( $field, $value, $form, $entry ) {
		...
		if ( $field->type === 'repeater' ) {
			$field_content = KDNAFormDisplay::get_field_content( $field, $value, true, $form['id'], $form );
		} else {
			$field_content = KDNAFormDisplay::get_field( $field, $value, true, $form );
		}
	 ...
	 *
	 * @param array             $entry
	 * @param KDNA_Field_Repeater $repeater_field
	 * @param string            $index
	 * @param bool              $apply_filters Whether to apply the filter_input_value filter to the entry.
	 *
	 * @return array
	...
	public function get_filter_sub_filters() {
		...
		foreach ( $fields as $field ) {
			...
			$filter_settings = array(
				'key'  => $field->id,
				'text' => KDNAFormsModel::get_label( $field, false, true ),
			);
	...
	public function get_value_export_recursive( $entry, $input_id = '', $use_text = false, $is_csv = false, $depth = 0, $padding = '    ' ) {
		...
		foreach ( $items as $item ) {
			...
			foreach ( $fields as $field ) {
				...
				$label = str_repeat( $padding, $depth ) . KDNAFormsModel::get_label( $field );
...
KDNA_Fields::register( new KDNA_Field_Repeater() );
