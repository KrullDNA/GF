 ...
 * @since 2.4
 *
 * Class KDNA_Field_Consent
 */
class KDNA_Field_Consent extends KDNA_Field {
	...
	/**
	 * KDNA_Field_Consent constructor.
	 *
	 * @since 2.4
	 *
	...
	public function __construct( $data = array() ) {
		 ...
		 */
		$this->checked_indicator_url = apply_filters( 'kdnaform_consent_checked_indicator', KDNACommon::get_base_url() . '/images/tick.png' );
		 ...
		 */
		$this->checked_indicator_markup = apply_filters( 'kdnaform_consent_checked_indicator_markup', '<img src="' . esc_url( $this->checked_indicator_url ) . '" alt="" />' );
	}
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'Consent', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_description() {
		return esc_attr__( 'Offers a “yes/no” consent checkbox and a detailed description of what is being consented to.', 'kdnaforms' );
	}
	...
	public function get_field_input( $form, $value = array(), $entry = null ) {
		...
		if ( $is_admin && ! KDNACommon::is_entry_detail_edit() ) {
			...
			$revision_id    = ! is_array( $value ) || empty( $value[ $id . '.3' ] ) ? KDNAFormsModel::get_latest_form_revisions_id( $form['id'] ) : $value[ $id . '.3' ];
	...
	public function get_value_export( $entry, $input_id = '', $use_text = false, $is_csv = false ) {
		...
		switch ( $input_id ) {
			case '1':
				$value = ! rgblank( $value ) ? esc_html__( 'Checked', 'kdnaforms' ) : esc_html__( 'Not Checked', 'kdnaforms' );
				break;
			case '3':
				$value = ! rgblank( $value ) ? $this->get_field_description_from_revision( $value ) : '';
				break;
...
KDNA_Fields::register( new KDNA_Field_Consent() );
