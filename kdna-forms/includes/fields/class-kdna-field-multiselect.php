...
// If the KDNA_Field class isn't available, bail.
if ( ! class_exists( 'KDNAForms' ) ) {
	die();
}

/**
 * Class KDNA_Field_MultiSelect
 *
 ...
 * @uses KDNA_Field
 */
class KDNA_Field_MultiSelect extends KDNA_Field {
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'Multi Select', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_description() {
		return esc_attr__( 'Allows users to select multiple options available in the multi select box.', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_icon() {
		return 'kform-icon--multi-select';
	}
	 ...
	 * @access public
	 *
	 * @uses KDNA_Field_MultiSelect::is_entry_detail()
	 * @uses KDNA_Field_MultiSelect::is_form_editor()
	 * @uses KDNA_Field_MultiSelect::get_conditional_logic_event()
	 * @uses KDNA_Field_MultiSelect::get_tabindex()
	 *
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		$css_class     = trim( $class . ' kfield_select' );
		...
		$placeholder = kdna_apply_filters( array(
			...
			$form_id,
			$this->id
		), __( 'Click to select...', 'kdnaforms' ), $form_id, $this );
	 ...
	 * @access public
	 *
	 * @uses KDNACommon::get_select_choices()
	 *
	 * @param string|array $value The field value. From default/dynamic population, $_POST, or a resumed incomplete submission.
	 *
	...
	public function get_choices( $value ) {
		...
		return KDNACommon::get_select_choices( $this, $value, false );
	...
	public function get_value_entry_detail( $value, $entry = array(), $use_text = false, $format = 'html', $media = 'screen' ) {
		...
		foreach ( $items as &$item ) {
			$item = esc_html( KDNACommon::selection_display( $item, $this, $currency, $use_text ) );
		}

		if ( $format === 'text' ) {
			return KDNACommon::implode_non_blank( ', ', $items );
		}

		return "<ul class='bulleted'><li>" . KDNACommon::implode_non_blank( '</li><li>', $items ) . '</li></ul>';
	}
	 ...
	 * @access public
	 *
	 * @uses KDNA_Field_MultiSelect::sanitize_entry_value()
	 *
	...
	public function get_value_merge_tag( $value, $input_id, $entry, $form, $modifier, $raw_value, $url_encode, $esc_html, $format, $nl2br ) {
		...
		if ( $this->type == 'post_category' ) {
			if ( is_array( $items ) ) {
				...
				foreach ( $items as &$item ) {
					$cat  = KDNACommon::format_post_category( $item, $use_id );
					$item = KDNACommon::format_variable_value( $cat, $url_encode, $esc_html, $format );
				}
			}
		} elseif ( ! in_array( 'value', $modifiers ) ) {

			foreach ( $items as &$item ) {
				$item = KDNACommon::selection_display( $item, $this, rgar( $entry, 'currency' ), true );
				$item = KDNACommon::format_variable_value( $item, $url_encode, $esc_html, $format );
			}
		}
	...
	public function get_value_export( $entry, $input_id = '', $use_text = false, $is_csv = false ) {
		...
		if ( ! empty( $value ) && ! $is_csv ) {
			...
			foreach ( $items as &$item ) {
				$item = KDNACommon::selection_display( $item, $this, rgar( $entry, 'currency' ), $use_text );
			}
...
KDNA_Fields::register( new KDNA_Field_MultiSelect() );
