...
/**
 * Class KDNA_Field_Name
 *
 * Handles the behavior of the Name field.
 *
 * @since Unknown
 */
class KDNA_Field_Name extends KDNA_Field {
	 ...
	 * @access public
	 *
	 * @used-by KDNACommon::get_field_type_title()
	 * @used-by KDNA_Field::get_form_editor_button()
	 *
	 * @return string
	 */
	public function get_form_editor_field_title() {
		return esc_attr__( 'Name', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_description() {
		return esc_attr__( 'Allows users to enter their name in the format you have specified.', 'kdnaforms' );
	}
	 ...
	 * @used-by KDNAFormSettings::output_field_scripts()
	 *
	 * @return bool true
	 */
	 ...
	 * @access public
	 *
	 * @used-by KDNAFormDisplay::validate()
	 * @uses    KDNA_Field_Name::$isRequired
	 * @uses    KDNA_Field_Name::$nameFormat
	 * @uses    KDNA_Field_Name::get_input_property
	 * @uses    KDNA_Field_Name::$failed_validation
	 * @uses    KDNA_Field_Name::$validation_message
	 * @uses    KDNA_Field_Name::$errorMessage
	 *
	 ...
	 * @access public
	 *
	 * @used-by KDNACommon::get_field_input()
	 * @uses    KDNA_Field::is_entry_detail()
	 * @uses    KDNA_Field::is_form_editor()
	 * @uses    KDNA_Field_Name::$size
	 * @uses    KDNA_Field_Name::$id
	 * @uses    KDNA_Field_Name::$subLabelPlacement
	 * @uses    KDNA_Field_Name::$isRequired
	 * @uses    KDNA_Field_Name::$failed_validation
	 * @uses    KDNAForms::get()
	 * @uses    KDNAFormsModel::get_input()
	 * @uses    KDNACommon::get_input_placeholder_attribute()
	 * @uses    KDNACommon::get_tabindex()
	 * @uses    KDNACommon::get_field_placeholder_attribute()
	 * @uses    KDNA_Field_Name::get_css_class()
	 *
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		if ( is_array( $value ) ) {
			$prefix = esc_attr( KDNAForms::get( $this->id . '.2', $value ) );
			$first  = esc_attr( KDNAForms::get( $this->id . '.3', $value ) );
			$middle = esc_attr( KDNAForms::get( $this->id . '.4', $value ) );
			$last   = esc_attr( KDNAForms::get( $this->id . '.6', $value ) );
			$suffix = esc_attr( KDNAForms::get( $this->id . '.8', $value ) );
		}

		$prefix_input = KDNAFormsModel::get_input( $this, $this->id . '.2' );
		$first_input  = KDNAFormsModel::get_input( $this, $this->id . '.3' );
		$middle_input = KDNAFormsModel::get_input( $this, $this->id . '.4' );
		$last_input   = KDNAFormsModel::get_input( $this, $this->id . '.6' );
		$suffix_input = KDNAFormsModel::get_input( $this, $this->id . '.8' );

		$first_placeholder_attribute  = KDNACommon::get_input_placeholder_attribute( $first_input );
		$middle_placeholder_attribute = KDNACommon::get_input_placeholder_attribute( $middle_input );
		$last_placeholder_attribute   = KDNACommon::get_input_placeholder_attribute( $last_input );
		$suffix_placeholder_attribute = KDNACommon::get_input_placeholder_attribute( $suffix_input );
		...
		switch ( $this->nameFormat ) {
			...
			case 'advanced' :
			case 'extended' :
				$prefix_tabindex = KDNACommon::get_tabindex();
				$first_tabindex  = KDNACommon::get_tabindex();
				$middle_tabindex = KDNACommon::get_tabindex();
				...
				if ( $is_sub_label_above ) {
					...
					if ( $is_admin || ! rgar( $prefix_input, 'isHidden' ) ) {
						...
						$prefix_markup       = "<span id='{$field_id}_2_container' class='name_prefix {$prefix_select_class} kform-grid-col kform-grid-col--size-auto' {$style}>
                                                    <label for='{$field_id}_2' class='kform-field-label kform-field-label--type-sub {$sub_label_class}'>{$prefix_sub_label}</label>
                                                    {$prefix_markup}
                                                  </span>";
					}
					...
					if ( $is_admin || ! rgar( $first_input, 'isHidden' ) ) {
						$first_markup = "<span id='{$field_id}_3_container' class='name_first kform-grid-col kform-grid-col--size-auto' {$style}>
                                                    <label for='{$field_id}_3' class='kform-field-label kform-field-label--type-sub {$sub_label_class}'>{$first_name_sub_label}</label>
					...
					if ( $is_admin || ( isset( $middle_input['isHidden'] ) && $middle_input['isHidden'] == false ) ) {
						$middle_markup = "<span id='{$field_id}_4_container' class='name_middle kform-grid-col kform-grid-col--size-auto' {$style}>
                                                    <label for='{$field_id}_4' class='kform-field-label kform-field-label--type-sub {$sub_label_class}'>{$middle_name_sub_label}</label>
					...
					if ( $is_admin || ! rgar( $last_input, 'isHidden' ) ) {
						$last_markup = "<span id='{$field_id}_6_container' class='name_last kform-grid-col kform-grid-col--size-auto' {$style}>
                                                            <label for='{$field_id}_6' class='kform-field-label kform-field-label--type-sub {$sub_label_class}'>{$last_name_sub_label}</label>
					...
					if ( $is_admin || ! rgar( $suffix_input, 'isHidden' ) ) {
						...
						$suffix_markup       = "<span id='{$field_id}_8_container' class='name_suffix {$suffix_select_class} kform-grid-col kform-grid-col--size-auto' {$style}>
                                                        <label for='{$field_id}_8' class='kform-field-label kform-field-label--type-sub {$sub_label_class}'>{$suffix_sub_label}</label>
				...
				return "<div class='kinput_complex{$class_suffix} kinput_container kinput_container--name {$css_class} kform-grid-row' id='{$field_id}'>
                            {$prefix_markup}
                            {$first_markup}
                            {$middle_markup}
                            {$last_markup}
			...
			case 'simple' :
				...
				return "<div class='kinput_container kinput_container_name'>
			...
			default :
				...
				if ( $is_sub_label_above ) {
					...
					$style        = ( $is_admin && rgar( $first_input, 'isHidden' ) ) ? "style='display:none;'" : '';
					if ( $is_admin || ! rgar( $first_input, 'isHidden' ) ) {
						$first_markup = "<span id='{$field_id}_3_container' class='name_first kform-grid-col' {$style}>
                                                    <label for='{$field_id}_3' class='kform-field-label kform-field-label--type-sub {$sub_label_class}'>{$first_name_sub_label}</label>
					...
					$style       = ( $is_admin && rgar( $last_input, 'isHidden' ) ) ? "style='display:none;'" : '';
					if ( $is_admin || ! rgar( $last_input, 'isHidden' ) ) {
						$last_markup = "<span id='{$field_id}_6_container' class='name_last kform-grid-col' {$style}>
                                                <label for='{$field_id}_6' class='kform-field-label kform-field-label--type-sub {$sub_label_class}'>" . $last_name_sub_label . "</label>
	 ...
	 * @access public
	 *
	 * @used-by KDNA_Field_Name::get_field_input()
	 * @uses    KDNAFormsModel::get_input()
	 *
	...
	public function get_css_class() {
		...
		$css_class .= "kdna_name_has_{$visible_input_count} kinput_container_name ";
	 ...
	 *
	 * @used-by KDNA_Field_Name::get_field_input()
	 * @uses    KDNACommon::get_input_placeholder_value()
	 * @uses    KDNACommon::get_input_placeholder_attribute()
	 *
	...
	public function get_name_prefix_field( $input, $id, $field_id, $value, $disabled_text, $tabindex ) {
		...
		if ( isset( $input['choices'] ) && is_array( $input['choices'] ) ) {
			$placeholder_value = KDNACommon::get_input_placeholder_value( $input );
			...
			$placeholder_attribute = KDNACommon::get_input_placeholder_attribute( $input );
	 ...
	 * @access public
	 *
	 * @used-by KDNAFormDetail::add_field()
	 * @used-by KDNAFormsModel::sanitize_settings()
	 * @uses    KDNA_Field::sanitize_settings()
	 * @uses    KDNA_Field::sanitize_settings_choices()
	 *
	 * @return void
	 */
	 ...
	 * @used-by KDNAAddOn::get_full_name()
	 *
...
KDNA_Fields::register( new KDNA_Field_Name() );
