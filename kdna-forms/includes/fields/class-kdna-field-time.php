 ...
 * @uses  KDNA_Field
 */
class KDNA_Field_Time extends KDNA_Field {
	 ...
	 * @used-by KDNAAddOn::get_field_map_choices()
	 * @used-by KDNAAddOn::prepare_field_select_field()
	 * @used-by KDNAAddOn::settings_field_map_select()
	 * @used-by KDNA_Field::get_form_editor_button()
	 *
	 * @return string The field title.
	 */
	public function get_form_editor_field_title() {
		return esc_attr__( 'Time', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_description() {
		return esc_attr__( 'Allows users to submit a time as hours and minutes.', 'kdnaforms' );
	}
	 ...
	 * @uses    KDNA_Field_Time::$failed_validation
	 * @uses    KDNA_Field_Time::$validation_message
	 * @uses    KDNA_Field_Time::$timeFormat
	 * @uses    KDNA_Field_Time::$errorMessage
	 *
	...
	public function validate( $value, $form ) {
		...
		if ( ! $is_valid_format || $hour < $min_hour || $hour > $max_hour || $minute < 0 || $minute > $max_minute ) {
			...
			$this->validation_message = empty( $this->errorMessage ) ? esc_html__( 'Please enter a valid time.', 'kdnaforms' ) : $this->errorMessage;
		}
	}
	 ...
	 * @uses    KDNA_Field_Time::$subLabelPlacement
	 * @uses    KDNAFormsModel::get_input()
	 * @uses    KDNA_Field::get_input_placeholder_attribute()
	 * @uses    KDNA_Field::get_tabindex()
	 *
	 * @param array      $form  The Form Object.
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		$hour_input   = KDNAFormsModel::get_input( $this, $this->id . '.1' );
		$minute_input = KDNAFormsModel::get_input( $this, $this->id . '.2' );

		$hour_placeholder_attribute   = $this->get_input_placeholder_attribute( $hour_input ) ? $this->get_input_placeholder_attribute( $hour_input ) : "placeholder='" . esc_attr__( 'HH', 'kdnaforms' ) . "'";
		$minute_placeholder_attribute = $this->get_input_placeholder_attribute( $minute_input ) ? $this->get_input_placeholder_attribute( $minute_input ) : "placeholder='" . esc_attr( _x( 'MM', 'Abbreviation: Minutes', 'kdnaforms' ) ) . "'";
		...
		$output_shim = $is_sub_label_above && KDNACommon::is_legacy_markup_enabled( $form );
		...
		if ( $is_form_editor || $this->timeFormat != '24' ) {
			...
			$ampm_field = "<div class='kfield_time_ampm kinput_container kinput_container_time {$colon_pmam_placement} kform-grid-col' {$ampm_field_style}>
                                " . ( $output_shim ? "<div class='kfield_time_ampm_shim' aria-hidden='true'>&nbsp;</div>" : "" ) . "
                                ...
                                <label class='kform-field-label kform-field-label--type-sub am_pm_label screen-reader-text' for='{$field_id}_3'>" . esc_html__( 'AM/PM', 'kdnaforms' ) . "</label>                                
                           </div>";
		} else {
			$ampm_field = '';
		}
		...
		if ( rgar( $hour_input, 'customLabel' ) !== '' ) {
			...
			$hour_label = esc_html__( 'HH', 'kdnaforms' );
		} else {
			$hour_label       = esc_html__( 'Hours', 'kdnaforms' );
		...
		if ( rgar( $minute_input, 'customLabel' ) !== '' ) {
			...
			$minute_label = esc_html( _x( 'MM', 'Abbreviation: Minutes', 'kdnaforms' ) );
		} else {
			$minute_label       = esc_html__( 'Minutes', 'kdnaforms' );
		...
		$new_markup_colon    = KDNACommon::is_legacy_markup_enabled( $form ) ? '' : '<div class="' . $colon_pmam_placement . ' hour_minute_colon kform-grid-col">:</div>';

		if ( $is_sub_label_above ) {
			$markup = "{$clear_multi_div_open}
                        <div class='kfield_time_hour kinput_container kinput_container_time kform-grid-col' id='{$field_id}'>
                            <label class='kform-field-label kform-field-label--type-sub hour_label{$hour_label_class}' for='{$field_id}_1'>{$hour_label}</label>
                        ...
                        {$new_markup_colon}
                        <div class='kfield_time_minute kinput_container kinput_container_time kform-grid-col'>
                            <label class='kform-field-label kform-field-label--type-sub minute_label{$minute_label_class}' for='{$field_id}_2'>{$minute_label}</label>
		...
		return sprintf( '<div class="kinput_container kinput_complex kform-grid-row">%s</div>', $markup );
	}
	 ...
	 * @access public
	 *
	 * @used-by KDNAFormDisplay::is_empty()
	 *
	...
	public function is_value_submission_empty( $form_id ) {
		...
		return KDNACommon::is_empty_array( $value );
	}
	 ...
	 * @access public
	 *
	 * @used-by KDNAFormsModel::prepare_value()
	 *
	...
	/**
	 * Overrides KDNA_Field to prevent the standard input ID from being used.
	 *
	 * @since  Unknown
	 * @access public
	 *
...
KDNA_Fields::register( new KDNA_Field_Time() );
