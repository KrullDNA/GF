...
// If KDNA Forms isn't loaded, bail.
if ( ! class_exists( 'KDNAForms' ) ) {
	die();
}

/**
 * Class KDNA_Field_List
 *
 * Handles the behavior of List fields.
 *
 * @since Unknown
 */
class KDNA_Field_List extends KDNA_Field {
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'List', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_description() {
		return esc_attr__( 'Allows the user to add/remove additional rows of information per field.', 'kdnaforms' );
	}
	 ...
	 * @access public
	 *
	 * @used-by KDNAFormDetail::inline_scripts()
	 *
	 ...
	 * @access public
	 *
	 * @uses KDNA_Field::is_form_editor()
	 * @uses KDNA_Field_List::$id
	 *
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		$class_has_columns = $has_columns ? 'kinput_container_list--columns' : '';

		$list = '';

		$list .= "<div class='kinput_container kinput_container_list kinput_list {$class_has_columns}'>" .
			"<div class='kfield_list kfield_list_container'>";

		if ( $has_columns ) {

			$list .= '<div class="kfield_list_header kform-grid-row">';
			foreach ( $columns as $column ) {
				$list .= '<div class="kform-field-label kfield_header_item kform-grid-col">' . esc_html( $column['text'] ) . '</div>';
			}

			if ( $this->maxRows != 1 ) {
				$list .= '<div class="kfield_header_item kfield_header_item--icons kform-grid-col">&nbsp;</div>';
			}
		...
		foreach ( $value as $item ) {
			...
			$list .= "<div class='kfield_list_row_{$odd_even} kfield_list_group kform-grid-row'>";
			$colnum = 1;
			foreach ( $columns as $column ) {
				$data_label = '';
				...
				$list .= "<div class='kfield_list_group_item kfield_list_cell kfield_list_{$this->id}_cell{$colnum} kform-grid-col' {$data_label}>" . $this->get_list_input( $has_columns, $column, $val, $form_id, $rownum ) . '</div>';
				$colnum ++;
			}
			...
			if ( $this->maxRows != 1 ) {
				...
				$icon_url_disabled = $is_form_editor ? 'kfield_url_icon_disabled' : '';
				
				$list .= "<div class='kfield_list_icons kform-grid-col'>";
				...
				if ( $this->addIconUrl ) {
					...
					$list .= "   <button type='button' {$disabled} class='add_list_item {$disabled_icon_class}' aria-label='" . esc_attr__( 'Add another row', 'kdnaforms' ) . "' {$add_events}>" . __( 'Add', 'kdnaforms' ) . "</button>";
				}
				
				if ( $this->deleteIconUrl ) {
					$list .= " <a href='javascript:void(0);' role='button' class='delete_list_item {$icon_url_disabled}' aria-label='" . esc_attr( str_replace( '{0}', $rownum, $aria_label_template ) ) . "' data-aria-label-template='{$aria_label_template}' {$delete_events} {$delete_display}><img src='{$this->deleteIconUrl}' title='" . esc_attr__( 'Remove this row', 'kdnaforms' ) . "' /></a>";
				} else {
					$list .= "   <button type='button' {$disabled} class='delete_list_item' aria-label='" . esc_attr( str_replace( '{0}', $rownum, $aria_label_template ) ) . "' data-aria-label-template='{$aria_label_template}' {$delete_events} {$delete_display}>" . __( 'Remove', 'kdnaforms' ) . "</button>";
				}
	...
	public function get_legacy_field_input( $form, $value = '', $entry = null ) {
		...
		if ( ! self::$_style_block_printed ){
			...
			$list_entry_edit = KDNAForms::get_page() === 'entry_detail_edit' ? '
				body .kinput_container_list table.kfield_list {
				...
				body .kinput_container_list table.kfield_list tbody tr {
					display: table-row !important;
				}

				body .kinput_container_list table.kfield_list tbody td.kfield_list_cell {
					vertical-align: top !important;
				}

				body .kinput_container_list table.kfield_list tbody td.kfield_list_cell > *:not([type="checkbox"]):not([type="radio"]) {
					width: 100% !important;
				}
				
				body .kinput_container_list table.kfield_list thead tr td {
					padding: 0;
				}
				
				body .kinput_container_list table.kfield_list tbody tr td.kfield_list_icons {
			...
			$list .= '<style type="text/css">'. $list_entry_edit .'
						...
						body .kinput_container_list table.kfield_list tbody tr td.kfield_list_icons img {
						...
						body .kinput_container_list table.kfield_list tbody tr td.kfield_list_icons a:hover img {
							opacity: 1.0;
						}
		...
		$list .= "<div class='kinput_container kinput_container_list kinput_list'>" .
			"<table class='kfield_list kfield_list_container'>";
		...
		if ( $has_columns ) {
			...
			for ( $colnum = 1; $colnum <= count( $columns ) + 1; $colnum++ ) {
				...
				$list .= sprintf( "<col id='kfield_list_%d_col_%d' class='kfield_list_col_%s' />", $this->id, $colnum, $odd_even );
			}
			$list .= '</colgroup>';
			...
			$list .=
				'<colgroup>' .
				"<col id='kfield_list_{$this->id}_col1' class='kfield_list_col_odd' />" .
				"<col id='kfield_list_{$this->id}_col2' class='kfield_list_col_even' />" .
				'</colgroup>';
		}
		...
		foreach ( $value as $item ) {
			...
			$list .= "<tr class='kfield_list_row_{$odd_even} kfield_list_group'>";
			$colnum = 1;
			foreach ( $columns as $column ) {
				$data_label = '';
				...
				$list .= "<td class='kfield_list_cell kfield_list_{$this->id}_cell{$colnum}' {$data_label}>" . $this->get_list_input( $has_columns, $column, $val, $form_id, $rownum ) . '</td>';
				$colnum ++;
			}
			...
			if ( $this->maxRows != 1 ) {
				...
				$list .= "<td class='kfield_list_icons'>";
	...
	public function get_list_input( $has_columns, $column, $value, $form_id, $row ) {
		...
		$aria_label_template .= __( ', Row {0}', 'kdnaforms' );
		 ...
		 * @param object KDNA_Field_List   Field object for this field type.
		 ...
		 * @param object KDNA_Field_List   An instance of the List field object.
		 * @param string $column['text'] The column text value.
		 * @param int    $form_id        The form ID.
		 */
	 ...
	 *
	 * @since unknown
	 * @since 2.5     Added `screen-reader-text` if the label hasn't been set; added `kfield_label_before_complex` if it has choices.
	...
	public function get_field_label_class() {
		...
		// Added `kfield_label_before_complex` if it has choices.
		$class .= is_array( $this->choices ) ? ' kfield_label_before_complex' : '';
	...
	public function get_value_entry_detail( $value, $entry = array(), $use_text = false, $format = 'html', $media = 'screen' ) {
		...
		if ( ! $has_columns ) {
			...
			switch ( $format ) {
				...
				default :
					if ( $media == 'email' ) {
						$list = "<table class='kfield_list' style='border-top: 1px solid #DFDFDF; border-left: 1px solid #DFDFDF; border-spacing: 0; padding: 0; margin: 2px 0 6px; width: 100%'><thead><tr>\n";
						...
						$list = "<table class='kfield_list'><thead><tr>";
	...
	public function get_value_merge_tag( $value, $input_id, $entry, $form, $modifier, $raw_value, $url_encode, $esc_html, $format, $nl2br ) {
		...
		return KDNACommon::get_lead_field_display( $this, $raw_value, $entry, true, $output_format );
	}
	 ...
	 * Use the kdnaform_display_field_select_columns_entry_list filter to make the list field available.
	 *
	 *
	 * @since 2.4
	 *
	...
	public function get_value_entry_list( $value, $entry, $field_id, $columns, $form ) {
		return KDNACommon::get_lead_field_display( $this, $value, $entry, true, 'html' );
	}
	 ...
	 * @access public
	 *
	 * @uses \KDNA_Field_MultiSelect::$storageType
	 *
	 ...
	 * @access public
	 *
	 * @used-by KDNAExport::start_export()
	 * @used-by KDNAAddOn::get_field_value()
	 * @uses    KDNA_Field_List::$id
	 * @uses    KDNA_Field_List::$enableColumns
	 * @uses    KDNA_Field_List::$choices
	...
	public function get_value_export( $entry, $input_id = '', $use_text = false, $is_csv = false ) {
		...
		return KDNACommon::implode_non_blank( ', ', $column_values );
	}
...
KDNA_Fields::register( new KDNA_Field_List() );
