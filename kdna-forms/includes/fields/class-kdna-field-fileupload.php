...
class KDNA_Field_FileUpload extends KDNA_Field {
	...
	public static function get_file_upload_path_info( $file_url, $entry_id = null ) {
		...
		$path_info = $entry_id ? kdnaform_get_meta( $entry_id, self::get_file_upload_path_meta_key_hash( $file_url ) ) : null;

		if ( empty( $path_info ) || ! is_array( $path_info ) ) {
			return array(
				'path' => KDNAFormsModel::get_upload_root(),
				'url'  => KDNAFormsModel::get_upload_url_root(),
			);
		}
	...
	public static function get_default_upload_roots( $form_id ) {
		...
		$y                       = substr( $time, 0, 4 );
		$m                       = substr( $time, 5, 2 );
		$default_target_root     = KDNAFormsModel::get_upload_path( $form_id ) . "/$y/$m/";
		$default_target_root_url = KDNAFormsModel::get_upload_url( $form_id ) . "/$y/$m/";
	...
	public function get_form_editor_field_title() {
		return esc_attr__( 'File Upload', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_description() {
		return esc_attr__( 'Allows users to upload a file.', 'kdnaforms' );
	}
	...
	public function is_check_type_and_ext_disabled() {
		...
		if ( ! is_bool( $disabled ) ) {
			 ...
			 */
			$disabled = (bool) apply_filters( 'kdnaform_file_upload_whitelisting_disabled', false );
		}
	...
	public function get_clean_allowed_extensions() {
		...
		if ( is_null( $extensions ) ) {
			$extensions = KDNACommon::clean_extensions( $this->allowedExtensions );
	...
	public function get_size_validation_message() {
		...
		return sprintf( esc_html__( 'File exceeds size limit. Maximum file size: %dMB.', 'kdnaforms' ), $max_upload_size_in_mb );
	}
	 ...
	 * @param bool  $is_new Whether the file is new (from $_FILES) or an existing file (from KDNAFormsModel::$uploaded_files).
	 *
	...
	public function is_invalid_file( $file, $is_new = true ) {
		KDNACommon::log_debug( __METHOD__ . '(): Validating file: ' . json_encode( $file ) );
			...
			if ( rgar( $file, 'error' ) !== UPLOAD_ERR_OK ) {
				switch ( rgar( $file, 'error' ) ) {
					...
					case UPLOAD_ERR_FORM_SIZE:
						KDNACommon::log_debug( __METHOD__ . '(): File exceeds size limit. Maximum file size: ' . $max_upload_size_in_mb . 'MB' );
						$message = $this->get_size_validation_message();
						break;
					default:
						/* translators: %d: PHP file upload error code. */
						$message = $this->errorMessage ?: sprintf( esc_html__( 'There was an error while uploading the file. Error code: %d.', 'kdnaforms' ), $file['error'] );
				}
				...
				KDNACommon::log_debug( __METHOD__ . '(): File was not uploaded via HTTP POST.' );
				$message = $this->errorMessage ?: esc_html__( 'The file is not valid.', 'kdnaforms' );

				return $this->get_invalid_file_result( $file, $message, $is_new );
			}

			if ( ! empty( $file_name ) && ! $this->is_check_type_and_ext_disabled() ) {
				$check_result = KDNACommon::check_type_and_ext( $file, $file_name );
				if ( is_wp_error( $check_result ) ) {
					KDNACommon::log_debug( sprintf( '%s(): %s; %s', __METHOD__, $check_result->get_error_code(), $check_result->get_error_message() ) );

					return $this->get_invalid_file_result( $file, $check_result->get_error_message(), $name_key );
				}
			}
		} elseif ( isset( $file['url'] ) && ( ! KDNACommon::is_valid_url( $file['url'] ) || empty( $file_name ) || ! str_contains( $file['url'], $file_name ) || empty( $file['hash'] ) || $this->get_populated_file_url_hash( $file ) !== $file['hash'] ) ) {
			$message = $this->errorMessage ?: esc_html__( 'The file URL is not valid.', 'kdnaforms' );
		...
		if ( empty( $allowed_extensions ) ) {
			if ( KDNACommon::file_name_has_disallowed_extension( $file_name ) ) {
				KDNACommon::log_debug( __METHOD__ . '(): The file has a disallowed extension.' );
				$message = $this->errorMessage ?: esc_html__( 'The uploaded file type is not allowed.', 'kdnaforms' );

				return $this->get_invalid_file_result( $file, $message, $name_key );
			}
		} else {
			if ( ! KDNACommon::match_file_extension( $file_name, $allowed_extensions ) ) {
				$allowed_extensions = implode( ', ', $allowed_extensions );
				KDNACommon::log_debug( __METHOD__ . '(): The file extension is not allowed. Allowed extensions: ' . $allowed_extensions . '.' );

				/* translators: %s: comma-separated list of allowed file extensions. */
				$message = $this->errorMessage ?: sprintf( esc_html__( 'The uploaded file type is not allowed. Must be one of the following: %s.', 'kdnaforms' ), $allowed_extensions );
	...
	public function validate( $value, $form ) {
		...
		KDNACommon::log_debug( __METHOD__ . sprintf( '(): Validating field #%d (name: input_%d).', $this->id, $this->id ) );
		...
		if ( ! $this->multipleFiles || ! rgblank( $this->maxFiles ) ) {
			...
			if ( $count && $count > $limit ) {
				...
				$this->validation_message = $this->errorMessage ?: sprintf( esc_html__( 'Number of files (%1$d) exceeds limit (%2$d).', 'kdnaforms' ), $count, $limit );
				KDNACommon::log_debug( __METHOD__ . sprintf( '(): Number of files (%d) exceeds limit (%d).', $count, $limit ) );
		...
		KDNACommon::log_debug( __METHOD__ . '(): Validation complete.' );

		if ( ! $this->failed_validation ) {
			return;
		}

		if ( KDNAFormDisplay::get_submission_context() !== 'form-submit' ) {
			...
			if ( $this->multipleFiles ) {
				...
				$this->set_context_property( 'validation_summary_message', sprintf( esc_html( _n( '%d file is invalid.', '%d files are invalid.', $count, 'kdnaforms' ) ), $count ) );
	...
	public function get_first_input_id( $form ) {
		...
		return $this->multipleFiles ? 'kdnaform_browse_button_' . $form['id'] . '_' . $this->id : 'input_' . $form['id'] . '_' . $this->id;
	}
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		$file_list_id    = 'kdnaform_preview_' . $form_id . '_' . $id;
		...
		if ( ! empty( $allowed_extensions ) ) {
			$upload_rules_messages[] = esc_attr( sprintf( __( 'Accepted file types: %s', 'kdnaforms' ), str_replace( ',', ', ', $allowed_extensions ) ) );
		}
		// File size.
		...
		$upload_rules_messages[] = esc_attr( sprintf( __( 'Max. file size: %s', 'kdnaforms' ), KDNACommon::format_file_size( $max_upload_size ) ) );
		// No. of files.
		...
		if ( $max_files ) {
			...
			$upload_rules_messages[] = esc_attr( sprintf( __( 'Max. files: %s', 'kdnaforms' ), $max_files ) );
		}

		$rules_messages = implode( ', ', $upload_rules_messages ) . '.';

		$rules_messages_id = empty( $rules_messages ) ? '' : "kfield_upload_rules_{$this->formId}_{$this->id}";
		...
		if ( $multiple_files ) {
			...
			$drag_drop_id      = 'kdnaform_drag_drop_area_' . $form_id . '_' . $id;

			$validation_message_id = 'kdnaform_multifile_messages_' . $form_id . '_' . $id;

			$messages_id        = "kdnaform_multifile_messages_{$form_id}_{$id}";
			if ( empty( $allowed_extensions ) ) {
				$allowed_extensions = '*';
			}
			$disallowed_extensions = KDNACommon::get_disallowed_file_extensions();
			if ( defined( 'DOING_AJAX' ) && DOING_AJAX && 'rg_change_input_type' === rgpost( 'action' ) ) {
				...
				$plupload_init = array(
					...
					'filters'             => array(
						'mime_types'    => array( array( 'title' => __( 'Allowed Files', 'kdnaforms' ), 'extensions' => $allowed_extensions ) ),
			...
			$upload             = "<div id='{$container_id}' data-settings='{$plupload_init_json}' class='kdnaform_fileupload_multifile'>
										<div id='{$drag_drop_id}' class='kform_drop_area kform-theme-field-control'>
											<span class='kdnaform_drop_instructions'>{$drop_files_here_text} </span>
											<button type='button' id='{$browse_button_id}' class='button kform_button_select_files kform-theme-button kform-theme-button--control' {$describedby} {$tabindex} {$disabled_text}>{$select_files_text}</button>
										</div>
									</div>";
			...
			if ( is_array( $messages_array ) ) {
				foreach ( $messages_array as $message_array ) {
					$messages .= sprintf( "<li id='error_%s' class='kfield_description kfield_validation_message'>%s</li>", esc_attr( rgar( $message_array, 'id' ) ), esc_html( rgar( $message_array, 'message' ) ) );
				}
			}

			// The JS will also populate this.
			$upload .= "<ul class='validation_message--hidden-on-empty kform-ul-reset' id='{$messages_id}'>{$messages}</ul>";
		...
		if ( $is_entry_detail && ! empty( $value ) ) { // edit entry
			...
			$preview .= sprintf( "<div id='%s' class='kinput_preview_list'></div>", $file_list_id );
			...
			foreach ( $file_urls as $file_index => $file_url ) {
				 ...
				 *
				 * By default KDNA Forms will attempt to determine if the schema of the URL should be overwritten for SSL.
				 ...
				 * @param KDNA_Field_FileUpload $field     The field object for further context.
				 */
				$field_ssl = apply_filters( 'kdnaform_secure_file_download_is_https', true, $file_url, $this );

				if ( $field_ssl === true && KDNACommon::is_ssl() && str_contains( $file_url, 'http:' ) ) {
					$file_url = str_replace( 'http:', 'https:', $file_url );
				}

				$download_file_text = esc_attr__( 'Download file', 'kdnaforms' );
				$delete_file_text   = esc_attr__( 'Delete file', 'kdnaforms' );
				$view_file_text     = esc_attr__( 'View file', 'kdnaforms' );
				...
				$preview .= "<div id='preview_file_{$file_index}' class='kinput_preview'>
								<a href='{$file_url}' target='_blank' aria-label='{$view_file_text}'>{$display_file_url}</a>
								<a href='{$file_url}' target='_blank' aria-label='{$download_file_text}' class='kinput_preview_control kform-icon kform-icon--circle-arrow-down'></a>
								<a href='javascript:void(0);' aria-label='{$delete_file_text}' onclick='DeleteFile({$lead_id},{$id},this);' onkeypress='DeleteFile({$lead_id},{$id},this);' class='kinput_preview_control kform-icon kform-icon--circle-delete'></a>
							</div>";
			}
			...
			if ( ! empty( $files ) ) {
				$preview   = sprintf( "<div id='%s' class='kinput_preview_list'>", $file_list_id );
				foreach ( $files as $file_info ) {
					...
					$preview           .= sprintf( "<div id='%s' class='kinput_preview'>%s</div>", esc_attr( rgar( $file_info, 'id' ) ), $file_upload_markup );
				}
				$preview .= '</div>';

				return "<div class='kinput_container kinput_container_fileupload'>" . $upload . " {$preview}</div>";
			} else {

				$preview = $multiple_files ? sprintf( "<div id='%s' class='kinput_preview_list'></div>", $file_list_id ) : '';
	 ...
	 * @since 2.7.4  Added the kdnaform_validate_required_file_exists filter.
	 * @since 2.9.2  Updated to use KDNAFormsModel::get_tmp_upload_location().
	...
	public function is_value_submission_empty( $form_id ) {
		...
		$tmp_path     = rgar( KDNAFormsModel::get_tmp_upload_location( $form_id ), 'path' );
		$file_removed = false;

		foreach ( $files['existing'] as $key => $file ) {
			if ( empty( $file['uploaded_filename'] ) ) {
				KDNACommon::log_debug( __METHOD__ . "(): Removing invalid file for {$input_name} key {$key}." );
	...
	public function unset_uploaded_file( $input_name, $key ) {
		KDNACommon::log_debug( __METHOD__ . "(): Removing invalid file for {$input_name} key {$key}." );
		unset( KDNAFormsModel::$uploaded_files[ $this->formId ][ $input_name ][ $key ] );
	}
	...
	public function get_multifile_value( $form_id, $input_name, $value, $entry_id = null ) {
		...
		if ( $this->is_submission_files_empty( $files ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Aborting; no files.' );
		...
		foreach ( $files['existing'] as $file ) {
			if ( ! isset( $file['temp_filename'] ) ) {
				...
				if ( isset( $file['url'] ) ) {
					...
					if ( ! is_string( $existing_file ) && KDNACommon::is_valid_url( $file['url'] ) ) {
				...
				$uploaded_path = kdnaform_get_meta( $entry_id, self::get_file_upload_path_meta_key_hash( $existing_file ) );
				...
				if ( $uploaded_path ) {
					...
					$uploaded_path = KDNAFormsModel::get_file_upload_path( $form_id, $existing_file, false );
		...
		if ( ! empty( $value ) ) {
			...
			if ( empty( $uploaded_files ) ) {
				KDNACommon::log_debug( __METHOD__ . '(): Aborting; no valid file, name, or URL to save.' );
	...
	public function check_existing_entry( $entry_id, $input_name, $file_info ) {
		$existing_entry = $entry_id ? KDNAAPI::get_entry( $entry_id ) : null;
		...
		$existing_files    = KDNACommon::maybe_decode_json( rgar( $existing_entry, $input_id ) );
	...
	public function get_parsed_list_of_files( $value, $form_id = 0, $input_name = '' ) {
		if ( func_num_args() === 1 ) {
			...
			$uploaded = rgar( KDNAFormsModel::$uploaded_files, $form_id . '/' . $input_name, array() );
		}
	 ...
	 *
	 * @since 1.9
	 * @since 2.9.18 Updated to use $this->get_submission_files(), $this->filter_submission_files_pre_save(), & KDNAFormsModel::get_tmp_upload_location().
	 *
	 * @param int    $form_id    ID of the form.
	 * @param string $input_name Name of the input (e.g. input_1).
	 *
	...
	public function get_single_file_value( $form_id, $input_name ) {
		...
		if ( $this->is_submission_files_empty( $files ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Aborting; no file.' );
		...
		$count = ( count( $files['existing'] ) + count( $files['new'] ) );
		if ( $count > 1 ) {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): %d files uploaded, only saving one.', $count ) );
		}
		...
		if ( ! empty( $files['new'][0] ) ) {
			if ( rgar( $files['new'][0], 'error' ) === UPLOAD_ERR_OK ) {
				KDNACommon::log_debug( __METHOD__ . '(): Calling upload_file.' );
				$value = $this->upload_file( $form_id, $files['new'][0] );
			}
		} elseif ( ! empty( $files['existing'][0]['url'] ) ) {
			if ( KDNACommon::is_valid_url( $files['existing'][0]['url'] ) ) {
				KDNACommon::log_debug( __METHOD__ . '(): Saving provided URL, not uploading file.' );
			...
			if ( ! file_exists( $tmp_path . $temp_filename ) ) {
				KDNACommon::log_debug( __METHOD__ . sprintf( '(): Temporary file %s does not exist.', $tmp_path . $temp_filename ) );

				return '';
			}

			KDNACommon::log_debug( __METHOD__ . '(): File already uploaded to tmp folder, moving.' );
			...
			if ( ! empty( $file_name ) ) {
				KDNACommon::log_debug( __METHOD__ . '(): Saving provided filename, not uploading file.' );
				$value = $file_name;
			}
		}
	...
	public function upload_file( $form_id, $file ) {
		KDNACommon::log_debug( __METHOD__ . '(): Uploading file: ' . $file['name'] );
		$target = KDNAFormsModel::get_file_upload_path( $form_id, $file['name'] );
		if ( ! $target ) {
			KDNACommon::log_debug( __METHOD__ . '(): FAILED (Upload folder could not be created.)' );

			return 'FAILED (Upload folder could not be created.)';
		}
		KDNACommon::log_debug( __METHOD__ . '(): Upload folder is ' . print_r( $target, true ) );

		if ( move_uploaded_file( $file['tmp_name'], $target['path'] ) ) {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): File (tmp_name: %s) successfully moved to %s.', $file['tmp_name'], $target['path'] ) );
			$this->set_permissions( $target['path'] );

			return $target['url'];
		} else {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): Aborting; move_uploaded_file() failed for file (tmp_name: %s) to %s.', $file['tmp_name'], $target['path'] ) );
	...
	public function get_value_entry_list( $value, $entry, $field_id, $columns, $form ) {
		if ( $this->multipleFiles ) {
			...
			if ( $file_count > 1 ) {
				...
				return sprintf( esc_html__( '%d files', 'kdnaforms' ), $file_count );
			} elseif ( $file_count === 1 ) {
		...
		if ( ! empty( $file_path ) ) {
			...
			$thumb     = KDNAEntryList::get_icon_url( $file_path );
			$file_path = $this->get_download_url( $file_path, false, rgar( $entry, 'id' ) );
			$file_path = esc_attr( $file_path );
			$value = "<a href='$file_path' target='_blank'><span class='screen-reader-text'>" . esc_html__( 'View the image', 'kdnaforms' ) . "</span><span class='screen-reader-text'>" . esc_html__( '(opens in a new tab)', 'kdnaforms' ) . "</span><img src='$thumb' alt='' /></a>";
		}
		return $value;
	}
	...
	public function get_value_entry_detail( $value, $entry = array(), $use_text = false, $format = 'html', $media = 'screen' ) {
		...
		if ( ! empty( $files ) ) {
			...
			foreach ( $files as $file_path ) {
				 ...
				 * @param KDNA_Field_FileUpload $field     The field object for further context.
				 */
				$field_ssl = apply_filters( 'kdnaform_secure_file_download_is_https', true, $file_path, $this );

				if ( $field_ssl === true && KDNACommon::is_ssl() && str_starts_with( $file_path, 'http:' ) ) {
				 ...
				 * @param KDNA_Field_FileUpload $field     The field object for further context.
				 */
				$file_path    = str_replace( ' ', '%20', apply_filters( 'kdnaform_fileupload_entry_value_file_path', $file_path, $this ) );
				$output_arr[] = $format === 'text' ? $file_path : sprintf( "<li><a href='%s' target='_blank' aria-label='%s'>%s</a></li>", esc_attr( $file_path ), esc_attr__( 'Click to view', 'kdnaforms' ), esc_html( $basename ) );
	 ...
	 * @access public
	 *
	 * @uses KDNA_Field::get_modifiers()
	 * @uses KDNA_Field_FileUpload::get_download_url()
	 *
	...
	public function move_temp_file( $form_id, $tempfile_info ) {
		...
		$target       = KDNAFormsModel::get_file_upload_path( $form_id, $tempfile_info['uploaded_filename'] );
		$tmp_location = KDNAFormsModel::get_tmp_upload_location( $form_id );
		$source       = $tmp_location['path'] . wp_basename( $tempfile_info['temp_filename'] );


		KDNACommon::log_debug( __METHOD__ . '(): Moving temp file from: ' . $source );

		if ( rename( $source, $target['path'] ) ) {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): File (temp_filename: %s) successfully moved to %s.', $tempfile_info['temp_filename'], $target['path'] ) );
			$this->set_permissions( $target['path'] );

			return $target['url'];
		} else {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): Aborting; rename() failed for file (temp_filename: %s) to %s.', $tempfile_info['temp_filename'], $target['path'] ) );
	...
	function set_permissions( $path ) {
		KDNACommon::log_debug( __METHOD__ . '(): Setting permissions on: ' . $path );

		KDNAFormsModel::set_permissions( $path );
	}
	...
	public function get_download_url( $file, $force_download = false, $entry_id = 0 ) {
		 ...
		 * @param KDNA_Field_FileUpload $this                     The Field
		 */
		$secure_download_location = apply_filters( 'kdnaform_secure_file_download_location', $secure_download_location, $file, $this );
		$secure_download_location = apply_filters( 'kdnaform_secure_file_download_location_' . $this->formId, $secure_download_location, $file, $this );
		...
		if ( ! $secure_download_location ) {
			 ...
			 * @param KDNA_Field_FileUpload $field        The field object for further context.
			 */
			return apply_filters( 'kdnaform_secure_file_download_url', $download_url, $this );

		}

		$upload_root = KDNAFormsModel::get_upload_url( $this->formId );
		...
		if ( str_contains( $file, $upload_root ) ) {
			...
			$args         = array(
				'kdna-download' => urlencode( $file ),
				'form-id'     => $this->formId,
				'field-id'    => $this->id,
				'entry-id'    => absint( $entry_id ),
				'hash'        => KDNACommon::generate_download_hash( $this->formId, $this->id, $file, $entry_id ),
			);
			if ( $force_download ) {
				$args['dl'] = 1;
			}
	...
	public function get_extra_entry_metadata( $form, $entry ) {
		...
		foreach ( $file_values as $file_value ) {
			...
			$stored_path_info = kdnaform_get_meta( $entry_id, self::get_file_upload_path_meta_key_hash( $file_value ) );
	 ...
	 *      }
	 *      @type array[] $existing {
	 *          File details from KDNAFormsModel::$uploaded_files.
	 *
	...
	public function get_submission_files() {
		...
		$input_name = 'input_' . absint( $this->id );
		$value      = rgars( KDNAFormsModel::$uploaded_files, absint( $this->formId ) . '/' . $input_name, array() );
	...
	/**
	 * Caches the submission files in a context property and the existing files in KDNAFormsModel::$uploaded_files.
	 *
	 * @since 2.9.18
	 *
	...
	public function set_submission_files( $files ) {
		...
		if ( empty( $files['existing'] ) ) {
			unset( KDNAFormsModel::$uploaded_files[ $form_id ][ $input_name ] );
		} else {
			KDNAFormsModel::$uploaded_files[ $form_id ] ??= array();

			KDNAFormsModel::$uploaded_files[ $form_id ][ $input_name ] = $files['existing'];
		}
	}
	...
	private function filter_submission_files_pre_save( $files ) {
		 ...
		 * @since 2.9.18
		 *
		 * @param array[]             $files See KDNA_Field_FileUpload::get_submission_files() for format.
		 * @param KDNA_Field_FileUpload $field The field the files are for.
		 * @param array               $entry The entry currently being saved. Only fields located before the current field will be included.
		 * @param array               $form  The form currently being processed.
		 */
		$filtered = apply_filters( 'kdnaform_submission_files_pre_save_field_value', $files, $field, $entry, $form );
	...
	public function get_tmp_file_details( $file_or_name ) {
		...
		$uuid      = KDNAFormsModel::get_uuid();

		// This is the approach used by upload.php. The only difference is the use of KDNAFormsModel::get_uuid() instead of a UUID provided by plupload.js.
		$tmp_file_name = KDNAFormsModel::get_form_unique_id( absint( $this->formId ) ) . '_input_' . absint( $this->id ) . '_' . KDNACommon::random_str( 16 ) . '_' . $uuid . '.' . $extension;
		...
		KDNACommon::log_debug( __METHOD__ . '(): Details to use for new temporary file are: ' . json_encode( $tmp_file ) );
	...
	public function upload_submission_tmp_files() {
		...
		if ( $this->failed_validation ) {
			KDNACommon::log_debug( __METHOD__ . "(): Skipping field because it failed validation: {$this->label}({$id} - {$this->type})." );
		...
		$files = $this->get_submission_files();
		if ( empty( $files['new'] ) ) {
			KDNACommon::log_debug( __METHOD__ . "(): Skipping field because there are no files to process: {$this->label}({$id} - {$this->type})." );

			return array();
		}

		KDNACommon::log_debug( __METHOD__ . "(): Processing files for field: {$this->label}({$id} - {$this->type})." );
		...
		foreach ( $files['new'] as $key => $file ) {
			if ( $file['error'] !== UPLOAD_ERR_OK ) {
				KDNACommon::log_debug( __METHOD__ . '(): Skipping file because there was an error: ' . json_encode( $file ) );
				continue;
			}

			if ( ! is_uploaded_file( $file['tmp_name'] ) ) {
				KDNACommon::log_debug( __METHOD__ . '(): Skipping file because it was not uploaded via HTTP POST: ' . json_encode( $file ) );
				continue;
			}

			$file_name = $file['name'];
			if ( KDNACommon::file_name_has_disallowed_extension( $file_name ) ) {
				KDNACommon::log_debug( __METHOD__ . '(): Skipping file because the file extension is disallowed: ' . $file_name );
				continue;
			}

			if ( ! empty( $allowed_extensions ) && ! KDNACommon::match_file_extension( $file_name, $allowed_extensions ) ) {
				KDNACommon::log_debug( __METHOD__ . '(): Skipping file because the file extension is not allowed: ' . $file_name );
				continue;
			}

			if ( empty( $allowed_extensions ) && ! $this->is_check_type_and_ext_disabled() ) {
				$valid_file_name = KDNACommon::check_type_and_ext( $file, $file_name );

				if ( is_wp_error( $valid_file_name ) ) {
					KDNACommon::log_debug( __METHOD__ . '(): Skipping file because the uploaded file type is not allowed: ' . $file_name );
					continue;
				}
			}
		...
		if ( ! empty( $uploaded_files ) ) {
			if ( ! empty( $files['new'] ) ) {
				...
				KDNACommon::log_debug( __METHOD__ . '(): The following files were not uploaded: ' . json_encode( $files['new'] ) );
			}
			$this->set_submission_files( $files );
		}

		KDNACommon::log_debug( __METHOD__ . "(): Processing completed for field: {$this->label}({$id} - {$this->type})." );
	...
	public function upload_tmp_file( $file ) {
		...
		$form_id  = absint( $this->formId );
		$location = KDNAFormsModel::get_tmp_upload_location( $form_id );
		...
		if ( file_exists( $tmp_file_path ) ) {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): Aborting; File (tmp_name: %s) already moved to %s.', $file['tmp_name'], $location['path'] ) );

			return $details;
		}

		if ( ! move_uploaded_file( $file['tmp_name'], $tmp_file_path ) ) {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): Aborting; move_uploaded_file() failed for file (tmp_name: %s) to %s.', $file['tmp_name'], $location['path'] ) );

			return array();
		}

		KDNACommon::log_debug( __METHOD__ . sprintf( '(): File (tmp_name: %s) successfully moved to %s.', $file['tmp_name'], $location['path'] ) );
	...
	/**
	 * Triggers population of KDNAFormsModel::$uploaded_files with any file URLs found in the given value.
	 *
	 * @since 2.9.18
	 *
	...
	public function populate_file_urls_from_value( $value ) {
		if ( KDNACommon::is_empty_array( $value ) ) {
			return;
		}
	...
	/**
	 * Populates the submission_files context property & KDNAFormsModel::$uploaded_files with the details array for the given file URL.
	 *
	 * @since 2.9.18
	 *
	...
	public function populate_file_url( $url ) {
		...
		// If this is not empty, it was populated before the kdnaform_field_value filter (e.g., old version of UR).
		if ( ! empty( $files['existing'] ) ) {
			foreach ( $files['existing'] as &$existing_file ) {
				...
				if ( empty( $existing_file['id'] ) ) {
					$existing_file['id'] = KDNAFormsModel::get_uuid();
				}
	...
	public function get_file_name_from_url( $url ) {
		if ( empty( $url ) || ! KDNACommon::is_valid_url( $url ) ) {
			return false;
		}
		...
		if ( $query_string ) {
			...
			$file_name = rgar( $query_args, 'kdna-download' ) ?? rgar( $query_args, 'kdna-signature' );
...
KDNA_Fields::register( new KDNA_Field_FileUpload() );
