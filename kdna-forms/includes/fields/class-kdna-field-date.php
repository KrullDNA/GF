...
class KDNA_Field_Date extends KDNA_Field {

	public $type = 'date';

	public function get_form_editor_field_title() {
		return esc_attr__( 'Date', 'kdnaforms' );
	}
	...
	public function get_form_editor_field_description() {
		return esc_attr__( 'Allows users to enter a date.', 'kdnaforms' );
	}
	...
	public function validate( $value, $form ) {
		...
		if ( ! empty( $value ) ) {
			...
			$date   = KDNACommon::parse_date( $value, $format );
			...
			if ( empty( $date ) || ! $this->checkdate( $date['month'], $date['day'], $date['year'] ) ) {
				...
				$message                  = $this->dateType == 'datepicker' ? sprintf( esc_html__( 'Please enter a valid date in the format (%s).', 'kdnaforms' ), $format_name ) : esc_html__( 'Please enter a valid date.', 'kdnaforms' );
	...
	public function get_field_container_tag( $form ) {
		...
		if ( KDNACommon::is_legacy_markup_enabled( $form ) || $this->dateType === 'datepicker' ) {
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		$picker_value = '';
		if ( is_array( $value ) ) {
			// KDNACommon::parse_date() takes a numeric array.
		...
		$date_info = KDNACommon::parse_date( $value, $format, true );
		...
		$month_input = KDNAFormsModel::get_input( $this, $this->id . '.1' );
		$day_input   = KDNAFormsModel::get_input( $this, $this->id . '.2' );
		$year_input  = KDNAFormsModel::get_input( $this, $this->id . '.3' );
		...
		$year_min = apply_filters( 'kdnaform_date_min_year', '1920', $form, $this );
		$year_max = apply_filters( 'kdnaform_date_max_year', date( 'Y' ) + 1, $form, $this );
		...
		$clear_multi_div_open = KDNACommon::is_legacy_markup_enabled( $form ) ? '<div class="clear-multi">' : '';
		$clear_multi_div_close = KDNACommon::is_legacy_markup_enabled( $form ) ? '</div>' : '';
		...
		if ( $is_form_editor ) {
			...
			if ( $is_sub_label_above ) {
				$month_field = "<div class='kdnafield_date_month kdnainput_date kdnainput_container kdnainput_container_date kdnaform-grid-col' id='kdnafield_input_date_month' style='display:$datefield_display'>
                                    <label for='{$field_id}_1' class='kdnaform-field-label kdnaform-field-label--type-sub {$month_sub_label_class}'>{$month_sub_label}</label>
                                    <input id='{$field_id}_1' name='kdnainput_month' type='text' {$month_placeholder_attribute} {$disabled_text} value='{$month_value}'/>
                                </div>";
				$day_field   = "<div class='kdnafield_date_day kdnainput_date kdnainput_container kdnainput_container_date kdnaform-grid-col' id='kdnafield_input_date_day' style='display:$datefield_display'>
                                    <label for='{$field_id}_2' class='kdnaform-field-label kdnaform-field-label--type-sub {$day_sub_label_class}'>{$day_sub_label}</label>
                                    <input id='{$field_id}_2' name='kdnainput_day' type='text' {$day_placeholder_attribute} {$disabled_text} value='{$day_value}'/>
                               </div>";
				$year_field  = "<div class='kdnafield_date_year kdnainput_date kdnainput_container kdnainput_container_date kdnaform-grid-col' id='kdnafield_input_date_year' style='display:$datefield_display'>
                                    <label for='{$field_id}_3' class='kdnaform-field-label kdnaform-field-label--type-sub {$year_sub_label_class}'>{$year_sub_label}</label>
				...
				$year_field  = "<div class='kdnafield_date_year kdnainput_date kdnainput_container kdnainput_container_date kdnaform-grid-col' id='kdnafield_input_date_year' style='display:$datefield_display'>
                                    <input type='text' id='{$field_id}_3' name='kdnainput_year' {$year_placeholder_attribute} {$disabled_text} value='{$year_value}'/>
                                    <label for='{$field_id}_3' class='kdnaform-field-label kdnaform-field-label--type-sub {$year_sub_label_class}'>{$year_sub_label}</label>
                               </div>";
			}

			$month_dropdown = "<div class='kdnafield_date_dropdown_month kdnainput_date_dropdown kdnainput_container kdnainput_container_date kdnaform-grid-col' id='kdnafield_dropdown_date_month' style='display:$dropdown_display'>" . $this->get_month_dropdown( '', "{$field_id}_1", rgar( $date_info, 'month' ), '', $disabled_text, $month_placeholder_value ) . '</div>';
			$day_dropdown   = "<div class='kdnafield_date_dropdown_day kdnainput_date_dropdown kdnainput_container kdnainput_container_date kdnaform-grid-col' id='kdnafield_dropdown_date_day' style='display:$dropdown_display'>" . $this->get_day_dropdown( '', "{$field_id}_2", rgar( $date_info, 'day' ), '', $disabled_text, $day_placeholder_value ) . '</div>';
			$year_dropdown  = "<div class='kdnafield_date_dropdown_year kdnainput_date_dropdown kdnainput_container kdnainput_container_date kdnaform-grid-col' id='kdnafield_dropdown_date_year' style='display:$dropdown_display'>" . $this->get_year_dropdown( '', "{$field_id}_3", rgar( $date_info, 'year' ), '', $disabled_text, $year_placeholder_value, $form ) . '</div>';

			$field_string = "<div class='kdnainput_container kdnainput_container_date' id='kdnafield_input_datepicker' style='display:$datepicker_display'><input name='kdnainput_datepicker' type='text' {$date_picker_placeholder} {$disabled_text} value='{$picker_value}'/><img src='" . KDNACommon::get_base_url() . "/images/datepicker/datepicker.svg' id='kdnafield_input_datepicker_icon' style='display:$icon_display'/></div>";
			...
			$field_string .= "<div id='{$field_id}' class='kdnainput_container kdnainput_complex kdnaform-grid-row'>{$date_inputs}</div>";
			...
			if ( in_array( $date_type, array( 'datefield', 'datedropdown' ) ) ) {
				...
				switch ( $field_position ) {
					...
					case 'dmy' :
						...
						if ( $date_type == 'datedropdown' ) {
							...
							$field_str = "{$clear_multi_div_open}<div class='kdnafield_date_dropdown_day kdnainput_container kdnainput_container_date kdnaform-grid-col' id='{$field_id}_2_container'>" . $this->get_day_dropdown( "input_{$id}[]", "{$field_id}_2", rgar( $date_info, 'day' ), $tabindex, $disabled_text, $day_placeholder_value, $day_aria_attributes ) . '</div>';

							$tabindex = $this->get_tabindex();
							$field_str .= "<div class='kdnafield_date_dropdown_month kdnainput_container kdnainput_container_date kdnaform-grid-col' id='{$field_id}_1_container'>" . $this->get_month_dropdown( "input_{$id}[]", "{$field_id}_1", rgar( $date_info, 'month' ), $tabindex, $disabled_text, $month_placeholder_value, $month_aria_attributes ) . '</div>';

							$tabindex = $this->get_tabindex();

							$field_str .= "<div class='kdnafield_date_dropdown_year kdnainput_container kdnainput_container_date kdnaform-grid-col' id='{$field_id}_3_container'>" . $this->get_year_dropdown( "input_{$id}[]", "{$field_id}_3", rgar( $date_info, 'year' ), $tabindex, $disabled_text, $year_placeholder_value, $form, $year_aria_attributes ) ."</div>{$clear_multi_div_close}";
						} else {

							$field_str = $is_sub_label_above
								? "{$clear_multi_div_open}
                                        <div class='kdnafield_date_day kdnainput_container kdnainput_container_date kdnaform-grid-col' id='{$field_id}_2_container'>
                                            <label for='{$field_id}_2' class='kdnaform-field-label kdnaform-field-label--type-sub {$day_sub_label_class}'>{$day_sub_label}</label>
							...
							$field_str .= $is_sub_label_above
								? "<div class='kdnafield_date_month kdnainput_container kdnainput_container_date kdnaform-grid-col' id='{$field_id}_1_container'>
                                        <label for='{$field_id}_1' class='kdnaform-field-label kdnaform-field-label--type-sub {$month_sub_label_class}'>{$month_sub_label}</label>
							...
							$field_str .= $is_sub_label_above
								? "<div class='kdnafield_date_year kdnainput_container kdnainput_container_date kdnaform-grid-col' id='{$field_id}_3_container'>
                                            <label for='{$field_id}_3' class='kdnaform-field-label kdnaform-field-label--type-sub {$year_sub_label_class}'>{$year_sub_label}</label>
					...
					case 'ymd' :
						...
						if ( $date_type == 'datedropdown' ) {
							...
							$field_str = "{$clear_multi_div_open}<div class='kdnafield_date_dropdown_year kdnainput_container kdnainput_container_date kdnaform-grid-col' id='{$field_id}_3_container'>" . $this->get_year_dropdown( "input_{$id}[]", "{$field_id}_3", rgar( $date_info, 'year' ), $tabindex, $disabled_text, $year_placeholder_value, $form, $year_aria_attributes ) . '</div>';
							...
							$field_str .= "<div class='kdnafield_date_dropdown_day kdnainput_container kdnainput_container_date kdnaform-grid-col' id='{$field_id}_2_container'>" . $this->get_day_dropdown( "input_{$id}[]", "{$field_id}_2", rgar( $date_info, 'day' ), $tabindex, $disabled_text, $day_placeholder_value, $day_aria_attributes ) . "</div>{$clear_multi_div_close}";
						} else {
					...
					default :
						...
						if ( $date_type == 'datedropdown' ) {
							...
							$field_str = "{$clear_multi_div_open}<div class='kdnafield_date_dropdown_month kdnainput_container kdnainput_container_date kdnaform-grid-col' id='{$field_id}_1_container'>" . $this->get_month_dropdown( "input_{$id}[]", "{$field_id}_1", rgar( $date_info, 'month' ), $tabindex, $disabled_text, $month_placeholder_value, $month_aria_attributes ) . '</div>';

							$tabindex = $this->get_tabindex();

							$field_str .= "<div class='kdnafield_date_dropdown_day kdnainput_container kdnainput_container_date kdnaform-grid-col' id='{$field_id}_2_container'>" . $this->get_day_dropdown( "input_{$id}[]", "{$field_id}_2", rgar( $date_info, 'day' ), $tabindex, $disabled_text, $day_placeholder_value, $day_aria_attributes ) . '</div>';
							...
							$field_str = $is_sub_label_above
								? "{$clear_multi_div_open}<div class='kdnafield_date_month kdnainput_container kdnainput_container_date kdnaform-grid-col' id='{$field_id}_1_container'>
                                            <label for='{$field_id}_1' class='kdnaform-field-label kdnaform-field-label--type-sub {$month_sub_label_class}'>{$month_sub_label}</label>
                                            <input type='{$date_input_type}' {$month_maxlength} name='input_{$id}[]' id='{$field_id}_1' value='{$month_value}' {$tabindex} {$disabled_text} {$month_aria_attributes} {$month_placeholder_attribute} {$month_html5_attributes}/>
                                        </div>"
								: "{$clear_multi_div_open}<div class='kdnafield_date_month kdnainput_container kdnainput_container_date kdnaform-grid-col' id='{$field_id}_1_container'>
				...
				return "<div id='{$field_id}' class='kdnainput_container kdnainput_complex kdnaform-grid-row'>$field_str</div>";
			} else {
				$picker_value = esc_attr( KDNACommon::date_display( $picker_value, $format ) );
				$icon_class   = $this->calendarIconType == 'none' ? 'datepicker_no_icon gdatepicker-no-icon' : 'datepicker_with_icon gdatepicker_with_icon';
				$icon_url     = empty( $this->calendarIconUrl ) ? KDNACommon::get_base_url() . '/images/datepicker/datepicker.svg' : $this->calendarIconUrl;
				...
				return "<div class='kdnainput_container kdnainput_container_date'>
                            <input name='input_{$id}' id='{$field_id}' type='text' value='{$picker_value}' class='datepicker kdnaform-datepicker {$format} {$icon_class}' {$tabindex} {$disabled_text} {$date_picker_placeholder} {$describedby_attribute} {$invalid_attribute} {$required_attribute}/>
                            <span id='{$field_id}_date_format' class='screen-reader-text'>{$date_format_sr_text}</span>
                        </div>
                        <input type='hidden' id='gforms_calendar_icon_$field_id' class='kdnaform_hidden' value='$icon_url'/>";
			}
		}
	}
	 ...
	 *
	 * @since unknown
	 * @since 2.5     Added `screen-reader-text` if the label hasn't been set; added `kdnafield_label_before_complex` if it is datefield.
	...
	public function get_field_label_class() {
		...
		// Added `kdnafield_label_before_complex` if it is datefield.
		$class .= $this->dateType === 'datefield' ? ' kdnafield_label_before_complex' : '';

		return $class;
	}

	public function get_field_css_class() {
		$class  = parent::get_field_css_class();
		$class .= sprintf( ' kdnafield--input-type-%s', $this->dateType );

		if ( $this->dateType === 'datepicker' ) {
			switch( $this->calendarIconType ) {
				case 'calendar':
					$class .= ' kdnafield--datepicker-default-icon';
					break;
				case 'custom':
					$class .= ' kdnafield--datepicker-custom-icon';
					break;
				case 'none':
					$class .= ' kdnafield--datepicker-no-icon';
					break;
				default:
					break;
			}
	...
	public function get_value_entry_list( $value, $entry, $field_id, $columns, $form ) {
		return KDNACommon::date_display( $value, $this->dateFormat );
	}
	...
	public function get_value_entry_detail( $value, $entry = array(), $use_text = false, $format = 'html', $media = 'screen' ) {
		...
		return KDNACommon::date_display( $value, $this->dateFormat, $this->get_output_date_format() );
	}
	 ...
	 * @access public
	 *
	 * @uses KDNACommon::date_display()
	 * @uses KDNA_Field_Date::$dateFormat
	 *
	...
	public function get_form_inline_script_on_page_render( $form ) {
		...
		if ( ! KDNAFormDisplay::has_js_merge_tag( $form ) ) {
			return '';
		}
	...
	private function get_month_dropdown( $name = '', $id = '', $selected_value = '', $tabindex = '', $disabled_text = '', $placeholder = '', $aria_attributes = '' ) {
		if ( $placeholder == '' ) {
			$placeholder = esc_html__( 'Month', 'kdnaforms' );
		}
	...
	private function get_day_dropdown( $name = '', $id = '', $selected_value = '', $tabindex = '', $disabled_text = '', $placeholder = '', $aria_attributes = '' ) {
		if ( $placeholder == '' ) {
			$placeholder = esc_html__( 'Day', 'kdnaforms' );
		}
	...
	private function get_year_dropdown( $name, $id, $selected_value, $tabindex, $disabled_text, $placeholder, $form, $aria_attributes = '' ) {
		...
		if ( $placeholder == '' ) {
			$placeholder = esc_html__( 'Year', 'kdnaforms' );
		}
		$year_min = apply_filters( 'kdnaform_date_min_year', '1920', $form, $this );
		$year_max = apply_filters( 'kdnaform_date_max_year', date( 'Y' ) + 1, $form, $this );
	...
	private function get_dropdown_label( $id, $placeholder ) {
		return "<label for='{$id}' class='kdnaform-field-label kdnaform-field-label--type-sub hidden_sub_label screen-reader-text'>{$placeholder}</label>";
	}
	...
	private function get_date_format( $type = 'label' ) {
		...
		switch ( $format ) {
			case 'mdy':
				if ( $type === 'label' ) {
					$format = esc_attr__( 'mm/dd/yyyy', 'kdnaforms' );
				} else {
					$format = esc_attr__( 'MM slash DD slash YYYY', 'kdnaforms' );
				}
				break;
			case 'dmy':
				if ( $type === 'label' ) {
					$format = esc_attr__( 'dd/mm/yyyy', 'kdnaforms' );
				} else {
					$format = esc_attr__( 'DD slash MM slash YYYY', 'kdnaforms' );
				}
				break;
			case 'dmy_dash':
				if ( $type === 'label' ) {
					...
					$format = esc_attr__( 'DD dash MM dash YYYY', 'kdnaforms' );
				}
				break;
			case 'dmy_dot':
				if ( $type === 'label' ) {
					...
					$format = esc_attr__( 'DD dot MM dot YYYY', 'kdnaforms' );
				}
				break;
			case 'ymd_slash':
				if ( $type === 'label' ) {
					$format = esc_attr__( 'yyyy/mm/dd', 'kdnaforms' );
				} else {
					$format = esc_attr__( 'YYYY slash MM slash DD', 'kdnaforms' );
				}
				break;
			case 'ymd_dash':
				if ( $type === 'label' ) {
					...
					$format = esc_attr__( 'YYYY dash MM dash DD', 'kdnaforms' );
				}
				break;
			case 'ymd_dot':
				if ( $type === 'label' ) {
					...
					$format = esc_attr__( 'YYYY dot MM dot DD', 'kdnaforms' );
				}
				break;
		}
	...
	private function get_input_date_format( $input ) {
		...
		switch ( $input_id ) {
			case $this->id . '.1':
				$format = esc_html( _x( 'MM', 'Abbreviation: Month', 'kdnaforms' ) );
				break;
			case $this->id . '.2':
				$format = esc_html( _x( 'DD', 'Abbreviation: Day', 'kdnaforms' ) );
				break;
			default:
				$format = esc_html( _x( 'YYYY', 'Abbreviation: Year', 'kdnaforms' ) );
		}
	...
	public function get_input_label( $input ) {
		...
		if ( rgar( $input, 'placeholder' ) && $placeholder_value !== $format ) {
			...
			switch ( $input_id ) {
				case $this->id . '.1':
					$sub_label = esc_html__( 'Month', 'kdnaforms' );
					break;
				case $this->id . '.2':
					$sub_label = esc_html__( 'Day', 'kdnaforms' );
					break;
				default:
					$sub_label = esc_html__( 'Year', 'kdnaforms' );
			}
		}
	...
	public function get_value_save_entry( $value, $form, $input_name, $lead_id, $lead ) {
		...
		$value = KDNAFormsModel::prepare_date( $this->dateFormat, $value );
	...
	public function get_filter_settings() {
		...
		$filter_settings['placeholder'] = esc_html__( 'yyyy-mm-dd', 'kdnaforms' );
		$filter_settings['cssClass']    = 'datepicker kdnaform-datepicker ymd_dash';
	 ...
	 * @since  2.5.7
	 * @access public
	 * @see    KDNA_Field::post_convert_field()
	 *
	 * @uses KDNA_Field::post_convert_field()
	 * @uses KDNA_Field_Date::maybe_upgrade_inputs()
	 *
	 * @return void
	 */
	...
	public function maybe_update_inputs() {
		...
		$inputs = array(
			array(
				'id' => "{$this->id}.1",
				'label' => esc_html__( 'Month', 'kdnaforms' ),
				'name' => ''
			),
			array(
				'id' => "{$this->id}.2",
				'label' => esc_html__( 'Day', 'kdnaforms' ),
				'name' => ''
			),
			array(
				'id' => "{$this->id}.3",
				'label' => esc_html__( 'Year', 'kdnaforms' ),
				'name' => ''
			)
		);
...
KDNA_Fields::register( new KDNA_Field_Date() );
