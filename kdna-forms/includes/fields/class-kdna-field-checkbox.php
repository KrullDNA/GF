...
require_once( plugin_dir_path( __FILE__ ) . 'field-decorator-choice/class-kdna-field-decorator-choice-checkbox-markup.php' );

class KDNA_Field_Checkbox extends KDNA_Field {
	...
	public function get_form_editor_field_title() {
		...
		return esc_attr__( 'Checkboxes', 'kdnaforms' );
	...
	public function get_form_editor_field_description() {
		return esc_attr__( 'Allows users to select one or many checkboxes.', 'kdnaforms' );
	}
	...
	public function get_default_properties() {
		return array(
			'selectAllText' => esc_html__( 'Select All', 'kdnaforms' ),
		);
	}
	 ...
	 * @since 2.7 Added `kfield_choice_all_toggle` class to Select All button.
	 *
	...
	public function get_field_input( $form, $value = '', $entry = null ) {
		...
		if ( $this->type == 'image_choice' ) {
			$this->image_markup = new KDNA_Field_Decorator_Choice_Checkbox_Markup( $this );
		...
		$select_all_enabled_class = $this->enableSelectAll ? 'kfield_choice--select_all_enabled' : '';

		$limit_message = $this->get_limit_message();

		if ( 'multi_choice' == $this->type || ! $this->enableSelectAll ) {
			return sprintf(
				"<div class='kinput_container kinput_container_checkbox'>%s<div class='kfield_checkbox %s' id='%s'>%s</div></div>",
				$limit_message,
		...
		return sprintf(
			"<div class='kinput_container kinput_container_checkbox'>%s<div class='kfield_checkbox %s' id='%s'>%s%s</div></div>",
			$limit_message,
			$select_all_enabled_class,
			esc_attr( $field_id ),
			$choices_markup,
	...
	public function get_limit_message() {
		...
		return "<span class='kfield_choice_limit_message kfield_description' id='kfield_choice_limit_message_{$form_id}_{$id}'>{$text}</span>";
	}
	...
	public function get_legacy_field_input( $form, $value = '', $entry = null ) {
		...
		return sprintf(
			"<div class='kinput_container kinput_container_checkbox'><{$tag} class='kfield_checkbox' id='%s'>%s</{$tag}></div>",
			esc_attr( $field_id ),
	...
	private function get_selected_choices_count( $value = '', $entry = null ) {
		...
		foreach ( $this->choices as $choice ) {
			...
			if ( ( $this->is_form_editor() || ( ! isset( $_GET['kdna_token'] ) && empty( $_POST ) ) ) && rgar( $choice, 'isSelected' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing
				...
				$checkboxes_selected++;
			} else if ( ! is_array( $value ) && KDNAFormsModel::choice_value_match( $this, $choice, $value ) ) {
				$checkboxes_selected++;
			}
	 ...
	 *
	 * @uses KDNAFormsModel::choice_value_match()
	 * @uses KDNAFormsModel::get_parameter_value()
	 *
	 * @return array|string
	 */
	public function get_value_submission( $field_values, $get_from_post_global_var = true ) {
		...
		$parameter_values = KDNAFormsModel::get_parameter_value( $this->inputName, $field_values, $this );
			...
			if ( ! empty( rgpost( 'is_submit_' . $this->formId ) ) && $get_from_post_global_var ) {
				...
				if ( is_array( $parameter_values ) ) {
					...
					foreach ( $parameter_values as $item ) {
						...
						if ( KDNAFormsModel::choice_value_match( $this, $choice, $item ) ) {
							$value[ $input['id'] . '' ] = $item;
							break;
						}
	 ...
	 * @uses KDNACommon::prepare_post_category_value()
	 * @uses KDNACommon::selection_display()
	 * @uses KDNA_Field_Checkbox::is_checkbox_checked()
	 *
	 * @return string
	 */
	public function get_value_entry_list( $value, $entry, $field_id, $columns, $form ) {
		...
		if ( absint( $field_id ) == $field_id ) {
			...
			$value = KDNACommon::implode_non_blank( ', ', $items );

			// Special case for post category checkbox fields.
			if ( $this->type == 'post_category' ) {
				$value = KDNACommon::prepare_post_category_value( $value, $this, 'entry_list' );
			}
		} else {
	 ...
	 *
	 * @uses KDNA_Field_Checkbox::sanitize_entry_value()
	 *
		...
		if ( empty( $input_id ) || absint( $input_id ) == $input_id ) {
			...
			foreach ( $this->inputs as $input ) {
				...
				if ( ! rgempty( $index, $entry ) ) {
					$selected[] = KDNACommon::selection_display( rgar( $entry, $index ), $this, rgar( $entry, 'currency' ), $use_text );
				}
			...
			$value = $this->is_checkbox_checked( $input_id, KDNACommon::get_label( $this, $input_id ), $entry );

			return empty( $value ) ? '' : $value;

		} else {

			return KDNACommon::selection_display( rgar( $entry, $input_id ), $this, rgar( $entry, 'currency' ), $use_text );
	 ...
	 * @uses KDNA_Field::get_conditional_logic_event()
	...
	public function get_checkbox_choices( $value, $disabled_text, $form_id = 0 ) {
		...
		if ( is_array( $this->choices ) ) {
			...
			if ( $this->type == 'multi_choice' && $this->enableSelectAll ) {
				$this->choice_field     = new KDNA_Field_Multiple_Choice( $this );
				$selected_choices_count = $this->get_selected_choices_count( $value );
				$tabindex               = $this->get_tabindex();
			...
			if ( $count < $total ) {
				$choices .= "<{$tag} class='kchoice_total'><span>" . sprintf( esc_html__( '%d of %d items shown. Edit choices to view all.', 'kdnaforms' ), $count, $total ) . "</span></{$tag}>";
			}
	...
	public function get_checked_attribute( $choice, $value, $input_id, $form_id ) {
		...
		if ( ( $is_form_editor || ( ! isset( $_GET['kdna_token'] ) && empty( $_POST ) ) ) && rgar( $choice, 'isSelected' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.NonceVerification.Missing
			...
			$checked = "checked='checked'";
		} elseif ( ! is_array( $value ) && KDNAFormsModel::choice_value_match( $this, $choice, $value ) && ! empty( $_POST[ 'is_submit_' . $form_id ] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			$checked = "checked='checked'";
		} else {
			$checked = '';
		}
	...
	public function get_choice_aria_describedby( $form_id, $describedby = array() ) {
		...
		if ( $this->get_limit_message_text() ) {
			$limit_describedby[] = 'kfield_choice_limit_message_' . $form_id . '_' . $this->id;
		}
	 ...
	 * @access public
	 *
	 * @uses KDNACommon::replace_variables_prepopulate()
	 * @uses KDNA_Field::is_form_editor()
	 *
	 * @return array|string
	 */
	 ...
	 *
	 * @uses KDNA_Field::get_allowable_tags()
	 *
	 * @return string
	 */
...
KDNA_Fields::register( new KDNA_Field_Checkbox() );
