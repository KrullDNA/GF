...
class KDNAAsyncUpload {

	public static function upload() {

		KDNACommon::log_debug( 'KDNAAsyncUpload::upload(): Starting.' );
		...
		if ( ! isset( $_REQUEST['form_id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			KDNACommon::log_debug( 'KDNAAsyncUpload::upload(): File upload aborted because the form_id was not found. The file may have been bigger than the max post size ini setting.' );
		...
		if ( KDNACommon::form_requires_login( $form ) && ! is_user_logged_in() ) {
			self::die_error( 401, __( 'You must be logged in to upload files to this form.', 'kdnaforms' ) );
		}

		/**
		 * Filter the field object that will be associated with the uploaded file.
		 *
		 * This is useful when you want to use KDNA Forms' upload system to upload files. Using this filter you can return a one-time-use field object
		...
		$target_dir = rgar( KDNAFormsModel::get_tmp_upload_location( $form_id ), 'path' );
		if ( empty( $target_dir ) || ( ! is_dir( $target_dir ) && ! wp_mkdir_p( $target_dir ) ) ) {
			KDNACommon::log_debug( "KDNAAsyncUpload::upload(): Couldn't create the tmp folder: " . $target_dir );
			self::die_error( 500, __( 'Failed to upload file.', 'kdnaforms' ) );
		}
		...
		if ( ! file_exists( KDNAFormsModel::get_upload_root() . '/index.html' ) ) {
			KDNAForms::add_security_files();
		} else if ( ! file_exists( KDNAFormsModel::get_upload_path( $form_id ) . '/index.html' ) ) { // nosemgrep audit.php.lang.security.file.phar-deserialization
			KDNACommon::recursive_add_index_file( KDNAFormsModel::get_upload_path( $form_id ) );
		} else if ( ! file_exists( KDNAFormsModel::get_upload_path( $form_id ) . "/$y/index.html" ) ) { // nosemgrep audit.php.lang.security.file.phar-deserialization 
			KDNACommon::recursive_add_index_file( KDNAFormsModel::get_upload_path( $form_id ) . "/$y" );
		} else {
			KDNACommon::recursive_add_index_file( KDNAFormsModel::get_upload_path( $form_id ) . "/$y/$m" );
		}

		if ( ! file_exists( $target_dir . '/index.html' ) ) { // nosemgrep audit.php.lang.security.file.phar-deserialization
			KDNACommon::recursive_add_index_file( $target_dir );
		}
		...
		if ( $_FILES['file']['size'] > 0 && $_FILES['file']['size'] > $max_upload_size_in_bytes ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated, WordPress.Security.NonceVerification.Missing
			...
			self::die_error( 104, sprintf( __( 'File exceeds size limit. Maximum file size: %dMB', 'kdnaforms' ), $max_upload_size_in_mb ) );
		}
		...
		if ( $chunk ) {
			if ( empty( $chunk_data['hash'] ) || ( $chunk_data['hash'] !== self::get_chunk_hash( $chunk_data['temp_filename'], ( $chunk - 1 ), $form_id, $field_id, $uploaded_filename ) ) ) {
				KDNACommon::log_debug( __METHOD__ . sprintf( '(): Invalid hash for chunk #%d.', $chunk ) );
				self::die_error( 105, __( 'Upload unsuccessful', 'kdnaforms' ) . ' ' . $uploaded_filename );
			}
			$tmp_file_name = $chunk_data['temp_filename'];
		}

		if ( empty( $tmp_file_name ) ) {
			$tmp_file_name = $form_unique_id . '_input_' . $field_id . '_' . KDNACommon::random_str( 16 ) . '_' . $file_name;
		}
		...
		$cleanup_target_dir = apply_filters( 'kdnaform_cleanup_target_dir', true ); // Remove old files
			...
			if ( is_dir( $target_dir ) && ( $dir = opendir( $target_dir ) ) ) {
				while ( ( $file = readdir( $dir ) ) !== false ) {
					...
					if ( preg_match( '/\.part$/', $file ) && ( filemtime( $tmp_file_path ) < time() - $max_file_age ) && ( $tmp_file_path != "{$file_path}.part" ) ) { // nosemgrep audit.php.lang.security.file.phar-deserialization
						KDNACommon::log_debug( 'KDNAAsyncUpload::upload(): Deleting file: ' . $tmp_file_path );
				...
				KDNACommon::log_debug( 'KDNAAsyncUpload::upload(): Failed to open temp directory: ' . $target_dir );
				self::die_error( 100, __( 'Failed to open temp directory.', 'kdnaforms' ) );
			}
		}
			...
			if ( isset( $_FILES['file']['tmp_name'] ) && is_uploaded_file( $_FILES['file']['tmp_name'] ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized, WordPress.Security.NonceVerification.Missing
				...
				if ( $out ) {
					...
					if ( $in ) {
						...
						self::die_error( 101, __( 'Failed to open input stream.', 'kdnaforms' ) );
					}
					...
					self::die_error( 102, __( 'Failed to open output stream.', 'kdnaforms' ) );
				}
			} else {
				self::die_error( 103, __( 'Failed to move uploaded file.', 'kdnaforms' ) );
			}
		} else {
			// Open temp file
			$out = @fopen( "{$file_path}.part", $chunk == 0 ? 'wb' : 'ab' );
		...
		if ( ! $chunks || $chunk == $chunks - 1 ) {
			...
			if ( file_exists( $file_path ) ) { // nosemgrep audit.php.lang.security.file.phar-deserialization
				KDNAFormsModel::set_permissions( $file_path );
			} else {
				self::die_error( 105, __( 'Upload unsuccessful', 'kdnaforms' ) . ' ' . $uploaded_filename );
			}
			...
			if ( file_exists( "{$file_path}.part" ) ) {
				KDNAFormsModel::set_permissions( "{$file_path}.part" );
			} else {
				self::die_error( 105, __( 'Upload unsuccessful', 'kdnaforms' ) . ' ' . $uploaded_filename );
			}

			self::send_headers( 200 );
			KDNACommon::log_debug( sprintf( 'KDNAAsyncUpload::upload(): Chunk upload complete. temp_filename: %s  uploaded_filename: %s chunk: %d', $tmp_file_name, $uploaded_filename, $chunk ) );
		}
	...
	public static function die_error( $status_code = 400, $message = '' ) {
		...
		wp_send_json(
			array(
				...
				'error'  => array(
					'code'    => $status_code,
					'message' => $message ?: __( 'Invalid request.', 'kdnaforms' ),
				),
			)
		);
	}
	...
	private static function die_if_nonce_invalid( $form_id, $field_id ) {
		$nonce  = rgar( $_REQUEST, "_kdnaform_file_upload_nonce_{$form_id}_{$field_id}" ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		$action = "kdnaform_file_upload_{$form_id}_{$field_id}";
		...
		if ( empty( $nonce ) ) {
			 ...
			 * @remove-in 3.0
			 */
			$nonce  = rgar( $_REQUEST, "_kdnaform_file_upload_nonce_{$form_id}" ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$action = "kdnaform_file_upload_{$form_id}";
		}

		if ( empty( $nonce ) || ! wp_verify_nonce( $nonce, $action ) ) { // nosemgrep scanner.php.wp.security.csrf.nonce-check-not-dying
			self::die_error( 403, __( 'Your session has expired. Please refresh the page and try again.', 'kdnaforms' ) );
		}
	}
	...
	private static function die_if_extension_disallowed( $file_name, $input_name ) {
		if ( KDNACommon::file_name_has_disallowed_extension( $file_name ) ) {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): Illegal file extension (input: %s): %s', $input_name, $file_name ) );
			self::die_error( 104, __( 'The uploaded file type is not allowed.', 'kdnaforms' ) );
		}
	}
	...
	private static function die_if_extensions_different( $uploaded_filename, $file_name ) {
		...
		if ( $uploaded_filename_ext !== $file_name_ext ) {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): File extensions do not match. uploaded_filename: %s; name: %s', $uploaded_filename_ext, $file_name_ext ) );
			self::die_error( 105, __( 'Upload unsuccessful', 'kdnaforms' ) . ' ' . $uploaded_filename );
		}
	}
	...
	private static function die_if_not_allowed_field_extension( $file_name, $input_name, $extensions ) {
		if ( ! KDNACommon::match_file_extension( $file_name, $extensions ) ) {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): The uploaded file type is not allowed (input: %s): %s', $input_name, $file_name ) );
			// translators: %s: list of allowed extensions.
			self::die_error( 104, sprintf( __( 'The uploaded file type is not allowed. Must be one of the following: %s', 'kdnaforms' ), implode( ', ', $extensions ) ) );
		}
	}
	...
	private static function die_if_invalid_type_and_ext( $file, $file_name, $input_name ) {
		$result = KDNACommon::check_type_and_ext( $file, $file_name );
		if ( is_wp_error( $result ) ) {
			KDNACommon::log_debug( sprintf( '%s(): %s (input: %s); %s; %s', __METHOD__, $file_name, $input_name, $result->get_error_code(), $result->get_error_message() ) );
