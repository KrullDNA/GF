...
namespace KDNA_Forms\KDNA_Forms\Editor_Button;

use KDNA_Forms\KDNA_Forms\Config\KDNA_Config_Service_Provider;
use KDNA_Forms\KDNA_Forms\Editor_Button\Config\KDNA_Editor_Config;
use KDNA_Forms\KDNA_Forms\Editor_Button\Dom\KDNA_Editor_Button;
...
/**
 * Class KDNA_Editor_Service_Provider
 *
 * Service provider for the Embed Form Service.
 *
 * @package KDNA_Forms\KDNA_Forms\Editor_Button;
 */
class KDNA_Editor_Service_Provider extends KDNA_Service_Provider {
	...
	protected $configs = array(
		self::EDITOR_CONFIG => KDNA_Editor_Config::class,
	);
	...
	public function register( KDNA_Service_Container $container ) {
		// Configs
		require_once( plugin_dir_path( __FILE__ ) . '/config/class-kdna-editor-config.php' );

		// Dom
		require_once( plugin_dir_path( __FILE__ ) . '/dom/class-kdna-editor-button.php' );

		// Endpoints
		require_once( plugin_dir_path( __FILE__ ) . '/endpoints/class-kdna-editor-save-editor-settings.php' );

		$container->add( self::ENDPOINTS_CONFIG, function () {
			return new KDNA_Editor_Save_Editor_Settings();
		} );
	...
	public function init( KDNA_Service_Container $container ) {
		add_action( 'kdnaform_after_toolbar_buttons', function () use ( $container ) {
			$container->get( self::DOM_EDITOR_BUTTON )->output_button();
		} );

		add_action( 'wp_ajax_' . KDNA_Editor_Save_Editor_Settings::ACTION_NAME, function () use ( $container ) {
	...
	public static function is_compact_view_enabled( $user_id, $form_id ) {
		$compact_view = get_user_meta( $user_id, 'kdnaform_compact_view_' . $form_id, true );
	...
	public static function is_field_id_enabled( $user_id, $form_id ) {
		$field_id = get_user_meta( $user_id, 'kdnaform_compact_view_show_id_' . $form_id, true );
