...
if ( ! defined( 'KDNAWEBAPI_REQUIRE_SIGNATURE' ) ) {
	define( 'KDNAWEBAPI_REQUIRE_SIGNATURE', true );
}

if ( ! defined( 'KDNAWEBAPI_SLUG' ) ) {
	define( 'KDNAWEBAPI_SLUG', 'kdnaformsapi' );
}

if ( ! defined( 'KDNAWEBAPI_ROUTE_VAR' ) ) {
	define( 'KDNAWEBAPI_ROUTE_VAR', 'kdnaapi_route' );
}

if ( ! defined( 'KDNAWEBAPI_API_BASE_URL' ) ) {
	define( 'KDNAWEBAPI_API_BASE_URL', site_url( KDNAWEBAPI_SLUG ) );
}
...
if ( class_exists( 'KDNAForms' ) ) {
	...
	class KDNAWebAPI extends KDNAAddOn {
		...
		protected $_url = 'https://www.kdnaforms.com';
		protected $_title = 'KDNA Forms REST API';
		...
		protected $_capabilities = array( 'kdnaforms_api', 'kdnaforms_api_settings' );

		// Permissions
		protected $_capabilities_settings_page = 'kdnaforms_api_settings';
		protected $_capabilities_uninstall = 'kdnaforms_webapi_uninstall';
		 ...
		 * @since 2.4.24
		 *
		 * @var null|KDNAWebAPI $_instance If available, contains an instance of this class.
		 */
		...
		public function init() {
			...
			if ( $is_v2_enabled ) {
				...
				if ( is_admin() && $this->current_user_can_any( array( 'manage_options', 'kdnaforms_api_settings' ) ) ) {
					$this->maybe_upgrade_schema();
				}

				if ( ! is_admin() ) {
					require_once( plugin_dir_path( __FILE__ ) . 'v2/class-kdna-rest-authentication.php' );
				}
			}
		 ...
		 * @since 2.4.24
		 *
		 * @param string $db_version          Current KDNA Forms database version.
		 * @param string $previous_db_version Previous KDNA Forms database version.
		...
		public function maybe_upgrade_schema( $force_upgrade = false ) {
			...
			if ( $force_upgrade || $this->requires_schema_upgrade() ) {
				...
				$table_name = KDNAFormsModel::get_rest_api_keys_table_name();
  ...
  KEY consumer_secret (consumer_secret)
				...
				kdna_upgrade()->dbDelta( $table );

				update_option( 'kdna_rest_api_db_version', KDNAForms::$version );
			}
		}
		...
		public function requires_schema_upgrade() {
			...
			$table_exists = $wpdb->query( $wpdb->prepare( 'SHOW TABLES LIKE %s', $wpdb->esc_like( KDNAFormsModel::get_rest_api_keys_table_name() ) ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching

			if ( ! $table_exists ) {
				return true;
			}

			$rest_api_db_version = get_option( 'kdna_rest_api_db_version' );

			$upgrade_required = version_compare( KDNAForms::$version, $rest_api_db_version, '>' );
			...
			if ( $upgrade_required ) {
				...
				$rest_api_db_version = kdna_upgrade()->get_wp_option( 'kdna_rest_api_db_version' );
		...
		public function is_api_enabled() {
			$web_api_settings = get_option( 'kdnaformsaddon_kdnaformswebapi_settings' );
		...
		public function init_admin() {
			...
			if( KDNAForms::get_page() == 'settings' && rgget( 'subview' ) == $this->get_slug() ) {
				require_once( plugin_dir_path( __FILE__ ) . 'includes/class-kdna-api-keys-table.php' );
			}

			// update the results cache meta
			add_action( 'kdnaform_after_update_entry', array( $this, 'entry_updated' ), 10, 2 );
			add_action( 'kdnaform_update_status', array( $this, 'update_entry_status' ), 10, 2 );
			add_action( 'kdnaform_after_save_form', array( $this, 'after_save_form' ), 10, 2 );
		...
		public function init_v1() {
			...
			add_rewrite_rule( KDNAWEBAPI_SLUG . '/(.*)', 'index.php?' . KDNAWEBAPI_ROUTE_VAR . '=$matches[1]', $after = 'top' );

			$rules = get_option( 'rewrite_rules' );
			if ( ! isset( $rules[ KDNAWEBAPI_SLUG . '/(.*)' ] ) ) {
				flush_rewrite_rules();
			}
			...
			// update the cache
			add_action( 'kdnaform_entry_created', array( $this, 'entry_created' ), 10, 2 );
		...
		public function scripts() {
			...
			$scripts = array(
				...
				array(
					...
					'src'     => KDNACommon::get_base_url() . '/includes/webapi/js/hmac-sha1.min.js',
					'enqueue' => array(
						array( 'admin_page' => array( 'plugin_settings' ) ),
					)
				),
				array(
					...
					'src'      => KDNACommon::get_base_url() . '/includes/webapi/js/enc-base64-min.js',
		...
		public function styles() {
			...
			$styles = array(
				array(
					...
					'src'     => KDNACommon::get_base_url() . "/includes/webapi/css/gfwebapi_settings{$min}.css",
		...
		public function plugin_settings_title() {
			return esc_html__( 'KDNA Forms API Settings', 'kdnaforms' );
		}

		public function get_users() {
			$args = apply_filters( 'kdnaform_webapi_get_users_settings_page', array( 'number' => 3000 ) );
		...
		/**
		 * Checks if a user has one or more capabilities to access KDNA Forms REST API endpoints.
		 *
		 * @since 2.4.24
		 *
		...
		private function user_can_access_api( $user ) {
			...
			$capabilities = (array) apply_filters(
				'kdnaform_webapi_key_user_capabilities',
				array(
					'kdnaform_full_access',
		...
		public function plugin_settings_fields() {
			...
			if ( ! $permalink_structure ) {
				return array(
					array(
						'description' => esc_html__( 'The KDNA Forms API allows developers to interact with this install via a JSON REST API.', 'kdnaforms' ),
						'fields'      => array(
							array(
								'name'  => 'requirements_check',
								'label' => esc_html__( 'Requirements check', 'kdnaforms' ),
								'type'  => 'requirements_check',
							),
							array(
								'id'    => 'save_button',
								'type'  => 'save',
								'value' => esc_attr__( 'Update', 'kdnaforms' ),
								'style' => 'display:none;',
							),
						)
					),
			...
			return array(
				array(
					...
					'fields'      => array(
						array(
							'type'       => 'checkbox',
							'label'      => esc_html__( 'Enable access to the API', 'kdnaforms' ),
							'name'       => 'activate',
							'onclick'    => 'jQuery(this).parents("form").submit();',
							'onkeypress' => 'jQuery(this).parents("form").submit();',
							'choices'    => array(
								array( 'label' => esc_html__( 'Enabled', 'kdnaforms' ), 'name' => 'enabled', 'data-js-button' => 'enable-api' ),
							),
						),
					),
				),
				array(
					'title'       => esc_html__( 'Authentication ( API version 2 )', 'kdnaforms' ),
					'id'          => 'kdnaform_section_authentication_v2',
					'description' => sprintf(
					...
					'fields'      => array(
						array(
							'name'  => 'api_keys',
							'type'  => 'api_keys',
							'label' => esc_html__( 'API Keys', 'kdnaforms' ),
						),
					),
				),
				array(
					'title'       => esc_html__( 'Authentication ( API version 1 )', 'kdnaforms' ),
					'id'          => 'kdnaform_section_authentication',
					'description' => sprintf(
					...
					'fields'      => array(
						array(
							'name'              => 'public_key',
							'label'             => esc_html__( 'Public API Key', 'kdnaforms' ),
							'type'              => 'text',
						...
						array(
							'name'              => 'private_key',
							'label'             => esc_html__( 'Private API Key', 'kdnaforms' ),
							'type'              => 'text',
		...
		public function is_v1_enabled( $settings = null ) {
			 ...
			 */
			$is_v1_enabled = apply_filters( 'kdnaform_is_rest_api_v1_enabled', true );
		...
		public function log_debug( $message ) {
			KDNAAPI::log_debug( $message );
		}

		public function query_vars( $query_vars ) {

			$query_vars[] = KDNAWEBAPI_ROUTE_VAR;
		...
		public function handle_page_request() {
			...
			$route = get_query_var( KDNAWEBAPI_ROUTE_VAR );
			if ( false == $route ) {
				return;
			}
			 ...
			 * kdnaform_webapi_authentication_required_[end point]
			 * e.g.
			 * kdnaform_webapi_authentication_required_post_form_submissions
			 *
			 * @param bool $authentication_required Whether authentication is required for this endpoint.
			 */
			$authentication_required = apply_filters( 'kdnaform_webapi_authentication_required_' . $endpoint, $authentication_required );
			...
			if ( empty( $collection2 ) ) {
				do_action( 'kdnaform_webapi_' . $endpoint, $id, $format, $args );
			} else {
				do_action( 'kdnaform_webapi_' . $endpoint, $id, $id2, $format, $args );
			}
		...
		public function authorize( $caps = array() ) {
			...
			if ( KDNACommon::current_user_can_any( $caps ) ) {

				KDNACommon::add_api_call();
		...
		public function ajax_edit_key() {
			...
			if ( rgget( 'key_id' ) ) {
				...
				if ( ! $key ) {
					wp_send_json_error( array( 'message' => __( 'Unable to retrieve key.', 'kdnaforms' ) ) );
				}
				...
				wp_send_json_success(
					array(
						'key' => array(
							...
							'last_access'  => rgobj( $key, 'last_access' ) ? KDNACommon::format_date( $key->last_access ) : __( 'Never Accessed', 'kdnaforms' ),
						),
					)
				);
			...
			if ( isset( $_POST['key_id'] ) ) {
				...
				$key = array(
					...
					'permissions' => KDNACommon::whitelist( rgpost( 'permissions' ), array(
						'read',
						'write',
						'read_write',
					) ),
				...
				if ( rgempty( 'description', $key ) ) {
					wp_send_json_error( array( 'message' => __( 'You must provide a description.', 'kdnaforms' ) ) );
				}
				...
				if ( rgempty( 'consumer_key', $saved ) ) {
					wp_send_json_success( array( 'message' => __( 'API Key successfully updated.', 'kdnaforms' ) ) );
				} else if ( rgar( $saved, 'consumer_key' ) ) {
					wp_send_json_success( array( 'message' => __( 'Make sure you have copied the consumer key and secret below. They will not be available once you leave this page.', 'kdnaforms' ), 'key' => $saved ) );
				} else {
					wp_send_json_error( array( 'message' => __( 'Unable to save API key.', 'kdnaforms' ) ) );
				}

			}

			wp_send_json_error( array( 'message' => __( 'Unable to process request.', 'kdnaforms' ) ) );
		...
		public function update_api_key( $key_id, $key ) {
			...
			if ( $key_id == 0 ) {
				...
				$wpdb->insert( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery
					KDNAFormsModel::get_rest_api_keys_table_name(),
					$key
				);
				...
				$wpdb->update( KDNAFormsModel::get_rest_api_keys_table_name(), $key, array( 'key_id' => $key_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		...
		public function get_feeds( $feed_ids, $form_id = null ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_get_feeds', 'kdnaforms_edit_forms' );
			$this->authorize( $capability );

			$addon_slug = rgget( 'addon' );
			$output     = KDNAAPI::get_feeds( $feed_ids, $form_id, $addon_slug );
		...
		public function delete_feeds( $feed_ids, $form_id = null ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_delete_feeds', 'kdnaforms_edit_forms' );
			$this->authorize( $capability );
			...
			if ( empty( $feed_ids ) ) {
				...
				foreach ( $feeds as $feed ) {
					$result = KDNAAPI::delete_feed( $feed['id'] );
				...
				if ( is_array( $feed_ids ) ) {
					foreach ( $feed_ids as $feed_id ) {
						$result = KDNAAPI::delete_feed( $feed_id );
						if ( is_wp_error( $result ) ) {
							break;
						}
						$count ++;
					...
					$result = KDNAAPI::delete_feed( $feed_ids );
					$count ++;
				}
			}
			...
			if ( isset( $result ) && is_wp_error( $result ) ) {
				...
				$status   = 200;
				$response = sprintf( __( 'Feeds deleted successfully: %d', 'kdnaforms' ), $count );
			}
		...
		public function put_feeds( $feed_data, $feed_id = null ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_put_feeds', 'kdnaforms_edit_forms' );
			$this->authorize( $capability );
			...
			if ( empty( $feed_id ) ) {
				foreach ( $feed_data as $feed ) {
					...
					$result = KDNAAPI::update_feed( $feed['id'], $feed['meta'], $feed['form_id'] );
					if ( is_wp_error( $result ) ) {
						break;
					}
					$count ++;
				...
				$result = KDNAAPI::update_feed( $feed_id, $feed_data['meta'], $feed_data['form_id'] );
				$count ++;
			}
			...
			if ( isset( $results ) && is_wp_error( $result ) ) {
				...
				$status   = 200;
				$response = sprintf( __( 'Feeds updated: %d', 'kdnaforms' ), $count );
			}
		...
		public function post_feeds( $feeds, $form_id = null ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_post_feeds', 'kdnaforms_edit_forms' );
			$this->authorize( $capability );
			...
			foreach ( $feeds as $feed ) {
				...
				if ( empty( $f_id ) ) {
					$result = new WP_Error( 'missing_form_id', __( 'Missing form id', 'kdnaforms' ) );
					break;
				}
				$result = KDNAAPI::add_feed( $f_id, $feed['meta'], $addon_slug );
				if ( is_wp_error( $result ) ) {
					break;
				}
				$feed_ids[] = $result;
		...
		public function submit_form( $data, $id ) {
			...
			add_filter( 'kdnaform_require_login', '__return_false' );

			$result = KDNAAPI::submit_form( $form_id, $data['input_values'], $field_values, $target_page, $source_page );
		...
		public function delete_forms( $form_ids ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_delete_forms', 'kdnaforms_delete_forms' );
			$this->authorize( $capability );

			$count = 0;
			if ( is_array( $form_ids ) ) {
				foreach ( $form_ids as $form_id ) {
					$result = KDNAAPI::delete_form( $form_id );
					if ( is_wp_error( $result ) ) {
						break;
					}
					$count ++;
				...
				$result = KDNAAPI::delete_form( $form_ids );
				$count ++;
			}
			...
			if ( isset( $result ) && is_wp_error( $result ) ) {
				...
				$status   = 200;
				$response = sprintf( __( 'Forms deleted successfully: %d', 'kdnaforms' ), $count );
		...
		public function post_entries( $data, $form_id = null ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_post_entries', 'kdnaforms_edit_entries' );
			$this->authorize( $capability );
			...
			$result = KDNAAPI::add_entries( $entries, $form_id );
		...
		public function put_entries( $data, $entry_id = null ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_put_entries', 'kdnaforms_edit_entries' );
			$this->authorize( $capability );
			$entries = array();
			if ( empty( $entry_id ) ) {
				foreach ( $data as $entry ) {
				...
				$result = KDNAAPI::update_entries( $entries );
			} else {
				$entry  = $this->maybe_serialize_list_fields( $data );
				$result = KDNAAPI::update_entry( $entry, $entry_id );
			}
			...
			if ( is_wp_error( $result ) ) {
				...
				$status   = 200;
				$response = empty( $entry_id ) ? __( 'Entries updated successfully', 'kdnaforms' ) : __( 'Entry updated successfully', 'kdnaforms' );
			}
		...
		public function put_forms_properties( $property_values, $form_id ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_put_forms_properties', 'kdnaforms_edit_forms' );
			$this->authorize( $capability );

			foreach ( $property_values as $key => $property_value ) {
				$result = KDNAAPI::update_form_property( $form_id, $key, $property_value );
				if ( is_wp_error( $result ) ) {
					break;
				}
			}
		...
		public function put_entry_properties( $property_values, $entry_id ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_put_entries_properties', 'kdnaforms_edit_entries' );
			$this->authorize( $capability );

			if ( is_array( $property_values ) ) {
				foreach ( $property_values as $key => $property_value ) {
					$result = KDNAAPI::update_entry_property( $entry_id, $key, $property_value );
					if ( is_wp_error( $result ) ) {
						break;
					}
				}
				...
				if ( empty( $property_values ) ) {
					$response = __( 'No property values were found in the request body', 'kdnaforms' );
				} else {
					$response = __( 'Property values should be sent as an array', 'kdnaforms' );
				}
			}
		...
		public function post_forms( $data ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_post_forms', 'kdnaforms_create_form' );
			$this->authorize( $capability );

			$form_ids = KDNAAPI::add_forms( $data );
		...
		public function put_forms( $data, $form_id = null ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_put_forms', 'kdnaforms_create_form' );
			$this->authorize( $capability );

			if ( empty( $form_id ) ) {
				$result = KDNAAPI::update_forms( $data );
			} else {
				$result = KDNAAPI::update_form( $data, $form_id );
			}
			...
			if ( is_wp_error( $result ) ) {
				...
				$status   = 200;
				$response = empty( $form_id ) ? __( 'Forms updated successfully', 'kdnaforms' ) : __( 'Form updated successfully', 'kdnaforms' );
			}
		...
		public function delete_entries( $entry_ids ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_delete_entries', 'kdnaforms_delete_entries' );
			$this->authorize( $capability );
			...
			if ( is_array( $entry_ids ) ) {
				foreach ( $entry_ids as $entry_id ) {
					...
					$result = KDNAAPI::delete_entry( $entry_id );
					if ( is_wp_error( $result ) ) {
						break;
					}
					$count ++;
				...
				$result = KDNAAPI::delete_entry( $entry_ids );
				$count ++;
			}
			...
			if ( isset( $result ) && is_wp_error( $result ) ) {
				...
				$status   = 200;
				$response = sprintf( __( 'Entries deleted successfully: %d', 'kdnaforms' ), $count );
			}
		...
		public function get_entries( $entry_ids, $form_ids = null, $schema = '', $field_ids = array() ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_get_entries', 'kdnaforms_view_entries' );
			$this->authorize( $capability );
			...
			if ( $entry_ids ) {
				...
				if ( is_array( $entry_ids ) ) {
					foreach ( $entry_ids as $entry_id ) {
						$result = KDNAAPI::get_entry( $entry_id );
						if ( ! is_wp_error( $result ) ) {
					...
					$result = KDNAAPI::get_entry( $entry_ids );
					if ( ! is_wp_error( $result ) ) {
						$result   = $this->maybe_json_encode_list_fields( $result );
						$response = $result;
				...
				$entry_count = KDNAAPI::count_entries( $form_ids, $search );

				$result = $entry_count > 0 ? KDNAAPI::get_entries( $form_ids, $search, $sorting, $paging ) : array();
		...
		public function get_forms( $form_ids = null, $schema = '' ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_get_forms', 'kdnaforms_edit_forms' );
			$this->authorize( $capability );

			$status   = 200;
			$response = array();
			if ( empty( $form_ids ) ) {
				$forms = KDNAFormsModel::get_forms( true );
				foreach ( $forms as $form ) {
					...
					if ( $include_totals ) {
						$totals               = KDNAFormsModel::get_form_counts( $form_id );
				...
				if ( is_array( $form_ids ) ) {
					foreach ( $form_ids as $form_id ) {
						$response[ $form_id ] = KDNAAPI::get_form( $form_id );
					}
				} else {
					$result = KDNAAPI::get_form( $form_ids );
					if ( is_wp_error( $result ) ) {
						$response = $this->get_error_response( $result );
						$status   = $this->get_error_status( $result );
					} elseif ( ! $result ) {
		...
		// If the results can be calculated within the time allowed in KDNAResults then the results are returned and nothing is cached.
		...
		public function get_results( $form_id ) {
			 ...
			 * @since 1.9.2
			 */
			$capability = apply_filters( 'kdnaform_web_api_capability_get_results', 'kdnaforms_view_entries' );
			$this->authorize( $capability );
		...
		public function get_results_cron_hook() {
			return 'kdnaforms_results_cron_' . $this->get_slug();
		}
			...
			foreach ( $fields as $field_id => $choice_counts ) {
				...
				if ( is_array( $choice_counts ) ) {
					...
					foreach ( $choice_counts as $choice_value => $choice_count ) {
						if ( class_exists( 'KDNASurvey' ) && 'likert' == $type && rgar( $field, 'gsurveyLikertEnableMultipleRows' ) ) {
							$row_text       = KDNASurvey::get_likert_row_text( $field, $i ++ );
							$counts_for_row = array();
							foreach ( $choice_count as $col_val => $col_count ) {
								$text                       = KDNASurvey::get_likert_column_text( $field, $choice_value . ':' . $col_val );
							...
							$text                                 = KDNAFormsModel::get_choice_text( $field, $choice_value );
							$fields[ $field_id ][ $choice_value ] = array(
								'text'  => $text,
								'value' => "$choice_value",
								'count' => $choice_count
		...
		private function check_signature() {
			if ( false === KDNAWEBAPI_REQUIRE_SIGNATURE ) {
				return true;
			}
			...
			$path    = strtolower( get_query_var( KDNAWEBAPI_ROUTE_VAR ) );
		...
		public function die_not_authorized() {
			$this->log_debug( __METHOD__ . '(): Running.' );
			$this->end( 401, __( 'Not authorized', 'kdnaforms' ) );
		}

		public function die_permission_denied() {
			$this->log_debug( __METHOD__ . '(): Running.' );
			$this->end( 401, __( 'Permission denied', 'kdnaforms' ) );
		}

		public function die_forbidden() {
			$this->log_debug( __METHOD__ . '(): Running.' );
			$this->end( 403, __( 'Forbidden', 'kdnaforms' ) );
		}

		public function die_bad_request() {
			$this->log_debug( __METHOD__ . '(): Running.' );
			$this->end( 400, __( 'Bad request', 'kdnaforms' ) );
		}

		public function die_not_found() {
			$this->log_debug( __METHOD__ . '(): Running.' );
			$this->end( 404, __( 'Not found', 'kdnaforms' ) );
		}

		public function die_not_implemented() {
			$this->log_debug( __METHOD__ . '(): Running.' );
			$this->end( 501, __( 'Not implemented', 'kdnaforms' ) );
		}

		public function die_error() {
			$this->log_debug( __METHOD__ . '(): Running.' );
			$this->end( 500, __( 'Internal Error', 'kdnaforms' ) );
		}
		...
		public static function get_form_metas() {
			...
			$form_metas = KDNAFormsModel::get_form_meta_by_id( $form_ids );
		...
		public function mtd_transform_entries_data( $output, $form_id ) {
			...
			if ( is_array( $entries ) ) {
				foreach ( $entries as $entry ) {
					...
					$entry_element['url']   = KDNAWEBAPI_API_BASE_URL . '/entries/' . rgar( $entry, 'id' ) . '?schema=mtd';
		...
		public function mtd_transform_forms_data( $forms ) {
			...
			foreach ( $forms as $form ) {
				...
				$element['url']        = KDNAWEBAPI_API_BASE_URL . '/forms/' . $form['id'] . '/entries.json?schema=mtd';
				$section               = array();
		...
		public static function mtd_transform_entry_data( $entry ) {
			...
			foreach ( $fields as $field ) {
				...
				if ( is_array( $value ) && isset( $field->choices ) ) {
					...
					$element            = array();
					$element['type']    = 'string';
					$element['caption'] = KDNAFormsModel::get_choice_text( $field, $value );
