...
class KDNA_REST_Notes_Controller extends KDNA_REST_Entry_Notes_Controller {
	...
	public function get_items( $request ) {
		...
		$notes = KDNAAPI::get_notes( $search_criteria, $sorting );
	...
	public function update_item( $request ) {
		...
		$result = KDNAAPI::update_note( $note, $request->get_param( 'note_id' ) );
		...
		$updated_note = KDNAAPI::get_note( $note['id'] );
	...
	public function delete_item( $request ) {
		...
		$note = KDNAAPI::get_note( $note_id );
		if ( is_wp_error( $note ) ) {
			return new WP_Error( 'kdna_entry_invalid_id', __( 'Invalid note id.', 'kdnaforms' ), array( 'status' => 404 ) );
		}

		$result = KDNAAPI::delete_note( $note_id );
