...
class KDNA_REST_Entry_Notes_Controller extends KDNA_REST_Controller {
	...
	public function get_items( $request ) {
		...
		if ( ! KDNAAPI::entry_exists( $entry_id ) ) {
			return new WP_Error( 'kdna_entry_invalid_id', __( 'Invalid entry id.', 'kdnaforms' ), array( 'status' => 404 ) );
		}
		...
		$notes = KDNAAPI::get_notes( $criteria, $sorting );

		if ( is_wp_error( $notes ) ) {
			return new WP_Error( 'kdna_entry_invalid_notes', __( 'Error retrieving notes.', 'kdnaforms' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		$note_id = KDNAAPI::add_note( $entry_id, $note['user_id'], $note['user_name'], $note['note'] );
	...
	public function get_items_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_get_notes', 'kdnaforms_view_entry_notes', $request );
	...
	public function create_item_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_post_notes', 'kdnaforms_edit_entry_notes', $request );
	...
	public function update_item_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_put_notes', 'kdnaforms_edit_entries', $request );
	...
	public function delete_item_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_delete_notes', 'kdnaforms_edit_entry_notes', $request );
