...
class KDNA_REST_Entry_Properties_Controller extends KDNA_REST_Form_Entries_Controller {
	...
	public function update_items( $request ) {
		...
		if ( empty( $key_value_pairs ) ) {
			$message = __( 'No property values were found in the request body', 'kdnaforms' );
			return new WP_REST_Response( $message, 400 );
		} elseif ( ! is_array( $key_value_pairs ) ) {
			$message = __( 'Property values should be sent as an array', 'kdnaforms' );
			return new WP_REST_Response( $message, 400 );
		}
		...
		$message = __( 'Entry updated successfully', 'kdnaforms' );
	...
	protected function prepare_item_for_database( $request ) {
		...
		if ( empty( $properties ) ) {
			return new WP_Error( 'missing_properties', __( 'Missing Key Value Pairs JSON', 'kdnaforms' ) );
		}
