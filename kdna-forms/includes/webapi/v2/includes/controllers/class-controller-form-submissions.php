...
class KDNA_REST_Form_Submissions_Controller extends KDNA_REST_Controller {
	...
	public function create_item( $request ) {
		if ( rgar( $request->get_query_params(), '_validate_only' ) ) {
			return ( new KDNA_REST_Form_Submissions_Validation_Controller() )->validate_form( $request );
		}
		...
		$result = KDNAAPI::submit_form( $form_id, $input_values, $field_values, $target_page, $source_page );
		...
		if ( ! current_user_can( 'kdnaforms_view_entries' ) && ! current_user_can( 'kdnaforms_edit_entries' ) ) {
		...
		$schema = array(
			...
			'properties' => array(
				'input_[Field ID]' => array(
					'description' => __( 'The input values.', 'kdnaforms' ),
					'type'        => 'string',
				),
				'field_values' => array(
					'description' => __( 'The field values.', 'kdnaforms' ),
