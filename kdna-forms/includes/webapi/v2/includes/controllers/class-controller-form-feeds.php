...
class KDNA_REST_Form_Feeds_Controller extends KDNA_REST_Controller {
	...
	public function get_items( $request ) {
		...
		$feeds = KDNAAPI::get_feeds( $feed_ids, $form_id, $addon_slug );
	...
	public function create_item( $request ) {
		...
		$feed_id = KDNAAPI::add_feed( $form_id, $feed['meta'], $feed['addon_slug'] );
	...
	protected function prepare_item_for_database( $request ) {
		...
		if ( empty( $feed ) ) {
			return new WP_Error( 'missing_feed', __( 'Missing feed JSON', 'kdnaforms' ), array( 'status' => 400 ) );
		}
		...
		if ( isset( $form_id ) ) {
			...
			return new WP_Error( 'missing_form_id', __( 'Missing form id', 'kdnaforms' ), array( 'status' => 400 ) );
		}

		$addon_slug = isset( $feed['addon_slug'] ) ? $feed['addon_slug'] : $request['addon'];
		if ( empty( $addon_slug ) ) {
			return new WP_Error( 'missing_addon_slug', __( 'Missing add-on slug', 'kdnaforms' ), array( 'status' => 400 ) );
		}


		if ( empty( $feed['meta'] ) ) {
			return new WP_Error( 'missing_feed_meta', __( 'Missing feed meta', 'kdnaforms' ), array( 'status' => 400 ) );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'id' => array(
					'description' => __( 'Unique identifier for the feed.', 'kdnaforms' ),
					'type'        => 'integer',
					'readonly'    => true,
				),
				'form_id' => array(
					'description' => __( 'The Form ID for the feed.', 'kdnaforms' ),
					'type'        => 'integer',
					'required'    => true,
					'readonly'    => true,
				),
				'meta' => array(
					'description' => __( 'The JSON string containing the feed meta.', 'kdnaforms' ),
					'type'        => 'object',
					'readonly'    => false,
				),
				'addon_slug' => array(
					'description' => __( 'The add-on the feed belongs to.', 'kdnaforms' ),
					'type'        => 'integer',
					'readonly'    => true,
				),
			),
