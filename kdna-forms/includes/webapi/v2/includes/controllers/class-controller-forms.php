...
class KDNA_REST_Forms_Controller extends KDNA_REST_Controller {
	...
	public function update_item( $request ) {
		...
		$result = KDNAAPI::update_form( $form, $form_id );
	...
	public function delete_item( $request ) {
		...
		$form = KDNAAPI::get_form( $form_id );
		if ( empty( $form ) ) {
			return new WP_Error( 'kdna_form_invalid_id', __( 'Invalid form id.', 'kdnaforms' ), array( 'status' => 404 ) );
		}
		...
		if ( $force ) {
			...
			// Trash the form
			KDNAAPI::update_form_property( $form_id, 'is_trash', 1 );
	...
	public function get_items_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_get_forms', 'kdnaforms_edit_forms', $request );
	...
	public function create_item_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_post_forms', 'kdnaforms_create_form', $request );
		return $this->current_user_can_any( $capability, $request );
	}
	...
	public function update_item_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_put_forms', 'kdnaforms_create_form', $request );
		return $this->current_user_can_any( $capability, $request );
	}
	...
	public function delete_item_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_delete_forms', 'kdnaforms_delete_forms', $request );
		return $this->current_user_can_any( $capability, $request );
	}
	...
	protected function prepare_item_for_database( $request ) {
		...
		if ( ! $form_json ) {
			...
			if ( empty( $form_json ) || is_array( $form_json ) ) {
				return new WP_Error( 'missing_form', __( 'The Form object must be sent as a JSON string in the request body with the content-type header set to application/json.', 'kdnaforms' ) );
			}
		}
		...
		$form = KDNAFormsModel::sanitize_settings( $form );
