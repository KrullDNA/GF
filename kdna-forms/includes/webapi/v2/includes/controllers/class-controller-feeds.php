...
class KDNA_REST_Feeds_Controller extends KDNA_REST_Form_Feeds_Controller {
	...
	public function get_items( $request ) {
		...
		$feeds = KDNAAPI::get_feeds( $feed_ids, null, $addon_slug );
	...
	public function get_item( $request ) {
		...
		if ( is_wp_error( $feed ) ) {
			return new WP_Error( 'kdna_feed_invalid_id', __( 'Invalid feed id.', 'kdnaforms' ), array( 'status' => 404 ) );
		}
	...
	public function update_item( $request ) {
		if ( ! KDNAAPI::feed_exists( $request['feed_id'] ) ) {
			return new WP_Error( 'kdna_feed_invalid_id', __( 'Invalid feed id.', 'kdnaforms' ), array( 'status' => 404 ) );
		}
		...
		return $this->prepare_item_for_response( KDNAAPI::get_feed( $request['feed_id'] ), $request );
	}
	...
	protected function prepare_item_for_database( $request ) {
		...
		if ( empty( $properties ) ) {
			return new WP_Error( 'missing_properties', __( 'Invalid JSON. Properties should be sent as key value pairs.', 'kdnaforms' ), array( 'status' => 400 ) );
		}

		if ( ! empty( $properties['meta'] ) ) {
			$feed               = KDNAAPI::get_feed( $request['feed_id'] );
	...
	protected function update_feed_properties( $feed_id, $properties ) {
		foreach ( $properties as $key => $value ) {
			$result = KDNAAPI::update_feed_property( $feed_id, $key, $value );
			if ( is_wp_error( $result ) ) {
				return new WP_Error(
	...
	public function get_items_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_get_feeds', 'kdnaforms_edit_forms', $request );
	...
	public function create_item_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_post_feeds', 'kdnaforms_edit_forms', $request );
	...
	public function update_item_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_put_feeds', 'kdnaforms_edit_forms', $request );
	...
	public function delete_item_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_delete_feeds', 'kdnaforms_edit_forms', $request );
