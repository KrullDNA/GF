...
class KDNA_REST_Form_Field_Filters_Controller extends KDNA_REST_Controller {
	...
	public function get_items( $request ) {
		...
		if ( ! empty( $request['_admin_labels'] ) ) {
			/** @var KDNA_Field $field The field object. */
			foreach ( $form['fields'] as $field ) {
				$field->set_context_property( 'use_admin_label', true );
			}
		}

		return new WP_REST_Response( KDNACommon::get_field_filter_settings( $form ) );
	}
	...
	public function get_items_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_get_field_filters', 'kdnaforms_view_entries', $request );
