...
class KDNA_REST_Form_Entries_Controller extends KDNA_REST_Controller {
	...
	public function get_items( $request ) {
		...
		if ( $entry_ids ) {
			...
			$entries = KDNAAPI::get_entries( $form_id, $entry_search_params['search_criteria'], $entry_search_params['sorting'], $entry_search_params['paging'], $entry_count );
			...
			if ( ! is_wp_error( $entries ) ) {
				foreach ( $entries as &$entry ) {
					...
					if ( $labels && ( empty( $form_id ) || is_array( $form_id ) ) ) {
						$form             = KDNAAPI::get_form( $form_id_for_entry );
						$entry['_labels'] = $this->get_entry_labels( $form, compact( 'field_ids' ) );
					}
				}
	...
	public function create_item( $request ) {
		...
		$entry_id = KDNAAPI::add_entry( $entry );
	...
	public function get_items_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_get_entries', 'kdnaforms_view_entries', $request );
	...
	public function create_item_permissions_check( $request ) {
		 ...
		 * @param WP_REST_Request $request    Full data about the request.
		 */
		$capability = apply_filters( 'kdnaform_rest_api_capability_post_entries', 'kdnaforms_edit_entries', $request );
	...
	protected function prepare_item_for_database( $request ) {
		...
		if ( empty( $entry ) ) {
			return new WP_Error( 'missing_entry', __( 'Missing entry JSON', 'kdnaforms' ) );
		}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'id' => array(
					'description' => __( 'Unique identifier for the resource.', 'kdnaforms' ),
					'type'        => 'integer',
					'readonly'    => true,
				),
				'form_id' => array(
					'description' => __( 'The Form ID for the entry.', 'kdnaforms' ),
					'type'        => 'integer',
					'required'    => true,
					'readonly'    => false,
				),
				'date_created' => array(
					'description' => __( 'The date the entry was created, in UTC.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
				'date_updated' => array(
					'description' => __( 'The date the entry was updated, in UTC.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
				'is_starred' => array(
					'description' => __( 'Whether the entry is starred.', 'kdnaforms' ),
					'type'        => 'integer',
					'readonly'    => false,
				),
				'is_read' => array(
					'description' => __( 'Whether the entry has been read.', 'kdnaforms' ),
					'type'        => 'integer',
					'readonly'    => false,
				),
				'ip' => array(
					'description' => __( 'The IP address of the entry creator.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
				'source_url' => array(
					'description' => __( 'The URL where the form was embedded.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
				'user_agent' => array(
					'description' => __( 'The user agent string for the browser used to submit the entry.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
				'payment_status' => array(
					'description' => __( 'The status of the payment, if applicable.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
				'payment_date' => array(
					'description' => __( 'The date of the payment, if applicable.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
				'payment_amount' => array(
					'description' => __( 'The amount of the payment, if applicable.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
				'payment_method' => array(
					'description' => __( 'The payment method for the payment, if applicable.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
				'transaction_id' => array(
					'description' => __( 'The transaction ID for the payment, if applicable.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
				'is_fulfilled' => array(
					'description' => __( 'Whether the transaction has been fulfilled, if applicable.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
				'created_by' => array(
					'description' => __( 'The user ID of the entry submitter.', 'kdnaforms' ),
					'type'        => 'integer',
					'readonly'    => false,
				),
				'transaction_type' => array(
					'description' => __( 'The type of the transaction, if applicable.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
				'status' => array(
					'description' => __( 'The status of the entry.', 'kdnaforms' ),
					'type'        => 'string',
					'readonly'    => false,
				),
			),
