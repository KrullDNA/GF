...
class KDNA_Results_Cache {
	...
	public function __construct() {
		if ( defined( 'DOING_CRON' ) && DOING_CRON ) {
			add_action( 'kdnaforms_results_cron', array( $this, 'results_cron' ), 10, 4 );
			return;
		}
	...
	/**
	 * Callback for the kdnaform_update_status action.
	 *
	 * @since 2.4-beta-1
	 *
	 * @param $entry_id
	...
	/**
	 * Callback for the kdnaform_after_update_entry action.
	 *
	 * @since 2.4-beta-1
	 *
	 * @param $form
	...
	/**
	 * Callback for the kdnaform_entry_created action.
	 *
	 * @since 2.4-beta-1
	 *
	 * @param $entry
	...
	/**
	 * Callback for the kdnaform_after_save_form action.
	 *
	 * @since 2.4-beta-1
	 *
	 * @param $form
	...
	public function calculate( $form, $search_criteria = array(), $state_array = array(), $args = array() ) {
		...
		$count_search_entries = KDNAAPI::count_entries( $form_id, $search_criteria );
		...
		while ( $entries_left > 0 ) {
			...
			$entries                   = KDNAAPI::get_entries( $form_id, $search_criteria, null, $paging );
				...
				foreach ( $fields as $field ) {
					...
					if ( $field_type == 'likert' && rgar( $field, 'gsurveyLikertEnableMultipleRows' ) ) {
						...
						if ( empty( $field->choices ) ) {
							if ( ( ! is_array( $value ) && ! empty( $value ) ) || ( is_array( $value ) && ! KDNACommon::is_empty_array( $value ) ) ) {
								$field_data[ $field_id ] ++;
							}
							continue;
						}
						...
						foreach ( $choices as $choice ) {
							...
							if ( is_array( $value ) ) {
								...
								if ( KDNAFormsModel::choice_value_match( $field, $choice, $value ) ) {
									$choice_is_selected = true;
								}
							}
	...
	private function get_likert_row_score( $row_val, $field, $entry ) {
		...
		) ) ? KDNASurvey::get_likert_row_score( $row_val, $field, $entry ) : 0;
	}
	...
	private function get_likert_score( $field, $entry ) {
		return is_callable( array(
			'KDNASurvey',
			'get_field_score',
		) ) ? KDNASurvey::get_field_score( $field, $entry ) : 0;
	}
	...
	private function get_labels( $form, $args ) {
		...
		foreach ( $fields as $field ) {
			...
			if ( is_array( $field->choices ) ) {
				...
				if ( $field instanceof KDNA_Field_Likert && $field->gsurveyLikertEnableMultipleRows ) {
