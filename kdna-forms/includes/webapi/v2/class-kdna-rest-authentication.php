 ...
 * @since 2.4-beta-1
 */
class KDNA_REST_Authentication {
	...
	public function override_rest_authentication_errors( $errors ) {
		_deprecated_function( __METHOD__, '2.4.22', 'KDNA_REST_Authentication::check_authentication_error' );
	...
	/**
	 * Check if is request to KDNA Forms REST API.
	 *
	 * @since 2.4-beta-1
	 *
	 * @return bool Returns true if this is a request to the KDNA Forms REST API. False otherwise
	 */
	protected function is_request_to_rest_api() {
		...
		/**
		 * Allows filtering of whether or not the current request is a request to the KDNA Forms REST API.
		 *
	 ...
	 *
	 * Necessary in cases where wp_get_current_user() is called before KDNA Forms is loaded.
	 *
	 * @since 2.4.22
	 *
	...
	private function perform_application_password_authentication() {
		...
		if ( empty( $user_id ) ) {
			...
			if ( is_wp_error( $wp_rest_application_password_status ) ) {
				$this->set_error( new WP_Error( 'kdnaform_rest_authentication_error', $wp_rest_application_password_status->get_error_message(), array( 'status' => 401 ) ) );
			}
	...
	private function perform_basic_authentication() {
		...
		if ( ! hash_equals( $user->consumer_secret, $consumer_secret ) ) {
			$this->set_error( new WP_Error( 'kdnaform_rest_authentication_error', __( 'Consumer secret is invalid.', 'kdnaforms' ), array( 'status' => 401 ) ) );
	...
	public function get_oauth_parameters() {
		...
		if ( ! empty( $errors ) ) {
			$message = sprintf(
				...
				_n( 'Missing OAuth parameter %s', 'Missing OAuth parameters %s', count( $errors ), 'kdnaforms' ),
				implode( ', ', $errors )
			);

			$this->set_error( new WP_Error( 'kdnaform_rest_authentication_missing_parameter', $message, array( 'status' => 401 ) ) );
	...
	private function perform_oauth_authentication() {
		...
		if ( empty( $user ) ) {
			$this->set_error( new WP_Error( 'kdnaform_rest_authentication_error', __( 'Consumer key is invalid.', 'kdnaforms' ), array( 'status' => 401 ) ) );
	...
	private function check_oauth_signature( $user, $params ) {
		...
		if ( ! uksort( $params, 'strcmp' ) ) {
			return new WP_Error( 'kdnaform_rest_authentication_error', __( 'Invalid signature - failed to sort parameters.', 'kdnaforms' ), array( 'status' => 401 ) );
		}
		...
		if ( 'HMAC-SHA1' !== $params['oauth_signature_method'] && 'HMAC-SHA256' !== $params['oauth_signature_method'] ) {
			return new WP_Error( 'kdnaform_rest_authentication_error', __( 'Invalid signature - signature method is invalid.', 'kdnaforms' ), array( 'status' => 401 ) );
		}
		...
		if ( ! hash_equals( $signature, $consumer_signature ) ) {
			...
			return new WP_Error( 'kdnaform_rest_authentication_error', __( 'Invalid signature - provided signature does not match.', 'kdnaforms' ), array( 'status' => 401 ) );
		}
	...
	private function check_oauth_timestamp_and_nonce( $user, $timestamp, $nonce ) {
		...
		if ( ( $timestamp < time() - $valid_window ) || ( $timestamp > time() + $valid_window ) ) {
			return new WP_Error( 'kdnaform_rest_authentication_error', __( 'Invalid timestamp.', 'kdnaforms' ), array( 'status' => 401 ) );
		}
		...
		if ( in_array( $nonce, $used_nonces ) ) {
			return new WP_Error( 'kdnaform_rest_authentication_error', __( 'Invalid nonce - nonce has already been used.', 'kdnaforms' ), array( 'status' => 401 ) );
		}
	...
	private function get_user_data_by_consumer_key( $consumer_key ) {
		...
		$consumer_key = KDNAWebAPI::api_hash( sanitize_text_field( $consumer_key ) );
		...
		switch ( $method ) {
			...
			case 'GET':
				if ( 'read' !== $permissions && 'read_write' !== $permissions ) {
					return new WP_Error( 'kdnaform_rest_authentication_error', __( 'The API key provided does not have read permissions.', 'kdnaforms' ), array( 'status' => 401 ) );
				}
				break;
			case 'POST':
			case 'PUT':
			...
			case 'DELETE':
				if ( 'write' !== $permissions && 'read_write' !== $permissions ) {
					return new WP_Error( 'kdnaform_rest_authentication_error', __( 'The API key provided does not have write permissions.', 'kdnaforms' ), array( 'status' => 401 ) );
				}
				break;
			case 'OPTIONS':
				return true;
			...
			default:
				return new WP_Error( 'kdnaform_rest_authentication_error', __( 'Unknown request method.', 'kdnaforms' ), array( 'status' => 401 ) );
		}
	...
	public function send_unauthorized_headers( $response ) {
		if ( is_wp_error( $this->get_error() ) && 'basic_auth' === $this->auth_method ) {
			$auth_message = __( 'KDNA Forms API. Use a consumer key in the username field and a consumer secret in the password field.', 'kdnaforms' );
	...
	/**
	 * Determines if the request is authenticated using credentials generated by KDNA Forms.
	 *
	 * @since 2.4.22
	 *
	 * @return bool
