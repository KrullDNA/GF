...
class KDNA_API_Keys_Table extends WP_List_Table {
	...
	function get_columns() {
		...
		return array(
			'description' => esc_html__( 'Description', 'kdnaforms' ),
			'user'        => esc_html__( 'User', 'kdnaforms' ),
			'permissions' => esc_html__( 'Permissions', 'kdnaforms' ),
			'last_access' => esc_html__( 'Last Access', 'kdnaforms' ),
		);
	}
	...
	function prepare_items() {
		...
		$this->items = KDNAWebAPI::get_api_keys();
	}
	...
	function column_description( $item ) {
		...
		$nonce_url = wp_nonce_url( '?page=gf_settings&subview=kdnaformswebapi', 'gf_revoke_key' );
	...
	function get_edit_url( $key_id ) {
		return sprintf( '?page=gf_settings&subview=kdnaformswebapi&action=edit&key_id=%s', absint( $key_id ) );
	}

	function column_last_access( $item ) {
		return empty( $item['last_access'] ) ? __( 'Never Accessed', 'kdnaforms' ) : KDNACommon::format_date( $item['last_access'], true, '', true );
	}
	...
	function no_items() {
		echo '<div style="padding:10px;">' . esc_html__( 'You don\'t have any API keys. Let\'s add one by clicking the Add Key button below.', 'kdnaforms' ) . '</div>';
	}
	...
	public function output_scripts() {
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function () {
				...
				const noItemsHTML = <?php echo json_encode('<tr class="no-items alternate"><td class="colspanchange" colspan="4"><div style="padding:10px;">' . esc_html__( "You don't have any API keys. Let's add one by clicking the Add Key button below.", "kdnaforms" ) . '</div></td></tr>'); ?>;
