...
abstract class KDNALocking {
	...
	public function register_scripts() {
		...
		$locking_path = KDNACommon::get_base_url() . '/includes/locking/';
		wp_register_script( 'gforms_locking', $locking_path . "js/locking{$min}.js", array( 'jquery', 'heartbeat' ), KDNACommon::$version );
		wp_register_script( 'gforms_locking_view', $locking_path . "js/locking-view{$min}.js", array( 'jquery', 'heartbeat' ), KDNACommon::$version );
		wp_register_script( 'gforms_locking_list', $locking_path . "js/locking-list{$min}.js", array( 'jquery', 'heartbeat' ), KDNACommon::$version );
		wp_register_style( 'gforms_locking_css', $locking_path . "css/locking{$min}.css", array(), KDNACommon::$version );
		wp_register_style( 'gforms_locking_list_css', $locking_path . "css/locking-list{$min}.css", array(), KDNACommon::$version );
	...
	protected function get_strings() {
		$strings = array(
			'currently_locked'  => wp_strip_all_tags( __( 'This page is currently locked. Click on the "Request Control" button to let %s know you\'d like to take over.', 'kdnaforms' ) ),
			'accept'            => wp_strip_all_tags( __( 'Accept', 'kdnaforms' ) ),
			'cancel'            => wp_strip_all_tags( __( 'Cancel', 'kdnaforms' ) ),
			'currently_editing' => wp_strip_all_tags( __( '%s is currently editing', 'kdnaforms' ) ),
			'taken_over'        => wp_strip_all_tags( __( '%s has taken over and is currently editing.', 'kdnaforms' ) ),
			'lock_requested'    => wp_strip_all_tags( __( '%s has requested permission to take over control.', 'kdnaforms' ) ),
			'gained_control'    => wp_strip_all_tags( __( 'You now have control', 'kdnaforms' ) ),
			'request_pending'   => wp_strip_all_tags( __( 'Pending', 'kdnaforms' ) ),
			'no_response'       => wp_strip_all_tags( __( 'No response', 'kdnaforms' ) ),
			'request_again'     => wp_strip_all_tags( __( 'Request again', 'kdnaforms' ) ),
			'request_error'     => wp_strip_all_tags( __( 'Error', 'kdnaforms' ) ),
			'request_rejected'  => wp_strip_all_tags( __( 'Your request was rejected', 'kdnaforms' ) ),
		);
	...
	protected function set_lock( $object_id ) {
		if ( ! KDNACommon::current_user_can_any( $this->_capabilities ) ) {
			return false;
		}
	...
	protected function request_lock( $object_id ) {
		...
		if ( ! $lock_holder_user_id ) {
			...
			$result['html']   = __( 'You now have control', 'kdnaforms' );
			$result['status'] = 'lock_obtained';
		} else {
			$user = get_userdata( $lock_holder_user_id );
			$this->update_lock_request_meta( $object_id, $user_id );
			$result['html']   = sprintf( __( 'Your request has been sent to %s.', 'kdnaforms' ), $user->display_name );
	...
	protected function get_lock_request_meta( $object_id ) {
		return KDNACache::get( self::PREFIX_EDIT_LOCK_REQUEST . $this->_object_type . '_' . $object_id );
	}

	protected function get_lock_meta( $object_id ) {
		return KDNACache::get( self::PREFIX_EDIT_LOCK . $this->_object_type . '_' . $object_id );
	}

	protected function update_lock_meta( $object_id, $lock_value ) {
		KDNACache::set( self::PREFIX_EDIT_LOCK . $this->_object_type . '_' . $object_id, $lock_value, true, 150 );
	}

	protected function update_lock_request_meta( $object_id, $lock_request_value ) {
		KDNACache::set( self::PREFIX_EDIT_LOCK_REQUEST . $this->_object_type . '_' . $object_id, $lock_request_value, true, 120 );
	}

	protected function delete_lock_request_meta( $object_id ) {
		KDNACache::delete( self::PREFIX_EDIT_LOCK_REQUEST . $this->_object_type . '_' . $object_id );

		return true;
	}

	protected function delete_lock_meta( $object_id ) {
		KDNACache::delete( self::PREFIX_EDIT_LOCK . $this->_object_type . '_' . $object_id );
	...
	public function heartbeat_check_locked_objects( $response, $data, $screen_id ) {
		$checked       = array();
		$heartbeat_key = 'kdnaform-check-locked-objects-' . $this->_object_type;
	...
	public function heartbeat_refresh_lock( $response, $data, $screen_id ) {
		$heartbeat_key = 'kdnaform-refresh-lock-' . $this->_object_type;
	...
	public function heartbeat_request_lock( $response, $data, $screen_id ) {
		$heartbeat_key = 'kdnaform-request-lock-' . $this->_object_type;
		if ( array_key_exists( $heartbeat_key, $data ) ) {
			$received = $data[ $heartbeat_key ];
			$send     = array();
	...
	public function heartbeat_refresh_nonces( $response, $data, $screen_id ) {
		if ( array_key_exists( 'kdnaform-refresh-nonces', $data ) ) {
			$received                         = $data['kdnaform-refresh-nonces'];
			$response['kdnaform-refresh-nonces'] = array( 'check' => 1 );
			...
			if ( ! KDNACommon::current_user_can_any( $this->_capabilities ) || empty( $received['post_nonce'] ) ) {
				return $response;
			}

			if ( 2 === wp_verify_nonce( $received['object_nonce'], 'update-contact_' . $object_id ) ) {
				$response['kdnaform-refresh-nonces'] = array(
					'replace'        => array(
	...
	public function get_lock_ui( $user_id ) {
		...
		if ( $locked ) {
			...
			$message = '<div class="kdnaform-locked-message">
                            <div class="kdnaform-locked-avatar">' . get_avatar( $user->ID, 64 ) . '</div>
                            <p class="currently-editing" tabindex="0">' . sprintf( $this->get_string( 'currently_locked' ), $user->display_name ) . '</p>
                            <p>

                                <a id="kdnaform-take-over-button" style="display:none" class="button button-primary wp-tab-first" href="' . esc_url( add_query_arg( 'get-edit-lock', '1', $edit_url ) ) . '">' . __( 'Take Over', 'kdnaforms' ) . '</a>
                                <button id="kdnaform-lock-request-button" class="button button-primary wp-tab-last">' . __( 'Request Control', 'kdnaforms' ) . '</button>
                                <a class="button" href="' . esc_url( $this->_redirect_url ) . '">' . $this->get_string( 'cancel' ) . '</a>
                            </p>
                            <div id="kdnaform-lock-request-status">
                                <!-- placeholder -->
                            </div>
                        </div>';
			...
			$message = '<div class="kdnaform-taken-over">
                            <div class="kdnaform-locked-avatar"></div>
                            <p class="wp-tab-first" tabindex="0">
                                <span class="currently-editing"></span><br>
                            </p>
                            <p>
                                <a id="kdnaform-release-lock-button" class="button button-primary wp-tab-last"  href="' . esc_url( add_query_arg( 'release-edit-lock', '1', $edit_url ) ) . '">' . $this->get_string( 'accept' ) . '</a>
                                <button id="kdnaform-reject-lock-request-button" style="display:none"  class="button button-primary wp-tab-last">' . __( 'Reject Request', 'kdnaforms' ) . '</button>
                            </p>
                        </div>';

		}
		$html = '<div id="kdnaform-lock-dialog" class="notification-dialog-wrap' . $hidden . '">
	...
	public function is_locked( $object_id ) {
		if ( ! $user_id = KDNACache::get( self::PREFIX_EDIT_LOCK . $this->_object_type . '_' . $object_id ) ) {
			return false;
		}
	...
	protected function is_page( $page_name ) {
		...
		return $page_name == KDNAForms::get_page();
	}
