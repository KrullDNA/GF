...
class KDNA_Upgrade {
	...
	public static function get_instance() {
		...
		if ( self::$_instance == null ) {
			self::$_instance = new KDNA_Upgrade();
		}
	...
	/**
	 * Decides to execute a fresh install of KDNA Forms, upgrade an existing installation, or do nothing if versions are up-to-date.
	 *
	 * @since  2.2
	 */
	public function maybe_upgrade() {
		...
		if ( $this->requires_install() ) {
			...
			if ( ! KDNACommon::get_key() ) {
				update_option( 'kdnaform_pending_installation', true );
			}
	...
	/**
	 * Performs an upgrade of KDNA Forms.
	 *
	 * @since  2.2
	 *
	...
	public function upgrade( $from_db_version = null, $force_upgrade = false ) {
		...
		if ( $force_upgrade || ! ( defined( 'KDNAFORM_AUTO_DB_MIGRATION_DISABLED' ) && KDNAFORM_AUTO_DB_MIGRATION_DISABLED ) ) {
	...
	public function set_license_network_option() {
		if ( ! KDNACommon::is_network_active() ) {
			return;
		}
		...
		$key = get_network_option( null, KDNAForms::LICENSE_KEY_OPT );
		if ( $key ) {
			return;
		}
		...
		update_network_option( null, KDNAForms::LICENSE_KEY_OPT, $key );
	}
	...
	public function sync_auto_updates( $previous_version ) {
		...
		// Auto-updates sync removed - license/updates functionality removed.
	}

	/**
	 * Performs initial install of KDNA Forms.
	 *
	 * @since  2.2
	 */
	public function install() {
		...
		// Auto-importing forms based on KDNA_IMPORT_FILE AND KDNA_THEME_IMPORT_FILE
		$this->maybe_import_forms();

		/**
		 * Fires after KDNA Forms is fully installed.
		 *
		 * @since 2.2
		 *
		 * @param int $version The new $version.
		 */
		do_action( 'kdnaform_post_install', KDNAForms::$version );
	...
	private function set_upgrade_started( $from_db_version = null, $force_upgrade = false ) {
		...
		if ( $lock_params && ! $force_upgrade ) {
			...
			KDNACommon::log_debug( __METHOD__ . '(): Upgrade in process. Aborting' );
		...
		$message = sprintf( esc_html__( 'KDNA Forms is currently upgrading the database to version %1$s. For sites with a large number of entries this may take a long time. Check the %2$sSystem Status%3$s page for further details.', 'kdnaforms' ), KDNAForms::$version, $system_status_link_open, $system_status_link_close );

		$key = sanitize_key( 'kdnaforms_upgrading_' . $versions['version'] );

		KDNACommon::add_dismissible_message( $message, $key, 'warning', 'kdnaform_full_access', true );
	...
	public function set_upgrade_ended() {
		...
		/**
		 * Fires after KDNA Forms is fully upgraded.
		 *
		 * @since 2.2
		 *
		 ...
		 *
		 */
		do_action( 'kdnaform_post_upgrade', $version, $from_db_version, $force_upgrade );
		...
		$key = sanitize_key( 'kdnaforms_upgrading_' . KDNAForms::$version );
		...
		KDNACommon::log_debug( __METHOD__ . '(): Upgrade Completed.' );
	}
	...
	private function test_auto_increment() {
		...
		KDNACommon::log_debug( __METHOD__ . '(): Testing whether tables have auto_increment set correctly.' );
	...
	private function get_auto_increment_tables() {
		...
		$table_rows = KDNACache::get( $this->auto_increment_tables_cache_key );
		...
		KDNACommon::log_debug( sprintf( '%s(): Checking tables for auto_increment flag: %s', __METHOD__, print_r( $table_rows, 1 ) ) );

		// Set cache so that only one query is executed if this method is called more than once in a page life cycle.
		KDNACache::set( $this->auto_increment_tables_cache_key, $table_rows );
	...
	private function fix_auto_increment( $table_name, $column_type ) {
		KDNACommon::log_debug( __METHOD__ . '(): Fixing the auto_increment settings for' . $table_name );
		...
		KDNACache::delete( $this->auto_increment_tables_cache_key );
	...
	/**
	 * Sets up the database for KDNA Forms
	 *
	 * @since  2.2
	 * @global $wpdb
	 *
	...
	public function get_db_schema( $table_name = null ) {
		...
		$entry_notes_table_name = KDNAFormsModel::get_entry_notes_table_name();
		...
		$draft_submissions_table_name = KDNAFormsModel::get_draft_submissions_table_name();
		...
		if ( version_compare( KDNACommon::get_db_version(), '8.0.17', '<' ) || ( KDNACommon::get_dbms_type() === 'MariaDB' ) ) {
	...
	public function check_table_schema( $table_name ) {
		...
		if ( empty( $to_update ) ) {
			...
			KDNACommon::log_debug( __METHOD__ . '(): Failed queries: ' . print_r( $to_update, true ) );
	...
	protected function post_upgrade_schema( $from_db_version, $force_upgrade ) {
		...
		$this->update_upgrade_status( esc_html__( 'Queued for upgrade.', 'kdnaforms' ) );
	...
	public function post_background_upgrade() {
		...
		KDNACommon::log_debug( __METHOD__ . '(): Background upgrade complete' );
	...
	public function update_upgrade_status( $new_status ) {
		update_option( 'kdnaform_upgrade_status', $new_status );
	}
	 ...
	 * @return bool
	 */
	public function kdna_upgrade_block_submissions() {
	 ...
	 * @return bool
	 */
	public function kdna_upgrade_release_submissions_block() {
	 ...
	 * @return bool
	 */
	public function kdna_upgrade_230_migrate_forms() {
		global $wpdb;
		$this->update_upgrade_status( esc_html__( 'Migrating forms.', 'kdnaforms' ) );
...
WHERE lfv.id NOT IN
      ( SELECT id
		...
		$this->update_upgrade_status( esc_html__( 'Forms migrated.', 'kdnaforms' ) );
		return false;
	}
	 ...
	 */
	public function kdna_upgrade_230_migrate_leads() {
		$lead_table = KDNAFormsModel::get_lead_table_name();
		if ( ! KDNACommon::table_exists( $lead_table ) ) {
			return false;
		}

		if ( defined( 'KDNAFORM_DB_MIGRATION_BATCH_SIZE' ) ) {
			$limit = KDNAFORM_DB_MIGRATION_BATCH_SIZE;
		} else {
			$limit = 20000;
		}
		...
		$this->update_upgrade_status( esc_html__( 'Entry details migrated.', 'kdnaforms' ) );
...
LIMIT {$limit}";
		...
		do {
			...
			if ( $wpdb->last_error ) {
				...
				$this->update_upgrade_status( sprintf( esc_html__( 'Error Migrating Entry Headers: %s', 'kdnaforms' ), $wpdb->last_error ) );
...
WHERE l2.id NOT IN ( SELECT e.id FROM {$entry_table} e )";
						...
						if ( $remaining > 0 ) {
							$this->update_upgrade_status( sprintf( esc_html__( 'Migrating leads. Step 1/3 Migrating entry headers. %d rows remaining.', 'kdnaforms' ), $remaining ) );
	...
	public function migrate_230_lead_details( $limit, $time_start ) {
		$lead_details_table = KDNAFormsModel::get_lead_details_table_name();
		if ( ! KDNACommon::table_exists( $lead_details_table ) ) {
			return false;
		}
...
LIMIT {$limit}";
		...
		do {
			...
			if ( $wpdb->last_error ) {
				...
				/* translators: %s: the database error */
				$this->update_upgrade_status( sprintf( esc_html__( 'Error Migrating Entry Details: %s', 'kdnaforms' ), $wpdb->last_error ) );
				// wp_die() is not used here because it would trigger another async task
				exit;
			}
...
WHERE ld.id NOT IN ( SELECT em.id FROM {$entry_meta_table} em )";
					$remaining     = $wpdb->get_var( $sql_remaining ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
					if ( $remaining > 0 ) {
						$this->update_upgrade_status( sprintf( esc_html__( 'Migrating leads. Step 2/3 Migrating entry details. %d rows remaining.', 'kdnaforms' ), $remaining ) );
	...
	public function migrate_230_lead_meta( $limit, $time_start ) {
		...
		if ( ! KDNACommon::table_exists( $lead_meta_table ) ) {
			return false;
		}
...
LIMIT {$limit}";
		...
		do {
			...
			if ( $wpdb->last_error ) {
				/* translators: %s: the database error */
				$this->update_upgrade_status( sprintf( esc_html__( 'Error Migrating Entry Meta: %s', 'kdnaforms' ), $wpdb->last_error ) );
				// wp_die() is not used here because it would trigger another async task
				exit;
			}
...
WHERE NOT EXISTS
      (SELECT * FROM {$entry_meta_table} em WHERE em.entry_id = lm.lead_id AND CONVERT(em.meta_key USING {$charset_db}) = CONVERT(lm.meta_key USING {$charset_db}) {$collate})";
					$remaining     = $wpdb->get_var( $sql_remaining ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
					if ( $remaining > 0 ) {
						$this->update_upgrade_status( sprintf( esc_html__( 'Migrating leads. Step 3/3 Migrating entry meta. %d rows remaining.', 'kdnaforms' ), $remaining ) );
	 ...
	 * @return bool
	 */
	public function kdna_upgrade_230_migrate_incomplete_submissions() {
		global $wpdb;

		$this->update_upgrade_status( esc_html__( 'Migrating incomplete submissions.', 'kdnaforms' ) );

		$incomplete_submissions_table = KDNAFormsModel::get_incomplete_submissions_table_name();

		if ( ! KDNACommon::table_exists( $incomplete_submissions_table ) ) {
			// The table doesn't exist. Maybe an upgrade from a very early version.
			return false;
		}

		$draft_submissions_table = KDNAFormsModel::get_draft_submissions_table_name();
...
WHERE CONVERT(insub.uuid USING {$charset_db}) {$collate} NOT IN
      ( SELECT uuid FROM {$draft_submissions_table} )";
		...
		if ( $wpdb->last_error ) {
			/* translators: %s: the database error */
			$this->update_upgrade_status( sprintf( esc_html__( 'Error Migrating incomplete submissions: %s', 'kdnaforms' ), $wpdb->last_error ) );
			// wp_die() is not used here because it would trigger another async task
			exit;
		}
		return false;
	 ...
	 * @return bool
	 */
	public function kdna_upgrade_230_migrate_lead_notes() {
		global $wpdb;

		$this->update_upgrade_status( esc_html__( 'Migrating entry notes.', 'kdnaforms' ) );

		$lead_notes_details_table = KDNAFormsModel::get_lead_notes_table_name();
...
WHERE ln.id NOT IN
	...
	/**
	 * Imports KDNA Forms license keys, and reCAPTCHA keys from global variables.
	 *
	 * @since  2.2
	 * @access protected
	 * @global $kdna_license_key
	 *
	 * @uses   KDNA_RECAPTCHA_PRIVATE_KEY
	 * @uses   KDNA_RECAPTCHA_PUBLIC_KEY
	 * @uses   KDNA_LICENSE_KEY
	 */
	protected function maybe_populate_keys() {

		global $kdna_license_key;
		$license_key = defined( 'KDNA_LICENSE_KEY' ) && empty( $kdna_license_key ) ? KDNA_LICENSE_KEY : $kdna_license_key;
		if ( ! empty( $license_key ) ) {
			KDNAFormsModel::save_key( $license_key );
			KDNACommon::cache_remote_message();
			KDNACommon::get_version_info( false );
		}
	...
	/**
	 * Auto imports forms when KDNA Forms is installed based on KDNA_IMPORT_FILE constant.
	 *
	 * @since  2.2
	 * @access protected
	 *
	 * @uses   KDNA_IMPORT_FILE
	 * @uses   KDNACommon::get_base_path()
	 * @uses   KDNAExport::import_file()
	 */
	protected function maybe_import_forms() {

		if ( defined( 'KDNA_IMPORT_FILE' ) && ! get_option( 'kdna_imported_file' ) ) {

			require_once( KDNACommon::get_base_path() . '/export.php' );
			KDNAExport::import_file( KDNA_IMPORT_FILE );
			update_option( 'kdna_imported_file', true );
		}
	}
	...
	public function maybe_import_theme_forms() {
		...
		if ( defined( 'KDNA_THEME_IMPORT_FILE' ) ) {
			...
			if ( ! isset( $themes[ $theme ] ) ) {
				...
				//importing forms
				KDNAExport::import_file( get_stylesheet_directory() . '/' . KDNA_THEME_IMPORT_FILE );
	...
	/**
	 * Upgrade routine from kdna forms version 2.0.4.7 and below
	 */
	protected function post_upgrade_schema_2047() {
		...
		$long_table_name = KDNAFormsModel::get_lead_details_long_table_name();
	...
	/**
	 * Fixes leading and trailing spaces within KDNA Forms tables.
	 *
	 * @since  Unknown
	 * @access private
	 * @global $wpdb
	...
	private function fix_leading_and_trailing_spaces() {
		...
		foreach ( $results as &$result ) {
			...
			$form         = KDNAFormsModel::unserialize( $result['display_meta'] );
			$form_updated = false;
			$form         = KDNAFormsModel::trim_form_meta_values( $form, $form_updated );
			if ( $form_updated ) {
				KDNAFormsModel::update_form_meta( $form_id, $form );
			}

			$confirmations         = KDNAFormsModel::unserialize( $result['confirmations'] );
			$confirmations_updated = false;
			$confirmations         = KDNAFormsModel::trim_conditional_logic_values( $confirmations, $form, $confirmations_updated );
			if ( $confirmations_updated ) {
				KDNAFormsModel::update_form_meta( $form_id, $confirmations, 'confirmations' );
			}

			$notifications         = KDNAFormsModel::unserialize( $result['notifications'] );
			$notifications_updated = false;
			$notifications         = KDNAFormsModel::trim_conditional_logic_values( $notifications, $form, $notifications_updated );
			if ( $notifications_updated ) {
				KDNAFormsModel::update_form_meta( $form_id, $notifications, 'notifications' );
			}
		}
	...
	private function fix_checkbox_value() {
		...
		$table_name = KDNAFormsModel::get_lead_details_table_name();

		$sql     = "select * from {$table_name} where value= '!'";
		$results = $wpdb->get_results( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
 		foreach ( $results as $result ) {
			$form  = KDNAFormsModel::get_form_meta( $result->form_id );
			$field = KDNAFormsModel::get_field( $form, $result->field_number );
			if ( $field->type == 'checkbox' ) {
				$input = KDNACommon::get_input( $field, $result->field_number );
	...
	public function drop_index( $table, $index ) {
		...
		if ( ! KDNAFormsModel::is_valid_table( $table ) || ! KDNAFormsModel::is_valid_index( $index ) ) {
			return;
		}
	...
	private function maybe_upgrade_lead_detail_table() {
		...
		if ( ! $this->can_upgrade_longtext() ) {
			KDNACommon::log_debug( __METHOD__ . '(): Bailing' );
...
SET d.value = l.value"
		...
		KDNACommon::remove_dismissible_message( 'kdnaform_long_table_upgrade' );

		KDNACommon::log_debug( __METHOD__ . '(): result: ' . print_r( $result, true ) );
	}
	...
	public function validate_upgrade( $do_upgrade, $hook_extra ) {
		...
		if ( rgar( $hook_extra, 'plugin' ) == 'kdnaforms/kdnaforms.php' && ! $this->has_database_permission( $error ) ) {
	...
	/**
	 * Checks if KDNA Forms has permissions to make changes to the database.
	 *
	 * @since   Unknown
	 * @access  private
	 * @global       $wpdb
	 *
	 * @used-by KDNAForms::validate_upgrade()
	 *
	...
	public function has_database_permission( &$error ) {
		...
		$wpdb->query( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
		$error = 'Current database user does not have necessary permissions to create tables. KDNA Forms requires that the database user has CREATE and ALTER permissions. If you need assistance in changing database user permissions, contact your hosting provider.';
		...
		if ( $has_permission ) {
			...
			$wpdb->query( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
			$error = 'Current database user does not have necessary permissions to modify (ALTER) tables. KDNA Forms requires that the database user has CREATE and ALTER permissions. If you need assistance in changing database user permissions, contact your hosting provider.';
			if ( ! empty( $wpdb->last_error ) ) {
				$has_permission = false;
			}
	...
	private function can_upgrade_longtext() {
		...
		// The kdnaform_longtext_ready option was set in 1.9.x to indicate that the lead details table had been upgraded.
		// It was also set for new installations of 1.9.x.
		$is_longtext_ready = (bool) get_option( 'kdnaform_longtext_ready' );

		if ( $is_longtext_ready ) {
			return false;
		}

		// The kdnaform_longtext_upgraded option was added by the Upgrade Wizard Support Tool used to help debug upgrade issues.
		$upgraded = (bool) get_option( 'kdnaform_longtext_upgraded' );
		...
		if ( ! $is_longtext ) {
			...
			KDNACommon::log_debug( __METHOD__ . '(): lead detail value column issue' );

			KDNACommon::add_dismissible_message( esc_html__( 'There appears to be an issue with one of the KDNA Forms database tables. Please get in touch with support.', 'kdnaforms' ), 'kdnaform_long_table_upgrade', 'error', 'kdnaform_full_access', true );
...
HAVING count(*) > 1;" );
			...
			if ( count( $results ) == 0 ) {
				...
				KDNACommon::log_debug( __METHOD__ . '(): lead detail IDs issue' );

				KDNACommon::add_dismissible_message( esc_html__( 'There appears to be an issue with the data in the KDNA Forms database tables. Please get in touch with support.', 'kdnaforms' ), 'kdnaform_long_table_upgrade', 'error', 'kdnaform_full_access', true );
			}
		}

		KDNACommon::log_debug( __METHOD__ . '(): can_upgrade: ' . $can_upgrade );
	...
	/**
	 * Returns true if KDNA Forms need to be installed. False otherwise.
	 * @since 2.2
	 * @return bool
	 */
	...
	/**
	 * Returns true if KDNA Forms need to be upgraded. False otherwise.
	 *
	 * @since 2.2
	 * @return bool
	 */
	...
	public function requires_install_wizard() {
		...
		if ( defined( 'KDNA_LICENSE_KEY' ) && is_multisite() && ! is_main_site() ) {
			return false;
		}

		$pending_installation = get_option( 'kdnaform_pending_installation' ) || isset( $_GET['kdnaform_installation_wizard'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public function remove_obsolete_admin_notices() {
		KDNACommon::remove_dismissible_message( 'kdnaforms_update_2_5' );
	}
	...
	public function add_post_upgrade_admin_notices() {
		...
		if ( $number_outdated == 1 ) {
			...
			$message = sprintf( esc_html__( 'The %s is not compatible with this version of KDNA Forms. See the plugins list for further details.', 'kdnaforms' ), $outdated[0] );
		} else {
			/* translators: %d: the number of outdated add-ons */
			$message = sprintf( esc_html__( 'There are %d add-ons installed that are not compatible with this version of KDNA Forms. See the plugins list for further details.', 'kdnaforms' ), $number_outdated );
		}

		KDNACommon::add_dismissible_message( $message, $key, 'error', 'kdnaform_full_access', true, 'site-wide' );
	}

	/**
	 * Returns an array of add-ons with the minimum version required for this version of KDNA Forms.
	 *
	 * @since 2.3
	 *
	 * @return array
	...
	public function get_min_addon_requirements() {
		return array(
			'kdnaformspaypal/paypal.php'                       => array(
				'name'        => 'KDNA Forms PayPal Add-On',
				'min_version' => '2.9',
			),
			'kdnaformsauthorizenet/authorizenet.php'           => array(
				'name'        => 'KDNA Forms Authorize.Net Add-On',
				'min_version' => '2.4',
			),
			'kdnaformspartialentries/partialentries.php'       => array(
				'name'        => 'KDNA Forms Partial Entries Add-On',
				'min_version' => '1.1',
			),
			'kdnaformspaypalpaymentspro/paypalpaymentspro.php' => array(
				'name'        => 'KDNA Forms PayPal Payments Pro Add-On',
				'min_version' => '2.3',
			),
			'kdnaformssignature/signature.php'                 => array(
				'name'        => 'KDNA Forms Signature Add-On',
				'min_version' => '3.4',
			),
			'kdnaformsuserregistration/userregistration.php'   => array(
				'name'        => 'KDNA Forms User Registration Add-On',
				'min_version' => '3.9',
			),
			'kdnaformspaypalpro/paypalpro.php'                 => array(
				'name'        => 'KDNA Forms PayPal Pro Add-On',
				'min_version' => '1.8',
			),
		);
	}
