...
trait Has_Block_Settings {
	...
	public function add_engine_block_settings() {
		$engine = $this->definition_engine_factory->get( KDNA_Theme_Layers_Provider::BLOCK_SETTINGS_DEFINITION_ENGINE );
		$engine->set_block_settings( $this->block_settings() );

		$this->definition_engines[] = $engine;

		add_filter( 'kdnaform_form_block_attributes', function( $attributes ) use ( $engine ) {
