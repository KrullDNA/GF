...
namespace KDNA_Forms\KDNA_Forms\Theme_Layers\Framework\Engines\Output_Engines;

/**
 * Engine to provide PHP Markup overrides for given views. Uses kdnaform_field_content and  kdnaform_field_content
...
class PHP_Markup_Output_Engine extends Output_Engine {
	...
	public function output() {
		...
		if ( isset( $views['form'] ) ) {
			add_filter( 'kdnaform_get_form_filter', array( $this, 'handle_form_override' ), 999, 2 );
			add_filter( 'kdnaform_get_form_save_confirmation_filter', array( $this, 'handle_form_override' ), 999, 2 );
			add_filter( 'kdnaform_get_form_confirmation_filter', array( $this, 'handle_form_override' ), 999, 2 );
			add_filter( 'kdnaform_get_form_save_email_confirmation_filter', array( $this, 'handle_form_override' ), 999, 2 );
		}

		if ( isset( $views['confirmation'] ) ) {
			add_filter( 'kdnaform_get_form_confirmation_filter', array( $this, 'handle_confirmation_override' ), 999, 2 );
		}

		// Add a filter for field output.
		add_filter( 'kdnaform_field_content', array( $this, 'handle_field_override' ), 999, 5 );
	}
	 ...
	 * @since 2.7
	 *
	 * @hook kdnaform_get_form_filter 999 2
	 * @hook kdnaform_get_form_save_confirmation_filter 999 2
	 *
	 * @return string
	 */
	 ...
	 * @since 2.7
	 *
	 * @hook kdnaform_field_content 999 5
	 *
	 * @return string
	 */
