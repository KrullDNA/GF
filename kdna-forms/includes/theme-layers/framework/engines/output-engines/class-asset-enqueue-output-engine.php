...
class Asset_Enqueue_Output_Engine extends Output_Engine {
	...
	public function output() {
		...
		add_action( 'admin_enqueue_scripts', function() use ( $self ) {
			...
			$form = $settings = array();
			if ( in_array( \KDNAForms::get_page(), array( 'form_editor', 'entry_detail', 'entry_detail_edit' ) ) ) {
				$form = KDNAAPI::get_form( rgget( 'id' ) );
				if ( ! $form ) {
					return;
				}
		...
		add_filter( 'kdnaform_preview_styles', function( $styles, $form ) use ( $self ) {
			global $wp_styles;
		...
		add_action( 'kdnaform_enqueue_scripts', function( $form, $ajax ) use ( $self ) {
			$settings = $this->get_settings( rgar( $form, 'id' ) );
		...
		add_filter( 'kdnaform_noconflict_styles', function ( $styles ) {
