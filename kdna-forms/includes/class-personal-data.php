...
class KDNA_Personal_Data {
	...
	private static function settings_fields( $form_id ) {
		...
		return array(
			array(
				...
				'title'  => esc_html__( 'General Settings', 'kdnaforms' ),
				'fields' => array(
					array(
						'name'    => 'preventIP',
						'type'    => 'toggle',
						'label'   => esc_html__( 'Prevent the storage of IP addresses during form submission', 'kdnaforms' ),
						'tooltip' => kdnaform_tooltip( 'personal_data_prevent_ip', null, true ),
					),
					array(
						'name'          => 'retention[policy]',
						'type'          => 'radio',
						'label'         => esc_html__( 'Retention Policy', 'kdnaforms' ),
						'tooltip'       => kdnaform_tooltip( 'personal_data_retention_policy', null, true ),
						'default_value' => 'retain',
						'choices'       => array(
							array(
								'label' => esc_html__( 'Retain entries indefinitely', 'kdnaforms' ),
								'value' => 'retain',
							),
							array(
								'label'   => esc_html__( 'Trash entries automatically', 'kdnaforms' ),
								'value'   => 'trash',
								'onclick' => sprintf(
									'alert( %s );',
									json_encode( __( 'Warning: this will affect all entries that are older than the number of days specified.', 'kdnaforms' ) )
								),
							),
							array(
								'label'   => esc_html__( 'Delete entries permanently automatically', 'kdnaforms' ),
								'value'   => 'delete',
								'onclick' => sprintf(
									'alert( %s );',
									json_encode( __( 'Warning: this will affect all entries that are older than the number of days specified.', 'kdnaforms' ) )
					...
					array(
						...
						'label'               => esc_html__( 'Number of days to retain entries before trashing/deleting:', 'kdnaforms' ),
						'type'                => 'text',
						'input_type'          => 'number',
						'default_value'       => 1,
						'dependency'          => array(
						...
						'validation_callback' => function( $field, $value ) {
							...
							if ( ! is_numeric( $value ) || ( is_numeric( $value ) && floatval( $value ) < 1 ) ) { // nosemgrep audit.php.lang.misc.flawed-logic-numeric
								$field->set_error( esc_html__( 'Form entries must be retained for at least one day.', 'kdnaforms' ) );
							}
			...
			array(
				'class'  => 'kdnaform-settings-panel--full',
				'title'  => esc_html__( 'Exporting and Erasing Data', 'kdnaforms' ),
				'fields' => array(
					array(
						'name'        => 'exportingAndErasing[enabled]',
						'type'        => 'toggle',
						'label'       => esc_html__( 'Enable integration with the WordPress tools for exporting and erasing personal data.', 'kdnaforms' ),
						'tooltip'     => kdnaform_tooltip( 'personal_data_enable', null, true ),
						...
						'after_input' => ! empty( $identification_field_choices ) ? '' : sprintf(
							...
							esc_html__( 'You must add an email address field to the form in order to enable this setting.', 'kdnaforms' )
						),
					),
					array(
						'name'       => 'exportingAndErasing[identificationField]',
						'type'       => 'select',
						'label'      => esc_html__( 'Identification Field', 'kdnaforms' ),
						'tooltip'    => kdnaform_tooltip( 'personal_data_identification', null, true ),
					...
					array(
						...
						'type'       => 'columns',
						'label'      => esc_html__( 'Personal Data', 'kdnaforms' ),
						'tooltip'    => kdnaform_tooltip( 'personal_data_field_settings', null, true ),
						'callback'   => array( 'KDNA_Personal_Data', 'settings_columns' ),
						'dependency' => array(
							'live'   => true,
							'fields' => array(
								array(
	...
	private static function get_identification_fields_choices( $form = array() ) {
		...
		// Get Email fields.
		$email_fields = KDNAAPI::get_fields_by_type( $form, 'email' );
		...
		$choices = gf_apply_filters( array(
			'kdnaform_personal_data_identification_fields',
			$form['id'],
		), $choices, $form );
		...
		if ( $current_value === 'created_by' ) {
			...
			$choices[] = array(
				'label' => esc_html__( 'Created By', 'kdnaforms' ),
				'value' => 'created_by',
			);

		} else if ( $selected_field = KDNAAPI::get_field( $form, $current_value ) ) {

			// Set admin label context.
			$selected_field->set_context_property( 'use_admin_label', true );

			$choices[] = array(
				'label' => KDNAFormsModel::get_label( $selected_field ),
				'value' => $current_value,
			);
	...
	public static function settings_columns( $props = array(), $echo = true ) {
		...
		$html = sprintf(
			'<table id="gf_personal_data_field_settings" class="form-table">
				...
				<tbody>',
			esc_html__( 'Fields', 'kdnaforms' ),
			esc_html__( 'Export', 'kdnaforms' ),
			esc_html__( 'Erase', 'kdnaforms' )
		);
		...
		'<tr>
				...
				<td class="gf_personal_data_cb_cell">
					<div class="kdnaform-settings-choice">
			...
			esc_html__( 'Select/Deselect All', 'kdnaforms' )
		);
		...
		foreach ( $pd_fields as $pd_field ) {
			...
			$export_field = \KDNA_Forms\KDNA_Forms\Settings\Fields::create(
				array(
			...
			$erase_field = \KDNA_Forms\KDNA_Forms\Settings\Fields::create(
				array(
		...
		if ( ! empty( $custom_items ) ) {
			...
			$html .= sprintf(
				...
				esc_html__( 'Other Data', 'kdnaforms' )
			);
	...
	public static function initialize_settings_renderer() {
		...
		$renderer = new Settings(
			array(
				'header'         => array(
					'icon'  => 'fa fa-lock',
					'title' => esc_html__( 'Personal Data', 'kdnaforms' ),
				),
				'fields'         => self::settings_fields( $form_id ),
				'initial_values' => rgar( $form, 'personalData' ),
				'save_callback'  => array( 'KDNA_Personal_Data', 'process_form_settings' ),
	...
	public static function get_form( $form_id ) {
		if ( empty( self::$_form ) ) {
			self::$_form = KDNAAPI::get_form( $form_id );
		}
	...
	public static function get_columns() {
		$columns = array(
			'ip'         => esc_html__( 'IP Address', 'kdnaforms' ),
			'source_url' => esc_html__( 'Embed URL', 'kdnaforms' ),
			'user_agent' => esc_html__( 'Browser details', 'kdnaforms' ),
		);
	...
	public static function get_custom_items( $form ) {
		 ...
		 * Example:
		 *
		 * add_filter( 'kdnaform_personal_data', 'filter_kdnaform_personal_data', 10, 2 );
		 * function filter_kdnaform_personal_data( $items, $form ) {
		 *       $items['test'] = array(
		 *          'label'             => 'A custom item',
		 ...
		 * @param array $form
		 */
		$custom_items = apply_filters( 'kdnaform_personal_data', $custom_items, $form );
	...
	public static function get_forms() {
		...
		if ( is_null( self::$_forms ) ) {
			$form_ids = KDNAFormsModel::get_form_ids( null );
			...
			$forms_by_id = KDNAFormsModel::get_form_meta_by_id( $form_ids );
	...
	public static function get_entries( $email_address, $page = 1, $limit = 50 ) {
		...
		foreach ( $forms as $form ) {
			...
			$field = KDNAAPI::get_field( $form, $identification_field );

			if ( $field && $field->get_input_type() == 'email' ) {

				$conditions[] = new KDNA_Query_Condition(
					new KDNA_Query_Column( $identification_field, intval( $form['id'] ) ),
					KDNA_Query_Condition::EQ,
					new KDNA_Query_Literal( $email_address )
				);
				...
				$conditions[] = new KDNA_Query_Condition(
					new KDNA_Query_Column( $identification_field, intval( $form['id'] ) ),
					KDNA_Query_Condition::EQ,
					new KDNA_Query_Literal( $user->ID )
				);
			}
		}
		...
		$all_conditions = call_user_func_array( array( 'KDNA_Query_Condition', '_or' ), $conditions );
	...
	public static function data_exporter( $email_address, $page = 1 ) {
		...
		foreach ( $entries as $entry ) {
			...
			$group_id = 'kdnaforms-entries';

			$group_label = __( 'Forms', 'kdnaforms' );
	...
	public static function get_draft_submissions_export_items( $email_address ) {
		...
		foreach ( $draft_submissions as $i => $draft_submission ) {
			...
			$group_id = 'kdnaforms-draft-submissions';

			$group_label = __( 'Draft Forms (Save and Continue Later)', 'kdnaforms' );
			...
			if ( is_array( $columns_settings ) ) {
				foreach ( $columns_settings as $column_key => $column_settings ) {
					if ( rgar( $column_settings, 'erase' ) ) {
						KDNAAPI::update_entry_property( $entry['id'], $column_key, '' );
						$items_removed = true;
					}
				}
			}
			...
			foreach ( $form['fields'] as $field ) {
				...
				if ( $field->personalDataErase ) {
					...
					if ( $input_type == 'fileupload' ) {
						KDNAFormsModel::delete_files( $entry['id'] );
						KDNAAPI::update_entry_field( $entry['id'], $field->id, '' );
						continue;
					}
					...
					if ( is_array( $value ) ) {
						...
						KDNAAPI::update_entry_field( $entry['id'], $field->id, $anonymous );
						$items_removed = true;
					}
				}
			}
		...
		if ( is_array( $value ) ) {
			...
			foreach ( $value as $key => $val ) {
				if ( is_array( $val ) ) {
					...
					KDNAAPI::update_entry_field( $entry_id, $key, '', $item_index );
				}
			}
		} else {
			KDNAAPI::update_entry_field( $entry_id, $input_id, '', $item_index );
		}
	...
	public static function get_draft_submissions( $email_address ) {
		...
		$draft_submissions = KDNAFormsModel::get_draft_submissions();
	...
	public static function erase_draft_submissions_data( $email_address ) {
		...
		foreach ( $draft_entries as $draft_entry ) {
				...
				if ( $field->personalDataErase ) {
					...
					if ( is_array( $value ) ) {
						...
						switch ( $input_type ) {
							...
							case 'textarea':
								/* translators: deleted text */
								$anonymous = __( '[deleted]', 'kdnaforms' );
								break;
							default:
								$anonymous = '';
						}
			...
			if ( $entry_dirty ) {
				...
				KDNAFormsModel::update_draft_submission( $resume_token, $form, $date_created, $draft_entry['ip'], $draft_entry['source_url'], $submission_json );
				$items_removed = true;
			}
		}
	...
	public static function cron_task() {
		...
		foreach ( $forms as $form ) {
			...
			$form_conditions[] = new KDNA_Query_Condition(
				new KDNA_Query_Column( 'date_created' ),
				KDNA_Query_Condition::LT,
				new KDNA_Query_Literal( $delete_date )
			);

			$form_conditions[] = new KDNA_Query_Condition(
				new KDNA_Query_Column( 'form_id' ),
				KDNA_Query_Condition::EQ,
				new KDNA_Query_Literal( $form['id'] )
			);
		...
		if ( ! empty( $trash_conditions ) ) {
			...
			$all_trash_conditions[] = call_user_func_array( array( 'KDNA_Query_Condition', '_or' ), $trash_conditions );

			$all_trash_conditions[] = new KDNA_Query_Condition(
				new KDNA_Query_Column( 'status' ),
				KDNA_Query_Condition::NEQ,
				new KDNA_Query_Literal( 'trash' )
			);

			$all_trash_conditions = call_user_func_array( array( 'KDNA_Query_Condition', '_and' ), $all_trash_conditions );
			...
			foreach ( $entry_ids as $entry_id ) {
				KDNAAPI::update_entry_property( $entry_id, 'status', 'trash' );
			}
		}
		...
		if ( ! empty( $delete_conditions ) ) {
			...
			$all_delete_conditions = call_user_func_array( array( 'KDNA_Query_Condition', '_or' ), $delete_conditions );
			 ...
			 */
			$entry_ids = apply_filters( 'kdnaform_entry_ids_automatic_deletion', $entry_ids );

			foreach ( $entry_ids as $entry_id ) {
				KDNAAPI::delete_entry( $entry_id );
			}
		}
