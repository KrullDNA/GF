 ...
 * @package   KDNAForms
 * @author    KDNA
 * @copyright Copyright (c) 2016, KDNA
 */
class KDNALogging extends KDNAAddOn {
	 ...
	 */
	protected $_slug = 'kdnaformslogging';
	 ...
	 */
	protected $_path = 'kdnaformslogging/logging.php';
	 ...
	 */
	protected $_capabilities_settings_page = 'kdnaforms_logging';
	 ...
	 */
	protected $_capabilities_form_settings = 'kdnaforms_logging';
	 ...
	 */
	protected $_capabilities = array( 'kdnaforms_logging' );
	 ...
	 */
	protected $_nonce_action = 'kdnaform_delete_log';
		...
		if ( $plugin_slug ) {
			...
			if ( isset( $supported_plugins[ $plugin_slug ] ) ) {
				if ( wp_verify_nonce( rgget( $this->_nonce_action ), $this->_nonce_action ) && $this->delete_log_file( $plugin_slug ) ) {
					...
					$redirect_url = remove_query_arg( array( 'delete_log', 'kdnaform_delete_log' ) );
					...
					KDNACommon::add_error_message( esc_html__( 'Log file could not be deleted.', 'kdnaforms' ) );
				}
			} else {
				KDNACommon::add_error_message( esc_html__( 'Invalid log file.', 'kdnaforms' ) );
			}
		}

		// If a log file was deleted, display message.
		if ( '1' === rgget( 'deleted' ) ) {
			KDNACommon::add_message( esc_html__( 'Log file was successfully deleted.', 'kdnaforms' ) );
		}
...
//			'type'     => 'save',
//			'messages' => array(
//				'success' => esc_html__( 'Plugin logging settings have been updated.', 'kdnaforms' ),
//			),
//		);
	...
	public function plugin_settings_description() {
		...
		$html  = '<p>';
		$html .= esc_html__( 'Logging assists in tracking down issues by logging debug and error messages in KDNA Forms Core and KDNA Forms Add-Ons. Important information may be included in the logging messages, including API usernames, passwords and credit card numbers. Logging is intended only to be used temporarily while trying to track down issues. Once the issue is identified and resolved, it should be disabled.', 'kdnaforms' );
		$html .= '</p>';
	...
	public function plugin_settings_title() {
		...
		return esc_html__( 'Plugin Logging Settings', 'kdnaforms' );
	...
	public function prepare_settings_checkbox_and_select( $field ) {
		...
		$checkbox_field = array(
			...
			'label'      => esc_html__( 'Enable', 'kdnaforms' ),
			'horizontal' => true,
			'value'      => '1',
			'choices'    => false,
			'tooltip'    => false,
	...
	public function supported_plugins_fields() {
		...
		foreach ( $supported_plugins as $plugin_slug => $plugin_name ) {
			...
			if ( $this->log_file_exists( $plugin_slug ) ) {
				$delete_url = add_query_arg( array( 'delete_log' => $plugin_slug, $this->_nonce_action => $nonce ), admin_url( 'admin.php?page=gf_settings&subview=kdnaformslogging' ) );
				$after_select  = '<br />';
				$after_select .= '<span style="font-size:85%"><a href="' . esc_attr( $this->get_log_file_url( $plugin_slug ) ) . '" target="_blank">' . esc_html__( 'view log', 'kdnaforms' ) . '<span class="screen-reader-text">' . esc_html__( '(opens in a new tab)', 'kdnaforms' ) . '</span>&nbsp;<span class="kdnaform-icon kdnaform-icon--external-link" aria-hidden="true"></span></a>';
				$after_select .= '&nbsp;&nbsp;<a href="' . $delete_url . '">' . esc_html__( 'delete log', 'kdnaforms' ) . '</a>';
			...
			if ( count( $logging_options ) < 2 ) {
				...
				$plugin_fields[] = array(
					...
					'choices' => array(
						array(
							'label' => esc_html__( 'Enable logging and log all messages', 'kdnaforms' ),
				...
				$plugin_fields[] = array(
					...
					'checkbox'     => array(
						'label' => esc_html__( 'Enable logging', 'kdnaforms' ),
						'name'  => $plugin_slug . '[enable]',
					),
					'select' => array(
						'name'    => $plugin_slug . '[log_level]',
	...
	public function get_logging_choices() {
		...
		$options = array(
			array(
				'label' => esc_html__( 'and log all messages', 'kdnaforms' ),
				'value' => KLogger::DEBUG,
			)
		);
		...
		// @todo Rename filter.
		$display_error_level = apply_filters( 'kdnaform_logging_display_error_log_level', in_array( KLogger::ERROR, $log_levels ) );

		// Add error level option.
		if ( $display_error_level ) {
			$options[] = array(
				'label' => esc_html__( 'and log only error messages', 'kdnaforms' ),
				'value' => KLogger::ERROR,
			);
		}
	...
	public static function log_message( $plugin, $message = null, $message_type = KLogger::DEBUG ) {
		 ...
		 */
		do_action( 'kdnaform_pre_log_message', $plugin, $message, $message_type, $enabled );

		// If message is empty, exit.
		if ( rgblank( $message ) || ! class_exists( 'KDNAForms' ) || ! get_option( 'kdnaform_enable_logging' ) ) {
			return;
		}
		...
		* @param object $KDNALogging      The KDNA Forms Logging object.
		*/
		$message = apply_filters( 'kdnaform_logging_message', $message, $message_type, $plugin_setting, $log, $instance );
	 ...
	 * @return bool
	 */
	public static function is_enabled( $plugin = 'kdnaforms' ) {
	...
	public function delete_log_files() {
		...
		if ( is_dir( $dir ) ) {
			$files = KDNACommon::glob( '*', $dir ); // Get all file names.
	...
	public function get_log_dir() {
		...
		return KDNAFormsModel::get_upload_root() . 'logs/';
	...
	public function get_log_dir_url() {
		...
		return KDNAFormsModel::get_upload_url_root() . 'logs/';
	...
	protected static function register_config() {
        require_once 'config/class-kdna-logging-config.php';
		$container = KDNAForms::get_service_container();
        $config_collection = $container->get( KDNA_Forms\KDNA_Forms\Config\KDNA_Config_Service_Provider::CONFIG_COLLECTION );
        $config_collection->add_config( new KDNA_Forms\KDNA_Forms\Logging\Config\GF_Logging_Config( $container->get( KDNA_Forms\KDNA_Forms\Config\KDNA_Config_Service_Provider::DATA_PARSER ) ) );
	}
	...
	private function maybe_reset_logs( $file_path, $gmt_offset ) {
		...
		$similar_files = KDNACommon::glob( $file_base . '*.*', $folder );
	...
	public function upgrade_from_pre_addon_framework() {
		...
		if ( is_multisite() ) {
			...
			foreach ( $sites as $site ) {
				...
				// Save new settings.
				update_blog_option( $blog_id, 'kdnaformsaddon_' . $this->get_slug() . '_settings', $new_settings );
	...
	public function get_supported_plugins() {
		...
		// Get core plugins.
		$core_plugins = KDNAForms::set_logging_supported( array() );
		asort( $core_plugins );

		// Get third party plugins.
		$supported_plugins = apply_filters( 'kdnaform_logging_supported', array() );
	...
	/**
	 * Initializing translations using the kdnaforms domain.
	 *
	 * @since  2.2
	 * @access public
	 */
...
/**
 * Returns an instance of the KDNALogging class
 *
 * @see    KDNALogging::get_instance()
 * @return object KDNALogging
 */
function gf_logging() {
	return KDNALogging::get_instance();
}
