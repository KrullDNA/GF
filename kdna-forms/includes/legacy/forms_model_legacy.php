 ...
 *
 * Legacy methods from KDNAFormsModel
 */
class GF_Forms_Model_Legacy {
	 ...
	 * @static
	 * @global $wpdb
	 * @see KDNAFormsModel::get_lead_table_name
	 * @see KDNACache::get
	 * @see KDNACache::set
	 *
	...
	public static function get_entry_count_per_form() {
		...
		if ( empty( $entry_count ) ) {
			...
			$entry_count = $wpdb->get_results( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
	...
	public static function update_lead_property( $lead_id, $property_name, $property_value, $update_akismet = true, $disable_hook = false ) {
		...
		if ( $update_akismet && KDNACommon::akismet_enabled( $lead['form_id'] ) && $property_name == 'status' && in_array( $property_value, array( 'active', 'spam' ) ) ) {
			...
			if ( $current_status == 'spam' && $property_value == 'active' ) {
				$form = KDNAFormsModel::get_form_meta( $lead['form_id'] );
		...
		//updating lead
		$result = $wpdb->update( $lead_table, array( $property_name => $property_value ), array( 'id' => $lead_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	...
	public static function delete_lead( $lead_id ) {
		...
		KDNACommon::log_debug( __METHOD__ . "(): Deleting entry #{$lead_id}." );
		...
		do_action( 'kdnaform_delete_lead', $lead_id );
		...
		KDNAFormsModel::delete_files( $lead_id );

		//Delete from lead details
		$sql = $wpdb->prepare( "DELETE FROM %i WHERE lead_id=%d", $lead_detail_table_name, $lead_id );
		...
		$sql = $wpdb->prepare( "DELETE FROM %i WHERE lead_id=%d", $lead_notes_table, $lead_id );
		$wpdb->query( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared

		//Delete from lead meta
		kdnaform_delete_meta( $lead_id );

		//Delete from lead
		$sql = $wpdb->prepare( "DELETE FROM %i WHERE id=%d", $lead_table, $lead_id );
		$wpdb->query( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
	...
	public static function delete_leads_by_form( $form_id, $status = '' ) {
		...
		$sql = $wpdb->prepare(
			" DELETE FROM %i
				WHERE lead_id IN (
					SELECT id FROM %i WHERE form_id=%d {$status_filter}
				)", $lead_detail_table, $lead_table, $form_id
		);
		$wpdb->query( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
		// phpcs:enable WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		...
		$sql = $wpdb->prepare(
			" DELETE FROM %i
				WHERE lead_id IN (
					SELECT id FROM %i WHERE form_id=%d {$status_filter}
				)", $lead_notes_table, $lead_table, $form_id
		);
		$wpdb->query( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
		// phpcs:enable WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		...
		$sql = $wpdb->prepare(
			" DELETE FROM %i
				WHERE lead_id IN (
					SELECT id FROM %i WHERE form_id=%d {$status_filter}
				)", $lead_meta_table, $lead_table, $form_id
		);
		$wpdb->query( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
		// phpcs:enable WordPress.DB.PreparedSQL.InterpolatedNotPrepared

		//Delete from lead
		$sql = $wpdb->prepare( "DELETE FROM %i WHERE form_id=%d {$status_filter}", $lead_table, $form_id ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$wpdb->query( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
	}
	...
	public static function delete_files_by_form( $form_id, $status = '' ) {
		 ...
		 */
		$field_types = gf_apply_filters( array( 'kdnaform_field_types_delete_files', $form_id ), $field_types, $form );


		$fields = KDNAFormsModel::get_fields_by_type( $form, $field_types );
		if ( empty( $fields ) ) {
			return;
		}
		...
		foreach ( $results as $result ) {
			KDNAFormsModel::delete_files( $result['id'], $form );
		}
	}
	...
	public static function delete_file( $entry_id, $field_id, $file_index = 0 ) {
		...
		$sql               = $wpdb->prepare( "SELECT id FROM %i WHERE lead_id=%d AND meta_key = %s", $lead_detail_table, $entry_id, $field_id );
		$entry_detail_id   = $wpdb->get_var( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching

		KDNAFormsModel::update_lead_field_value( $form, $entry, $field, $entry_detail_id, $field_id, $field_value );
	...
	private static function delete_physical_file( $file_url ) {
		...
		$file_path = KDNAFormsModel::get_physical_file_path( $url );
	...
	public static function get_lead( $lead_id ) {
		return KDNAAPI::get_entry( $lead_id );
	}
	...
	public static function delete_field_values( $form_id, $field_id ) {
		...
		// Delete from lead details
		$sql = $wpdb->prepare( "DELETE FROM %i WHERE form_id=%d AND field_number >= %d AND field_number < %d", $lead_detail_table, $form_id, $field_id, $field_id + 1 );
		...
		$sql = $wpdb->prepare(
			" DELETE FROM %i
                                WHERE form_id=%d
                                AND id NOT IN(
                                    SELECT DISTINCT(lead_id) FROM %i WHERE form_id=%d
                                )", $lead_table, $form_id, $lead_detail_table, $form_id
		);
		$wpdb->query( $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery
	}
	...
	public static function save_lead( $form, &$lead ) {
		...
		if ( $is_admin && ! KDNACommon::current_user_can_any( 'kdnaforms_edit_entries' ) ) {
			die( esc_html__( "You don't have adequate permission to edit entries.", 'kdnaforms' ) );
		}
		...
		if ( $is_new_lead ) {
			...
			$source_url = self::truncate( KDNAFormsModel::get_current_page_url(), 200 );
			 ...
			 *
			 */
			$currency = gf_apply_filters( array( 'kdnaform_currency_pre_save_entry', $form['id'] ), KDNACommon::get_currency(), $form );

			$wpdb->query( $wpdb->prepare( "INSERT INTO %i(form_id, ip, source_url, date_created, user_agent, currency, created_by) VALUES(%d, %s, %s, utc_timestamp(), %s, %s, {$user_id})", $lead_table, $form['id'], KDNAFormsModel::get_ip(), $source_url, $user_agent, $currency ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			...
			if ( $lead_id == 0 ) {
				...
				die( esc_html__( 'An error prevented the entry for this form submission being saved. Please contact support.', 'kdnaforms' ) );
			}
		...
		$current_fields   = $wpdb->get_results( $wpdb->prepare( "SELECT id, field_number FROM %i WHERE lead_id=%d", $lead_detail_table, $lead['id'] ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( $is_entry_detail || ! KDNAFormsModel::is_field_hidden( $form, $field, array(), $read_value_from_post ? null : $lead ) ) {
			...
			foreach ( $calculation_fields as $calculation_field ) {
				...
				if ( is_array( $inputs ) ) {
					foreach ( $inputs as $input ) {
						...
						KDNAFormsModel::refresh_lead_field_value( $lead['id'], $input['id'] );
					}
				} else {
					self::save_input( $form, $calculation_field, $lead, $current_fields, $calculation_field->id );
					KDNAFormsModel::refresh_lead_field_value( $lead['id'], $calculation_field->id );
				}
			}
			KDNAFormsModel::refresh_product_cache( $form, $lead = KDNAFormsModel::get_lead( $lead['id'] ) );
		}
		...
		if ( ! empty( $total_fields ) ) {
			foreach ( $total_fields as $total_field ) {
				...
				KDNAFormsModel::refresh_lead_field_value( $lead['id'], $total_field->id );
			}
		}
	...
	public static function save_input( $form, $field, &$lead, $current_fields, $input_id ) {
		...
		$value = KDNAFormsModel::maybe_trim_input( $value, $form['id'], $field );
		...
		$multiple_files = $field->multipleFiles;
		$uploaded_files = KDNAFormsModel::$uploaded_files;
		$form_id        = $form['id'];
		...
		if ( empty( $value ) && $field->is_administrative() && ! $is_admin ) {
			$value = KDNAFormsModel::get_default_value( $field, $input_id );
		}

		//processing values so that they are in the correct format for each input type
		$value = KDNAFormsModel::prepare_value( $form, $field, $value, $input_name, rgar( $lead, 'id' ) );
		...
		$lead_detail_id = KDNAFormsModel::get_lead_detail_id( $current_fields, $input_id );
		$result         = KDNAFormsModel::update_lead_field_value( $form, $lead, $field, $lead_detail_id, $input_id, $value );
		KDNACommon::log_debug( __METHOD__ . "(): Saving: {$field->label}(#{$input_id} - {$field->type}). Result: " . var_export( $result, 1 ) ); // phpcs:ignore QITStandard.PHP.DebugCode.DebugFunctionFound
	...
	public static function is_duplicate( $form_id, $field, $value ) {
		...
		if ( is_array( $field->get_entry_inputs() ) ) {
			...
			foreach ( $field->inputs as $input ) {
				...
				$inner_sql .= $union . $wpdb->prepare( $inner_sql_template, $input['id'], $form_id, $form_id, $input['id'] - 0.0001, $input['id'] + 0.0001, $value[ $input['id'] ] ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,
			}
		} else {
			$inner_sql = $wpdb->prepare( $inner_sql_template, $field->id, $form_id, $form_id, doubleval( $field->id ) - 0.0001, doubleval( $field->id ) + 0.0001, $value ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		}
		...
		return $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, 
			$wpdb->prepare(
				"  SELECT n.id, n.user_id, n.date_created, n.value, n.note_type, ifnull(u.display_name,n.user_name) as user_name, u.user_email
                                                    ...
                                                    WHERE lead_id=%d ORDER BY id", $notes_table, $lead_id
			)
		);
	}
	...
	public static function get_leads( $form_id, $sort_field_number = 0, $sort_direction = 'DESC', $search = '', $offset = 0, $page_size = 30, $star = null, $read = null, $is_numeric_sort = false, $start_date = null, $end_date = null, $status = 'active', $payment_status = false ) {
		...
		//initializing rownum
		$wpdb->query( 'select @rownum:=0' ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	...
	private static function sort_by_default_field_query( $form_id, $sort_field, $sort_direction = 'DESC', $search = '', $offset = 0, $page_size = 30, $star = null, $read = null, $is_numeric_sort = false, $start_date = null, $end_date = null, $status = 'active', $payment_status = false ) {
		...
		$grid_columns = KDNAFormsModel::get_grid_columns( $form_id );
	...
	public static function get_submitted_fields( $form_id ) {
		...
		$lead_detail_table_name = self::get_lead_details_table_name();
		$field_list             = '';
		$fields                 = $wpdb->get_results( $wpdb->prepare( "SELECT DISTINCT field_number FROM %i WHERE form_id=%d", $lead_detail_table_name, $form_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery
	...
	public static function search_lead_ids( $form_id, $search_criteria = array() ) {
		...
		$detail_table_name = KDNAFormsModel::get_lead_details_table_name();
		$lead_table_name   = KDNAFormsModel::get_lead_table_name();
	...
	public static function build_lead_array( $results ) {
		...
		//running entry through kdnaform_get_field_value filter
		foreach ( $leads as &$lead ) {
			foreach ( $form['fields'] as $field ) {
				...
				if ( is_array( $inputs ) ) {
					foreach ( $inputs as $input ) {
						$lead[ (string) $input['id'] ] = gf_apply_filters( array( 'kdnaform_get_input_value', $form['id'], $field->id, $input['id'] ), rgar( $lead, (string) $input['id'] ), $lead, $field, $input['id'] );
					}
				} else {

					$value = rgar( $lead, (string) $field->id );

					if ( KDNAFormsModel::is_openssl_encrypted_field( $lead['id'], $field->id ) ) {
						$value = KDNACommon::openssl_decrypt( $value );
					}

					$lead[ $field->id ] = gf_apply_filters( array( 'kdnaform_get_input_value', $form['id'], $field->id ), $value, $lead, $field, '' );
		...
		$entry_meta_data_rows = kdnaform_get_meta_values_for_entries( $entry_ids, $meta_keys );
		foreach ( $leads as &$lead ) {
	...
	private static function get_form_id_where( $form_id ) {
		...
		if ( is_array( $form_id ) ) {
			...
			$form_id_where = $wpdb->prepare( "l.form_id IN ($in_str)", $form_id ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
		} else {
	...
	private static function sort_by_field_query( $form_id, $search_criteria, $sorting, $paging ) {
		...
		$lead_detail_table_name = KDNAFormsModel::get_lead_details_table_name();
		$lead_table_name        = KDNAFormsModel::get_lead_table_name();
	...
	private static function sort_by_column_query( $form_id, $search_criteria, $sorting, $paging ) {
		...
		$lead_detail_table_name = KDNAFormsModel::get_lead_details_table_name();
		$lead_table_name        = KDNAFormsModel::get_lead_table_name();
		$lead_meta_table_name   = KDNAFormsModel::get_lead_meta_table_name();
	...
	private static function get_field_filters_where( $form_id, $search_criteria ) {
		...
		$sql_array               = array();
		$lead_details_table_name = KDNAFormsModel::get_lead_details_table_name();
		$lead_meta_table_name    = KDNAFormsModel::get_lead_meta_table_name();
		if ( is_array( $form_id ) ) {
			$in_str_arr    = array_fill( 0, count( $form_id ), '%d' );
			$in_str        = join( ',', $in_str_arr );
			$form_id_where = $wpdb->prepare( "AND form_id IN ($in_str)", $form_id ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
		} else {
			...
			switch ( $search_type ) {
				case 'field':
					...
					if ( $operator != 'like' && ! is_array( $form_id ) && $form_id > 0 ) {
						...
						if (  KDNAFormsModel::get_input_type( $field ) == 'number' ){
					...
					if ( is_array( $search_term ) ) {
						...
						$search_terms_in = $wpdb->prepare( '( ' . implode( ', ', $search_terms ) . ' )', $search_term ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
						...
						/* doesn't support "<>" for checkboxes */
						// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared
						$field_query = $wpdb->prepare( // phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber
							"
                        l.id IN
                        (
                        SELECT
				...
				case 'global':
					...
					$forms = array();
					if ( $form_id == 0 ) {
						$forms = KDNAAPI::get_forms();
					} elseif ( is_array( $form_id ) ) {
						foreach ( $form_id as $id ){
							$forms[] = KDNAAPI::get_form( $id );
						}
					} else {
								...
								if ( is_array( $field->choices ) ) {
									foreach ( $field->choices as $choice ) {
										if ( ( $operator == '=' && strtolower( $choice['text'] ) == strtolower( $val ) ) || ( $operator == 'like' && ! empty( $val ) && strpos( strtolower( $choice['text'] ), strtolower( $val ) ) !== false ) ) {
											...
											$choice_texts_clauses_for_field[] = $wpdb->prepare( "(field_number BETWEEN %s AND %s AND value {$choice_search_operator} %s)", (float) $field->id - 0.0001, (float) $field->id + 0.9999, $choice_value ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
										}
									}
								}
					...
					$choice_value_clause = $wpdb->prepare( "value {$operator} %s", $search_term ); //  phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	public static function get_entry_meta( $form_ids ) {
		...
		if ( $form_ids == 0 ) {
			$form_ids = KDNAFormsModel::get_form_ids();
		}
	...
	private static function get_info_search_where( $search_criteria ) {
		...
		$info_column_keys = KDNAFormsModel::get_lead_db_columns();
		array_push( $info_column_keys, 'id' );
		...
		foreach ( $field_filters as $filter ) {
			...
			$operator = KDNAFormsModel::is_valid_operator( rgar( $filter, 'operator' ) ) ? strtolower( $filter['operator'] ) : '=';
			...
			if ( 'date_created' == $key && '=' === $operator ) {
				...
				$where_array[] = $wpdb->prepare( "l.{$key} $operator %d", $search_term ); //  phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			} else {
				$where_array[] = $wpdb->prepare( "l.{$key} $operator %s", $search_term ); //  phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			}
		}
	...
	public static function kdnaform_get_meta( $entry_id, $meta_key ) {
		...
		$table_name                   = KDNAFormsModel::get_lead_meta_table_name();
		$results                      = $wpdb->get_results( $wpdb->prepare( "SELECT meta_value FROM {$table_name} WHERE lead_id=%d AND meta_key=%s", $entry_id, $meta_key ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	...
	public static function kdnaform_get_meta_values_for_entries( $entry_ids, $meta_keys ) {
		global $wpdb;
		...
		$results = $wpdb->get_results( $sql_query ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
	 ...
	 */
	public static function kdnaform_update_meta( $entry_id, $meta_key, $meta_value, $form_id = null ) {
		global $wpdb, $_kdnaform_lead_meta;
		...
		if ( $meta_exists ) {
			$wpdb->update( $table_name, array( 'meta_value' => $serialized_meta_value ), array( 'lead_id' => $entry_id, 'meta_key' => $meta_key ), array( '%s' ), array( '%d', '%s' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.SlowDBQuery.slow_db_query_meta_value, WordPress.DB.SlowDBQuery.slow_db_query_meta_key
		} else {

			if ( empty( $form_id ) ) {
				$lead_table_name = KDNAFormsModel::get_lead_table_name();
				$form_id         = $wpdb->get_var( $wpdb->prepare( "SELECT form_id from %i WHERE id=%d", $lead_table_name, $entry_id ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			} else {
				$form_id = intval( $form_id );
			}

			$wpdb->insert( $table_name, array( 'form_id' => $form_id, 'lead_id' => $entry_id, 'meta_key' => $meta_key, 'meta_value' => $serialized_meta_value ), array( '%d', '%d', '%s', '%s' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.SlowDBQuery.slow_db_query_meta_value, WordPress.DB.SlowDBQuery.slow_db_query_meta_key
		}
	 ...
	 * @param int|null $form_id The form ID of the entry (optional, saves extra query if passed when creating the metadata)
	 */
	public static function kdnaform_add_meta( $entry_id, $meta_key, $meta_value, $form_id = null ) {
		global $wpdb, $_kdnaform_lead_meta;
		$table_name = KDNAFormsModel::get_lead_meta_table_name();
		if ( false === $meta_value ) {
			$meta_value = '0';
		...
		if ( empty( $form_id ) ) {
			$lead_table_name = KDNAFormsModel::get_lead_table_name();
			$form_id         = $wpdb->get_var( $wpdb->prepare( "SELECT form_id from %i WHERE id=%d", $lead_table_name, $entry_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		} else {
			$form_id = intval( $form_id );
		}

		$wpdb->insert( $table_name, array( 'form_id' => $form_id, 'lead_id' => $entry_id, 'meta_key' => $meta_key, 'meta_value' => $serialized_meta_value ), array( '%d', '%d', '%s', '%s' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.SlowDBQuery.slow_db_query_meta_value, WordPress.DB.SlowDBQuery.slow_db_query_meta_key

		$cache_key                      = get_current_blog_id() . '_' . $entry_id . '_' . $meta_key;
		$_kdnaform_lead_meta[ $cache_key ] = $meta_value;
	}

	public static function kdnaform_delete_meta( $entry_id, $meta_key = '' ) {
		global $wpdb, $_kdnaform_lead_meta;
		$table_name  = KDNAFormsModel::get_lead_meta_table_name();
		$meta_filter = empty( $meta_key ) ? '' : $wpdb->prepare( 'AND meta_key=%s', $meta_key );

		$wpdb->query( $wpdb->prepare( "DELETE FROM %i WHERE lead_id=%d {$meta_filter}", $table_name, $entry_id ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	 ...
	 * @uses \KDNAAPI::form_id_exists
	 * @uses \KDNAFormsModel::get_ip
	 * @uses \KDNAFormsModel::get_current_page_url
	 * @uses \KDNACommon::get_currency
	 * @uses \KDNAFormsModel::get_lead_table_name
	 * @uses \KDNAFormsModel::get_lead_details_table_name
	 * @uses \KDNAFormsModel::get_form_meta
	 * @uses \KDNAFormsModel::get_input_type
	 * @uses \KDNA_Field::get_entry_inputs
	 * @uses \KDNAFormsModel::get_lead_detail_id
	 * @uses \KDNAFormsModel::update_lead_field_value
	...
	public static function update_entry( $entry, $entry_id = null ) {
		...
		if ( empty( $entry_id ) ) {
			return new WP_Error( 'missing_entry_id', __( 'Missing entry id', 'kdnaforms' ) );
		}

		$current_entry = $original_entry = KDNAFormsModel::get_entry( $entry_id );

		if ( ! $current_entry ) {
			return new WP_Error( 'not_found', __( 'Entry not found', 'kdnaforms' ), $entry_id );
		}
		...
		if ( false === self::form_id_exists( $form_id ) ) {
			return new WP_Error( 'invalid_form_id', __( 'The form for this entry does not exist', 'kdnaforms' ) );
		}
		 ...
		 */
		$entry = apply_filters( 'kdnaform_entry_pre_update', $entry, $original_entry );
		...
		$ip             = isset( $entry['ip'] ) ? $entry['ip'] : KDNAFormsModel::get_ip();
		$source_url     = isset( $entry['source_url'] ) ? $entry['source_url'] : KDNAFormsModel::get_current_page_url();
		$user_agent     = isset( $entry['user_agent'] ) ? $entry['user_agent'] : 'API';
		$currency       = isset( $entry['currency'] ) ? $entry['currency'] : KDNACommon::get_currency();
		...
		if ( false === $result ) {
			return new WP_Error( 'update_entry_properties_failed', __( 'There was a problem while updating the entry properties', 'kdnaforms' ), $wpdb->last_error );
		}
		...
		$lead_detail_table = KDNAFormsModel::get_lead_details_table_name();
		$current_fields    = $wpdb->get_results( $wpdb->prepare( "SELECT id, field_number FROM %i WHERE lead_id=%d", $lead_detail_table, $entry_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( is_array( $inputs ) ) {
				foreach ( $field->inputs as $input ) {
					...
					if ( isset( $entry[ $input_id ] ) ) {
						if ( $entry[ $input_id ] != $current_entry[ $input_id ] ) {
							$lead_detail_id = KDNAFormsModel::get_lead_detail_id( $current_fields, $input_id );
							$result         = KDNAFormsModel::update_lead_field_value( $form, $entry, $field, $lead_detail_id, $input_id, $entry[ $input_id ] );
							if ( false === $result ) {
								return new WP_Error( 'update_input_value_failed', __( 'There was a problem while updating one of the input values for the entry', 'kdnaforms' ), $wpdb->last_error );
							}
						}
				...
				if ( $field_value != $current_entry[ $field_id ] ) {
					$lead_detail_id = KDNAFormsModel::get_lead_detail_id( $current_fields, $field_id );
					$result         = KDNAFormsModel::update_lead_field_value( $form, $entry, $field, $lead_detail_id, $field_id, $field_value );
					if ( false === $result ) {
						return new WP_Error( 'update_field_values_failed', __( 'There was a problem while updating the field values', 'kdnaforms' ), $wpdb->last_error );
					}
				}
			...
			foreach ( array_keys( $entry_meta ) as $key ) {
				if ( isset( $entry[ $key ] ) ) {
					if ( $entry[ $key ] != $current_entry[ $key ] ) {
						kdnaform_update_meta( $entry_id, $key, $entry[ $key ] );
					}
		...
		if ( is_array( $entry_meta ) ) {
			foreach ( array_keys( $entry_meta ) as $meta_key ) {
				if ( isset( $current_entry[ $meta_key ] ) ) {
					kdnaform_delete_meta( $entry_id, $meta_key );
		...
		foreach ( $current_entry as $k => $v ) {
			$lead_detail_id = KDNAFormsModel::get_lead_detail_id( $current_fields, $k );
			$field          = KDNAFormsModel::get_field( $form, $k );
			$result         = KDNAFormsModel::update_lead_field_value( $form, $entry, $field, $lead_detail_id, $k, '' );
			if ( false === $result ) {
				return new WP_Error( 'update_field_values_failed', __( 'There was a problem while updating the field values', 'kdnaforms' ), $wpdb->last_error );
			}
		}
	...
	public static function form_id_exists( $form_id ) {
		global $wpdb;
		$form_table_name = KDNAFormsModel::get_form_table_name();
		$form_id         = intval( $form_id );
	...
	public static function add_entry( $entry ) {
		...
		if ( ! is_array( $entry ) ) {
			return new WP_Error( 'invalid_entry_object', __( 'The entry object must be an array', 'kdnaforms' ) );
		}

		// Make sure the form id exists.
		$form_id = rgar( $entry, 'form_id' );
		if ( empty( $form_id ) ) {
			return new WP_Error( 'empty_form_id', __( 'The form id must be specified', 'kdnaforms' ) );
		}
		...
		$is_read        = isset( $entry['is_read'] ) ? $entry['is_read'] : 0;
		$ip             = isset( $entry['ip'] ) ? $entry['ip'] : KDNAFormsModel::get_ip();
		$source_url     = isset( $entry['source_url'] ) ? $entry['source_url'] : esc_url_raw( KDNAFormsModel::get_current_page_url() );
		$user_agent     = isset( $entry['user_agent'] ) ? $entry['user_agent'] : 'API';
		$currency       = isset( $entry['currency'] ) ? $entry['currency'] : KDNACommon::get_currency();
		$payment_status = isset( $entry['payment_status'] ) ? sprintf( "'%s'", esc_sql( $entry['payment_status'] ) ) : 'NULL';
		...
		if ( false === $result ) {
			return new WP_Error( 'insert_entry_properties_failed', __( 'There was a problem while inserting the entry properties', 'kdnaforms' ), $wpdb->last_error );
		}
		// Reading newly created lead id.
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( is_array( $inputs ) ) {
				foreach ( $inputs as $input ) {
					$input_id = (string) $input['id'];
					if ( isset( $entry[ $input_id ] ) ) {
						$result = KDNAFormsModel::update_lead_field_value( $form, $entry, $field, 0, $input_id, $entry[ $input_id ] );
						if ( false === $result ) {
							return new WP_Error( 'insert_input_value_failed', __( 'There was a problem while inserting one of the input values for the entry', 'kdnaforms' ), $wpdb->last_error );
						}
					}
				}
			} else {
				$field_id    = $field->id;
				$field_value = isset( $entry[ (string) $field_id ] ) ? $entry[ (string) $field_id ] : '';
				$result      = KDNAFormsModel::update_lead_field_value( $form, $entry, $field, 0, $field_id, $field_value );
				if ( false === $result ) {
					return new WP_Error( 'insert_field_values_failed', __( 'There was a problem while inserting the field values', 'kdnaforms' ), $wpdb->last_error );
				}
			}
		}
		...
		if ( is_array( $entry_meta ) ) {
			foreach ( array_keys( $entry_meta ) as $key ) {
				if ( isset( $entry[ $key ] ) ) {
					kdnaform_update_meta( $entry_id, $key, $entry[ $key ], $form['id'] );
				}
			}
		}
		 ...
		 */
		do_action( 'kdnaform_post_add_entry', $entry, $form );
	...
	public static function update_entry_field( $entry_id, $input_id, $value ) {
		...
		$lead_detail_id = $wpdb->get_var( $wpdb->prepare( "SELECT id FROM %i WHERE lead_id=%d AND field_number BETWEEN %s AND %s", $lead_details_table_name, $entry_id, $input_id_min, $input_id_max ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching

		$result = true;
		if ( ! isset( $entry[ $input_id ] ) || $entry[ $input_id ] != $value ){
			$result = KDNAFormsModel::update_lead_field_value( $form, $entry, $field, $lead_detail_id, $input_id, $value );
		}
	...
	public static function update_lead_field_value( $form, $lead, $field, $lead_detail_id, $input_id, $value ) {
		 ...
		 */
		$value = apply_filters( 'kdnaform_save_field_value', $value, $lead, $field, $form, $input_id );
		$value = apply_filters( "kdnaform_save_field_value_{$form['id']}", $value, $lead, $field, $form, $input_id );

		if ( is_object( $field ) ) {
			$value = apply_filters( "kdnaform_save_field_value_{$form['id']}_{$field->id}", $value, $lead, $field, $form, $input_id );
		}
		...
		if ( ! rgblank( $value ) ) {
			...
			if ( $lead_detail_id > 0 ) {
				...
				$result = $wpdb->update( $lead_detail_table, array( 'value' => $value ), array( 'id' => $lead_detail_id ), array( '%s' ), array( '%d' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
				if ( false === $result ) {
					return false;
				}

			} else {
				$result = $wpdb->insert( $lead_detail_table, array( 'lead_id' => $lead_id, 'form_id' => $form_id, 'field_number' => $input_id, 'value' => $value ), array( '%d', '%d', '%F', '%s' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
				if ( false === $result ) {
					return false;
				}
			...
			$sql    = $wpdb->prepare( "DELETE FROM %i WHERE lead_id=%d AND field_number BETWEEN %s AND %s ", $lead_detail_table, $lead_id, doubleval( $input_id ) - 0.0001, doubleval( $input_id ) + 0.0001 );
			$result = $wpdb->query( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
			if ( false === $result ) {
				return false;
			}
	...
	public static function add_note( $lead_id, $user_id, $user_name, $note, $note_type = 'note' ) {
		...
		$sql        = $wpdb->prepare( "INSERT INTO %i(lead_id, user_id, user_name, value, note_type, date_created) values(%d, %d, %s, %s, %s, utc_timestamp())", $table_name, $lead_id, $user_id, $user_name, $note, $note_type );
		 ...
		 */
		do_action( 'kdnaform_post_note_added', $wpdb->insert_id, $lead_id, $user_id, $user_name, $note, $note_type );
	}
	...
	public static function delete_note( $note_id ) {
		...
		$lead_id = $wpdb->get_var( $wpdb->prepare( "SELECT lead_id FROM %i WHERE id = %d", $table_name, $note_id ) );  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	...
	public static function get_lead_count_all_forms( $status = 'active' ) {
		...
		$result = $wpdb->get_var( "SHOW COLUMNS FROM $lead_table_name LIKE 'status'" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching

		if ( $result ) {
			$sql = $wpdb->prepare( "SELECT count(id)
								FROM %i
								WHERE status=%s", $lead_table_name, $status );
		} else {
