<?php
/**
 * Handles logic for adjusting KDNA Forms to different environment configurations.
 *
...
/**
 * Class KDNA_Environment_Config_Handler
 *
 * @since 2.6.7
 *
...
class KDNA_Environment_Config_Handler {
	...
	/**
	 * KDNA_Environment_Config_Handler constructor.
	 *
	 * @since 2.6.7
	 *
	 * @param \KDNACache $cache Provides caching service.
	 */
	...
	public function get_support_url() {
		return $this->get_environment_setting( 'support_url', 'https://www.kdnaforms.com/open-support-ticket/' );
	}
	...
	/**
	 * Target of the pre_option_kdnaform_pending_installation filter. Bypasses the installation wizard by returning 0 for the kdnaform_pending_installation option.
	 *
	 * @hook pre_option_kdnaform_pending_installation 10, 1
	 *
	 ...
	 */
	public function maybe_override_kdnaform_pending_installation() {

		// If environment config is set to hide install wizard, override kdnaform_pending_intallation option with 0. Otherwise, use existing option.
	...
	public function maybe_hide_plugin_page_message() {
		...
		if ( $this->get_hide_update_message() ) {
			remove_filter( 'transient_update_plugins', array( 'KDNAForms', 'check_update' ) );
			remove_filter( 'site_transient_update_plugins', array( 'KDNAForms', 'check_update' ) );
		}
	}
	...
	/**
	 * Target of the pre_option_kdna_forms_key filter. Uses the license key configured by the environment settings if one is set.
	 *
	 * @hook pre_option_kdna_forms_key 10, 1
	 *
	 * @since 2.6.7
	 *
	 * @return string Returns the environment license key if one is set. If not set, return false so that value is not overridden.
	 */
	public function maybe_override_kdna_forms_key() {

		// Use environment license key if one is set. Otherwise, use kdna_forms_key option.
	...
	/**
	 * Target of the kdnaform_plugin_settings_fields filter. Removes sections from the settings page that are configured to be hidden.
	 *
	 * @since 2.6.9
	 *
