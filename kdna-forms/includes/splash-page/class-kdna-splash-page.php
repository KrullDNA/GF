...
use \KDNAForms;
use \KDNACommon;
use \KDNA_Forms\KDNA_Forms\Splash_Page_Template_Tags;

/**
 * Class KDNA_Splash_Page
 *
 * @since 2.6
 *
 * Displays a splash page when a user updates to a new version.
...
class KDNA_Splash_Page {
	 ...
	 *
	 * @var Splash_Page_Template_Tags\KDNA_Splash_Page_Template_Tags
	 */
	protected $tags;
	...
	/**
	 * KDNA_Splash_Page_Constructor
	 *
	 * @param Splash_Page_Template_Tags\KDNA_Splash_Page_Template_Tags $tags
	 */
	public function __construct( $tags ) {
		...
		$this->img_dir = 'https://cdn.gravity.com/kdnaforms/about-page/2.9/';
	}
	...
	public function admin_title( $title ) {
		if ( $this->is_splash_page() ) {
			...
			$title = sprintf( __( 'About %1$s &lsaquo; System Status &lsaquo; KDNA Forms &#8212; WordPress', 'kdnaforms' ), $this->about_version );
		}
		return $title;
	}
	...
	public function set_upgrade_transient( $version, $from_db_version, $force_upgrade ) {
		...
		if ( $this->need_splash_page( $from_db_version, KDNAForms::$version, $this->about_version ) ) {
	...
	public function system_status_link( $subviews ) {
		$subviews[19] = array(
			'name'  => 'about',
			'label' => sprintf( __( 'About %s', 'kdnaforms' ), $this->about_version ),
		);
	...
	public function should_display() {
		return $this->about_version == get_transient( 'gf_updated' ) && KDNAForms::is_kdna_page() && KDNACommon::current_user_can_any( KDNACommon::all_caps() );
	}
