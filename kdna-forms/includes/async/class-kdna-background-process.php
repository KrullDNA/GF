...
/**
 * Abstract KDNA_Background_Process class.
 *
 ...
 */
abstract class KDNA_Background_Process extends WP_Async_Request {
	...
	public function __construct( $allowed_batch_data_classes = true ) {
		...
		add_action( 'kdnaform_uninstalling', array( $this, 'uninstall' ) );
	}
	...
	protected function can_process_task( $task, $batch, $task_num ) {
		 ...
		 */
		$max_attempts = apply_filters( 'kdnaform_max_async_task_attempts', $max_attempts, $task, $batch, $identifier );
	...
	public function schedule_cron_healthcheck( $schedules ) {
		...
		if ( 1 === $interval ) {
			$display = __( 'Every Minute', 'kdnaforms' );
		} else {
			$display = sprintf( __( 'Every %d Minutes', 'kdnaforms' ), $interval );
		}
	...
	public function handle_cron_healthcheck() {
		$this->log_debug( sprintf( '%s(): Running%s.', __METHOD__, $this->get_action_for_log() ) );
		KDNACommon::record_cron_event( $this->cron_hook_identifier );
	...
	public function filter_form( $form, $entry ) {
		return gf_apply_filters( array(
			'kdnaform_form_pre_process_async_task',
			absint( rgar( $form, 'id' ) ),
		), $form, $entry );
	}
