...
class KDNA_Form_CRUD_Handler {
	 ...
	 * @var \KDNAFormsModel
	 */
	private $kdna_forms_model;

	/**
	 * Holds an instance of the KDNACommon class.
	 *
	 * @since 2.6
	 *
	 * @var \KDNACommon
	...
	/**
	 * Holds an instance of the KDNAAPI class.
	 *
	 * @since 2.6
	 *
	 * @var \KDNAAPI
	...
	/**
	 * KDNA_Form_CRUD_Handler constructor.
	 *
	 * @since 2.6
	 *
	...
	public function save( $form_id, $form_json ) {
		...
		ob_start();
			 ...
			 */
			do_action( 'kdnaform_after_save_form', $this->form_meta, rgar( $save_result, 'is_new' ), $this->deleted_fields );

		$after_save_markup = ob_get_clean();

		$save_result['actions_markup'] = array(
			'kdnaform_after_save_form' => $after_save_markup,
		);
	...
	private function validate() {
		...
		$forms = $kdna_forms_model::get_forms();
	...
	private function cleanup() {
		...
		$this->form_meta = $kdna_forms_model::convert_field_objects( $this->form_meta );

		// Set version of KDNA Forms form was created with.
		if ( $this->form_id === 0 ) {
			$this->form_meta['version'] = $kdna_forms::$version;
		}
	...
	private function update() {
			...
			$this->form_meta = $kdna_forms_model::trim_form_meta_values( $this->form_meta );
			$deleted_fields  = $this->get_deleted_fields();
			// Delete fields.
		if ( ! empty( $deleted_fields ) ) {
			foreach ( $deleted_fields as $deleted_field_id ) {
				$this->form_meta = $kdna_forms_model::delete_field( $this->form_meta, $deleted_field_id, false );
			}
		}
		...
		if ( ! empty( $deleted_fields ) ) {
			...
			foreach ( $deleted_fields as $deleted_field ) {
				$this->form_meta = $kdna_forms_model::delete_field_from_confirmations( $this->form_meta, $deleted_field );
				$this->form_meta = $kdna_forms_model::delete_field_from_notifications( $this->form_meta, $deleted_field );
			}
		}
	...
	private function insert() {
			...
			$kdna_forms_model::save_form_confirmations( $this->form_id, $confirmations );
			...
			// Get form meta.
			$this->form_meta = $kdna_forms_model::get_form_meta( $this->form_id );
	...
	private function get_default_notification() {
		if ( ! apply_filters( 'kdnaform_default_notification', true ) ) {
			return array();
		}
		...
		$default_notification = array(
			...
			'isActive' => true,
			'to'       => '{admin_email}',
			'name'     => __( 'Admin Notification', 'kdnaforms' ),
			'event'    => 'form_submission',
			'toType'   => 'email',
			'subject'  => __( 'New submission from', 'kdnaforms' ) . ' {form_title}',
			'message'  => '{all_fields}',
		);
	...
	private function get_default_confirmation() {
		...
		$confirmation  = $kdna_forms_model::get_default_confirmation();
