 ...
 */
class KDNA_Save_Form_Endpoint_Admin {
	 ...
	 * @since 2.6
	 *
	 * @var KDNA_Form_CRUD_Handler
	 */
	protected $form_crud_handler;

	/**
	 * An instance of KDNAFormsModel to call common static functions.
	 *
	 * @since 2.6
	 *
	 * @var \KDNAFormsModel
	 */
	protected $kdna_forms_model;

	/**
	 * An instance of KDNAForms to call common static functions.
	 *
	 * @since 2.6
	 *
	 * @var \KDNAForms
	...
	/**
	 * KDNA_Save_Form_Endpoint_Admin constructor.
	 *
	 * @since 2.6
	 *
	...
	public function __construct( $dependencies ) {
		$this->form_crud_handler = rgar( $dependencies, 'kdna_form_crud_handler' );
		$this->kdna_forms_model    = rgar( $dependencies, 'kdna_forms_model' );
		$this->kdna_forms          = rgar( $dependencies, 'kdna_forms' );
	}
	...
	protected function get_error_status_response( $result ) {
		$status = rgar( $result, 'status', KDNA_Form_CRUD_Handler::STATUS_FAILURE );

		if ( $status === KDNA_Form_CRUD_Handler::STATUS_DUPLICATE_TITLE ) {
			$result['error'] = esc_html__( 'Please enter a unique form title, this title is used for an existing form.', 'kdnaforms' );
