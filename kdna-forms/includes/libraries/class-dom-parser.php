...
namespace KDNA_Forms\KDNA_Forms\Libraries;
...
class Dom_Parser {
	...
	public function get_injected_html() {
		...
		$has_printed = \KDNAFormDisplay::$hooks_js_printed;
	...
	private function get_scripts( $for_output = false ) {
		...
		if ( empty( $scripts ) ) {
			$script  = \KDNACommon::get_hooks_javascript_code( false );
			$scripts = array(
				\KDNACommon::get_inline_script_tag( $script, false ),
				\wp_get_inline_script_tag( $script, array( 'id' => 'kdnaform_kdnaforms-js-before' ) ),
				$script,
			);
		}
	...
	public function is_parseable_request( $check_empty = true ) {
		 ...
		 *
		 * Example:
		 * add_filter( 'kdnaform_disable_dom_parser', '__return_true' ); // Disables the Dom parser on every page.
		 */
 		$is_disabled = apply_filters( 'kdnaform_disable_dom_parser', false );
		return ! $is_disabled;
	}
