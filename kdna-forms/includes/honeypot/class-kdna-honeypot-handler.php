...
class KDNA_Honeypot_Handler {
	...
	/**
	 * Target of the kdnaform_entry_is_spam filter. Checks entry for honeypot validation and returns true or false depending on the result.
	 *
	 * @since 2.7
	...
	public function handle_entry_is_spam( $is_spam, $form ) {
		...
		if ( $is_spam ) {
			...
			$message = rgar( $result, 'message' ) ?: __( 'Failed Honeypot Validation.', 'kdnaforms' );
			\KDNACommon::set_spam_filter( $form_id, __( 'Honeypot Spam Filter', 'kdnaforms' ), $message );
		}
	...
	/**
	 * Target of the kdnaform_abort_submission_with_confirmation filter. Aborts form submission early with a confirmation when honeypot fails and it is configured not to create an entry.
	 *
	 * @since 2.7
	...
	public function handle_abort_submission( $do_abort, $form ) {
		...
		$saving_for_later = (bool) rgpost( 'kform_save' );
		...
		\KDNACommon::log_debug( __METHOD__ . '(): Result from Honeypot: ' . json_encode( $do_abort ) );

		if ( $do_abort ) {
			\KDNAFormDisplay::$submission[ (int) rgar( $form, 'id' ) ]['is_spam'] = true;
		}
	...
	/**
	 * Target of the kdnaform_after_submission. Clears the cached results.
	 *
	 * @since 2.7
	 *
	...
	public function handle_after_submission( $entry, $form ) {
		\KDNACache::delete( "honeypot_{$form['id']}" );
	}
	...
	public function get_cached_result( $form_id ) {
		return \KDNACache::get( "honeypot_{$form_id}", $found, false );
	}
	...
	public function cache_result( $form_id, $result ) {
		\KDNACache::set( "honeypot_{$form_id}", $result );
	}
	...
	public function validate_honeypot( $form ) {
		...
		\KDNACommon::log_debug( __METHOD__ . sprintf( '(): Is honeypot input (name: %s) empty? %s', $input_name, ( $is_field_input_empty ? 'Yes.' : 'No.' ) ) );

		if ( ! $is_field_input_empty ) {
			\KDNACommon::log_debug( __METHOD__ . '(): Is submission valid? No.' );
			$result['message'] = __( 'The honeypot input is not empty.', 'kdnaforms' );
		...
		if ( $this->is_api_submission() ) {
			\KDNACommon::log_debug( __METHOD__ . '(): Submission initiated by KDNAAPI. version_hash validation and speed check bypassed.' );
			\KDNACommon::log_debug( __METHOD__ . '(): Is submission valid? Yes.' );
		...
		if ( empty( $version_hash ) ) {
			\KDNACommon::log_debug( __METHOD__ . '(): Is submission valid? No; version_hash input is empty.' );
			$result['message'] = __( 'The version_hash was not included in the submission.', 'kdnaforms' );
			$this->cache_result( $form_id, $result );
		...
		\KDNACommon::log_debug( __METHOD__ . '(): Is version_hash input valid? ' . ( $is_version_hash_valid ? 'Yes.' : 'No.' ) );

		if ( ! $is_version_hash_valid ) {
			\KDNACommon::log_debug( __METHOD__ . '(): Is submission valid? No.' );
			$result['message'] = __( 'The wrong value was submitted for the version_hash.', 'kdnaforms' );
			$this->cache_result( $form_id, $result );
	...
	public function get_honeypot_field_id( $form ) {
		...
		$honeypot_field = \KDNAFormsModel::get_fields_by_type( $form, array( 'honeypot' ) );
		if ( count( $honeypot_field ) > 0 ) {
			return $honeypot_field[0]->id;
		}

		// If no honeypot field in the form, return the largest field ID + 1.
		return \KDNAFormsModel::get_next_field_id( $form['fields'] );
	 ...
	 * @param null|int $id   The ID of the honeypot field, or null to use KDNA_Honeypot_Handler::get_honeypot_field_id().
	 *
	 * @return string
	 */
	public function get_input_name( $form, $id = null ) {
		 ...
		 * @param array  $form The form being rendered or validated.
		 */
		return apply_filters( 'kdnaform_honeypot_input_name', $name, $form );
	}
	 ...
	 *
	 * @return bool|\KDNA_Field Returns the honeypot field.
	 */
	public function get_honeypot_field( $form ) {
		...
		$field_data = array(
			...
			'cssClass'    => 'kdnaform_validation_container',
			'description' => __( 'This field is for validation purposes and should be left unchanged.', 'kdnaforms' ),
			'formId'      => absint( rgar( $form, 'id' ) ),
		);

		return \KDNA_Fields::create( $field_data );
	}
	...
	private function get_honeypot_labels() {
		 ...
		 */
		return apply_filters( 'kdnaform_honeypot_labels_pre_render', $honeypot_labels );
	}
	 ...
	 *
	 * @return bool Returns true if the hash is validated against the current and previous version of KDNA Forms
	 */
	...
	public function is_honeypot_enabled( $form ) {
		...
		$is_disabled     = ! rgar( $form, 'enableHoneypot' ) || \KDNACommon::is_preview() || $is_wp_dashboard;

		return ! $is_disabled;
	}

	/**
	 * Determines if the current form submission was initiated via KDNAAPI.
	 *
	 * @since 2.7
	 *
	 * @return bool True if the current form submission was initiated via KDNAAPI. False otherwise.
	 */
	public function is_api_submission() {
		return \KDNAFormDisplay::$submission_initiated_by == \KDNAFormDisplay::SUBMISSION_INITIATED_BY_API;
	}
	...
	public function is_valid_submission_speed( $form ) {
		...
		if ( ! $this->is_speed_check_enabled( $form ) ) {
			\KDNACommon::log_debug( __METHOD__ . '(): Submission speed check is disabled.' );
		...
		if ( empty( $speeds ) ) {
			\KDNACommon::log_debug( __METHOD__ . '(): Is speed check valid? No; kdnaform_submission_speeds input is empty or invalid.' );
			$result['is_valid'] = false;
			$result['message']  = __( 'The kdnaform_submission_speeds input is empty or invalid.', 'kdnaforms' );
		...
		\KDNACommon::log_debug( __METHOD__ . sprintf( '(): Is speed check valid? %s; %d of %d submissions met the threshold (%d ms). Min required: %d. All speeds: %s', ( $is_valid ? 'Yes' : 'No' ), $counts['valid_count'], $counts['total'], $threshold, $min_count, json_encode( $speeds ) ) );
		...
		if ( ! $is_valid ) {
			...
			$result['message'] = sprintf( __( '%1$d of %2$d submissions met the speed check threshold (%3$d ms). Min required: %4$d. All speeds (ms): %5$s.', 'kdnaforms' ), $counts['valid_count'], $counts['total'], $threshold, $min_count, $this->format_submission_speeds_for_note( $speeds ) );
	...
	/**
	 * Parses and sanitizes the submission speeds from the kdnaform_submit_speeds input for the current form.
	 *
	 * @since 2.9.21
	 *
	 * @param int $form_id The current form ID.
	...
	public function get_submission_speeds_array( $form_id ) {
		...
		$json = rgpost( 'kform_submission_speeds' );
		if ( empty( $json ) ) {
			return array();
		}
	 ...
	 *
	 * Callback for the kdnaform_entry_meta filter added via KDNA_Honeypot_Service_Provider::init().
	 *
	 * @since 2.9.21
	 *
	...
	public function submission_speeds_entry_meta( $entry_meta ) {
		$entry_meta['submission_speeds'] = array(
			'label'                      => esc_html__( 'Submission Speed (ms)', 'kdnaforms' ),
			'is_numeric'                 => false,
			'update_entry_meta_callback' => function ( $key, $entry, $form ) {
				$existing = rgar( $entry, 'submission_speeds' );
				if ( \KDNACommon::is_entry_detail_edit() || ! rgblank( $existing ) ) {
					return $existing;
				}
	 ...
	 *
	 * Callback for the kdnaform_entries_field_value filter added via KDNA_Honeypot_Service_Provider::init().
	 *
	 * @since 2.9.21
	 *
	 ...
	 *
	 * Callback for the kform_entry_detail_meta_boxes filter added via KDNA_Honeypot_Service_Provider::init().
	 *
	 * @since 2.9.21
	 *
	...
	public function submission_speeds_entry_detail_meta_box( $meta_boxes, $entry ) {
		...
		$meta_boxes['submission_speeds'] = array(
			'title'    => esc_html__( 'Submission Speed (ms)', 'kdnaforms' ),
