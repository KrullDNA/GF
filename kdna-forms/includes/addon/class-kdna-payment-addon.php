 ...
 *
 * @package KDNAPaymentAddOn
 */

// If KDNA Forms doesn't exist, bail.
if ( ! class_exists( 'KDNAForms' ) ) {
	die();
}

// Require KDNAFeedAddOn.
require_once( 'class-kdna-feed-addon.php' );
 ...
 *
 * Used to extend KDNA Forms. Specifically, payment add-ons.
 *
 * @since Unknown
 *
 * @uses KDNAFeedAddOn
...
abstract class KDNAPaymentAddOn extends KDNAFeedAddOn {
	...
	/**
	 * Defines the version of KDNAPaymentAddOn.
	 *
	 * @since  Unknown
	 * @access private
	 *
	 * @used-by KDNAPaymentAddOn::setup()
	 *
	 ...
	 * @access protected
	 *
	 * @used-by KDNAPaymentAddOn::before_delete_field()
	 * @used-by KDNAPaymentAddOn::feed_list_message()
	 * @used-by KDNAPaymentAddOn::init_admin()
	 *
	 ...
	 * @access protected
	 *
	 * @used-by KDNAPaymentAddOn::upgrade_payment()
	 *
	 ...
	 * @access protected
	 *
	 * @used-by KDNAPaymentAddOn::entry_post_save()
	 * @used-by KDNAPaymentAddOn::validation()
	 *
	 * @var array
	 */
	 ...
	 * @access protected
	 *
	 * @used-by KDNAPaymentAddOn::confirmation()
	 * @used-by KDNAPaymentAddOn::entry_post_save()
	 *
	 ...
	 *
	 * @used-by KDNAPaymentAddOn::entry_post_save()
	 * @used-by KDNAPaymentAddOn::is_payment_gateway()
	 * @used-by KDNAPaymentAddOn::is_payment_gateway()
	 *
	 ...
	 * @access protected
	 *
	 * @used-by KDNAFeedAddOn::maybe_process_feed()
	 *
	 ...
	 * @access protected
	 *
	 * @used-by KDNAPaymentAddOn::get_amount_export()
	 * @used-by KDNAPaymentAddOn::get_amount_import()
	 *
	 ...
	 * @access public
	 *
	 * @used-by KDNAAddOn::__construct()
	 * @uses    KDNAAddOn::pre_init()
	 * @uses    KDNAPaymentAddOn::payment_method_is_overridden()
	 * @uses    KDNAPaymentAddOn::setup_cron()
	 * @uses    KDNAPaymentAddOn::maybe_process_callback()
	 *
	 * @return void
	 */
	 ...
	 * @access public
	 *
	 * @uses KDNAFeedAddOn::init()
	 * @uses KDNAPaymentAddOn::confirmation()
	 * @uses KDNAPaymentAddOn::maybe_validate()
	 * @uses KDNAPaymentAddOn::entry_post_save()
	 *
	 * @return void
	 */
	public function init() {
		...
		add_filter( 'kdnaform_confirmation', array( $this, 'confirmation' ), 20, 4 );

		add_filter( 'kdnaform_validation', array( $this, 'maybe_validate' ), 20, 2 );
		add_filter( 'kdnaform_entry_post_save', array( $this, 'entry_post_save' ), 11, 2 );

		if ( $this->_requires_credit_card ) {
			add_filter( 'kdnaform_register_init_scripts', array( $this, 'register_creditcard_token_script' ), 10, 3 );
			add_filter( 'kdnaform_field_content', array( $this, 'add_creditcard_token_input' ), 10, 5 );
			add_filter( 'kdnaform_form_args', array( $this, 'force_ajax_for_creditcard_tokens' ), 10, 1 );
		}

		add_filter( 'kdnaform_is_delayed_pre_process_feed', array( $this, 'maybe_delay_feed_processing' ), 1, 4 );

		// Maybe support payment status in Confirmation conditional logic.
		add_filter( 'kdnaform_entry_meta_conditional_logic_confirmations', function( $entry_meta, $form ) {
		...
		add_filter( 'kdnaform_entry_meta_pre_render_feed_settings', function( $entry_meta, $form ) {
            return $this->maybe_add_payment_status_to_meta( $entry_meta, $form, 'feed' );
        }, 10, 2 );


		add_filter( 'kdnaform_entry_meta_pre_evaluate_conditional_logic', function( $entry_meta, $form ) {
            return $this->maybe_add_payment_status_to_meta( $entry_meta, $form, 'pre_evaluate' );
        }, 10, 2 );

        // Trigger payment status change when payment_status is updated via kdnaform_update_payment_status or kdnaform_post_update_entry.
		add_action( "kdnaform_update_payment_status", function( $entry_id, $property_value, $previous_value ) {
            $this->payment_status_changed( $entry_id, $previous_value );
		}, 10, 3 );

		add_action( 'kdnaform_post_update_entry', function( $entry, $original_entry ) {
	 ...
	 * @access public
	 *
	 * @uses KDNAFeedAddOn::init_admin()
	 * @uses KDNAPaymentAddOn::$_requires_credit_card
	 * @uses KDNAPaymentAddOn::supported_currencies()
	 * @uses KDNAPaymentAddOn::entry_deleted()
	 * @uses KDNAPaymentAddOn::entry_info()
	 *
	 * @return void
	 */
	public function init_admin() {
		...
		if ( $this->_requires_credit_card ) {
			...
			add_filter( 'kdnaform_enable_credit_card_field', '__return_true' );
		}

		add_filter( 'kdnaform_currencies', array( $this, 'supported_currencies' ) );

		add_filter( 'kdnaform_delete_lead', array( $this, 'entry_deleted' ) );
		add_action( 'kdnaform_before_delete_field', array( $this, 'before_delete_field' ), 10, 2 );

		if ( KDNAForms::get_page_query_arg() == 'gf_entries' ) {
			add_action( 'kdnaform_payment_details', array( $this, 'entry_info' ), 10, 2 );
			if ( ! has_filter( 'kdnaform_entries_filter_count_queries', array( __CLASS__, 'entries_filter_count_queries', ) ) ) {
				add_filter( 'kdnaform_entries_filter_count_queries', array( __CLASS__, 'entries_filter_count_queries' ) );
			}
			if ( ! has_filter( 'kdnaform_filter_links_entry_list', array( __CLASS__, 'filter_links_entry_list' ) ) ) {
				add_filter( 'kdnaform_filter_links_entry_list', array( __CLASS__, 'filter_links_entry_list' ), 90, 4 );
			}
		}
	}
	 ...
	 * @access public
	 *
	 * @uses KDNAFeedAddOn::init_ajax()
	 * @uses KDNAPaymentAddOn::ajax_cancel_subscription()
	 * @uses KDNAPaymentAddOn::before_delete_field()
	 *
	 * @return void
	 */
	 ...
	 * @access public
	 *
	 * @uses KDNAFeedAddOn::setup()
	 * @uses KDNAPaymentAddOn::upgrade_payment()
	 * @uses KDNAAddOn::$_slug
	 * @uses KDNAPaymentAddOn::$_payment_version
	 *
	 * @return void
	 */
	public function setup() {
		...
		$installed_version = get_option( 'kdnaformsaddon_payment_version' );


		$installed_addons = get_option( 'kdnaformsaddon_payment_addons' );
		...
		if ( $installed_version != $this->_payment_version ) {
			...
			update_option( 'kdnaformsaddon_payment_addons', $installed_addons );
		}
		...
		update_option( 'kdnaformsaddon_payment_version', $this->_payment_version );
	 ...
	 * @access private
	 *
	 * @uses KDNAFormsModel::dbDelta()
	 * @uses KDNAPaymentAddOn::$_supports_callbacks
	 * @uses KDNAForms::drop_index()
	 *
	 * @global $wpdb
	...
	private function upgrade_payment( $previous_versions ) {
		...
		$charset_collate = KDNAFormsModel::get_db_charset();

		$sql = "CREATE TABLE {$wpdb->prefix}kdna_addon_payment_transaction (
		...
		if ( $this->_supports_callbacks ) {
			$sql = "CREATE TABLE {$wpdb->prefix}kdna_addon_payment_callback (
                      id int(10) unsigned not null auto_increment,
                      lead_id int(10) unsigned not null,
			...
			kdna_upgrade()->drop_index( "{$wpdb->prefix}kdna_addon_payment_callback", 'slug_callback_id' );
		}
	...
	/**
	 * Gets called when KDNA Forms upgrade process is completed. This function is intended to be used internally, override the upgrade() function to execute database update scripts.
	 * @param $db_version - Current KDNA Forms database version
	 * @param $previous_db_version - Previous KDNA Forms database version
	 ...
	 */
	public function post_kdnaforms_upgrade( $db_version, $previous_db_version, $force_upgrade ){
		...
		parent::post_kdnaforms_upgrade( $db_version, $previous_db_version, $force_upgrade );
	}
	 ...
	 * @param string $slug       The Add-On slug e.g. kdnaformsmailchimp
	 *
	 * @return bool
	 */
    ...
    /**
     * Reprocess feeds and triggers kdnaform_post_payment_status_change hook when the payment status changes.
     *
     * @since 2.9.20
     *
    ...
    public function payment_status_changed( $entry_id, $previous_status ) {
        ...
        if ( has_filter( 'kdnaform_post_payment_status_change' ) ) {
            $this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_post_payment_status_change.' );
             ...
             */
            do_action( 'kdnaform_post_payment_status_change', $entry, $previous_status );
        }
    }
	...
	public function trigger_payment_delayed_feeds( $transaction_id, $payment_feed, $entry, $form ) {
		if ( has_filter( 'kdnaform_trigger_payment_delayed_feeds' ) ) {
			$this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_trigger_payment_delayed_feeds.' );

			/**
			 * Used in KDNAFeedAddOn to trigger processing of feeds delayed until payment is completed.
			 *
			 * @since 2.4.13
			 *
			 * @param string $transaction_id The transaction or subscription ID.
			 ...
			 * @param array  $form           The form currently being processed.
			 */
			do_action( 'kdnaform_trigger_payment_delayed_feeds', $transaction_id, $payment_feed, $entry, $form );
		}
	}
	...
	public function get_post_payment_actions_config( $feed_slug ) {
		...
		if ( $this->get_slug() === 'kdnaformspaypal' ) {
			$config = array(
				'position' => 'after',
				'setting'  => 'options',
			);
	 ...
	 * @access public
	 *
	 * @uses KDNAPaymentAddOn::$redirect_url
	 *
	...
	public function maybe_validate( $validation_result, $context = 'api-submit' ) {
		...
		$is_last_page    = KDNAFormDisplay::is_last_page( $form );
	...
	public function maybe_add_payment_status_to_meta( $entry_meta, $form, $context = '' ) {
		...
		if ( rgempty( 'payment_status', $entry_meta ) ) {
			$entry_meta['payment_status'] = array(
				'label'      => esc_html__( 'Payment Status', 'kdnaformsstripe' ),
				'is_numeric' => false,
				'filter'     => array(
	 ...
	 * Example:
	 * return array(
	 *    'Paid' => __( 'Paid', 'kdnaforms' ),
	 *    'Failed' => __( 'Failed', 'kdnaforms' ),
	 *    'Processing' => __( 'Processing', 'kdnaforms' ),
	 * );
	 *
	 * @since 2.9.19
	 *
	...
	public function get_conditional_logic_payment_statuses( $form ) {
		$all_statuses = KDNACommon::get_entry_payment_statuses();
        $all_statuses['Active']    = esc_html__( 'Active Subscription', 'kdnaforms' );
        $all_statuses['Cancelled'] = esc_html__( 'Cancelled Subscription', 'kdnaforms' );
        return $all_statuses;
	}
	 ...
	 * @access public
	 *
	 * @uses KDNAPaymentAddOn::get_payment_feed
	 * @uses KDNAPaymentAddOn::get_submission_data
	 * @uses KDNAPaymentAddOn::$is_payment_gateway
	 * @uses KDNAPaymentAddOn::$current_feed
	 * @uses KDNAPaymentAddOn::$current_submission_data
	 * @uses KDNAPaymentAddOn::payment_method_is_overridden
	 * @uses KDNAPaymentAddOn::authorize
	 * @uses KDNAPaymentAddOn::subscribe
	 * @uses KDNAPaymentAddOn::get_validation_result
	 * @uses KDNAPaymentAddOn::$authorization
	 * @uses KDNAFeedAddOn::$_single_submission_feed
	 * @uses KDNAFormsModel::create_lead
	 * @uses KDNAAddOn::log_debug
	 * @uses KDNAFormDisplay::set_current_page
	 *
	...
	public function validation( $validation_result ) {
		...
		$form  = $validation_result['form'];
		$entry = KDNAFormsModel::get_current_lead( $form );
		...
		if ( KDNACommon::is_spam_entry( $entry, $form ) ) {
		...
		if ( $performed_authorization && ! rgar( $this->authorization, 'is_authorized' ) ) {
			...
			KDNAFormDisplay::set_current_page( $validation_result['form']['id'], $current_page );
		}
	...
	public function is_valid_payment_amount( $submission_data, $feed, $form, $entry ) {
		...
		$tag      = sprintf( 'kdnaform_%s_is_valid_payment_amount', $this->get_short_slug() );
	 ...
	 * single payments. For subscriptions or recurring payments, use the KDNAPaymentAddOn::subscribe() method.
	 *
	 * @since  Unknown
	 * @access public
	 *
	 ...
	 * @access public
	 *
	 * @used-by KDNAPaymentAddOn::ajax_cancel_subscription()
	 *
	 * @param array $entry Current entry array containing entry information (i.e data submitted by users).
	 * @param array $feed  Current configured payment feed.
	 *
	 ...
	 * @access public
	 *
	 * @used-by KDNAPaymentAddOn::init()
	 * @uses    KDNAPaymentAddOn::$is_payment_gateway
	 * @uses    KDNAPaymentAddOn::$current_feed
	 * @uses    KDNAPaymentAddOn::$authorization
	 * @uses    KDNAPaymentAddOn::process_subscription()
	 * @uses    KDNAPaymentAddOn::payment_method_is_overridden()
	 * @uses    KDNAPaymentAddOn::process_capture()
	 * @uses    KDNAPaymentAddOn::redirect_url()
	 *
	...
	public function entry_post_save( $entry, $form ) {
		...
		kdnaform_update_meta( $entry['id'], 'payment_gateway', $this->get_slug() );
		...
		$order = KDNA_Order_Factory::create_from_feed( $feed, $form, $entry, $this->current_submission_data, $this );
		kdnaform_add_meta(
			$entry['id'],
			'kdnaform_order',
			( new KDNA_Save_Entry_Order_Exporter( $order ) )->export()
		);
	 ...
	 *
	 * @used-by KDNAPaymentAddOn::entry_post_save()
	 * @uses    KDNAPaymentAddOn::complete_authorization()
	 * @uses    KDNAPaymentAddOn::complete_payment()
	 * @uses    KDNAPaymentAddOn::fail_payment()
	 *
	...
	public function process_capture( $authorization, $feed, $submission_data, $form, $entry ) {
		...
		if ( $payment['is_success'] ) {
			...
			$payment['note']         = sprintf( esc_html__( 'Payment failed to be captured. Reason: %s', 'kdnaforms' ), $payment['error_message'] );
	 ...
	 *
	 * @used-by KDNAPaymentAddOn::entry_post_save()
	 * @uses    KDNAPaymentAddOn::insert_transaction()
	 * @uses    KDNACommon::to_money()
	 * @uses    KDNAAddOn::add_note()
	 * @uses    KDNAPaymentAddOn::start_subscription()
	 * @uses    KDNAAPI::update_entry()
	 * @uses    KDNAPaymentAddOn::post_payment_action()
	 *
	 * @param array $authorization   The payment authorization details.
	 * @param array $feed            The Feed Object.
	 * @param array $submission_data The form submission data.
	...
	public function process_subscription( $authorization, $feed, $submission_data, $form, $entry ) {
		...
		$payment_name = rgempty( 'name', $payment ) ? esc_html__( 'Initial payment', 'kdnaforms' ) : $payment['name'];
		...
		if ( $payment && $payment['is_success'] ) {
			...
			$amount_formatted = KDNACommon::to_money( $payment['amount'], $entry['currency'] );
			$note             = sprintf( esc_html__( '%s has been captured successfully. Amount: %s. Transaction Id: %s', 'kdnaforms' ), $payment_name, $amount_formatted, $payment['transaction_id'] );
			$this->add_note( $entry['id'], $note, 'success' );

		} elseif ( $payment && ! $payment['is_success'] ) {

			$this->add_note( $entry['id'], sprintf( esc_html__( 'Failed to capture %s. Reason: %s.', 'kdnaforms' ), $payment['error_message'], $payment_name ), 'error' );
		...
		if ( $subscription['is_success'] ) {
			...
			$this->add_note( $entry['id'], sprintf( esc_html__( 'Subscription failed to be created. Reason: %s', 'kdnaforms' ), $subscription['error_message'] ), 'error' );
	 ...
	 * @access public
	 *
	 * @used-by KDNAPaymentAddOn::add_subscription_payment()
	 * @used-by KDNAPaymentAddOn::complete_authorization()
	 * @used-by KDNAPaymentAddOn::process_subscription()
	 * @used-by KDNAPaymentAddOn::refund_payment()
	 * @uses    wpdb::get_var()
	 * @uses    wpdb::prepare()
	 * @uses    wpdb::query()
	 * @uses    wpdb::$insert_id
	...
	public function insert_transaction( $entry_id, $transaction_type, $transaction_id, $amount, $is_recurring = null, $subscription_id = null ) {
		...
		$payment_count = $wpdb->get_var( $wpdb->prepare( "SELECT count(id) FROM {$wpdb->prefix}kdna_addon_payment_transaction WHERE lead_id=%d", $entry_id ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$is_recurring  = $payment_count > 0 && $transaction_type == 'payment' ? 1 : 0;
		$subscription_id = empty( $subscription_id ) ? '' : $subscription_id;

		$sql = $wpdb->prepare(
			" INSERT INTO {$wpdb->prefix}kdna_addon_payment_transaction (lead_id, transaction_type, transaction_id, amount, is_recurring, date_created, subscription_id)
		...
		/**
		 * Fires after a payment transaction is created in KDNA Forms.
		 *
		 * @since Unknown
		 *
		 ...
		 */
		do_action( 'kdnaform_post_payment_transaction', $txn_id, $entry_id, $transaction_type, $transaction_id, $amount, $is_recurring, $subscription_id );
		if ( has_filter( 'kdnaform_post_payment_transaction' ) ) {
			$this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_post_payment_transaction.' );
		}
	 ...
	 *
	 * @used-by KDNAPaymentAddOn::ajax_cancel_subscription()
	 * @used-by KDNAPaymentAddOn::process_callback_action()
	 * @used-by KDNAPaymentAddOn::validation()
	 * @uses    KDNAFeedAddOn::get_feeds_by_entry()
	 * @uses    KDNAFeedAddOn::get_feed()
	 * @uses    KDNAFeedAddOn::get_feeds()
	 * @uses    KDNAFeedAddOn::pre_process_feeds()
	 * @uses    KDNAFeedAddOn::is_feed_condition_met()
	 *
	 * @param array      $entry The Entry Object.
	 ...
	 * @access public
	 *
	 * @used-by KDNAPaymentAddOn::entry_info()
	 * @uses    KDNAPaymentAddOn::$is_payment_gateway()
	 * @uses    KDNAAddOn::$_slug
	 *
	 ...
	 *
	 * @used-by KDNAPaymentAddOn::validation()
	 * @uses    KDNAPaymentAddOn::billing_info_fields()
	 * @uses    KDNAPaymentAddOn::get_credit_card_field()
	 * @uses    KDNAAddOn::get_field_value()
	 * @uses    KDNAPaymentAddOn::remove_spaces_from_card_number()
	 * @uses    KDNAPaymentAddOn::get_order_data()
	 *
	 * @param array $feed  The Feed Object.
	 * @param array $form  The Form Object.
	 * @param array $entry The Entry Object.
	...
	public function get_submission_data( $feed, $form, $entry ) {
		...
		return gf_apply_filters( array( 'kdnaform_submission_data_pre_process_payment', $form['id'] ), $submission_data, $feed, $form, $entry );
	}
	 ...
	 *
	 * @used-by KDNAPaymentAddOn::before_delete_field()
	 * @used-by KDNAPaymentAddOn::get_submission_data()
	 * @used-by KDNAPaymentAddOn::has_credit_card_field()
	 * @uses    KDNAAPI::get_fields_by_type()
	 *
	 * @param array $form The Form Object.
	 *
	 * @return bool|KDNA_Field_CreditCard The credit card field object, if found. Otherwise, false.
	 */
	 ...
	 *
	 * @used-by KDNAPaymentAddOn::get_submission_data()
	 * @uses    KDNACommon::get_product_fields()
	 * @uses    KDNACommon::to_number()
	 *
	 * @param array $feed  The Feed Object.
	 * @param array $form  The Form Object.
	 * @param array $entry The Entry Object.
	...
	public function get_order_data( $feed, $form, $entry ) {
		...
		foreach ( $products['products'] as $field_id => $product ) {
			...
			$product_price = KDNACommon::to_number( $product['price'], $entry['currency'] );
			...
			$description = '';
			if ( ! empty( $options ) ) {
				$description = esc_html__( 'options: ', 'kdnaforms' ) . ' ' . implode( ', ', $options );
			}
			...
			if ( $product_price >= 0 ) {
				$line_items[] = array(
					...
					'quantity'    => $quantity,
					'unit_price'  => KDNACommon::to_number( $product_price, $entry['currency'] ),
		...
		if ( $trial_field == 'enter_amount' ) {
			$trial_amount = rgar( $feed['meta'], 'trial_amount' ) ? KDNACommon::to_number( rgar( $feed['meta'], 'trial_amount' ), $entry['currency'] ) : 0;
		}
		...
		if ( ! empty( $products['shipping']['name'] ) && ! is_numeric( $payment_field ) ) {
			$line_items[] = array(
				...
				'description' => '',
				'quantity'    => 1,
				'unit_price'  => KDNACommon::to_number( $products['shipping']['price'], $entry['currency'] ),
				'is_shipping' => 1
			);
		...
		$amount   = KDNACommon::round_number( $amount, $decimals );
		return array(
	 ...
	 * @access public
	 *
	 * @used-by KDNAPaymentAddOn::maybe_process_callback()
	 * @uses    KDNAAddOn::$_slug
	 *
	 ...
	 * @access public
	 *
	 * @used-by KDNAPaymentAddOn::pre_init()
	 * @uses    KDNAPaymentAddOn::is_callback_valid()
	 * @uses    KDNAAddOn::$_slug
	 * @uses    KDNAPaymentAddOn::callback()
	 * @uses    KDNAPaymentAddOn::display_callback_error()
	 * @uses    KDNAPaymentAddOn::process_callback_action()
	 * @uses    KDNAPaymentAddOn::post_callback()
	 *
	 * @return void
	 */
	 ...
	 * @access private
	 *
	 * @uses KDNAPaymentAddOn::is_duplicate_callback()
	 * @uses KDNAAPI::get_entry()
	 * @uses KDNAPaymentAddOn::complete_payment()
	 * @uses KDNAPaymentAddOn::refund_payment()
	 * @uses KDNAPaymentAddOn::fail_payment()
	 * @uses KDNAPaymentAddOn::add_pending_payment()
	 * @uses KDNAPaymentAddOn::void_authorization()
	 * @uses KDNAPaymentAddOn::start_subscription()
	 * @uses KDNAPaymentAddOn::get_payment_feed()
	 * @uses KDNAPaymentAddOn::cancel_subscription()
	 * @uses KDNAPaymentAddOn::expire_subscription()
	 * @uses KDNAPaymentAddOn::add_subscription_payment()
	 * @uses KDNAPaymentAddOn::fail_subscription_payment()
	 * @uses KDNAPaymentAddOn::register_callback()
	 *
	 * @param array $action {
	...
	private function process_callback_action( $action ) {
		...
		if ( rgar( $action, 'id' ) && $this->is_duplicate_callback( $action['id'] ) ) {
			return new WP_Error( 'duplicate', sprintf( esc_html__( 'This webhook has already been processed (Event Id: %s)', 'kdnaforms' ), $action['id'] ) );
		}

		$entry = KDNAAPI::get_entry( $action['entry_id'] );
		 ...
		 * @param array $entry  The Entry Object.
		 */
		do_action( 'kdnaform_action_pre_payment_callback', $action, $entry );
		if ( has_filter( 'kdnaform_action_pre_payment_callback' ) ) {
			$this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_action_pre_payment_callback.' );
		}
		 ...
		 */
		do_action( 'kdnaform_post_payment_callback', $entry, $action, $result );
		if ( has_filter( 'kdnaform_post_payment_callback' ) ) {
			$this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_post_payment_callback.' );
		}
	...
	public function is_duplicate_callback( $callback_id ) {
		...
		$sql = $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}kdna_addon_payment_callback WHERE addon_slug=%s AND callback_id=%s", $this->get_slug(), $callback_id );
	...
	public function add_pending_payment( $entry, $action ) {
		...
		if ( empty( $action['note'] ) ) {
			$action['note'] = sprintf( esc_html__( 'Payment is pending. Amount: %s. Transaction Id: %s.', 'kdnaforms' ), $action['amount_formatted'], $action['transaction_id'] );
		}

		KDNAAPI::update_entry_property( $entry['id'], 'payment_status', $action['payment_status'] );
	...
	public function complete_authorization( &$entry, $action ) {
		...
		if ( ! rgar( $action, 'note' ) ) {
			$action['note'] = sprintf( esc_html__( 'Payment has been authorized. Amount: %s. Transaction Id: %s.', 'kdnaforms' ), $action['amount_formatted'], $action['transaction_id'] );
		}
	...
	public function complete_payment( &$entry, $action ) {
		...
		if ( ! rgar( $action, 'note' ) ) {
			$action['note'] = sprintf( esc_html__( 'Payment has been completed. Amount: %s. Transaction Id: %s.', 'kdnaforms' ), $action['amount_formatted'], $action['transaction_id'] );
		}
		 ...
		 */
		do_action( 'kdnaform_post_payment_completed', $entry, $action );
		if ( has_filter( 'kdnaform_post_payment_completed' ) ) {
			$this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_post_payment_completed.' );
		}
		$this->post_payment_action( $entry, $action );
	...
	public function refund_payment( $entry, $action ) {
		...
		if ( empty( $action['note'] ) ) {
			$action['note'] = sprintf( esc_html__( 'Payment has been refunded. Amount: %s. Transaction Id: %s.', 'kdnaforms' ), $action['amount_formatted'], $action['transaction_id'] );
		}
		 ...
		 * - fail_subscription_payment
		 */
		do_action( 'kdnaform_post_payment_refunded', $entry, $action );
		if ( has_filter( 'kdnaform_post_payment_refunded' ) ) {
			$this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_post_payment_refunded.' );
		}
		$this->post_payment_action( $entry, $action );
	...
	public function fail_payment( $entry, $action ) {
		...
		if ( empty( $action['note'] ) ) {
			$action['note'] = sprintf( esc_html__( 'Payment has failed. Amount: %s.', 'kdnaforms' ), $action['amount_formatted'] );
		}
	...
	public function void_authorization( $entry, $action ) {
		...
		if ( empty( $action['note'] ) ) {
			$action['note'] = sprintf( esc_html__( 'Authorization has been voided. Transaction Id: %s', 'kdnaforms' ), $action['transaction_id'] );
		}
	...
	public function start_subscription( $entry, $subscription ) {
		...
		if ( ! $has_active_subscription ) {
			...
			$result = KDNAAPI::update_entry( $entry );
			$this->add_note( $entry['id'], sprintf( esc_html__( 'Subscription has been created. Subscription Id: %s.', 'kdnaforms' ), $subscription['subscription_id'] ), 'success' );
			 ...
			 */
			do_action( 'kdnaform_post_subscription_started', $entry, $subscription );
			if ( has_filter( 'kdnaform_post_subscription_started' ) ) {
				$this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_post_subscription_started.' );
			}
	...
	public function add_subscription_payment( $entry, $action ) {
		...
		if ( strtolower( $entry['payment_status'] ) === 'processing' ) {
			...
			$entry['payment_status'] = 'Active';
			KDNAAPI::update_entry_property( $entry['id'], 'payment_status', 'Active' );
		}

		$action = $this->maybe_add_action_amount_formatted( $action, $entry['currency'] );

		if ( empty( $action['note'] ) ) {
			$action['note'] = sprintf( esc_html__( 'Subscription has been paid. Amount: %s. Subscription Id: %s', 'kdnaforms' ), $action['amount_formatted'], $action['subscription_id'] );
		}
		 ...
		 * - fail_subscription_payment
		 */
		do_action( 'kdnaform_post_add_subscription_payment', $entry, $action );
		if ( has_filter( 'kdnaform_post_add_subscription_payment' ) ) {
			$this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_post_add_subscription_payment.' );
		}
		$this->post_payment_action( $entry, $action );
	...
	public function fail_subscription_payment( $entry, $action ) {
		...
		if ( empty( $action['note'] ) ) {
			$action['note'] = sprintf( esc_html__( 'Subscription payment has failed. Amount: %s. Subscription Id: %s.', 'kdnaforms' ), $action['amount_formatted'], $action['subscription_id'] );
		}

		KDNAAPI::update_entry_property( $entry['id'], 'payment_status', 'Failed' );
		$this->add_note( $entry['id'], $action['note'], 'error' );

		// keep 'kdnaform_subscription_payment_failed' for backward compatability
		/**
		 * @deprecated Use kdnaform_post_fail_subscription_payment now.
		 * @remove-in 3.0
		 */
		do_action( 'kdnaform_subscription_payment_failed', $entry, $action['subscription_id'] );
		if ( has_filter( 'kdnaform_subscription_payment_failed' ) ) {
			trigger_error( 'kdnaform_subscription_payment_failed is deprecated and will be removed in version 3.0. Use kdnaform_post_fail_subscription_payment.', E_USER_DEPRECATED ); // phpcs:ignore QITStandard.PHP.DebugCode.DebugFunctionFound
			$this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_subscription_payment_failed.' );
		}
		/**
		 ...
		 * - fail_subscription_payment
		 */
		do_action( 'kdnaform_post_fail_subscription_payment', $entry, $action );
		if ( has_filter( 'kdnaform_post_fail_subscription_payment' ) ) {
			$this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_post_fail_subscription_payment.' );
		}
		$this->post_payment_action( $entry, $action );
	...
	public function cancel_subscription( $entry, $feed, $note = null ) {
		$this->log_debug( __METHOD__ . '(): Processing request.' );
		if ( ! $note ) {
			$note = sprintf( esc_html__( 'Subscription has been cancelled. Subscription Id: %s.', 'kdnaforms' ), $entry['transaction_id'] );
		}
		...
		KDNAAPI::update_entry_property( $entry['id'], 'payment_status', 'Cancelled' );
		$this->add_note( $entry['id'], $note, 'error' );

		// Include $subscriber_id as 3rd parameter for backwards compatibility
		do_action( 'kdnaform_subscription_canceled', $entry, $feed, $entry['transaction_id'] );

		// Include alternative spelling of "cancelled".
		do_action( 'kdnaform_subscription_cancelled', $entry, $feed, $entry['transaction_id'] );

		if ( has_filter( 'kdnaform_subscription_canceled' ) || has_filter( 'kdnaform_subscription_cancelled' ) ) {
			$this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_subscription_canceled.' );
		}
	...
	public function expire_subscription( $entry, $action ) {
		...
		if ( empty( $action['note'] ) ) {
			$action['note'] = sprintf( esc_html__( 'Subscription has expired. Subscriber Id: %s', 'kdnaforms' ), $action['subscription_id'] );
		}

		KDNAAPI::update_entry_property( $entry['id'], 'payment_status', 'Expired' );
		$this->add_note( $entry['id'], $action['note'] );
		$this->post_payment_action( $entry, $action );
	 ...
	 *
	 * @since 2.3.3.9 Updated to search the _kdna_addon_payment_transaction table if the ID was not found in the entry table.
	 * @since unknown
	 *
	...
	public function get_entry_by_transaction_id( $transaction_id ) {
		...
		if ( ! $entry_id ) {
			$sql      = $wpdb->prepare( "SELECT lead_id FROM {$wpdb->prefix}kdna_addon_payment_transaction WHERE transaction_id = %s", $transaction_id );
			$entry_id = $wpdb->get_var( $sql ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
		}
	...
	/**
	 * Helper for making the kdnaform_post_payment_action hook available to the various payment interaction methods. Also handles sending notifications for payment events.
	 *
	 * @since 2.3.6.6 Added the $action to the KDNAAPI::send_notifications() $data param.
	 * @since unknown
	 *
	 * @param array $entry
	 * @param array $action
	...
	public function post_payment_action( $entry, $action ) {
		do_action( 'kdnaform_post_payment_action', $entry, $action );
		if ( has_filter( 'kdnaform_post_payment_action' ) ) {
			$this->log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_post_payment_action.' );
		}
		...
		if ( ! empty( $supported_events ) ) {
			if ( ! empty( $action['payment_status'] ) ) {
				$action['payment_status'] = KDNACommon::get_entry_payment_status_text( $action['payment_status'] );
			}
			KDNAAPI::send_notifications( $form, $entry, rgar( $action, 'type' ), array( 'payment_action' => $action ) );
		}
	}
    ...
    public function reprocess_feeds( $entry ) {
        ...
        add_filter( 'kdnaform_addon_pre_process_feeds', array( $this, 'get_feeds_to_reprocess' ), 10, 3 );

	    // Reprocessing feeds.
        KDNAAPI::maybe_process_feeds( $entry, KDNAAPI::get_form( $entry['form_id'] ), '', false, true );

        // Make sure to remove the filter so that it doesn't affect other feed processing.
        remove_filter( 'kdnaform_addon_pre_process_feeds', array( $this, 'get_feeds_to_reprocess' ) );
    }
	...
	public function feed_list_columns() {
		return array(
			...
			'amount'          => esc_html__( 'Amount', 'kdnaforms' )
		);
	}
	...
	public function get_column_value_transactionType( $feed ) {
		switch ( rgar( $feed['meta'], 'transactionType' ) ) {
			case 'subscription' :
				return esc_html__( 'Subscription', 'kdnaforms' );
				break;
			case 'product' :
				return esc_html__( 'Products and Services', 'kdnaforms' );
				break;
			case 'donation' :
				return esc_html__( 'Donations', 'kdnaforms' );
				break;

		}

		return esc_html__( 'Unsupported transaction type', 'kdnaforms' );
	}
	...
	public function get_column_value_amount( $feed ) {
		...
		if ( $field_id == 'form_total' ) {
			...
			$label = KDNACommon::get_label( $field );
		}
	...
	public function requires_credit_card_message() {
		...
		return sprintf( esc_html__( "You must add a Credit Card field to your form before creating a feed. Let's go %sadd one%s!", 'kdnaforms' ), "<a href='" . esc_url( $url ) . "'>", '</a>' );
	}
			...
			array(
				...
				'fields'      => array(
					array(
						...
						'class'    => 'medium',
						'required' => true,
						'tooltip'  => '<h6>' . esc_html__( 'Name', 'kdnaforms' ) . '</h6>' . esc_html__( 'Enter a feed name to uniquely identify this setup.', 'kdnaforms' )
					),
					array(
						...
						'choices'  => array(
							array(
								'label' => esc_html__( 'Select a transaction type', 'kdnaforms' ),
								'value' => ''
							),
							array(
							...
							array( 'label' => esc_html__( 'Subscription', 'kdnaforms' ), 'value' => 'subscription' ),
						),
						'tooltip'  => '<h6>' . esc_html__( 'Transaction Type', 'kdnaforms' ) . '</h6>' . esc_html__( 'Select a transaction type.', 'kdnaforms' )
					),
				)
			),
			array(
				'title'      => esc_html__( 'Subscription Settings', 'kdnaforms' ),
				'dependency' => array(
				...
				'fields'     => array(
					array(
						...
						'label'    => esc_html__( 'Recurring Amount', 'kdnaforms' ),
						'type'     => 'select',
						'choices'  => $this->recurring_amount_choices(),
						'required' => true,
						'tooltip'  => '<h6>' . esc_html__( 'Recurring Amount', 'kdnaforms' ) . '</h6>' . esc_html__( "Select which field determines the recurring payment amount, or select 'Form Total' to use the total of all pricing fields as the recurring amount.", 'kdnaforms' )
					),
					array(
						'name'    => 'billingCycle',
						'label'   => esc_html__( 'Billing Cycle', 'kdnaforms' ),
						'type'    => 'billing_cycle',
						'tooltip' => '<h6>' . esc_html__( 'Billing Cycle', 'kdnaforms' ) . '</h6>' . esc_html__( 'Select your billing cycle.  This determines how often the recurring payment should occur.', 'kdnaforms' )
					),
					array(
						'name'    => 'recurringTimes',
						'label'   => esc_html__( 'Recurring Times', 'kdnaforms' ),
						'type'    => 'select',
						'choices' => array(
							             array(
								             'label' => esc_html__( 'infinite', 'kdnaforms' ),
								             'value' => '0'
							             )
						             ) + $this->get_numeric_choices( 1, 100 ),
						'tooltip' => '<h6>' . esc_html__( 'Recurring Times', 'kdnaforms' ) . '</h6>' . esc_html__( 'Select how many times the recurring payment should be made.  The default is to bill the customer until the subscription is canceled.', 'kdnaforms' )
					),
					array(
						'name'   => 'setupFee',
						'label'  => esc_html__( 'Setup Fee', 'kdnaforms' ),
						'type'   => 'setup_fee',
						'hidden' => $this->get_setting( 'trial_enabled' ),
					),
					array(
						'name'    => 'trial',
						'label'   => esc_html__( 'Trial', 'kdnaforms' ),
						'type'    => 'trial',
						'hidden'  => $this->get_setting( 'setupFee_enabled' ),
						'tooltip' => '<h6>' . esc_html__( 'Trial Period', 'kdnaforms' ) . '</h6>' . esc_html__( 'Enable a trial period.  The user\'s recurring payment will not begin until after this trial period.', 'kdnaforms' )
					),
				)
			),
			array(
				'title'      => esc_html__( 'Products &amp; Services Settings', 'kdnaforms' ),
				'dependency' => array(
					'field'  => 'transactionType',
				...
				'fields'     => array(
					array(
						...
						'tooltip'       => '<h6>' . esc_html__( 'Payment Amount', 'kdnaforms' ) . '</h6>' . esc_html__( "Select which field determines the payment amount, or select 'Form Total' to use the total of all pricing fields as the payment amount.", 'kdnaforms' )
					),
				)
			),
			array(
				'title'      => esc_html__( 'Other Settings', 'kdnaforms' ),
				'dependency' => array(
					'field'  => 'transactionType',
	...
	public function other_settings_fields() {
		$other_settings = array(
			array(
				...
				'tooltip'   => '<h6>' . esc_html__( 'Billing Information', 'kdnaforms' ) . '</h6>' . esc_html__( 'Map your Form Fields to the available listed fields.', 'kdnaforms' )
			),
		);
		...
		if ( ! empty( $option_choices ) ) {
			$other_settings[] = array(
				'name'    => 'options',
				'label'   => esc_html__( 'Options', 'kdnaforms' ),
				'type'    => 'checkbox',
		...
		$other_settings[] = array(
			...
			'type'    => 'feed_condition',
			'tooltip' => '<h6>' . esc_html__( 'Conditional Logic', 'kdnaforms' ) . '</h6>' . esc_html__( 'When conditions are enabled, form submissions will only be sent to the payment gateway when the conditions are met. When disabled, all form submissions will be sent to the payment gateway.', 'kdnaforms' )
		);
	...
	public function settings_setup_fee( $field, $echo = true ) {
		...
		$enabled_field = array(
			...
			'choices'    => array(
				array(
					'label'    => esc_html__( 'Enabled', 'kdnaforms' ),
	...
	public function settings_trial( $field, $echo = true ) {
		...
		$html .= '<div class="kdnaform-settings-field__toggleable-inputs">';

		//--- Select Product field ---
		$form            = $this->get_current_form();
		$payment_choices = array_merge( $this->get_payment_choices( $form ), array(
			array(
				'label' => esc_html__( 'Enter an amount', 'kdnaforms' ),
				'value' => 'enter_amount'
			)
		) );
		...
		$amount_field = array(
			...
			'class' => $this->get_setting( "{$field['name']}_enabled" ) && $this->get_setting( "{$field['name']}_product" ) == 'enter_amount' ? 'kdnaform_currency' : 'hidden kdnaform_currency',
		);
	...
	public function recurring_amount_choices() {
		...
		$recurring_choices[] = array( 'label' => esc_html__( 'Form Total', 'kdnaforms' ), 'value' => 'form_total' );
	...
	public function product_amount_choices() {
		...
		$product_choices[] = array( 'label' => esc_html__( 'Form Total', 'kdnaforms' ), 'value' => 'form_total' );
	...
	public function option_choices() {
		...
		$option_choices = array(
			array(
				'label' => esc_html__( 'Sample Option', 'kdnaforms' ),
				'name'  => 'sample_option',
				'value' => 'sample_option'
			),
		);
	...
	public function supported_billing_intervals() {
		...
		$billing_cycles = array(
			'day'   => array( 'label' => esc_html__( 'day(s)', 'kdnaforms' ), 'min' => 1, 'max' => 365 ),
			'week'  => array( 'label' => esc_html__( 'week(s)', 'kdnaforms' ), 'min' => 1, 'max' => 52 ),
			'month' => array( 'label' => esc_html__( 'month(s)', 'kdnaforms' ), 'min' => 1, 'max' => 12 ),
			'year'  => array( 'label' => esc_html__( 'year(s)', 'kdnaforms' ), 'min' => 1, 'max' => 10 )
		);
	...
	public function get_payment_choices( $form ) {
		$fields  = KDNAAPI::get_fields_by_type( $form, array( 'product' ) );
		$choices = array(
			array( 'label' => esc_html__( 'Select a product field', 'kdnaforms' ), 'value' => '' ),
		);
	...
	public function get_results_page_config() {
		...
		return array(
			'title'        => _x( 'Sales', 'toolbar label', 'kdnaforms' ),
			'search_title' => _x( 'Filter', 'metabox title', 'kdnaforms' ),
			'capabilities' => array( 'kdnaforms_view_entries' ),
			'callbacks'    => array(
	...
	public function results_markup( $html, $data, $form, $fields ) {
		...
		$html = '<div class="kdnaform-results wide">' ;
		$boxes = array(
			'today'     => esc_html__( 'Today', 'kdnaforms' ),
			'yesterday' => esc_html__( 'Yesterday', 'kdnaforms' ),
			'last30'    => esc_html__( 'Last 30 Days', 'kdnaforms' ),
			'total'     => esc_html__( 'Total', 'kdnaforms' )
		);

			foreach ( $boxes as $key => $label ){
				$html .= '
				<div class="kdnaform-result-box">
					<div class="kdnaform-result-box__primary">
					...
					<div class="kdnaform-result-box__secondary">
						<div class="box-orders">'.$data['summary'][$key]['orders'].' '. esc_html__( 'orders', 'kdnaforms' ) .'</div>
						<div class="box-subscriptions">'.$data['summary'][$key]['subscriptions'].' '.esc_html__( 'subscriptions', 'kdnaforms' ) .'</div>
					</div>
				</div>';
			}
		$html .= '</div>';
		...
		if ( $data['row_count'] == '0' ) {
			$html .= "<div class='alert info'>" . esc_html__( "There aren't any transactions that match your criteria.", 'kdnaforms' ) . '</div>';
		} else {
			$chart_data = $this->get_chart_data( $data );
			$html .= $this->get_sales_chart( $chart_data );

			//Getting sales table markup
			$sales_table = new KDNAPaymentStatsTable( $data['table']['header'], $data['data'], $data['row_count'], $data['page_size'] );
	...
	public static function get_sales_chart( $sales_data ) {
		...
		$div_id          = 'kdnaquiz-results-chart-field-score-frequencies';
	...
	public function get_sales_data( $form_id, $search, $state ) {
		...
		$data = array(
			'chart' => array(
				...
				'vAxis' => array(
					'column' => 'revenue',
					'label'  => esc_html__( 'Revenue', 'kdnaforms' )
				)
			),
			'table' => array(
				'header' => array(
					'orders'             => esc_html__( 'Orders', 'kdnaforms' ),
					'subscriptions'      => esc_html__( 'Subscriptions', 'kdnaforms' ),
					'recurring_payments' => esc_html__( 'Recurring Payments', 'kdnaforms' ),
					'refunds'            => esc_html__( 'Refunds', 'kdnaforms' ),
					'revenue'            => esc_html__( 'Revenue', 'kdnaforms' )
				)
			),
			'rows'  => array()
		);
		...
		switch ( $group ) {
			...
			case 'weekly' :
				...
				$data['chart']['hAxis']['label']  = esc_html__( 'Week', 'kdnaforms' );
				$data['table']['header']          = array_merge( array( 'week' => esc_html__( 'Week', 'kdnaforms' ) ), $data['table']['header'] );
			...
			case 'monthly' :
				...
				$data['chart']['hAxis']['label']  = esc_html__( 'Month', 'kdnaforms' );
				$data['table']['header']          = array_merge( array( 'month_year' => esc_html__( 'Month', 'kdnaforms' ) ), $data['table']['header'] );
			...
			default : //daily
				...
				$data['chart']['hAxis']['label']  = esc_html__( 'Day', 'kdnaforms' );
				$data['table']['header']          = array_merge( array(
					'date'        => esc_html__( 'Date', 'kdnaforms' ),
					'day_of_week' => esc_html__( 'Day', 'kdnaforms' )
				), $data['table']['header'] );
		...
		$sql = $wpdb->prepare(
			" SELECT SQL_CALC_FOUND_ROWS {$select}, leads.orders, leads.subscriptions, transaction.refunds, transaction.recurring_payments, transaction.revenue
                                ...
                                RIGHT OUTER JOIN(
                                  ...
                                  FROM {$wpdb->prefix}kdna_addon_payment_transaction t
		...
		KDNACommon::log_debug( "sales sql: {$sql}" );
	...
	public function format_chart_h_axis( $result ) {
		$months = array(
			esc_html__( 'Jan', 'kdnaforms' ),
			esc_html__( 'Feb', 'kdnaforms' ),
			esc_html__( 'Mar', 'kdnaforms' ),
			esc_html__( 'Apr', 'kdnaforms' ),
			esc_html_x( 'May', 'Abbreviated month name', 'kdnaforms' ),
			esc_html__( 'Jun', 'kdnaforms' ),
			esc_html__( 'Jul', 'kdnaforms' ),
			esc_html__( 'Aug', 'kdnaforms' ),
			esc_html__( 'Sep', 'kdnaforms' ),
			esc_html__( 'Oct', 'kdnaforms' ),
			esc_html__( 'Nov', 'kdnaforms' ),
			esc_html__( 'Dec', 'kdnaforms' ),
		);
	...
	public function get_sales_summary( $form_id ) {
		...
		$result = array(
			'today'     => array( 'revenue' => KDNACommon::to_money( 0 ), 'orders' => 0, 'subscriptions' => 0 ),
			'yesterday' => array( 'revenue' => KDNACommon::to_money( 0 ), 'orders' => 0, 'subscriptions' => 0 ),
			'last30'    => array( 'revenue' => 0, 'orders' => 0, 'subscriptions' => 0 ),
			'total'     => array(
				'revenue'       => KDNACommon::to_money( $total_revenue ),
				'orders'        => $total_summary[0]['orders'],
				'subscriptions' => $total_summary[0]['subscriptions']
			)
		);

		$local_time = KDNACommon::get_local_timestamp();
		$today      = gmdate( 'Y-m-d', $local_time );
		$yesterday  = gmdate( 'Y-m-d', strtotime( '-1 day', $local_time ) );

		foreach ( $summary as $day ) {
			if ( $day['date'] == $today ) {
				$result['today']['revenue']       = KDNACommon::to_money( $day['revenue'] );
				$result['today']['orders']        = $day['orders'];
				$result['today']['subscriptions'] = $day['subscriptions'];
			} elseif ( $day['date'] == $yesterday ) {
				$result['yesterday']['revenue']       = KDNACommon::to_money( $day['revenue'] );
		...
		$result['last30']['revenue'] = KDNACommon::to_money( $result['last30']['revenue'] );
	...
	public function results_filter_ui( $filter_ui, $form_id, $page_title, $gf_page, $gf_view ) {
		...
		$view_markup = "<div>
                    <select id='gaddon-sales-group' name='group'>
                        <option value='daily' " . selected( 'daily', rgget( 'group' ), false ) . '>' . esc_html__( 'Daily', 'kdnaforms' ) . "</option>
                        <option value='weekly' " . selected( 'weekly', rgget( 'group' ), false ) . '>' . esc_html__( 'Weekly', 'kdnaforms' ) . "</option>
                        <option value='monthly' " . selected( 'monthly', rgget( 'group' ), false ) . '>' . esc_html__( 'Monthly', 'kdnaforms' ) . '</option>
                    </select>
                  </div>';
		$view_filter = array(
			'view' => array(
				'label'   => esc_html__( 'View', 'kdnaforms' ),
				'tooltip' => '<h6>' . esc_html__( 'View', 'kdnaforms' ) . '</h6>' . esc_html__( 'Select how you would like the sales data to be displayed.', 'kdnaforms' ),
				'markup'  => $view_markup
			)
		);
			...
			$payment_method_markup = "
                <div>
                    <select id='gaddon-sales-group' name='payment_method'>
                        <option value=''>" . esc_html__( _x( 'Any', 'regarding a payment method', 'kdnaforms' ) ) . '</option>';
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormsModel::get_entry_table_name()
	 * @uses   KDNAFormsModel::get_lead_table_name()
	 * @uses   KDNAPayPalPaymentsPro::get_kdnaforms_db_version()
	 *
	 * @return string
	 */
	public static function get_entry_table_name() {

		return version_compare( self::get_kdnaforms_db_version(), '2.3-dev-1', '<' ) ? KDNAFormsModel::get_lead_table_name() : KDNAFormsModel::get_entry_table_name();
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormsModel::get_entry_meta_table_name()
	 * @uses   KDNAFormsModel::get_lead_meta_table_name()
	 * @uses   KDNAPayPalPaymentsPro::get_kdnaforms_db_version()
	 *
	 * @return string
	 */
	public static function get_entry_meta_table_name() {
		...
		return version_compare( self::get_kdnaforms_db_version(), '2.3-dev-1', '<' ) ? KDNAFormsModel::get_lead_meta_table_name() : KDNAFormsModel::get_entry_meta_table_name();

	}

	/**
	 * Get version of KDNA Forms database.
	 *
	 * @since  2.3
	 * @access public
	 *
	 * @uses   KDNAFormsModel::get_database_version()
	 *
	 * @return string
	 */
	public static function get_kdnaforms_db_version() {

		return method_exists( 'KDNAFormsModel', 'get_database_version' ) ? KDNAFormsModel::get_database_version() : KDNAForms::$version;
	...
	public function uninstall() {
		...
		// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$sql = $wpdb->prepare(
			"DELETE FROM {$wpdb->prefix}kdna_addon_payment_transaction
                                WHERE lead_id IN
		...
		if ( $this->_supports_callbacks ) {
			...
			$sql = $wpdb->prepare( "DELETE FROM {$wpdb->prefix}kdna_addon_payment_callback WHERE addon_slug=%s", $this->get_slug() );
			$wpdb->query( $sql );  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.PreparedSQL.NotPrepared
		}
	...
	public function scripts() {
		...
		$scripts = array(
			array(
				...
				'strings' => array(
					'subscriptionCancelWarning' => __( "Warning! This subscription will be canceled. This cannot be undone. 'OK' to cancel subscription, 'Cancel' to stop", 'kdnaforms' ),
					'subscriptionCancelNonce'   => wp_create_nonce( 'gaddon_cancel_subscription' ),
					'subscriptionCanceled'      => __( 'Canceled', 'kdnaforms' ),
					'subscriptionError'         => __( 'The subscription could not be canceled. Please try again later.', 'kdnaforms' )
				),
				'enqueue' => array(
	...
	public function add_creditcard_token_input( $content, $field, $value, $entry_id, $form_id ) {
		...
		if ( ! $this->has_feed( $form_id ) || KDNAFormsModel::get_input_type( $field ) != 'creditcard' ) {
			return $content;
		}
	...
	public function force_ajax_for_creditcard_tokens( $args ) {
		...
		$form = KDNAAPI::get_form( rgar( $args, 'form_id' ) );
	...
	/**
	 * Determines if KDNAToken script should be enqueued.
	 *
	 * @access public
	 *
	 * @param array $form
	...
	public function register_creditcard_token_script( $form, $field_values, $is_ajax ) {
		...
		/* Prepare KDNAToken object. */
		$gftoken = array(
			'callback'      => 'KDNA_' . str_replace( ' ', '', $this->_short_title ),
			'feeds'         => $this->creditcard_token_info( $form ),
			'formId'        => rgar( $form, 'id' ),
			'hasPages'      => KDNACommon::has_pages( $form ),
			'pageCount'     => KDNAFormDisplay::get_max_page_number( $form ),
			'responseField' => '#gf_' . str_replace( 'kdnaforms', '', $this->get_slug() ) . '_response'
		);

		/* Get needed fields. */
		$gftoken['fields'] = $this->get_creditcard_token_entry_fields( $gftoken['feeds'] );

		$script = 'new KDNAToken( ' . json_encode( $gftoken ) . ' );';
		KDNAFormDisplay::add_init_script( $form['id'], 'KDNAToken', KDNAFormDisplay::ON_PAGE_RENDER, $script );
	...
	public function maybe_add_action_amount_formatted( $action, $currency_code = '' ) {
		if ( ! empty( $action['amount'] ) && empty( $action['amount_formatted'] ) ) {
			$action['amount_formatted'] = KDNACommon::to_money( $action['amount'], $currency_code );
		}
	...
	public function entry_info( $form_id, $entry ) {
		...
		if ( $entry['transaction_type'] == '2' && $entry['payment_status'] <> 'Cancelled' && $entry['payment_status'] <> 'Failed' && $this->is_payment_gateway( $entry['id'] ) ) {
			...
			<input id="cancelsub" type="button" name="cancelsub"
			       value="<?php esc_html_e( 'Cancel Subscription', 'kdnaforms' ) ?>" class="button"
			       onclick="cancel_subscription(<?php echo absint( $entry['id'] ); ?>);"
			       onkeypress="cancel_subscription(<?php echo absint( $entry['id'] ); ?>);"/>
			<img src="<?php echo esc_url( KDNACommon::get_base_url() ); ?>/images/spinner.svg" id="subscription_cancel_spinner"
			     style="display: none;"/>
	...
	/**
	 * Target of kdnaform_delete_lead hook. Deletes all transactions and callbacks when an entry is deleted.
	 *
	...
	public function entry_deleted( $entry_id ) {
		...
		$wpdb->delete( "{$wpdb->prefix}kdna_addon_payment_transaction", array( 'lead_id' => $entry_id ), array( '%d' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching

		//deleting from callback table
		$wpdb->delete( "{$wpdb->prefix}kdna_addon_payment_callback", array( 'lead_id' => $entry_id ), array( '%d' ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	}
	...
	public function ajax_cancel_subscription() {
		...
		if ( $feed && $this->get_slug() === $feed['addon_slug'] && ! KDNACommon::current_user_can_any( $this->_capabilities_settings_page ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Access denied.', 'kdnaforms' ) ) );
		}
	...
	/**
	 * Target of kdnaform_before_delete_field hook. Sets relevant payment feeds to inactive when the credit card field is
	 * deleted.
	 *
	...
	private function payment_method_is_overridden( $method_name, $base_class = 'KDNAPaymentAddOn' ) {
	...
	private static function get_statuses_for_filter_links() {
		...
		$statuses = array(
			array(
				...
				'label'  => esc_html__( 'Payment Processing', 'kdnaforms' ),
			),
			array(
				'id'     => 'payment_authorized',
				'values' => array( 'Authorized' ),
				'label'  => esc_html__( 'Payment Authorized', 'kdnaforms' ),
			),
			array(
				'id'     => 'payment_completed',
				'values' => array( 'Paid', 'Active' ),
				'label'  => esc_html__( 'Payment Completed', 'kdnaforms' ),
			),
			array(
				'id'     => 'payment_failed',
				'values' => array( 'Failed', 'Voided' ),
				'label'  => esc_html__( 'Payment Failed', 'kdnaforms' ),
			),
		);
		...
		foreach ( $all_statuses as $status => $label ) {
			...
			$statuses[] = array(
				...
				'values' => array( $status ),
				'label'  => sprintf( esc_html__( 'Payment %s', 'kdnaforms' ), $label ),
			);
		}
	...
	/**
	 * Callback for kdnaform_entries_filter_count_queries; registers the db queries to be used to get the entry counts for the filters.
	 *
	 * @since 2.9.16.
	 *
	...
	/**
	 * Callback for kdnaform_filter_links_entry_list; registers the filter links to be displayed on the entries list page.
	 *
	 * @since 2.9.16
	 *
	...
	public static function filter_links_entry_list( $filter_links, $form, $include_counts, $counts ) {
		$statuses       = self::get_statuses_for_filter_links();
		$default_filter = rgar( KDNAEntryList::get_screen_options_values(), 'default_filter' );
...
class KDNAPaymentStatsTable extends WP_List_Table {
	...
	function __construct( $columns, $rows, $total_count, $page_size ) {
		...
		parent::__construct(
			array(
				'singular' => esc_html__( 'sale', 'kdnaforms' ),
				'plural'   => esc_html__( 'sales', 'kdnaforms' ),
				'ajax'     => false,
				'screen'   => 'gaddon_sales',
			)
		);
	...
	function no_items() {
		esc_html_e( "There hasn't been any sales in the specified date range.", 'kdnaforms' );
	}
	...
	function column_revenue( $item ) {
		return KDNACommon::to_money( $item['revenue'] );
	}
	...
	function pagination( $which ) {
		...
		$output = '<span class="displaying-num">' . sprintf( _n( '1 item', '%s items', $total_items, 'kdnaforms' ), number_format_i18n( $total_items ) ) . '</span>';
		...
		$page_links[] = sprintf(
			...
			esc_attr__( 'Go to the first page', 'kdnaforms' ),
			'&laquo;'
		);
		...
		$page_links[] = sprintf(
			...
			esc_attr__( 'Go to the previous page', 'kdnaforms' ),
			max( 1, $current - 1 ),
			max( 1, $current - 1 ),
			'&lsaquo;'
		);
		...
		$page_links[]     = '<span class="paging-input">' . sprintf( esc_html_x( '%1$s of %2$s', 'paging', 'kdnaforms' ), $html_current_page, $html_total_pages ) . '</span>';

		$page_links[] = sprintf(
			"<a class='%s' title='%s' style='cursor:pointer;' onclick='gresults.setCustomFilter(\"paged\", \"%s\"); gresults.getResults(); gresults.setCustomFilter(\"paged\", \"1\");' onkeypress='gresults.setCustomFilter(\"paged\", \"%s\"); gresults.getResults(); gresults.setCustomFilter(\"paged\", \"1\");'>%s</a>",
			'next-page' . $disable_last,
			esc_attr__( 'Go to the next page', 'kdnaforms' ),
		...
		$page_links[] = sprintf(
			...
			esc_attr__( 'Go to the last page', 'kdnaforms' ),
			$total_pages,
			'&raquo;'
		);
