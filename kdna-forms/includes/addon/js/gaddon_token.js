window.KDNAToken = null;

( function( $ ) {
	
	KDNAToken = function( args ) {
		...
		this.form = $( '#kdnaform_' + this.formId );

		this.init = function() {
		
			var KDNATokenObj = this;
			...
			if ( this.hasPages ) {
				...
				$( document ).bind( 'kdnaform_page_loaded', function( event, form_id, current_page ) {
				
					if ( form_id != KDNATokenObj.formId)
						return;
					
					if ( current_page != KDNATokenObj.pageCount)
						KDNATokenObj.saveEntryData();
		...
		this.onSubmit = function() {
			...
			if ( this.form.data('kdnatokensubmitting') ) {
				return;
			} else {
				event.preventDefault();
				this.form.data( 'kdnatokensubmitting', true );
			}
