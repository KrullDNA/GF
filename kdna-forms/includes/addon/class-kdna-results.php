...
if ( ! class_exists( 'KDNAResults' ) ) {
	...
	class KDNAResults {
		...
		public function __construct( $slug, $config ) {
			...
			$this->_search_title = rgempty( 'search_title', $config ) ? esc_html__( 'Results Filters', 'kdnaforms' ) : rgar( $config, 'search_title' );
		...
		public function init() {
			...
			if ( KDNAForms::is_kdna_page() ) {
				...
				add_filter( 'kdnaform_toolbar_menu', array( $this, 'add_toolbar_menu_item' ), 10, 2 );

				// add custom form action
				add_filter( 'kdnaform_form_actions', array( $this, 'add_form_action' ), 10, 2 );
			...
			if ( rgget( 'view' ) == "gf_results_{$this->_slug}" ) {
				...
				add_action( 'kdnaform_entries_view', array( $this, 'add_view' ), 10, 2 );
				add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_admin_scripts' ) );

				// tooltips
				require_once( KDNACommon::get_base_path() . '/tooltips.php' );
				add_filter( 'kdnaform_tooltips', array( $this, 'add_tooltips' ) );
		...
		public static function localize_results_scripts() {
			...
			$vars = array(
				...
				'imagesUrl' => KDNACommon::get_base_url() . '/images'
			);

			wp_localize_script( 'gaddon_results_js', 'gresultsVars', $vars );

			$strings = array(
				'ajaxError' => wp_strip_all_tags( __( 'Error retrieving results. If the problem persists, please contact support.', 'kdnaforms' ) )
			);
		...
		public function filter_menu_items( $menu_items, $form_id, $compact ) {
			$form_meta      = KDNAFormsModel::get_form_meta( $form_id );
			...
			if ( false === empty( $results_fields ) ) {
				...
				$sub_menu_items[] = array(
					'label'        => $this->_title,
					'icon'         => $icon,
					'title'        => esc_html__( 'View results generated by this form', 'kdnaforms' ),
				...
				if ( isset( $menu_items['results'] ) ) {
					...
					$menu_items['results']['label']          = esc_html__( 'Results', 'kdnaforms' );
					...
					$menu_items['results'] = array(
						'label'            => $compact ? esc_html__( 'Results', 'kdnaforms' ) : $this->_title,
						'icon'             => $icon,
						'title'            => esc_attr__( 'View results generated by this form', 'kdnaforms' ),
						'url'              => admin_url( "admin.php?page=gf_entries&view=gf_results_{$this->_slug}&id={$form_id}" ),
						'menu_class'       => 'kdna_form_toolbar_results',
						'link_class'       => $link_class,
						'capabilities'     => $this->_capabilities,
		...
		public function add_view( $view, $form_id ) {
			if ( $view == 'gf_results_' . $this->_slug ) {
				...
				KDNAResults::results_page( $form_id, $this->_title, 'gf_entries', $view );
			}
		}
		...
		public function results_page($form_id, $page_title, $gf_page, $gf_view ) {
			...
			$form = gf_apply_filters( array( 'kdnaform_form_pre_results', $form_id ), $form );
			...
			$filter_settings = apply_filters( 'kdnaform_filters_pre_results', $filter_settings, $form );
			...
			<div class="wrap gforms_edit_form <?php echo esc_attr( KDNACommon::get_browser_class() ); ?>">

				<?php //KDNACommon::form_page_title( $form ); ?>
				<?php //KDNACommon::display_dismissible_message(); ?>
				<?php //KDNAForms::top_toolbar(); ?>
				<?php KDNAForms::admin_header(array(), true ) ;?>
				<?php if ( false === empty( $all_fields ) ) : ?>

					<div id="poststuff" class="metabox-holder has-right-sidebar">
						<div id="side-info-column" class="inner-sidebar">
							<div id="gresults-results-filter" class="kdnaform-settings-panel__content postbox">
								...
								<div id="gresults-results-filter-content">
									<form id="gresults-results-filter-form" action="" method="GET">
										...
										$filter_ui = array(
											'fields'     => array(
												'label'   => esc_attr__( 'Include results if', 'kdnaforms' ),
												'tooltip' => 'gresults_filters',
												'markup'  => '<div class="kdnaform-settings-field__conditional-logic"><div id="gresults-results-field-filters-container">
																<!-- placeholder populated by js -->
															 </div></div>',
											),
											'start_date' => array(
												'label'   => esc_attr__( 'Start date', 'kdnaforms' ),
												'markup'  => '<div class="kdnaform-settings-field kdnaform-settings-field__date_time">
																	<span class="kdnaform-settings-input__container"><input type="text" id="gresults-results-filter-date-start" name="start" value="' . esc_attr( $start_date ) . '"/><button type="button" class="ui-datepicker-trigger"><span class="screen-reader-text">'.esc_html__( 'Open Date Picker', 'kdnaforms' ).'</span><svg width="18" height="18" role="presentation" focusable="false" fill="#9092B2" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M13.0909 1.6364V1.231C13.0909.5513 13.6357 0 14.3182 0c.6778 0 1.2273.5468 1.2273 1.2311v.4053h.8254c.8997 0 1.6291.7349 1.6291 1.6288v13.106C18 17.2707 17.2721 18 16.3709 18H1.6291C.7294 18 0 17.2651 0 16.3712V3.2652c0-.8996.728-1.6288 1.6291-1.6288h.8254V1.231C2.4545.5513 2.9993 0 3.6818 0c.6778 0 1.2273.5468 1.2273 1.2311v.4053h2.4545V1.231C7.3636.5513 7.9084 0 8.591 0c.6778 0 1.2273.5468 1.2273 1.2311v.4053h3.2727zM1.6364 7.3636v9h14.7272v-9H1.6364z"></path></svg></button></span>
																</div>',
											),
											'end_date'   => array(
												'label'   => esc_attr__( 'End date', 'kdnaforms' ),
												'markup'  => '<div class="kdnaform-settings-field kdnaform-settings-field__date_time" >
																	<span class="kdnaform-settings-input__container"><input type="text" id="gresults-results-filter-date-end" name="end" value="' . esc_attr( $end_date ) . '"/><button type="button" class="ui-datepicker-trigger"><span class="screen-reader-text">'.esc_html__( 'Open Date Picker', 'kdnaforms' ).'</span><svg width="18" height="18" role="presentation" focusable="false"  fill="#9092B2" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M13.0909 1.6364V1.231C13.0909.5513 13.6357 0 14.3182 0c.6778 0 1.2273.5468 1.2273 1.2311v.4053h.8254c.8997 0 1.6291.7349 1.6291 1.6288v13.106C18 17.2707 17.2721 18 16.3709 18H1.6291C.7294 18 0 17.2651 0 16.3712V3.2652c0-.8996.728-1.6288 1.6291-1.6288h.8254V1.231C2.4545.5513 2.9993 0 3.6818 0c.6778 0 1.2273.5468 1.2273 1.2311v.4053h2.4545V1.231C7.3636.5513 7.9084 0 8.591 0c.6778 0 1.2273.5468 1.2273 1.2311v.4053h3.2727zM1.6364 7.3636v9h14.7272v-9H1.6364z"></path></svg></button></span>
																</div>',
											),
										);
										$filter_ui = apply_filters( 'kdnaform_filter_ui', $filter_ui, $form_id, $page_title, $gf_page, $gf_view );

										foreach ( $filter_ui as $name => $filter ) {
											?>
											<div class="kdnaform-settings-field__header"><label class='kdnaform-settings-label'><?php echo esc_html( $filter['label'] ); ?><?php kdnaform_tooltip( rgar( $filter, 'tooltip' ), 'tooltip_bottomleft' ) ?></label></div>
											<?php
										...
										<div id="gresults-results-filter-buttons">
											<input type="submit" id="gresults-results-filter-submit-button"
											       ...
											       value="<?php esc_attr_e( 'Apply filters', 'kdnaforms' ); ?>">
											<input type="button" id="gresults-results-filter-clear-button"
											       class="button large"
											       value="<?php esc_attr_e( 'Clear', 'kdnaforms' ); ?>"
					...
					<div class="gresults-filter-loading" style="display:none;margin:0 5px 10px 0;">
						<i class='kdnaform-spinner'></i>&nbsp;
						<a href="javascript:void(0);" onclick="javascript:gresultsAjaxRequest.abort()" onkeypress="javascript:gresultsAjaxRequest.abort()"><?php esc_html_e( 'Cancel', 'kdnaforms' ); ?></a>
					</div>
				...
				<?php
					...
					esc_html_e( 'This form does not have any fields that can be used for results', 'kdnaforms' );
				endif ?>
			</div>

			<?php KDNAForms::admin_footer() ?>
		...
		public static function add_tooltips( $tooltips ) {
			$tooltips['gresults_total_score']          = '<h6>' . esc_html__( 'Total Score', 'kdnaforms' ) . '</h6>' . esc_html__( 'Scores are weighted calculations. Items ranked higher are given a greater score than items that are ranked lower. The total score for each item is the sum of the weighted scores.', 'kdnaforms' );
			$tooltips['gresults_agg_rank']             = '<h6>' . esc_html__( 'Aggregate Rank', 'kdnaforms' ) . '</h6>' . esc_html__( 'The aggregate rank is the overall rank for all entries based on the weighted scores for each item.', 'kdnaforms' );
			$tooltips['gresults_date_range']           = '<h6>' . esc_html__( 'Date Range', 'kdnaforms' ) . '</h6>' . esc_html__( 'Date Range is optional, if no date range is specified it will be ignored.', 'kdnaforms' );
			$tooltips['gresults_filters']              = '<h6>' . esc_html__( 'Filters', 'kdnaforms' ) . '</h6>' . esc_html__( 'Narrow the results by adding filters. Note that some field types support more options than others.', 'kdnaforms' );
			$tooltips['gresults_average_row_score']    = '<h6>' . esc_html__( 'Average Row Score', 'kdnaforms' ) . '</h6>' . esc_html__( 'The average (mean) score for each row: the sum of all the scores for each row divided by the total number of entries.', 'kdnaforms' );
			$tooltips['gresults_average_global_score'] = '<h6>' . esc_html__( 'Average Global Score', 'kdnaforms' ) . '</h6>' . esc_html__( 'The average (mean) score for the whole field. The sum of the total scores divided by the number of entries.', 'kdnaforms' );
			$tooltips['gresults_average_score']        = '<h6>' . esc_html__( 'Average Score', 'kdnaforms' ) . '</h6>' . esc_html__( 'The average (mean) score: The sum of the scores divided by the number of entries.', 'kdnaforms' );
		...
		public function ajax_get_results() {
			...
			if ( ! KDNAAPI::current_user_can_any( $this->_capabilities ) ) {
				wp_die( 'Not allowed' );
			}
			...
			$total_entries             = KDNAAPI::count_entries( $form_id, $search_criteria );
			if ( $total_entries == 0 ) {
				$html = esc_html__( 'No results.', 'kdnaforms' );
			} else {
				...
				if ( isset( $_POST['state'] ) ) {
					...
					if ( $generated_check_sum !== $posted_check_sum ) {
						...
						$output['html']   = esc_html__( 'There was an error while processing the entries. Please contact support.', 'kdnaforms' );
				...
				if ( 'incomplete' === rgar( $data, 'status' ) ) {
					...
					$output['html']        = sprintf( esc_html__( 'Entries processed: %1$d of %2$d', 'kdnaforms' ), rgar( $data, 'offset' ), $entry_count );
					echo json_encode( $output );
					die();
				}
				...
				if ( $total_entries > 0 ) {
					...
					if ( empty( $html ) ) {
						foreach ( $fields as $field ) {
							$field_id = $field->id;
							$html .= "<div class='gresults-results-field kdnaform-settings-panel' id='gresults-results-field-{$field_id}'>";
							$html .= "<header class='kdnaform-settings-panel__header'><legend class='kdnaform-settings-panel__title'>" . esc_html( KDNACommon::get_label( $field ) ) . '</legend></header>';
							$html .= '<div class="kdnaform-settings-panel__content">' . self::get_field_results( $form_id, $data, $field, $search_criteria ) . '</div>';
							$html .= '</div>';
						}
					}
				} else {
		...
		public static function get_total_entries( $form ) {
			$totals = KDNAFormsModel::get_form_counts( $form['id'] );
		...
		public static function get_field_results( $form_id, $data, $field, $search_criteria ) {
			...
			if ( empty( $data['entry_count'] ) || empty ( $data['field_data'] ) ) {
				return esc_html__( 'No entries for this field', 'kdnaforms' );
			}
			...
			switch ( $field_type ) {
				...
				case 'multiselect' :
					...
					if ( empty( $non_zero_results ) ) {
						$field_results .= esc_html__( 'No entries for this field', 'kdnaforms' );
					...
					$data_table    = array();
					$data_table [] = array( esc_html__( 'Choice', 'kdnaforms' ), esc_html__( 'Frequency', 'kdnaforms' ) );
					...
					$chart_options = array(
						...
						'hAxis'     => array(
							...
							'title'          => esc_html__( 'Frequency', 'kdnaforms' ),
						),
						'tooltip'   => array(
							'isHtml' => true,
						),
				...
				case 'likert' :
					...
					$scoring_enabled = $field->gsurveyLikertEnableScoring && class_exists( 'KDNASurvey' ) ? true : false;
					...
					if ( $multiple_rows && $scoring_enabled ) {
						$field_results .= sprintf( '<td>%s %s</td>', esc_html__( 'Average Score', 'kdnaforms' ), kdnaform_tooltip( 'gresults_average_row_score', null, true ) );
					}
					...
					if ( $scoring_enabled ) {
						...
						if ( $multiple_rows ) {
							$average_global_score_tooltip = kdnaform_tooltip( 'gresults_average_global_score', null, true );
							$field_results .= sprintf( "<div class='gsurvey-likert-score'>%s %s: %s</div>", esc_html__( 'Average global score', 'kdnaforms' ), $average_global_score_tooltip, $average_score );
						} else {
							$field_results .= sprintf( "<div class='gsurvey-likert-score'>%s %s: %s</div>", esc_html__( 'Average score', 'kdnaforms' ), kdnaform_tooltip( 'gresults_average_score', null, true ), $average_score );
						}
					}
				...
				case 'rank' :
					...
					$field_results .= esc_html__( 'Item', 'kdnaforms' );
					$field_results .= '         </td>';
					$field_results .= "         <td class='gresults-rank-field-score'>";
					$field_results .= esc_html__( 'Total Score', 'kdnaforms' ) . '&nbsp;' . kdnaform_tooltip( 'gresults_total_score', null, true );
					$field_results .= '         </td>';
					$field_results .= "         <td class='gresults-rank-field-rank'>";
					$field_results .= esc_html__( 'Aggregate Rank', 'kdnaforms' ) . '&nbsp;' . kdnaform_tooltip( 'gresults_agg_rank', null, true );
					$field_results .= '         </td>';
					$field_results .= '     </tr>';
					...
					foreach ( $results as $choice_val => $score ) {
						...
						$field_results .= KDNAFormsModel::get_choice_text( $field, $choice_val );
						$field_results .= '      </td>';
						$field_results .= "      <td class='gresults-rank-field-score'>";
						$field_results .= $score;
						$field_results .= '      </td>';
				...
				default :
					...
					$field_results .= "<div class='gresults-results-field-sub-label'>" . esc_html__( 'Latest values:', 'kdnaforms' ) . '</div>';
					...
					if ( $more_remaining ) {
						$field_results .= "<a id='gresults-results-field-more-link-{$field_id}' class='gresults-results-field-more-link' href='javascript:void(0)' onclick='gresults.getMoreResults({$form_id},{$field_id})' onkeypress='gresults.getMoreResults({$form_id},{$field_id})'>" . esc_html__( 'Show more', 'kdnaforms' ) . '</a>';
					}
					break;
			}
		...
		public function get_results_data( $form, $fields, $search_criteria = array(), $state_array = array(), $max_execution_time = 15 /* seconds */ ) {
			...
			$count_search_leads  = KDNAAPI::count_entries( $form_id, $search_criteria );
			...
			while ( $entries_left > 0 ) {
				...
				$leads                   = KDNAFormsModel::search_leads( $form_id, $search_criteria, null, $paging );
		...
		public function set_unique_page_title( $admin_title, $title ) {
			...
			$form_title  = rgar( $form, 'title', esc_html__( 'Form Not Found', 'kdnaforms' ) );
		...
		public static function get_default_field_results( $form_id, $field, $search_criteria, &$offset, $page_size, &$more_remaining = false ) {
			...
			do {
				...
				$leads  = KDNAFormsModel::search_leads( $form_id, $search_criteria, $sorting, $paging );
				foreach ( $leads as $lead ) {

					$value = KDNAFormsModel::get_lead_field_value( $lead, $field );

					$content = apply_filters( 'kdnaform_entries_field_value', $value, $form_id, $field->id, $lead );
