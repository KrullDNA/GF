...
/**
 * KDNA_Feed_Processor Class.
 *
 * @since 2.2
 */
class KDNA_Feed_Processor extends KDNA_Background_Process {
	 ...
	 * @var    string
	 */
	protected $action = 'kdna_feed_processor';
	 ...
	 * @param null|KDNAFeedAddOn $add_on                     Optional. The add-on instance.
	 */
	 ...
	 * @param null|KDNAFeedAddOn $add_on Optional. The add-on instance.
	 *
	 * @return KDNA_Feed_Processor
	 */
	public static function get_instance( $add_on = null ) {
		$key = $add_on instanceof KDNAFeedAddOn ? $add_on->get_slug() : 0;
	...
	protected function task( $item ) {
		$feed      = $item['feed'];
		$feed_name = KDNAAPI::get_feed_name( $feed );
		$feed_id   = (int) rgar( $feed, 'id' );
		$entry_id  = (int) rgar( $item, 'entry_id' );

		if ( $this->add_on instanceof KDNAFeedAddOn ) {
			$addon     = $this->add_on;
			...
			if ( ! $addon instanceof KDNAFeedAddOn ) {
				KDNACommon::log_error( __METHOD__ . "(): Aborting. Add-on ({$feed['addon_slug']}) not found for feed (#{$feed_id} - {$feed_name}) and entry #{$entry_id}." );
		...
		$form    = $this->filter_form( KDNAAPI::get_form( $form_id ), $entry );
		 ...
		 *
		 * if $max_attempts > 1 and if KDNAFeedAddOn::process_feed() throws an error or returns a WP_Error then the feed
		 ...
		 */
		$max_attempts = apply_filters( 'kdnaform_max_async_feed_attempts', $max_attempts, $form, $entry, $addon_slug, $feed );
		...
		KDNAAPI::update_processed_feeds_meta( $entry_id, $addon_slug, $feed_id, $form_id );
...
/**
 * Returns an instance of the KDNA_Feed_Processor class
 *
 * @since 2.2
 ...
 * @return KDNA_Feed_Processor
 */
function kdna_feed_processor( $add_on = null ) {
	return KDNA_Feed_Processor::get_instance( $add_on );
}
