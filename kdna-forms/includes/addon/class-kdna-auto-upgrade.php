...
class KDNAAutoUpgrade {
	...
	protected $_path;
	protected $_url;
	protected $_is_kdnaforms_supported;


	public function __construct( $slug, $version, $min_kdnaforms_version, $title, $full_path, $path, $url, $is_kdnaforms_supported ) {
		$this->_slug                      = $slug;
		$this->_version                   = $version;
		$this->_min_kdnaforms_version  = $min_kdnaforms_version;
		$this->_title                     = $title;
		$this->_full_path                 = $full_path;
		$this->_path                      = $path;
		$this->_url                       = $url;
		$this->_is_kdnaforms_supported = $is_kdnaforms_supported;
	...
	public function init() {
		if ( is_admin() ) {
			...
			add_action( 'kdnaform_after_check_update', array( $this, 'flush_version_info' ) );
			add_action( 'kdnaform_updates', array( $this, 'display_updates' ) );

			add_filter( 'kdnaform_updates_list', array( $this, 'get_update_info' ) );
		...
		add_filter( 'auto_update_plugin', function ( $update, $item ) {
			return KDNAForms::maybe_auto_update( $update, $item, $this->_slug, $this->_version );
		}, 10, 2 );
	 ...
	 *
	 * Displays if KDNA Forms isn't supported.
	 *
	 * @since  Unknown
	...
	public function rg_plugin_row() {
		...
		if ( ! $this->_is_kdnaforms_supported ) {
			...
			KDNAAddOn::display_plugin_message( $message, 'error' );
		}
	}
	...
	public function check_update( $option ) {
		...
		$minimum_requirements_evaluation_result = KDNACommon::evaluate_minimum_requirements(
	...
	private function get_changelog() {
		...
		$raw_response = KDNACommon::post_to_manager( 'changelog.php', $this->get_remote_request_params( $this->_slug, $key, $this->_version ), $options );

		if ( is_wp_error( $raw_response ) || 200 != $raw_response['response']['code'] ) {
			$text = sprintf( esc_html__( 'Oops!! Something went wrong.%sPlease try again or %scontact us%s.', 'kdnaforms' ), '<br/>', "<a href='" . esc_attr( KDNACommon::get_support_url() ). "'>", '</a>' );
		} else {
			$text = $raw_response['body'];
			if ( substr( $text, 0, 10 ) != '<!--KDNAM-->' ) {
				$text = '';
			}
		}
	...
	private function get_remote_request_params( $offering, $key, $version ) {
		...
		return sprintf( 'of=%s&key=%s&v=%s&wp=%s&php=%s&mysql=%s', urlencode( $offering ), urlencode( $key ), urlencode( $version ), urlencode( get_bloginfo( 'version' ) ), urlencode( phpversion() ), urlencode( KDNACommon::get_db_version() ) );
	}

	private function get_key() {
		if ( $this->_is_kdnaforms_supported ) {
			return KDNACommon::get_key();
		} else {
			return '';
		}
	}
	...
	public function get_update_info( $updates ) {
		...
		if ( ! rgar( $version_info, 'is_valid_key' ) ) {
			...
			$version_message = sprintf(
				'<p>%s</p>',
				sprintf(
					esc_html( '%sRegister%s your copy of KDNA Forms to receive access to automatic updates and support. Need a license key? %sPurchase one now%s.', 'kdnaforms' ),
			...
			$message_link_text = sprintf( esc_html__( 'View version %s details', 'kdnaforms' ), $version_info['version'] );
			$message_link      = sprintf( '<a href="%s" class="thickbox" title="%s">%s</a>', esc_url( $details_url ), esc_attr( $this->_title ), $message_link_text );
			$message           = sprintf( esc_html__( 'There is a new version of %1$s available. %s.', 'kdnaforms' ), $this->_title, $message_link );
			...
			$version_message = sprintf( esc_html__( 'Your version of %s is up to date.', 'kdnaforms' ), $this->_title );
		}
		...
		<div class="wrap <?php echo esc_attr( KDNACommon::get_browser_class() ); ?>">
			...
			if ( ! rgar( $version_info, 'is_valid_key' ) ) {
				...
				<div class="gf_update_expired alert_red">
					<?php printf( esc_html__( '%sRegister%s your copy of KDNA Forms to receive access to automatic updates and support. Need a license key? %sPurchase one now%s.', 'kdnaforms' ), '<a href="admin.php?page=gf_settings">','</a>','<a href="https://www.kdnaforms.com">', '</a>' ); ?>
				</div>
			...
			<?php
				...
				if ( rgar( $version_info, 'is_valid_key' ) ) {
					...
					<div class="gf_update_outdated alert_yellow">
						<?php echo $message . ' <p>' . sprintf( esc_html__( 'You can update to the latest version automatically or download the update and install it manually. %sUpdate Automatically%s %sDownload Update%s', 'kdnaforms' ), "</p><a class='button-primary' href='{$upgrade_url}'>", '</a>', "&nbsp;<a class='button' href='{$version_info['url']}'>", '</a>' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped?>
					</div>
				<?php
				}
			} else {
				...
				<div class="gf_update_current alert_green">
					<?php printf( esc_html__( 'Your version of %s is up to date.', 'kdnaforms' ), esc_html( $this->_title ) ); ?>
				</div>
			<?php
			}
