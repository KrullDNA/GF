 ...
 *
 * @package KDNAFeedAddOn
 */

require_once( 'class-kdna-addon.php' );

abstract class KDNAFeedAddOn extends KDNAAddOn {
	 ...
	 * @since 2.9.2
	 *
	 * @return (KDNAFeedAddOn|KDNAPaymentAddOn)[]
	 */
	public static function get_registered_feed_addons() {
		$addons = KDNAAddOn::get_registered_addons( true, true );

		return array_filter( $addons, function ( $addon ) {
			return $addon instanceof KDNAFeedAddOn;
		} );
	}
	...
	public function init() {
		...
		add_filter( 'kdnaform_entry_post_save', array( $this, 'maybe_process_feed' ), 10, 2 );
		add_action( 'kdnaform_after_delete_form', array( $this, 'delete_feeds' ) );
		add_action( 'kdnaform_update_status', array( $this, 'process_feed_when_unspammed' ), 10, 3 );

		// Register KDNAFrontendFeeds.
		if ( $this->_supports_frontend_feeds && ! has_action( 'kdnaform_register_init_scripts', array( __class__, 'register_frontend_feeds_init_script' ) ) ) {
			// Use priority 20 so other add-ons that support frontend feeds can all load their scripts first.
			add_action( 'kdnaform_register_init_scripts', array( __class__, 'register_frontend_feeds_init_script' ), 20 );
		}
	...
	public function init_admin() {
		...
		add_filter( 'kdnaform_notification_events', array( $this, 'notification_events' ), 10, 2 );
		add_filter( 'kdnaform_notes_avatar', array( $this, 'notes_avatar' ), 10, 2 );
		add_action( 'kdnaform_post_form_duplicated', array( $this, 'post_form_duplicated' ), 10, 2 );
	...
	public function setup() {
		...
		$installed_version = get_option( 'kdnaformsaddon_feed-base_version' );
		if ( $installed_version != $this->_feed_version ) {
			$this->upgrade_base( $installed_version );
			update_option( 'kdnaformsaddon_feed-base_version', $this->_feed_version );
		}
	...
	public function uninstall() {
		...
		if ( ! $this instanceof KDNAPaymentAddOn ) {
			kdna_feed_processor( $this )->uninstall();
		}
	}
	...
	public function maybe_process_feed( $entry, $form ) {
		...
		if ( ! empty( $processed_feeds ) ) {
			KDNAAPI::update_processed_feeds_meta( $entry_id, $this->get_slug(), $processed_feeds, $form_id );
		}
	...
	public function get_feed_name( $feed, $key = '' ) {
		return KDNAAPI::get_feed_name( $feed, $key );
	}
	...
	public function save_entry_feed_status( $result, $entry_id, $feed_id, $form_id ) {
		...
		KDNAAPI::update_entry_feed_status( $entry_id, $feed_id, $status, $form_id );
	}
	...
	public function post_process_feed( $feed, $entry, $form ) {
		$has_action = array();
		if ( has_action( 'kdnaform_post_process_feed' ) ) {
			$has_action[] = 'kdnaform_post_process_feed';
		}

		$kdnaform_slug_post_process_feed = "kdnaform_{$this->get_slug()}_post_process_feed";
		if ( has_action( $kdnaform_slug_post_process_feed ) ) {
			$has_action[] = $kdnaform_slug_post_process_feed;
		}
		 ...
		 * @param KDNAFeedAddOn $addon The current instance of the add-on.
		 */
		do_action( 'kdnaform_post_process_feed', $feed, $entry, $form, $addon );
		do_action( $kdnaform_slug_post_process_feed, $feed, $entry, $form, $addon );
	}
	...
	public function fulfill_entry( $entry_id, $form_id ) {
		if ( kdnaform_update_meta( $entry_id, "{$this->get_slug()}_is_fulfilled", true, $form_id ) ) {
	 ...
	 *
	 * Also enables use of the kdnaform_is_delayed_pre_process_feed filter.
	 *
	 * @param array $entry The Entry Object currently being processed.
	 * @param array $form The Form Object currently being processed.
	 *
	...
	public function maybe_delay_feed( $entry, $form ) {
		if ( $this->_bypass_feed_delay || $this instanceof KDNAPaymentAddOn ) {
			return false;
		}
		 ...
		 */
		$is_delayed = apply_filters( 'kdnaform_is_delayed_pre_process_feed', $is_delayed, $form, $entry, $slug );
		$is_delayed = apply_filters( 'kdnaform_is_delayed_pre_process_feed_' . $form['id'], $is_delayed, $form, $entry, $slug );
	...
	public function can_process_feed( $feed, $entry, $form ) {
		$entry_id          = (int) rgar( $entry, 'id' );
		$processed_feeds   = KDNAAPI::get_processed_feeds_meta( $entry_id, $this->get_slug() );
		 ...
		 * @param array       $entry              The entry being processed.
		 * @param array       $form               The form the entry belongs to.
		 * @param KDNAFeedAddOn $addon              The current instance of the add-on the feed belongs to.
		 * @param array       $processed_feeds    An array of feed IDs that have already been processed for the given entry.
		 *
		 * @deprecated 2.9.19 Use kdnaform_allow_feed_reprocessing instead.
		 *
		 */
		$allow_reprocessing = apply_filters_deprecated( 'kdnaform_allow_async_feed_reprocessing', array( false, $feed, $entry, $form, $this, $processed_feeds ), '2.9.19', 'kdnaform_allow_feed_reprocessing' );
		 ...
		 * @param array       $processed_feeds    An array of feed IDs that have already been processed for the given entry.
		 */
		$allow_reprocessing = apply_filters( 'kdnaform_allow_feed_reprocessing', false, $feed, $entry, $form, $this, $processed_feeds );
	...
	public function is_feed_error_retryable( $retry, $error, $feed, $entry, $form ) {
		 ...
		 * @param array    $form  The form the entry and feed belong to.
		 */
		return (bool) apply_filters( 'kdnaform_is_feed_error_retryable', $retry, $error, $feed, $entry, $form );
	}
	...
	public function create_feed_nonce() {
		...
		$action = 'kdnaform_' . $this->get_slug() . '_process_feed';
		$i      = wp_nonce_tick();
	...
	public function get_current_feed_id() {
		...
		} elseif ( ! rgempty( 'kdna_feed_id' ) ) {
			return rgpost( 'kdna_feed_id' );
		} else {
			return rgget( 'fid' );
		}
	}
	...
	public function get_feeds_by_entry( $entry_id ) {
		$processed_feeds = kdnaform_get_meta( $entry_id, 'processed_feeds' );
		if ( ! $processed_feeds ) {
			return false;
		}
	...
	public function pre_process_feeds( $feeds, $entry, $form ) {
		 ...
		 */
		$feeds = apply_filters( 'kdnaform_addon_pre_process_feeds', $feeds, $entry, $form );
		$feeds = apply_filters( "kdnaform_addon_pre_process_feeds_{$form_id}", $feeds, $entry, $form );
		$feeds = apply_filters( "kdnaform_{$this->get_slug()}_pre_process_feeds", $feeds, $entry, $form );
		$feeds = apply_filters( "kdnaform_{$this->get_slug()}_pre_process_feeds_{$form_id}", $feeds, $entry, $form );
	...
	public function update_feed_active( $id, $is_active ) {
		 ...
		 * @param KDNAFeedAddOn $this      The current instance of the add-on for which the feed is being updated.
		 */
		do_action( 'kdnaform_update_feed_active', $id, $is_active, $this );
	...
	public function delete_feed( $id ) {
		 ...
		 * @param KDNAFeedAddOn $this The current instance of the add-on for which the feed is being deleted.
		 */
	 ...
	 *
	 * @uses   KDNAFeedAddOn::can_duplicate_feed()
	 * @uses   KDNAFeedAddOn::get_feed()
	 * @uses   KDNAFeedAddOn::insert_feed()
	 * @uses   KDNAFeedAddOn::is_unique_feed_name()
	 *
	...
	public function duplicate_feed( $id, $new_form_id = false ) {
		...
		$count     = 2;
		$feed_name = $original_feed_name . ' - ' . esc_html__( 'Copy 1', 'kdnaforms' );
		while ( ! $this->is_unique_feed_name( $feed_name, $original_feed['form_id'] ) ) {
			$feed_name = $original_feed_name . ' - ' . sprintf( esc_html__( 'Copy %d', 'kdnaforms' ), $count );
			$count++;
		}
	...
	public function ajax_save_feed_order() {
		check_ajax_referer( 'kdnaform_feed_order', 'nonce' );
	 ...
	 *
	 * This method first evaluates some base criteria (whether we're in the right view of the KDNA Forms admin),
	...
	public function form_settings_header() {
		if ( $this->is_feed_list_page() ) {
			...
			return $title . " <a class='add-new-h2' href='" . esc_url( $url ) . "'>" . esc_html__( 'Add New', 'kdnaforms' ) . '</a>';
		}
	}

	public function form_settings_title() {
		return sprintf( esc_html__( '%s Feeds', 'kdnaforms' ), $this->get_short_title() );
	}
		...
		$renderer = new Settings(
			array(
				...
				'before_fields'  => function() use ( $form ) {
					...
					'<input type="hidden" name="kdna_feed_id" value="%d" />%s',
						(int) $this->get_current_feed_id(),
						KDNACommon::get_inline_script_tag( $script, false )
					);
	...
	public function get_feed_settings_entry_meta( $form, $entry_meta = array() ) {
		 ...
		 * @param array       $entry_meta An empty array or the entry meta fields to be assigned to the JavaScript entry_meta variable.
		 * @param array       $form       The form the feed is being created or edited for.
		 * @param KDNAFeedAddOn $addon      The current add-on instance.
		 */
		return apply_filters( 'kdnaform_entry_meta_pre_render_feed_settings', $entry_meta, $form, $this );
	}
	...
	public function feed_edit_page( $form, $feed_id ) {
		...
		printf( '<script type="text/javascript">%s</script>', KDNAFormSettings::output_field_scripts( false ) );  // phpcs:ignore WordPress.Security.EscapeOutput
		...
		if ( ! $this->get_settings_renderer() ) {
			...
			printf( '%s<p>%s</p>', $title, esc_html__( 'Unable to render feed settings.', 'kdnaforms' ) );  // phpcs:ignore WordPress.Security.EscapeOutput
	...
	public function settings( $sections ) {
		...
		?>
		<input type="hidden" name="kdna_feed_id" value="<?php echo esc_attr( $this->get_current_feed_id() ); ?>" />
		<?php

	}

	public function feed_settings_title() {
		return esc_html__( 'Feed Settings', 'kdnaforms' );
	}
			...
			<?php
				...
				if ( $this->_supports_feed_ordering ) {
					...
					$feed_order_options = array(
						'addon'  => $this->get_slug(),
						'formId' => rgar( $form, 'id' ),
						'nonce'  => wp_create_nonce( 'kdnaform_feed_order' ),
					);

					echo 'jQuery( document ).ready( function() {
						window.KDNAFeedOrderObj = new KDNAFeedOrder( ' . json_encode( $feed_order_options ) . ');
					} );';
	...
	public function get_feed_table( $form ) {
		...
		return new KDNAAddOnFeedsTable( $feeds, $this->get_slug(), $columns, $bulk_actions, $action_links, $column_value_callback, $no_item_callback, $message_callback, $this );
	}
	...
	public function maybe_save_feed_settings( $feed_id, $form_id ) {
		...
		if ( ! $this->current_user_can_any( $this->get_form_settings_capabilities() ) ) {
			KDNACommon::add_error_message( esc_html__( "You don't have sufficient permissions to update the form settings.", 'kdnaforms' ) );
			return $feed_id;
		}
	...
	public function trim_conditional_logic_vales( $settings, $form_id ) {
		...
		if ( isset( $settings['feed_condition_conditional_logic_object'] ) && is_array( $settings['feed_condition_conditional_logic_object'] ) ) {
			$form                                                = KDNAFormsModel::get_form_meta( $form_id );
			$settings['feed_condition_conditional_logic_object'] = KDNAFormsModel::trim_conditional_logic_values_from_element( $settings['feed_condition_conditional_logic_object'], $form );
		}
	...
	public function get_save_success_message( $sections ) {
		...
		return isset( $save_button['messages']['success'] ) ? $save_button['messages']['success'] : esc_html__( 'Feed updated successfully.', 'kdnaforms' );
	}
	...
	public function get_save_error_message( $sections ) {
		...
		return isset( $save_button['messages']['error'] ) ? $save_button['messages']['error'] : esc_html__( 'There was an error updating this feed. Please review all errors below and try again.', 'kdnaforms' );
	}
	...
	public function save_feed_settings( $feed_id, $form_id, $settings ) {
		 ...
		 * @param KDNAAddOn $addon 	The current instance of the KDNAAddOn object which extends KDNAFeedAddOn or KDNAPaymentAddOn (i.e. KDNACoupons, KDNA_User_Registration, KDNAStripe).
		 *
		 * @since 2.4.12.3
		 */
		do_action( 'kdnaform_post_save_feed_settings', $result, $form_id, $settings, $this );
	...
	public function get_feed_settings_fields() {
		 ...
		 * @param object $addon The current instance of the KDNAAddon object (i.e. KDNA_User_Registration, KDNAPayPal).
		 *
		 * @since 2.0
		 *
		 * @return array
		 */
		$feed_settings_fields = apply_filters( 'kdnaform_addon_feed_settings_fields', $this->feed_settings_fields(), $this );
		$feed_settings_fields = apply_filters( "kdnaform_{$this->get_slug()}_feed_settings_fields", $feed_settings_fields, $this );
	...
	public function get_bulk_actions() {
		$bulk_actions = array(
			'delete'    => esc_html__( 'Delete', 'kdnaforms' ),
		);
	...
	public function feed_list_no_item_message() {
		$url = add_query_arg( array( 'fid' => 0 ) );
		return sprintf( esc_html__( "You don't have any feeds configured. Let's go %screate one%s!", 'kdnaforms' ), "<a href='" . esc_url( $url ) . "'>", '</a>' );
	}
	...
	public function configure_addon_message() {
		...
		$settings_label = sprintf( __( '%s Settings', 'kdnaforms' ), $this->get_short_title() );
		$settings_link  = sprintf( '<a href="%s">%s</a>', esc_url( $this->get_plugin_settings_url() ), $settings_label );

		return sprintf( __( 'To get started, please configure your %s.', 'kdnaforms' ), $settings_link );
	...
	public function settings_feed_condition( $field, $echo = true ) {
		...
		$instructions = isset( $field['instructions'] ) ? $field['instructions'] : esc_html__( 'Process this feed if', 'kdnaforms' );
	...
	public function get_feed_condition_checkbox( $field ) {
		$checkbox_label = isset( $field['checkbox_label'] ) ? $field['checkbox_label'] : esc_html__( 'Enable Condition', 'kdnaforms' );
	...
	public function get_feed_condition_conditional_logic() {
		...
		if ( $conditional_logic_object ) {
			$form_id           = rgget( 'id' );
			$form              = KDNAFormsModel::get_form_meta( $form_id );
			$conditional_logic = json_encode( KDNAFormsModel::trim_conditional_logic_values_from_element( $conditional_logic_object, $form ) );
		} else {
			$conditional_logic = '{}';
		}
	...
	public function validate_feed_condition_settings( $field, $settings ) {
		...
		$conditional_logic_safe = KDNAFormsModel::sanitize_conditional_logic( $conditional_logic );
		if ( serialize( $conditional_logic ) != serialize( $conditional_logic_safe ) ) {
			$this->set_field_error( $field, esc_html__( 'Invalid value', 'kdnaforms' ) );
		}
	}
	...
	public function add_delayed_payment_support( $options ) {
		...
		if ( is_admin() ) {
			add_filter( 'kdnaform_addon_feed_settings_fields', array( $this, 'add_post_payment_actions' ), 10, 2 );
		}

		add_action( 'kdnaform_trigger_payment_delayed_feeds', array( $this, 'action_trigger_payment_delayed_feeds' ), 10, 4 );
	}
	 ...
	 * @param KDNAAddOn $addon                The current instance of the add-on (i.e. KDNA_User_Registration, KDNAPayPal).
	 *
	 * @return array
	 */
	public function add_post_payment_actions( $feed_settings_fields, $addon ) {
		...
		if ( $this->has_feed( $form_id ) ) {
			...
			$choice      = array(
				'label' => $addon_label ? $addon_label : sprintf( esc_html__( 'Process %s feed only when payment is received.', 'kdnaforms' ), $this->get_short_title() ),
			...
			if ( ! $field ) {
				...
				$fields = array(
					array(
						'name'    => $field_name,
						'label'   => esc_html__( 'Post Payment Actions', 'kdnaforms' ),
						'type'    => 'checkbox',
						'choices' => array( $choice ),
						'tooltip' => '<h6>' . esc_html__( 'Post Payment Actions', 'kdnaforms' ) . '</h6>' . esc_html__( 'Select which actions should only occur after payment has been received.', 'kdnaforms' )
					)
				);
	...
	public function action_trigger_payment_delayed_feeds( $transaction_id, $payment_feed, $entry, $form = null ) {
		...
		$is_fulfilled = kdnaform_get_meta( $entry['id'], "{$this->get_slug()}_is_fulfilled" );
		...
		if ( is_null( $form ) ) {
			$form = KDNAFormsModel::get_form_meta( $entry['form_id'] );
		}
	...
	public function get_paypal_feed( $form_id, $entry ) {
		...
		if ( ! class_exists( 'KDNAPayPal' ) ) {
			return false;
		}

		if ( method_exists( 'KDNAPayPal', 'get_config_by_entry' ) ) {
			$feed = KDNAPayPal::get_config_by_entry( $entry );
		} elseif ( method_exists( 'KDNAPayPal', 'get_config' ) ) {
			$feed = KDNAPayPal::get_config( $form_id );
		} else {
			$feed = false;
		}
	...
	public function has_paypal_payment( $feed, $form, $entry ) {
		...
		foreach ( $products['products'] as $field_id => $product ) {
			...
			$product_price = KDNACommon::to_number( $product['price'] );
		...
		if ( ! empty( $products['shipping']['name'] ) && ! is_numeric( $payment_field ) ) {
			$line_items[] = array( 'id'          => '',
			                       ...
			                       'description' => '',
			                       'quantity'    => 1,
			                       'unit_price'  => KDNACommon::to_number( $products['shipping']['price'] ),
			                       'is_shipping' => 1
			);
	...
	public static function get_paypal_payment_amount( $form, $entry, $paypal_config ) {
		...
		$products        = KDNACommon::get_product_fields( $form, $entry, true );
		...
		foreach ( $products['products'] as $id => $product ) {
			...
			if ( $paypal_config['meta']['type'] != 'subscription' || $recurring_field == $id || $recurring_field == 'all' ) {
				$price = KDNACommon::to_number( $product['price'] );
				if ( is_array( rgar( $product, 'options' ) ) ) {
					foreach ( $product['options'] as $option ) {
						$price += KDNACommon::to_number( $option['price'] );
					}
				}
	...
	public function get_frontend_feeds( $form ) {
		...
		foreach ( $feeds as $feed ) {
			...
			$_feed = apply_filters( 'kdnaform_addon_frontend_feed',                           $_feed, $form, $feed );
			$_feed = apply_filters( "kdnaform_addon_frontend_feed_{$form['id']}",             $_feed, $form, $feed );
			$_feed = apply_filters( "kdnaform_{$this->get_slug()}_frontend_feed",                  $_feed, $form, $feed );
			$_feed = apply_filters( "kdnaform_{$this->get_slug()}_frontend_feed_{$form['id']}",    $_feed, $form, $feed );
...
class KDNAAddOnFeedsTable extends WP_List_Table {
	...
	function __construct( $feeds, $slug, $columns, $bulk_actions, $action_links, $column_value_callback, $no_items_callback, $message_callback, $addon_class ) {
		...
		$standard_cols = array(
			'cb'        => esc_html__( 'Checkbox', 'kdnaforms' ),
			'is_active' => '',
		);
		...
		parent::__construct(
			array(
				'singular' => esc_html__( 'feed', 'kdnaforms' ),
				'plural'   => esc_html__( 'feeds', 'kdnaforms' ),
				'ajax'     => false,
			)
		);
	}
