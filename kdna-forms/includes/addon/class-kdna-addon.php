...
use KDNA_Forms\KDNA_Forms\Settings\KDNA_Settings_Encryption;
...
use KDNA_Forms\KDNA_Forms\Save_Form\KDNA_Save_Form_Helper;
use KDNA_Forms\KDNA_Forms\Theme_Layers\Framework\Engines\Output_Engines\Form_CSS_Properties_Output_Engine;
use KDNA_Forms\KDNA_Forms\Theme_Layers\API\Fluent\Theme_Layer_Builder;

/**
 * Class KDNAAddOn
 *
 * Handles all tasks mostly common to any KDNA Forms Add-On, including third party ones.
 */
abstract class KDNAAddOn {
	...
	protected $_version;
	/**
	 * The minimum KDNA Forms version required for the add-on to load.
	 *
	 * @var string KDNA Forms minimum version requirement
	 */
	protected $_min_kdnaforms_version;
	 ...
	 * @since 2.7.12
	 */
	protected $_min_compatible_kdnaforms_version;
	...
	protected $_slug;
	/**
	 * @var string Relative path to the plugin from the plugins folder. Example "kdnaforms/kdnaforms.php"
	 */
	protected $_path;
	/**
	...
	/**
	 * @var string URL to the KDNA Forms website. Example: 'http://www.kdnaforms.com' OR affiliate link.
	 */
	protected $_url;
	/**
	 * @var string Title of the plugin to be used on the settings page, form settings and plugins page. Example: 'KDNA Forms Mailchimp Add-On'
	 */
	protected $_title;
	/**
	 ...
	 *
	 * @var false|\KDNA_Forms\KDNA_Forms\Settings\Settings
	 */
	 ...
	 *
	 * @var \KDNA_Forms\KDNA_Forms\Settings\KDNA_Settings_Encryption
	 */
	private $_encryptor;
	...
	/**
	 * @var bool Used by KDNA plugins to activate auto-upgrade.
	 * @ignore
	 */
	...
	public function init_theme_layer() {
		...
		$layer->set_name( $this->theme_layer_slug() )
			  ...
			  ->register();
		add_action( 'kdnaform_form_after_open', array( $this, 'output_third_party_styles' ), 998, 2 );
	}

	/**
	 * Helper method that returns the theme styles that should be enqueued for the add-on. Returns an array in the format accepted by the KDNA Forms theme layer set_styles() method
	 *
	 * @since 2.9.0
	 *
	...
	public function get_themes_to_enqueue ( $form, $field_types = '' ) {
		return \KDNAFormDisplay::get_themes_to_enqueue( $form, $field_types );
	}
	 ...
	 *
	 * @return string[]|(KDNAAddOn|KDNAFeedAddOn|KDNAPaymentAddOn)[] An array of class names or instances.
	 */
	public static function get_registered_addons( $return_instances = false, $slug_as_key = false ) {
		...
		foreach ( $active_addons as $addon ) {
			...
			/** @var KDNAAddOn|KDNAFeedAddOn|KDNAPaymentAddOn $instance */
	 ...
	 *
	 * @return KDNAAddOn Returns an instance of the add-on with the specified slug.
	 */
	...
	public function pre_init() {
		...
		if ( $this->is_kdnaforms_supported() ) {
	...
	public function init() {
		...
		add_filter( 'kdnaform_logging_supported', array( $this, 'set_logging_supported' ) );

		add_action( 'kdnaform_post_upgrade', array( $this, 'post_kdnaforms_upgrade' ), 10, 3 );
		...
		if ( RG_CURRENT_PAGE == 'admin-ajax.php' ) {
			...
			//If kdna forms is supported, initialize AJAX
			if ( $this->is_kdnaforms_supported() && $meets_requirements['meets_requirements'] ) {
				$this->init_ajax();
			}
		} elseif ( is_admin() ) {
	...
	public function init_admin() {
		...
		// message enforcing min version of KDNA Forms
		if ( isset( $this->_min_kdnaforms_version ) && RG_CURRENT_PAGE == 'plugins.php' ) {
		...
		add_filter( 'kdnaform_noconflict_styles', array( $this, 'register_noconflict_styles' ) );
		add_action( 'kdnaform_enqueue_scripts', array( $this, 'enqueue_scripts' ), 10, 2 );
	...
	public function init_frontend() {
		...
		add_filter( 'kdnaform_preview_styles', array( $this, 'enqueue_preview_styles' ), 10, 2 );
		add_filter( 'kdnaform_print_styles', array( $this, 'enqueue_print_styles' ), 10, 2 );
		add_action( 'kdnaform_enqueue_scripts', array( $this, 'enqueue_scripts' ), 10, 2 );
	 ...
	 * @access public
	 *
	 * @uses KDNAAddOn::minimum_requirements()
	 * @uses KDNAAddOn::get_slug()
	 *
	 * @return bool|array
	 */
	public function meets_minimum_requirements() {
		...
		// Set an error if the minimum version of KDNA Forms is defined and the requirement is not met.
		if ( ! empty( $this->_min_kdnaforms_version ) && ! $this->is_kdnaforms_supported( $this->_min_kdnaforms_version ) ) {
			$meets_requirements = array(
				'meets_requirements' => false,
				'errors'             => array(
					esc_html__(
						sprintf(
							'%s requires KDNA Forms %s or newer. Please upgrade your installation of KDNA Forms or disable this add-on to remove this message.',
							$this->_title,
							$this->_min_kdnaforms_version
						),
						'kdnaforms'
					),
				),
			...
			switch ( $requirement_type ) {
				...
				case 'add-ons':
					...
					foreach ( $requirement as $addon_slug => $addon_requirements ) {
						...
						if ( ! isset( $active_addons[ $addon_slug ] ) ) {
							...
							$meets_requirements['errors'][]           = sprintf( esc_html__( 'Required KDNA Forms Add-On is missing: %s.', 'kdnaforms' ), $addon_name );
							continue;
						...
						if ( rgar( $addon_requirements, 'version' ) && ! version_compare( $active_addons[ $addon_slug ]['version'], $addon_requirements['version'], '>=' ) ) {
							$meets_requirements['meets_requirements'] = false;
							$meets_requirements['errors'][]           = sprintf( esc_html__( 'Required KDNA Forms Add-On "%s" does not meet minimum version requirement: %s.', 'kdnaforms' ), $active_addons[ $addon_slug ]['title'], $addon_requirements['version'] );
							continue;
						}
					}
					...
					foreach ( $requirement as $plugin_path => $plugin_config ) {
						...
						if ( ! is_plugin_active( $plugin_path ) ) {
							...
							if( ! empty( $plugin_version ) ) {
								$meets_requirements['errors'][] = sprintf( esc_html__( 'Required WordPress plugin is missing: %1$s %2$s or newer.', 'kdnaforms' ), $plugin_name, $plugin_version );
							} else {
								$meets_requirements['errors'][] = sprintf( esc_html__( 'Required WordPress plugin is missing: %s.', 'kdnaforms' ), $plugin_name );
							}
							continue;
						}
						...
						if ( ! empty( $plugin_version ) ) {
							...
							if ( ! version_compare( $installed_version, $plugin_version, '>=' ) ) {
								$meets_requirements['meets_requirements'] = false;
								$meets_requirements['errors'][]           = sprintf( esc_html__( 'Required WordPress plugin "%1$s" is installed but does not meet minimum version requirement: %2$s.', 'kdnaforms' ), $plugin_name, $plugin_version );
								continue;
							}
						}
					}
				...
				case 'php':
					...
					if ( rgar( $requirement, 'version' ) && ! version_compare( PHP_VERSION, $requirement['version'], '>=' ) ) {
						$meets_requirements['meets_requirements'] = false;
						$meets_requirements['errors'][]           = sprintf( esc_html__( 'Current PHP version (%s) does not meet minimum PHP version requirement (%s).', 'kdnaforms' ), PHP_VERSION, $requirement['version'] );
					}
					...
					if ( rgar( $requirement, 'extensions' ) ) {
						...
						foreach ( $requirement['extensions'] as $extension => $extension_requirements ) {
							...
							if ( ! extension_loaded( $extension ) ) {
								$meets_requirements['meets_requirements'] = false;
								$meets_requirements['errors'][]           = sprintf( esc_html__( 'Required PHP extension missing: %s', 'kdnaforms' ), $extension );
								continue;
							}
							...
							if ( rgar( $extension_requirements, 'version' ) && ! version_compare( phpversion( $extension ), $extension_requirements['version'], '>=' ) ) {
								$meets_requirements['meets_requirements'] = false;
								$meets_requirements['errors'][]           = sprintf( esc_html__( 'Required PHP extension "%s" does not meet minimum version requirement: %s.', 'kdnaforms' ), $extension, $extension_requirements['version'] );
								continue;
							}
					...
					if ( rgar( $requirement, 'functions' ) ) {
						...
						foreach ( $requirement['functions'] as $function ) {
							if ( ! function_exists( $function ) ) {
								$meets_requirements['meets_requirements'] = false;
								$meets_requirements['errors'][]           = sprintf( esc_html__( 'Required PHP function missing: %s', 'kdnaforms' ), $function );
							}
						}
				...
				case 'wordpress':
					...
					if ( rgar( $requirement, 'version' ) && ! version_compare( get_bloginfo( 'version' ), $requirement['version'], '>=' ) ) {
						$meets_requirements['meets_requirements'] = false;
						$meets_requirements['errors'][]           = sprintf( esc_html__( 'Current WordPress version (%s) does not meet minimum WordPress version requirement (%s).', 'kdnaforms' ), get_bloginfo( 'version' ), $requirement['version'] );
					}
	...
	/**
	 * Register failed requirements page under KDNA Forms settings.
	 *
	 * @since  2.2
	 * @access public
	 *
	 * @uses KDNAAddOn::current_user_can_any()
	 * @uses KDNAAddOn::get_short_title()
	 * @uses KDNAAddOn::plugin_settings_title()
	 * @uses KDNACommon::get_base_path()
	 * @uses KDNAForms::add_settings_page()
	 */
	public function failed_requirements_init() {
		...
		$error_message = sprintf(
			'%s<br />%s<ol>%s</ol>',
			sprintf( esc_html__( '%s is not able to run because your WordPress environment has not met the minimum requirements.', 'kdnaforms' ), $this->_title ),
			sprintf( esc_html__( 'Please resolve the following issues to use %s:', 'kdnaforms' ), $this->get_short_title() ),
			$errors
		);

		// Add error message.
		if ( $this->is_form_list() || $this->is_entry_list() || $this->is_form_settings() || $this->is_plugin_settings() || KDNAForms::get_page() === 'system_status' ) {
	...
	public function setup() {
		...
		//Upgrading add-on
		$installed_version = get_option( 'kdnaformsaddon_' . $this->get_slug() . '_version' );
		...
		if ( $installed_version != $this->_version ) {
			$installed_version = KDNAForms::get_wp_option( 'kdnaformsaddon_' . $this->get_slug() . '_version' );
		}
		...
		if ( $installed_version != $this->_version ) {
			...
			update_option( 'kdnaformsaddon_' . $this->get_slug() . '_version', $this->_version );
		}
	}
	 ...
	 *        array(
	 *            'handle'    => 'maskedinput',
	 *            'src'       => KDNACommon::get_base_url() . '/js/jquery.maskedinput-1.3.min.js',
	 *            'version'   => KDNACommon::$version,
	 *            'deps'      => array( 'jquery' ),
	 *            'in_footer' => false,
	 *
	 ...
	 *                'stringKey1' => __( 'The string', 'kdnaforms' ),
	 *                'stringKey2' => __( 'Another string.', 'kdnaforms' )
	 *            )
	 *            "enqueue"  => array(
	...
	/**
	 * Target of admin_enqueue_scripts and kdnaform_enqueue_scripts hooks.
	...
	/**
	 * Target of kdnaform_preview_styles. Enqueue styles to the preview page.
	 * Not intended to be overridden by child classes
	 *
	 * @ignore
	 */
	...
	/**
	 * Target of kdnaform_print_styles. Enqueue styles to the print entry page.
	 * Not intended to be overridden by child classes
	 *
	 * @ignore
	 */
	...
	public function output_third_party_styles( $markup, $form ) {
		...
		$all_block_settings = apply_filters( 'kdnaform_form_block_attribute_values', array() );
		...
		return $markup . KDNACommon::get_inline_script_tag( $script, false );
	}
	...
	private function _field_condition_matches( $field_types, $form ) {
		...
		$fields = KDNAAPI::get_fields_by_type( $form, $field_types );
		if ( count( $fields ) > 0 ) {
			foreach ( $fields as $field ) {
				if ( $field->is_administrative() && ! $field->allowsPrepopulate && ! KDNAForms::get_page() ) {
					continue;
				}
	...
	/**
	 * Target for the kdnaform_noconflict_scripts filter. Adds scripts to the list of white-listed no conflict scripts.
	 *
	...
	public function register_noconflict_scripts( $scripts ) {
		//registering scripts with KDNA Forms so that they get enqueued when running in no-conflict mode
		return array_merge( $scripts, $this->_no_conflict_scripts );
	}

	/**
	 * Target for the kdnaform_noconflict_styles filter. Adds styles to the list of white-listed no conflict scripts.
	 *
	 * Not intended to be overridden or called directed by Add-Ons.
	 *
	 * @ignore
	...
	public function register_noconflict_styles( $styles ) {
		//registering styles with KDNA Forms so that they get enqueued when running in no-conflict mode
	 ...
	 *
	 *
	 * @param array $entry_meta An array of entry meta already registered with the kdnaform_entry_meta filter.
	 ...
	 *      return array(
	 *       "title" => 'Quiz Results',
	 *       "capabilities" => array("kdnaforms_quiz_results"),
	 *       "callbacks" => array(
	...
	public function has_members_plugin() {
		return KDNAForms::has_members_plugin();
	}

	/**
	 * Register the KDNA Forms Add-Ons capabilities group with the Members plugin.
	 *
	 * @since  2.4
	 * @access public
	 */
	public function members_register_cap_group() {
		...
		members_register_cap_group(
			'kdnaforms_addons',
			array(
				'label' => esc_html__( 'KDNA Add-Ons', 'kdnaforms' ),
	...
	public function get_members_caps() {
		...
		if ( ! empty( $this->get_form_settings_capabilities() ) && is_string( $this->get_form_settings_capabilities() ) ) {
			$caps[ $this->get_form_settings_capabilities() ] = esc_html__( 'Form Settings', 'kdnaforms' );
		}
		if ( ! empty( $this->_capabilities_uninstall ) && is_string( $this->_capabilities_uninstall ) ) {
			$caps[ $this->_capabilities_uninstall ] = esc_html__( 'Uninstall', 'kdnaforms' );
		}
		if ( ! empty( $this->_capabilities_plugin_page ) && is_string( $this->_capabilities_plugin_page ) ) {
			$caps[ $this->_capabilities_plugin_page ] = esc_html__( 'Add-On Page', 'kdnaforms' );
		}
		if ( ! empty( $this->_capabilities_settings_page ) && is_string( $this->_capabilities_settings_page ) ) {
			$caps[ $this->_capabilities_settings_page ] = esc_html__( 'Add-On Settings', 'kdnaforms' );
		}

		$results_cap = rgars( $this->get_results_page_config(), 'capabilities/0' );
		if ( ! empty( $results_cap ) && $results_cap !== 'kdnaforms_view_entries' && ! isset( $caps[ $results_cap ] ) ) {
			$caps[ $results_cap ] = esc_html__( 'Results Page', 'kdnaforms' );
		}
	...
	/**
	 * Register KDNA Forms Add-Ons capabilities group with User Role Editor plugin.
	 *
	 * @since  2.4
	 *
	...
	public static function filter_ure_capabilities_groups_tree( $groups = array() ) {
		...
		$groups['kdnaforms_addons'] = array(
			'caption' => esc_html__( 'KDNA Forms Add-Ons', 'kdnaforms' ),
			'parent'  => 'kdnaforms',
			'level'   => 3,
		);
	...
	public function current_user_can_any( $caps ) {
		return KDNACommon::current_user_can_any( $caps );
	}
	 ...
	 * @since 2.5
	 *
	 * @return false|\KDNA_Forms\KDNA_Forms\Settings
	 */
	 ...
	 * @since 2.5
	 *
	 * @param \KDNA_Forms\KDNA_Forms\Settings\Settings $renderer Settings renderer.
	 *
	 * @return bool|WP_Error
	 */
	public function set_settings_renderer( $renderer ) {

		// Ensure renderer is an instance of KDNA_Forms\KDNA_Forms\Settings\Settings.
		...
		foreach ( $sections as $s => &$section ) {
			if ( array_key_exists( 'sections', $section ) ) {
				...
				if ( $s === $first_section && in_array( $type, array( 'plugin_settings' ) ) && ! rgar( $section, 'title', false ) ) {
					$section['title'] = sprintf( esc_html__( '%s Settings', 'kdnaforms' ), $this->get_short_title() );
				}
	...
	public function single_setting_row( $field ) {
		...
		if ( $this->get_settings_renderer() ) {
			...
			// Initialize field.
			$field = \KDNA_Forms\KDNA_Forms\Settings\Fields::create( $field, $this->get_settings_renderer() );

			if ( is_wp_error( $field ) ) {
				esc_html_e( 'Field could not be rendered.', 'kdnaforms' );
				return;
			}
	...
	public function single_setting( $field ) {
		...
		if ( is_callable( rgar( $field, 'callback' ) ) ) {
			...
			printf( esc_html__( "Field type '%s' has not been implemented", 'kdnaforms' ), esc_html( $field['type'] ) );
		}
	}
	...
	public static function maybe_decode_json( $value ) {
		return KDNACommon::maybe_decode_json( $value );
	}

	public static function is_json( $value ) {
		return KDNACommon::is_json( $value );
	}
	...
	public function get_save_success_message( $sections ) {
		...
		return isset( $save_button['messages']['success'] ) ? $save_button['messages']['success'] : sprintf( esc_html__( '%s settings updated.', 'kdnaforms' ), $this->get_short_title() );
	}

	public function get_save_error_message( $sections ) {
		$save_button = $this->get_save_button( $sections );

		return isset( $save_button['messages']['error'] ) ? $save_button['messages']['error'] : esc_html__( 'There was an error while saving your settings.', 'kdnaforms' );
	}
	 ...
	 * @since 2.7.17
	 *
	 * @param \KDNA_Forms\KDNA_Forms\Settings\KDNA_Settings_Encryption $encryptor Settings encryptor.
	 *
	 * @return void
	 */
	 ...
	 * @since 2.7.17
	 *
	 * @return KDNA_Settings_Encryption Returns the current instance of the settings encryptor.
	 */
	public function get_encryptor() {
		if ( ! $this->_encryptor ) {
			require_once( KDNACommon::get_base_path() . '/includes/settings/class-kdna-settings-encryption.php' );
			$this->_encryptor = new KDNA_Settings_Encryption();
		}
	...
	public function settings_text( $field, $echo = true ) {
		...
		$field = \KDNA_Forms\KDNA_Forms\Settings\Fields::create(
			$field,
	...
	public function checkbox_input( $choice, $attributes, $value, $tooltip ) {
		return \KDNA_Forms\KDNA_Forms\Settings\Fields\Checkbox::render_input( $choice, $attributes, $value, $tooltip );
	}
	...
	public function choices_have_icon( $choices = array() ) {
		...
		return \KDNA_Forms\KDNA_Forms\Settings\Fields\Base::has_icons( $choices );
	...
	public function get_select_options( $choices, $selected_value ) {
		return \KDNA_Forms\KDNA_Forms\Settings\Fields\Select::get_options( $choices, $selected_value );
	}
	 ...
	 * @access public
	 *
	 * @uses KDNAAddOn::field_failed_validation()
	 * @uses KDNACommon::get_base_url()
	 * @uses KDNAAddOn::get_current_forn()
	 * @uses KDNAAddOn::get_error_icon()
	 * @uses KDNAAddOn::get_mapping_field()
	 * @uses KDNAAddOn::settings_hidden()
	 *
	 * @param array $field Field array containing the configuration options of this field.
	 ...
	 * @access public
	 *
	 * @uses KDNAAddOn::get_field_map_choices()
	 * @uses KDNA_Field::get_form_editor_field_title()
	 *
	 * @param array $field    Field array containing the configuration options of this field.
	...
	public function settings_field_map_select( $field, $form_id ) {
		...
		if ( empty( $field['choices'] ) || ( count( $field['choices'] ) == 1 && rgblank( $field['choices'][0]['value'] ) ) ) {
			...
			if ( ( ! is_array( $field_type ) && ! rgblank( $field_type ) ) || ( is_array( $field_type ) && count( $field_type ) == 1 ) ) {
				...
				$type = ucfirst( KDNA_Fields::get( $type )->get_form_editor_field_title() );

				return sprintf( __( 'Please add a %s field to your form.', 'kdnaforms' ), $type );
	 ...
	 * @access public
	 *
	 * @uses KDNAAddOn::get_current_form()
	 * @uses KDNAAddOn::get_field_map_choices()
	 *
		...
		if ( empty( $select_field['choices'] ) ) {
			...
			if ( ! $has_gf_custom ) {
				...
				if ( $type == 'key' ) {
					...
					$label         = esc_html__( 'Add Custom Key', 'kdnaforms' );
					...
					$label         = esc_html__( 'Add Custom Value', 'kdnaforms' );
	 ...
	 * @access public
	 *
	 * @uses KDNAAddOn::field_map_title()
	 *
	 * @return string
	 */
	public function field_map_table_header() {
		...
		return '<thead>
					<tr>
						...
						<th>' . esc_html__( 'Form Field', 'kdnaforms' ) . '</th>
					</tr>
				</thead>';
	 ...
	 * @access public
	 *
	 * @used-by KDNAAddOn::field_map_table_header()
	 *
	 * @return string
	 */
	public function field_map_title() {

		return esc_html__( 'Field', 'kdnaforms' );
	 ...
	 * @uses KDNAFormsModel::get_entry_meta()
	 * @uses KDNAFormsModel::get_form_meta()
	 * @uses KDNA_Field::get_entry_inputs()
	 * @uses KDNA_Field::get_form_editor_field_title()
	 * @uses KDNA_Field::get_input_type()
	 *
	...
	public static function get_field_map_choices( $form_id, $field_type = null, $exclude_field_types = null ) {
		...
		if ( rgblank( $field_type ) || ( is_array( $field_type ) && count( $field_type ) > 1 ) ) {
			...
			$first_choice_label = __( 'Select a Field', 'kdnaforms' );
			...
			$first_choice_label = sprintf( __( 'Select a %s Field', 'kdnaforms' ), $type );
		...
		if ( is_null( $field_type ) ) {
			$fields[] = array( 'value' => 'id', 'label' => esc_html__( 'Entry ID', 'kdnaforms' ) );
			$fields[] = array( 'value' => 'date_created', 'label' => esc_html__( 'Entry Date', 'kdnaforms' ) );
			$fields[] = array( 'value' => 'ip', 'label' => esc_html__( 'User IP', 'kdnaforms' ) );
			$fields[] = array( 'value' => 'source_url', 'label' => esc_html__( 'Source Url', 'kdnaforms' ) );
			$fields[] = array( 'value' => 'form_title', 'label' => esc_html__( 'Form Title', 'kdnaforms' ) );
			...
			foreach ( $form['fields'] as $field ) {
				...
				if ( is_array( $inputs ) && $field_is_valid_type && ! $exclude_field ) {
					...
					if ( $input_type == 'address' ) {
						$fields[] = array(
							'value' => $field->id,
							'label' => strip_tags( KDNACommon::get_label( $field ) . ' (' . esc_html__( 'Full', 'kdnaforms' ) . ')' )
						);
					}
					...
					if ( $input_type == 'checkbox' ) {
						$fields[] = array(
							'value' => $field->id,
							'label' => strip_tags( KDNACommon::get_label( $field ) . ' (' . esc_html__( 'Selected', 'kdnaforms' ) . ')' )
						);
					}
					...
					foreach ( $inputs as $input ) {
						$fields[] = array(
							'value' => $input['id'],
							'label' => strip_tags( KDNACommon::get_label( $field, $input['id'] ) )
						);
					}
					...
					foreach ( $field->choices as $column ) {
						$fields[] = array(
							...
							'label' => strip_tags( KDNACommon::get_label( $field ) . ' (' . esc_html( rgar( $column, 'text' ) ) . ')' ),
						);
						$col_index ++;
					}
				} elseif ( ! $field->displayOnly && $field_is_valid_type && ! $exclude_field ) {
					$fields[] = array( 'value' => $field->id, 'label' => strip_tags( KDNACommon::get_label( $field ) ) );
				}
			}
		}
		 ...
		 */
		$fields = apply_filters( 'kdnaform_addon_field_map_choices', $fields, $form_id, $field_type, $exclude_field_types );
		...
		if ( function_exists( 'get_called_class' ) ) {
			...
			if ( is_callable( $callable ) ) {
				...
				$fields = apply_filters( "kdnaform_{$slug}_field_map_choices", $fields, $form_id, $field_type, $exclude_field_types );
			}
		}
	 ...
	 * @access public
	 *
	 * @used-by KDNAAddOn::settings_field_map()
	 * @used-by KDNAAddOn::validate_field_map_settings()
	 *
	 ...
	 *
	 * @uses KDNACommon::replace_variables()
	 *
	 * @return array
	 */
	...
	public function get_default_field_select_field( $field ) {
		...
		if ( ! is_a( $field, 'KDNA_Forms\KDNA_Forms\Settings\Field\Field_Select' ) ) {
			$field['type'] = 'field_select';
			$field         = \KDNA_Forms\KDNA_Forms\Settings\Fields::create( $field, $this->get_settings_renderer() );
		}
	...
	public function get_form_fields_as_choices( $form, $args = array() ) {
		 ...
		 *
		 * @var \KDNA_Forms\KDNA_Forms\Settings\Fields\Field_Select|WP_Error $field
		 */
		$field = \KDNA_Forms\KDNA_Forms\Settings\Fields::create(
			array(
				'type' => 'field_select',
	 ...
	 *
	 * @deprecated 2.5 Use \KDNA_Forms\KDNA_Forms\Settings\Fields\Button to add a Save button.
	 *
	 * @param array $field - Field array containing the configuration options of this field
	 * @param bool  $echo  = true - true to echo the output to the screen, false to simply return the contents as a string
	 *
	...
	public function settings_save( $field, $echo = true ) {
		...
		$field['type']  = 'submit';
		$field['name']  = 'kdnaform-settings-save';
	...
	public function get_field_attributes( $field, $default = array() ) {
		...
		if ( ! $field instanceof KDNA_Forms\KDNA_Forms\Settings\Fields\Base ) {
	...
	public function get_choice_attributes( $choice, $field_attributes, $default_choice_attributes = array() ) {
		...
		return \KDNA_Forms\KDNA_Forms\Settings\Fields\Base::get_choice_attributes( $choice, $field_attributes, $default_choice_attributes );
	...
	private function perform_legacy_validation_check( $field, $settings ) {
		...
		if ( ! is_a( $field_object, '\KDNA_Forms\KDNA_Forms\Settings\Fields\Base' ) ) {
	...
	public function validate_text_settings( $field, $settings ) {
		...
		_deprecated_function( __METHOD__, '2.5', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Text::is_valid()' );
	...
	public function validate_textarea_settings( $field, $settings ) {
		...
		_deprecated_function( __METHOD__, '2.5', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Textarea::is_valid()' );
	...
	public function validate_radio_settings( $field, $settings ) {
		...
		_deprecated_function( __METHOD__, '2.5', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Radio::is_valid()' );
	...
	public function validate_select_settings( $field, $settings ) {
		...
		_deprecated_function( __METHOD__, '2.5', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Select::is_valid()' );
	...
	public function validate_checkbox_settings( $field, $settings ) {
		...
		_deprecated_function( __METHOD__, '2.5', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Checkbox::is_valid()' );
	...
	public function validate_select_custom_settings( $field, $settings ) {
		...
		_deprecated_function( __METHOD__, '2.5', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Select_Custom::is_valid()' );
	...
	public function validate_field_select_settings( $field, $settings ) {
		...
		_deprecated_function( __METHOD__, '2.5', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Field_Select::is_valid()' );
	...
	public function validate_field_map_settings( $field, $settings ) {
		...
		_deprecated_function( __METHOD__, '2.5', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Generic_Map::is_valid()' );
	...
	public function validate_checkbox_and_select_settings( $field, $settings ) {
		...
		_deprecated_function( __METHOD__, '2.5', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Checkbox_And_Select::is_valid()' );
	 ...
	 * @since Unknown
	 *
	 * @param \KDNA_Forms\KDNA_Forms\Settings\Fields\Base|array $field         Field object.
	...
	public function set_field_error( &$field, $error_message = '' ) {
		...
		if ( ! $error_message ) {
			$error_message = esc_html__( 'This field is required.', 'kdnaforms' );
		}

		if ( is_a( $field, 'KDNA_Forms\KDNA_Forms\Settings\Fields\Base' ) ) {
	 ...
	 * @since Unknown
	 *
	 * @param \KDNA_Forms\KDNA_Forms\Settings\Fields\Base|array|boolean $field - Optional. The field meta. When specified, errors for this field will be returned
	 *
	...
	public function get_field_errors( $field = false ) {
		...
		} elseif ( is_a( $field, 'KDNA_Forms\KDNA_Forms\Settings\Fields\Base' ) ) {
	...
	public function maybe_get_tooltip( $item ) {
		...
		return KDNA_Forms\KDNA_Forms\Settings\Settings::maybe_get_tooltip( $item );
	 ...
	 * @since Unknown
	 *
	 * @param array|\KDNA_Forms\KDNA_Forms\Settings\Fields\Base $field - The field meta to be checked
	 *
	...
	public function field_failed_validation( $field ) {
		...
		$field_error = is_a( $field, 'KDNA_Forms\KDNA_Forms\Settings\Fields\Base' ) ? $field->get_error() : null;
	 ...
	 * @param array|KDNA_Forms\KDNA_Forms\Settings\Fields\Base[] $fields   Field(s) to add.
	 ...
	 *
	 * @return \KDNA_Forms\KDNA_Forms\Settings\Fields\Base|array|bool
	 */
	...
	public function simple_condition( $setting_name_root ) {
		...
		$value_input = esc_js( '_kdnaform_setting_' . esc_attr( $setting_name_root ) . '_value' );
		...
		$str .= $this->settings_select( array(
			...
			'choices'  => array(
				array(
					'value' => 'is',
					'label' => esc_html__( 'is', 'kdnaforms' ),
				),
				array(
					'value' => 'isnot',
					'label' => esc_html__( 'is not', 'kdnaforms' ),
				),
				array(
					'value' => '>',
					'label' => esc_html__( 'greater than', 'kdnaforms' ),
				),
				array(
					'value' => '<',
					'label' => esc_html__( 'less than', 'kdnaforms' ),
				),
				array(
					'value' => 'contains',
					'label' => esc_html__( 'contains', 'kdnaforms' ),
				),
				array(
					'value' => 'starts_with',
					'label' => esc_html__( 'starts with', 'kdnaforms' ),
				),
				array(
					'value' => 'ends_with',
					'label' => esc_html__( 'ends with', 'kdnaforms' ),
				),
			),
	...
	public function is_simple_condition_met( $setting_name_root, $form, $entry, $feed = array() ) {
		...
		// Build the logic array to be used by KDNA Forms when evaluating the rules.
		$logic = array(
			'logicType' => 'all',
			'rules'     => array(
				array(
		...
		return KDNACommon::evaluate_conditional_logic( $logic, $form, $entry );
	}
	...
	public function form_settings_init() {
		...
		if ( KDNAForms::get_page_query_arg() == 'gf_edit_forms' && $view == 'settings' && $subview == $this->get_slug() && $this->current_user_can_any( $this->get_form_settings_capabilities() ) ) {
			...
			if ( $this->method_is_overridden( 'form_settings_fields' ) ) {
				...
				// Get current form.
				$form = KDNACommon::kdnaform_admin_pre_render( $this->get_current_form() );
	...
	public function plugin_page_init() {
		...
		if ( $this->current_user_can_any( $this->_capabilities_plugin_page ) ) {
			...
			add_filter( 'kdnaform_addon_navigation', array( $this, 'create_plugin_page_menu' ) );
		}
	 ...
	 * Target of kdnaform_addon_navigation filter. Creates a menu item in the left nav, linking to the plugin page
	 *
	...
	public function form_settings_page() {
		...
		// Display page header.
		KDNAFormSettings::page_header( $this->_title );
		...
		if ( $this->method_is_overridden( 'form_settings' ) ) {
			...
			if ( $renderer ) {
				...
				printf( '<p>%s</p>', esc_html__( 'Unable to render form settings.', 'kdnaforms' ) );
			}

		}

		// Display page footer.
		KDNAFormSettings::page_footer();
	...
	public function save_form_settings( $form, $settings ) {
		$existing_meta     = KDNAFormsModel::get_form_meta( $form['id'] );
		...
		$result               = KDNAFormsModel::update_form_meta( $form['id'], $form );
	...
	public function create_app_menu() {
		...
		$min_cap         = KDNACommon::current_user_can_which( $this->_capabilities_app_menu );
		if ( empty( $min_cap ) ) {
		 ...
		 */
		$addon_menus = apply_filters( 'kdnaform_addon_app_navigation_' . $this->get_slug(), $addon_menus );
		 ...
		 */
		$menu_position = apply_filters( 'kdnaform_app_menu_position_' . $this->get_slug(), $menu_position );
		$this->app_hook_suffix = add_menu_page( $this->get_short_title(), $this->get_short_title(), $has_full_access ? 'kdnaform_full_access' : $min_cap, $parent_menu['name'], $callback, $this->get_app_menu_icon(), $menu_position );
		...
		foreach ( $menu_items as $menu_item ) {
			...
			add_submenu_page( $parent_menu['name'], $menu_item['label'], $menu_item['label'], $has_full_access || empty( $menu_item['permission'] ) ? 'kdnaform_full_access' : $menu_item['permission'], $menu_item['name'], $callback );
		}

		if ( is_array( $addon_menus ) ) {
			foreach ( $addon_menus as $addon_menu ) {
				add_submenu_page( $parent_menu['name'], $addon_menu['label'], $addon_menu['label'], $has_full_access ? 'kdnaform_full_access' : $addon_menu['permission'], $addon_menu['name'], $addon_menu['callback'] );
			}
		}

		if ( $this->has_app_settings() ) {
			add_submenu_page( $parent_menu['name'], esc_html__( 'Settings', 'kdnaforms' ), esc_html__( 'Settings', 'kdnaforms' ), $has_full_access ? 'kdnaform_full_access' : $this->_capabilities_app_settings, $this->get_slug() . '_settings', array( $this, 'app_tab_page' ) );
		}
	...
	private function get_parent_menu( $menu_items, $addon_menus ) {
		$parent = false;
		if ( KDNACommon::current_user_can_any( $this->_capabilities_app_menu ) ) {
			foreach ( $menu_items as $menu_item ) {
	...
	public function app_tab_page() {
		$page        = sanitize_text_field( KDNAForms::get_page_query_arg() );
		...
		if ( empty( $current_tab ) ) {
			wp_die( esc_html__( "You don't have adequate permission to view this page", 'kdnaforms' ) );
		}
		 ...
		 */
		$action_hook = 'kdnaform_addon_app_' . $page . '_' . str_replace( ' ', '_', $current_tab );
	...
	public function plugin_settings_page() {
		...
		if ( $this->has_deprecated_elements() ) {
			printf(
				...
				esc_html__( 'This add-on needs to be updated. Please contact the developer.', 'kdnaforms' )
			);
		}
		...
		if ( $this->method_is_overridden( 'plugin_settings' ) ) {
			...
			printf(
				...
				sprintf(
					esc_html__( '%s has been successfully uninstalled. It can be re-activated from the %splugins page%s.', 'kdnaforms' ),
	...
	public function plugin_settings_title() {
		return sprintf( esc_html__( "%s Settings", "kdnaforms" ), $this->get_short_title() );
	}
	...
	public function get_plugin_settings() {
		...
		self::$_plugin_settings[ $this->get_slug() ] = $this->get_encryptor()->decrypt( get_option( 'kdnaformsaddon_' . $this->get_slug() . '_settings' ) );
	...
	public function update_plugin_settings( $settings ) {
		...
		update_option( 'kdnaformsaddon_' . $this->get_slug() . '_settings', $this->get_encryptor()->encrypt( $settings ) );
	}
	...
	public function get_app_settings_tabs() {
		...
		$setting_tabs = array( array( 'name' => 'settings', 'label' => esc_html__( 'Settings', 'kdnaforms' ), 'callback' => array( $this, 'app_settings_tab' ) ) );

		/**
		 * Filters the tabs within the settings menu.
		 *
		 * This filter is appended by the page slug.  Ex: kdnaform_addon_app_settings_menu_SLUG
		 *
		 * @param array $setting_tabs Contains the information on the settings tabs.
		 */
		$setting_tabs = apply_filters( 'kdnaform_addon_app_settings_menu_' . $this->get_slug(), $setting_tabs );

		if ( $this->current_user_can_uninstall() ) {
			$setting_tabs[] = array( 'name' => 'uninstall', 'label' => esc_html__( 'Uninstall', 'kdnaforms' ), 'callback' => array( $this, 'app_settings_uninstall_tab' ) );
		}
	...
	public function app_settings_uninstall_tab() {
		...
		if ( $this->maybe_uninstall() ) {
			...
			<div class="push-alert-gold" style="border-left: 1px solid #E6DB55; border-right: 1px solid #E6DB55;">
				<?php printf( esc_html__( '%s has been successfully uninstalled. It can be re-activated from the %splugins page%s.', 'kdnaforms' ), esc_html( $this->_title ), "<a href='plugins.php'>", '</a>' ); ?>
			</div>
		<?php
			...
			<form action="" method="post">
				...
				<?php  ?>
					<h3>
						<span><i class="fa fa-times"></i> <?php printf( esc_html__( 'Uninstall %s', 'kdnaforms' ), $this->get_short_title() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
					</h3>
					...
					<div class="delete-alert alert_red">
						...
						<?php
						$uninstall_button = '<input type="submit" name="uninstall" value="' . sprintf( esc_attr__( 'Uninstall %s', 'kdnaforms' ), $this->get_short_title() ) . '" class="button" onclick="return confirm(\'' . esc_js( $this->uninstall_confirm_message() ) . '\');" onkeypress="return confirm(\'' . esc_js( $this->uninstall_confirm_message() ) . '\');"/>';
	...
	public function app_tab_page_header( $tabs, $current_tab, $title, $message = '' ) {
		...
		<div class="wrap <?php echo esc_attr( KDNACommon::get_browser_class() ); ?>">
			...
			<div class="kdnaform-settings__content" id="tab_<?php echo esc_attr( $current_tab ); ?>">
	...
	public function app_tab_page_footer() {
				...
				<!-- / kdnaform-settings__content -->
			</div>
			<!-- / kdnaform-settings__wrapper -->
	...
	public function get_app_settings() {
		return get_option( 'kdnaformsaddon_' . $this->get_slug() . '_app_settings' );
	}
	...
	public function update_app_settings( $settings ) {
		update_option( 'kdnaformsaddon_' . $this->get_slug() . '_app_settings', $settings );
	}
	...
	public function maybe_save_app_settings() {
		...
		if ( $this->is_save_postback() ) {
			...
			if ( ! $this->current_user_can_any( $this->_capabilities_app_settings ) ) {
				KDNACommon::add_error_message( esc_html__( "You don't have sufficient permissions to update the settings.", 'kdnaforms' ) );
				return false;
			}
			...
			if ( $is_valid ) {
				...
				KDNACommon::add_message( $this->get_save_success_message( $sections ) );
			} else {
				KDNACommon::add_error_message( $this->get_save_error_message( $sections ) );
			}
		}
	...
	public function uninstall_message() {
		return sprintf(
				__( 'This operation deletes ALL %s settings.', 'kdnaforms' ),
	...
	public function render_uninstall() {
		...
		if ( rgget( 'subview' ) == 'uninstall' ) {
			...
			<form action="" method="post" class="kdnaform-settings-panel kdnaform-settings-panel__addon-uninstall">
				...
				<div class="kdnaform-settings-panel__content">
					...
					<div class="addon-uninstall-text">
						<h4 class="kdnaform-settings-panel__title"><?php printf( esc_html__( '%s', 'kdnaforms' ), $this->get_short_title() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></h4>
			...
			?>
			<form action="" method="post" class="kdnaform-settings-panel kdnaform-settings-panel--collapsible kdnaform-settings-panel--collapsed kdnaform-settings-panel__uninstall">
				<?php wp_nonce_field( 'uninstall', 'kdna_addon_uninstall' ); ?>
				<header class="kdnaform-settings-panel__header">
					<h4 class="kdnaform-settings-panel__title"><?php printf( esc_html__( 'Uninstall %s Add-On', 'kdnaforms' ), $this->get_short_title() ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></h4>
					<span class="kdnaform-settings-panel__collapsible-control">
						<input
							type="checkbox"
							name="kdnaform_settings_section_collapsed_uninstall"
							id="kdnaform_settings_section_collapsed_uninstall"
							value="1"
							onclick="this.checked ? this.closest( '.kdnaform-settings-panel' ).classList.add( 'kdnaform-settings-panel--collapsed' ) : this.closest( '.kdnaform-settings-panel' ).classList.remove( 'kdnaform-settings-panel--collapsed' )"
							checked
						/>
						<label class="kdnaform-settings-panel__collapsible-toggle" for="kdnaform_settings_section_collapsed_uninstall"><span class="screen-reader-text"><?php esc_html_e( 'Toggle Uninstall Section' ); ?></span></label>
					</span>
				</header>
				<div class="kdnaform-settings-panel__content">
					...
					<button type="submit" name="uninstall" value="uninstall" class="button red" onclick="return confirm('<?php echo esc_js( $this->uninstall_confirm_message() ); ?>');" onkeypress="return confirm('<?php echo esc_js( $this->uninstall_confirm_message() ); ?>');"><?php esc_attr_e( 'Uninstall Add-On', 'kdnaforms' ); ?></button>
				</div>
			</form>
			<?php
		}
			...
			<div class="kdnaform-settings-panel__content">
				...
				<div class="addon-uninstall-button">
					<a href="<?php echo esc_url( $url ); ?>" aria-label="<?php echo 'Visit ' . $this->get_short_title() . ' Settings page'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" class="button addon-settings">
						<i class="kdnaform-icon kdnaform-icon--cog"></i>
	...
	public function uninstall_warning_message() {
		return sprintf( esc_html__( '%sThis operation deletes ALL %s settings%s. If you continue, you will NOT be able to retrieve these settings.', 'kdnaforms' ), '<strong>', esc_html( $this->get_short_title() ), '</strong>' );
	}

	public function uninstall_confirm_message() {
		return sprintf( __( "Warning! ALL %s settings will be deleted. This cannot be undone. 'OK' to delete, 'Cancel' to stop", 'kdnaforms' ), __( $this->get_short_title() ) );
	}
	/**
	 * Not intended to be overridden or called directly by Add-Ons.
	 *
	...
	public function uninstall_addon() {
		...
		if ( ! $this->current_user_can_uninstall() ) {
			die( esc_html__( "You don't have adequate permission to uninstall this add-on: " . $this->_title, 'kdnaforms' ) );
		}
		...
		$forms        = KDNAFormsModel::get_forms();
		$all_form_ids = array();

		// remove entry meta
		$meta_table = version_compare( KDNAFormsModel::get_database_version(), '2.3-dev-1', '<' ) ? KDNAFormsModel::get_lead_meta_table_name() : KDNAFormsModel::get_entry_meta_table_name();
		...
		if ( ! empty( $all_form_ids ) ) {
			$form_metas = KDNAFormsModel::get_form_meta_by_id( $all_form_ids );
			...
			foreach ( $form_metas as $form_meta ) {
				if ( isset( $form_meta[ $this->get_slug() ] ) ) {
					...
					KDNAFormDetail::save_form_info( $form_meta['id'], addslashes( $form_json ) );
				}
			}
		}

		//removing options
		delete_option( 'kdnaformsaddon_' . $this->get_slug() . '_settings' );
		delete_option( 'kdnaformsaddon_' . $this->get_slug() . '_app_settings' );
		delete_option( 'kdnaformsaddon_' . $this->get_slug() . '_version' );
	...
	/**
	 * Target for the after_plugin_row action hook. Checks whether the current version of KDNA Forms
	...
	public function plugin_row( $plugin_name, $plugin_data ) {
		if ( false === $this->_enable_rg_autoupgrade && ! self::is_kdnaforms_supported( $this->_min_kdnaforms_version ) ) {
			$message = $this->plugin_message();
			self::display_plugin_message( $message, 'error' );
		}

		if ( self::is_kdnaforms_supported( $this->_min_kdnaforms_version ) && ! self::is_kdnaforms_compatible() ) {
		...
		KDNAForms::maybe_display_update_notification( $plugin_name, $plugin_data, $this->get_slug(), $this->_version );
		...
		$minimum_requirements_evaluation_result = KDNACommon::evaluate_minimum_requirements( $minimum_requirements );
		if ( $minimum_requirements_evaluation_result['block'] == true ) {
			...
			if ( version_compare( $this->_version, $plugin_data['new_version'], '<' ) ) {
				...
				$message_link_text   = sprintf( esc_html__( 'View version %s details', 'kdnaforms' ), $plugin_data['new_version'] );
				$message_link        = sprintf( '<a href="%s" class="thickbox" title="%s">%s</a>', esc_url( $details_url ), esc_attr( $this->_title ), $message_link_text );
				$message_new_version = sprintf( esc_html__( 'There is a new version of %s available. %s.', 'kdnaforms' ), $this->_title, $message_link );
	...
	/**
	 * Returns the message that will be displayed if the current version of KDNA Forms is not supported.
	 *
	 * Override this method to display a custom message.
	 */
	public function plugin_message() {
		$message = sprintf( esc_html__( 'KDNA Forms %s is required. Activate it now or %spurchase it today!%s', 'kdnaforms' ), $this->_min_kdnaforms_version, "<a href='https://www.kdnaforms.com'>", '</a>' );

		return $message;
	}

	/**
	 * Returns the message that will be displayed if the current version of KDNA Forms is not compatible with the add-on.
	 *
	 * Override this method to display a custom message.
	 *
	 * @since 2.7.12
	...
	public function compatibility_message() {
		$message = esc_html__( 'Some features of the add-on are not available on the current version of KDNA Forms. Please update to the latest KDNA Forms version for full compatibility.', 'kdnaforms' );
	...
	public function get_field_value( $form, $entry, $field_id ) {
		 ...
		 * @param string $slug The add-on slug e.g. kdnaformsactivecampaign.
		 *
		 * @since 1.9.15.12
		 *
		 * @return string
	...
	/**
	 * Enables use of the kdnaform_SLUG_field_value filter to override the field value. Override this function to prevent the filter being used or to implement a custom filter.
	 *
	...
	public function get_full_address( $entry, $field_id ) {
		...
		return KDNA_Fields::get( 'address' )->get_value_export( $entry, $field_id );
	}
	...
	public function get_full_name( $entry, $field_id ) {
		...
		return KDNA_Fields::get( 'name' )->get_value_export( $entry, $field_id );
	}
	...
	public function notes_avatar( $avatar, $note ) {
		if ( $note->user_name == $this->_short_title && empty( $note->user_id ) && $this->method_is_overridden( 'note_avatar', 'KDNAAddOn' ) ) {
	...
	public function add_note( $entry_id, $note, $sub_type = null ) {
		...
		return KDNAFormsModel::add_note( $entry_id, $user_id, $user_name, $note, $note_type, $sub_type );
	}
	 ...
	 * @return bool
	 */
	protected final function method_is_overridden( $method_name, $base_class = 'KDNAAddOn' ) {
		try {
	...
	private static function _get_base_path() {
		...
		return KDNACommon::get_base_path() . '/includes/' . $folder;
	}
	...
	private static function _get_base_url() {
		...
		return KDNACommon::get_base_url() . '/includes/' . $folder;
	}

	/**
	 * Checks whether the KDNA Forms is installed.
	 *
	 * @return bool
	 */
	public function is_kdnaforms_installed() {
		return class_exists( 'KDNAForms' );
	}

	public function table_exists( $table_name ) {

		return KDNACommon::table_exists( $table_name );

	}

	/**
	 * Checks whether the current version of KDNA Forms is supported
	 *
	 * @param $min_kdnaforms_version
	 *
	 * @return bool|mixed
	 */
	public function is_kdnaforms_supported( $min_kdnaforms_version = '' ) {
		if ( isset( $this->_min_kdnaforms_version ) && empty( $min_kdnaforms_version ) ) {
			$min_kdnaforms_version = $this->_min_kdnaforms_version;
		}

		if ( empty( $min_kdnaforms_version ) ) {
			return true;
		}

		return version_compare( KDNAForms::$version, $min_kdnaforms_version, '>=' );
	}

	/**
	 * Checks whether the current version of KDNA Forms is compatible with all features of an add-on.
	 *
	 * @since 2.7.12
	 *
	 * @param string $min_compatible_kdnaforms_version The version to compare the current version with.
	 *
	 * @return bool|mixed
	 */
	public function is_kdnaforms_compatible( $min_compatible_kdnaforms_version = '' ) {
		if ( isset( $this->_min_kdnaforms_version ) && empty( $min_compatible_kdnaforms_version ) ) {
			$min_compatible_kdnaforms_version = $this->_min_compatible_kdnaforms_version;
		}

		if ( empty( $min_compatible_kdnaforms_version ) ) {
			return true;
		}

		static $results = array();

		if ( ! isset( $results[ $min_compatible_kdnaforms_version ] ) ) {
			$results[ $min_compatible_kdnaforms_version ] = version_compare( KDNAForms::$version, $min_compatible_kdnaforms_version, '>=' );
		}

		return $results[ $min_compatible_kdnaforms_version ];
	}

	/**
	 * Display an upgrade notice if the current version of KDNA Forms is not fully supported.
	 *
	 * @since 2.7.12
	 */
	public function maybe_display_upgrade_notice() {
		if ( $this->is_kdnaforms_compatible() ) {
			return;
		}
		...
		$message = sprintf(
			...
			esc_html__(
				'Some features of the %1$s Add-on are not available on this version of KDNA Forms. Please update to the latest version for full compatibility.',
				'kdnaforms'
			),
			$this->get_short_title()
		);
	 ...
	 * is setup in a similar fashion to KDNA Forms (`class="kdnaform-icon kdnaform-icon--icon-name"`).
	...
	public function get_current_form() {
		return rgempty( 'id', $_GET ) ? false : KDNAFormsModel::get_form_meta( rgget( 'id' ) );  // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	}
	...
	public function is_save_postback() {
		return ! rgempty( 'kdnaform-settings-save' );
	}
	...
	public function is_app_settings( $tab = '' ) {
		...
		$is_app_settings = KDNAForms::get_page_query_arg() == $this->get_slug() . '_settings';
		$is_tab          = $this->_tab_matches( $tab );
	...
	public function is_plugin_page() {
		...
		return KDNAForms::get_page_query_arg() == strtolower( $this->get_slug() );
	}
	...
	public function has_deprecated_elements() {
		$deprecated = KDNAAddOn::get_all_deprecated_protected_methods( get_class( $this ) );
		if ( ! empty( $deprecated ) ) {
			return true;
		}
	...
	public static function get_all_deprecated_protected_methods($add_on_class_name = ''){
		$deprecated = array();
		$deprecated = array_merge( $deprecated, self::get_deprecated_protected_methods_for_base_class( 'KDNAAddOn', $add_on_class_name )) ;
		$deprecated = array_merge( $deprecated, self::get_deprecated_protected_methods_for_base_class( 'KDNAFeedAddOn', $add_on_class_name ) ) ;
		$deprecated = array_merge( $deprecated, self::get_deprecated_protected_methods_for_base_class( 'KDNAPaymentAddOn', $add_on_class_name ) ) ;
		return $deprecated;
	}
	...
	/**
	 * Returns the add-on slug with the kdnaforms prefix removed.
	 *
	 * @since 2.4.18
	 *
	 * @return string
	...
	public function get_short_slug() {
		return str_replace( 'kdnaforms', '', $this->get_slug() );
	}
	...
	public function get_installed_locales() {
		...
		return KDNACommon::get_installed_translations( $this->get_slug() );
	}
	...
	public function current_user_can_uninstall(){
		...
		return KDNACommon::current_user_can_uninstall( $this->_capabilities_uninstall, $this->get_path() );
	 ...
	 *
	 * @see KDNAAddOn::uninstall_addon()
	 *
	 * @since  2.5
	 *
	 * @param array $uninstallable_addons Array of KDNAAddOn objects.
	 */
	public static function addons_for_uninstall( $uninstallable_addons ) {
		?>
		<div class="kdnaform-addons-uninstall-panel">
			<?php
