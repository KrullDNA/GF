...
/**
 * Class KDNA_Settings_Encryption
 *
 * @since 2.7.17
 *
...
class KDNA_Settings_Encryption {
	 ...
	 * @since 2.7.17
	 *
	 * @param string $encryption_key Encryption key to use for encrypting / decrypting settings. Defaults to KDNA_ENCRYPTION_KEY constant.
	 *
	 * @return void
	 */
	function __construct( $encryption_key = '' ) {
		if ( ! empty( $encryption_key ) ) {
			...
			$this->_encryption_key = defined( 'KDNA_ENCRYPTION_KEY' ) ? KDNA_ENCRYPTION_KEY : '';
		}
	}
	...
	/**
	 * Returns the value of the KDNA_ENCRYPTION_KEY constant for the settings page.
	 *
	 * @since 2.7.17
	 *
	 * @return string The value of the KDNA_ENCRYPTION_KEY constant.
	 */
	...
	public function encrypt( $settings ) {
		...
		if ( ! empty( $settings_wrapper ) && ! rgar( $settings_wrapper, 'encrypted' ) ) {
			...
			$settings = json_encode(
				array(
					'encrypted' => true,
					'value'     => \KDNACommon::openssl_encrypt( json_encode( $settings_wrapper ), $key, 'aes-256-ctr', $key ),
				)
			);
		}
	...
	public function decrypt( $settings ) {
		...
		if ( rgar( $settings, 'encrypted' ) ) {
			$key      = $this->get_key();
			$settings = json_decode( \KDNACommon::openssl_decrypt( rgar( $settings, 'value' ), $key, 'aes-256-ctr', $key ), true );
		}
