...
class Conditional_Logic extends Base {
	...
	public function __construct( $props, $settings ) {
		...
		if ( $this->object_type === 'feed_condition' && ! isset( $this->instructions ) ) {
			$this->instructions = esc_html__( 'Process this feed if', 'kdnaforms' );
		}

		// Translate Checkbox label.
		if ( $this->checkbox['label'] === 'Enable Condition' ) {
			$this->checkbox['label'] = esc_html__( 'Enable Condition', 'kdnaforms' );
		}
	...
	public function markup() {
		...
		$html .= sprintf(
			'<div id="%s_conditional_logic_container" class="kform-settings-field__conditional-logic">
	...
	private function get_logic_object() {
		...
		return KDNAFormsModel::trim_conditional_logic_values_from_element( $object, $this->settings->get_current_form() );
	}
}

Fields::register( 'conditional_logic', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Conditional_Logic' );
