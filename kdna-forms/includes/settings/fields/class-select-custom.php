...
class Select_Custom extends Base {
		...
		if ( ! $has_custom_option ) {
			...
			if ( rgar( $this->inputs['select'], 'label' ) ) {
				$custom_label = sprintf(
					'%s %s',
					esc_html__( 'Add Custom', 'kdnaforms' ),
					esc_html( $this->inputs['select']['label'] )
				);
			} else {
				$custom_label = esc_html__( 'Add Custom Value', 'kdnaforms' );
			}
	...
	public function markup() {
		...
		$html .= sprintf(
			'<span class="%s">%s<div class="kform-settings-select-custom__custom" %s>%s%s</div>%s</span>',
			esc_attr( $this->get_container_classes() ),
			$this->inputs['select']->markup(),
			$custom_input_display,
			count( $this->inputs['select']->choices ) > 1 ? '<button type="button" class="kform-settings-select-custom__reset"><span class="screen-reader-text">' . esc_html__( 'Reset', 'kdnaforms' ) . '</span></button>' : '',
	...
	public function do_validation( $value ) {
		...
		if ( $select_value === 'kdna_custom' ) {
			...
			$this->inputs['select']->set_error( esc_html__( 'Invalid selection.', 'kdnaforms' ) );
...
Fields::register( 'select_custom', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Select_Custom' );
