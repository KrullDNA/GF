	...
	public function markup() {
		...
		if ( ( $choices === false || empty( $choices ) ) && ! empty( $this->no_choices ) ) {
			...
			if ( $this->enhanced_ui ) {
				...
				// Wrap select input.
				$html .= sprintf( '<span class="kform-settings-field__select--enhanced">%s</span>', $select_input );

				$html .= '<script type="text/javascript">
					jQuery( document ).ready( function () {
						jQuery( "#' . esc_attr( $input_id ) . '" ).select2( {
							minimumResultsForSearch: Infinity,
							dropdownCssClass: "kform-settings-field__select-enhanced-container",
...
Fields::register( 'select', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Select' );
