...
class Radio extends Base {
	...
	public function __construct( $props, $settings ) {
		...
		// Prepare class.
		$class = self::has_icons( rgar( $props, 'choices', array() ) ) ? array( 'kform-settings-field__radio--visual' ) : array();
	...
	public function markup() {
		...
		$horizontal_class = rgobj( $this, 'horizontal' ) || self::has_icons( $choices ) ? ' kform-settings-choice--inline' : '';
		$icon_class       = self::has_icons( $choices ) ? ' kform-settings-choice--visual' : '';

		if ( $this->image_select ) {
			$icon_class = ' kform-settings-choice--image-select';
		}
		...
		foreach ( $choices as $i => $choice ) {
			...
			if ( self::has_icons( $choices ) ) {
				...
				$icon_markup = KDNACommon::get_icon_markup( $choice, 'fa-cog' );

			}

			$html .= sprintf(
				'<div id="kform-settings-radio-choice-%1$s" class="kform-settings-choice%2$s%3$s">
					%4$s
					<label for="%1$s">
						<span>%5$s <span class="kform-settings-choice-label">%6$s</span> %7$s</span>
					</label>
				</div>',
				esc_attr( $choice['id'] ),
				$icon_class,
	...
	public function do_validation( $value ) {
		...
		$this->set_error( esc_html__( 'Invalid selection.', 'kdnaforms' ) );

	}

}

Fields::register( 'radio', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Radio' );
