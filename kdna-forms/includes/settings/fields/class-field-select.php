...
class Field_Select extends Select {
	 ...
	 * @param \KDNA_Forms\KDNA_Forms\Settings\Settings $settings Settings instance.
	 */
	public function __construct( $props, $settings ) {
		...
		if ( ! $this->args['disable_first_choice'] ) {
			...
			if ( ( ! empty( $this->args['input_types'] ) && is_string( $this->args['input_types'] ) ) || ( is_array( $this->args['input_types'] ) && count( $this->args['input_types'] ) === 1 ) ) {
				...
				$field      = KDNA_Fields::get( $input_type );

				// Prepare first choice label.
				if ( $field ) {
					$first_choice_label = sprintf( __( 'Select a %s Field', 'kdnaforms' ), ucwords( $field->get_form_editor_field_title() ) );
				}
				...
				switch ( $field->get_input_type() ) {
					...
					case 'name':
						$choices[] = array(
							...
							'label' => sprintf(
								'%s (%s)',
								strip_tags( KDNACommon::get_label( $field ) ),
								esc_html__( 'Full', 'kdnaforms' )
							),
						);
						break;
...
Fields::register( 'field_select', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Field_Select' );
