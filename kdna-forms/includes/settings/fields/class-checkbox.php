...
class Checkbox extends Base {
	...
	public function markup() {
		...
		$horizontal_class          = rgobj( $this, 'horizontal' ) || $have_icon ? ' kform-settings-choice--inline' : '';
		...
		foreach ( array_values( $choices ) as $i => $choice ) {
			...
			$html .= sprintf(
				'<div id="kform-settings-checkbox-choice-%s" class="kform-settings-choice%s%s">',
				esc_attr( $choice['id'] ),
				$horizontal_class,
				rgar( $choice, 'icon' ) ? ' kform-settings-choice--visual' : ''
			);
	...
	public function array_markup() {
		...
		foreach ( array_values( $choices ) as $i => $choice ) {
			...
			$choice_attributes          = self::get_choice_attributes( $choice, $this->get_attributes(), $default_choice_attributes );
			$choice_attributes['name']  = 'name="_kdnaform_setting_' . $this->name . '[]"';
...
Fields::register( 'checkbox', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Checkbox' );
