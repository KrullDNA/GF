...
class Base implements ArrayAccess {
	...
	public function prepare_markup() {
		...
		if ( ! empty( $this->fields ) ) {
			$html .= '<fieldset class="kform-settings-nested-fields">';
	...
	public function get_attributes( $default = array() ) {
		...
		$excluded_atts = apply_filters(
			'kaddon_no_output_field_properties',
			array(
	...
	public function get_description() {
		...
		return isset( $this->description ) ? sprintf( '<span class="kform-settings-description" id="description-%s">%s</span>', $this->name, $this->description ) : '';
	...
	public function get_container_classes() {
		...
		$container_classes = array( 'kform-settings-input__container' );

		if ( is_bool( $is_feedback_valid ) ) {
			$container_classes[] = $is_feedback_valid ? 'kform-settings-input__container--feedback-success' : 'kform-settings-input__container--feedback-error';
		}

		if ( is_string( $is_feedback_valid ) ) {
			$container_classes[] = sprintf( 'kform-settings-input__container--feedback-%s', $is_feedback_valid );
		}

		if ( $is_invalid ) { $container_classes[] = 'kform-settings-input__container--invalid'; }
		if ( isset( $this->append ) ) { $container_classes[] = 'kform-settings-input__container--with-append'; }
		if ( $this->class && strpos( $this->class, 'merge-tag-support' ) !== false ) { $container_classes[] = 'kform-settings-input__container--with-merge-tag'; }
	...
	public function validate( $value ) {
		_deprecated_function( __METHOD__, '2.5-beta-3', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Base::handle_validation()' );
	...
	public function is_valid( $value ) {
		...
		_deprecated_function( __METHOD__, '2.5-beta-3', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Base::do_validation()' );
	...
	public function get_error_icon() {
		...
		return $error ? sprintf( '<div class="kform-settings-validation__error" id="error-%s">%s</div>', $this->name, $error ) : '';
	...
	public function save_field( $field_values, $field_value ) {
		...
		if ( is_array( $name ) ) {
			return KDNACommon::set_array_value( $field_values, $name, $field_value );
		}
	 ...
	 *
	 * Examples:
	 * - A string of kdnaformsapi will return kdnaformsapi.
	 * - A string of kdnaformsapi[log_level] will return an array of [ 'kdnaformsapi', 'log_level' ].
	 *
	 * @since 2.5
	 *
	 * @return string|array
