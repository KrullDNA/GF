<?php

namespace KDNA_Forms\KDNA_Forms\Settings\Fields;

use KDNA_Forms\KDNA_Forms\Settings\Fields;

defined( 'ABSPATH' ) || die();

class Button extends Base {

	/**
	 * Field type.
	 *
	 * @since 2.5
	 *
	 * @var string
	 */
	public $type = 'button';

	/**
	 * Button input type.
	 *
	 * @since 2.5
	 *
	 * @var string
	 */
	public $input_type = 'button';

	/**
	 * Button classes.
	 *
	 * @var string
	 */
	public $class = 'kdnaform_settings_button button';

	/**
	 * Initialize Button field.
	 *
	 * @since 2.5
	 *
	 * @param array                                $props    Field properties.
	 * @param \KDNA_Forms\KDNA_Forms\Settings\Settings $settings Settings instance.
	 */
	public function __construct( $props, $settings ) {

		parent::__construct( $props, $settings );

		// Set default value.
		if ( ! isset( $this->default_value ) ) {
			$this->default_value = esc_html__( 'Submit', 'kdnaforms' );
		}

	}





	// # RENDER METHODS ------------------------------------------------------------------------------------------------

	/**
	 * Render field.
	 *
	 * @since 2.5
	 *
	 * @return string
	 */
	public function markup() {

		// Prepare markup.
		$html = sprintf(
			'<button type="%s" name="%s" value="save" %s>%s</button>',
			$this->input_type,
			$this->name,
			implode( ' ', $this->get_attributes() ),
			esc_attr( $this->default_value )
		);

		return $html;

	}

}

Fields::register( 'button', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Button' );
