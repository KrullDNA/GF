...
class Generic_Map extends Base {
	 ...
	 * @param \KDNA_Forms\KDNA_Forms\Settings\Settings $settings Settings framework instance.
	 */
	public function __construct( $props, $settings ) {
		...
		if ( empty( $this->key_field['title'] ) ) {
			$this->key_field['title'] = esc_html__( 'Key', 'kdnaforms' );
		}
		if ( empty( $this->key_field['custom'] ) ) {
			$this->key_field['custom'] = esc_html__( 'Custom Key', 'kdnaforms' );
		}
		if ( empty( $this->value_field['title'] ) ) {
			$this->value_field['title'] = esc_html__( 'Value', 'kdnaforms' );
		}
		if ( empty( $this->value_field['custom'] ) ) {
			$this->value_field['custom'] = esc_html__( 'Custom Value', 'kdnaforms' );
		}
	...
	public function markup() {
		...
		if ( empty( $this->key_field['choices'] ) && $this->type !== 'generic_map' && ! $this->key_field['allow_custom'] ) {
			return esc_html__( 'No mapping fields are available.', 'kdnaforms' );
		}
	...
	public function populate_default_key_values( $choices ) {
		...
		$global_aliases = array(
			__( 'First Name', 'kdnaforms' ) => array( __( 'Name (First)', 'kdnaforms' ) ),
			__( 'Last Name', 'kdnaforms' )  => array( __( 'Name (Last)', 'kdnaforms' ) ),
			__( 'Address', 'kdnaforms' )    => array( __( 'Address (Street Address)', 'kdnaforms' ) ),
			__( 'Address 2', 'kdnaforms' )  => array( __( 'Address (Address Line 2)', 'kdnaforms' ) ),
			__( 'City', 'kdnaforms' )       => array( __( 'Address (City)', 'kdnaforms' ) ),
			__( 'State', 'kdnaforms' )      => array( __( 'Address (State / Province)', 'kdnaforms' ) ),
			__( 'Zip', 'kdnaforms' )        => array( __( 'Address (Zip / Postal Code)', 'kdnaforms' ) ),
			__( 'Country', 'kdnaforms' )    => array( __( 'Address (Country)', 'kdnaforms' ) ),
		);
	...
	public function get_value_choices( $required_types = array(), $excluded_types = array() ) {
		...
		$choices[] = array(
			'label' => esc_html__( 'Select a Value', 'kdnaforms' ),
			'value' => '',
		);
			...
			if ( is_array( $inputs ) ) {
				...
				switch ( $input_type ) {
					...
					case 'address':
						...
						$form_field_choices[] = array(
							'label' => strip_tags( KDNACommon::get_label( $field ) . ' (' . esc_html__( 'Full Address', 'kdnaforms' ) . ')' ),
							'value' => $field->id,
							'type' => 'address'
						);
					...
					case 'name':
						...
						$form_field_choices[] = array(
							'label' => strip_tags( KDNACommon::get_label( $field ) . ' (' . esc_html__( 'Full Name', 'kdnaforms' ) . ')' ),
							'value' => $field->id,
							'type' => 'name'
						);
				...
				$form_field_choices[] = array(
					'label' => strip_tags( KDNACommon::get_label( $field ) ),
					'value' => $field->id,
					'type'  => $field['type'],
				);
		...
		if ( count( $form_field_choices ) ) {
			...
			$choices['fields'] = array(
				'label'   => esc_html__( 'Form Fields', 'kdnaforms' ),
		...
		if ( empty( $required_types ) ) {
			...
			// Add default fields.
			$choices[] = array(
				'label'   => esc_html__( 'Entry Properties', 'kdnaforms' ),
				'choices' => array(
					array(
						'label' => esc_html__( 'Entry ID', 'kdnaforms' ),
						'value' => 'id',
					),
					array(
						'label' => esc_html__( 'Entry Date', 'kdnaforms' ),
						'value' => 'date_created',
					),
					array(
						'label' => esc_html__( 'User IP', 'kdnaforms' ),
						'value' => 'ip',
					),
					array(
						'label' => esc_html__( 'Source Url', 'kdnaforms' ),
						'value' => 'source_url',
					),
					array(
			...
			// Get Entry Meta.
			$entry_meta = KDNAFormsModel::get_entry_meta( $this->settings->get_current_form_id() );
			...
			if ( ! empty( $entry_meta ) ) {
				...
				$choices[] = array(
					'label'   => esc_html__( 'Entry Meta', 'kdnaforms' ),
		 ...
		 * @since 2.0.7.11
		 *
		 * @deprecated Deprecated since 2.5. Use kdnaform_field_map_choices instead.
		 *
		 ...
		 */
		$choices = apply_filters( 'kdnaform_addon_field_map_choices', $choices, $form_id, $required_types, $excluded_types );
		 ...
		 * @param null|array|string $exclude_field_types Null or the field type(s) to be excluded from the drop down.
		 */
		$choices = apply_filters( 'kdnaform_field_map_choices', $choices, $form_id, $required_types, $excluded_types );
...
Fields::register( 'generic_map', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Generic_Map' );
