...
class Textarea extends Base {
	...
	public function do_validation( $value ) {
		...
		$message = sprintf(
			...
			esc_html__( 'The text you have entered is not valid. For security reasons, some characters are not allowed. ', 'kdnaforms' ),
			$this->get_validation_correction_script(),
			htmlspecialchars( $sanitized_value, ENT_QUOTES ),
			esc_html__( 'Fix it', 'kdnaforms' )
		);
	...
	private function get_sanitized_value( $value ) {
		...
		$sanitized = ( $this->use_editor || $this->allow_html ) ? KDNACommon::maybe_wp_kses( $value ) : sanitize_textarea_field( $value );
...
Fields::register( 'textarea', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Textarea' );
