...
class Notification_Routing extends Base {
	...
	public function markup() {
		...
		if ( empty( $routing_fields ) ) {
			...
			$html = sprintf(
				'<div id="kdnaform_notification_to_routing_rules"><div class="gold_notice"><p>%s</p></div></div>',
				esc_html__( 'To use notification routing, your form must have a field supported by conditional logic.', 'kdnaforms' )
			);
		...
		$html .= '<div id="kdnaform_notification_to_routing_rules">';

		// Get routing operators.
		$operators = array(
			'is'          => esc_html__( 'is', 'kdnaforms' ),
			'isnot'       => esc_html__( 'is not', 'kdnaforms' ),
			'>'           => esc_html__( 'greater than', 'kdnaforms' ),
			'<'           => esc_html__( 'less than', 'kdnaforms' ),
			'contains'    => esc_html__( 'contains', 'kdnaforms' ),
			'starts_with' => esc_html__( 'starts with', 'kdnaforms' ),
			'ends_with'   => esc_html__( 'ends with', 'kdnaforms' ),
		);
		...
		foreach ( $routing as $i => $route ) {
			...
			if ( ! KDNANotification::is_valid_notification_email( rgar( $route, 'email' ) ) && $this->settings->is_dependency_met( rgobj( $this, 'dependency' ) ) ) {
				$valid_rule       = false;
			...
			// Prepare email input.
			$email_input = sprintf(
				'<input type="text" id="routing_email_%d" value="%s" class="kfield_routing_email" />',
				$i,
			...
			$field_input = sprintf(
				'<select id="routing_field_id_%1$d" class="kfield_routing_select" onchange="jQuery( \'#routing_value_%1$d\' ).replaceWith( GetRoutingValues( %1$d, jQuery( this ).val() ) ); SetRouting( %1$d );">%2$s</select>',
				$i,
			...
			$operator_input = sprintf( '<select id="routing_operator_%1$d" onchange="SetRouting(%1$d);" class="kdnaform_routing_operator">%2$s</select>', $i, $operator_options );

			// Prepare add button.
			$add_button = sprintf(
				'<button class="kform-settings-field__notification-routing-button kform-settings-field__notification-routing-button--add kform-st-icon kform-st-icon--circle-plus" onclick="SetRouting(%2$d); InsertRouting(%3$d);">
			...
			if ( count( $routing ) > 1 ) {
				...
				$delete_button = sprintf(
					'<button class="kform-settings-field__notification-routing-button kform-settings-field__notification-routing-button--delete kform-st-icon kform-st-icon--circle-minus" onclick="DeleteRouting(%2$d);">
						<span class="screen-reader-text">%1$s</span>
					</button>',
					esc_attr__( 'Remove This Rule', 'kdnaforms' ),
					$i
			...
			$html .= sprintf(
				...
				$valid_rule ? '' : ' class="kform-settings-field__notification-routing-route--invalid"',
		...
		if ( $has_invalid_rule ) {
			$html .= sprintf(
				'<span class="kform-settings-validation__error">%s</span>',
		...
		<script type="text/javascript">
			...
			jQuery( document ).ready( function () {
				jQuery( document ).on( 'input propertychange', '.kfield_routing_email', function () {
					SetRoutingEmail( jQuery( this ) );
				} );
				jQuery( document ).on( 'change', '.kfield_routing_value_dropdown', function () {
			...
			function CreateRouting( routings ) {
				...
				for ( var i = 0; i < routings.length; i ++ ) {
					...
					str += "<div>" + <?php echo json_encode( esc_html__( 'Send to', 'kdnaforms' ) ); ?> + " <input type='text' id='routing_email_" + i + "' value='" + email + "' class='kfield_routing_email' />";
					...
					str += "<option value='isnot' " + isNotSelected + ">" + <?php echo json_encode( esc_html__( 'is not', 'kdnaforms' ) ); ?> + "</option>";
					str += "<option value='>' " + greaterThanSelected + ">" + <?php echo json_encode( esc_html__( 'greater than', 'kdnaforms' ) ); ?> + "</option>";
					str += "<option value='<' " + lessThanSelected + ">" + <?php echo json_encode( esc_html__( 'less than', 'kdnaforms' ) ); ?> + "</option>";
					str += "<option value='contains' " + containsSelected + ">" + <?php echo json_encode( esc_html__( 'contains', 'kdnaforms' ) ); ?> + "</option>";
					str += "<option value='starts_with' " + startsWithSelected + ">" + <?php echo json_encode( esc_html__( 'starts with', 'kdnaforms' ) ); ?> + "</option>";
					str += "<option value='ends_with' " + endsWithSelected + ">" + <?php echo json_encode( esc_html__( 'ends with', 'kdnaforms' ) ); ?> + "</option>";
					str += "</select>&nbsp;";
					str += GetRoutingValues( i, routings[ i ].fieldId, routings[ i ].value ) + "&nbsp;";

					str += "<button class='kform-settings-field__notification-routing-button kform-settings-field__notification-routing-button--add kform-st-icon kform-st-icon--circle-plus' onclick='InsertRouting(" + ( i + 1 ) + ");'>";
					str += "<span class='screen-reader-text'><?php esc_attr_e( 'Add Another Rule', 'kdnaforms' ); ?></span>";
					str += "</button>";

					if ( routings.length > 1 ) {
						str += "<button class='kform-settings-field__notification-routing-button kform-settings-field__notification-routing-button--delete kform-st-icon kform-st-icon--circle-minus' onclick='DeleteRouting(" + ( i ) + ");'>";
				...
				jQuery( "#kdnaform_notification_to_routing_rules" ).html( str );
			...
			function GetRoutingFields( index, selectedItem ) {
				var str = "<select id='routing_field_id_" + index + "' class='kfield_routing_select' onchange='jQuery(\"#routing_value_" + index + "\").replaceWith(GetRoutingValues(" + index + ", jQuery(this).val())); SetRouting(" + index + "); '>";
			...
			function IsNotificationConditionalLogicField( field ) {
				...
				var supported_fields = <?php echo json_encode( KDNANotification::get_routing_field_types() ); ?>;
			...
			function GetFieldValues(index, fieldId, selectedValue, labelMaxCharacters) {
				...
				if (field["type"] == 'post_category' && field["displayAllCategories"]) {
					...
					var dropdown = jQuery('#' + dropdown_id + ".kfield_category_dropdown");
					...
					if (dropdown.length > 0) {
						...
						str = "<select id='" + dropdown_id + "' class='kfield_routing_select kfield_category_dropdown kfield_routing_value_dropdown'>" + options + "</select>";
					}
					else {
						...
						jQuery.post(ajaxurl, {   action: "kdna_get_notification_post_categories",
							...
							function (dropdown_string) {
								if (dropdown_string) {
									jQuery('#kfield_ajax_placeholder_' + index).replaceWith(dropdown_string.trim());
								}
							}
						);

						// Will be replaced by real drop down during the ajax callback.
						str = "<select id='kfield_ajax_placeholder_" + index + "' class='kfield_routing_select'><option>" + <?php json_encode( esc_html__( 'Loading...', 'kdnaforms' ) ); ?> + "</option></select>";
					}
				}
				else if (field.choices) {
					// Create a drop down for fields that have choices (i.e. drop down, radio, checkboxes, etc...).
					str = "<select class='kfield_routing_select kfield_routing_value_dropdown' id='routing_value_" + index + "'>";
					...
					str = "<input type='text' placeholder='" + <?php echo json_encode( esc_html__( 'Enter value', 'kdnaforms' ) ); ?> +"' class='kfield_routing_select' id='routing_value_" + index + "' value='" + selectedValue.replace(/'/g, "&#039;") + "' onchange='SetRouting(" + index + ");' onkeyup='SetRouting(" + index + ");'>";
				}
				return str;
			}
	...
	public function do_validation( $value ) {
		...
		foreach ( $value as $route ) {
			if ( ! KDNANotification::is_valid_notification_email( rgar( $route, 'email' ) ) ) {
				$this->set_error( rgobj( $this, 'error_message' ) );
			}
		}
	...
	private static function get_routing_fields( $form, $selected_field_id ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( in_array( $field->get_input_type(), KDNANotification::get_routing_field_types() ) ) {
	...
	private static function get_field_values( $i, $form, $field_id, $selected_value ) {
		...
		if ( $field->type == 'post_category' && $field->displayAllCategories == true ) {
			...
			return wp_dropdown_categories(
				array(
					'class'        => 'kfield_routing_select kfield_category_dropdown kfield_routing_value_dropdown',
					'orderby'      => 'name',
					'id'           => 'routing_value_' . $i,
			...
			$html = sprintf( '<select id="routing_value_%d" class="kfield_routing_select kfield_routing_value_dropdown">', $i );
			...
			return sprintf(
				'<input type="text" placeholder="%1$s" class="kfield_routing_select" id="routing_value_%3$d" value="%2$s" onchange="SetRouting( %3$d );" onkeyup="SetRouting( %3$d );" />',
...
Fields::register( 'notification_routing', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Notification_Routing' );
