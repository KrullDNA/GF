...
class Date_Time extends Base {
	...
	public function markup() {
		...
		$html .= sprintf(
			'%s %s<span class="kform-settings-input__separator">:</span>%s %s',
		...
		$html .= sprintf(
			...
			KDNACommon::get_image_url( 'datepicker/datepicker.svg' ),
			esc_html__( 'Open Date Picker', 'kdnaforms' )
		);
	...
	public function do_validation( $value ) {
		...
		if ( wp_strip_all_tags( $value['date'] ) !== $value['date'] ) {
			$this->inputs['date']->set_error( esc_html__( 'Date must not include HTML tags.', 'kdnaforms' ) );
			return;
		}

		// Test for valid hour.
		if ( (int) $value['hour'] < 1 || (int) $value['hour'] > 12 ) {
			$this->inputs['hour']->set_error( esc_html__( 'You must select a valid hour.', 'kdnaforms' ) );
			return;
		}

		// Test for valid minute.
		if ( (int) $value['minute'] < 0 || (int) $value['minute'] > 59 ) {
			$this->inputs['minute']->set_error( esc_html__( 'You must select a valid minute.', 'kdnaforms' ) );
			return;
		}

		// Test for valid AM/PM.
		if ( ! in_array( rgar( $value, 'ampm' ), array( 'am', 'pm' ) ) ) {
			$this->inputs['ampm']->set_error( esc_html__( 'You must select either am or pm.', 'kdnaforms' ) );
			return;
		}
...
Fields::register( 'date_time', '\KDNA_Forms\KDNA_Forms\Settings\Fields\Date_Time' );
