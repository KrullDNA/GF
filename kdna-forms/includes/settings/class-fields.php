...
use KDNA_Forms\KDNA_Forms\Settings\Fields\Base;
use WP_Error;
...
class Fields {
	...
	public static function create( $props, $settings ) {
		...
		if ( is_a( $props, 'KDNA_Forms\KDNA_Forms\Settings\Fields\Base' ) && ! self::exists( $props['type'] ) ) {
			return $props;
		}

		// Ensure Settings instance was provided.
		if ( ! is_a( $settings, 'KDNA_Forms\KDNA_Forms\Settings\Settings' ) && ! is_subclass_of( $settings, 'KDNA_Forms\KDNA_Forms\Settings\Settings' ) ) {
