...
class Settings {
	 ...
	 */
	protected $input_name_prefix = '_kdnaform_setting';
	...
	public function __construct( $args = array() ) {
		...
		add_filter( 'kdnaform_noconflict_scripts', array( $this, 'filter_kdnaform_noconflict_scripts' ) );
		add_filter( 'kdnaform_noconflict_styles', array( $this, 'filter_kdnaform_noconflict_styles' ) );
	...
	public function scripts() {
		...
		$scripts = array(
			array(
				'handle'  => 'kdnaform_settings_dependencies',
				'src'     => KDNACommon::get_base_url() . "/includes/settings/js/dependencies{$min}.js",
			...
			array(
				'handle'  => 'kdnaform_settings_select_custom',
				'src'     => KDNACommon::get_base_url() . "/includes/settings/js/select-custom{$min}.js",
				'version' => KDNAForms::$version,
				'enqueue' => array(
					function() {
			...
			array(
				'handle'  => 'kdnaform_settings_tabs',
				'src'     => KDNACommon::get_base_url() . "/includes/settings/js/tabs{$min}.js",
				'version' => KDNAForms::$version,
				'enqueue' => array(
					function() {
	 ...
	 * @return array
	 */
	public function filter_kdnaform_noconflict_scripts( $scripts ) {
	 ...
	 * @return array
	 */
	public function filter_kdnaform_noconflict_styles( $styles ) {
	 ...
	 * @since 2.5
	 *
	 * @param \KDNAAddOn $addon The Add-On responsible for rendering the settings page.
	 */
	public function render() {
		...
		if ( rgget( 'subview' ) === 'confirmation' && rgget( 'duplicatedcid' ) ) {
			KDNA_Confirmation::output_duplicate_confirmation_notice();
		}
		...
		<form id="kdnaform-settings" class="kdnaform_settings_form" data-js="page-loader" action="" method="post" enctype="multipart/form-data" novalidate>
			<?php
				...
				if ( ! empty( $save ) ) {
					printf( '<div class="kdnaform-settings-save-container">%s</div>', $save ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
				...
				wp_nonce_field( 'kdnaform_settings_save', 'kdnaform_settings_save_nonce' );
		...
		if ( ! empty( $live_dependencies ) ) {
			...
			foreach ( $live_dependencies as $dependency ) {
				echo 'new KDNA_Settings_Dependencies( ' . wp_json_encode( $dependency ) . ' );';
			}
			echo '</script>';
		}
	...
	public function render_tab_navigation() {
		...
		echo '<nav class="kdnaform-settings-tabs__navigation" role="tablist">';
		foreach ( $tabs as $i => $tab ) {
			printf(
				'<a href="#" role="tab" aria-selected="%3$s" id="kdnaform-settings-tab-%2$s" data-tab="%2$s" class="%4$s"%5$s>%1$s</a>',
				esc_html( $tab['label'] ),
				esc_attr( $tab['name'] ),
		...
		printf(
			'<input type="hidden" name="kdnaform_settings_tab" value="%s" />',
			esc_attr( $active_tab )
		);
	...
	public function render_tab( $tab ) {
		...
		// Open tab container.
		printf(
			'<div class="kdnaform-settings-tabs__container%2$s" role="tabpanel" aria-hidden="%3$s" data-tab="%1$s" aria-labelledby="kdnaform-settings-tab-%1$s">',
			esc_attr( $tab['id'] ),
	...
	public function render_section( $section ) {
		...
		$class = array( 'kdnaform-settings-panel' );
		...
		if ( rgar( $section, 'title' ) || ( rgar( $section, 'collapsible' ) && rgar( $section, 'id' ) ) ) {
			$class[] = 'kdnaform-settings-panel--with-title';
		}

		// Add collapsible classes.
		if ( rgar( $section, 'collapsible' ) && rgar( $section, 'id' ) ) {

			$class[] = 'kdnaform-settings-panel--collapsible';

			// Add collapsed class.
			if ( self::is_section_collapsed( $section ) ) {
				$class[] = 'kdnaform-settings-panel--collapsed';
			}
		...
		if ( self::has_card_layout( $section ) ) {
			$class[] = 'kdnaform-settings-panel--card';
		}
		...
		if ( rgar( $section, 'title' ) || ( rgar( $section, 'collapsible' ) && rgar( $section, 'id' ) ) ) {
			...
			if ( rgar( $section, 'title' ) ) {
				printf(
					'<legend class="kdnaform-settings-panel__title kdnaform-settings-panel__title--header">%s %s</legend>',
			...
			if ( rgar( $section, 'collapsible' ) && rgar( $section, 'id' ) ) {
				...
				<span class="kdnaform-settings-panel__collapsible-control">
					...
					/>
					<label class="kdnaform-settings-panel__collapsible-toggle" for="kdnaform_settings_section_collapsed_uninstall"><span class="screen-reader-text"><?php printf( esc_html__( 'Toggle %s Section', 'kdnaforms' ), esc_html( $section['title' ]) ); ?></span></label>
				</span>
				<?php
			}
		}
		...
		// Open settings table.
		echo '<div class="kdnaform-settings-panel__content">';

		// Display section description.
		if ( rgar( $section, 'description' ) ) {
			printf( '<div class="kdnaform-settings-description kdnaform-kitchen-sink">%s</div>', $section['description'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
	...
	public function render_field( $field ) {
		...
		printf(
			'<div id="kdnaform_setting_%s" class="kdnaform-settings-field kdnaform-settings-field__%s" %s>',
			esc_attr( $field_name ),
			esc_attr( $field->type ),
		...
		if ( rgobj( $field, 'label' ) ) {
			printf(
				'<div class="kdnaform-settings-field__header"><label class="kdnaform-settings-label" for="%s">%s%s</label>%s</div>',
				esc_attr( $field->name ),
	...
	private static function is_section_collapsed( $props = array() ) {
		...
		return empty( $_POST ) ? rgar( $props, 'is_collapsed' ) : (bool) rgpost( 'kdnaform_settings_section_collapsed_' . $props['id'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	public function render_save_button( $html = '' ) {
		...
		$html .= sprintf(
			'<button type="submit" id="kdnaform-settings-save" name="kdnaform-settings-save" value="save" form="kdnaform-settings" class="%2$s"%3$s>%1$s</button>',
		 ...
		 * @param \KDNA_Forms\KDNA_Forms\Settings\Settings $this Current instance of the Settings Framework.
		 */
		$html = apply_filters( 'kdnaform_settings_save_button', $html, $this );
	...
	private function add_save_button( $props = array() ) {
		...
		$this->save_button = wp_parse_args(
			...
			array(
				'name'     => 'save',
				'value'    => esc_html__( 'Save Settings', 'kdnaforms' ) . ' &nbsp;&rarr;',
				'class'    => 'primary button large',
				'messages' => array(
					'save'  => esc_html__( 'Settings updated.', 'kdnaforms' ),
					'error' => esc_html__( 'There was an error while saving your settings.', 'kdnaforms' ),
				),
			)
		);
	...
	private function get_section_id( $section ) {
		$section_prefix = 'kdnaform-settings-section-';
		if ( rgar( $section, 'id' ) ) {
	...
	/**
	 * Determines if the conditions to display a field or section have been met.
	 *    Handles legacy dependencies for pre-KDNA Forms 2.5.
	 *
	 * @since 2.5
	 *
	 * @param string|array|callable|null $dependency Condition to be met.
	...
	public function process_postback() {
		...
		// Verify nonce.
		check_admin_referer( 'kdnaform_settings_save', 'kdnaform_settings_save_nonce' );

		// If user does not have access, exit.
		if ( ! $this->current_user_has_access() ) {
			esc_html_e( 'Access denied.', 'kdnaforms' );
			return;
		}
	...
	public function current_user_has_access() {
		return ! $this->capability || KDNACommon::current_user_can_any( $this->capability );
	}
	...
	public function get_active_tab() {
		...
		if ( empty( $errors ) ) {
			return rgpost( 'kdnaform_settings_tab' ) ? sanitize_text_field( $_POST['kdnaform_settings_tab'] ) : rgars( $tabs, '0/id' ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated, WordPress.Security.ValidatedSanitizedInput.MissingUnslash, WordPress.Security.NonceVerification.Missing
		}
	 ...
	 */
	public function set_input_name_prefix( $input_name_prefix = '_kdnaform_setting' ) {
