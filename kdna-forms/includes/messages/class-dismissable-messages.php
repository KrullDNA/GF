...
namespace KDNA_Forms\KDNA_Forms\Messages;
...
class Dismissable_Messages {
	...
	public function add( $text, $key, $type = 'warning', $capabilities = false, $sticky = false, $page = null ) {
		...
		if ( $sticky ) {
			$sticky_messages = get_option( 'kdnaform_sticky_admin_messages', array() );
			if ( ! is_array( $sticky_messages ) ) {
				$sticky_messages = array();
			}

			$sticky_messages[ $key ] = $message;
			update_option( 'kdnaform_sticky_admin_messages', $sticky_messages );
		} else {
	...
	private function get_sticky_messages() {
		$messages = get_option( 'kdnaform_sticky_admin_messages', array() );
	...
	public function display( $messages = false, $page = null ) {
		...
		if ( ! empty( $messages ) ) {
			...
			foreach ( $messages as $message ) {
				...
				if ( isset( $message['capabilities'] ) && $message['capabilities'] && ! \KDNACommon::current_user_can_any( $message['capabilities'] ) ) {
					continue;
				}
