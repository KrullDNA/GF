 ...
 *
 * @since 1.0
 * @package KDNA_Forms\KDNA_Forms_RECAPTCHA
 */
...
class RECAPTCHA_API {
	...
	/**
	 * The KDNA_RECAPTCHA instance.
	 *
	 * @since 1.7.0
	 *
	 * @var KDNA_RECAPTCHA|null The KDNA_RECAPTCHA instance.
	 */
	protected $addon;
	 ...
	 * @param KDNA_RECAPTCHA  $addon     The KDNA_RECAPTCHA instance.
	 */
	...
	/**
	 * Get KDNA API URL for path.
	 *
	 * @since 1.7.0
	 *
	 * @param string $path Endpoint path.
	 *
	 * @return string URL for KDNA API endpoint.
	 */
	public static function get_kdna_api_url( $path = '' ) {

		if ( '/' !== substr( $path, 0, 1 ) ) {
			$path = '/' . $path;
		}

		$base_url = 'https://kdnaforms.com/wp-json/kdnaforms/v1';

		return defined( 'KDNA_API_URL' ) ? KDNA_API_URL . '/auth/googlerecaptcha' . $path : $base_url . '/auth/googlerecaptcha' . $path;
	}
	...
	private function make_request( $path = '', $body = array(), $headers = array(), $method = 'GET', $base_path = null ) {
		...
		kdna_recaptcha()->log_debug( __METHOD__ . '(): Making request to: ' . $path );
		...
		if ( 200 !== $retrieved_response_code ) {
			...
			kdna_recaptcha()->log_error( __METHOD__ . '(): Unable to validate with the Google Cloud API: ' . $error_message );
	...
	public function refresh_token( $refresh_token ) {
		// Connect to KDNA Forms API.
		$response = wp_remote_post(
			$this->get_kdna_api_url( 'refresh' ),
			array(
				'body' => array(
		...
		$response_body = kdna_recaptcha()->maybe_decode_json( wp_remote_retrieve_body( $response ) );
	...
	public function get_enterprise_site_keys( $project ) {
		...
		$page_token = null;
		$page_size  = apply_filters( 'kdnaform_recaptcha_enterprise_keys_page_size', 100 );
