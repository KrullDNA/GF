...
abstract class KDNA_Config {
	 ...
	 * Constructor
	 *
	 * @param KDNA_Config_Data_Parser $parser
	 */
	public function __construct( KDNA_Config_Data_Parser $parser ) {
		$this->parser = $parser;
	}
	 ...
	 *    return str_starts_with( $config_path, 'kdnaform_theme_config/common/form/product_meta' );
	 * }
	 *
	 * @since 2.9.0
	 *
	...
	public function get_data() {
		 ...
		 * @return array
		 */
		$data = apply_filters( 'kdnaform_config_data_' . $this->name(), $this->data(), $this->script_to_localize() );
	...
	public static function validate_ajax() {
		...
		// Check nonce
		$nonce_result = check_ajax_referer( 'kdnaform_config_ajax', 'kdnaform_ajax_nonce', false );

		if ( ! $nonce_result ) {
			wp_send_json_error( esc_html__( 'Unable to verify nonce. Please refresh the page and try again.', 'kdnaforms' ) );
		}

		$config = json_decode( rgpost( 'config' ), true );
		$hash   = rgar( $config, 'hash' );
		if ( ! $hash ) {
			wp_send_json_error( esc_html__( 'Invalid config.', 'kdnaforms' ) );
		}
		...
		if ( $hash !== self::hash( $config ) ) {
			wp_send_json_error( esc_html__( 'Config validation failed. Hash does not match', 'kdnaforms' ) );
		}
...
add_action('wp_ajax_kdnaform_validate_config', array( 'KDNA_Forms\KDNA_Forms\Config\KDNA_Config', 'validate_ajax' ) );
add_action('wp_ajax_nopriv_kdnaform_validate_config', array( 'KDNA_Forms\KDNA_Forms\Config\KDNA_Config', 'validate_ajax' ) );
