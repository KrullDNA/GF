...
/**
 * Collection to hold KDNA_Config items and provide their structured data when needed.
 *
 * @since 2.6
 *
 * @package KDNA_Forms\KDNA_Forms\Config
 */
class KDNA_Config_Collection {

	/**
	 * @var KDNA_Config[] $configs
	 */
	 ...
	 *
	 * @param KDNA_Config $config
	 */
	public function add_config( KDNA_Config $config ) {
		$this->configs[] = $config;
	}
	...
	public function handle_ajax() {
		...
		if ( ! $nonce_result ) {
			KDNACommon::send_json_error( esc_html__( 'Unable to verify nonce. Please refresh the page and try again.', 'kdnaforms' ) );
		}
		...
		if ( ! $configs ) {
			KDNACommon::send_json_error( sprintf( esc_html__( 'Unable to find config: %s', 'kdnaforms' ), esc_html( $config_path ) ) );
		}

		$data = $this->get_merged_data_for_object( $configs, $args );
		KDNACommon::send_json_success( $data );
	}
	/**
	...
	private function localize_data_for_script( $script, $items, $localize = true, $args = null ) {
		...
		foreach ( $items as $name => $configs ) {
			 ...
			 * @return array
			 */
			$localized_data = apply_filters( 'kdnaform_localized_script_data_' . $name, $localized_data, $script, $configs );
	 ...
	 * @return int
	 */
	public function sort_by_priority( KDNA_Config $a, KDNA_Config $b ) {
