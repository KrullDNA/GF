...
class KDNA_Asset_Processor {
	...
	private function process_versions() {
		...
		foreach( $registered as &$asset ) {
			...
			if ( $asset->src && strpos( $asset->src, 'kdnaforms/assets/js/dist' ) === false  || is_null( $asset->src ) ) {
				continue;
			}
			...
			$hash = defined( 'KDNA_DEV_TIME_AS_VER' ) && KDNA_DEV_TIME_AS_VER ?
					filemtime( $path ) :
