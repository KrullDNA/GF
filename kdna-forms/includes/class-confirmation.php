 ...
 * @since 2.5
 *
 * Class KDNA_Confirmations
 */
class KDNA_Confirmation {
	 ...
	 * @access public
	 *
	 * @used-by KDNAFormSettings::confirmations_page()
	 * @uses    KDNAFormSettings::maybe_process_confirmation_list_action()
	 * @uses    KDNAFormSettings::page_header()
	 * @uses    KDNAFormsModel::get_form_meta()
	 * @uses    KDNAConfirmationTable::prepare_items()
	 * @uses    KDNAConfirmationTable::display()
	...
	private static function confirmations_list_page( $form_id ) {
		...
		KDNAFormSettings::maybe_process_confirmation_list_action();

		// Get form object.
		$form = KDNAFormsModel::get_form_meta( $form_id );

		// Prepare list table.
		$confirmation_table = new KDNAConfirmationTable( $form );
		...
		<div class="kdnaform-settings-panel">
			<header class="kdnaform-settings-panel__header">
				<h4 class="kdnaform-settings-panel__title"><?php esc_html_e( 'Confirmations', 'kdnaforms' ); ?></h4>
			</header>
			...
			<div class="kdnaform-settings-panel__content">
				...
				<form id="confirmation_list_form" method="post">
					...
					wp_nonce_field( 'kdnaform_confirmation_list_action', 'kdnaform_confirmation_list_action' );
					?>
	...
	private static function settings_fields( $confirmation, $form ) {
		...
		$fields = array(
			array(
				'title'  => esc_html__( 'Confirmations', 'kdnaforms' ),
				'fields' => array(
					array(
						'name'     => 'name',
						'label'    => esc_html__( 'Confirmation Name', 'kdnaforms' ),
						'type'     => 'text',
						'required' => true,
					...
					array(
						'name'          => 'type',
						'label'         => esc_html__( 'Confirmation Type', 'kdnaforms' ),
						'type'          => 'radio',
						'horizontal'    => true,
						...
						'choices'       => array(
							...
							array(
								'label' => esc_html__( 'Page', 'kdnaforms' ),
								'value' => 'page',
							),
							array(
								'label' => esc_html__( 'Redirect', 'kdnaforms' ),
								'value' => 'redirect',
							),
						),
					),
					array(
						...
						'tooltip'    => kdnaform_tooltip( 'form_confirmation_message', null, true ),
						'use_editor' => true,
						'dependency' => array(
							'live'     => true,
							'operator' => 'ALL',
					...
					array(
						...
						'choices'    => array(
							array(
								...
								'tooltip' => kdnaform_tooltip( 'form_confirmation_autoformat', null, true ),
							),
						),
						'dependency' => array(
							'live'     => true,
					...
					array(
						'name'                => 'url',
						'label'               => esc_html__( 'Redirect URL', 'kdnaforms' ),
						'type'                => 'text',
						'required'            => true,
						'dependency'          => array(
							'live'     => true,
						...
						'validation_callback' => function( $field, $value ) {
							...
							if ( ( empty( $value ) || ! KDNACommon::is_valid_url( $value ) ) && ! KDNACommon::has_merge_tag( $value ) ) {
								$field->set_error( __( 'You must specify a valid Redirect URL.', 'kdnaforms' ) );
							}
					...
					array(
						'name'        => 'queryString',
						'type'        => 'text',
						'label'       => esc_html__( 'Pass Field Data via Query String', 'kdnaforms' ),
						'class'       => 'merge-tag-support mt-position-right mt-hide_all_fields mt-option-url',
						'tooltip'     => kdnaform_tooltip( 'form_redirect_querystring', null, true ),
						'dependency'  => array(
							'live'     => true,
							'operator' => 'ALL',
							'fields'   => array(
					...
					array(
						'type'  => 'save',
						'value' => esc_html__( 'Save Confirmation', 'kdnaforms' ),
					),
				),
			),
		);
		 ...
		 * @param array $form         The Form Object.
		 */
		$legacy_settings = apply_filters( 'kdnaform_confirmation_ui_settings', array(), $confirmation, $form );
	...
	public static function initialize_settings_renderer() {
		...
		if ( ! empty( $confirmation['message'] ) && self::confirmation_looks_unsafe( $confirmation['message'] ) ) {
			$dismissible_message = esc_html__( 'Your confirmation message appears to contain a merge tag as the value for an HTML attribute. Depending on the attribute and field type, this might be a security risk. %sFurther details%s', 'kdnaforms' );
		...
		$renderer = new Settings(
			array(
				...
				'header'         => array(
					'icon'  => 'fa fa-cogs',
					'title' => esc_html__( 'Confirmation Settings', 'kdnaforms' ),
				),
				...
				'save_callback'  => function( $values ) use ( &$confirmation, &$form, &$confirmation_id ) {
					...
					$confirmation = KDNAFormSettings::save_changed_form_settings_fields( $confirmation, $values );

					// Apply values.
					$confirmation['name']              = rgar( $values, 'name' );
					$confirmation['event']             = rgar( $values, 'event' );
					$confirmation['type']              = KDNACommon::whitelist( rgar( $values, 'type' ), array(
						'message',
						'page',
						'redirect',
					) );
					...
					$confirmation = KDNAFormsModel::trim_conditional_logic_values_from_element( $confirmation, $form );

					// Save confirmation.
					$form['confirmations'][ $confirmation['id'] ] = $confirmation;
					$result                                       = KDNAFormsModel::save_form_confirmations( $form['id'], $form['confirmations'] );
				...
				'before_fields'  => function() use ( &$confirmation, $confirmation_id, $form ) {
					 ...
					 */
					$entry_meta = apply_filters( 'kdnaform_entry_meta_conditional_logic_confirmations', $entry_meta, $form, $confirmation_id );
						...
						function MaybeAddSaveMergeTags( mergeTags, elementId, hideAllFields, excludeFieldTypes, isPrepop, option ) {
							...
							if ( event === 'form_saved' || event === 'form_save_email_sent' ) {
								mergeTags[ 'other' ].tags.push( {
									tag: '{save_link}',
									label: <?php echo json_encode( __( 'Save &amp; Continue Link', 'kdnaforms' ) ) ?> } );
								mergeTags[ 'other' ].tags.push( {
									tag: '{save_token}',
									label: <?php echo json_encode( __( 'Save &amp; Continue Token', 'kdnaforms' ) ) ?> } );
							}

							if ( event === 'form_saved' ) {
								mergeTags[ 'other' ].tags.push( {
									tag: '{save_email_input}',
									label: <?php echo json_encode( __( 'Save &amp; Continue Email Input', 'kdnaforms' ) ) ?> } );
							}
	...
	/**
	 * Alias for KDNACommon::maybe_wp_kses().
	 *
	 * @since  2.5
	 *
	...
	public static function output_duplicate_confirmation_notice() {
		echo '<div class="kdnaform-alert kdnaform-alert--notice" data-js="kdnaform-alert">
				<span class="kdnaform-alert__icon kdnaform-icon kdnaform-icon--circle-notice" aria-hidden="true"></span>
  				<div class="kdnaform-alert__message-wrap">
    				<p class="kdnaform-alert__message">In order to avoid conflicts with other confirmations on this form, please ensure these conditional logic rules are unique.</p>
  				</div>
			</div>';
	}
 ...
 */
class KDNAConfirmationTable extends WP_List_Table {
	...
	/**
	 * KDNAConfirmationTable constructor.
	 *
	 * @since  Unknown
	 *
	...
	public function __construct( $form ) {
		...
		$this->_column_headers = array(
			array(
				'cb'      => '',
				'name'    => __( 'Name', 'kdnaforms' ),
				'type'    => __( 'Type', 'kdnaforms' ),
				'content' => __( 'Content', 'kdnaforms' ),
			),
			array(),
	...
	public function column_name( $item ) {
		...
		$actions       = apply_filters(
			'kdnaform_confirmation_actions', array(
				'edit'      => '<a href="' . esc_url( $edit_url ) . '">' . __( 'Edit', 'kdnaforms' ) . '</a>',
				'duplicate' => '<a href="' . esc_url( $duplicate_url ) . '">' . __( 'Duplicate', 'kdnaforms' ) . '</a>',
				'delete'    => '<a class="submitdelete" onclick="javascript: if(confirm(\'' . __( 'WARNING: You are about to delete this confirmation.', 'kdnaforms' ) . __( "\'Cancel\' to stop, \'OK\' to delete.", 'kdnaforms' ) . '\')){ DeleteConfirmation(\'' . esc_js( $item['id'] ) . '\'); }" onkeypress="javascript: if(confirm(\'' . __( 'WARNING: You are about to delete this confirmation.', 'kdnaforms' ) . __( "\'Cancel\' to stop, \'OK\' to delete.", 'kdnaforms' ) . '\')){ DeleteConfirmation(\'' . esc_js( $item['id'] ) . '\'); }" style="cursor:pointer;">' . __( 'Delete', 'kdnaforms' ) . '</a>',
			)
		);
	...
	public static function get_column_content( $item ) {
		...
		switch ( rgar( $item, 'type' ) ) {
			...
			case 'page':
				...
				if ( empty( $page ) ) {
					return __( '<em>This page does not exist.</em>', 'kdnaforms' );
				}
	...
	public static function get_column_type( $item ) {
		...
		switch ( rgar( $item, 'type' ) ) {
			case 'message':
				return __( 'Text', 'kdnaforms' );

			case 'page':
				return __( 'Page', 'kdnaforms' );

			case 'redirect':
				return __( 'Redirect', 'kdnaforms' );
		}
