...
abstract class KDNA_Installation_Wizard_Step extends stdClass {
	...
	function get_next_button_text(){
		return __( 'Next', 'kdnaforms' );
	}

	function get_previous_button_text(){
		return __( 'Back', 'kdnaforms' );
	}
	...
	function update( $posted_values = array() ){
		...
		update_option( 'kdnaform_installation_wizard_' . $this->get_name(), $new_values );
	...
	function flush_values(){
		delete_option( 'kdnaform_installation_wizard_' . $this->get_name() );
	}
