...
class KDNA_Installation_Wizard {
	private $_step_class_names = array();

	function __construct(){
		$path = KDNACommon::get_base_path() . '/includes/wizard/steps/';
		require_once( $path . 'class-kdna-installation-wizard-step.php' );
		$classes = array();
		$files   = KDNACommon::glob_require_once( 'class-kdna-installation-wizard-step-*.php', $path );

		foreach ( $files as $filename ) {
			$regex = '/class-kdna-installation-wizard-step-(.*?).php/';
			preg_match( $regex, $filename, $matches );
			$class_name = 'KDNA_Installation_Wizard_Step_' . str_replace( '-', '_', $matches[1] );
			$step = new $class_name;
	...
	public function display(){
		...
		$nonce_key = '_kdnaform_installation_wizard_step_' . $current_step->get_name();
		...
		<h1><?php esc_html_e( 'Welcome to KDNA Forms', 'kdnaforms' ) ?></h1>

		<div id="kdnaform_installation_progress">
	 ...
	 * @param bool $name
	 *
	 * @return KDNA_Installation_Wizard_Step
	 */
	public function get_step( $name = false ){
		...
		$current_step_values = get_option( 'kdnaform_installation_wizard_' . $name );
	 ...
	 * @param $current_step
	 *
	 * @return bool|KDNA_Installation_Wizard_Step
	 */
	...
	/**
	 * @param KDNA_Installation_Wizard_Step $current_step
	 *
	 * @return bool|KDNA_Installation_Wizard_Step
	 */
	...
	public function complete_installation() {
		...
		update_option( 'kdnaform_pending_installation', false );
	}
	...
	public function progress( $current_step, $echo = true ){
		$html = '<ul id="kdnaform_installation_progress">';
		$done = true;
		$current_step_name = $current_step->get_name();
		foreach ( array_keys( $this->_step_class_names ) as $step_name ) {
			$class = '';
			...
			if ( $current_step_name == $step_name ) {
				$class .= 'kdnaform_installation_progress_current_step ';
				$done = $step->is('complete') ? true : false;
			} else {
				$class .= $done  ?  'kdnaform_installation_progress_step_complete' : 'kdnaform_installation_progress_step_pending';
			}
			$check = $done ? '<i class="fa fa-check" style="color:green"></i>'  : '<i class="fa fa-check" style="visibility:hidden"></i>';

			$html .= sprintf( '<li id="kdnaform_installation_progress_%s" class="%s">%s&nbsp;%s</li>', esc_attr( $step->get_name() ), esc_attr( $class ), esc_html( $step->get_title() ), $check );
		}
		$html .= '</ul>';
