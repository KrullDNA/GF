...
/**
 * API for standard KDNA Forms functionality.
 *
 * Supports:
 * - Forms
 * - Entries
...
class KDNAAPI {
	...
	public static function get_form( $form_id ) {
		...
		$form_info = KDNAFormsModel::get_form( $form_id, true );
		if ( ! $form_info ) {
			return false;
		}
	 ...
	 * @access public
	 *
	 * @uses KDNAFormsModel::get_form_ids()
	...
	public static function get_forms( $active = true, $trash = false, $sort_column = 'id', $sort_dir = 'ASC' ) {
		...
		$form_ids = KDNAFormsModel::get_form_ids( $active, $trash, $sort_column, $sort_dir );
		if ( empty( $form_ids ) ) {
			return array();
		}
	 ...
	 * @access public
	 *
	 * @uses KDNAFormsModel::delete_forms()
	 *
	...
	public static function delete_forms( $form_ids ) {
		...
		KDNAFormsModel::delete_forms( $form_ids );
	}
	...
	public static function delete_form( $form_id ) {
		...
		if ( empty( $form ) ) {
			return new WP_Error( 'not_found', sprintf( __( 'Form with id: %s not found', 'kdnaforms' ), $form_id ), $form_id );
		}
	 ...
	 * @access public
	 *
	 * @uses KDNAFormsModel::duplicate_form()
	 *
	 * @param int $form_id The ID of the Form to delete.
	 *
	 * @return mixed True for success, or a WP_Error instance
	...
	public static function duplicate_form( $form_id ) {
		...
		return KDNAFormsModel::duplicate_form( $form_id );
	 ...
	 * @uses \KDNAFormsModel::update_form_meta()
	 *
	...
	public static function update_form( $form, $form_id = null ) {
		...
		if ( ! $form ) {
			return new WP_Error( 'invalid', __( 'Invalid form object', 'kdnaforms' ) );
		}
		...
		if ( empty( $form_id ) ) {
			return new WP_Error( 'missing_form_id', __( 'Missing form id', 'kdnaforms' ) );
		}
		...
		if ( isset( $form['fields'] ) ) {
			...
			$next_field_id = KDNAFormsModel::get_next_field_id( $form['fields'] );

			$form['fields'] = self::add_missing_ids( $form['fields'], $next_field_id );
		}

		$meta_table_name = KDNAFormsModel::get_meta_table_name();
		...
		if ( intval( $wpdb->get_var( $wpdb->prepare( "SELECT count(0) FROM {$meta_table_name} WHERE form_id=%d", $form_id ) ) ) == 0 ) { // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			return new WP_Error( 'not_found', __( 'Form not found', 'kdnaforms' ) );
		}
		...
		$result = KDNAFormsModel::update_form_meta( $form_id, $form_display_meta );
		if ( false === $result ) {
			return new WP_Error( 'error_updating_form', __( 'Error updating form', 'kdnaforms' ), $wpdb->last_error );
		}

		if ( isset( $form['confirmations'] ) && is_array( $form['confirmations'] ) ) {
			$confirmations = self::set_property_as_key( $form['confirmations'], 'id' );
			$result        = KDNAFormsModel::update_form_meta( $form_id, $confirmations, 'confirmations' );
			if ( false === $result ) {
				return new WP_Error( 'error_updating_confirmations', __( 'Error updating form confirmations', 'kdnaforms' ), $wpdb->last_error );
			}
		}

		if ( isset( $form['notifications'] ) && is_array( $form['notifications'] ) ) {
			$notifications = self::set_property_as_key( $form['notifications'], 'id' );
			$result        = KDNAFormsModel::update_form_meta( $form_id, $notifications, 'notifications' );
			if ( false === $result ) {
				return new WP_Error( 'error_updating_notifications', __( 'Error updating form notifications', 'kdnaforms' ), $wpdb->last_error );
			}
		}
		...
		KDNAFormsModel::flush_current_form( KDNAFormsModel::get_form_cache_key( $form_id ) );

		if ( false === $result ) {
			return new WP_Error( 'error_updating_title', __( 'Error updating title', 'kdnaforms' ), $wpdb->last_error );
		}
	...
	public static function update_forms_property( $form_ids, $property_key, $value ) {
		...
		$table      = KDNAFormsModel::get_form_table_name();
		$db_columns = KDNAFormsModel::get_form_db_columns();

		if ( ! in_array( strtolower( $property_key ), $db_columns ) ) {
			return new WP_Error( 'property_key_incorrect', __( 'Property key incorrect', 'kdnaforms' ) );
		}
	...
	public static function add_forms( $forms, $continue_on_error = false ) {
		...
		if ( ! $forms || ! is_array( $forms ) ) {
			return new WP_Error( 'invalid', __( 'Invalid form objects', 'kdnaforms' ) );
		}
		$form_ids = array();
		$failed_forms = array();
	...
	public static function add_form( $form_meta ) {
		...
		if ( rgar( $form_meta, 'title' ) == '' ) {
			return new WP_Error( 'missing_title', __( 'The form title is missing', 'kdnaforms' ) );
		}

		if ( ! isset( $form_meta['fields'] ) || ! is_array( $form_meta['fields'] ) ) {
			return new WP_Error( 'missing_fields', __( 'The form fields are missing', 'kdnaforms' ) );
		}
		...
		if ( ! isset( $form_meta['confirmations'] ) || empty( $form_meta['confirmations'] ) ) {
			...
			$confirmation = KDNAFormsModel::get_default_confirmation();
		...
		if ( isset( $form_meta['confirmations'] ) ) {
			...
			KDNAFormsModel::update_form_meta( $form_id, $form_meta['confirmations'], 'confirmations' );
		...
		if ( isset( $form_meta['notifications'] ) ) {
			...
			KDNAFormsModel::update_form_meta( $form_id, $form_meta['notifications'], 'notifications' );
			unset( $form_meta['notifications'] );
		}

		// Make sure the formId is correct.
		$form_meta = KDNAFormsModel::convert_field_objects( $form_meta );

		$next_field_id = KDNAFormsModel::get_next_field_id( $form_meta['fields'] );

		$form_meta['fields'] = self::add_missing_ids( $form_meta['fields'], $next_field_id );

		// Updating form meta.
		$result = KDNAFormsModel::update_form_meta( $form_id, $form_meta );

		if ( false === $result ) {
			return new WP_Error( 'insert_form_error', __( 'There was a problem while inserting the form', 'kdnaforms' ), $wpdb->last_error );
		}
	 ...
	 *     $search_criteria['field_filters'][] = array('key' => '1', 'value' => 'kdnaquiz159982170');
	 *
	 *  Filter Operators
	 ...
	 *     $search_criteria['field_filters'][] = array('key' => '2.2', 'value' => 'kdnaquiz246fec995');
	 *     NOTES:
	 ...
	 *     $search_criteria['field_filters'][] = array('key' => '2', 'value' => 'kdnaquiz246fec995');
	 ...
	 *
	 *  Filter entries by Entry meta (added using the kdnaform_entry_meta hook)
	 *     $search_criteria['field_filters'][] = array('key' => 'kdnaquiz_score', 'value' => '1');
	 *     $search_criteria['field_filters'][] = array('key' => 'kdnaquiz_is_pass', 'value' => '1');
	 *
	...
	public static function get_entries( $form_ids, $search_criteria = array(), $sorting = null, $paging = null, &$total_count = null ) {
		...
		$q = new KDNA_Query( $form_ids, $search_criteria, $sorting, $paging );
		$entries = $q->get();
	...
	public static function get_entry_ids( $form_id, $search_criteria = array(), $sorting = null, $paging = null, &$total_count = null ) {
		...
		$the_query = new KDNA_Query( $form_id, $search_criteria, $sorting, $paging  );
	 ...
	 * @access public
	 *
	 * @uses KDNAFormsModel::count_search_leads()
	 *
	...
	public static function get_entry( $entry_id ) {
		...
		if ( version_compare( KDNAFormsModel::get_database_version(), '2.3-dev-1', '<' ) ) {
			...
			if ( empty( $entries ) ) {
				return new WP_Error( 'not_found', sprintf( __( 'Entry with id %s not found', 'kdnaforms' ), $entry_id ), $entry_id );
			}
	...
	public static function update_entries( $entries ) {
		...
		foreach ( $entries as $entry ) {
			...
			KDNACommon::log_debug( __METHOD__ . '(): Updating entry ' . $entry_id );
	...
	public static function update_entry( $entry, $entry_id = null ) {
		...
		if ( ! isset( $entry['ip'] ) ) {
			$entry['ip'] = rgars( $form, 'personalData/preventIP' ) ? '' : KDNAFormsModel::get_ip();
		}
		$ip = $entry['ip'];

		if ( ! isset( $entry['source_url'] ) ) {
			$entry['source_url'] = KDNAFormsModel::get_current_page_url();
		}
		...
		if ( empty( $entry['currency'] ) ) {
			$entry['currency'] = KDNACommon::get_currency();
		}
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( ! $has_product_field && KDNACommon::is_product_field( $field->type ) ) {
		...
		// Unset to prevent KDNAFormsModel::queue_batch_field_operation() setting them to empty strings in $entry during the next foreach.
		$entry_columns = KDNAFormsModel::get_lead_db_columns();
		...
		KDNAFormsModel::commit_batch_field_operations();

		if ( $has_product_field ) {
			KDNAFormsModel::refresh_product_cache( $form, $entry );
		}
	...
	private static function purge_missing_entry_values( $form, &$entry, $current_entry, $current_fields, $item_index = '' ) {
		...
		if ( $current_entry !== null ) {
			foreach ( $current_entry as $k => $v ) {
				...
				if ( $field instanceof KDNA_Field_Repeater && ! empty( $v ) ) {
					...
					$lead_detail_id = KDNAFormsModel::get_lead_detail_id( $current_fields, $k, $item_index );
					$result         = KDNAFormsModel::queue_batch_field_operation( $form, $entry, $field, $lead_detail_id, $k, '', $item_index );
					if ( false === $result ) {
		...
		if ( is_array( $inputs ) ) {
			foreach ( $field->inputs as $input ) {
				...
				if ( empty( $current_entry ) || $input_value != $current_value ) {
					$lead_detail_id = $current_fields ? KDNAFormsModel::get_lead_detail_id( $current_fields, $input_id, $item_index ) : 0;
					$result         = KDNAFormsModel::queue_batch_field_operation( $form, $entry, $field, $lead_detail_id, $input_id, $input_value, $item_index );
					if ( false === $result ) {
						return new WP_Error( 'update_input_value_failed', __( 'There was a problem while updating one of the input values for the entry', 'kdnaforms' ) );
					}
			...
			if ( empty( $current_entry ) || $field_value != $current_value ) {
				$lead_detail_id = $current_fields ? KDNAFormsModel::get_lead_detail_id( $current_fields, $field_id, $item_index ) : 0;
				$result         = KDNAFormsModel::queue_batch_field_operation( $form, $entry, $field, $lead_detail_id, $field_id, $field_value, $item_index );
				if ( false === $result ) {
					return new WP_Error( 'update_field_values_failed', __( 'There was a problem while updating the field values', 'kdnaforms' ) );
				}
				foreach ( $current_fields as $current_field ) {
	...
	public static function delete_entry( $entry_id ) {
		...
		$entry = KDNAFormsModel::get_entry( $entry_id );
		if ( empty( $entry ) ) {
			return new WP_Error( 'invalid_entry_id', sprintf( __( 'Invalid entry id: %s', 'kdnaforms' ), $entry_id ), $entry_id );
		}
		KDNAFormsModel::delete_entry( $entry_id );
	...
	public static function update_entry_property( $entry_id, $property, $value ) {
		...
		return KDNAFormsModel::update_entry_property( $entry_id, $property, $value );
	}
	...
	public static function update_entry_field( $entry_id, $input_id, $value, $item_index = '' ) {
		...
		$entry_meta_table_name = KDNAFormsModel::get_entry_meta_table_name();
		$result                = true;
		...
		if ( $field instanceof KDNA_Field_Repeater && isset( $field->fields ) && is_array( $field->fields ) ) {
			if ( isset( $entry[ $input_id ] ) ) {
				...
				$result = KDNAFormsModel::update_entry_field_value( $form, $entry, $field, 0, $input_id, '' );
			}
			if ( true !== $result ) {
				return $result;
			}
			...
			if ( ! isset( $entry[ $input_id ] ) || ( $value === 0 && $entry[ $input_id ] !== '0' ) || $entry[ $input_id ] != $value ) {
				$result = KDNAFormsModel::update_entry_field_value( $form, $entry, $field, $lead_detail_id, $input_id, $value, $item_index );
			}
		}
	...
	public static function get_notes( $search_criteria = array(), $sorting = null ) {
		...
		$notes = KDNAFormsModel::get_notes( $search_criteria, $sorting );
	...
	public static function get_note( $note_id ) {
		$note = KDNAFormsModel::get_notes( array( 'id' => $note_id ) );

		if ( empty( $note ) ) {
			return new WP_Error( 'note_not_found', __( 'Note not found', 'kdnaforms' ) );
		}
	...
	public static function add_note( $entry_id, $user_id, $user_name, $note, $note_type = 'user', $sub_type = null ) {
		...
		if ( ! self::entry_exists( $entry_id ) ) {
			return new WP_Error( 'invalid_entry', __( 'Invalid entry', 'kdnaforms' ), $entry_id );
		}

		if ( empty( $note ) || ! is_string( $note ) ) {
			return new WP_Error( 'invalid_note', __( 'Invalid or empty note', 'kdnaforms' ), $entry_id );
		}

		$new_note = KDNAFormsModel::add_note( intval( $entry_id ), $user_id, $user_name, wp_kses_post( $note ), sanitize_text_field( $note_type ), sanitize_text_field( $sub_type ) );
	...
	public static function delete_note( $note_id ) {
		$result = KDNAFormsModel::delete_note( $note_id );

		if ( ! $result ) {
			return new WP_Error( 'invalid_note', __( 'Invalid note', 'kdnaforms' ), $note_id );
		}
	...
	public static function update_note( $note, $note_id = '' ) {
		...
		if ( ! is_array( $note ) || empty( $note ) ) {
			return new WP_Error( 'invalid_note_format', __( 'Invalid note format', 'kdnaforms' ) );
		}
		...
		if ( empty( $note_id ) ) {
			return new WP_Error( 'missing_note_id', __( 'Missing note id', 'kdnaforms' ) );
		}
		...
		$result = KDNAFormsModel::update_note( $note['id'], $note['entry_id'], $note['user_id'], $note['user_name'], $note['date_created'], $note['value'], $note['note_type'], $note['sub_type'] );
	 ...
	 * @param null|int $initiated_by Optional. The process that initiated the submission. Supported integers are 1 (aka KDNAFormDisplay::SUBMISSION_INITIATED_BY_WEBFORM) or 2 (aka KDNAFormDisplay::SUBMISSION_INITIATED_BY_API). Defaults to KDNAFormDisplay::SUBMISSION_INITIATED_BY_API.
	 *
	...
	public static function submit_form( $form_id, $input_values, $field_values = array(), $target_page = 0, $source_page = 1, $initiated_by = null ) {
		...
		add_filter( 'kdnaform_suppress_confirmation_redirect', '__return_true' );

		try {
			require_once KDNACommon::get_base_path() . '/form_display.php';
			$initiated_by = KDNACommon::whitelist( $initiated_by, array( KDNAFormDisplay::SUBMISSION_INITIATED_BY_API, KDNAFormDisplay::SUBMISSION_INITIATED_BY_WEBFORM ) );
			KDNAFormDisplay::process_form( $form_id, $initiated_by );
		} catch ( Exception $ex ) {
			remove_filter( 'kdnaform_suppress_confirmation_redirect', '__return_true' );
			remove_filter( 'kdnaform_pre_validation', array( 'KDNAAPI', 'submit_form_filter_kdnaform_pre_validation' ), 50 );
			return new WP_Error( 'error_processing_form', __( 'There was an error while processing the form:', 'kdnaforms' ) . ' ' . $ex->getCode() . ' ' . $ex->getMessage() );
		}
		...
		$submission_details = rgar( KDNAFormDisplay::$submission, $form_id );
		if ( empty( $submission_details ) ) {
			return new WP_Error( 'error_processing_form', __( 'There was an error while processing the form.', 'kdnaforms' ) );
		}
		...
		if ( $result['page_number'] !== 0 ) {
			$files = rgar( KDNAFormsModel::$uploaded_files, $form_id );
		...
		if ( isset( $submission_details['resume_token'] ) ) {
			...
			$result['confirmation_message'] = KDNAFormDisplay::replace_save_variables( $result['confirmation_message'], $form, $result['resume_token'] );
		}
	 ...
	 * @param array $field_values Optional. An array of dynamic population parameter keys with their corresponding values used to populate the fields. Overwrites `$_POST['kdnaform_field_values']`.
	 * @param int   $target_page  Optional. For multi-page forms; indicates which page would be loaded next if the current page passes validation. Overwrites `$_POST[ 'kdnaform_target_page_number_' . $form_id ]`.
	 * @param int   $source_page  Optional. For multi-page forms; indicates which page was active when the values were submitted for validation. Overwrites `$_POST[ 'kdnaform_source_page_number_' . $form_id ]`.
	 *
	...
	public static function validate_form( $form_id, $input_values = array(), $field_values = array(), $target_page = 0, $source_page = 1 ) {
		...
		if ( KDNACommon::form_requires_login( $form ) && ! is_user_logged_in() ) {
			return new WP_Error( 'login_required', __( 'You must be logged in to use this form.', 'kdnaforms' ) );
		}

		self::hydrate_post( $form_id, $input_values, $field_values, $target_page, $source_page );

		// Support validation of multi-file enabled fields by getting the details from the kdnaform_uploaded_files input.
		KDNAFormsModel::set_uploaded_files( $form_id );

		$failed_validation_page = $source_page;

		require_once KDNACommon::get_base_path() . '/form_display.php';
		KDNAFormDisplay::$submission_initiated_by = KDNAFormDisplay::SUBMISSION_INITIATED_BY_API_VALIDATION;

		$is_valid = KDNAFormDisplay::validate( $form, $field_values, $source_page, $failed_validation_page );
		remove_filter( 'kdnaform_pre_validation', array( 'KDNAAPI', 'submit_form_filter_kdnaform_pre_validation' ), 50 );
		...
		if ( $is_valid ) {
			if ( $target_page === 0 ) {
				$result['is_spam'] = KDNACommon::is_spam_entry( KDNAFormsModel::create_lead( $form ), $form );
			}

			return $result;
		}

		$form_restriction_error = rgars( KDNAFormDisplay::$submission, $form_id . '/form_restriction_error' );
		if ( $form_restriction_error ) {
			return new WP_Error( 'form_restriction_error', $form_restriction_error );
		}

		$button_logic_error = rgars( KDNAFormDisplay::$submission, $form_id . '/button_logic_error' );
		if ( $button_logic_error ) {
			return new WP_Error( 'button_logic_error', $button_logic_error );
		}
	 ...
	 *
	 * @since 2.7
	 * @since 2.8.7 Added the kdnaform_pre_validation filter.
	 *
	 * @param int   $form_id      The ID of the form this submission belongs to.
	...
	public static function validate_field( $form_id, $field_id, $input_values = array() ) {
		...
		if ( ! $field ) {
			return new WP_Error( 'field_not_found', __( 'Field not found.', 'kdnaforms' ) );
		}

		require_once KDNACommon::get_base_path() . '/form_display.php';
		if ( ! KDNAFormDisplay::is_field_validation_supported( $field ) ) {
			return new WP_Error( 'not_supported', __( 'Field does not support validation.', 'kdnaforms' ) );
		}

		self::hydrate_post( $form_id, $input_values, array(), 0, $field->pageNumber );

		// Ensure the state input is populated.
		self::submit_form_filter_kdnaform_pre_validation( $form );

		return KDNAFormDisplay::validate_field( $field, $form, 'api-validate' );
	}
	...
	private static function get_submission_form( $form_id ) {
		...
		if ( empty( $form ) || ! $form['is_active'] || $form['is_trash'] ) {
			return new WP_Error( 'form_not_found', __( 'Your form could not be found', 'kdnaforms' ) );
		}

		if ( ! KDNACommon::form_has_fields( $form ) ) {
			return new WP_Error( 'no_fields', __( "Your form doesn't have any fields.", 'kdnaforms' ) );
		}
	...
	private static function hydrate_post( $form_id, $input_values, $field_values, $target_page, $source_page ) {
		...
		$_POST['kdnaform_submit']                           = $form_id;
		$_POST[ 'kdnaform_target_page_number_' . $form_id ] = absint( $target_page );
		$_POST[ 'kdnaform_source_page_number_' . $form_id ] = absint( $source_page );
		$_POST['kdnaform_field_values']                     = $field_values;
	...
	/**
	 * Ensure that the state field is set when the form is submitted via KDNAAPI::submit_form()
	...
	public static function get_feeds( $feed_ids = null, $form_ids = null, $addon_slug = null, $is_active = true ) {
		...
		if ( empty( $results ) ) {
			return new WP_Error( 'not_found', __( 'Feed not found', 'kdnaforms' ) );
		}
	...
	public static function encrypt_feed_meta( $feed_meta, $addon_slug ) {
		...
		$addon = KDNAAddon::get_addon_by_slug( $addon_slug );
		if ( ! is_a( $addon, 'KDNAAddon' ) ) {
			return $feed_meta;
		}
	 ...
	 * @since 2.7.17
	 *
	 * @var \KDNA_Forms\KDNA_Forms\Settings\KDNA_Settings_Encryption The encryption service object.
	 */
	 ...
	 * @since 2.7.17
	 *
	 * @return \KDNA_Forms\KDNA_Forms\Settings\KDNA_Settings_Encryption An instance of the encryption service object.
	 */
	public static function get_encryptor() {
		if ( ! self::$_encryptor ) {
			require_once( KDNACommon::get_base_path() . '/includes/settings/class-kdna-settings-service-provider.php' );
			self::$_encryptor = KDNAForms::get_service_container()->get( \KDNA_Forms\KDNA_Forms\Settings\KDNA_Settings_Service_Provider::SETTINGS_ENCRYPTION );
		}
	...
	/**
	 * Sets the encryption service object to be used by KDNAAPI
	 *
	 * @since 2.7.17
	 *
	 * @param KDNA_Forms\KDNA_Forms\Settings\KDNA_Settings_Encryption $encryptor The encryption service object to be used.
	 *
	 * @return void
	 */
	...
	public static function delete_feed( $feed_id ) {
		...
		if ( false === $results ) {
			return new WP_Error( 'error_deleting', sprintf( __( 'There was an error while deleting feed id %s', 'kdnaforms' ), $feed_id ), $wpdb->last_error );
		}
	...
	public static function update_feed( $feed_id, $feed_meta, $form_id = null ) {
		...
		if ( false === $results ) {
			return new WP_Error( 'error_updating', sprintf( __( 'There was an error while updating feed id %s', 'kdnaforms' ), $feed_id ), $wpdb->last_error );
		}
	...
	public static function add_feed( $form_id, $feed_meta, $addon_slug ) {
		...
		if ( false === $results ) {
			return new WP_Error( 'error_inserting', __( 'There was an error while inserting a feed', 'kdnaforms' ), $wpdb->last_error );
		}
		 ...
		 * @param string $addon_slug The slug of the add-on to which the feed belongs
		 */
		do_action( 'kdnaform_post_add_feed', $wpdb->insert_id, $form_id, $feed_meta, $addon_slug );
	...
	public static function update_feed_property( $feed_id, $property_name, $property_value ) {
		return KDNAFormsModel::update_feed_property( $feed_id, $property_name, $property_value );
	}
	...
	private static function get_missing_table_wp_error( $table ) {
		return new WP_Error( 'missing_table', sprintf( __( 'The %s table does not exist.', 'kdnaforms' ), $table ) );
	}
	...
	public static function maybe_process_feeds( $entry, $form, $addon_slug = '', $reset_meta = false, $bypass_feed_delay = false ) {
		if ( ! class_exists( 'KDNAFeedAddOn' ) || empty( $entry['id'] ) ) {
			return false;
		}

		$addons = KDNAFeedAddOn::get_registered_feed_addons();

		if ( ! empty( $addon_slug ) ) {
			$addon = rgar( $addons, $addon_slug );
			if ( empty( $addon ) || $addon instanceof KDNAPaymentAddOn ) {
				return false;
			}
			...
			foreach ( $addons as $slug => $addon ) {
				if ( $addon instanceof KDNAPaymentAddOn ) {
					continue;
				}
	...
	public static function get_processed_feeds_meta( $entry_id, $addon_slug = '' ) {
		$meta = kdnaform_get_meta( $entry_id, 'processed_feeds' );
	...
	public static function update_processed_feeds_meta( $entry_id, $addon_slug, $value, $form_id = null ) {
		if ( empty( $addon_slug ) ) {
			if ( empty( $value ) ) {
				kdnaform_delete_meta( $entry_id, 'processed_feeds' );
			} elseif ( ! isset( $value[0] ) ) {
				kdnaform_update_meta( $entry_id, 'processed_feeds', $value, $form_id );
			}
		...
		kdnaform_update_meta( $entry_id, 'processed_feeds', $meta, $form_id );
	}
	...
	public static function get_entry_feed_status( $entry_id, $feed_id, $return_latest = false, $return_latest_details = false ) {
		$meta = kdnaform_get_meta( $entry_id, self::get_entry_feed_status_key( $feed_id ) );
		if ( empty( $meta ) ) {
	...
	public static function update_entry_feed_status( $entry_id, $feed_id, $status, $form_id = null ) {
		...
		if ( is_null( $status ) ) {
			kdnaform_delete_meta( $entry_id, $key );
		...
		kdnaform_update_meta( $entry_id, $key, $meta, $form_id );
	}
	...
	public static function send_notifications( $form, $entry, $event = 'form_submission', $data = array() ) {
		...
		KDNACommon::log_debug( __METHOD__ . "(): Gathering notifications for {$event} event for entry #{$entry_id}." );
	...
	public static function current_user_can_any( $capabilities ) {
		return KDNACommon::current_user_can_any( $capabilities );
	}
	 ...
	 *
	 * @uses KDNAFormsModel::get_fields_by_type()
	 *
	 * @return KDNA_Field[]
	 */
	public static function get_fields_by_type( $form, $types, $use_input_type = false ) {
		return KDNAFormsModel::get_fields_by_type( $form, $types, $use_input_type );
	}
	 ...
	 * @uses   KDNAFormsModel::get_field()
	 *
	 * @return KDNA_Field|false
	 */
	public static function get_field( $form_or_id, $field_id ) {
		$field = KDNAFormsModel::get_field( $form_or_id, $field_id );
	...
	public static function form_id_exists( $form_id ) {
		return KDNAFormsModel::id_exists_in_table( $form_id, KDNAFormsModel::get_form_table_name() );
	}
	...
	public static function entry_exists( $entry_id ) {
		return KDNAFormsModel::entry_exists( $entry_id );
	}
	...
	public static function feed_exists( $feed_id ) {
		return KDNAFormsModel::id_exists_in_table( $feed_id, KDNAFormsModel::get_addon_feed_table_name() );
	}

	/**
	 * Write an error message to the KDNA Forms API log.
	 *
	 * @since 2.4.11
	 *
	...
	/**
	 * Write a debug message to the KDNA Forms API log.
	 *
	 * @since 2.4.11
	 *
	 * @param string $message The message to be logged.
	...
	public static function unique_title( $title, $form_id = '' ) {
		return KDNAFormsModel::maybe_increment_title( $title, $form_id );
	}
