...
namespace KDNA_Forms\KDNA_Forms\Orders\Factories;
...
final class KDNA_Order_Factory {
	...
	public static function create_from_entry( $form, $entry, $use_choice_text = false, $use_admin_labels = false, $receipt = false ) {
		...
		if ( ! self::validate( $form, $entry ) ) {
			return new KDNA_Order();
		}

		$order_meta = kdnaform_get_meta( $entry['id'], 'kdnaform_order' );
		...
		foreach ( $meta_rows as $group_label => $items ) {
			foreach ( $items as $item ) {
				...
				if ( ! rgar( $item, 'name' ) && $order->get_item( $item['id'] ) ) {
					...
					$order->add_item( new KDNA_Order_Item( $item['id'], $item ) );
				}
			}
		}
	 ...
	 * @param \KDNAPaymentAddOn|null  $addon       An instance of the addon that is creating the order.
	 *
	 * @return KDNA_Order
	 */
	public static function create_from_feed( $feed, $form, $entry, $submission, $addon = null ) {
		...
		if ( rgars( $feed, 'meta/transactionType' ) === 'subscription' && rgars( $feed, 'meta/trial_enabled' ) ) {
			...
			$trial_discount = KDNACommon::to_number( $submission['payment_amount'], $entry['currency'] ) * -1;
			/**
			...
			$order->add_item(
				new KDNA_Order_Item(
					...
					array(
						...
						'price'        => abs( KDNACommon::to_number( $trial_discount, $entry['currency'] ) ) * -1,
						'belongs_to'   => 'footer',
						'is_line_item' => true,
						'is_discount'  => true,
					)
			...
			if (
				...
				$price = rgars( $feed, 'meta/trial_product' ) === 'enter_amount' ? KDNACommon::to_number( rgars( $feed, 'meta/trial_amount' ), $order->currency ) : 0;
		 ...
		 * @param array  $entry             The entry object to filter through
		 */
		$additional_items = kdna_apply_filters( array( 'kdnaform_order_additional_items', $form['id'] ), array(), $form, $feed, $entry );
		...
		foreach ( $additional_items as $id => $data ) {
			...
			$order->add_item( new KDNA_Order_Item( $id, $data ) );
		}
	...
	public static function load_dependencies() {
		if ( ! class_exists( 'KDNA_Order' ) ) {
			require_once KDNACommon::get_base_path() . '/includes/orders/class-kdna-order.php';
			require_once KDNACommon::get_base_path() . '/includes/orders/items/class-kdna-order-item.php';
			require_once KDNACommon::get_base_path() . '/includes/orders/items/class-kdna-form-product-order-item.php';
			require_once KDNACommon::get_base_path() . '/includes/orders/exporters/class-kdna-order-exporter.php';
			require_once KDNACommon::get_base_path() . '/includes/orders/exporters/class-kdna-entry-details-order-exporter.php';
			require_once KDNACommon::get_base_path() . '/includes/orders/exporters/class-kdna-save-entry-order-exporter.php';
			require_once KDNACommon::get_base_path() . '/includes/orders/summaries/class-kdna-order-summary.php';
		}
	}
