		...
		<table cellspacing="0" style="border-left:1px solid #DFDFDF; border-top:1px solid #DFDFDF; width: 97%">
			...
			<?php foreach ( rgars( $order_summary, 'rows/body', array() ) as $row ) { ?>
				<tr>
					<td style="border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; font-family: sans-serif; font-size:11px;">
						<strong style="color:#BF461E; font-size:12px; margin-bottom:5px"><?php echo \KDNACommon::maybe_wp_kses( rgar( $row, 'name' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></strong>
							<ul style="margin:0">
						<?php if ( is_array( rgar( $row, 'options' ) ) ) { ?>
								...
								for ( $i = 0; $i < $count; $i ++ ) {
									?>
									<li style="padding:4px 0 4px 0"><?php echo \KDNACommon::maybe_wp_kses( rgar( $row['options'][ $i ], 'option_label' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped?></li>
								<?php } ?>
						<?php } ?>
							</ul>
					</td>
			...
			<tr>
				...
				<td style="border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; text-align:right; width:155px; font-family: sans-serif;">
					<strong style="font-size:12px;"><?php esc_html_e( 'Sub Total', 'kdnaforms' ); ?></strong></td>
			...
			<?php foreach ( rgars( $order_summary, 'rows/footer', array() ) as $row ) { ?>
				<tr>
					<td colspan="2" style="background-color:#F4F4F4; border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; font-size:11px;">&nbsp;</td>
					<td style="border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; width:155px; font-family: sans-serif;font-size:12px;text-align:right;"><?php echo \KDNACommon::maybe_wp_kses( rgar( $row, 'name' ) );  // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></td>
			...
			<tr>
				<td colspan="2" style="background-color:#F4F4F4; border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; font-size:11px;">&nbsp;</td>
				<td style="border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; text-align:right; width:155px; font-family: sans-serif;">
					<strong style="font-size:12px;"><?php esc_html_e( 'Total', 'kdnaforms' ); ?></strong></td>
				<td style="border-bottom:1px solid #DFDFDF; border-right:1px solid #DFDFDF; padding:7px; width:155px; font-family: sans-serif;">
