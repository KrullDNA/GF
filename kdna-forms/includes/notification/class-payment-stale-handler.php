...
class Payment_Stale_Handler {
	...
	public function activate_cron() {
		$cron_name = 'kdnaform_stale_payment_cron';
	...
	/**
	 * Deactivates the cron for processing stale payment notifications. This function is hooked to the 'kdnaform_uninstalling' action and will run when KDNA Forms is uninstalled, to clean up the scheduled cron job.
	 *
	 * @since 2.9.29
	 *
	 * @return void
	...
	public function deactivate_cron() {
		wp_clear_scheduled_hook( 'kdnaform_stale_payment_cron' );
	}
	...
	public function maybe_execute_cron() {
		if ( defined( 'KDNA_DEBUG' ) && KDNA_DEBUG && isset( $_GET['_execute_stale_payment_cron'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended -- This is for debugging purposes only and requires the KDNA_DEBUG constant to be set.
			$this->process_stale_payment_notifications();
		}
	}

	/**
	 * Processes stale payment notifications. This function is hooked to the 'kdnaform_stale_payment_cron' and will run hourly to check for entries with a payment status of "Processing" that are older than a specified threshold (default is 60 minutes).
	 * For each of those entries, it will send a notification and update the entry meta to indicate that the stale notification has been processed.
	 *
	 * @since 2.9.29
	 *
	...
	public function process_stale_payment_notifications() {
		...
		KDNACommon::log_debug( __METHOD__ . '(): Starting to process stale payment notifications.' );

		// Recording the cron event for system report page.
		KDNACommon::record_cron_event( 'kdnaform_stale_payment_cron' );

		// Getting entries that are considered to have a "stale" payment.
		$entries = $this->get_stale_entries();

		if ( empty( $entries ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): There are no entries with a stale payment. Nothing to do. Aborting.' );
			return;
		}

		foreach ( $entries as $entry ) {
			KDNACommon::log_debug( __METHOD__ . '(): Sending stale payment notification for entry #' . $entry['id'] );
	...
	public function get_stale_entries() {
		 ...
		 */
		$threshold_minutes = absint( apply_filters( 'kdnaform_payment_stale_threshold_minutes', 60 ) );
		...
		$paging = [ 'offset' => 0, 'page_size' => (int) apply_filters( 'kdnaform_payment_stale_batch_size', 100 ) ];

		return KDNAAPI::get_entries( 0, $search_criteria, null, $paging );
	}
	...
	public function send_stale_payment_notifications( $entry ) {
		...
		$form = KDNAAPI::get_form( (int) $entry['form_id'] );

		$notification_ids = array_keys( $this->get_stale_payment_notifications( $form ) );

		// Sending stale payment notifications.
		KDNACommon::send_notifications( $notification_ids, $form, $entry, true, 'payment_stale' );
	}

	/**
	 * Handles the logic for when an entry's payment status changes. This function is hooked to the 'kdnaform_post_payment_status_change' action and will run whenever an entry's payment status is updated.
	...
	public function handle_payment_status_change( $entry ) {
		...
		if ( $entry['payment_status'] === 'Processing' ) {
			...
			if ( $this->has_stale_payment_notification( KDNAAPI::get_form( (int) $entry['form_id'] ) ) ) {
	...
	public function add_entry_meta( $entry ) {
		$alreadyProcessed = kdnaform_get_meta( $entry['id'], 'payment_stale_notification_processed' );
		...
		KDNACommon::log_debug( __METHOD__ . '(): Entry #' . $entry['id'] . ' marked as "Processing". Adding payment_stale_notification_processed=0 entry meta to include this entry in the stale payment notification cron process.' );

		// Add the entry meta. This will include this entry in the stale payment notification cron process.
		kdnaform_update_meta( $entry['id'], 'payment_stale_notification_processed', 0, $entry['form_id'] );
	}
	...
	public function update_entry_meta( $entry ) {
		...
		kdnaform_update_meta( $entry['id'], 'payment_stale_notification_processed', 1, $entry['form_id'] );

		KDNACommon::log_debug( __METHOD__ . '(): Stale payment notification processed for entry #' . $entry['id'] . '. Updating payment_stale_notification_processed=1 entry meta to prevent resending.' );
	}
	...
	public function remove_entry_meta( $entry ) {
		KDNACommon::log_debug( __METHOD__ . '(): Entry #' . $entry['id'] . ' completed payment processing. Deleting payment_stale_notification_processed entry meta to exclude this entry in the stale payment notification cron process.' );

		// Deleting the entry meta. This will exclude this entry in the stale payment notification cron process.
		kdnaform_delete_meta( $entry['id'], 'payment_stale_notification_processed' );
	}
}
