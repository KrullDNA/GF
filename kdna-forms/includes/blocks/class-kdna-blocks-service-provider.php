...
use KDNA_Forms\KDNA_Forms\Blocks\Config\KDNA_Blocks_Config;
use KDNA_Forms\KDNA_Forms\Blocks\KDNA_Block_Attributes;
...
/**
 * Class KDNA_Blocks_Service_Provider
 *
 * Service provider for the Blocks Service.
 *
 * @package KDNA_Forms\KDNA_Forms\Blocks;
 */
class KDNA_Blocks_Service_Provider extends KDNA_Service_Provider {
	...
	protected $configs = array(
		self::BLOCKS_CONFIG => KDNA_Blocks_Config::class,
	);
	...
	public function register( KDNA_Service_Container $container ) {
		...
		require_once( plugin_dir_path( __FILE__ ) . 'class-kdna-block-attributes.php' );
		require_once( plugin_dir_path( __FILE__ ) . '/config/class-kdna-blocks-config.php' );

		$container->add( self::FORM_BLOCK_ATTRIBUTES, function () {
			require_once( KDNACommon::get_base_path() . '/form_display.php' );
			$global_styles = KDNAFormDisplay::validate_form_styles( apply_filters( 'kdnaform_default_styles', false ) );
	...
	private function add_configs( KDNA_Service_Container $container ) {
		foreach ( $this->configs as $name => $class ) {
			$container->add( $name, function () use ( $container, $class ) {
				if ( $class == KDNA_Blocks_Config::class ) {
					return new $class( $container->get( KDNA_Config_Service_Provider::DATA_PARSER ), $container->get( self::FORM_BLOCK_ATTRIBUTES ) );
				}
	 ...
	 * @return void
	 */
	private function block_attributes( KDNA_Service_Container $container ) {
		$container->add( self::BLOCK_ATTRIBUTES, function () use ( $container ) {
			return new KDNA_Block_Attributes();
		} );
	}
