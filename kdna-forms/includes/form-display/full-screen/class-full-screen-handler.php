...
class Full_Screen_Handler {
	...
	public function __construct( GF_JSON_Handler $handler ) {
		...
		add_filter( 'redirect_canonical', function ( $redirect_url ) use ( $handler ) {
			...
			$form_for_display = apply_filters( 'kdnaform_full_screen_form_for_display', null, '', $handler );
	...
	public function load_full_screen_template( $template ) {
		 ...
		 * @return string
		 */
		$form_for_display = apply_filters( 'kdnaform_full_screen_form_for_display', null, $template, $this->json_handler );

		if ( ! $form_for_display ) {
			return $template;
		}

		$form = \KDNAFormsModel::get_form( $form_for_display );
		 ...
		 * @return string
		 */
		$new_template = apply_filters( 'kdnaform_full_screen_template_path', $new_template, $form_for_display );
		 ...
		 * the kdnaform_full_screen_form_for_display filter.
		 *
		 * @since 2.7
		 *
		 ...
		 * @return string.
		 */
		add_filter( 'kdnaform_full_screen_form_id', function () use ( $form_for_display ) {
