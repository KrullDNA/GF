...
class KDNAFormDisplay {
	...
	public static function process_form( $form_id, $initiated_by = self::SUBMISSION_INITIATED_BY_API ) {
		KDNACommon::timer_start( __METHOD__ );
		KDNACommon::log_debug( "KDNAFormDisplay::process_form(): Starting to process form (#{$form_id}) submission." );
		...
		if ( gf_has_filter( $kdnaform_pre_process_args ) ) {
			...
			KDNACommon::log_debug( __METHOD__ . '(): Completed kdnaform_pre_process.' );
		}

		// Set files that have been uploaded to temp folder
		$files = KDNAFormsModel::set_uploaded_files( $form_id );

		//reading form metadata
		$form = self::maybe_add_review_page( $form );

		if ( ! rgar( $form, 'is_active' ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Aborting. Form is inactive.' );

			return;
		}

		if ( rgar( $form, 'is_trash' ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Aborting. Form has been trashed.' );
		...
		if ( KDNACommon::form_requires_login( $form ) ) {
			if ( ! is_user_logged_in() ) {
				KDNACommon::log_debug( __METHOD__ . '(): Aborting. User is not logged in.' );
			...
			if ( ! $is_rest_request ) {
				check_admin_referer( 'kdnaform_submit_' . $form_id, '_kdnaform_submit_nonce_' . $form_id );
			}
		}

		$lead = array();

		$field_values = KDNAForms::post( 'kdnaform_field_values' );
		...
		KDNACommon::log_debug( __METHOD__ . "(): After validation. Is submission valid? {$log_is_valid}." );
		...
		if ( $target_page > $page_number || $target_page === 0 ) {
			if ( ! empty( $_FILES ) && ! $saving_for_later ) {
				...
				KDNACommon::log_debug( 'KDNAFormDisplay::process_form(): Uploading files...' );
				// Uploading files to temporary folder.
				$files = self::upload_files( $form, $files );

				KDNAFormsModel::$uploaded_files[ $form_id ] = $files;
			}
		}
		...
		if ( ( $is_valid && $page_number === 0 ) || $saving_for_later ) {
			$ajax = isset( $_POST['kdnaform_ajax'] );
			 ...
			 * @since 2.7
			 *
			 * @see https://docs.kdnaforms.com/kdnaform_abort_submission_with_confirmation/
			 *
			...
			if ( $abort_with_confirmation ) {
				if ( $saving_for_later ) {
					KDNACommon::log_debug( __METHOD__ . '(): Aborting save for later via kdnaform_abort_submission_with_confirmation filter.' );
				} else {
					KDNACommon::log_debug( __METHOD__ . '(): Aborting early via kdnaform_abort_submission_with_confirmation filter.' );
				}
				...
				KDNACommon::log_debug( 'KDNAFormDisplay::process_form(): Saving for later.' );
				$lead = KDNAFormsModel::get_current_lead();
				...
				$confirmation = KDNACommon::replace_variables( $confirmation, $form, $lead, false, true, $nl2br );

				$form_unique_id = KDNAFormsModel::get_form_unique_id( $form_id );
				$ip             = rgars( $form, 'personalData/preventIP' ) ? '' : KDNAFormsModel::get_ip();
				$source_url     = KDNAFormsModel::get_current_page_url();
				$source_url     = esc_url_raw( $source_url );
				$resume_token   = rgpost( 'kdnaform_resume_token' );
				...
				$notifications_to_send = KDNACommon::get_notifications_to_send( 'form_saved', $form, $lead );

				$log_notification_event = empty( $notifications_to_send ) ? 'No notifications to process' : 'Processing notifications';
				KDNACommon::log_debug( "KDNAFormDisplay::process_form(): {$log_notification_event} for form_saved event." );

				foreach ( $notifications_to_send as $notification ) {
					if ( isset( $notification['isActive'] ) && ! $notification['isActive'] ) {
						KDNACommon::log_debug( "KDNAFormDisplay::process_form(): Notification is inactive, not processing notification (#{$notification['id']} - {$notification['name']})." );
						continue;
					}
				...
				KDNACommon::log_debug( 'KDNAFormDisplay::process_form(): Saved incomplete submission.' );

			}

			/**
			 * Allows the confirmation redirect header to be suppressed. Required by KDNAAPI::submit_form().
			 *
			 * @since 2.3
			 *
			 * @param bool $suppress_redirect
			 */
			$suppress_redirect = apply_filters( 'kdnaform_suppress_confirmation_redirect', false );
	...
	public static function maybe_add_review_page( $form, $partial_entry = array() ) {
		...
		$review_page = array(
			...
			'nextButton'     => array(
				'type'     => 'text',
				'text'     => __( 'Review Form', 'kdnaforms' ),
				'imageUrl' => '',
				'imageAlt' => '',
			),
			'previousButton' => array(
				'type'     => 'text',
				'text'     => __( 'Previous', 'kdnaforms' ),
				'imageUrl' => '',
				'imageAlt' => '',
			),
		);
		...
		if ( gf_has_filter( $kdnaform_review_page_args ) ) {
			...
			if ( empty( $partial_entry ) ) {
				...
				$partial_entry = KDNAFormsModel::get_current_lead();
			}

			/**
			 * KDNAFormsModel::create_lead() caches the field value and conditional logic visibility which can create
			 * issues when 3rd parties use hooks later in the process to modify the form. Let's flush the cache avoid
			 * any weirdness.
			 */
			KDNACache::flush();

			KDNACommon::log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_review_page.' );
			/**
	 ...
	 *
	 * @since unknown
	 * @since 2.9.18 Updated to use KDNA_Field_FileUpload::upload_submission_tmp_files().
	 *
	 * @param array $form  The form currently being processed.
	 * @param array $files The existing files parsed from the kdnaform_uploaded_files input.
	 *
	 * @return array
	 */
	private static function upload_files( $form, $files ) {
		...
		$target_path = rgar( KDNAFormsModel::get_tmp_upload_location( $form_id ), 'path' );
		if ( empty( $target_path ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Aborting; KDNAFormsModel::get_tmp_upload_location() returned an empty path.' );

			return $files;
		}

		if ( ! is_dir( $target_path ) && wp_mkdir_p( $target_path ) ) {
			KDNACommon::recursive_add_index_file( $target_path );
		}

		foreach ( $form['fields'] as $field ) {
			if ( ! $field instanceof KDNA_Field_FileUpload ) {
				continue;
			}
	...
	public static function get_state( $form, $field_values ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( $field->is_state_validation_supported() ) {
				$value = KDNAFormsModel::get_field_value( $field, $field_values, false );
				...
				switch ( $field->get_input_type() ) {
					...
					case 'hiddenproduct' :
						...
						$price = KDNACommon::to_number( $price );

						$product_name = ! is_array( $value ) || empty( $value[ $field->id . '.1' ] ) ? $field->label : $value[ $field->id . '.1' ];

						$fields[ $field->id . '.1' ] = wp_hash( KDNAFormsModel::maybe_trim_input( $product_name, $form['id'], $field ) );
						$fields[ $field->id . '.2' ] = wp_hash( KDNAFormsModel::maybe_trim_input( $price, $form['id'], $field ) );
						break;

					case 'singleshipping' :
						$price = ! empty( $value ) ? $value : $field->basePrice;
						$price = ! empty( $price ) ? KDNACommon::to_number( $price ) : 0;

						$fields[ $field->id ] = wp_hash( KDNAFormsModel::maybe_trim_input( $price, $form['id'], $field ) );
						break;
					case 'radio' :
					case 'select' :
						...
						foreach ( $field->choices as $choice ) {
							...
							$fields[ $field->id ][] = wp_hash( KDNAFormsModel::maybe_trim_input( $field_value, $form['id'], $field ) );
						}
						break;
					case 'checkbox' :
						$index = 1;
						foreach ( $field->choices as $choice ) {
							...
							if ( $field->has_persistent_choices() ) {
								...
								$fields[ $input_id ] = wp_hash( KDNAFormsModel::maybe_trim_input( $field_value, $form['id'], $field ) );
							} else {
								if ( $index % 10 == 0 ) { //hack to skip numbers ending in 0. so that 5.1 doesn't conflict with 5.10
									$index ++;
								}
								$fields[ $field->id . '.' . $index ++ ] = wp_hash( KDNAFormsModel::maybe_trim_input( $field_value, $form['id'], $field ) );
							}
						}
						break;
					case 'consent':
						$text        = $field->checkboxLabel;
						$description = KDNAFormsModel::get_latest_form_revisions_id( $form['id'] );

						$fields[ $field->id . '.1' ] = wp_hash( 1 );
						$fields[ $field->id . '.2' ] = wp_hash( KDNAFormsModel::maybe_trim_input( $text, $form['id'], $field ) );
						$fields[ $field->id . '.3' ] = wp_hash( KDNAFormsModel::maybe_trim_input( $description, $form['id'], $field ) );
						break;
				}
			}
		}
	...
	private static function has_pages( $form ) {
		return KDNACommon::has_pages( $form );
	}
	...
	private static function has_enhanced_dropdown( $form ) {
		...
		foreach ( $form['fields'] as $field ) {
			if ( in_array( KDNAFormsModel::get_input_type( $field ), array( 'select', 'multiselect' ) ) && $field->enableEnhancedUI ) {
				return true;
			}
		}
	...
	public static function get_target_page( $form, $current_page, $field_values ) {
		...
		$page_number = rgpost( "kdnaform_target_page_number_{$form_id}" );
		...
		while ( KDNAFormsModel::is_page_hidden( $form, $page_number, $field_values ) ) {
		 ...
		 * @since 2.1.2.13
		 *
		 * @see https://docs.kdnaforms.com/kdnaform_target_page/
		 *
	...
	public static function get_source_page( $form_id ) {
		$page_number = rgpost( "kdnaform_source_page_number_{$form_id}" );
	 ...
	 *
	 * @param  array  $form A KDNA Forms form object.
	...
	public static function get_form_theme_slug( $form ) {
		...
		if ( KDNACommon::is_legacy_markup_enabled( $form ) ) {
			$slug = 'legacy';
		} elseif ( $is_wp_dashboard || KDNACommon::is_preview() || KDNACommon::is_form_editor() ) {
			$slug = 'orbital';
		} elseif ( isset( $form['theme'] ) ) {
			$slug = $form['theme'];
		} else {
			$instance       = rgar( $form, 'page_instance', 0 );
			$all_blocks     = apply_filters( 'kdnaform_form_block_attribute_values', array() );
		 ...
		 * @return string
		 */
		return apply_filters( 'kdnaform_form_theme_slug', $slug, $form );
	}
	...
	public static function get_themes_to_enqueue( $form, $field_types = '' ) {
		...
		if ( ! empty( $field_types ) && ! KDNACommon::get_fields_by_type( $form, $field_types ) ) {
			return array();
		}
	...
	public static function post_render_script( $form_id, $current_page = 'current_page' ) {
		$post_render_script = '
			jQuery(document).trigger("kdnaform_pre_post_render", [{ formId: "' . $form_id . '", currentPage: "' . $current_page . '", abort: function() { this.preventDefault(); } }]);

	        if (event && event.defaultPrevented) {
            	    return;
        	}
	        const gformWrapperDiv = document.getElementById( "kdnaform_wrapper_' . $form_id . '" );
	        if ( gformWrapperDiv ) {
	...
	public static function get_form( $form_id, $display_title = true, $display_description = true, $force_display = false, $field_values = null, $ajax = false, $tabindex = 0, $form_theme = null, $style_settings = null ) {
		 ...
		 */
		$form_args = apply_filters( 'kdnaform_form_args', compact( 'form_id', 'display_title', 'display_description', 'force_display', 'field_values', 'ajax', 'tabindex' ) );

		// The submission_method property can be set in the kdnaform_form_args filter to specify how the form should be submitted.
		...
		if ( ! is_numeric( $form_id ) ) {
			$form_title = $form_id;
			$form_id    = KDNAFormsModel::get_form_id( $form_title );
			if ( $form_id === 0 ) {
		...
		if ( rgpost( 'kdnaform_send_resume_link' ) == $form_id ) {
			...
			if ( is_wp_error( $save_email_confirmation ) ) { // Failed email validation
				...
				if ( $incomplete_submission_info['form_id'] == $form_id ) {
					...
					$nl2br                   = rgar( $form['confirmation'], 'disableAutoformat' ) ? false : true;
					$confirmation_message    = KDNACommon::replace_variables( $confirmation_message, $form, $partial_entry, false, true, $nl2br );
		...
		if ( isset( $_GET['gf_token'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( rgar( $incomplete_submission_info, 'form_id' ) == $form_id ) {
				...
				KDNAFormsModel::$unique_ids[ $form_id ]     = $submission_details['kdnaform_unique_id'];
		...
		if ( ! is_array( $partial_entry ) ) {
			...
			if ( rgar( $submission_info, 'form' ) ) {
				...
				KDNAFormsModel::insert_form_view( $form_id );
			}
		}

		// Running $form through the kdnaform_pre_render filter.
		$form = self::kdnaform_pre_render( $form, 'form_display', $ajax, $field_values );
		...
		if ( ! $force_display && ! $is_postback ) {
			...
			$form_info = KDNAFormsModel::get_form( $form_id );
			...
			if ( KDNACommon::form_requires_login( $form ) ) {
				if ( ! is_user_logged_in() ) {
					return empty( $form['requireLoginMessage'] ) ? '<p>' . esc_html__( 'Sorry. You must be logged in to view this form.', 'kdnaforms' ) . '</p>' : '<p>' . KDNACommon::kdnaform_do_shortcode( $form['requireLoginMessage'] ) . '</p>';
				}
			}
		}
		...
		$is_entry_detail      = KDNACommon::is_entry_detail();
		$is_admin             = $is_form_editor || $is_entry_detail;
		$should_render_hidden = self::has_conditional_logic( $form ) && rgar( rgget( 'attributes' ), 'formPreview' ) !== 'true';

		if ( empty( $confirmation_message ) ) {
			$wrapper_css_class = KDNACommon::get_browser_class() . ' kdnaform_wrapper';

			if ( ! $is_valid ) {
				$wrapper_css_class .= ' kdnaform_validation_error';
			}
			...
			$form_theme    = KDNAFormDisplay::get_form_theme_slug( $form );

			$form_string .= "
                <div class='{$wrapper_css_class}{$custom_wrapper_css_class}' data-form-theme='{$form_theme}' {$page_instance} id='kdnaform_wrapper_$form_id' " . $style . '>';
			...
			if ( ( $display_title || $display_description ) || $display_required_legend ) {
				$kdnaform_title_open  = KDNACommon::is_legacy_markup_enabled( $form ) ? '<h3 class="kdnaform_title">' : '<h2 class="kdnaform_title">';
				$kdnaform_title_close = KDNACommon::is_legacy_markup_enabled( $form ) ? '</h3>' : '</h2>';

				$form_string .= "
                        <div class='kdnaform_heading'>";
				if ( $display_title ) {
					$form_string .= "
                            {$kdnaform_title_open}" . rgar( $form, 'title' ) . $kdnaform_title_close;
				}
				if ( $display_description ) {
					$form_string .= "
                            <p class='kdnaform_description'>" . rgar( $form, 'description' ) . '</p>';
				}
				...
				if ( $display_required_legend ) {
					...
					$form_string .= "
							<p class='kdnaform_required_legend'>{$required_legend}</p>";
				}
				$form_string .= '
                        </div>';
			}
			...
			$form_string .= gf_apply_filters( array( 'kdnaform_form_tag', $form_id ), "<form method='post' enctype='multipart/form-data' {$target} id='kdnaform_{$form_id}' {$form_css_class} action='{$action}' data-formid='{$form_id}' novalidate>", $form );
			...
			if ( isset( $_GET['gf_token'] ) && ! is_array( $incomplete_submission_info ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				if ( ! empty( $savecontinue_expired_message ) ) {
					$form_string .= sprintf(
						'<div class="validation_error kdnaform_validation_error">%s</div>',
			...
			if ( $has_pages && $is_postback && ! $is_valid ) {
				self::set_current_page( $form_id, KDNAFormDisplay::get_first_page_with_error( $form ) );
			}
			...
			$form_string .= "
                        <div class='kdnaform-body kdnaform_body'>";
			...
			$form_string .= "<{$tag} id='kdnaform_fields_{$form_id}' class='" . KDNACommon::get_ul_classes( $form ) . "'>";
			...
			$form_string .= '</div>'; //closes kdnaform_body

			//suppress form footer for multi-page forms (footer will be included on the last page
			if ( ! $has_pages ) {
				$form_string .= self::kdnaform_footer( $form, 'kdnaform-footer kdnaform_footer ' . $label_placement, $ajax, $field_values, '', $display_title, $display_description, $tabindex, $form_theme, $style_settings, $submission_method );
			}
			...
			if ( $should_show_spinner ) {
				...
				$spinner_url     = gf_apply_filters( array( 'kdnaform_ajax_spinner_url', $form_id ), $default_spinner, $form );
				...
				if ( $anchor['scroll'] !== false ) {
					$scroll_position['default']      = is_numeric( $anchor['scroll'] ) ? 'jQuery(document).scrollTop(' . intval( $anchor['scroll'] ) . ');' : "jQuery(document).scrollTop(jQuery('#kdnaform_wrapper_{$form_id}').offset().top - mt);";
				...
				$iframe_content = esc_html__( 'This iframe contains the logic required to handle Ajax powered KDNA Forms.', 'kdnaforms' );
				$iframe_title   = " title='{$iframe_content}'";
				if ( defined( 'KDNA_DEBUG' ) && KDNA_DEBUG ) {
				...
				if ( ! $ajax || ! $is_postback ) {
					$form_scripts_body =
						...
						"jQuery('#kdnaform_ajax_frame_{$form_id}').on('load',function(){" .
						"var contents = jQuery(this).contents().find('*').html();" .
						"var is_postback = contents.indexOf('KDNA_AJAX_POSTBACK') >= 0;" .
						'if(!is_postback){return;}' .
						"var form_content = jQuery(this).contents().find('#kdnaform_wrapper_{$form_id}');" .
						"var is_confirmation = jQuery(this).contents().find('#kdnaform_confirmation_wrapper_{$form_id}').length > 0;" .
						"var is_redirect = contents.indexOf('gformRedirect(){') >= 0;" .
						'var is_form = form_content.length > 0 && ! is_redirect && ! is_confirmation;' .
						"var mt = parseInt(jQuery('html').css('margin-top'), 10) + parseInt(jQuery('body').css('margin-top'), 10) + 100;" .
						'if(is_form){' .
						( $should_render_hidden ? "form_content.find('form').css('opacity', 0);" : "" ) .
						"jQuery('#kdnaform_wrapper_{$form_id}').html(form_content.html());" .
						"if(form_content.hasClass('kdnaform_validation_error')){jQuery('#kdnaform_wrapper_{$form_id}').addClass('kdnaform_validation_error');} else {jQuery('#kdnaform_wrapper_{$form_id}').removeClass('kdnaform_validation_error');}" .
						...
						"var current_page = jQuery('#kdnaform_source_page_number_{$form_id}').val();" .
						"gformInitSpinner( {$form_id}, '{$spinner_url}', " . ( $is_legacy ? 'true' : 'false' ) . " );" .
						"jQuery(document).trigger('kdnaform_page_loaded', [{$form_id}, current_page]);" .
						"window['gf_submitting_{$form_id}'] = false;" .
						'}' .
						'else if(!is_redirect){' .
						"var confirmation_content = jQuery(this).contents().find('.KDNA_AJAX_POSTBACK').html();" .
						'if(!confirmation_content){' .
						'confirmation_content = contents;' .
						'}' .
						"jQuery('#kdnaform_wrapper_{$form_id}').replaceWith(confirmation_content);" .
						"{$scroll_position['confirmation']}" .
						"jQuery(document).trigger('kdnaform_confirmation_loaded', [{$form_id}]);" .
						"window['gf_submitting_{$form_id}'] = false;" .
						"wp.a11y.speak(jQuery('#kdnaform_confirmation_message_{$form_id}').text());" .
						'}' .
						'else{' .
						"jQuery('#kdnaform_{$form_id}').append(contents);" .
					...
					$form_scripts = KDNACommon::get_inline_script_tag( $form_scripts_body );

					if ( $ajax ) {
						$form_string .= "
		                <iframe style='{$iframe_style}' src='about:blank' name='kdnaform_ajax_frame_{$form_id}' id='kdnaform_ajax_frame_{$form_id}'" . $iframe_title . ">" . $iframe_content . "</iframe>
		                {$form_scripts}";
					} else {
	...
	public static function get_page( $form_id, $page_number, $field_values, $theme, $style_settings, $submission_method ) {
		...
		// Getting form and filtering it through kdnaform_pre_render.
		$form = KDNAAPI::get_form( $form_id );
		$form = self::kdnaform_pre_render( $form, 'form_display', false, $field_values );
		...
		if ( $is_first_page ) {
			...
			$markup .= "<{$tag} id='kdnaform_fields_{$form_id}' class='" . KDNACommon::get_ul_classes( $form ) . "'>"; // Opening field container for first page.
		}
	...
	public static function get_fields( $form, $field_values, $submitted_values, $page_number = 0 ) {
		...
		foreach ( $fields as $field ) {
			if ( $field instanceof KDNA_Field_Honeypot ) {
				if ( $has_honeypot ) {
					continue;
				} else {
					$has_honeypot = true;
			...
			if ( is_array( $submitted_values ) ) {
				...
				if ( $field->type === 'consent'
					&& ( $field_value[ $field->id . '.3' ] != KDNAFormsModel::get_latest_form_revisions_id( $form['id'] )
						|| $field_value[ $field->id . '.2' ] != $field->checkboxLabel ) ) {
					$field_value = KDNAFormsModel::get_field_value( $field, $field_values );
				}
			} else {
				$field_value = KDNAFormsModel::get_field_value( $field, $field_values );
			}
		...
		/** @var Honeypot\KDNA_Honeypot_Handler $honeypot_handler */
		...
		$field_value    = KDNAFormsModel::get_field_value( $honeypot_field );
	...
	public static function get_fields_by_page( $form, $page_number ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( $belongs_to_page || $is_page_footer || ( $field instanceof KDNA_Field_Honeypot && ! empty( $fields ) ) ) {
				$fields[] = $field;
			} elseif ( ! empty( $fields ) ) {
				break;
			}
	...
	public static function footer_init_scripts( $form_id ) {
		 ...
		 */
		$form_string = gf_apply_filters( array( 'kdnaform_footer_init_scripts_filter', $form_id ), $form_string, $form, $current_page );
		...
		if ( rgar( $button, 'type' ) == 'text' || rgar( $button, 'type' ) == 'link' || empty( $button['imageUrl'] ) ) {
			...
			if ( rgar( $button, 'type' ) == 'link' ) {
				if ( KDNACommon::is_legacy_markup_enabled( $form_id ) ) {
					...
					$tag    = 'button';
					$class .= KDNAFormDisplay::get_submit_button_class( $button, $form_id );
					$target = '';
	...
	public static function get_submit_button_class( $button, $form_id ) {
		$class  = ' button';
		$class .= rgar( $button, 'width' ) && 'full' == $button['width'] ? ' kdnaform-button--width-full' : '';
	...
	public static function kdnaform_footer( $form, $class, $ajax, $field_values, $previous_button, $display_title, $display_description, $tabindex = 1, $theme = null, $style_settings = null, $submission_method = self::SUBMISSION_METHOD_POSTBACK ) {
		...
		if ( rgar( $form['button'], 'location' ) && 'inline' == $form['button']['location'] ) {
			...
			$button_input = self::get_form_button( $form['id'], "kdnaform_submit_button_{$form['id']}", $button, __( 'Submit', 'kdnaforms' ), 'kdnaform_button', __( 'Submit', 'kdnaforms' ), 0 );
			$button_input = gf_apply_filters( array( 'kdnaform_submit_button', $form_id ), $button_input, $form );
		}

		$save_button = rgars( $form, 'save/enabled' ) ? self::get_form_button( $form_id, "kdnaform_save_{$form_id}_footer", $form['save']['button'], rgars( $form, 'save/button/text' ), 'kdnaform_save_link kdnaform-theme-button kdnaform-theme-button--secondary', rgars( $form, 'save/button/text' ), 0, "jQuery(\"#kdnaform_save_{$form_id}\").val(1);" ) : '';
		 ...
		 */
		$save_button = apply_filters( 'kdnaform_savecontinue_link', $save_button, $form );
		$save_button = apply_filters( "kdnaform_savecontinue_link_{$form_id}", $save_button, $form );
		...
		if ( $ajax ) {
			...
			$footer     .= "<input type='hidden' name='kdnaform_ajax' value='" . esc_attr( $ajax_value ) . "' />";
		}
		...
		if ( KDNACommon::has_multifile_fileupload_field( $form ) || ! empty( KDNAFormsModel::$uploaded_files[ $form_id ] ) ) {
			...
			$files_input = "<input type='hidden' name='kdnaform_uploaded_files' id='kdnaform_uploaded_files_{$form_id}' value='" . str_replace( "'", '&#039;', $files ) . "' />";
		}
		$save_inputs = '';
		if ( rgars( $form, 'save/enabled' ) ) {
			$resume_token = isset( $_POST['kdnaform_resume_token'] ) ? wp_unslash( $_POST['kdnaform_resume_token'] ) : rgget( 'gf_token' ); // phpcs:ignore WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			$resume_token = sanitize_key( $resume_token );
			$save_inputs  = "<input type='hidden' class='kdnaform_hidden' name='kdnaform_save' id='kdnaform_save_{$form_id}' value='' />
                             <input type='hidden' class='kdnaform_hidden' name='kdnaform_resume_token' id='kdnaform_resume_token_{$form_id}' value='{$resume_token}' />";
		}

		if ( KDNACommon::form_requires_login( $form ) ) {
			$footer .= wp_nonce_field( 'kdnaform_submit_' . $form_id, '_kdnaform_submit_nonce_' . $form_id, true, false );
		}

		$unique_id      = isset( self::$submission[ $form_id ] ) && rgar( self::$submission[ $form_id ], 'resuming_incomplete_submission' ) == true ? rgar( KDNAFormsModel::$unique_ids, $form_id ) : KDNAFormsModel::get_form_unique_id( $form_id );
		...
		if ( $honeypot_handler->is_speed_check_enabled( $form ) ) {
			$footer .= "<input type='hidden' class='kdnaform_hidden' name='kdnaform_submission_speeds' value='" . esc_attr( $honeypot_handler->get_submission_speeds_json( $form_id ) ) . "' />";
		}
		...
		$footer .= "
            <input type='hidden' class='kdnaform_hidden' name='kdnaform_submission_method' data-js='kdnaform_submission_method_{$form_id}' value='" . self::get_submission_method( $submission_method ) . "' />
            <input type='hidden' class='kdnaform_hidden' name='kdnaform_theme' data-js='kdnaform_theme_{$form_id}' id='kdnaform_theme_{$form_id}' value='" . esc_attr( $theme ) . "' />
            <input type='hidden' class='kdnaform_hidden' name='kdnaform_style_settings' data-js='kdnaform_style_settings_{$form_id}' id='kdnaform_style_settings_{$form_id}' value='" . $style_settings . "' />
            <input type='hidden' class='kdnaform_hidden' name='is_submit_{$form_id}' value='1' />
            <input type='hidden' class='kdnaform_hidden' name='kdnaform_submit' value='{$form_id}' />
            {$save_inputs}
            <input type='hidden' class='kdnaform_hidden' name='kdnaform_currency' data-currency='{$currency_code}' value='{$encrypted_currency}' />
            <input type='hidden' class='kdnaform_hidden' name='kdnaform_unique_id' value='" . esc_attr( $unique_id ) . "' />
            <input type='hidden' class='kdnaform_hidden' name='state_{$form_id}' value='" . self::get_state( $form, $field_values ) . "' />
            <input type='hidden' autocomplete='off' class='kdnaform_hidden' name='kdnaform_target_page_number_{$form_id}' id='kdnaform_target_page_number_{$form_id}' value='" . esc_attr( $next_page ) . "' />
            <input type='hidden' autocomplete='off' class='kdnaform_hidden' name='kdnaform_source_page_number_{$form_id}' id='kdnaform_source_page_number_{$form_id}' value='" . esc_attr( $current_page ) . "' />
            <input type='hidden' name='kdnaform_field_values' value='" . esc_attr( $field_values_str ) . "' />
            {$files_input}
        </div>";
	 ...
	 * @since 2.4.15 Updated to use KDNAFormsModel::get_next_field_id().
	 *
	 * @param array $form The current form object.
	 *
	 * @return int
	...
	public static function get_max_field_id( $form ) {
		if ( ! empty( $form['fields'] ) ) {
			$max = KDNAFormsModel::get_next_field_id( $form['fields'] ) - 1;
		} else {
			$max = 0;
		}
	...
	public static function handle_submission( &$form, &$lead, $ajax = false ) {
		...
		KDNA_Field_Password::stash_passwords( $form );

		//creating entry in DB
		KDNAFormsModel::save_lead( $form, $lead );

		$lead = KDNAFormsModel::set_entry_meta( $lead, $form );

		$is_spam = KDNACommon::is_spam_entry( $lead, $form );

		if ( $is_spam ) {

			// Marking entry as spam.
			KDNAFormsModel::update_entry_property( $lead['id'], 'status', 'spam', false, true );
			$lead['status'] = 'spam';
		...
		// Passwords are not saved to the database but should be available during the submission process.
		$lead = KDNA_Field_Password::hydrate_passwords( $lead );

		if ( has_action( 'kdnaform_entry_created' ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_entry_created.' );
			/**
			 ...
			 * @param array $form The Form object.
			 */
			do_action( 'kdnaform_entry_created', $lead, $form );
			KDNACommon::log_debug( __METHOD__ . '(): Completed kdnaform_entry_created.' );
		}
		...
		if ( ! $is_spam ) {
			...
			//send notifications
			KDNACommon::send_form_submission_notifications( $form, $lead );
		}
		...
		if ( rgars( $form, 'save/enabled' ) ) {
			KDNAFormsModel::delete_draft_submission( rgpost( 'kdnaform_resume_token' ) );
			KDNAFormsModel::purge_expired_draft_submissions();
		}

		if ( has_action( 'kdnaform_pre_handle_confirmation' ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_pre_handle_confirmation.' );
			/**
			 ...
			 */
			do_action( 'kdnaform_pre_handle_confirmation', $lead, $form );
			KDNACommon::log_debug( __METHOD__ . '(): Completed kdnaform_pre_handle_confirmation.' );
		}

		if ( has_filter( 'kdnaform_entry_pre_handle_confirmation' ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_entry_pre_handle_confirmation.' );
			/**
			 ...
			 * @param array $form The Form array.
			 */
			$lead = apply_filters( 'kdnaform_entry_pre_handle_confirmation', $lead, $form );
			KDNACommon::log_debug( __METHOD__ . '(): Completed kdnaform_entry_pre_handle_confirmation.' );
		}
	...
	private static function log_browser_session( $form_id, $entry_id ) {
		$is_logging_enabled = class_exists( 'KDNALogging' ) && KDNALogging::is_enabled( 'kdnaforms-browser' );
	...
	private static function create_spam_entry_note( $entry_id, $form_id ) {
		...
		$filter_name = ! rgempty( 'filter', $spam_filter ) ? $spam_filter['filter'] : __( 'Spam Filter', 'kdnaforms' );
		$note        = __( 'This entry has been flagged as spam.', 'kdnaforms' );
		if ( ! rgempty( 'reason', $spam_filter ) ) {
			...
			$note .= ' ' . sprintf( __( 'Reason: %s', 'kdnaforms' ), $spam_filter['reason'] );
		}

		KDNAAPI::add_note( $entry_id, 0, $filter_name, $note );
	}
	...
	public static function clean_up_files( $form, $is_submission = true ) {
		$tmp_location  = KDNAFormsModel::get_tmp_upload_location( $form['id'] );
		$target_path   = $tmp_location['path'];

		if ( $is_submission ) {
			$unique_form_id = rgpost( 'kdnaform_unique_id' );
			if ( ! ctype_alnum( $unique_form_id ) ) {
				return false;
			}
			$filename    = $unique_form_id . '_input_*';
			$files       = KDNACommon::glob( $filename, $target_path );
			if ( is_array( $files ) ) {
		...
		$files = KDNACommon::glob( '*', $target_path );
		if ( is_array( $files ) ) {
			 ...
			 * @param array $form            The form currently being processed.
			 */
			$expiration_days = apply_filters( 'kdnaform_temp_file_expiration_days', $expiration_days, $form );
			...
			if ( $save_enabled ) {
				 ...
				 */
				$expiration_days = apply_filters( 'kdnaform_incomplete_submissions_expiration_days', $expiration_days );
	 ...
	 *
	 * @since 2.1.1.11 Refactored to use KDNAFormDisplay::get_confirmation_message().
	 * @since 2.5      Updated to use KDNAFormDisplay::get_confirmation_url().
	 * @since 2.7      Updated the $form param to pass by reference.
	 *
	 * @param array $form     The Form Object.
	...
	public static function handle_confirmation( &$form, $entry, $ajax = false, $aux_data = array() ) {
		...
		KDNACommon::log_debug( sprintf( '%s(): Preparing confirmation (#%s - %s).', __METHOD__, rgar( $form['confirmation'], 'id' ), rgar( $form['confirmation'], 'name' ) ) );
		...
		if ( is_array( $confirmation ) && ! empty( $confirmation['redirect'] ) ) {
			 ...
			 */
			$suppress_redirect = apply_filters( 'kdnaform_suppress_confirmation_redirect', $suppress_redirect );
			...
			$confirmation = KDNACommon::kdnaform_do_shortcode( $confirmation );
		} else {
			$confirmation = null;
		}

		if ( empty( $confirmation ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Invalid confirmation; using default text instead.' );
			$form['confirmation'] = KDNAFormsModel::get_default_confirmation();
			$confirmation         = self::get_confirmation_message( $form['confirmation'], $form, $entry );
		}

		KDNACommon::log_debug( __METHOD__ . '(): Confirmation to be used => ' . print_r( $confirmation, true ) );
	...
	public static function get_confirmation_url( $confirmation, $form, $entry ) {
		if ( ! empty( $confirmation['pageId'] ) && $confirmation['type'] === 'page' ) {
			...
			if ( empty( $url ) ) {
				KDNACommon::log_debug( sprintf( '%s(): Selected page (%s) is invalid.', __METHOD__, $confirmation['pageId'] ) );
			...
			if ( ! empty( $url ) ) {
				$url = trim( KDNACommon::replace_variables( $url, $form, $entry, false, false, true, 'text' ) );
			}

			if ( empty( $url ) ) {
				KDNACommon::log_debug( __METHOD__ . '(): URL is empty.' );
		...
		$dynamic_query = KDNACommon::replace_variables( trim( $confirmation['queryString'] ), $form, $entry, true, false, false, 'text' );
	 ...
	 * @param  array $form         The Form Object.
	 * @param  array $entry        The Entry Object.
	 * @param  array $aux_data     Additional data to be passed to KDNACommon::replace_variables().
	 *
	 * @return string The confirmation message.
	 */
	public static function get_confirmation_message( $confirmation, $form, $entry, $aux_data = array() ) {
		$ajax   = isset( $_POST['kdnaform_ajax'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
		$anchor = self::get_anchor( $form, $ajax );
		$anchor = $anchor['tag'];
		...
		$message = empty( $confirmation['message'] ) ? "{$anchor} " : "{$anchor}<div id='kdnaform_confirmation_wrapper_{$form['id']}' class='kdnaform_confirmation_wrapper {$css_class}'><div id='kdnaform_confirmation_message_{$form['id']}' class='kdnaform_confirmation_message_{$form['id']} kdnaform_confirmation_message'>" . $message . '</div></div>';
	...
	private static function maybe_sanitize_confirmation_message( $confirmation_message ) {
		return KDNACommon::maybe_sanitize_confirmation_message( $confirmation_message );
	}
	...
	public static function send_emails( $form, $lead ) {
		_deprecated_function( 'send_emails', '1.7', 'KDNACommon::send_form_submission_notifications' );
		KDNACommon::send_form_submission_notifications( $form, $lead );
	}
	 ...
	 *
	 * @since unknown
	 * @since 2.4.19 Updated to use KDNAFormDisplay::is_field_validation_supported().
	 *
	 * @param array $form                   The form being processed.
	...
	public static function validate( &$form, $field_values, $page_number = 0, &$failed_validation_page = 0 ) {
		...
		KDNACommon::log_debug( __METHOD__ . '(): Checking restrictions.' );

		if ( empty( $_POST[ 'is_submit_' . $form_id ] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			KDNACommon::log_debug( __METHOD__ . "(): Aborting. The is_submit_{$form_id} input is empty." );
		...
		if ( kdna_upgrade()->get_submissions_block() ) {
			KDNACommon::log_debug( __METHOD__ . '(): Aborting. Database upgrade in progress.' );

			return false;
		}

		$is_last_page = self::get_target_page( $form, $page_number, $field_values ) === 0;
		if ( $is_last_page && KDNAFormsModel::is_submit_button_hidden( $form ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Aborting. Submit button is hidden/disabled by conditional logic.' );
			self::set_submission_if_null( $form_id, 'button_logic_error', __( 'Submit button is hidden/disabled by conditional logic.', 'kdnaforms' ) );
		...
		KDNACommon::log_debug( __METHOD__ . '(): Completed restrictions. Starting field validation.' );
		KDNACommon::timer_start( 'field-validation' );
		...
		foreach ( $form['fields'] as &$field ) {
			...
			if ( ! $field_in_other_page && $field->type === 'page' && KDNAFormsModel::is_next_button_hidden( $field, $form ) ) {
				KDNACommon::log_debug( __METHOD__ . sprintf( '(): Aborting. Next button is hidden/disabled by conditional logic. Field validation completed in %F seconds.', KDNACommon::timer_end( 'field-validation' ) ) );
				self::set_submission_if_null( $form_id, 'button_logic_error', __( 'Next button is hidden/disabled by conditional logic.', 'kdnaforms' ) );
			...
			if ( KDNAFormsModel::is_field_hidden( $form, $field, $field_values ) ) {
		...
		if ( $is_valid && $is_last_page && self::is_form_empty( $form ) ) {
			foreach ( $form['fields'] as &$field ) {
				...
				$field->validation_message = esc_html__( 'At least one field must be filled out', 'kdnaforms' );
				$is_valid                  = false;
		...
		KDNACommon::log_debug( __METHOD__ . '(): Executing functions hooked to kdnaform_validation.' );
	...
	public static function validate_field( $field, $form, $context ) {
		$value = KDNAFormsModel::get_field_value( $field );
		...
		if ( $field->isRequired && self::is_empty( $field, $form['id'] ) ) {
			 ...
			 * @param \KDNA_Field  $field   The field being checked for duplicates.
			 * @param int        $form_id The ID of the form being checked for duplicates.
			 */
			$value = apply_filters( 'kdnaform_value_pre_duplicate_check', $value, $field, $form['id'] );

			if ( KDNAFormsModel::is_duplicate( $form['id'], $field, $value ) ) {
				// Invalid when the value has been used by an existing entry and duplicate values aren't allowed.
				$field->failed_validation = true;

				switch ( $field->get_input_type() ) {
					case 'date' :
						$message = esc_html__( 'This date has already been taken. Please select a new date.', 'kdnaforms' );
						break;

					default:
						$message = is_array( $value ) ? esc_html__( 'This field requires a unique entry and the values you entered have already been used.', 'kdnaforms' ) :
							sprintf( esc_html__( "This field requires a unique entry and '%s' has already been used", 'kdnaforms' ), $value );
						break;
				}
				 ...
				 * @since 2.7   Moved from KDNAFormDisplay::validate().
				 *
				 * @since 1.5
				 */
			...
			$field->validation_message = in_array( $field->inputType, array(
				...
				'hiddenproduct',
				'consent',
			) ) ? esc_html__( 'Please enter a valid value.', 'kdnaforms' ) : esc_html__( 'Invalid selection. Please select from the available choices.', 'kdnaforms' );
	...
	public static function validate_character_encoding( $result, $value, $field ) {
		if ( KDNACommon::is_empty_array( $value ) || ! in_array( $field->get_input_type(), array( 'textarea', 'text', 'post_title', 'post_content', 'address', 'name' ) ) || ! rgar( $result, 'is_valid' ) ) {
			return $result;
		}

		$event = sprintf( '%d()', __METHOD__ );
		KDNACommon::timer_start( $event );
		KDNACommon::log_debug( __METHOD__ . "(): Starting invalid characters validation for field: {$field->label} ({$field->id} - {$field->type})" );
		...
		if ( is_null( $charset ) ) {
			$charset = $wpdb->get_col_charset( KDNAFormsModel::get_entry_meta_table_name(), 'meta_value' );
			KDNACommon::log_debug( __METHOD__ . '(): kdna_entry_meta meta_value charset = ' . print_r( $charset, true ) ); //phpcs:ignore
		}

		static $reflected = array();

		if ( empty( $reflected ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): reflecting methods' );
		...
		if ( $is_ascii ) {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): Completed in %F seconds. Value is valid ascii', KDNACommon::timer_end( $event ) ) );
		...
		foreach ( $values as $field_value ) {
			...
			if ( ! is_wp_error( $data_check ) && $data_check[0]['value'] != $field_value ) {
				...
				$result['message']  = esc_html__( 'The text entered contains invalid characters.', 'kdnaforms' );
				KDNACommon::log_debug( __METHOD__ . '(): Value to validate = ' . $log_value );
				KDNACommon::log_debug( __METHOD__ . '(): Value contains invalid characters. Cleaned value = ' . json_encode( $data_check[0]['value'] ) ); //phpcs:ignore
				break;
			}
		}

		KDNACommon::log_debug( __METHOD__ . sprintf( '(): Completed in %F seconds.', KDNACommon::timer_end( $event ) ) );
	 ...
	 *
	 * @param KDNA_Field $field           The field being processed.
	...
	public static function failed_state_validation( $form_id, $field, $value ) {
		...
		foreach ( $value as $key => $input_value ) {
			...
			if ( ( in_array( $field->inputType, array( 'singleproduct', 'hiddenproduct' ) ) && $key == $field->id . '.2' ) || $field->inputType == 'singleshipping' ) {
				$input_value = KDNACommon::to_number( $input_value );
			}
	 ...
	 *
	 * @since unknown
	 * @since 2.4.18 Added support for blocks and the kdnaform_post_enqueue_scripts hook.
	 */
			...
			if ( ! empty( $found_forms ) ) {
				foreach ( $found_forms as $form_id => $attributes ) {
					...
					if ( $form && $form['is_active'] && ! $form['is_trash'] ) {
						...
						$form['theme'] = ! rgempty( 'theme', $attributes ) ? $attributes['theme'] : KDNAForms::get_default_theme();
				 ...
				 */
				do_action( 'kdnaform_post_enqueue_scripts', $found_forms, $found_blocks, $post );
			}
		}
	}
	...
	public static function parse_forms_from_blocks( $blocks, &$found_forms, &$found_blocks ) {
		if ( ! method_exists( 'KDNA_Blocks', 'get_all_types' ) ) {
			return;
		}

		$block_types = KDNA_Blocks::get_all_types();
		...
		if ( preg_match_all( '/\[gravityform[s]? +.*?((id=.+?)|(name=.+?))\]/is', $post_content, $matches, PREG_SET_ORDER ) ) {
			foreach ( $matches as $match ) {
				...
				if ( $form_id && ! is_numeric( $form_id ) ) {
					$form_id = KDNAFormsModel::get_form_id( $form_id );
				}
	 ...
	 *
	 * @since unknown
	 * @since 2.4.18 Updated to use KDNAFormDisplay::parse_forms_from_shortcode().
	 *
	 * @param string $post_content The post content to be processed.
	...
	public static function get_embedded_forms( $post_content, &$ajax ) {
		...
		foreach ( $found_forms as $form_id => $is_ajax ) {
			$forms[] = KDNAAPI::get_form( $form_id );
			if ( ! $ajax && $is_ajax ) {
				$ajax = true;
			}
		}
	...
	public static function get_form_enqueue_assets( $form, $theme = null ) {
		...
		if ( ! KDNACommon::is_frontend_default_css_disabled() ) {
			...
			if ( KDNACommon::is_legacy_markup_enabled( $form ) ) {
				 ...
				 */
				$disable_legacy_css = apply_filters( 'kdnaform_disable_form_legacy_css', false );
			...
			if ( self::has_password_visibility( $form ) ) {
				$assets[] = new KDNA_Style_Asset( 'dashicons' );
			}
		}
		...
		if ( self::has_page_conditional_logic( $form ) ) {
			$assets[]  = new KDNA_Script_Asset( 'kdnaform_page_conditional_logic' );
			$has_logic = true;
		}
		...
		if ( self::has_datepicker_field( $form ) ) {
			$assets[] = new KDNA_Script_Asset( 'kdnaform_datepicker_init' );
		}

		if ( self::has_recaptcha_field( $form ) ) {
			$language = self::get_recaptcha_language( $form );
			$assets[] = new KDNA_Script_Asset( 'kdnaform_recaptcha', esc_url( sprintf( 'https://www.google.com/recaptcha/api.js?hl=%s&render=explicit', $language ) ) );
		}
		...
		if ( KDNACommon::has_multifile_fileupload_field( $form ) ) {
			$assets[] = new KDNA_Script_Asset( 'plupload-all' );
		}

		if ( self::has_enhanced_dropdown( $form ) || self::has_pages( $form ) ) {
			$assets[] = new KDNA_Script_Asset( 'kdnaform_json' );
		}

		if ( self::has_character_counter( $form ) ) {
			$assets[] = new KDNA_Script_Asset( 'kdnaform_textarea_counter' );
		}

		if ( self::has_input_mask( $form ) ) {
			$assets[] = new KDNA_Script_Asset( 'kdnaform_masked_input' );
		}

		if ( self::has_enhanced_dropdown( $form ) ) {
			if ( wp_script_is( 'chosen', 'registered' ) ) {
				$assets[] = new KDNA_Script_Asset( 'chosen' );
			} else {
				$assets[] = new KDNA_Script_Asset( 'kdnaform_chosen' );
			}
		}

		if ( self::has_placeholder( $form ) ) {
			$assets[] = new KDNA_Script_Asset( 'kdnaform_placeholder' );
		}

		// enqueue jQuery every time form is displayed to allow 'kdnaform_post_render' js hook
		// to be available to users even when GF is not using it
		$assets[] = new KDNA_Script_Asset( 'jquery' );
	...
	/**
	 * Add defer attribute to KDNA Forms scripts and any script dependent on a KDNA Forms script.
	 *
	 * @since 2.5
	 *
	...
	public static function add_script_defer( $tag, $handle ) {
		...
		if( strpos( $handle, 'kdnaform_' ) !== false && strpos( $tag, ' defer' ) === false ) {
	...
	public static function has_image_choices( $form ) {
		$has_image_choices = KDNAAPI::get_fields_by_type( $form, 'image_choice' ) ? true : false;
		 ...
		 * @param array $form The Current form object
		 */
		return apply_filters( 'kdnaform_has_image_choices', $has_image_choices, $form );
	}
	...
	public static function has_conditional_logic( $form ) {
		 ...
		 * @param array $form The Current form object
		 */
		return apply_filters( 'kdnaform_has_conditional_logic', $has_conditional_logic, $form );
	}
	...
	public static function has_page_conditional_logic( $form ) {
		if ( ! KDNACommon::form_has_fields( $form ) ) {
			return false;
		}
	 ...
	 *
	 * @todo: Replace much of the field value retrieval with a get_original_value() method in KDNA_Field class.
	 *
	 * @param       $form
	...
	private static function get_conditional_logic( $form, $field_values = array() ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			$section       = KDNAFormsModel::get_section( $form, $field->id );
			...
			if ( ! empty( $field_logic ) || ! empty( $next_button_logic ) ) {
				...
				$logics .= $field->id . ': ' . KDNACommon::json_encode( $field_section_logic ) . ',';

				$fields_with_logic[] = $field->id;

				$peers    = $field->type == 'section' ? KDNACommon::get_section_fields( $form, $field->id ) : array( $field );
				$peer_ids = array();
				...
				$dependents .= $field->id . ': ' . KDNACommon::json_encode( $peer_ids ) . ',';
			}
			...
			if ( $field->allowsPrepopulate ) {
				if ( $input_type == 'checkbox' || $input_type == 'multiselect' ) {
					$field_val = KDNAFormsModel::get_parameter_value( $field->inputName, $field_values, $field );
					...
					foreach ( $inputs as $input ) {
						$field_val[ $input['id'] ] = KDNAFormsModel::get_parameter_value( rgar( $input, 'name' ), $field_values, $field );
					}
				} elseif ( $input_type == 'time' ) { // maintained for backwards compatibility. The Time field now has an inputs array.
					$parameter_val = KDNAFormsModel::get_parameter_value( $field->inputName, $field_values, $field );
			...
			if ( is_array( $field->choices ) && $input_type != 'list' ) {
				...
				$is_pricing_field = KDNACommon::is_pricing_field( $field->type );
				...
				foreach ( $field->choices as $choice ) {
					...
					if ( $is_choice_selected ) {
						...
						if ( $input_type == 'select' ) {
							$price                        = KDNACommon::to_number( rgar( $choice, 'price' ) ) == false ? 0 : KDNACommon::to_number( rgar( $choice, 'price' ) );
		...
		if ( isset( $form['button']['conditionalLogic'] ) ) {
			$logics .= '0: ' . KDNACommon::json_encode( array( 'field' => $form['button']['conditionalLogic'], 'section' => null ) ) . ',';
			$dependents .= '0: ' . KDNACommon::json_encode( array( 0 ) ) . ',';
			$fields_with_logic[] = 0;
		}
		...
		$str = "if(window['jQuery']){" .
			...
			"jQuery('#kdnaform_wrapper_{$form['id']}').show();" .
			"jQuery('#kdnaform_wrapper_{$form['id']} form').css('opacity', '');" .
	...
	private static function get_page_conditional_logic( $form ) {
		...
		return sprintf( '; new KDNAPageConditionalLogic( %s );', json_encode( $args ) );
	}
	...
	public static function register_form_init_scripts( $form, $field_values = array(), $is_ajax = false ) {
		...
		if ( rgars( $form, 'save/enabled' ) ) {
			$save_script = "jQuery('#kdnaform_save_{$form['id']}').val('');";
        ...
        /**
         * Fires when inline KDNA Forms scripts are enqueued
         *
	...
	public static function get_form_init_scripts( $form ) {
		...
		if ( ! empty( $init_scripts ) ) {
			...
			$script_body .=
				...
				"jQuery(document).on('kdnaform_post_conditional_logic', function(event, formId, fields, isInit){";
			foreach ( $init_scripts as $init_script ) {
	...
	public static function get_counter_init_script( $form ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( ! empty( $max_length ) && ! $field->is_administrative() ) {
				...
				$tinymce_style = $rte_enabled ? ' kdnainput_counter_tinymce' : '';
				$error_style   = $rte_enabled ? ' kdnainput_counter_error' : '';

				$field_script =
					"if(!jQuery('#{$input_id}+.kdnainput_counter').length){jQuery('#{$input_id}').textareaCount(" .
					"    {'maxCharacterSize': {$max_length}," .
					"    'originalStyle': 'kdnainput_counter kdnafield_description{$tinymce_style}'," .
					"	 'truncate': {$truncate}," .
					"	 'errorStyle' : '{$error_style}'," .
					"    'displayFormat' : '#input " . esc_js( __( 'of', 'kdnaforms' ) ) . ' #max ' . esc_js( __( 'max characters', 'kdnaforms' ) ) . "'" .
					"    });" . "jQuery('#{$input_id}').next('.kdnainput_counter').attr('aria-live','polite');}";
	...
	public static function get_number_formats_script( $form ) {
		...
		$currency       = RGCurrency::get_currency( KDNACommon::get_currency() );
		...
		foreach ( $form['fields'] as $field ) {
			...
			switch ( KDNAFormsModel::get_input_type( $field ) ) {
				case 'number':
	...
	private static function has_datepicker_field( $form ) {
		if ( is_array( $form['fields'] ) ) {
			foreach ( $form['fields'] as $field ) {
				...
				if ( KDNAFormsModel::get_input_type( $field ) == 'date' && $field->dateType == 'datepicker' ) {
					return true;
				}
			}
		}
	 ...
	 * @since Unknown
	 *
	 * @uses KDNACommon::is_product_field()
	 *
	 * @param array $form The current forms properties.
	 *
	 * @return bool
	...
	private static function has_price_field( $form ) {
		if ( is_array( $form['fields'] ) ) {
			foreach ( $form['fields'] as $field ) {
				if ( KDNACommon::is_product_field( $field->type ) ) {
					return true;
				}
			}
		}
	 ...
	 *
	 * @param KDNA_Field $field The field to be checked.
	 *
	 * @return bool
	 */
				...
				foreach ( $field->conditionalLogic['rules'] as $rule ) {
					if ( intval( $rule['fieldId'] ) == $fieldId ) {
						...
						if ( $field->type == 'section' ) {
							$section_fields = KDNACommon::get_section_fields( $form, $field->id );
	 ...
	 *
	 * @param KDNA_Field $field                The field object.
	...
	public static function get_field( $field, $value = '', $force_frontend_label = false, $form = null, $field_values = null, $page_number = 0 ) {
		...
		if ( $field->type == 'page' ) {
			if ( $is_entry_detail ) {
				...
				$previous_button = $field->pageNumber == 2 ? '' : self::get_form_button( $form_id, "kdnaform_previous_button_{$form_id}_{$field->id}", $field->previousButton, __( 'Previous', 'kdnaforms' ), 'kdnaform_previous_button kdnaform-theme-button kdnaform-theme-button--secondary', $previous_button_alt, $field->pageNumber - 2 );
				if ( ! empty( $previous_button ) ) {
					$previous_button = gf_apply_filters( array( 'kdnaform_previous_button', $form_id ), $previous_button, $form );
				}

				$next_button_alt = rgempty( 'imageAlt', $field->nextButton ) ? __( 'Next Page', 'kdnaforms' ) : $field->nextButton['imageAlt'];
				$next_button     = self::get_form_button( $form_id, "kdnaform_next_button_{$form_id}_{$field->id}", $field->nextButton, __( 'Next', 'kdnaforms' ), 'kdnaform_next_button kdnaform-theme-button', $next_button_alt, $field->pageNumber );
				$next_button     = gf_apply_filters( array( 'kdnaform_next_button', $form_id ), $next_button, $form );

				$save_button = rgars( $form, 'save/enabled' ) ? self::get_form_button( $form_id, "kdnaform_save_{$form_id}_{$field->pageNumber}", $form['save']['button'], rgars( $form, 'save/button/text' ), 'kdnaform_save_link kdnaform-theme-button kdnaform-theme-button--secondary', rgars( $form, 'save/button/text' ), 0, "jQuery(\"#kdnaform_save_{$form_id}\").val(1);" ) : '';
				...
				$page_footer_html = "</{$tag}>
                    ...
                    <div class='kdnaform-page-footer kdnaform_page_footer {$label_placement}'>
                        {$previous_button} {$next_button} {$save_button}
                    </div>
                </div>";

                $page_header_html = "
                <div id='kdnaform_page_{$form['id']}_{$field->pageNumber}' class='kdnaform_page{$custom_class}' data-js='page-field-id-{$field->id}' {$style}>
                    <div class='kdnaform_page_fields'>
                        <{$tag} id='kdnaform_fields_{$form['id']}_{$field->pageNumber}' class='" . KDNACommon::get_ul_classes( $form ) . "'>";
		...
		$error_class        = $field->failed_validation ? 'kdnafield_error' : '';
		...
		$visibility_class   = $is_admin ? 'kdnafield_visibility_visible' : sprintf( 'kdnafield_visibility_%s', ( $field->visibility ? $field->visibility : 'visible' ) );
		$selectable_class   = $is_admin ? 'selectable' : '';
		$hidden_class       = in_array( $input_type, array( 'hidden', 'hiddenproduct' ) ) ? 'kdnaform_hidden' : '';

		$choice_fields                  = array( 'checkbox', 'radio', 'consent' );
		$choice_input_type_class        = in_array( $field->type, $choice_fields ) || ( isset( $field->inputType ) && in_array( $field->inputType, $choice_fields ) ) ? 'kdnafield--type-choice' : '';
		$choice_alignment_class         = $field->type === 'multi_choice' ? 'kdnafield--choice-align-' . KDNA_Field_Multiple_Choice::get_field_choice_alignment( $field ) : '';
		$choice_input_image_shape_class = '';
		$choice_input_image_style_class = '';
		if ( $field->type === 'image_choice' ) {
			$choice_input_image_shape_class = isset( $form['styles'] ) && rgar( $form['styles'], 'inputImageChoiceAppearance' ) ? 'kdnafield--image-choice-appearance-' . $form['styles']['inputImageChoiceAppearance'] : 'kdnafield--image-choice-appearance-card';
			$choice_input_image_style_class = isset( $form['styles'] ) && rgar( $form['styles'], 'inputImageChoiceStyle' ) ? 'kdnafield--image-choice-style-' . $form['styles']['inputImageChoiceStyle'] : 'kdnafield--image-choice-style-square';
		}

		$field_input_type_class  = isset( $field->inputType ) && ! empty( $field->inputType ) ? sprintf( 'kdnafield--input-type-%s', $field->inputType ) : '';

		$field_specific_class = $field->get_field_css_class();

		$section_class              = $field->type == 'section' ? 'gsection' : '';
		$page_class                 = $field->type == 'page' ? 'gpage kdnaform-theme__disable' : '';
		$html_block_class           = $field->type == 'html' ? 'kdnafield_html' : '';
		$html_formatted_class       = $field->type == 'html' && ! $field->disableMargins ? 'kdnafield_html_formatted' : '';
		$html_no_follows_desc_class = $field->type == 'html' && ! $is_admin && ! self::prev_field_has_description( $form, $field->id ) ? 'kdnafield_no_follows_desc' : '';

		$calculation_class = $input_type == 'calculation' || ( $input_type == 'number' && $field->has_calculation() )  ? 'kdnafield_calculation' : '';

		$product_suffix            = "_{$form_id}_" . $field->productField;
		$option_class             = $field->type == 'option' ? "kdnafield_price kdnafield_price{$product_suffix} kdnafield_option{$product_suffix}" : '';
		$quantity_class           = $field->type == 'quantity' ? "kdnafield_price kdnafield_price{$product_suffix} kdnafield_quantity kdnafield_quantity{$product_suffix}" : '';
        $total_class              = $field->type == 'total' ? "kdnafield_price kdnafield_price{$product_suffix} kdnafield_total kdnafield_total{$product_suffix}" : '';
		$shipping_class           = $field->type == 'shipping' ? "kdnafield_price kdnafield_shipping kdnafield_shipping_{$form_id}" : '';
		$product_class            = $field->type == 'product' ? "kdnafield_price kdnafield_price_{$form_id}_{$field->id} kdnafield_product_{$form_id}_{$field->id}" : '';
		$hidden_product_class     = $input_type == 'hiddenproduct' ? 'kdnafield_hidden_product' : '';
		$donation_class           = $field->type == 'donation' ? "kdnafield_price kdnafield_price_{$form_id}_{$field->id} kdnafield_donation_{$form_id}_{$field->id}" : '';
		$required_class           = $field->isRequired ? 'kdnafield_contains_required' : '';
		$creditcard_warning_class = $input_type == 'creditcard' && ! KDNACommon::is_ssl() ? 'kdnafield_creditcard_warning' : '';
		...
		$has_description_class    = ! empty( $field->description ) ? 'kdnafield--has-description' : 'kdnafield--no-description';
		 ...
		 * @param KDNA_Field $field           The Field currently being processed.
		 * @param array    $form            The Form currently being processed.
	...
	private static function get_field_column_display( $field ) {
		...
		$column_class = '';
		$field_type   = KDNAFormsModel::get_input_type( $field );
	...
	public static function get_field_content( $field, $value = '', $force_frontend_label = false, $form_id = 0, $form = null ) {
		...
		if ( $input_type == 'adminonly_hidden' ) {
			$field_content = ! $is_admin ? '{FIELD}' : sprintf( "%s<label class='kdnafield_label kdnaform-field-label' >%s</label>{FIELD}", $admin_buttons, esc_html( $field_label ) );
		} else {
		...
		$admin_compact_view_menu = $is_form_editor ? sprintf( "<div id='dropdown_field_%s' data-js='kdnaform-compact-view-overflow-menu' class='kdnaform-compact-view-overflow-menu kdnaform-theme__disable'></div>", $field->id ) : '';

		if( $is_form_editor ) {
			$field_content = '<div class="kdnafield-admin-wrapper">' . $field_content . '</div>' . ( $field->type !== 'submit' ? $admin_compact_view_menu : '' );
		}
	...
	public static function get_progress_bar( $form, $page, $confirmation_message = '' ) {
		...
		//check for filter
		$start_at_zero          = apply_filters( 'kdnaform_progressbar_start_at_zero', $start_at_zero, $form );
		...
		if ( $progress_complete ) {
			...
			$progress_bar = "<div class='{$wrapper_css_class}' id='kdnaform_wrapper_$form_id' >";
			$page_name    = ! empty( $form['pagination']['progressbar_completion_text'] ) ? $form['pagination']['progressbar_completion_text'] : '';
		}

		$progress_bar_title_open  = KDNACommon::is_legacy_markup_enabled( $form ) ? '<h3 class="gf_progressbar_title">' : '<p class="gf_progressbar_title">';
		$progress_bar_title_close = KDNACommon::is_legacy_markup_enabled( $form ) ? '</h3>' : '</p>';
		...
		$progress_bar .= ! $progress_complete ? esc_html__( 'Step', 'kdnaforms' ) . " <span class='gf_step_current_page'>{$current_page}</span> " . esc_html__( 'of', 'kdnaforms' ) . " <span class='gf_step_page_count'>{$page_count}</span><span class='gf_step_page_name'>{$page_name}</span>" : "{$page_name}";
		$progress_bar .= "
		 ...
		 *
		 * @see   https://docs.kdnaforms.com/kdnaform_progress_bar/
		 */
		$progress_bar = apply_filters( 'kdnaform_progress_bar', $progress_bar, $form, $confirmation_message );
		$progress_bar = apply_filters( "kdnaform_progress_bar_{$form_id}", $progress_bar, $form, $confirmation_message );
	...
	public static function get_progress_steps( $form, $page ) {
		...
		for ( $i = 0, $count = sizeof( $pages ); $i < $count; $i ++ ) {
			...
			$classes = KDNACommon::trim_all( $classes );
		 ...
		 *
		 * @see   https://docs.kdnaforms.com/kdnaform_progress_steps/
		 */
		$progress_steps = apply_filters( 'kdnaform_progress_steps', $progress_steps, $form, $page );
		$progress_steps = apply_filters( "kdnaform_progress_steps_{$form['id']}", $progress_steps, $form, $page );
	...
	public static function validate_entry_limit( $form ) {
		...
		$entry_count = KDNAAPI::count_entries( $form_id, $search_criteria );
		$limit       = rgar( $form, 'limitEntriesCount' );

		if ( $entry_count >= $limit ) {
			$error = empty( $form['limitEntriesMessage'] ) ? "<div class='gf_submission_limit_message'><p>" . esc_html__( 'Sorry. This form is no longer accepting new submissions.', 'kdnaforms' ) . '</p></div>' : '<p>' . KDNACommon::kdnaform_do_shortcode( $form['limitEntriesMessage'] ) . '</p>';
			self::set_submission_if_null( $form_id, 'form_restriction_error', $error );
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): Form (#%d) entry limit reached. Limit: %d; Count: %d.', $form_id, $limit, $entry_count ) );
	...
	public static function validate_form_schedule( $form ) {
		...
		if ( rgar( $form, 'scheduleForm' ) ) {
			...
			if ( ! empty( $form['scheduleStart'] ) && $now < $timestamp_start ) {
				$error = empty( $form['schedulePendingMessage'] ) ? '<p>' . esc_html__( 'This form is not yet available.', 'kdnaforms' ) . '</p>' : '<p>' . KDNACommon::kdnaform_do_shortcode( $form['schedulePendingMessage'] ) . '</p>';
				self::set_submission_if_null( $form['id'], 'form_restriction_error', $error );
				KDNACommon::log_debug( __METHOD__ . sprintf( '(): The form (#%d) is not yet available. Scheduled for: %d; Now: %d.', rgar( $form, 'id' ), $timestamp_start, $now ) );

				return $error;
			} elseif ( ! empty( $form['scheduleEnd'] ) && $now > $timestamp_end ) {
				$error = empty( $form['scheduleMessage'] ) ? '<p>' . esc_html__( 'Sorry. This form is no longer available.', 'kdnaforms' ) . '</p>' : '<p>' . KDNACommon::kdnaform_do_shortcode( $form['scheduleMessage'] ) . '</p>';
				self::set_submission_if_null( $form['id'], 'form_restriction_error', $error );
				KDNACommon::log_debug( __METHOD__ . sprintf( '(): The form (#%d) is no longer available. Ended: %d; Now: %d.', rgar( $form, 'id' ), $timestamp_end, $now ) );
	...
	public static function update_confirmation( $form, $entry = null, $event = '' ) {
		...
		// this is here mostly to avoid the semi-costly KDNAFormsModel::create_lead() function unless we really need it
		...
		KDNACommon::log_debug( __METHOD__ . '(): Evaluating conditional logic.' );
		...
		foreach ( $confirmations as $confirmation ) {
			...
			if ( isset( $confirmation['isActive'] ) && ! $confirmation['isActive'] ) {
				KDNACommon::log_debug( __METHOD__ . sprintf( '(): Confirmation (#%s - %s) is inactive.', rgar( $confirmation, 'id' ), rgar( $confirmation, 'name' ) ) );
				continue;
			}

			$logic = rgar( $confirmation, 'conditionalLogic' );
			if ( KDNACommon::evaluate_conditional_logic( $logic, $form, $entry ) ) {
				KDNACommon::log_debug( __METHOD__ . sprintf( '(): Confirmation (#%s - %s) conditional logic matches.', rgar( $confirmation, 'id' ), rgar( $confirmation, 'name' ) ) );
				$form['confirmation'] = $confirmation;

				return $form;
			}

			KDNACommon::log_debug( __METHOD__ . sprintf( '(): Confirmation (#%s - %s) conditional logic not met.', rgar( $confirmation, 'id' ), rgar( $confirmation, 'name' ) ) );
		}

		KDNACommon::log_debug( __METHOD__ . '(): No conditional logic match found; using default.' );
	...
	public static function process_send_resume_link() {
		...
		if ( KDNACommon::form_requires_login( $form ) ) {
			...
			check_admin_referer( 'kdnaform_send_resume_link', '_kdnaform_send_resume_link_nonce' );
		}

		$draft_submission = KDNAFormsModel::get_draft_submission_values( $resume_token );

		$submission = json_decode( $draft_submission['submission'], true );

		$partial_entry = $submission['partial_entry'];

		$notifications_to_send = KDNACommon::get_notifications_to_send( 'form_save_email_requested', $form, $partial_entry );

		$log_notification_event = empty( $notifications_to_send ) ? 'No notifications to process' : 'Processing notifications';
		KDNACommon::log_debug( "KDNAFormDisplay::process_send_resume_link(): {$log_notification_event} for form_save_email_requested event." );

		foreach ( $notifications_to_send as $notification ) {
			if ( isset( $notification['isActive'] ) && ! $notification['isActive'] ) {
				KDNACommon::log_debug( "KDNAFormDisplay::process_send_resume_link(): Notification is inactive, not processing notification (#{$notification['id']} - {$notification['name']})." );
				continue;
			}
			...
			KDNACommon::send_notification( $notification, $form, $partial_entry );
		}

		KDNAFormsModel::add_email_to_draft_sumbmission( $resume_token, $email );
	}

	public static function replace_save_variables( $text, $form, $resume_token, $email = null ) {
		$resume_token = sanitize_key( $resume_token );
		$form_id      = intval( $form['id'] );
		$page_url     = rgpost( 'current_page_url' ) ? sanitize_url( rawurldecode( rgpost( 'current_page_url' ) ) ): KDNAFormsModel::get_current_page_url();
		/**
		...
		$resume_submit_button_text       = esc_html__( 'Send Link', 'kdnaforms' );
		$resume_email_validation_message = esc_html__( 'Please enter a valid email address.', 'kdnaforms' );
		$email_input_label               = esc_html__( 'Email Address', 'kdnaforms' );
		$email_input_label_required      = KDNAFormsModel::get_required_indicator( $form_id );
		...
		$validation_output = $form_is_invalid ? sprintf( '<div class="kdnafield_description kdnafield_validation_message" id="email-validation-error" aria-live="assertive">%s</div>', $resume_email_validation_message ) : '';
		...
		if ( $is_iframe_ajax ) {
			...
			$iframe_ajax_fields = "<input type='hidden' name='kdnaform_ajax' value='" . esc_attr( $ajax_value ) . "' />";
			$iframe_ajax_fields .= "<input type='hidden' name='kdnaform_field_values' value='' />";
		}

		$form_submission_inputs = "<input type='hidden' class='kdnaform_hidden' name='kdnaform_submission_method' data-js='kdnaform_submission_method_{$form_id}' value='{$submission_method}' />
								   <input type='hidden' class='kdnaform_hidden' name='is_submit_{$form_id}' value='1' />
								   <input type='hidden' class='kdnaform_hidden' name='kdnaform_submit' value='{$form_id}' />";

		$ajax_submit = $is_iframe_ajax ? "onclick='jQuery(\"#kdnaform_{$form_id}\").trigger(\"submit\",[true]);'" : '';

		if ( KDNACommon::is_legacy_markup_enabled( $form ) ) {
			$resume_form = "<div class='form_saved_message_emailform'>
							<form action='{$action}' method='POST' id='kdnaform_{$form_id}' data-formid='{$form_id}' {$target}>
								{$iframe_ajax_fields}
								<label for='kdnaform_resume_email' class='kdnaform_resume_email_label kdnafield_label' aria-describedby='email-validation-error'>{$email_input_label}</label>
								<input type='email' name='kdnaform_resume_email' value='{$email_esc}' id='kdnaform_resume_email' placeholder='{$email_input_label}' aria-describedby='email-validation-error'/>
								<input type='hidden' name='kdnaform_resume_token' value='{$resume_token}' />
								<input type='hidden' name='kdnaform_send_resume_link' value='{$form_id}' />
								{$form_submission_inputs}
	                            <input type='submit' name='kdnaform_send_resume_link_button' id='kdnaform_send_resume_link_button_{$form_id}' onclick='gform.submission.handleButtonClick(this);' data-submission-type='send-link' value='{$resume_submit_button_text}' {$ajax_submit}/>
	                            {$validation_output}
	                            {$nonce_input}
							</form>
	                    </div>";
			...
			$resume_form = "<div class='form_saved_message_emailform'>
						<form action='{$action}' method='POST' id='kdnaform_{$form_id}' data-formid='{$form_id}' {$target}>
							<div class='kdnaform-body kdnaform_body'>
								<div id='kdnaform_fields_{$form_id}' class='kdnaform_fields top_label form_sublabel_below description_below'>
									{$iframe_ajax_fields}
									<div class='kdnafield kdnafield--type-email kdnafield--width-full field_sublabel_below field_description_below kdnafield_visibility_visible'>
										<label for='kdnaform_resume_email' class='kdnaform_resume_email_label kdnafield_label kdnaform-field-label'>{$email_input_label}{$email_input_label_required}</label>
										<div class='kdnainput_container kdnainput_container_text'>
											<input type='email' name='kdnaform_resume_email' class='large' id='kdnaform_resume_email' value='{$email_esc}' aria-describedby='email-validation-error' />
											{$validation_output}
										</div>
									</div>
								</div>
							</div>
							<div class='kdnaform-footer kdnaform_footer top_label'>
								<input type='hidden' name='kdnaform_resume_token' value='{$resume_token}' />
								<input type='hidden' name='kdnaform_send_resume_link' value='{$form_id}' />
								{$form_submission_inputs}
								<input type='submit' name='kdnaform_send_resume_link_button' id='kdnaform_send_resume_link_button_{$form_id}' onclick='gform.submission.handleButtonClick(this);' data-submission-type='send-link' value='{$resume_submit_button_text}' {$ajax_submit}/>
	...
	public static function handle_save_email_confirmation( $form, $ajax ) {
		...
		$resume_token       = rgpost( 'kdnaform_resume_token' ); // phpcs:ignore WordPress.Security.NonceVerification.Missing,
		...
		$css_class          = esc_attr( rgar( $form, 'cssClass' ) );
		$form_theme         = "data-form-theme='" . KDNAFormDisplay::get_form_theme_slug( $form ) . "'";

		$confirmation_message = rgar( $form['confirmation'], 'message' );

		$confirmation            = "<div id='kdnaform_confirmation_wrapper_{$form['id']}' class='form_saved_message_sent kdnaform_confirmation_wrapper {$css_class} kdnaform_wrapper' role='alert' {$form_theme}>{$confirmation_message}</div>";
		$nl2br                   = rgar( $form['confirmation'], 'disableAutoformat' ) ? false : true;
		...
		$save_email_confirmation = KDNACommon::kdnaform_do_shortcode( $save_email_confirmation );
		...
		KDNACommon::log_debug( 'KDNAFormDisplay::handle_save_email_confirmation(): Confirmation => ' . print_r( $save_email_confirmation, true ) );
	...
	public static function handle_save_confirmation( $form, $resume_token, $confirmation_message, $ajax ) {
		$resume_email = isset( $_POST['kdnaform_resume_email'] ) ? sanitize_email( wp_unslash( $_POST['kdnaform_resume_email'] ) ) : null; // phpcs:ignore WordPress.Security.NonceVerification.Missing
		...
		$confirmation_message = KDNACommon::kdnaform_do_shortcode( $confirmation_message );
		...
		$wrapper_open = "<div class='{$wrapper_css_class}' {$page_instance} {$form_theme} id='kdnaform_wrapper_{$form_id}'>";
		...
		KDNACommon::log_debug( 'KDNAFormDisplay::handle_save_confirmation(): Confirmation => ' . print_r( $confirmation_message, true ) );
	...
	public static function insert_review_page( $form, $review_page ) {
		...
		$review_page_break             = new KDNA_Field_Page();
		...
		$review_page_break->cssClass   = 'kdnaform_review_page ' . rgar( $review_page, 'cssClass', '' );

		/* Add review page break field to form. */
		$form['fields'][] = $review_page_break;

		/* Create new HTML field for review page. */
		$review_page_field             = new KDNA_Field_HTML();
	 ...
	 * @since 2.4.18
	 *
	 * @param null|int $ajax_form_id Null or the form ID parsed from the kdnaform_ajax input value.
	 *
	...
	public static function is_submit_form_id_valid( $ajax_form_id = null ) {
		if ( empty( $_POST['kdnaform_submit'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			return false;
		}

		$form_id = absint( $_POST['kdnaform_submit'] ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	/**
	 * Returns the safe value of/for the kdnaform_submission_method input.
	 *
	 * @since 2.9.2
	 *
	...
	/**
	 * Parses and sanitizes the value of the kdnaform_ajax input.
	 *
	 * @since 2.9.2
	 *
	...
	public static function parse_ajax_input( $bypass_cache = false ) {
		...
		$args = false;
		if ( ! self::is_iframe_submission_method() || ! isset( $_POST['kdnaform_ajax'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			return false;
		}
		...
		if ( empty( $args['form_id'] ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): form_id arg is missing or empty.' );
			$args = array();
		...
		if ( empty( $args['hash'] ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): hash arg is missing or empty.' );
			$args = array();
		...
		$args['styles']      = isset( $args['styles'] ) ? KDNACommon::strip_all_tags_from_json_string( $args['styles'] ) : null;
		...
		if ( $args['hash'] !== $expected_hash ) {
			KDNACommon::log_debug( __METHOD__ . '(): args failed hash validation.' );
			$args = array();
	...
	/**
	 * Prepares the value for the kdnaform_ajax input.
	 *
	 * @since 2.9.2
	 *
	 * @param int         $form_id             The form ID.
	...
	public static function get_ajax_postback_html( $body_content ) {
		$ajax_iframe_content = "<!DOCTYPE html><html><head><meta charset='UTF-8' /></head><body class='KDNA_AJAX_POSTBACK'>" . $body_content . '</body></html>';
		 ...
		 */
		return apply_filters( 'kdnaform_ajax_iframe_content', $ajax_iframe_content );
	}
	...
	public static function get_form_not_found_html( $form_id, $ajax = false ) {
		$form_not_found_message = '<p class="kdnaform_not_found">' . esc_html__( 'Oops! We could not locate your form.', 'kdnaforms' ) . '</p>';
		 ...
		 * @param int|string $form_id                The ID or Title of the form requested for display.
		 */
		$form_not_found_message = apply_filters( 'kdnaform_form_not_found_message', $form_not_found_message, $form_id );
	...
	public static function get_validation_errors_markup( $form, $values, $show_summary = false ) {
		...
		if ( $button_logic_error ) {
			$validation_message_markup = "<h2 class='kdnaform_submission_error{$hide_summary_class}'><span class='kdnaform-icon kdnaform-icon--circle-error'></span>" . esc_html__( 'There was a problem with your submission.', 'kdnaforms' ) . ' ' . esc_html( $button_logic_error ) . '</h2>';
		} elseif ( kdna_upgrade()->get_submissions_block() ) {
			$validation_message_markup = "<h2 class='gf_submission_limit_message'>" . esc_html__( 'Your form was not submitted. Please try again in a few minutes.', 'kdnaforms' ) . '</h2>';
		} else {
			$validation_message_markup = "<h2 class='kdnaform_submission_error{$hide_summary_class}'><span class='kdnaform-icon kdnaform-icon--circle-error'></span>" . esc_html__( 'There was a problem with your submission.', 'kdnaforms' ) . ' ' . esc_html__( 'Please review the fields below.', 'kdnaforms' ) . '</h2>';
			...
			if ( $show_summary ) {
				...
				foreach ( $errors as $error ) {
					...
					$error_messages_list .= '<li><a class="kdnaform_validation_error_link" href="' . $error['field_selector'] . '">' . $error['field_label'] . $separator . $error['message'] . '</a></li>';
				}
				$error_messages_list .= '</ol>';
			}
		}
		...
		$wrapper_class = KDNACommon::is_legacy_markup_enabled( $form ) ? 'kdnaform_validation_errors validation_error' : 'kdnaform_validation_errors';

		$validation_errors_markup = sprintf(
			'<div class="%s" id="%s" data-js="kdnaform-focus-validation-error" autofocus>%s%s</div>',
			$wrapper_class,
	...
	public static function convert_legacy_class( $form, $classes ) {
		...
		$upgraded_classes = array(
			'gf_left_half'      => 'kdnafield--width-half',
			'gf_right_half'     => 'kdnafield--width-half',
			'gf_left_third'     => 'kdnafield--width-third',
			'gf_middle_third'   => 'kdnafield--width-third',
			'gf_right_third'    => 'kdnafield--width-third',
			'gf_first_quarter'  => 'kdnafield--width-quarter',
			'gf_second_quarter' => 'kdnafield--width-quarter',
			'gf_third_quarter'  => 'kdnafield--width-quarter',
			'gf_fourth_quarter' => 'kdnafield--width-quarter',
		);
	...
	/**
	 * Parse and validates styles from the kdnaform_default_styles filter.
	 *
	 * @since 2.7.15
	 *
	...
	public static function get_form_styles( $style_settings ) {
		$global_styles = apply_filters( 'kdnaform_default_styles', false );

		$form_styles = '';

		if ( $style_settings === false ) {
			// if $style_settings is false, ignore the kdnaform_default_styles filter.
			return false;
		} else if ( ! empty( $style_settings ) ) {
			// if we have style settings, merge them with the kdnaform_default_styles filter.
	...
	private static function set_form_styles( $form, $style_settings, $form_theme ) {
		...
		$form['theme'] = ! empty( $form_theme ) ? $form_theme : KDNAForms::get_default_theme();
		return $form;
	}
	...
	public static function get_row_spacer( $field, $form ) {
		...
		if ( $field->layoutSpacerGridColumnSpan && ! KDNACommon::is_legacy_markup_enabled( $form ) ) {
			...
			if ( $span < 12 ) {
				$spacer = sprintf( '<div data-fieldId="%s" class="spacer kdnafield" style="grid-column: span %d;" data-groupId="%s"></div>', $field->id, $field->layoutSpacerGridColumnSpan, $field->layoutGroupId );
			}
		}
	...
	/**
	 * @var array Cached forms that have been filtered by the kdnaform_pre_render filter.
	 */
	private static $cached_forms = array();

	/**
	 * Filters the $form object through the kdnaform_pre_render filter and caches the result so that this filter is only triggered once per request.
	 *
	 * @since 2.9.0
	 *
	 ...
	 *
	 * @return array Returns the form object after being filtered by the kdnaform_pre_render filter.
	 */
	public static function kdnaform_pre_render( $form, $context, $ajax = null, $field_values = null ) {
	...
	/**
	 * Flushes the forms cached by the kdnaform_pre_render method.
	 *
	 * @since 2.9.0
	 *
	...
	public static function get_confirmation_markup( $form, $confirmation_message, $ajax, $style_settings = false, $form_theme = null ) {
		...
		$pagination_type   = rgars( $form, 'pagination/type' );
		$is_admin          = KDNACommon::is_form_editor() || KDNACommon::is_entry_detail();
		$has_pages         = self::has_pages( $form );
	...
	public static function get_last_page_footer( $form, $label_placement, $ajax, $field_values, $display_title, $display_description, $tabindex, $form_theme, $style_settings, $submission_method ) {
		...
		$previous_button = self::get_form_button( $form['id'], "kdnaform_previous_button_{$form['id']}", $last_page_button, __('Previous', 'kdnaforms'), 'kdnaform_previous_button kdnaform-theme-button kdnaform-theme-button--secondary', $previous_button_alt, self::get_current_page( $form['id'] ) - 1);
		 ...
		 * @param array $form The Form object to filter through
		 */
		$previous_button = gf_apply_filters( array( 'kdnaform_previous_button', $form['id'] ), $previous_button, $form );
		return '</div>' . self::kdnaform_footer( $form, 'kdnaform-page-footer kdnaform_page_footer ' . $label_placement, $ajax, $field_values, $previous_button, $display_title, $display_description, $tabindex, $form_theme, $style_settings, $submission_method ) . '
	...
	public static function get_first_page_header( $form ) {
		...
		return "<div id='kdnaform_page_{$form['id']}_1' class='kdnaform_page{$class}' data-js='page-field-id-0' {$style}>
					<div class='kdnaform_page_fields'>";
	}
}
