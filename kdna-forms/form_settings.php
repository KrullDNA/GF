...
class KDNAFormSettings {
	...
	public static function form_settings_page() {
		...
		switch ( $subview ) {
			...
			case 'confirmation':
				...
				self::page_header( __( 'Confirmations', 'kdnaforms' ) );
				KDNA_Confirmation::render_page();
				self::page_footer();
				break;
			case 'notification':
			...
			default:
                 ...
                 */
				do_action( "kdnaform_form_settings_page_{$subview}" );
		}
	...
	public static function form_settings_ui() {
		...
		if ( ! KDNAAPI::form_id_exists( $form_id ) ) {
			KDNACommon::log_error( __METHOD__ . '(): Invalid Form ID: ' . $form_id );
	...
	public static function form_settings_fields( $form ) {
		...
		$deprecated_confirmation_classes_field_notice = function ( $value, $field ) use ( $form ) {
			...
			if ( in_array( $value, $deprecated_confirmation_classes ) ) {
				return '<div id="kdnafield-warning-deprecated" class="kdnaform-alert kdnaform-alert--notice kdnaform-alert--inline" role="alert" style="margin-block-start: 1rem;">
					<span class="kdnaform-alert__icon kdnaform-icon kdnaform-icon--circle-notice-fine" aria-hidden="true"></span>
					<div class="kdnaform-alert__message-wrap">
						<p class="kdnaform-alert__message">' . esc_html__( 'This form uses the "' . $value . '" Ready Class, which will be removed in KDNA Forms 4.0. You can use a CSS code snippet instead.', 'kdnaforms' ) .
					   ...
					   '<span class="kdnaform-icon kdnaform-icon--external-link" aria-hidden="true"></span></a></p>
					</div>
				</div>';
			}
			return '';
		...
		$fields = array(
			'form_basics'       => array(
				'title'  => esc_html__( 'Form Basics', 'kdnaforms' ),
				'fields' => array(
					array(
						'name'                => 'title',
						'type'                => 'text',
						'label'               => esc_html__( 'Form Title', 'kdnaforms' ),
						'tooltip'             => kdnaform_tooltip( 'form_title', '', true ),
						'required'            => true,
						'validation_callback' => function ( $field, $value ) use ( $form ) {
							...
							foreach ( $forms as $f ) {
								...
								// Set field error.
								$field->set_error( esc_html__( 'The form title you have entered has already been used. Please enter a unique form title.', 'kdnaforms' ) );
					...
					array(
						'name'       => 'description',
						'type'       => 'textarea',
						'label'      => esc_html__( 'Form Description', 'kdnaforms' ),
						'tooltip'    => kdnaform_tooltip( 'form_description', '', true ),
						'allow_html' => true,
					),
				),
			),
			'form_layout'       => array(
				'title'  => esc_html__( 'Form Layout', 'kdnaforms' ),
				'fields' => array(
					array(
						'name'          => 'labelPlacement',
						'type'          => 'select',
						'label'         => esc_html__( 'Label Placement', 'kdnaforms' ),
						'default_value' => 'top_label',
						'tooltip'       => kdnaform_tooltip( 'form_label_placement', '', true ),
						'choices'       => array(
							array(
								'label' => __( 'Top aligned', 'kdnaforms' ),
								'value' => 'top_label',
							),
							array(
								'label' => __( 'Left aligned', 'kdnaforms' ),
								'value' => 'left_label',
							),
							array(
								'label' => __( 'Right aligned', 'kdnaforms' ),
								'value' => 'right_label',
							),
						),
					),
					array(
						...
						'type'          => 'select',
						'label'         => esc_html__( 'Description Placement', 'kdnaforms' ),
						'default_value' => 'below',
						'tooltip'       => kdnaform_tooltip( 'form_description_placement', '', true ),
						'dependency'    => array(
							'live'   => true,
							'fields' => array(
								array(
						...
						'choices'       => array(
							array(
								'label' => __( 'Below inputs', 'kdnaforms' ),
								'value' => 'below',
							),
							array(
								'label' => __( 'Above inputs', 'kdnaforms' ),
								'value' => 'above',
							),
						),
					),
					array(
						...
						'type'          => 'select',
						'label'         => esc_html__( 'Validation Message Placement', 'kdnaforms' ),
						'default_value' => 'below',
						'tooltip'       => kdnaform_tooltip( 'form_validation_placement', '', true ),
						'choices'       => array(
							array(
								'label' => __( 'Below inputs', 'kdnaforms' ),
								'value' => 'below',
					...
					array(
						...
						'type'    => 'select',
						'label'   => esc_html__( 'Sub-Label Placement', 'kdnaforms' ),
						'tooltip' => kdnaform_tooltip( 'form_sub_label_placement', '', true ),
						'choices' => array(
							array(
								'label' => __( 'Below inputs', 'kdnaforms' ),
								'value' => 'below',
					...
					array(
						...
						'type'          => 'toggle',
						'label'         => esc_html__( 'Validation Summary', 'kdnaforms' ),
						'default_value' => false,
						'tooltip'       => kdnaform_tooltip( 'validation_summary', '', true ),
					),
					array(
						'name'          => 'requiredIndicator',
						'label'         => esc_html__( 'Required Field Indicator', 'kdnaforms' ),
						'type'          => 'radio',
						'default_value' => ( KDNACommon::is_legacy_markup_enabled( $form ) ) ? 'asterisk' : 'text',
						'horizontal'    => true,
						'tooltip'       => kdnaform_tooltip( 'form_required_indicator', '', true ),
						'choices'       => array(
							array(
								'label' => esc_html__( 'Text: (Required)', 'kdnaforms' ),
								'value' => 'text',
							),
							array(
								'label' => esc_html__( 'Asterisk: *', 'kdnaforms' ),
								'value' => 'asterisk',
							),
							array(
					...
					array(
						...
						'type'          => 'text',
						'label'         => esc_html__( 'Custom Required Indicator', 'kdnaforms' ),
						'default_value' => esc_html__( '(Required)', 'kdnaforms' ),
						'dependency'    => array(
							'live'   => true,
							'fields' => array(
								array(
					...
					array(
						...
						'label'       => esc_html__( 'CSS Class Name', 'kdnaforms' ),
						'tooltip'     => kdnaform_tooltip( 'form_css_class', '', true ),
					),
				),
			),
			'form_button'       => array(
				'title'  => esc_html__( 'Form Button', 'kdnaforms' ),
				'fields' => array(
					array(
						'name' => 'deprecated',
						'type' => 'html',
						'html' => esc_html__( 'Form button settings are now located in the form editor! To edit the button settings, go to the form editor and click on the submit button.', 'kdnaforms' ),
					),
				),
			),
			'save_and_continue' => array(
				'title'  => esc_html__( 'Save and Continue', 'kdnaforms' ),
				'fields' => array(
					array(
						'name'  => 'saveEnabled',
						'type'  => 'toggle',
						'label' => __( 'Enable Save and Continue', 'kdnaforms' ),
					),
					array(
						'name'          => 'saveButtonText',
						'type'          => 'text',
						'label'         => esc_html__( 'Link Text', 'kdnaforms' ),
						'default_value' => __( 'Save & Continue', 'kdnaforms' ),
						'dependency'    => array(
							'live'   => true,
							'fields' => array(
								array(
			...
			'restrictions'      => array(
				...
				'fields' => array(
					array(
						...
						'fields'  => array(
							array(
								...
								'label'      => __( 'Number of Entries', 'kdnaforms' ),
								'dependency' => array(
									'live'   => true,
									'fields' => array(
										array(
								...
								'inputs'     => array(
									...
									'select' => array(
										...
										'choices' => array(
											array(
												'label' => __( 'total entries', 'kdnaforms' ),
												'value' => '',
											),
											array(
												'label' => __( 'per day', 'kdnaforms' ),
												'value' => 'day',
											),
											array(
												'label' => __( 'per week', 'kdnaforms' ),
												'value' => 'week',
											),
											array(
												'label' => __( 'per month', 'kdnaforms' ),
												'value' => 'month',
											),
											array(
												'label' => __( 'per year', 'kdnaforms' ),
												'value' => 'year',
											),
										),
									),
							...
							array(
								...
								'type'       => 'textarea',
								'label'      => esc_html__( 'Entry Limit Reached Message', 'kdnaforms' ),
								'allow_html' => true,
								'dependency' => array(
									'live'   => true,
									'fields' => array(
					...
					array(
						'name'    => 'scheduleForm',
						'type'    => 'checkbox',
						'label'   => __( 'Schedule Form', 'kdnaforms' ),
						'tooltip' => kdnaform_tooltip( 'form_schedule_form', '', true ),
						'choices' => array(
							array(
								'name'  => 'scheduleForm',
								'label' => __( 'Schedule Form', 'kdnaforms' ),
						...
						'fields'  => array(
							array(
								'name'       => 'scheduleStart',
								'type'       => 'date_time',
								'label'      => esc_html__( 'Schedule Start Date/Time', 'kdnaforms' ),
								'dependency' => array(
									'live'   => true,
									'fields' => array(
										array(
							...
							array(
								'name'       => 'scheduleEnd',
								'type'       => 'date_time',
								'label'      => esc_html__( 'Schedule Form End Date/Time', 'kdnaforms' ),
								'dependency' => array(
									'live'   => true,
									'fields' => array(
										array(
							...
							array(
								...
								'type'       => 'textarea',
								'label'      => esc_html__( 'Form Pending Message', 'kdnaforms' ),
								'allow_html' => true,
								'dependency' => array(
									'live'   => true,
									'fields' => array(
							...
							array(
								...
								'type'       => 'textarea',
								'label'      => esc_html__( 'Form Expired Message', 'kdnaforms' ),
								'allow_html' => true,
								'dependency' => array(
									'live'   => true,
									'fields' => array(
					...
					array(
						'name'    => 'requireLogin',
						'type'    => 'checkbox',
						'label'   => __( 'Require user to be logged in', 'kdnaforms' ),
						'tooltip' => kdnaform_tooltip( 'form_require_login', '', true ),
						'choices' => array(
							array(
								'name'  => 'requireLogin',
								'label' => __( 'Require user to be logged in', 'kdnaforms' ),
						...
						'fields'  => array(
							array(
								...
								'type'       => 'textarea',
								'label'      => esc_html__( 'Require Login Message', 'kdnaforms' ),
								'tooltip'    => kdnaform_tooltip( 'form_require_login_message', '', true ),
								'allow_html' => true,
								'dependency' => array(
									'live'   => true,
									'fields' => array(
			...
			'spam'              => array(
				'title'  => esc_html__( 'Spam Detection', 'kdnaforms' ),
				'fields' => array(
					array(
						'name'    => 'enableHoneypot',
						'type'    => 'toggle',
						'label'   => esc_html__( 'Honeypot', 'kdnaforms' ),
						'tooltip' => kdnaform_tooltip( 'form_honeypot', '', true ),
					),
					array(
						'name'          => 'honeypotAction',
						'type'          => 'radio',
						'default_value' => 'spam',
						'horizontal'    => true,
						'label'         => esc_html__( 'If the honeypot flags a submission as spam:', 'kdnaforms' ),
						'dependency'    => array(
							'live'   => true,
							'fields' => array(
								array(
						...
						'choices'       => array(
							array(
								'label' => esc_html__( 'Do not create an entry', 'kdnaforms' ),
								'value' => 'abort',
							),
							array(
								'label' => esc_html__( 'Create an entry and mark it as spam', 'kdnaforms' ),
								'value' => 'spam',
							),
						),
					),
					array(
						...
						'type'          => 'toggle',
						'label'         => esc_html__( 'Submission Speed Check', 'kdnaforms' ),
						'description'   => esc_html__( 'Flags the submission as spam if the elapsed time between page load and form submission is less than the threshold.', 'kdnaforms' ),
						'default_value' => false,
						'dependency'    => array(
							'live'   => true,
							'fields' => array(
					...
					array(
						...
						'min'                 => 1,
						'default_value'       => 2000,
						'label'               => esc_html__( 'Submission Speed Check: Threshold (milliseconds)', 'kdnaforms' ),
						'dependency'          => array(
							'live'   => true,
							'fields' => array(
								array(
						...
						'validation_callback' => function ( $field, $value ) {
							if ( ! ctype_digit( $value ) || (int) $value < 1 ) {
								$field->set_error( esc_html__( 'Please enter a valid number greater than zero.', 'kdnaforms' ) );
							}
						},
					),
					array(
						...
						'label'         => esc_html__( 'Submission Speed Check: Mode', 'kdnaforms' ),
						'description'   => esc_html__( 'Submission speed is captured for each page of a multi-page form and for each submission attempt after a validation error. If there are multiple submission speeds for one submission, which mode should be used to evaluate the submission?', 'kdnaforms' ),
						'dependency'    => array(
							'live'   => true,
							'fields' => array(
								array(
						...
						'choices'       => array(
							array(
								'label' => esc_html__( 'Normal: at least one speed must be above the threshold.', 'kdnaforms' ),
								'value' => 'normal',
							),
							array(
								'label' => esc_html__( 'Strict: all speeds must be above the threshold.', 'kdnaforms' ),
								'value' => 'strict',
							),
						),
					),
			...
			'form_options'      => array(
				'title'  => esc_html__( 'Form Options', 'kdnaforms' ),
				'fields' => array(
					array(
						'name'    => 'enableAnimation',
						'type'    => 'toggle',
						'label'   => __( 'Animated transitions', 'kdnaforms' ),
						'tooltip' => kdnaform_tooltip( 'form_animation', '', true ),
					),
				),
			),
		);
		...
		if ( has_filter( 'kdnaform_form_settings' ) ) {
			trigger_error( 'kdnaform_form_settings is deprecated and will be removed in version 3.0.', E_USER_DEPRECATED ); // phpcs:ignore QITStandard.PHP.DebugCode.DebugFunctionFound
		}
	...
	public static function legacy_markup_enabled_or_posted( $form ) {
		if ( $_POST && empty( $_POST['_kdnaform_setting_markupVersion'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing, 
			return apply_filters( 'kdnaform_show_legacy_markup_setting', false );
		}

		return apply_filters( 'kdnaform_show_legacy_markup_setting', KDNACommon::is_legacy_markup_enabled( $form ) );
	}
	...
	public static function show_legacy_markup_setting() {
		...
		return apply_filters( 'kdnaform_show_legacy_markup_setting', $show_legacy_setting );
	}
	...
	public static function legacy_is_in_use() {
		$legacy_is_in_use = KDNACache::get( 'legacy_is_in_use', $found_in_cache );

		if ( ! $found_in_cache ) {
			$legacy_is_in_use = KDNAFormsModel::has_legacy_markup();

			KDNACache::set( 'legacy_is_in_use', $legacy_is_in_use, true,  DAY_IN_SECONDS );
		}
	...
	public static function legacy_markup_warning() {
		return '<div class="kdnaform-alert" data-js="kdnaform-alert" role="status">
		    <span
		        class="kdnaform-alert__icon kdnaform-icon kdnaform-icon--campaign"
		        aria-hidden="true"
		    ></span>
		    <div class="kdnaform-alert__message-wrap">
		        <p class="kdnaform-alert__message">' . esc_html__( 'Legacy markup is incompatible with many new features, including the Orbital Theme.', 'kdnaforms' ) . '</p>
		        <p class="kdnaform-alert__message">' . esc_html__( 'Legacy markup will be removed in KDNA Forms 4.0, and then all forms will use modern markup.  We recommend using modern markup on all forms.', 'kdnaforms' ) . '</p>
			    <a
		...
		if ( isset( $form['cssClass'] ) ) {
			...
			foreach ( $field_classes as $class ) {
				if ( in_array( $class, $deprecated_confirmation_classes ) ) {
					return '<div class="kdnaform-alert" data-js="kdnaform-alert" style="grid-column: 1/-1;">
						<span class="kdnaform-alert__icon kdnaform-icon kdnaform-icon--campaign" aria-hidden="true"></span>
						<div class="kdnaform-alert__message-wrap">
							<p class="kdnaform-alert__message">' . esc_html__( 'This form uses a deprecated CSS Ready Class, which will be removed in KDNA Forms 4.0.', 'kdnaforms' ) . '</p>
							<a class="kdnaform-alert__cta kdnaform-button kdnaform-button--white kdnaform-button--size-xs" href="https://docs.kdnaforms.com/migrating-your-forms-from-ready-classes/" target="_blank">'
						   	. esc_html__( 'Learn More', 'kdnaforms' ) .
	...
	public static function initialize_settings_renderer() {
		...
		$form_id = rgget( 'id' );
		$form    = KDNACommon::kdnaform_admin_pre_render( KDNAFormsModel::get_form_meta( $form_id ) );
		...
		$renderer = new Settings(
			array(
				...
				'save_callback'  => function( $values ) use ( &$form, $form_id ) {
					...
					// Form Layout
					$form['labelPlacement']          = KDNACommon::whitelist( rgar( $values, 'labelPlacement' ), array( 'top_label', 'left_label', 'right_label' ) );
					$form['descriptionPlacement']    = KDNACommon::whitelist( rgar( $values, 'descriptionPlacement' ), array( 'below', 'above' ) );
					$form['validationPlacement']     = KDNACommon::whitelist( rgar( $values, 'validationPlacement' ), array( 'below', 'above' ) );
					$form['subLabelPlacement']       = KDNACommon::whitelist( rgar( $values, 'subLabelPlacement' ), array( 'below', 'above' ) );
					$form['validationSummary']       = rgar( $values, 'validationSummary', false );
					$form['requiredIndicator']       = KDNACommon::whitelist( rgar( $values, 'requiredIndicator' ), array( 'text', 'asterisk', 'custom' ) );
					...
					$form['limitEntriesPeriod']  = rgar( $values, 'limitEntriesPeriod' ) ? KDNACommon::whitelist( $values['limitEntriesPeriod'], array( '', 'day', 'week', 'month', 'year' ) ) : '';
					...
					$form['honeypotAction'] = KDNACommon::whitelist(
					...
					$form['submitSpeedCheckMode']      = KDNACommon::whitelist(
					...
					if ( $form['save']['enabled'] ) {
						$form = KDNAFormSettings::activate_save( $form );
					} else {
						$form = KDNAFormSettings::deactivate_save( $form );
					}
					 ...
					 */
					$form = apply_filters( 'kdnaform_pre_form_settings_save', $form );

					// Save form.
					KDNAFormDetail::save_form_info( $form_id, addslashes( json_encode( $form ) ) );
				...
				'before_fields' => function() use ( &$form ) {
					...
					<script type="text/javascript">
						...
						<?php KDNAFormSettings::output_field_scripts() ?>
	...
	private static function get_initial_values( $form ) {
		 ...
		 */
		return apply_filters( 'kdnaform_form_settings_initial_values', $initial_values, $form );
	}
	 ...
	 * @since 2.5
	 *
	 * @return false|KDNA_Forms\KDNA_Forms\Settings\Settings
	 */
	 ...
	 * @access public
	 *
	 * @used-by KDNAFormSettings::form_settings_page()
	 * @uses    KDNANotification::notification_page()
	 *
	 * @return void
	 */
	public static function notification_page() {
		...
		KDNANotification::notification_page();
	 ...
	 * @access public
	 *
	 * @used-by KDNAFormSettings::confirmations_edit_page()
	 * @used-by KDNAFormSettings::confirmations_list_page()
	 * @used-by KDNAFormSettings::form_settings_ui()
	 * @used-by KDNANotification::notification_edit_page()
	 * @used-by KDNANotification::notification_list_page()
	 * @used-by KDNAAddOn::form_settings_page()
	 * @uses    SCRIPT_DEBUG
	 * @uses    KDNAFormsModel::get_form_meta()
	 * @uses    KDNAFormSettings::get_tabs()
	 * @uses    KDNACommon::display_dismissible_message()
	 * @uses    KDNACommon::display_admin_message()
	 * @uses    KDNAForms::top_toolbar()
	...
	public static function page_header( $title = '' ) {
		...
		$setting_tabs = KDNAFormSettings::get_tabs( $form['id'] );
	...
	public static function page_footer() {
		return KDNASettings::page_footer();
	}
	 ...
	 * @access public
	 *
	 * @used-by KDNAFormSettings::page_header()
	 * @used-by KDNAForms::get_form_settings_sub_menu_items()
	 * @used-by KDNAForms::modify_admin_title()
	 *
	...
	public static function get_tabs( $form_id ) {
		...
		$setting_tabs = array(
			...
			'20' => array(
				...
				'icon'         => 'kdnaform-icon--confirmations',
		 ...
		 */
		$setting_tabs = apply_filters( 'kdnaform_form_settings_menu', $setting_tabs, $form_id );
	 ...
	 * @access public
	 *
	 * @used-by KDNAFormSettings::get_tabs()
	 */
	 ...
	 * @uses    KDNACommon::add_message()
	 * @uses    KDNACommon::add_error_message()
	 *
	 * @return void
	 */
	public static function maybe_process_confirmation_list_action() {

		if ( empty( $_POST ) || ! check_admin_referer( 'kdnaform_confirmation_list_action', 'kdnaform_confirmation_list_action' ) )
			return;
		...
		switch ( $action ) {
			case 'delete':
				$confirmation_deleted = KDNAFormsModel::delete_form_confirmation( $object_id, rgget( 'id' ) );
				if ( $confirmation_deleted ) {
					KDNACommon::add_message( __( 'Confirmation deleted.', 'kdnaforms' ) );
				} else {
					KDNACommon::add_error_message( __( 'There was an issue deleting this confirmation.', 'kdnaforms' ) );
				}
				break;
		}
	...
	public static function delete_confirmation( $confirmation_id, $form_id ) {
		...
		return KDNAFormsModel::delete_form_confirmation( $confirmation_id, $form_id );
	 ...
	 * @access public
	 *
	 * @uses KDNA_Fields::get_all()
	 * @uses KDNA_Field::is_conditional_logic_supported()
	 *
	 ...
	 * @uses    KDNAFormsModel::save_form_confirmations()
	 *
	...
	public static function activate_save( $form ) {
		...
		if ( ! $has_save_notification ) {
			...
			$form['notifications'][ $notification_id ] = array(
				'id'      => $notification_id,
				'isDefault' => true,
				'name'    => __( 'Save and Continue Email', 'kdnaforms' ),
				'event'   => 'form_save_email_requested',
				'toType'  => 'hidden',
				'from' => '{admin_email}',
				'subject' => __( 'Link to continue {form_title}' ),
				'message' => __( 'Thank you for saving {form_title}. Please use the unique link below to return to the form from any computer. <br /><br /> {save_link} <br /><br /> Remember that the link will expire after 30 days so please return via the provided link to complete your form submission.', 'kdnaforms' ),
			);
			KDNAFormsModel::save_form_notifications( $form_id, $form['notifications'] );
		}
		...
		if ( ! $has_save_confirmation ) {
			$confirmation_1 = KDNAFormsModel::get_default_confirmation( 'form_saved' );
			$confirmation_2 = KDNAFormsModel::get_default_confirmation( 'form_save_email_sent' );

			$form['confirmations'][ $confirmation_1['id'] ] = $confirmation_1;
			$form['confirmations'][ $confirmation_2['id'] ] = $confirmation_2;
			KDNAFormsModel::save_form_confirmations( $form_id, $form['confirmations'] );
		}
		return $form;
	}
	...
	public static function save_form_title() {
		...
		if ( empty( $form_title ) ) {
			...
			$result['message'] = __( 'Please enter a form title.', 'kdnaforms' );

		} elseif ( ! KDNAFormsModel::is_unique_title( $form_title, $form_id ) ) {
			$result['isValid'] = false;
			$result['message'] = __( 'Please enter a unique form title.', 'kdnaforms' );
			...
			KDNAAPI::update_form( $form, $form_id );
