...
class KDNAFormsModel {
	...
	/**
	 * Flushes the data stored within KDNAFormsModel::$_current_forms.
	 *
	 * @since  Unknown
	 * @access public
	 *
	 * @uses KDNAFormsModel::$_current_forms
	 *
	 * @return void
	 */
	...
	/**
	 * Flushes the data stored within KDNAFormsModel::$_current_lead.
	 *
	 * @since  Unknown
	 * @access public
	 *
	 * @uses KDNAFormsModel::$_current_lead
	 *
	 * @return void
	 */
	...
	/**
	 * Flushes the data stored within KDNAFormsModel::$_confirmations
	 *
	 * @since  Unknown
	 * @access public
	 *
	 * @uses KDNAFormsModel::$_confirmations
	 *
	 * @return void
	 */
	 ...
	 * @global $wpdb
	 *
	 * @uses KDNAFormsModel::get_form_table_name()
	 * @uses KDNAFormsModel::get_form_db_columns()
	 * @uses KDNAFormsModel::get_entry_count_per_form()
	 * @uses KDNAFormsModel::get_view_count_per_form()
	 *
	 ...
	 * @static
	 * @global $wpdb
	 * @see KDNAFormsModel::get_form_table_name
	 * @see KDNAFormsModel::get_form_db_columns
	 * @see KDNAFormsModel::get_entry_count_per_form
	 * @see KDNAFormsModel::get_view_count_per_form
	 *
	 ...
	 * @global $wpdb
	 *
	 * @uses KDNAFormsModel::get_lead_table_name()
	 * @uses KDNACache::get()
	 * @uses KDNACache::set()
	 *
	...
	public static function get_entry_count_per_form() {
		...
		$entry_count = KDNACache::get( 'get_entry_count_per_form' );
		if ( empty( $entry_count ) ) {
			...
			KDNACache::set( 'get_entry_count_per_form', $entry_count, true, 30 );
		}
	 ...
	 * @global $wpdb
	 *
	 * @uses KDNAFormsModel::get_form_view_table_name()
	 * @uses KDNACache::get()
	 * @uses KDNACache::set()
	 *
	...
	public static function get_view_count_per_form() {
		...
		$view_count = KDNACache::get( 'get_view_count_per_form' );
		if ( empty( $view_count ) ){
			...
			KDNACache::set( 'get_view_count_per_form', $view_count, true, 30 );
		}
	 ...
	 *
	 * @uses KDNAFormsModel::get_lead_table_name()
	 * @uses KDNAFormsModel::get_lead_details_table_name()
	 *
	...
	public static function get_form_counts( $form_id ) {
		...
		$results = KDNACache::get( $cache_key );
		 ...
		 */
		$filtered_queries = apply_filters( 'kdnaform_entries_filter_count_queries', $queries, $form_id );
		...
		if ( $wpdb->timer_stop() > 1 ) {
			KDNACache::set( $cache_key, $results[0], true, 10 * MINUTE_IN_SECONDS );
		}
	...
	public static function get_form_summary() {
		 ...
		 */
		$forms = apply_filters( 'kdnaform_form_summary', $forms );
	...
	public static function get_form_count() {
		...
		if ( ! KDNACommon::table_exists( $form_table_name ) ) {
			return array(
				'total'    => 0,
				'active'   => 0,
				'inactive' => 0,
	 ...
	 * @global $wpdb
	 *
	 * @uses KDNAFormsModel::get_meta_table_name()
	 * @uses KDNAFormsModel::unserialize()
	 * @uses KDNAFormsModel::convert_field_objects()
	 * @uses KDNAFormsModel::load_notifications_from_legacy()
	 * @uses KDNAFormsModel::$_current_forms
	 *
	 * @param int $form_id The form ID.
	 *
	 ...
	 * @access public
	 *
	 * @uses KDNA_Field_CreditCard::maybe_upgrade_inputs()
	 *
	 * @param array $form The Form object.
	 *
	 ...
	 * @since 2.4
	 *
	 * @param KDNA_Field $field         The repeater field to be converted to objects.
	 ...
	 * @access private
	 *
	 * @uses KDNACommon::has_admin_notification()
	 * @uses KDNAFormsModel::convert_property_to_merge_tag()
	 * @uses KDNACommon::has_user_notification()
	 * @uses KDNAFormsModel::save_form_notifications()
	 *
	 * @param array $form The Form object.
	 *
	 * @return array $form The Form object.
	...
	private static function load_notifications_from_legacy( $form ) {
		...
		if ( KDNACommon::has_admin_notification( $form ) ) {
			...
			$admin_notification['name']  = esc_html__( 'Admin Notification', 'kdnaforms' );
		...
		if ( KDNACommon::has_user_notification( $form ) ) {
			...
			$user_notification['name']  = esc_html__( 'User Notification', 'kdnaforms' );
	 ...
	 * @access private
	 *
	 * @uses KDNAFormsModel::get_field_merge_tag()
	 *
	 * @param array  $form            The Form object.
	 ...
	 * @access private
	 *
	 * @uses KDNAFormsModel::get_field()
	 * @uses KDNACommon::get_label()
	 *
	 * @param array $form     The Form object.
	...
	private static function get_field_merge_tag( $form, $field_id ) {
		...
		return '{' . KDNACommon::get_label( $field, $field_id ) . ':' . $field_id . '}';
	}
	...
	public static function add_default_properties( $form ) {
		_deprecated_function( 'KDNAFormsModel::add_default_properties', '1.9' );
	...
	public static function update_form_active( $form_id, $is_active ) {
		...
		if ( $is_active ) {
			 ...
			 */
			do_action( 'kdnaform_post_form_activated', $form_id );
		} else {
			 ...
			 * @param int $form_id The Form ID used to specify which form to activate
			 */
			do_action( 'kdnaform_post_form_deactivated', $form_id );
		}
	...
	public static function update_notification_active( $form_id, $notification_id, $is_active ) {
		...
		if ( ! isset( $form['notifications'][ $notification_id ] ) ) {
			return new WP_Error( 'not_found', __( 'Notification not found', 'kdnaforms' ) );
		}
		...
		if ( (bool) $is_active ) {
             ...
             * @param array $form                                      The Form object
             */
			do_action( 'kdnaform_pre_notification_activated', $form['notifications'][ $notification_id ], $form );
		} else {
            /**
             ...
             * @param array $form                                      The Form object
             */
			do_action( 'kdnaform_pre_notification_deactivated', $form['notifications'][ $notification_id ], $form );
		}

		$result = KDNAFormsModel::update_form_meta( $form_id, $form['notifications'], 'notifications' );
	...
	public static function update_confirmation_active( $form_id, $confirmation_id, $is_active ) {
		...
		if ( ! isset( $form['confirmations'][ $confirmation_id ] ) ) {
			return new WP_Error( 'not_found', __( 'Confirmation not found', 'kdnaforms' ) );
		}
		...
		$result = KDNAFormsModel::update_form_meta( $form_id, $form['confirmations'], 'confirmations' );
	...
	public static function change_entry_status( $lead_id, $status ) {
		...
		kdnaform_update_meta( $lead_id, 'previous_status', $previous_status );
	...
	public static function restore_entry_status( $lead_id ) {
		...
		$previous_status = kdnaform_get_meta( $lead_id, 'previous_status' );
	...
	public static function update_entry_property( $lead_id, $property_name, $property_value, $update_akismet = true, $disable_hook = false ) {
		...
		if ( $update_akismet && KDNACommon::akismet_enabled( $lead['form_id'] ) && $property_name == 'status' && in_array( $property_value, array( 'active', 'spam' ) ) ) {

			$current_status = $lead['status'];
			if ( $current_status == 'spam' && $property_value == 'active' ) {
				$form = self::get_form_meta( $lead['form_id'] );
				KDNACommon::mark_akismet_spam( $form, $lead, false );
			} else if ( $current_status == 'active' && $property_value == 'spam' ) {
				$form = self::get_form_meta( $lead['form_id'] );
				KDNACommon::mark_akismet_spam( $form, $lead, true );
			}
		}

		// If property is trash, log user login
		if ( $property_name == 'status' && $property_value == 'trash' && ! empty( $current_user->user_login ) ) {
			KDNACommon::log_debug( __METHOD__ . "(): User ID {$current_user->ID} requested moving of entry #{$lead_id} to trash." );
		}
		...
		if ( ! $disable_hook ) {
			...
			if ( $previous_value != $property_value ) {
				...
				if ( $property_name == 'status' && $previous_value == 'spam' && $property_value == 'active' && ! rgar( $lead, 'post_id' ) ) {
					...
					$lead['post_id']      = KDNACommon::create_post( isset( $form ) ? $form : KDNAAPI::get_form( $lead['form_id'] ), $lead );
				}
				 ...
				 */
				do_action( "kdnaform_update_{$property_name}", $lead_id, $property_value, $previous_value );
	...
	public static function delete_entries_by_form( $form_id, $status = '' ) {
		...
		KDNACommon::log_debug( __METHOD__ . "(): Deleting entries for form #{$form_id}." );
		 ...
		 */
		do_action( 'kdnaform_delete_entries', $form_id, $status );
		...
		if ( ! empty( $current_user->user_login ) ) {
			KDNACommon::log_debug( __METHOD__ . "(): User ID {$current_user->ID} requested deletion of entries: " . json_encode( $entry_ids ) );
		}
			...
			foreach ( $entry_ids as $entry_id ) {
				 ...
				 */
				do_action( 'kdnaform_delete_entry', $entry_id );


				/**
				 * Fires before a lead is deleted
				 * @param $lead_id
				 * @deprecated Use kdnaform_delete_entry instead
				 * @see kdnaform_delete_entry
				 * @remove-in 3.0
				 */

			if ( has_action( 'kdnaform_delete_lead' ) ) {
				trigger_error( 'The kdnaform_delete_lead action is deprecated and will be removed in 3.0. Use kdnaform_delete_entry instead.', E_USER_DEPRECATED ); // phpcs:ignore QITStandard.PHP.DebugCode.DebugFunctionFound
			}
				do_action( 'kdnaform_delete_lead', $entry_id );
	...
	public static function delete_views( $form_id ) {
		...
		if ( ! empty( $current_user->user_login ) ) {
			KDNACommon::log_debug( __METHOD__ . "(): User ID {$current_user->ID} requested deletion of views for form #{$form_id}." );
		}
         ...
         */
		do_action( 'kdnaform_post_form_views_deleted', $form_id );
	}
	...
	public static function delete_form( $form_id ) {
		...
		if ( ! empty( $current_user->user_login ) ) {
			KDNACommon::log_debug( __METHOD__ . "(): User ID {$current_user->ID} requested deletion of form #{$form_id}." );
		}
         ...
         */
		do_action( 'kdnaform_before_delete_form', $form_id );
         ...
         */
		do_action( 'kdnaform_after_delete_form', $form_id );
	}
	...
	public static function trash_form( $form_id ) {
		...
		if ( ! empty( $current_user->user_login ) ) {
			KDNACommon::log_debug( __METHOD__ . "(): User ID {$current_user->ID} requested moving of form #{$form_id} to trash." );
		}
		$form_table_name = self::get_form_table_name();
		 ...
		 */
		do_action( 'kdnaform_post_form_trashed', $form_id );
	...
	public static function restore_form( $form_id ) {
         ...
         */
		do_action( 'kdnaform_post_form_restored', $form_id );
	...
	public static function duplicate_form( $form_id ) {
		...
		// The kdnaform_after_duplicate_form action is deprecated since version 1.9. Please use kdnaform_post_form_duplicated instead

        /**
         * @deprecated
         * @see kdnaform_post_form_duplicated
         * @remove-in 3.0
         */

		if ( has_action( 'kdnaform_after_duplicate_form' ) ) {
			trigger_error( 'The kdnaform_after_duplicate_form action is deprecated and will be removed in 3.0. Use kdnaform_post_form_duplicated instead.', E_USER_DEPRECATED ); // phpcs:ignore QITStandard.PHP.DebugCode.DebugFunctionFound
		}
        do_action( 'kdnaform_after_duplicate_form', $form_id, $new_id );
		 ...
		 */
		do_action( 'kdnaform_post_form_duplicated', $form_id, $new_id );
	...
	public static function ensure_tables_exist() {
		...
		if ( $wpdb->last_error ) {
			KDNACommon::log_debug( 'KDNAFormsModel::ensure_tables_exist(): Blog ' . get_current_blog_id() . ' - Form database table does not exist. Forcing database setup.' );
	...
	public static function maybe_create_form_revision( $new_display_meta, $form_id ) {
		...
		if ( KDNACommon::has_consent_field( $new_display_meta ) ) {
	...
	public static function get_latest_form_revisions_id( $form_id ) {
		global $wpdb;
		$revisions_table_name = KDNAFormsModel::get_form_revisions_table_name();
	 ...
	 *
	 * Note: Does not delete the file URLs from the entries, that is done by KDNAFormsModel::delete_field_values().
	 *
	 * @since 2.4.6.1
	 *
	...
	private static function delete_physical_file( $file_url, $entry_id ) {
		...
		/**
		 * Allow the file path to be overridden so files stored outside the /wp-content/uploads/kdna_forms/ directory can be deleted.
		 *
		 * @since 2.2.3.1
		 *
		 ...
		 */
		$file_path = apply_filters( 'kdnaform_file_path_pre_delete_file', $file_path, $url );

		if ( file_exists( $file_path ) ) {
			unlink( $file_path );
			kdnaform_delete_meta( $entry_id, KDNA_Field_FileUpload::get_file_upload_path_meta_key_hash( $url ) );
		}
	...
	public static function get_physical_file_path( $url, $entry_id = null ) {
		...
		$path_info = KDNA_Field_FileUpload::get_file_upload_path_info( $url, $entry_id );

		// convert from url to physical path
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			$file_path = preg_replace( "|^(.*?)/files/kdna_forms/|", BLOGUPLOADDIR . 'kdna_forms/', $url );
		} else {
	...
	public static function delete_field( $form_or_id, $field_id, $save_form = true ) {
         ...
         */
		do_action( 'kdnaform_before_delete_field', $form_id, $field_id );
		...
		// The field has already been removed from the form passed by KDNAFormDetail::save_form_info(), get the field from the db.
		...
		// Notifications/confirmations are not present in the form passed by KDNAFormDetail::save_form_info() but they could be present in other scenarios.
		$form = KDNAFormsModel::delete_field_from_confirmations( $form, $field_id );
		$form = KDNAFormsModel::delete_field_from_notifications( $form, $field_id );
         ...
         *
		 */
		do_action( 'kdnaform_after_delete_field', $form_id, $field_id );
	...
	public static function delete_field_from_confirmations( $form, $field_id ) {
		...
		if ( $save ) {
			KDNAFormsModel::update_form_meta( $form['id'], $form['confirmations'], 'confirmations' );
		}
	...
	public static function delete_field_from_notifications( $form, $field_id ) {
		...
		if ( $save ) {
			KDNAFormsModel::update_form_meta( $form['id'], $form['notifications'], 'notifications' );
		}
	...
	public static function delete_entry( $entry_id ) {
		...
		if ( ! empty( $current_user->user_login ) ) {
			KDNACommon::log_debug( __METHOD__ . "(): User ID {$current_user->ID} requested deletion of entry #{$entry_id}" );
		}
		...
		KDNACommon::log_debug( __METHOD__ . "(): Deleting entry #{$entry_id}." );
		...
		if ( has_action( 'kdnaform_delete_lead' ) ) {
			trigger_error( 'The kdnaform_delete_lead action is deprecated and will be removed in version 3.0. Use kdnaform_delete_entry instead.', E_USER_DEPRECATED ); // phpcs:ignore QITStandard.PHP.DebugCode.DebugFunctionFound
		}
		do_action( 'kdnaform_delete_lead', $entry_id );
	...
	public static function add_note( $entry_id, $user_id, $user_name, $note, $note_type = 'user', $sub_type = null ) {
		 ...
		 *
		 */
		do_action( 'kdnaform_post_note_added', $wpdb->insert_id, $entry_id, $user_id, $user_name, $note, $note_type, $sub_type );
	...
	public static function delete_note( $note_id ) {
		 ...
		 */
		do_action( 'kdnaform_pre_note_deleted', $note_id, $lead_id );
	...
	public static function add_notification_note( $entry_id, $result, $notification, $error_info = '', $email = array(), $note_args = array() ) {
		...
		// If $note_args is empty, use default arguments for KDNA Forms core notifications.
		if ( empty( $note_args ) ) {
			if ( $result === true ) {

				$note_args['type'] = 'notification';
				$note_args['subtype'] = 'success';
				$note_args['text'] = esc_html__( 'WordPress successfully passed the notification email to the sending server.', 'kdnaforms' );

			} elseif ( $result === false ) {

				$note_args['type'] = 'notification';
				$note_args['subtype'] = 'error';
				$note_args['text'] = esc_html__( 'WordPress was unable to send the notification email.', 'kdnaforms' );
		 ...
		 * @param array  $email            Array containing email details.
		 */
		$note_args = apply_filters( 'kdnaform_notification_note', $note_args, $entry_id, $result, $notification, $error_info, $email );

		if ( ! empty( $note_args['text'] ) ) {
			// translators: Notification name followed by its ID. e.g. Admin Notification (ID: 5d4c0a2a37204).
			self::add_note( $entry_id, 0, sprintf( esc_html__( '%1$s (ID: %2$s)', 'kdnaforms' ), $notification['name'], $notification['id'] ), $note_args['text'], $note_args['type'], $note_args['subtype'] );
		}
	...
	public static function get_ip() {
		 ...
		 *
		 * @since 2.2
		 * @example https://docs.kdnaforms.com/kdnaform_ip_address/
		 *
		 * @param string $ip The IP being used.
		 */
		$ip = apply_filters( 'kdnaform_ip_address', $ip );
	...
	public static function save_entry( $form, &$entry ) {
		...
		KDNACommon::log_debug( __METHOD__ . '(): Saving entry.' );

		$is_form_editor  = KDNACommon::is_form_editor();
		$is_entry_detail = KDNACommon::is_entry_detail();
		$is_admin        = $is_form_editor || $is_entry_detail;

		if ( $is_admin && ! KDNACommon::current_user_can_any( 'kdnaforms_edit_entries' ) ) {
			wp_die( esc_html__( "You don't have adequate permission to edit entries.", 'kdnaforms' ) );
		}
		...
		if ( ! $is_new_lead && ! empty( $entry['id'] ) && ! empty( $current_user->ID ) ) {
			KDNACommon::log_debug( __METHOD__ . "(): User ID {$current_user->ID} requested update of entry #{$entry['id']}." );
		}
		...
		if ( ! $is_new_lead && ! self::entry_exists( rgar( $entry, 'id' ) ) ) {
			...
			$entry       = array();
			$is_new_lead = true;
			KDNACommon::log_debug( __METHOD__ . '(): Existing entry not found. Saving new entry instead.' );
		}

		$die_message = esc_html__( 'An error prevented the entry for this form submission being saved. Please contact support.', 'kdnaforms' );
		...
		if ( $is_new_lead ) {
			 ...
			 *
			 * @param string $currency The three character ISO currency code to be stored in the entry. Default is value returned by KDNACommon::get_currency()
			...
			if ( $lead_id == 0 ) {
				KDNACommon::log_error( __METHOD__ . '(): Unable to save entry. ' . $wpdb->last_error );
			...
			KDNACommon::log_debug( __METHOD__ . "(): Entry record created in the database. ID: {$lead_id}." );
		} else {
			KDNACommon::log_debug( __METHOD__ . "(): Updating existing entry. ID: {$entry['id']}." );
			...
			KDNAAPI::update_entry_property( $entry['id'], 'date_updated', $current_date );
		...
		$total_fields = array();
		/* @var $calculation_fields KDNA_Field[] */
		$calculation_fields = array();
		$has_post_field     = false;

		KDNACommon::log_debug( __METHOD__ . '(): Saving entry fields.' );
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( $is_entry_detail && KDNACommon::is_pricing_field( $field->type ) ) {
				continue;
			}
			...
			if ( $is_entry_detail || ! KDNAFormsModel::is_field_hidden( $form, $field, array(), $read_value_from_post ? null : $entry ) ) {
				...
				if ( $field->type === 'post_category' ) {
					$field          = KDNACommon::add_categories_as_choices( $field, '' );
					$has_post_field = true;
				} elseif ( ! $has_post_field && KDNACommon::is_post_field( $field ) ) {
					$has_post_field = true;
				}
		...
		KDNACache::set( 'has_post_field_entry_' . $entry['id'], $has_post_field );

		$results = KDNAFormsModel::commit_batch_field_operations();

		if ( $is_new_lead && is_wp_error( $results['inserts'] ) ) {
			/* @var WP_Error $error */
			$error = $results['inserts'];
			KDNACommon::log_error( __METHOD__ . '(): Error while saving field values for new entry. ' . $error->get_error_message() );
			self::add_note( $entry['id'], 0, 'KDNA Forms', sprintf( esc_html__( 'Error while saving field values: %s', 'kdnaforms' ), $error->get_error_message() ), 'save_entry', 'error' );
			wp_die( esc_html( $die_message ) );
		}
		...
		if ( ! empty( $calculation_fields ) ) {
			...
			if ( $is_new_lead && is_wp_error( $results['inserts'] ) ) {
				/* @var WP_Error $error */
				$error = $results['inserts'];
				KDNACommon::log_error( __METHOD__ . '(): Error while saving calculation field values for new entry. ' . $error->get_error_message() );
				self::add_note( $entry['id'], 0, 'KDNA Forms', sprintf( esc_html__( 'Error while saving calculation field values: %s', 'kdnaforms' ), $error->get_error_message() ), 'save_entry', 'error' );
				wp_die( esc_html( $die_message ) );
			}
		...
		if ( ! empty( $total_fields ) ) {
			...
			if ( $is_new_lead && is_wp_error( $results['inserts'] ) ) {
				/* @var WP_Error $error */
				$error = $results['inserts'];
				KDNACommon::log_error( __METHOD__ . '(): Error while saving total field values for new entry. ' . $error->get_error_message() );
				self::add_note( $entry['id'], 0, 'KDNA Forms', sprintf( esc_html__( 'Error while saving total field values: %s', 'kdnaforms' ), $error->get_error_message() ), 'save_entry', 'error' );
				wp_die( esc_html( $die_message ) );
			}
		}
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( is_array( $inputs ) ) {
				...
				if ( KDNAFormsModel::is_openssl_encrypted_field( $entry['id'], $field->id ) ) {
		...
		KDNACommon::log_debug( __METHOD__ . '(): Finished saving entry fields.' );
		KDNACommon::log_debug( __METHOD__ . sprintf( '(): %s entry completed in %F seconds.', $is_new_lead ? 'Saving' : 'Updating', KDNACommon::timer_end( __METHOD__ ) ) );
	}
	 ...
	 * @since 2.5.16
	 *
	 * @param KDNA_Field $field The current field.
	...
	public static function get_entry_meta_batch_processor() {
		if ( is_null( self::$entry_meta_batch_processor ) ) {
			$processor = KDNAForms::get_service_container()->get( KDNA_Batch_Operations_Service_Provider::ENTRY_META_BATCH_PROCESSOR );
	...
	public static function add_meta_to_entry( &$entry ) {
		...
		foreach ( $meta_keys as $meta_key ) {
			if ( ! isset( $entry[ $meta_key ] ) ) {
				...
				$entry[ $meta_key ] = kdnaform_get_meta( $entry['id'], $meta_key );
			}
		}
	}
	...
	public static function hydrate_repeaters( &$entry, $form, $apply_filters = false ) {
		$fields = $form['fields'];
		foreach ( $fields as $field ) {
			if ( $field instanceof KDNA_Field_Repeater && isset( $field->fields ) && is_array( $field->fields ) ) {
				/* @var KDNA_Field_Repeater $field */
	...
	public static function create_lead( $form ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			if ( ! KDNAFormsModel::is_field_hidden( $form, $field, array() ) ) {
		...
		KDNACommon::log_debug( __METHOD__ . sprintf( '(): Draft entry created for form (#%d) in %F seconds.', $form_id, KDNACommon::timer_end( __METHOD__ ) ) );
	...
	public static function get_prepared_input_value( $form, $field, $lead, $input_id ) {
		...
		switch ( self::get_input_type( $field ) ) {
			...
			case 'post_image':
				/** @var KDNA_Field_Post_Image $field */
			...
			case 'fileupload':
				/** @var KDNA_Field_FileUpload $field */
				$files = $field->get_submission_files();
				if ( $field->is_submission_files_empty( $files ) ) {
					$value = '';
					break;
				...
				$tmp_location = KDNAFormsModel::get_tmp_upload_location( $form_id );
	...
	public static function refresh_product_cache( $form, $lead, $use_choice_text = false, $use_admin_label = false ) {
		...
		foreach ( $cache_options as $cache_option ) {
			...
			if ( kdnaform_get_meta( rgar( $lead, 'id' ), "kdnaform_product_info_{$use_choice_text}_{$use_admin_label}" ) ) {
				kdnaform_delete_meta( rgar( $lead, 'id' ), "kdnaform_product_info_{$use_choice_text}_{$use_admin_label}" );
				KDNACommon::get_product_fields( $form, $lead, $use_choice_text, $use_admin_label );
			}
		}
	 ...
	 *
	 * @param array    $form         Form object.
	 * @param KDNA_Field $field        Field object.
	...
	public static function is_field_hidden( $form, $field, $field_values, $lead = null ) {
		...
		$cache_key = 'KDNAFormsModel::is_field_hidden_' . $form['id'] . '_' . $field->id;
		...
		$display   = KDNACache::get( $cache_key, $is_hit, false );
		if ( $display !== false ) {
			return $display;
		}
		...
		KDNACache::set( $cache_key, $display );
	 ...
	 * @since 2.9.25
	 *
	 * @param KDNA_Field $field The page field containing the next button logic.
	 * @param array    $form  The current form.
	 *
	 * @return bool
	 */
	public static function is_next_button_hidden( $field, $form ) {
		if ( ! $field instanceof KDNA_Field_Page || ! rgars( $field->nextButton, 'conditionalLogic/enabled' ) ) {
			return false;
		}
	 ...
	 * @param null|KDNA_Field $source_field The field the rule is based on.
	...
	public static function is_value_match( $field_value, $target_value, $operation = 'is', $source_field = null, $rule = null, $form = null ) {
		...
		if ( $source_field && is_subclass_of( $source_field, 'KDNA_Field' ) ) {
			if ( $source_field->type == 'post_category' ) {
				$field_value = KDNACommon::prepare_post_category_value( $field_value, $source_field, 'conditional_logic' );
			} elseif ( $source_field instanceof KDNA_Field_MultiSelect && ! empty( $field_value ) && ! is_array( $field_value ) ) {
				// Convert the comma-delimited string into an array.
				$field_value = $source_field->to_array( $field_value );
			} elseif ( $source_field instanceof KDNA_Field_Consent ) {
		...
		$form_id = $source_field instanceof KDNA_Field ? $source_field->formId : 0;

		$target_value = KDNAFormsModel::maybe_trim_input( $target_value, $form_id, $source_field );
		...
		if ( is_array( $field_value ) ) {
			...
			foreach ( $field_value as $val ) {
				$val = KDNAFormsModel::maybe_trim_input( KDNACommon::get_selection_value( $val ), $form_id, $source_field );
		...
		} else if ( self::matches_conditional_operation( KDNAFormsModel::maybe_trim_input( KDNACommon::get_selection_value( $field_value ), $form_id, $source_field ), $target_value, $operation ) ) {
			$is_match = true;
		}

		return apply_filters( 'kdnaform_is_value_match', $is_match, $field_value, $target_value, $operation, $source_field, $rule );
	}

	/*
	 * @deprecated 2.9.1.  Use KDNACommon::maybe_format_numeric instead.
	 *
	 * @remove-in 4.0
	 */
	private static function try_convert_float( $text ) {
		_deprecated_function( __METHOD__, '2.9.1', 'KDNACommon::maybe_format_numeric' );
		...
		if ( is_numeric( $text ) && $number_format == 'decimal_comma' ) {
			return KDNACommon::format_number( $text, 'decimal_comma' );
		} else if ( KDNACommon::is_numeric( $text, $number_format ) ) {
		...
		*/
		...
		if ( KDNACommon::is_numeric( $text, 'currency' ) ) {
	...
	/*
	 * @deprecated 2.9.1.  Use KDNAFormsModel::matches_conditional_operation instead.
	 *
	 * @remove-in 4.0
	 */
	public static function matches_operation( $val1, $val2, $operation ) {
		_deprecated_function( __METHOD__, '2.9.1', 'KDNAFormsModel::matches_conditional_operation' );
	...
	public static function get_custom_choices() {
		$choices = get_option( 'kdnaform_custom_choices' );
		if ( ! $choices ) {
			$choices = array();
		}
	...
	public static function delete_custom_choice( $name ) {
		...
		update_option( 'kdnaform_custom_choices', $choices );
	}
	...
	public static function save_custom_choice( $previous_name, $new_name, $choices ) {
		...
		update_option( 'kdnaform_custom_choices', $all_choices );
	}
	...
	public static function purge_expired_incomplete_submissions( $expiration_days = 30 ) {
		_deprecated_function( 'KDNAFormsModel::purge_expired_incomplete_submissions', '2.4', 'KDNAFormsModel::purge_expired_draft_submissions' );
	...
	public static function purge_expired_draft_submissions( $expiration_days = 30 ) {
		 ...
		 */
		$query = apply_filters( 'kdnaform_purge_expired_incomplete_submissions_query', $query );
	...
	public static function delete_incomplete_submission( $token ) {
		_deprecated_function( 'KDNAFormsModel::delete_incomplete_submission', '2.4', 'KDNAFormsModel::delete_draft_submission' );
	...
	public static function save_incomplete_submission( $form, $entry, $field_values, $page_number, $files, $form_unique_id, $ip, $source_url, $resume_token = '' ) {
		_deprecated_function( 'KDNAFormsModel::save_incomplete_submission', '2.4', 'KDNAFormsModel::save_draft_submission' );
	...
	public static function save_draft_submission( $form, $entry, $field_values, $page_number, $files, $form_unique_id, $ip, $source_url, $resume_token = '' ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			$submitted_values[ $field->id ] = KDNAFormsModel::get_field_value( $field, $field_values );
		}
		 ...
		 * @param array $form             The Form object
		 */
		$submitted_values = apply_filters( 'kdnaform_submission_values_pre_save', $submitted_values, $form );
		 ...
		 */
		do_action( 'kdnaform_incomplete_submission_post_save', $submission, $resume_token, $form, $entry );
	...
	private static function filter_draft_submission_pre_save( $submission_json, $resume_token, $form ) {
		 ...
		 *    @type string     $kdnaform_unique_id  The unique id for this submission.
		 * }
		 ...
		 */
		$submission_json = apply_filters( 'kdnaform_incomplete_submission_pre_save', $submission_json, $resume_token, $form );
	...
	public static function get_incomplete_submission_values( $resume_token ) {
		_deprecated_function( 'KDNAFormsModel::get_incomplete_submission_values', '2.4', 'KDNAFormsModel::get_draft_submission_values' );
	...
	private static function filter_draft_submission_post_get( $submission_json, $resume_token, $form ) {
		 ...
		 * @param array  $form         The form which this incomplete submission was created for.
		 */
		$submission_json = apply_filters( 'kdnaform_incomplete_submission_post_get', $submission_json, $resume_token, $form );
		return $submission_json;
	}
	...
	public static function add_email_to_incomplete_sumbmission( $token, $email ) {
		_deprecated_function( 'KDNAFormsModel::add_email_to_incomplete_sumbmission', '2.4', 'KDNAFormsModel::add_email_to_draft_sumbmission' );
	 ...
	 * @param KDNA_Field          $field   The field object the value belongs to.
	 *
	...
	public static function maybe_trim_input( $value, $form_id, $field ) {
		...
		$trim_value = apply_filters( 'kdnaform_trim_input_value', true, $form_id, $field );

		if ( $trim_value ) {
			$value = is_array( $value ) ? KDNACommon::trim_deep( $value ) : trim( $value );
		}
	...
	public static function get_parameter_value( $name, $field_values, $field ) {
		...
		if ( ! empty( $value ) && KDNAFormsModel::get_input_type( $field ) == 'list' ) {
			...
			if ( ! empty( $rows ) ) {
				...
				foreach ( $rows as $row ) {
					 ...
					 * @param array  $field        KDNA_Field object for the current field.
					 ...
					 */
					$delimiter = apply_filters( 'kdnaform_list_field_parameter_delimiter', '|', $field, $name, $field_values );
	...
	public static function get_default_value( $field, $input_id ) {
		if ( ! is_array( $field->choices ) ) {
			...
			if ( is_array( $field->get_entry_inputs() ) || ( $field->get_input_type() == 'email' && is_array( $field->inputs ) ) ) {
				$input = KDNAFormsModel::get_input( $field, $input_id );
				...
				if( ! IS_ADMIN ) {
					if( is_array( $value ) ) {
						foreach( $value as &$_value ) {
							$_value = KDNACommon::replace_variables_prepopulate( $_value );
						}
					} else {
						$value = KDNACommon::replace_variables_prepopulate( $value );
					}
				}
				return $value;
			}
		...
		foreach ( $form['fields'] as $field ) {
			...
			switch ( $field->type ) {
				...
				case 'post_content' :
					...
					$post_data[ $field->type ] = KDNACommon::encode_shortcodes( $value );
					break;
	...
	public static function get_input_masks() {
		...
		return apply_filters( 'kdnaform_input_masks', $masks );
	}
	...
	public static function prepare_date( $date_format, $value ) {
		...
		$date_info = KDNACommon::parse_date( $value, $format );
		if ( ! empty( $date_info ) && ! KDNACommon::is_empty_array( $date_info ) ) {
	 ...
	 * @param mixed    $lead    passed by the KDNAFormsModel::create_lead() method, lead ID is not available for leads created by this function
	 *
	 * @return mixed
	 */
	...
	public static function get_temp_filename( $form_id, $input_name ) {
		...
		KDNACommon::log_debug( __METHOD__ . '(): Uploaded filename is ' . $ext_check . ' and temporary filename is ' . $temp_filename );
	...
	public static function choice_value_match( $field, $choice, $value ) {
		$choice_value = KDNAFormsModel::maybe_trim_input( $choice['value'], $field->formId, $field );
		$value        = KDNAFormsModel::maybe_trim_input( $value, $field->formId, $field );
		...
		else if ( KDNAFormsModel::get_input_type( $field ) == 'multiselect' ) {
	...
	public static function create_post( $form, &$lead ) {
		KDNACommon::timer_start( __METHOD__ );
		KDNACommon::log_debug( 'KDNAFormsModel::create_post(): Starting.' );

		$has_post_field = (bool) KDNACache::get( 'has_post_field_entry_' . rgar( $lead, 'id' ), $found, false );
		if ( ! $found ) {
			// Fallback for when post creation is triggered at a later date/time.
			foreach ( $form['fields'] as $field ) {
				if ( KDNACommon::is_post_field( $field ) && ! self::is_field_hidden( $form, $field, array(), $lead ) ) {
					$has_post_field = true;
					break;
				}
			}
		...
		if ( ! $has_post_field ) {
			KDNACommon::log_debug( "KDNAFormsModel::create_post(): Stopping. The form doesn't have any post fields." );

			return $lead;
		}

		//processing post fields
		KDNACommon::log_debug( 'KDNAFormsModel::create_post(): Getting post fields.' );
		...
		// inserting post
		KDNACommon::log_debug( 'KDNAFormsModel::create_post(): Inserting post via wp_insert_post().' );
		$post_id = wp_insert_post( $post_data, true );
		KDNACommon::log_debug( 'KDNAFormsModel::create_post(): Result from wp_insert_post(): ' . print_r( $post_id, 1 ) );

		if ( is_wp_error( $post_id ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): $post_data => ' . print_r( $post_data, 1 ) );
		...
		if ( ! empty( $post_data['images'] ) ) {
			...
			KDNACommon::log_debug( 'KDNAFormsModel::create_post(): Processing post images.' );

			foreach ( $post_data['images'] as $image ) {
				if ( empty( $image['url'] ) ) {
					KDNACommon::log_debug( __METHOD__ . '(): No image to process for field #' . $image['field_id'] );
					continue;
				}
				...
				KDNACommon::log_debug( sprintf( '%s(): Field #%s. URL: %s', __METHOD__, $image['field_id'], $image['url'] ) );
				...
				if ( $media_id ) {
					...
					$field = KDNAFormsModel::get_field( $form, $image['field_id'] );
					if ( $field->postFeaturedImage ) {
						$result = set_post_thumbnail( $post_id, $media_id );
						KDNACommon::log_debug( __METHOD__ . '(): Setting the featured image. Result from set_post_thumbnail(): ' . var_export( $result, 1 ) ); // phpcs:ignore QITStandard.PHP.DebugCode.DebugFunctionFound
					}
				}
			}
		}
		...
		KDNACommon::log_debug( 'KDNAFormsModel::create_post(): Adding custom fields.' );
		foreach ( $post_data['post_custom_fields'] as $meta_name => $meta_value ) {
			...
			foreach ( $meta_value as $value ) {
				KDNACommon::log_debug( 'KDNAFormsModel::create_post(): Getting custom field: ' . $meta_name );
				...
				switch ( KDNAFormsModel::get_input_type( $custom_field ) ) {
					case 'list' :
					...
					case 'date' :
						$value = KDNACommon::date_display( $value, rgar( $custom_field, 'dateFormat' ) );
						if ( ! rgblank( $value ) ) {
		...
		if ( is_object( $post ) ) {
			KDNACommon::log_debug( 'KDNAFormsModel::create_post(): Updating post.' );
			wp_update_post( $post );
		}
		...
		KDNACommon::log_debug( 'KDNAFormsModel::create_post(): Updating entry with post id.' );
		...
		KDNACommon::log_debug( __METHOD__ . sprintf( '(): Post creation completed in %F seconds.', KDNACommon::timer_end( __METHOD__ ) ) );
	...
	public static function process_post_template( $template, $field_type, $post_images, $post_data, $form, $entry ) {
		KDNACommon::log_debug( __METHOD__ . "(): Processing {$field_type} template." );

		//replacing post image variables
		$template = KDNACommon::replace_variables_post_image( $template, $post_images, $entry );

		//replacing all other variables
		$template = KDNACommon::replace_variables( $template, $form, $entry, false, false, false );
		...
		if ( $field_type != 'post_content' ) {
			 ...
			 * @since 2.0.0.4
			 */
			$process_template_shortcodes = apply_filters( 'kdnaform_process_template_shortcodes_pre_create_post', $process_template_shortcodes, $field_type, $post_data, $form, $entry );
			$process_template_shortcodes = apply_filters( 'kdnaform_process_template_shortcodes_pre_create_post_' . $form['id'], $process_template_shortcodes, $field_type, $post_data, $form, $entry );
	...
	private static function copy_post_image( $url, $post_id ) {
		...
		// the source path
		$upload_root_info = KDNA_Field_FileUpload::get_upload_root_info( $form_id );
	...
	public static function media_handle_upload( $url, $post_id, $post_data = array() ) {
		...
		if ( ! $file ) {
			KDNACommon::log_debug( __METHOD__ . '(): Image could not be copied to the media directory.' );
		...
		KDNACommon::log_debug( __METHOD__ . '(): Image copied to the media directory. Result from wp_insert_attachment(): ' . print_r( $id, 1 ) );
	...
	public static function save_input( $form, $field, &$lead, $current_fields, $input_id ) {
			...
			foreach( $field->fields as $sub_field ) {
				...
				foreach ( $current_fields as $current_field ) {
					if ( intval( $current_field->meta_key ) == $sub_field->id && ! isset( $current_field->update ) ) {
						...
						KDNACommon::log_debug( __METHOD__ . "(): Deleting: {$field->label}(#{$sub_field->id}{$current_field->item_index} - {$field->type}). Result: " . var_export( $result, 1 ) ); // phpcs:ignore QITStandard.PHP.DebugCode.DebugFunctionFound
					}
				}
			}
			return;
		...
		$type     = self::get_input_type( $field );
		$is_admin = KDNACommon::is_entry_detail();
	...
	public static function queue_save_input_value( $value, $form, $field, &$lead, $current_fields, $input_id, $item_index = '' ) {
		...
		if ( is_array( $value ) && ! ( $field->is_value_submission_array() && ! is_array( rgar( $value, 0 ) ) ) ) {
			foreach ( $value as $i => $v ) {
				...
				KDNACommon::log_debug( __METHOD__ . "(): Saving: {$field->label}(#{$input_id}{$item_index} - {$field->type}). Result: " . var_export( $result, 1 ) ); // phpcs:ignore QITStandard.PHP.DebugCode.DebugFunctionFound
				foreach ( $current_fields as $current_field ) {
			...
			KDNACommon::log_debug( __METHOD__ . "(): Queued field operation: {$field->label}(#{$input_id} - {$field->type})." );
	...
	public static function update_entry_field_value( $form, $entry, $field, $entry_meta_id, $input_id, $value, $item_index = '' ) {
		 ...
		 * @param array $entry The current entry object.
		 * @param KDNA_Field $field The current field object.
		 * @param array $form The current form object.
		 * @param string $input_id The ID of the input being saved or the field ID for single input field types.
		 */
		$value = apply_filters( 'kdnaform_save_field_value', $value, $entry, $field, $form, $input_id );
		$value = apply_filters( "kdnaform_save_field_value_{$form['id']}", $value, $entry, $field, $form, $input_id );

		if ( is_object( $field ) ) {
			$value = apply_filters( "kdnaform_save_field_value_{$form['id']}_{$field->id}", $value, $entry, $field, $form, $input_id );
		}

		if ( is_array( $value ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): bailing. value is an array.' );
			return false;
		}
		...
		if ( ! rgblank( $value ) ) {
			...
			if ( empty( $entry_meta_id ) && $field instanceof KDNA_Field_Repeater && isset( $field->fields ) && is_array( $field->fields ) ) {
				foreach ( $field->fields as $subfield ) {
					self::update_entry_field_value( $form, $entry, $subfield, 0, $subfield->id, '' );
				}
			} else {
				// Deleting details for this field
				if ( is_a( $field, 'KDNA_Field' ) && is_array( $field->get_entry_inputs() ) ) {
	...
	public static function queue_batch_field_operation( $form, &$entry, $field, $entry_meta_id, $input_id, $value, $item_index = '' ) {
		...
		} elseif ( $entry_meta_id > 0 && ! in_array( $input_id, KDNAFormsModel::get_lead_db_columns() ) ) {
	...
	private static function move_temp_file( $form_id, $tempfile_info ) {
		_deprecated_function( 'move_temp_file', '1.9', 'KDNA_Field_Fileupload::move_temp_file' );
	...
	public static function set_permissions( $path ) {
		$permission = apply_filters( 'kdnaform_file_permission', 0644, $path );
		if ( $permission ) {
	...
	public static function upload_file( $form_id, $file ) {
		_deprecated_function( 'upload_file', '1.9', 'KDNA_Field_Fileupload::upload_file' );
		$target = self::get_file_upload_path( $form_id, $file['name'] );
		if ( ! $target ) {
			KDNACommon::log_debug( 'KDNAFormsModel::upload_file(): FAILED (Upload folder could not be created.)' );
		...
		if ( move_uploaded_file( $file['tmp_name'], $target['path'] ) ) {
			KDNACommon::log_debug( 'KDNAFormsModel::upload_file(): Setting permissions on ' . $target['path'] );
			self::set_permissions( $target['path'] );

			return $target['url'];
		} else {
			KDNACommon::log_debug( 'KDNAFormsModel::upload_file(): FAILED (Temporary file could not be copied.)' );
	...
	public static function get_upload_url( $form_id ) {
		...
		return $dir['baseurl'] . "/kdna_forms/$form_id" . '-' . wp_hash( $form_id );
	}
	...
	public static function get_file_upload_path( $form_id, $file_name, $increment_found = true ) {
		...
		$default_upload_root_info =  KDNA_Field_FileUpload::get_default_upload_roots( $form_id );
		...
		if ( ! is_dir( $target_root ) ) {
			...
			if ( $default_target_root != $target_root && ! file_exists( $target_root . 'index.html' ) ) {
				KDNACommon::recursive_add_index_file( $target_root );
			} elseif ( ! file_exists( self::get_upload_root() . '/index.html' ) ) {
				KDNACommon::recursive_add_index_file( self::get_upload_root() );
			} elseif ( ! file_exists( self::get_upload_path( $form_id ) . '/index.html' ) ) {
				KDNACommon::recursive_add_index_file( self::get_upload_path( $form_id ) );
			} elseif ( ! file_exists( self::get_upload_path( $form_id ) . "/$y/index.html" ) ) {
				KDNACommon::recursive_add_index_file( self::get_upload_path( $form_id ) . "/$y" );
			} else {
				KDNACommon::recursive_add_index_file( self::get_upload_path( $form_id ) . "/$y/$m" );
			}
		}
	...
	/**
	 * Target for the wpmu_drop_tables filter. Adds all tables for KDNA Forms and the Add-On Framework to list
	...
	public static function is_duplicate( $form_id, $field, $value ) {
		...
		switch ( KDNAFormsModel::get_input_type( $field ) ) {
			...
			case 'date':
				...
				break;
			case 'number':
				$value = KDNACommon::clean_number( $value, $field->numberFormat );
				break;
			case 'phone':
	...
	public static function get_entry( $entry_id ) {
		return KDNAAPI::get_entry( $entry_id );
	}
	...
	public static function refresh_lead_field_value( $lead_id, $field_id ) {
		if ( version_compare( KDNAForms::$version, '2.3-dev', '>=' ) ) {
			_deprecated_function( 'KDNAFormsModel::refresh_lead_field_value', '2.3' );
		}
		$cache_key = 'KDNAFormsModel::get_lead_field_value_' . $lead_id . '_' . $field_id;
		KDNACache::delete( $cache_key );
	}

	/**
	 * @param $lead
	 * @param $field KDNA_Field
	 *
	...
	public static function get_lead_field_value( $lead, $field ) {
		...
		$field_id = $field instanceof KDNA_Field ? $field->id : rgar( $field, 'id' );

		$value = array();

		$inputs = $field instanceof KDNA_Field ? $field->get_entry_inputs() : rgar( $field, 'inputs' );
		...
		$value = apply_filters( 'kdnaform_get_field_value', $value, $lead, $field );
	...
	public static function get_leads( $form_id, $sort_field_number = 0, $sort_direction = 'DESC', $search = '', $offset = 0, $page_size = 30, $star = null, $read = null, $is_numeric_sort = false, $start_date = null, $end_date = null, $status = 'active', $payment_status = false ) {
		...
		_deprecated_function( 'KDNAFormsModel::get_leads', '2.3', 'KDNAAPI::get_entries' );
		...
		return KDNAAPI::get_entries( $form_id, $search_criteria, $sorting, $paging );
	}
	...
	/***
	 * Saves the KDNA Forms license key to the database and registers the site and license key with the KDNA Forms licensing server.
	 *
	 * @since 1.0
	 *
	 * @param string $new_key KDNA Forms license key to be saved.
	 */
	...
	/**
	 * Use KDNAAPI::count_entries() instead.
	 *
	 * @deprecated 2.3.0.1
	 * @remove-in 3.0
	 *
	...
	public static function get_lead_count( $form_id, $search, $star = null, $read = null, $start_date = null, $end_date = null, $status = null, $payment_status = null ) {
		...
		_deprecated_function( 'KDNAFormsModel::get_lead_count', '2.3.0.1', 'KDNAAPI::count_entries');
	 ...
	 *
	 * This function is not used and is only included for backwards compatibility. Use KDNAAPI::count_entries() instead.
	 *
	 * @deprecated 2.3.0.1
	 * @remove-in 3.0
	 *
	...
	public static function get_entries_where_sql( $args ) {
		...
		_doing_it_wrong( 'KDNAFormsModel::get_entries_where_sql', 'Use KDNAAPI::count_entries instead', '2.3.0.1');
	...
	public static function get_grid_columns( $form_id, $input_label_only = false ) {
		...
		if ( ! is_array( $field_ids ) ) {
			...
			$entry_metas = KDNAFormsModel::get_entry_meta( $form_id );
		...
		foreach ( $field_ids as $field_id ) {
			...
			switch ( $field_id ) {
				case 'id' :
					$columns[ $field_id ] = array( 'label' => esc_html__( 'Entry Id', 'kdnaforms' ), 'type' => 'id' );
					break;
				case 'ip' :
					$columns[ $field_id ] = array( 'label' => esc_html__( 'User IP', 'kdnaforms' ), 'type' => 'ip' );
					break;
				case 'date_created' :
					$columns[ $field_id ] = array( 'label' => esc_html__( 'Entry Date', 'kdnaforms' ), 'type' => 'date_created' );
					break;
				case 'source_url' :
					$columns[ $field_id ] = array( 'label' => esc_html__( 'Source Url', 'kdnaforms' ), 'type' => 'source_url' );
					break;
				case 'payment_status' :
					$columns[ $field_id ] = array( 'label' => esc_html__( 'Payment Status', 'kdnaforms' ), 'type' => 'payment_status' );
					break;
				case 'transaction_id' :
					$columns[ $field_id ] = array( 'label' => esc_html__( 'Transaction Id', 'kdnaforms' ), 'type' => 'transaction_id' );
					break;
				case 'payment_date' :
					$columns[ $field_id ] = array( 'label' => esc_html__( 'Payment Date', 'kdnaforms' ), 'type' => 'payment_date' );
					break;
				case 'payment_amount' :
					$columns[ $field_id ] = array( 'label' => esc_html__( 'Payment Amount', 'kdnaforms' ), 'type' => 'payment_amount' );
					break;
				case 'created_by' :
				...
				default :
					...
					if ( $field ) {
						if ( $field->type === 'consent' ) {
							...
							$input_label_only               = apply_filters( 'kdnaform_entry_list_column_input_label_only', $input_label_only, $form, $field );
	 ...
	 * @since 2.4
	 *
	 * @param KDNA_Field_Repeater $repeater_field The repeater field.
	 * @param int $field_id                     The field ID.
	 *
	 * @return null|KDNA_Field
	 */
	 ...
	 *
	 * @param array $form The form being processed or an empty array to get the form based on the ID from the kdnaform_submit input.
	 *
	 * @return false|array
	 */
	...
	public static function convert_confirmation( $form ) {
		...
		if ( ! empty( $confirmation['type'] ) ) {
			...
			$confirmation['name']      = esc_html__( 'Default Confirmation', 'kdnaforms' );
		...
		switch ( $event ) {
			case 'form_saved':
				return array(
					'id'          => uniqid( 'sc1' ),
					'event'       => 'form_saved',
					'name'        => __( 'Save and Continue Confirmation', 'kdnaforms' ),
					'isDefault'   => true,
					'type'        => 'message',
					'message'     => sprintf(
						'<h2>%s</h2><p role="alert">%s</p><p class="resume_form_link_wrapper">{save_link}</p><p>%s<br />%s</p>{save_email_input}',
						__( 'Link to continue editing later', 'kdnaforms' ),
						__( 'Please use the following link to return and complete this form from any computer.', 'kdnaforms' ),
						__( 'Note: This link will expire after 30 days.', 'kdnaforms' ),
						__( 'Enter your email address if you would like to receive the link via email.', 'kdnaforms' )
					),
					'url'         => '',
					'pageId'      => '',
					'queryString' => '',
			...
			case 'form_save_email_sent':
				return array(
					...
					'name'        => __( 'Save and Continue Email Sent Confirmation', 'kdnaforms' ),
					'isDefault'   => true,
					'type'        => 'message',
					'message'     => sprintf(
						'<h2 class="saved_message_success">%s</h2><p>%s <span class="saved_message_email">{save_email}</span></p>',
						__( 'Success!', 'kdnaforms' ),
						__( 'The link was sent to the following email address:', 'kdnaforms' )
					),
					'url'         => '',
					'pageId'      => '',
					'queryString' => '',
			...
			default:
				return array(
					'id'          => uniqid(),
					'name'        => __( 'Default Confirmation', 'kdnaforms' ),
					'isDefault'   => true,
					'type'        => 'message',
					'message'     => __( 'Thanks for contacting us! We will get in touch with you shortly.', 'kdnaforms' ),
					'url'         => '',
					'pageId'      => '',
					'queryString' => '',
				);
	...
	public static function get_form_confirmations( $form_id ) {
		...
		$tablename     = KDNAFormsModel::get_meta_table_name();
	...
	public static function delete_form_confirmation( $confirmation_id, $form_id ) {
		 ...
		 * @param array $form The Form object.
		 */
		do_action( 'kdnaform_pre_confirmation_deleted', $form['confirmations'][ $confirmation_id ], $form );
	...
	/**
	 * Remove corrupt confirmations created by old versions of KDNAFormsModel::convert_confirmation().
	 *
	 * @since 2.4.15
	 *
	...
	public static function get_form_ids( $active = true, $trash = false, $sort_column = 'id', $sort_dir = 'ASC' ) {
		...
		if ( ! in_array( strtolower( $sort_column ), KDNAFormsModel::get_form_db_columns() ) ) {
			$sort_column = 'id';
		}
	 ...
	 *
	 * @see KDNAFormsModel::get_form_ids()
	 * @since 2.7.5
	 *
	 ...
	 * @param string $sort_column The column to sort the results on.
	 * @param string $sort_dir    The sort direction, ASC or DESC.
	 * @param array  $columns     The columns to return. Defaults to ['id']. Other options are 'title', 'date_created', 'is_active', 'is_trash'. 'date_updated' may be supported, depending on the KDNA Forms version.
	 *
	...
	public static function get_forms_columns( $active = true, $trash = false, $sort_column = 'id', $sort_dir = 'ASC', $columns = array( 'id' ) ) {
		...
		$columns = array_intersect( $columns, KDNAFormsModel::get_form_db_columns() );

		$sql   = 'SELECT ' . implode( ', ', $columns ) . ' FROM ' . KDNAFormsModel::get_form_table_name();
		$where = array();
	...
	public static function get_entry_meta( $form_ids ) {
		...
		foreach ( $form_ids as $form_id ) {
			if ( ! isset( $_entry_meta[ $form_id ] ) ) {
				$_entry_meta           = array();
				$_entry_meta[ $form_id ] = apply_filters( 'kdnaform_entry_meta', array(), $form_id );
			}
	...
	public static function set_entry_meta( $lead, $form ) {
		...
		foreach ( $keys as $key ) {
			if ( isset( $entry_meta[ $key ]['update_entry_meta_callback'] ) ) {
				...
				kdnaform_update_meta( $entry_id, $key, $value );
				$lead[ $key ] = $value;
			}
		}

		KDNACommon::log_debug( __METHOD__ . sprintf( '(): Saving meta for entry (#%d) completed in %F seconds.', $entry_id, KDNACommon::timer_end( __METHOD__ ) ) );
	...
	public static function search_lead_ids( $form_id, $search_criteria = array() ) {
		return KDNAAPI::get_entry_ids( $form_id, $search_criteria );
	}

	/**
	 * Returns the kdna_entry table field names.
	 *
	...
	public static function count_search_leads( $form_id, $search_criteria = array() ) {
		return KDNAAPI::count_entries( $form_id, $search_criteria );
	}
	...
	public static function get_entry_count_all_forms( $status = 'active' ) {
		...
		if ( ! KDNACommon::table_exists( $entry_table_name ) ) {
			return 0;
		}
	...
	public static function get_entry_meta_counts() {
		...
		if ( ! KDNACommon::table_exists( $meta_table_name ) ) {
			return array(
				'details' => 0,
				'meta'    => 0,
				'notes'   => 0,
	...
	/**
	 * @deprecated 2.2 Use kdna_upgrade()->dbDelta() instead
	 * @remove-in 3.0
	 */
	public static function dbDelta( $sql ) {
		_deprecated_function( 'dbDelta', '2.2', 'kdna_upgrade()->dbDelta()' );

		kdna_upgrade()->dbDelta( $sql );
	...
	public static function is_valid_table( $table_name ){
		...
		$tables = array(
			KDNAFormsModel::get_form_table_name(),
			KDNAFormsModel::get_form_view_table_name(),
			KDNAFormsModel::get_meta_table_name(),
			KDNAFormsModel::get_lead_table_name(),
			KDNAFormsModel::get_lead_notes_table_name(),
			KDNAFormsModel::get_lead_details_table_name(),
			KDNAFormsModel::get_lead_details_long_table_name(),
			...
			KDNAFormsModel::get_entry_meta_table_name(),
			KDNAFormsModel::get_draft_submissions_table_name(),
		);
	...
	public static function trim_form_meta_values( $form, &$form_updated = false ) {
		$form_id = $form['id'];
		KDNACommon::log_debug( 'KDNAFormsModel::trim_form_meta_values(): Starting.' );
		if ( isset( $form['fields'] ) && is_array( $form['fields'] ) ) {
			...
			$form['fields'] = KDNAFormsModel::trim_conditional_logic_values( $form['fields'], $form, $form_updated );
		}
		if ( $form_updated ) {
			KDNACommon::log_debug( 'KDNAFormsModel::trim_form_meta_values(): Form values trimmed.' );
		}
	...
	public static function trim_conditional_logic_values( $meta_array, $form, &$updated = false ) {
		KDNACommon::log_debug( 'KDNAFormsModel::trim_conditional_logic_values(): Starting.' );
		...
		if ( $updated ) {
			KDNACommon::log_debug( 'KDNAFormsModel::trim_conditional_logic_values(): Conditional logic values trimmed.' );
		}
	...
	public static function trim_conditional_logic_values_from_element( $element, $form = array(), &$updated = false ) {
		if ( $element instanceof KDNA_Field ) {

			/* @var KDNA_Field $element */
			if ( is_array( $element->conditionalLogic ) && isset( $element->conditionalLogic['rules'] ) && is_array( $element->conditionalLogic['rules'] ) ) {
				foreach ( $element->conditionalLogic['rules'] as &$rule ) {
					...
					if ( $value !== trim( $value ) ) {
						$field      = isset( $form['fields'] ) ? KDNAFormsModel::get_field( $form, $rule['fieldId'] ) : array();
						$trim_value = apply_filters( 'kdnaform_trim_input_value', true, rgar( $form, 'id' ), $field );
						if ( $trim_value ) {
	...
	/**
	 * Returns an array of field IDs that have been encrypted using KDNACommon::encrypt()
	 *
	 * @deprecated
	 * @remove-in 3.0
	 *
	...
	public static function get_encrypted_fields( $entry_id ) {
		...
		_deprecated_function( 'KDNACommon:get_encrypted_fields', '2.3', 'KDNACommon:get_openssl_encrypted_fields' );

		$encrypted_fields = kdnaform_get_meta( $entry_id, '_encrypted_fields' );
	...
	/**
	 * Stores the field IDs that have been encrypted using KDNACommon::encrypt()
	 *
	 * @deprecated
	 * @remove-in 3.0
	 *
	...
	public static function set_encrypted_fields( $entry_id, $field_ids ) {
		...
		_deprecated_function( 'KDNACommon:set_encrypted_fields', '2.3', 'KDNACommon:set_openssl_encrypted_fields' );
		...
		kdnaform_update_meta( $entry_id, '_encrypted_fields', $encrypted_fields );

		return true;
	}

	/**
	 * Checks whether the given field was encrypted using KDNACommon::encrpyt() and registered using KDNACommon::set_encrypted_fields()
	 *
	 * @deprecated
	 * @remove-in 3.0
	 *
	...
	public static function is_encrypted_field( $entry_id, $field_id ) {
		...
		_deprecated_function( 'KDNACommon:is_encrypted_field', '2.3', 'KDNACommon:is_openssl_encrypted_field' );
		 ...
		 */
		$is_encrypted = apply_filters('kdnaform_is_encrypted_field', '', $entry_id, $field_id );
		if (  $is_encrypted !== '' ){
			return $is_encrypted;
		}
	...
	/**
	 * Returns an array of field IDs that have been encrypted using KDNACommon::openssl_encrypt()
	 *
	 * @since 2.3
	 *
	 * @param $entry_id
	...
	public static function get_openssl_encrypted_fields( $entry_id ) {
		...
		$encrypted_fields = kdnaform_get_meta( $entry_id, '_openssl_encrypted_fields' );
	 ...
	 *
	 * @return array Returns an array with entry_id as the key and an array of field IDs (that have been encrypted using KDNACommon::openssl_encrypt()) as the value.
	 */
	...
	/**
	 * Adds the field IDs that have been encrypted using KDNACommon::encrypt(). Merges the new IDs with the existing IDs.
	 *
	 * @since 2.3
	 *
	 * @param $entry_id
	...
	public static function set_openssl_encrypted_fields( $entry_id, $field_ids ) {
		...
		kdnaform_update_meta( $entry_id, '_openssl_encrypted_fields', $encrypted_fields );
	...
	public static function sanitize_settings( $form ) {
		...
		$form['version'] = KDNAForms::$version;

		if ( apply_filters( 'kdnaform_disable_form_settings_sanitization', false ) ) {
			return $form;
		}
		...
		if ( isset( $form['labelPlacement'] ) ) {
			$form['labelPlacement'] = KDNACommon::whitelist( $form['labelPlacement'], array( 'top_label', 'left_label', 'right_label' ) );
		}

		if ( isset( $form['descriptionPlacement'] ) ) {
			$form['descriptionPlacement'] = KDNACommon::whitelist( $form['descriptionPlacement'], array( 'below', 'above' ) );
		}

		if ( isset( $form['validationPlacement'] ) ) {
			$form['validationPlacement'] = KDNACommon::whitelist( $form['validationPlacement'], array( 'below', 'above' ) );
		}

		if ( isset( $form['subLabelPlacement'] ) ) {
			$form['subLabelPlacement']    = KDNACommon::whitelist( $form['subLabelPlacement'], array( 'below', 'above' ) );
		}
		...
		if ( isset( $form['honeypotAction'] ) ) {
			$form['honeypotAction']  = KDNACommon::whitelist( $form['honeypotAction'], array( 'abort', 'spam' ) );
		}
		...
		if ( isset( $form['button'] ) ) {
			$form['button']['type']     = KDNACommon::whitelist( $form['button']['type'], array( 'text', 'image' ) );
		...
		if ( isset( $form['limitEntries'] ) ) {
			...
			$form['limitEntriesPeriod']  = $form['limitEntries'] ? KDNACommon::whitelist( $form['limitEntriesPeriod'], array( '', 'day', 'week', 'month', 'year' ) ) : '';
		...
		if ( isset( $form['scheduleForm'] ) ) {
			...
			$form['scheduleStartHour']      = $form['scheduleForm'] ? KDNACommon::int_range( $form['scheduleStartHour'], 1, 12 ) : '';
			$form['scheduleStartMinute']    = $form['scheduleForm'] ? KDNACommon::int_range( $form['scheduleStartMinute'], 0, 59 ) : '';
			$form['scheduleStartAmpm']      = $form['scheduleForm'] ? KDNACommon::whitelist( $form['scheduleStartAmpm'], array( 'am', 'pm' ) ) : '';
			$form['scheduleEnd']            = $form['scheduleForm'] ? wp_strip_all_tags( $form['scheduleEnd'] ) : '';
			$form['scheduleEndHour']        = $form['scheduleForm'] ? KDNACommon::int_range( $form['scheduleEndHour'], 1, 12 ) : '';
			$form['scheduleEndMinute']      = $form['scheduleForm'] ? KDNACommon::int_range( $form['scheduleEndMinute'], 0, 59 ) : '';
			$form['scheduleEndAmpm']        = $form['scheduleForm'] ? KDNACommon::whitelist( $form['scheduleEndAmpm'], array( 'am', 'pm' ) ) : '';
	...
	private static function maybe_wp_kses( $html, $allowed_html = 'post', $allowed_protocols = array() ) {
		return KDNACommon::maybe_wp_kses( $html, $allowed_html, $allowed_protocols );
	}
	 ...
	 *
	 * @since  2.0.7.20 Refactored and added filter kdnaform_is_valid_conditional_logic_operator.
	 * @access public
	 *
	...
	public static function is_valid_operator( $operator ) {
		 ...
		 */
		return apply_filters( 'kdnaform_is_valid_conditional_logic_operator', $is_valid, $operator );
	}
	...
	public static function update_recent_forms( $form_id, $trashed = false ) {
		...
		update_user_meta( $current_user_id, 'kdnaform_recent_forms', $recent_form_ids );
	}
	...
	public static function get_recent_forms( $current_user_id = 0 ) {
		...
		$recent_form_ids = get_user_meta( $current_user_id, 'kdnaform_recent_forms', true );
	 ...
	 * NOTE: There is a future refactoring opportunity to reduce code duplication by merging this method with KDNACommon::evaluate_conditional_logic(), which currently handles non-field conditional logic (i.e. notification, confirmation and feed). A possible solution is for this method to call KDNACommon::evaluate_conditional_logic().
	 *
	 * @param array $form         The current Form object.
	...
	private static function evaluate_conditional_logic( $form, $logic, $field_values, $entry = null ) {
		...
		foreach ( $logic['rules'] as $rule ) {
			try {
				 ...
				 */
				$rule = apply_filters( 'kdnaform_rule_pre_evaluation', $rule, $form, $logic, $field_values, $entry );
			} catch ( Error $e ) {
				KDNACommon::log_error( __METHOD__ . '(): Error from function hooked to kdnaform_rule_pre_evaluation. ' . $e->getMessage() );
			}
			$source_field   = KDNAFormsModel::get_field( $form, $rule['fieldId'] );
			...
			$source_value = KDNACommon::maybe_format_numeric( $source_value, $rule['operator'], $number_format );
				...
				if ( isset( $input_files[0] ) && is_array( $input_files[0] ) ) {
					foreach ( $input_files as $key => &$file ) {
						...
						if ( isset( $file['id'] ) ) {
							...
							$file['id'] = KDNAFormsModel::get_uuid();
						}
					}
				}
			} else {
	 ...
	 *
	 * @since 2.4.6
	 * @since 2.4.24 Updated to use KDNAFormsModel::id_exists_in_table().
	 *
	...
	public static function entry_exists( $entry_id ) {
		return self::id_exists_in_table( $entry_id, KDNAFormsModel::get_entry_table_name() );
	}
	...
	public static function update_feed_property( $feed_id, $property_name, $property_value ) {
		if ( $property_name === 'id' ) {
			return new WP_Error( 'invalid_property', __( 'Updating the id property is not supported', 'kdnaforms' ) );
		}

		if ( ! in_array( $property_name, self::get_addon_feed_db_columns() ) ) {
			return new WP_Error( 'invalid_property', sprintf( __( '%s is not a valid feed property', 'kdnaforms' ), $property_name ) );
		}
		...
		if ( $property_name === 'meta' ) {
			...
			if ( ! $is_valid_format ) {
				return new WP_Error( 'invalid_meta', __( 'Feed meta should be an associative array or JSON', 'kdnaforms' ) );
			}

			$feed = KDNAAPI::get_feed( $feed_id );
			if ( is_wp_error( $feed ) ) {
				return $feed;
			}
			$meta           = is_array( $property_value ) ? $property_value : json_decode( $property_value, true );
			$encrypted_meta = KDNAAPI::encrypt_feed_meta( $meta, $feed['addon_slug'] );
		...
		if ( $result === false ) {
			return new WP_Error( 'error_updating', sprintf( __( 'There was an error while updating feed id %s', 'kdnaforms' ), $feed_id ) );
		}

		if ( $result === 0 && ! self::id_exists_in_table( $feed_id, $table ) ) {
			return new WP_Error( 'not_found', sprintf( __( 'Feed id %s not found', 'kdnaforms' ), $feed_id ) );
		}
 ...
 * @since unknown
 *
 * @global array $_kdnaform_lead_meta
 */
global $_kdnaform_lead_meta;
$_kdnaform_lead_meta = array();
...
function kdnaform_get_meta( $entry_id, $meta_key ) {
	...
	if ( array_key_exists( $cache_key, $_kdnaform_lead_meta ) ) {
		return maybe_unserialize( $_kdnaform_lead_meta[ $cache_key ] );
	}
	...
	$_kdnaform_lead_meta[ $cache_key ] = $meta_value;

	return $meta_value;
}

function kdnaform_get_meta_values_for_entries( $entry_ids, $meta_keys ) {
	global $wpdb;
 ...
 */
function kdnaform_update_meta( $entry_id, $meta_key, $meta_value, $form_id = null ) {
	global $wpdb, $_kdnaform_lead_meta;
	...
	$meta_exists = kdnaform_get_meta( $entry_id, $meta_key ) !== false;
	if ( $meta_exists ) {
		...
		if ( empty( $form_id ) ) {
			$entry_table_name = KDNAFormsModel::get_entry_table_name();
 ...
 * @return int|false           Returns the number of affected rows, or false if the operation fails.
 */
function kdnaform_add_meta( $entry_id, $meta_key, $meta_value, $form_id = null ) {
	global $wpdb, $_kdnaform_lead_meta;
 ...
 */
function kdnaform_delete_meta( $entry_id, $meta_key = '' ) {
	global $wpdb, $_kdnaform_lead_meta;
