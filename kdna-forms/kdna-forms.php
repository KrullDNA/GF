...
Requires PHP: 7.4
Author: KrullDNA
Author URI: https://kdnaforms.com
License: GPL-2.0+
Text Domain: kdnaforms
Domain Path: /languages

------------------------------------------------------------------------
Copyright 2026 KrullDNA
...
use KDNA_Forms\KDNA_Forms\Libraries\Dom_Parser;

//------------------------------------------------------------------------------------------------------------------
//---------- reCAPTCHA Keys -----------------------------------------------------------------------------
//------------------------------------------------------------------------------------------------------------------
// If you hardcode your reCAPTCHA Keys here, it will automatically populate on activation.
$kdna_recaptcha_private_key = '';
$kdna_recaptcha_public_key  = '';
...
// define('KDNA_RECAPTCHA_SITE_KEY','YOUR_SITE_KEY_GOES_HERE');
// define('KDNA_RECAPTCHA_SECRET_KEY','YOUR_SECRET_KEY_GOES_HERE');
//------------------------------------------------------------------------------------------------------------------
...
/**
 * Defines the current view within KDNA Forms.
 *
 ...
 */
define( 'RG_CURRENT_VIEW', KDNAForms::get( 'view' ) );

/**
 * Defines the minimum version of WordPress required for KDNA Forms.
 *
 * @since   Unknown
 *
 * @var string KDNA_MIN_WP_VERSION Minimum version number.
 */
define( 'KDNA_MIN_WP_VERSION', '6.5' );
 ...
 * @since   Unknown
 *
 * @var boolean KDNA_SUPPORTED_VERSION True if supported.  False otherwise.
 */
define( 'KDNA_SUPPORTED_WP_VERSION', version_compare( get_bloginfo( 'version' ), KDNA_MIN_WP_VERSION, '>=' ) );

/**
 * Defines the minimum version of WordPress that will be officially supported.
 *
 * @var string KDNA_MIN_WP_VERSION_SUPPORT_TERMS The version number
 */
define( 'KDNA_MIN_WP_VERSION_SUPPORT_TERMS', '6.8' );
 ...
 * @since 2.9.4
 *
 * @var string KDNA_MIN_PHP_VERSION The version number
 */
define( 'KDNA_MIN_PHP_VERSION', '7.4' );
 ...
 * @var string
 */
define( 'KDNA_PLUGIN_DIR_PATH', plugin_dir_path( __FILE__ ) );
 ...
 * @var string
 */
define( 'KDNA_PLUGIN_BASENAME', plugin_basename( __FILE__ ) );


require_once KDNA_PLUGIN_DIR_PATH . 'includes/class-kdna-service-container.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/class-kdna-service-provider.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/transients/interface-kdna-transient-strategy.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/transients/class-kdna-wp-transient-strategy.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/external-api/interface-kdna-api-response-factory.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/external-api/class-kdna-api-connector.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/external-api/class-kdna-api-response.php';
...
require_once KDNA_PLUGIN_DIR_PATH . 'includes/api.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/webapi/webapi.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/fields/class-kdna-fields.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/class-kdna-download.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/query/class-kdna-query.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/assets/class-kdna-asset.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/assets/class-kdna-script-asset.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/assets/class-kdna-style-asset.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/trait-redirects-on-save.php';
require_once KDNA_PLUGIN_DIR_PATH . 'includes/messages/class-dismissable-messages.php';
...
// Load built-in reCAPTCHA integration
require_once KDNA_PLUGIN_DIR_PATH . 'includes/recaptcha/recaptcha-bootstrap.php';

// Load Elementor integration
require_once KDNA_PLUGIN_DIR_PATH . 'includes/elementor/class-kdna-elementor-loader.php';

add_action( 'init', array( 'KDNAForms', 'init' ) );
...
add_action( 'wp', array( 'KDNAForms', 'process_exterior_pages' ) );
add_action( 'admin_init', array( 'KDNAForms', 'process_exterior_pages' ) );
add_action( 'admin_head', array( 'KDNACommon', 'find_admin_notices' ) );
add_action( 'admin_head', array( 'KDNACommon', 'admin_notices_style' ) );
add_action( 'upgrader_process_complete', array( 'KDNAForms', 'install_addon_translations' ), 10, 2 );
add_action( 'update_option_WPLANG', array( 'KDNAForms', 'update_translations' ), 10, 2 );
add_action( 'plugins_loaded', array( 'KDNAForms', 'register_services' ), 10, 0 );
add_action( 'init', array( 'KDNAForms', 'register_image_sizes' ) );
add_action( 'init', array( 'KDNAForms', 'init_buffer' ) );
add_filter( 'upgrader_pre_install', array( 'KDNAForms', 'validate_upgrade' ), 10, 2 );
add_filter( 'tiny_mce_before_init', array( 'KDNAForms', 'modify_tiny_mce_4' ), 20 );
add_filter( 'user_has_cap', array( 'KDNAForms', 'user_has_cap' ), 10, 4 );
add_filter( 'plugin_auto_update_setting_html', array( 'KDNAForms', 'auto_update_message' ), 9, 3 );
...
register_activation_hook( __FILE__, array( 'KDNAForms', 'activation_hook' ) );
register_deactivation_hook( __FILE__, array( 'KDNAForms', 'deactivation_hook' ) );
 ...
 * Class KDNAForms
 *
 * Handles the loading of KDNA Forms and other core functionality
 */
class KDNAForms {

	/**
	* @var \KDNA_Forms\KDNA_Forms\KDNA_Service_Container $container
	*/
	private static $container;

	/**
	 * Defines this version of KDNA Forms.
	 *
	 * @since  Unknown
	 *
	...
	public static function loaded() {
		...
		require_once( KDNACommon::get_base_path() . '/includes/settings/class-settings.php' );

		/**
		 * Fires when KDNA Forms has loaded.
		 *
		 * When developing Add-Ons, use this hook to initialize any functionality that depends on KDNA Forms functionality.
		 */
		do_action( 'kdnaform_loaded' );
	...
	* @since 2.5.11
	*
	* @return \KDNA_Forms\KDNA_Forms\KDNA_Service_Container
	*/
	public static function get_service_container() {
		require_once KDNA_PLUGIN_DIR_PATH . 'includes/duplicate-submissions/class-kdna-duplicate-submissions-service-provider.php';
		require_once KDNA_PLUGIN_DIR_PATH . 'includes/util/class-kdna-util-service-provider.php';
		require_once KDNA_PLUGIN_DIR_PATH . 'includes/config/class-kdna-config-service-provider.php';
        require_once KDNA_PLUGIN_DIR_PATH . 'includes/editor-button/class-kdna-editor-service-provider.php';
		...
		require_once KDNA_PLUGIN_DIR_PATH . 'includes/settings/class-kdna-settings-service-provider.php';
		require_once KDNA_PLUGIN_DIR_PATH . 'includes/author-select/class-kdna-author-select-service-provider.php';
		...
		self::$container = new \KDNA_Forms\KDNA_Forms\KDNA_Service_Container();
	...
	/**
	 * Initializes KDNA Forms.
	 *
	 * @since Unknown
	 * @since 2.6.9 Moved background upgrader and feed processor init to \KDNA_Forms\KDNA_Forms\Async\KDNA_Background_Process_Service_Provider().
	 *
	 * @return void
	 */
	public static function init() {

		if ( ! wp_next_scheduled( 'kdnaforms_cron' ) ) {
			wp_schedule_event( time(), 'daily', 'kdnaforms_cron' );
		}

		add_action( 'kdnaforms_cron', array( 'KDNAForms', 'cron' ) );

		KDNA_Download::maybe_process();

		//load text domains
		KDNACommon::load_kdna_text_domain( 'kdnaforms' );
		...
		add_action( 'dynamic_sidebar_before', array( 'KDNACommon', 'check_for_kdna_widgets' ), 10 );
		add_action( 'kdnaform_enqueue_scripts', array( 'KDNACommon', 'localize_kdna_legacy_multi' ), 9999 );
		...
		KDNACommon::localize_kdnaform_i18n();

		// Maybe set up KDNA Forms
		...
		if ( function_exists( 'register_block_type' ) ) {
			...
			if ( ! class_exists( 'KDNA_Blocks' ) ) {
				require_once KDNA_PLUGIN_DIR_PATH . 'includes/blocks/class-kdna-blocks.php';
			}

			// Load included Blocks.
			KDNACommon::glob_require_once( '/includes/blocks/class-kdna-block-*.php' );

		}

		// Plugin update actions
		add_filter( 'transient_update_plugins', array( 'KDNAForms', 'check_update' ) );
		add_filter( 'site_transient_update_plugins', array( 'KDNAForms', 'check_update' ) );
		add_filter( 'auto_update_plugin', array( 'KDNAForms', 'maybe_auto_update' ), 10, 2 );

		if ( IS_ADMIN ) {

			global $current_user;

			//Members plugin integration. Adding KDNA Forms roles to the checkbox list
			if ( self::has_members_plugin() ) {
				add_action( 'members_register_cap_groups', array( 'KDNAForms', 'members_register_cap_group' ) );
				add_action( 'members_register_caps', array( 'KDNAForms', 'members_register_caps' ) );
			}

            // User Role Editor integration.
            add_filter( 'ure_capabilities_groups_tree', array( 'KDNAForms', 'filter_ure_capabilities_groups_tree' ) );
            add_filter( 'ure_custom_capability_groups', array( 'KDNAForms', 'filter_ure_custom_capability_groups' ), 10, 2 );

			if ( is_multisite() ) {
				add_filter( 'wpmu_drop_tables', array( 'KDNAFormsModel', 'mu_drop_tables' ) );
			}

			add_action( 'admin_enqueue_scripts', array( 'KDNAForms', 'enqueue_admin_scripts' ) );
			add_action( 'print_media_templates', array( 'KDNAForms', 'action_print_media_templates' ) );

			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				add_action( 'admin_footer', array( 'KDNAForms', 'deprecate_add_on_methods' ) );
			}

			//Loading KDNA Forms if user has access to any functionality
			if ( KDNACommon::current_user_can_any( KDNACommon::all_caps() ) ) {
				require_once( KDNACommon::get_base_path() . '/export.php' );
				KDNAExport::maybe_export();

				//Imports theme forms if configured to be automatic imported
				kdna_upgrade()->maybe_import_theme_forms();

				//creates the "Forms" left menu
				add_action( 'admin_menu', array( 'KDNAForms', 'create_menu' ) );

				// Add site-wide admin notices
				add_action( 'admin_notices', array( 'KDNAForms', 'action_admin_notices' ) );

				if ( KDNA_SUPPORTED_WP_VERSION ) {

					add_action( 'admin_footer', array( 'KDNAForms', 'check_upload_folder' ) );
					add_action( 'wp_dashboard_setup', array( 'KDNAForms', 'dashboard_setup' ) );
					...
					if ( self::is_kdna_page() ) {
						...
						add_action( 'wp_ajax_rg_change_input_type', array( 'KDNAForms', 'change_input_type' ) );
						add_action( 'wp_ajax_rg_refresh_field_preview', array( 'KDNAForms', 'refresh_field_preview' ) );
						add_action( 'wp_ajax_rg_add_field', array( 'KDNAForms', 'add_field' ) );
						add_action( 'wp_ajax_rg_duplicate_field', array( 'KDNAForms', 'duplicate_field' ) );
						add_action( 'wp_ajax_rg_delete_field', array( 'KDNAForms', 'delete_field' ) );
						add_action( 'wp_ajax_rg_delete_file', array( 'KDNAForms', 'delete_file' ) );
						add_action( 'wp_ajax_rg_ajax_get_form', array( 'KDNAForms', 'ajax_get_form' ) );
						add_action( 'wp_ajax_rg_select_export_form', array( 'KDNAForms', 'select_export_form' ) );
						add_action( 'wp_ajax_rg_start_export', array( 'KDNAForms', 'start_export' ) );
						add_action( 'wp_ajax_kdna_delete_custom_choice', array( 'KDNAForms', 'delete_custom_choice' ) );
						add_action( 'wp_ajax_kdna_save_custom_choice', array( 'KDNAForms', 'save_custom_choice' ) );
						add_action( 'wp_ajax_kdna_get_post_categories', array( 'KDNAForms', 'get_post_category_values' ) );
						add_action( 'wp_ajax_kdna_get_address_rule_values_select', array(
							'KDNAForms',
							'get_address_rule_values_select'
						) );
						add_action( 'wp_ajax_kdna_get_notification_post_categories', array(
							'KDNAForms',
							'get_notification_post_category_values'
						) );
						//add_action( 'wp_ajax_kdna_save_confirmation', array( 'KDNAForms', 'save_confirmation' ) );
						add_action( 'wp_ajax_kdna_delete_confirmation', array( 'KDNAForms', 'delete_confirmation' ) );
						add_action( 'wp_ajax_kdna_save_new_form', array( 'KDNAForms', 'save_new_form' ) );
						add_action( 'wp_ajax_kdna_save_title', array( 'KDNAForms', 'save_form_title' ) );

						//entry list ajax operations
						add_action( 'wp_ajax_rg_update_lead_property', array( 'KDNAForms', 'update_lead_property' ) );
						add_action( 'wp_ajax_delete-kdna_entry', array( 'KDNAForms', 'update_lead_status' ) );

						//form list ajax operations
						add_action( 'wp_ajax_rg_update_form_active', array( 'KDNAForms', 'update_form_active' ) );
						...
						add_action( 'wp_ajax_rg_captcha_image', array( 'KDNAForms', 'captcha_image' ) );

						//dashboard message "dismiss upgrade" link
						add_action( 'wp_ajax_rg_dismiss_upgrade', array( 'KDNAForms', 'dashboard_dismiss_upgrade' ) );

						// entry detail: resend notifications
						add_action( 'wp_ajax_kdna_resend_notifications', array( 'KDNAForms', 'resend_notifications' ) );

						// Shortcode UI
						add_action( 'wp_ajax_kdna_do_shortcode', array( 'KDNAForms', 'handle_ajax_do_shortcode' ) );

						// Export
						add_filter( 'wp_ajax_kdna_process_export', array( 'KDNAForms', 'ajax_process_export' ) );
						add_filter( 'wp_ajax_kdna_download_export', array( 'KDNAForms', 'ajax_download_export' ) );

						// Dismiss message
						add_action( 'wp_ajax_kdna_dismiss_message', array( 'KDNAForms', 'ajax_dismiss_message' ) );

						// Check background tasks for the system report
						add_action( 'wp_ajax_kdna_check_background_tasks', array( 'KDNAForms', 'check_background_tasks' ) );

						// Check status of upgrade
						add_action( 'wp_ajax_kdna_force_upgrade', array( 'KDNAForms', 'ajax_force_upgrade' ) );

						// Disable logging.
						add_action( 'wp_ajax_kdna_disable_logging', array( 'KDNAForms', 'ajax_disable_logging' ) );

						// Get change log.
						add_action( 'wp_ajax_kdna_get_changelog', array( 'KDNAForms', 'ajax_display_changelog' ) );

					}

					add_filter( 'plugins_api', array( 'KDNAForms', 'get_addon_info' ), 100, 3 );
					add_action( 'after_plugin_row', array( 'KDNAForms', 'plugin_row' ), 10, 2 );
					add_action( 'in_plugin_update_message-kdnaforms/kdnaforms.php', array( 'KDNAForms', 'in_plugin_update_message' ), 10, 2 );
					add_action( 'install_plugins_pre_plugin-information', array( 'KDNAForms', 'display_changelog' ), 9 );
					add_filter( 'plugin_action_links', array( 'KDNAForms', 'plugin_settings_link' ), 10, 2 );
				}
			}
			add_action( 'admin_init', array( 'KDNAForms', 'ajax_parse_request' ), 10 );

			add_filter(	'wp_privacy_personal_data_exporters', array( 'KDNAForms', 'register_data_exporter' ),	10 );
			add_filter(	'wp_privacy_personal_data_erasers', array( 'KDNAForms', 'register_data_eraser' ),	10 );

		} else {
			add_action( 'wp_enqueue_scripts', array( 'KDNAForms', 'enqueue_scripts' ), 11 );
			add_action( 'wp', array( 'KDNAForms', 'ajax_parse_request' ), 10 );
		}

		// Add admin bar items
		add_action( 'wp_before_admin_bar_render', array( 'KDNAForms', 'admin_bar' ) );

		add_shortcode( 'kdnaform', array( 'KDNAForms', 'parse_shortcode' ) );
		add_shortcode( 'kdnaforms', array( 'KDNAForms', 'parse_shortcode' ) );

		// ManageWP premium update filters
		add_filter( 'mwp_premium_update_notification', array( 'KDNAForms', 'premium_update_push' ) );
		add_filter( 'mwp_premium_perform_update', array( 'KDNAForms', 'premium_update' ) );

		// Push KDNA Forms to the top of the list of plugins to make sure it's loaded before any add-ons
		add_action( 'activated_plugin', array( 'KDNAForms', 'load_first' ) );

		// Add the "Add Form" button to the editor. The customizer doesn't run in the admin context.
		if ( KDNAForms::page_supports_add_form_button() ) {
			require_once( KDNACommon::get_base_path() . '/tooltips.php' );

			// Adding "embed form" button to the editor
			add_action( 'media_buttons', array( 'KDNAForms', 'add_form_button' ), 20 );
			// Adding the modal
			add_action( 'admin_print_footer_scripts', array( 'KDNAForms', 'add_mce_popup' ) );
		}

		// Add a special classname to the body element when admin.css is loaded
		add_filter( 'admin_body_class', array( 'KDNAForms', 'add_admin_body_class' ), 99, 1 );
	}
	...
	public static function init_preview() {
		...
		wp_register_style( 'kdna-preview', "$base_url/css/preview$min.css" );
		wp_register_style( 'kdna-preview-rtl', "$base_url/css/rtl$min.css" );

		add_filter( 'kdnaform_preview_styles', function( $styles ) {
			$styles[] = 'kdna-preview-reset';
			$styles[] = 'kdna-preview';
	...
	public static function screen_options_filters() {
		...
		if ( $kdna_page == 'form_list' ) {
			...
			add_filter( 'set-screen-option', array( 'KDNAForms', 'set_screen_options' ), 10, 3 );
			// For WP 5.4.2+.
			add_filter( 'set_screen_option_kdnaform_forms_screen_options', array( 'KDNAForms', 'set_screen_options' ), 10, 3 );
		}
	}
	...
	public static function initialize_admin_settings() {
		...
		if ( $kdna_page === 'settings' && ( ! rgget( 'subview' ) || rgget( 'subview' ) === 'settings' ) ) {
			...
			KDNASettings::initialize_plugin_settings();
		}
		...
		if ( $kdna_page === 'settings' && rgget( 'subview' ) === 'recaptcha' ) {
			...
			KDNASettings::initialize_recaptcha_settings();
		}
		...
		if ( $kdna_page === 'confirmation' && isset( $_GET['cid'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			KDNA_Confirmation::initialize_settings_renderer();
		}
		...
		if ( $kdna_page === 'notification_edit' ) {
			...
			KDNANotification::initialize_settings_renderer();
		}
	}

	/**
	 * Ensures that KDNA Forms is loaded first.
	 *
	 * @since  Unknown
	 * @access public
	 *
	...
	public static function load_first() {
		$plugin_path    = basename( dirname( __FILE__ ) ) . '/kdnaforms.php';
	...
	/**
	 * Performs KDNA Forms deactivation tasks.
	 *
	 * @since Unknown
	...
	public static function deactivation_hook() {
		...
		KDNACache::flush( true );
		flush_rewrite_rules();
	}

	/**
	 * Performs KDNA Forms activation tasks.
	 *
	 * @since 2.3
	 * @since 2.6.9 Moved background upgrader init to \KDNA_Forms\KDNA_Forms\Async\KDNA_Background_Process_Service_Provider().
	 */
	...
	/**
	 * Add KDNA Forms to the plugins that support logging.
	 *
	 * @since  Unknown
	 * @access public
	 *
	...
	public static function set_logging_supported( $plugins ) {
		...
		$plugins['kdnaforms-browser'] = 'KDNA Forms Browser Session';
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormsModel::get_form()
	 * @uses   KDNACommon::get_base_path()
	 * @uses   KDNAFormDisplay::process_form()
	 * @uses   KDNAFormDisplay::process_send_resume_link()
	 *
	 * @return void
	 */
	public static function maybe_process_form() {
		...
		if ( KDNAFormDisplay::get_submission_method() === KDNAFormDisplay::SUBMISSION_METHOD_AJAX ) {
			return;
		}
	 ...
	 * @access public
	 *
	 * @uses   KDNACommon::get_upload_page_slug()
	 * @uses   KDNACommon::get_base_path()
	 *
	 * @return void
	 */
	public static function process_exterior_pages() {
		...
		$is_upload_page = $_SERVER['REQUEST_METHOD'] == 'POST' && $page == KDNACommon::get_upload_page_slug(); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidated

		if ( $is_upload_page ) {
			require_once( KDNACommon::get_base_path() . '/includes/upload.php' );
			exit();
		}
		...
		switch ( $page ) {
			case 'preview':
				require_once( KDNACommon::get_base_path() . '/preview.php' );
				break;

			case 'print-entry' :
				require_once( KDNACommon::get_base_path() . '/print-entry.php' );
				break;

			case 'select_columns' :
				require_once( KDNACommon::get_base_path() . '/select_columns.php' );
				break;
		}
		exit();
	}
	...
	/**
	 * Checks for KDNA Forms updates.
	 *
	 * @since  Unknown
	 * @access public
	 *
	 * @uses   KDNACommon::check_update()
	 *
	 * @param KDNAAutoUpgrade $update_plugins_option The KDNAAutoUpgrade object.
	 *
	 * @return KDNAAutoUpgrade The KDNAAutoUpgrade object.
	 */
	public static function check_update( $update_plugins_option ) {
		if ( ! class_exists( 'KDNACommon' ) ) {
			require_once( 'common.php' );
		}

		return KDNACommon::check_update( $update_plugins_option, true );
	}
	...
	public static function add_security_files() {
		KDNACommon::log_debug( __METHOD__ . '(): Start adding security files' );

		$upload_root = KDNAFormsModel::get_upload_root();

		if ( ! is_dir( $upload_root ) ) {
			return;
		}

		KDNACommon::recursive_add_index_file( $upload_root );

		KDNACommon::add_htaccess_file();
	}
	...
	private static function do_self_healing() {
		...
		KDNACommon::log_debug( __METHOD__ . '(): Start self healing' );

		$kdna_upload_root = KDNAFormsModel::get_upload_root();

		if ( ! is_dir( $kdna_upload_root ) || is_link( $kdna_upload_root ) ) {
			return;
		}

		self::rename_suspicious_files_recursive( $kdna_upload_root );
	}

	/**
	 * Renames files with a .bak extension if they have a file extension that is not allowed in the KDNA Forms uploads folder.
	 *
	 * @since   Unknown
	 * @access  private
	 *
	 * @used-by KDNAForms::do_self_healing()
	 *
	...
	private static function rename_suspicious_files_recursive( $dir ) {
		...
		while ( false !== ( $file = readdir( $dir_handle ) ) ) {
			...
			} elseif ( KDNACommon::file_name_has_disallowed_extension( $file )
			           && ! KDNACommon::match_file_extension( $file, array( 'htaccess', 'bak', 'html' ) )
			) {
	 ...
	 * @global $wp_styles
	 *
	 * @uses   KDNAForms::no_conflict_mode()
	 */
	public static function no_conflict_mode_style() {
		if ( ! get_option( 'kdnaform_enable_noconflict' ) ) {
			return;
		}

		global $wp_styles;
		$wp_required_styles = array( 'admin-bar', 'colors', 'ie', 'wp-admin', 'editor-style' );
		$kdna_required_styles = array(
			'common'                     => array( 'kdnaform_tooltip', 'kdnaform_font_awesome', 'kdnaform_admin', 'kdnaform_settings', 'setup_wizard_styles' ),
			'kdna_edit_forms'              => array(
				'thickbox',
				'editor-buttons',
				'wp-jquery-ui-dialog',
				'media-views',
			...
			'kdna_edit_forms_settings' => array(
				'thickbox',
				'editor-buttons',
				'wp-jquery-ui-dialog',
				'media-views',
			...
			'kdna_new_form'                => array( 'thickbox', 'template_library_styles' ),
			'kdna_entries'                 => array( 'thickbox', 'kdnaform_chosen' ),
			'kdna_settings'                => array(),
			'kdna_export'                  => array(),
			'kdna_help'                    => array(),
			'kdna_system_status'			 => array( 'thickbox' ),
		);

		self::no_conflict_mode( $wp_styles, $wp_required_styles, $kdna_required_styles, 'styles' );
	}
	...
	public static function no_conflict_mode_script() {
		...
		$kdna_required_scripts = array(
			'common'                     => array( 'kdnaform_tooltip_init', 'sack' ),
			'kdna_edit_forms'              => array(
				'backbone',
				'editor',
				'kdnaform_forms',
			...
			'kdna_entries'                 => array(
				...
				'wp-lists',
				'kdnaform_json',
				'kdnaform_field_filter',
				'plupload-all',
				'postbox',
				'kdnaform_chosen',
				'kdnaform_selectwoo',
			),
			'kdna_settings'                => array( 'kdnaform_kdnaforms_admin' ),
			'kdna_export'                  => array( 'kdnaform_form_admin', 'jquery-ui-datepicker', 'kdnaform_field_filter', 'kdnaform_kdnaforms_admin' ),
			'kdna_help'                    => array( 'kdnaform_kdnaforms_admin' ),
			'kdna_system_status'           => array(
				'kdnaform_system_report_clipboard',
				'thickbox',
				'kdnaform_placeholder',
			),
		);

		$load_scripts_globally = apply_filters( 'kdnaform_load_admin_scripts_globally', true );
		if ( $load_scripts_globally ) {
			$kdna_required_scripts[ 'common' ][] = 'kdnaform_kdnaforms_admin';
		}

		self::no_conflict_mode( $wp_scripts, $wp_required_scripts, $kdna_required_scripts, 'scripts' );
	}
	 ...
	 * @access  private
	 *
	 * @used-by KDNAForms::no_conflict_mode_style()
	 * @used-by KDNAForms::no_conflict_mode_script()
	 *
	 * @param WP_Scripts $wp_objects          WP_Scripts object.
	 * @param array      $wp_required_objects Scripts required by WordPress Core.
	 * @param array      $kdna_required_objects Scripts required by KDNA Forms.
	 * @param string     $type                Determines if scripts or styles are being run through the function.
	 */
	private static function no_conflict_mode( &$wp_objects, $wp_required_objects, $kdna_required_objects, $type = 'scripts' ) {

		$current_page = self::get_page_query_arg();
		if ( empty( $current_page ) ) {
			$current_page = trim( strtolower( (string) rgget( 'kdna_page' ) ) );
		}
		if ( empty( $current_page ) ) {
		...
		$page_objects = isset( $kdna_required_objects[ $current_page . '_' . $view ] ) ? $kdna_required_objects[ $current_page . '_' . $view ] : rgar( $kdna_required_objects, $current_page );
		...
		//merging wp scripts with kdna forms scripts
		$required_objects = array_merge( $wp_required_objects, $kdna_required_objects['common'], $page_objects );

		//allowing addons or other products to change the list of no conflict scripts
		$required_objects = apply_filters( "kdnaform_noconflict_{$type}", $required_objects );
	 ...
	 * @access  private
	 *
	 * @used-by KDNAForms::no_conflict_mode()
	 *
	...
	/**
	 * Validates that KDNA Forms is doing the database upgrade, and has permissions to do so.
	 *
	 * @since  Unknown
	 * @access public
	 *
	...
	public static function validate_upgrade( $do_upgrade, $hook_extra ) {
		...
		return kdna_upgrade()->validate_upgrade( $do_upgrade, $hook_extra );
	}
	 ...
	 */
	public static function install_translations( $new_language = '', $slug = 'kdnaforms' ) {
		// TranslationsPress_Updater removed - license functionality removed.
	}
	...
	public static function update_translations( $old_language, $new_language ) {
		...
		$kdna_addons = KDNAAddOn::get_registered_addons( true );

		foreach ( $kdna_addons as $kdna_addon ) {
			$kdna_addon->install_translations( $new_language );
		}
	}
	...
	public static function install_addon_translations( $upgrader_object, $hook_extra ) {
		...
		$addons_list = KDNACache::get( 'addons_list' );

		if ( empty( $addons_list ) ) {
			$addons_api = KDNACommon::post_to_manager( 'api.php', 'op=get_plugins', array() );
			...
			KDNACache::set( 'addons_list', $addons_list, true, WEEK_IN_SECONDS );
		}
	...
	public static function user_has_cap( $all_caps, $cap, $args, $user = null ) {
		$kdna_caps    = KDNACommon::all_caps();
		$capability = rgar( $cap, 0 );
		if ( $capability != 'kdnaform_full_access' ) {
			return $all_caps;
		}
		...
		if ( ! self::has_members_plugin() ) {
			...
			if ( user_can( $user, 'administrator' ) || ( is_multisite() && is_super_admin( $user->ID ) ) ) {
				$all_caps['kdnaform_full_access'] = true;
			}
		} elseif ( user_can( $user, 'administrator' ) || ( is_multisite() && is_super_admin( $user->ID ) ) ) {

			//checking if user has any GF permission.
			$has_kdna_cap = false;
			foreach ( $kdna_caps as $kdna_cap ) {
				if ( rgar( $all_caps, $kdna_cap ) ) {
					$has_kdna_cap = true;
				}
			}

			if ( ! $has_kdna_cap && ! isset( $all_caps['kdnaform_full_access'] ) ) {
	...
	/**
	 * Register the KDNA Forms capabilities group with the Members plugin.
	 *
	 * @since  2.4
	 * @access public
	 */
	public static function members_register_cap_group() {
		...
		members_register_cap_group(
			'kdnaforms',
			array(
				'label' => esc_html__( 'KDNA Forms', 'kdnaforms' ),
				'icon'  => 'dashicons-kdnaforms',
				'caps'  => array(),
			)
		);
	...
	public static function members_register_caps() {
		...
		$caps = array(
			'kdnaforms_create_form'      => esc_html__( 'Create Forms', 'kdnaforms' ),
			'kdnaforms_delete_forms'     => esc_html__( 'Delete Forms', 'kdnaforms' ),
			'kdnaforms_edit_forms'       => esc_html__( 'Edit Forms', 'kdnaforms' ),
			'kdnaforms_preview_forms'    => esc_html__( 'Preview Forms', 'kdnaforms' ),
			'kdnaforms_view_entries'     => esc_html__( 'View Entries', 'kdnaforms' ),
			'kdnaforms_edit_entries'     => esc_html__( 'Edit Entries', 'kdnaforms' ),
			'kdnaforms_delete_entries'   => esc_html__( 'Delete Entries', 'kdnaforms' ),
			'kdnaforms_view_entry_notes' => esc_html__( 'View Entry Notes', 'kdnaforms' ),
			'kdnaforms_edit_entry_notes' => esc_html__( 'Edit Entry Notes', 'kdnaforms' ),
			'kdnaforms_export_entries'   => esc_html__( 'Import/Export', 'kdnaforms' ),
			'kdnaforms_view_settings'    => esc_html__( 'View Plugin Settings', 'kdnaforms' ),
			'kdnaforms_edit_settings'    => esc_html__( 'Edit Plugin Settings', 'kdnaforms' ),
			'kdnaforms_view_updates'     => esc_html__( 'Manage Updates', 'kdnaforms' ),
			'kdnaforms_view_addons'      => esc_html__( 'Manage Add-Ons', 'kdnaforms' ),
			'kdnaforms_system_status'    => esc_html__( 'View System Status', 'kdnaforms' ),
			'kdnaforms_uninstall'        => esc_html__( 'Uninstall KDNA Forms', 'kdnaforms' ),
			'kdnaforms_logging'          => esc_html__( 'Logging Settings', 'kdnaforms' ),
			'kdnaforms_api_settings'     => esc_html__( 'REST API Settings', 'kdnaforms' ),
		);
	...
	/**
	 * Register KDNA Forms capabilities group with User Role Editor plugin.
	 *
	 * @since  2.4
	 *
	...
	public static function filter_ure_capabilities_groups_tree( $groups = array() ) {
		...
		$groups['kdnaforms'] = array(
			'caption' => esc_html__( 'KDNA Forms', 'kdnaforms' ),
			'parent'  => 'custom',
			'level'   => 2,
		);
	...
	/**
	 * Register KDNA Forms capabilities with KDNA Forms group in User Role Editor plugin.
	 *
	 * @since  2.4
	 *
	...
	public static function filter_ure_custom_capability_groups( $groups = array(), $cap_id = '' ) {
		...
		// Get KDNA Forms capabilities.
		$caps = KDNACommon::all_caps();

		// If capability belongs to KDNA Forms, register it to group.
	...
	public static function check_upload_folder() {
		...
		$folder = KDNAFormsModel::get_upload_root();
		if ( empty( $folder ) ) {
			echo "<div class='error'>" . esc_html__( 'Upload folder is not writable. Export and file upload features will not be functional.', 'kdnaforms' ) . '</div>';
		}
	}

	/**
	 * Checks if a KDNA Forms AJAX action is being performed.
	 *
	 * @since  Unknown
	 * @access public
	 *
	 * @return bool True if performing a KDNA Forms AJAX request. False, otherwise.
	 */
	public static function is_kdna_ajax_action() {
		//KDNA Forms AJAX requests
		$current_action  = self::post( 'action' );
		$kdna_ajax_actions = array(
			...
			'rg_start_export',
			// 'kdna_upgrade_license', // Removed - license functionality removed.
			'kdna_delete_custom_choice',
			'kdna_save_custom_choice',
			'kdna_get_notification_post_categories',
			'rg_update_lead_property',
			'delete-kdna_entry',
			'rg_update_form_active',
			'rg_update_notification_active',
			'rg_update_confirmation_active',
			'kdna_resend_notifications',
			'rg_dismiss_upgrade',
			'kdna_save_confirmation',
			'kdna_process_export',
			'kdna_download_export',
			'kdna_dismiss_message',
			'kdna_force_upgrade',
		);

		 /**
		 * Filters the AJAX actions that are used to determine if the request is a KDNA Forms AJAX request.
 		 *
 		 * @since 2.6
 		 *
 		 * @param array $kdna_ajax_actions The AJAX action names.
 		 */
		$kdna_ajax_actions = apply_filters( 'kdnaform_ajax_actions', $kdna_ajax_actions );

		if ( defined( 'DOING_AJAX' ) && DOING_AJAX && in_array( $current_action, $kdna_ajax_actions ) ) {
			return true;
		}

		// Not a KDNA Forms ajax request.
		return false;
	}
	...
	/**
	 * Determines if the current page is part of KDNA Forms.
	 *
	 * Returns true if the current page is one of KDNA Forms page or first-party add-on page. False otherwise.
	 *
	 * @since  Unknown
	 * @access public
	 *
	 * @return bool
	 */
	public static function is_kdna_page() {

		// KDNA Forms pages
		$current_page   = self::get_page_query_arg();
		$kdna_pages       = array( 'kdna_edit_forms', 'kdna_new_form', 'kdna_entries', 'kdna_settings', 'kdna_export', 'kdna_help', 'kdna_addons', 'kdna_system_status' );
		$kdna_addon_pages = array( 'kdnaformscoupons' );

		return in_array( $current_page, array_merge( $kdna_pages, $kdna_addon_pages ) );
	}
	 ...
	 *  e.g. When an update for KDNA Forms is available an icon will be added to the the menu title.
	...
	public static function create_menu() {
		...
		$addon_menus = array();
		$addon_menus = apply_filters( 'kdnaform_addon_navigation', $addon_menus );
		...
		add_action( 'load-' . $forms_hook_suffix, array( 'KDNAForms', 'load_screen_options' ) );

		// Adding submenu pages
		add_submenu_page( $parent_menu['name'], __( 'Forms - KDNA Forms', 'kdnaforms' ), __( 'Forms', 'kdnaforms' ), $has_full_access ? 'kdnaform_full_access' : 'kdnaforms_edit_forms', 'kdna_edit_forms', array(
			'KDNAForms',
			'forms'
		) );

		add_submenu_page( $parent_menu['name'], __( 'New Form - KDNA Forms', 'kdnaforms' ), __( 'New Form', 'kdnaforms' ), $has_full_access ? 'kdnaform_full_access' : 'kdnaforms_create_form', 'kdna_new_form', array(
			'KDNAForms',
			'new_form'
		) );

		$entries_hook_suffix = add_submenu_page( $parent_menu['name'], __( 'Entries - KDNA Forms', 'kdnaforms' ), __( 'Entries', 'kdnaforms' ), $has_full_access ? 'kdnaform_full_access' : 'kdnaforms_view_entries', 'kdna_entries', array(
			'KDNAForms',
			'all_leads_page'
		) );

		add_action( 'load-' . $entries_hook_suffix, array( 'KDNAForms', 'load_screen_options' ) );
		add_action( 'load-' . $entries_hook_suffix, array( 'KDNAEntryList', 'redirect_on_restore' ) );

		if ( is_array( $addon_menus ) ) {
			foreach ( $addon_menus as $addon_menu ) {
				add_submenu_page( esc_html( $parent_menu['name'] ), esc_html( $addon_menu['label'] ), esc_html( $addon_menu['label'] ), $has_full_access ? 'kdnaform_full_access' : $addon_menu['permission'], esc_html( $addon_menu['name'] ), $addon_menu['callback'] );
			}
		}

		add_submenu_page( $parent_menu['name'], __( 'Settings - KDNA Forms', 'kdnaforms' ), __( 'Settings', 'kdnaforms' ), $has_full_access ? 'kdnaform_full_access' : 'kdnaforms_view_settings', 'kdna_settings', array(
			'KDNAForms',
			'settings_page'
		) );

		add_submenu_page( $parent_menu['name'], __( 'Import/Export - KDNA Forms', 'kdnaforms' ), __( 'Import/Export', 'kdnaforms' ), $has_full_access ? 'kdnaform_full_access' : ( current_user_can( 'kdnaforms_export_entries' ) ? 'kdnaforms_export_entries' : 'kdnaforms_edit_forms' ), 'kdna_export', array(
			'KDNAForms',
			'export_page'
		) );

		add_submenu_page( $parent_menu['name'], __( 'Help - KDNA Forms', 'kdnaforms' ), __( 'Help', 'kdnaforms' ), $has_full_access ? 'kdnaform_full_access' : $min_cap, 'kdna_help', array(
			'KDNAForms',
			'help_page'
		) );
	...
	public static function get_parent_menu( $addon_menus ) {
		...
		if ( KDNACommon::current_user_can_any( 'kdnaforms_edit_forms' ) ) {
			$parent = array( 'name' => 'kdna_edit_forms', 'callback' => array( 'KDNAForms', 'forms' ) );
		} else if ( KDNACommon::current_user_can_any( 'kdnaforms_create_form' ) ) {
			$parent = array( 'name' => 'kdna_new_form', 'callback' => array( 'KDNAForms', 'new_form' ) );
		} else if ( KDNACommon::current_user_can_any( 'kdnaforms_view_entries' ) ) {
			$parent = array( 'name' => 'kdna_entries', 'callback' => array( 'KDNAForms', 'all_leads_page' ) );
		} else if ( is_array( $addon_menus ) && sizeof( $addon_menus ) > 0 ) {
			foreach ( $addon_menus as $addon_menu ) {
				if ( KDNACommon::current_user_can_any( $addon_menu['permission'] ) ) {
					$parent = array( 'name' => $addon_menu['name'], 'callback' => $addon_menu['callback'] );
					break;
				}
			}
		} else if ( KDNACommon::current_user_can_any( 'kdnaforms_view_settings' ) ) {
			$parent = array( 'name' => 'kdna_settings', 'callback' => array( 'KDNAForms', 'settings_page' ) );
		} else if ( KDNACommon::current_user_can_any( 'kdnaforms_export_entries' ) ) {
			$parent = array( 'name' => 'kdna_export', 'callback' => array( 'KDNAForms', 'export_page' ) );
		} else if ( KDNACommon::current_user_can_any( 'kdnaforms_view_addons' ) ) {
			$parent = array( 'name' => 'kdna_addons', 'callback' => array( 'KDNAForms', 'addons_page' ) );
		} else if ( KDNACommon::current_user_can_any( 'kdnaforms_system_status' ) ) {
			$parent = array( 'name' => 'kdna_system_status', 'callback' => array( 'KDNAForms', 'system_status_page' ) );
		} else if ( KDNACommon::current_user_can_any( KDNACommon::all_caps() ) ) {
			$parent = array( 'name' => 'kdna_help', 'callback' => array( 'KDNAForms', 'help_page' ) );
		}
	...
	/**
	 * Modifies the page title when on KDNA Forms settings pages.
	 *
	 * @since  Unknown
	 * @access public
	 *
	...
	public static function modify_admin_title( $admin_title, $title ) {
		...
		$form_title = rgar( $form, 'title', __( 'Form Not Found', 'kdnaforms' ) );
		...
		switch ( $page ) {
			...
			case 'form_list':
				...
				if ( $filter === 'inactive' ) {
					$filter_title = __( 'Inactive', 'kdnaforms' );
				}

				if ( $filter === 'active' ) {
					$filter_title =__( 'Active', 'kdnaforms' );
				}

				if ( $filter === 'trash' ) {
					$filter_title = __( 'Trash', 'kdnaforms' );
				}

				$search = rgget( 's' );
				if ( ! rgblank( $search ) ) {
					/* translators: Search entries page title. 1. Search value */
					$filter_title = sprintf( __( 'Search Forms: %1$s', 'kdnaforms' ), esc_html( $search ) );
				}
			...
			case 'confirmation':
				$page_title      = __( 'Confirmations', 'kdnaforms' );
				$confirmation_id = rgget( 'cid' );

				if ( $confirmation_id !== ''  ) {
					$confirmation_name = rgars( $form, "confirmations/{$confirmation_id}/name", __( 'New Confirmation', 'kdnaforms' ) );
			...
			case 'entry_list':
				...
				if ( $filter === 'star' ) {
					...
					$form_title = sprintf( __( 'Starred &#8212; %1$s', 'kdnaforms' ), esc_html( $form_title ) );
				}

				if ( $filter === 'unread' ) {
					/* translators: Unread entry list page title. 1. form title */
					$form_title = sprintf( __( 'Unread &#8212; %1$s', 'kdnaforms' ), esc_html( $form_title ) );
				}

				if ( $filter === 'spam' ) {
					/* translators: Active entry list page title. 1. form title */
					$form_title = sprintf( __( 'Spam &#8212; %1$s', 'kdnaforms' ), esc_html( $form_title ) );
				}

				if ( $filter === 'trash' ) {
					/* translators: Trash entry list page title. 1. form title */
					$form_title = sprintf( __( 'Trash &#8212; %1$s', 'kdnaforms' ), esc_html( $form_title ) );
				}

				$search = rgget( 's' );
				if ( ! rgblank( $search ) ) {
					/* translators: Search entries page title. 1. Search value, 2. Form title. */
					$form_title = sprintf( __( 'Search Entries: %1$s &#8212; %2$s', 'kdnaforms' ), esc_html( $search ),  esc_html( $form_title ) );
				}
			...
			case 'entry_detail_edit':
				require_once( KDNACommon::get_base_path() . '/entry_detail.php' );

				$entry = KDNAEntryDetail::get_current_entry();

				if ( ! is_wp_error( $entry ) && isset( $entry['id'] ) ) {
					/* translators: Single entry page title. 1: entry ID, 2: form title, 3: admin title. */
					$admin_title = sprintf( __( 'Entry # %1$d &lsaquo; %2$s &lsaquo; %3$s', 'kdnaforms' ), esc_html( $entry['id'] ), esc_html( $form_title ), esc_html( $admin_title ) );
				} else {
					/* translators: Entry not found page title. 1: form title, 2: admin title. */
					$admin_title = sprintf( __( 'Entry not found &lsaquo; %1$s &lsaquo; %2$s', 'kdnaforms' ), esc_html( $form_title ), esc_html( $admin_title ) );
				}
				break;
			...
			case 'notification_edit':
				...
				$notification_name = rgar( $form, "notifications/{$notification_id}/name", __( 'New Notification', 'kdnaforms' ) );
				$page_title        = __( 'Notifications', 'kdnaforms' );
			...
			case 'settings':
				$page_title = __( 'Settings', 'kdnaforms' );
				$subview    = rgget( 'subview' );

				if ( $subview === 'recaptcha' ) {
					$page_title = __( 'reCAPTCHA', 'kdnaforms' );
				}

				if ( $subview === 'uninstall' ) {
					$page_title = __( 'Uninstall', 'kdnaforms' );
				}
				if ( $page_title === 'Settings' ) {
					$addon_page = KDNASettings::$addon_pages;
			...
			case 'export_form':
				...
				$admin_title = sprintf( __( 'Export Forms &lsaquo; %1$s', 'kdnaforms' ), esc_html( $admin_title ) );
				break;

			case 'import_form':
				/* Translators: Import form page title. 1: Admin title. */
				$admin_title = sprintf( __( 'Import Forms &lsaquo; %1$s', 'kdnaforms' ), esc_html( $admin_title ) );
				break;

			case 'imported_forms_list':
				/* Translators: Imported forms page title. 1: Admin title. */
				$admin_title = sprintf( __( 'Imported Forms &lsaquo; %1$s', 'kdnaforms' ), esc_html( $admin_title ) );
				break;

			case 'export_entry':
				/* Translators: Export Entry page title. 1: Admin title. */
				$admin_title = sprintf( __( 'Export Entries &lsaquo; %1$s', 'kdnaforms' ), esc_html( $admin_title ) );
				break;

			case 'updates':
				/* Translators: Updates page title. 1: Admin title. */
				$admin_title = sprintf( __( 'Updates &lsaquo; %1$s', 'kdnaforms' ), esc_html( $admin_title ) );
				break;

			case 'system_status':
				$subview    = rgget( 'subview' );
				$page_title = __( 'System Status', 'kdnaforms' );

		if ( ! $form_id || self::get_page_query_arg() != 'kdna_edit_forms' || rgget( 'view' ) != 'settings' ) {
			return $admin_title;
		}
			...
			default:
				if ( rgget( 'view' ) === 'settings' ) {
					...
					$form_id      = rgget( 'id' );
					$setting_tabs = KDNAFormSettings::get_tabs( $form_id );
					$page_title   = '';
					$subview      = rgget( 'subview' );
					...
					foreach ( $setting_tabs as $tab ) {
						if ( $tab['name'] === $subview || ( $subview === 'kdna_theme_layers' && rgget( 'theme_layer' ) === $tab['name'] ) ) {
	...
	/**
	 * Parses KDNA Forms shortcode attributes and displays the form.
	 *
	 * @since  Unknown
	 * @access public
	 *
	...
	public static function parse_shortcode( $attributes, $content = null ) {
		...
		switch ( $action ) {
			case 'conditional':
				$shortcode_string = KDNACommon::conditional_shortcode( $attributes, $content );
				break;
		 ...
		 */
		$shortcode_string = apply_filters( "kdnaform_shortcode_{$action}", $shortcode_string, $attributes, $content );
	...
	public static function include_addon_framework() {
		require_once( KDNACommon::get_base_path() . '/includes/addon/class-kdna-addon.php' );
	}
	...
	public static function include_feed_addon_framework() {
		require_once( KDNACommon::get_base_path() . '/includes/addon/class-kdna-feed-addon.php' );
	}
	...
	/**
	 * Includes the KDNA API
	 *
	 * @since  Unknown
	 * @access public
	 */
	public static function include_kdna_api() {
		require_once( KDNACommon::get_base_path() . '/includes/class-kdna-api.php' );
	}
	...
	public static function get_ajax_form_response() {
		require_once( KDNACommon::get_base_path() . '/form_display.php' );
		$args    = KDNAFormDisplay::parse_ajax_input();
		...
		if ( $form_id && KDNAFormDisplay::is_submit_form_id_valid( $form_id ) ) {
			...
			// The following $args keys have been defined during sanitization by KDNAFormDisplay::parse_ajax_input().
			$result = KDNAFormDisplay::get_form( $form_id, $args['title'], $args['description'], false, $field_values_array, true, $args['tabindex'], $args['theme'], $args['styles'] );
		} else {
			...
			$result = KDNAFormDisplay::get_ajax_postback_html( KDNAFormDisplay::get_confirmation_message( KDNAFormsModel::get_default_confirmation(), array( 'id' => $form_id ), array() ) );
		}
	...
	public static function page_supports_add_form_button() {
		$display_add_form_button = ( ! class_exists( 'KDNA_Block' ) || class_exists( 'Classic_Editor' ) ) && in_array( RG_CURRENT_PAGE, array(
			'post.php',
			'page.php',
			'page-new.php',
			'post-new.php',
		 ...
		 *
		 */
		return apply_filters( 'kdnaform_display_add_form_button', $display_add_form_button );
	}
	...
	public static function add_form_button() {
		...
		echo '<style>.kdnaform_media_icon{
                ...
                .kdnaform_media_icon:before{
                color: #999;
			    padding: 7px 0;
			    transition: all 0.1s ease-in-out 0s;
                }
                .wp-core-ui a.kdnaform_media_link{
                 padding-left: 0.4em;
                }
             </style>
	...
	public static function add_mce_popup() {
		...
		<script>
			function InsertForm() {
				...
				if (form_id == "") {
					alert(<?php echo json_encode( __( 'Please select a form', 'kdnaforms' ) ); ?>);
					return;
				}
				...
				var ajax = jQuery("#kdnaform_ajax").is(":checked");
				var title_qs = !display_title ? " title=\"false\"" : "";
				var description_qs = !display_description ? " description=\"false\"" : "";
				var ajax_qs = ajax ? " ajax=\"true\"" : "";

				window.send_to_editor("[kdnaform id=\"" + form_id + "\" name=\"" + form_name + "\"" + title_qs + description_qs + ajax_qs + "]");
			}
		</script>

		<div id="select_kdna_form" style="display:none;">

			<div id="kform-shortcode-ui-wrap" class="wrap <?php echo esc_attr( KDNACommon::get_browser_class() ); ?>">

				<div id="kform-shortcode-ui-container"></div>
	...
	public static function plugin_settings_link( $links, $file ) {
		...
		array_unshift( $links, '<a href="' . esc_url( admin_url( 'admin.php' ) ) . '?page=kdna_settings">' . esc_html__( 'Settings', 'kdnaforms' ) . '</a>' );

		return $links;
	}

	/**
	 * Displays messages for the KDNA Forms listing on the Plugins page.
	 *
	...
	public static function plugin_row( $plugin_name, $plugin_data ) {
		...
		$add_ons = kdna_upgrade()->get_min_addon_requirements();
		...
		if ( isset( $add_ons[ $plugin_name ] ) ) {
			...
			if ( version_compare( $current_version, $min_version, '<' ) ) {
				...
				$message = esc_html__( 'This version of the %1$s is not compatible with the version of KDNA Forms that is installed. Upgrade this add-on to version %2$s or greater to avoid compatibility issues and potential loss of data.', 'kdnaforms' );
	...
	/**
	 * Display KDNA Forms and add-ons update notifications if needed.
	 *
	 * @since 2.4.15
	 *
	 * @param string $plugin_name The plugin filename.  Immediately overwritten.
	...
	public static function get_status_messages( $plugin_name, $plugin_data, $slug = '', $version = '' ) {
		// License checks removed - plugin is now free.
		return array();
	}

	/**
	 * Hooks into in_plugin_update_message-kdnaforms/kdnaforms.php and displays an update message specifically for KDNA Forms 2.3.
	 *
	 * @param $args
	 * @param $response
	 */
	public static function in_plugin_update_message( $args, $response ) {
		...
		if ( version_compare( $args['new_version'], '2.3', '>=' ) && version_compare( KDNAForms::$version, '2.3', '<' ) ) {
	...
	public static function display_changelog() {
		if ( $_REQUEST['plugin'] != 'kdnaforms' ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended, WordPress.Security.ValidatedSanitizedInput.InputNotValidated
			return;
		}
	...
	/**
	 * Get changelog with admin-ajax.php in KDNAForms::maybe_display_update_notification().
	 *
	 * @since 2.4.15
	 */
	...
	public static function get_changelog() {
		...
		$params       = KDNACommon::get_remote_request_params();
		$params       .= '&v_requested=' . urlencode( rgar( $version_info, 'version' ) );

		$raw_response = KDNACommon::post_to_manager( 'changelog.php', $params, $options );

		if ( is_wp_error( $raw_response ) || 200 != $raw_response['response']['code'] ) {
			$page_text = sprintf( esc_html__( 'Oops!! Something went wrong. %sPlease try again or %scontact us%s.', 'kdnaforms' ), '<br/>', "<a href='" . esc_attr( KDNACommon::get_support_url() ) . "'>", '</a>' );
		} else {
			$page_text = $raw_response['body'];
			if ( substr( $page_text, 0, 10 ) != '<!--KDNAM-->' ) {
				$page_text = '';
			} else {
	...
	public static function dashboard_setup() {
		if ( '' === get_option( 'kdnaform_enable_dashboard_widget' ) ) {
			return;
		}
		 ...
		 */
		$dashboard_title = apply_filters( 'kdnaform_dashboard_title', __( 'KDNA Forms', 'kdnaforms' ) );
		wp_add_dashboard_widget( 'rg_forms_dashboard', $dashboard_title, array( 'KDNAForms', 'dashboard' ) );
	}
	...
	public static function dashboard() {
		$forms = KDNAFormsModel::get_form_summary();
		...
		if ( sizeof( $forms ) > 0 ) {
			...
			<table class="widefat kdna_dashboard_view" cellspacing="0" style="border:0px;">
				...
				foreach ( $forms as $form ) {
					...
					$date_display = KDNACommon::format_date( $form['last_entry_date'] );
		...
		if ( KDNACommon::current_user_can_any( 'kdnaforms_view_updates' ) && ( ! function_exists( 'is_multisite' ) || ! is_multisite() || is_super_admin() ) ) {
	...
	public static function dashboard_update_message() {
		...
		$ary_dismissed = get_option( 'kdna_dismissed_upgrades' );
	...
	public static function dashboard_dismiss_upgrade() {
		$ary = get_option( 'kdna_dismissed_upgrades' );
		if ( ! is_array( $ary ) ) {
			$ary = array();
		}

		$ary[] = rgpost( 'version' );
		update_option( 'kdna_dismissed_upgrades', $ary );
	}
	...
	public static function get_local_dev_base_url() {
		$url = KDNACommon::get_base_url();

		if ( ! defined( 'KDNA_ENABLE_HMR' ) || ! KDNA_ENABLE_HMR ) {
	...
	public static function register_scripts() {
		...
		$util_deps     = is_admin() ? array( 'kdnaform_kdnaforms_admin_vendors' ) : array();

		wp_register_script( 'kdnaform_chosen', $base_url . "/js/chosen.jquery.min.js", array( 'jquery' ), $version );
		wp_register_script( 'kdnaform_selectwoo', $base_url . "/js/vendor/selectWoo.full.js", array( 'jquery' ), $version );
		wp_register_script( 'kdnaform_simplebar', $base_url . "/js/vendor/simplebar.js", array( 'jquery' ), $version );
		wp_register_script( 'kdnaform_conditional_logic', $base_url . "/js/conditional_logic{$min}.js", array(
			'jquery',
			'kdnaform_kdnaforms'
		), $version );
		wp_register_script( 'kdnaform_page_conditional_logic', $base_url . "/js/page_conditional_logic{$min}.js", array(
			'jquery',
			'kdnaform_kdnaforms'
		), $version );
		wp_register_script( 'kdnaform_datepicker_legacy', $base_url . "/js/datepicker-legacy{$min}.js", array(), $version, true );
		wp_register_script( 'kdnaform_datepicker_init', $base_url . "/js/datepicker{$min}.js", array(
			'jquery',
			'jquery-ui-datepicker',
			'kdnaform_kdnaforms',
			'kdnaform_datepicker_legacy',
		), $version, true );
		wp_register_script( 'kdnaform_form_editor_conditional_flyout', $base_url . "/js/components/form_editor/conditional_flyout/conditional_flyout{$min}.js", array(
			'jquery',
			'kdnaform_kdnaforms',
		), $version );
		wp_register_script( 'kdnaform_form_admin', $base_url . "/js/form_admin{$min}.js", array(
			'jquery',
			'jquery-ui-autocomplete',
			'kdnaform_placeholder',
			'kdnaform_kdnaforms',
		...
		wp_register_script( 'kdnaform_kdnaforms_admin_components', $base_url . "/assets/js/dist/admin-components{$dev_min}.js", array( 'kdnaform_kdnaforms_react_utils' ), $version, false );
		wp_register_script( 'kdnaform_kdnaforms', $base_url . "/js/kdnaforms{$min}.js", array(
			'jquery',
			'kdnaform_json',
		), $version, false );
		wp_register_script( 'kdnaform_json', $base_url . "/js/jquery.json{$min}.js", array( 'jquery' ), $version, true );
		wp_register_script( 'kdnaform_masked_input', $base_url . "/js/jquery.maskedinput{$min}.js", array( 'jquery' ), $version, true );
		wp_register_script( 'kdnaform_menu', $base_url . "/js/menu{$min}.js", array( 'jquery' ), $version );
		wp_register_script( 'kdnaform_placeholder', $base_url . '/js/placeholders.jquery.min.js', array( 'jquery' ), $version, true );
		wp_register_script( 'kdnaform_tooltip_init', $base_url . "/js/tooltip_init{$min}.js", array( 'jquery-ui-tooltip' ), $version );
		wp_register_script( 'kdnaform_textarea_counter', $base_url . "/js/jquery.textareaCounter.plugin{$min}.js", array( 'jquery' ), $version, true );
		wp_register_script( 'kdnaform_field_filter', $base_url . "/js/kdna_field_filter{$min}.js", array(
			'jquery',
			'kdnaform_datepicker_init'
		), $version );
		wp_register_script( 'kdnaform_shortcode_ui', $base_url . "/js/shortcode-ui{$min}.js", array(
			'jquery',
			'wp-backbone',
			'kdnaform_kdnaforms'
		), $version, true );
		wp_register_script( 'kdnaform_kdnaforms_libraries', $base_url . "/assets/js/dist/libraries{$dev_min}.js", array(), $version );
		wp_register_script( 'kdnaform_kdnaforms_admin_vendors', $base_url . "/assets/js/dist/vendor-admin{$dev_min}.js", array( 'kdnaform_kdnaforms_libraries' ), $version, true );
		wp_register_script( 'kdnaform_kdnaforms_admin', $local_dev_url . "/scripts-admin{$dev_min}.js", array(
			'kdnaform_kdnaforms_admin_components',
            'kdnaform_kdnaforms_admin_vendors',
			'kdnaform_simplebar',
		), $version, true );
		wp_register_script( 'kdnaform_kdnaforms_theme_vendors', $base_url . "/assets/js/dist/vendor-theme{$dev_min}.js", array(
			'kdnaform_kdnaforms_utils',
			'kdnaform_kdnaforms',
		), $version, true );
		...
		wp_register_style( 'kdnaform_common_css_utilities', $base_url . "/assets/css/dist/common-css-utilities{$dev_min}.css", array(), $version );
		wp_register_style( 'kdnaform_admin_components', $base_url . "/assets/css/dist/admin-components{$dev_min}.css", array( 'kdnaform_admin_css_utilities' ), $version );
		wp_register_style( 'kdnaform_admin_css_utilities', $base_url . "/assets/css/dist/admin-css-utilities{$dev_min}.css", array(), $version );
		wp_register_style( 'kdnaform_admin_icons', $base_url . "/assets/css/dist/admin-icons{$dev_min}.css", array(), $version );
		wp_register_style( 'kdnaform_admin', $base_url . "/assets/css/dist/admin{$dev_min}.css", array(), $version );
		wp_register_style( 'kdnaform_admin_setup_wizard', $base_url . "/assets/css/dist/setup-wizard{$dev_min}.css", array(), $version );
		wp_register_style( 'kdnaform_chosen', $base_url . "/legacy/css/chosen{$min}.css", array(), $version );
		wp_register_style( 'kdnaform_shortcode_ui', $base_url . "/css/shortcode-ui{$min}.css", array(), $version );
		wp_register_style( 'kdnaform_font_awesome', $base_url . "/assets/css/dist/font-awesome{$dev_min}.css", null, $version );
		wp_register_style( 'kdnaform_dashicons', $base_url . "/css/dashicons{$min}.css", array(), $version );
		wp_register_style( 'kdnaform_settings', $base_url . "/assets/css/dist/settings{$dev_min}.css", array(), $version );
		wp_register_style( 'kdnaform_editor', $base_url . "/assets/css/dist/editor{$dev_min}.css", array(), $version );

		wp_register_style( 'kdnaform_theme_components', $base_url . "/assets/css/dist/theme-components{$dev_min}.css", array(), $version );
		wp_register_style( 'kforms_reset_css', $base_url . "/legacy/css/formreset{$min}.css", null, $version );
		wp_register_style( 'kforms_datepicker_css', $base_url . "/legacy/css/datepicker{$min}.css", null, $version );
		wp_register_style( 'kforms_formsmain_css', $base_url . "/legacy/css/formsmain{$min}.css", null, $version );
		wp_register_style( 'kforms_ready_class_css', $base_url . "/legacy/css/readyclass{$min}.css", null, $version );
		wp_register_style( 'kforms_browsers_css', $base_url . "/legacy/css/browsers{$min}.css", null, $version );
		wp_register_style( 'kforms_rtl_css', $base_url . "/legacy/css/rtl{$min}.css", null, $version );

		wp_register_style( 'kdnaform_basic', $base_url . "/assets/css/dist/basic{$dev_min}.css", null, $version );
		wp_register_style( 'kdnaform_theme', $base_url . "/assets/css/dist/theme{$dev_min}.css", array( 'kdnaform_theme_components' ), $version );
		wp_register_style( 'kdnaform_theme_admin', $base_url . "/assets/css/dist/theme-admin{$dev_min}.css", array(), $version );
	}
	...
	public static function init_hook_vars() {
		...
		add_filter( 'kdnaform_get_form_filter', array( 'KDNAForms', 'maybe_prepend_hooks_js_script'), $prio );
		add_filter( 'kdnaform_get_form_confirmation_filter', array( 'KDNAForms', 'maybe_prepend_hooks_js_script'), $prio );
	}
	...
	public static function maybe_prepend_hooks_js_script( $form_string ) {
		...
		$is_kdna_ajax = ! empty( rgpost( 'kform_ajax' ) );
		$doing_ajax = defined( 'DOING_AJAX' ) && DOING_AJAX;

		if ( $doing_ajax || $is_kdna_ajax ) {
			return $form_string;
		}

		$needed = KDNACommon::requires_kdna_hooks_javascript();

		if ( ! $needed ) {
			return $form_string;
		}

		$scripts = KDNACommon::get_hooks_javascript_code();

		if ( empty( $scripts ) ) {
			return $form_string;
		}

		return KDNACommon::get_inline_script_tag( $scripts, false ) . $form_string;
	}
	...
	public static function load_hooks_with_actions() {
		add_action( 'kdnaform_preview_header', array( 'KDNACommon', 'output_hooks_javascript' ) );
		add_action( 'wp_head', array( 'KDNACommon', 'output_hooks_javascript' ) );
		add_action( 'admin_head', array( 'KDNACommon', 'output_hooks_javascript' ) );
		add_action( 'kdnaform_pre_print_scripts', array( 'KDNACommon', 'output_hooks_javascript' ) );
	}
	...
	public static function load_hooks_with_inline_script() {
		...
		$hooks_code = KDNACommon::get_hooks_javascript_code();
		wp_add_inline_script( 'kdnaform_kdnaforms', $hooks_code, 'before' );
	}
	...
	public static function localize_hook_vars() {
		 ...
		 * @return bool
		 */
		$force_output = apply_filters( 'kdnaform_force_hooks_js_output', false );
		$is_enqueued  = wp_script_is( 'kdnaform_kdnaforms', 'enqueued' );
		$script       = wp_scripts()->query( 'kdnaform_kdnaforms' );
	...
	/**
	 * Enqueues registered KDNA Forms scripts.
	 *
	 * @since  Unknown
	 * @access public
	 *
	...
	public static function enqueue_admin_scripts( $hook ) {
		...
		if ( self::page_supports_add_form_button() ) {
			...
			wp_localize_script( 'kdnaform_shortcode_ui', 'kdnaShortcodeUIData', array(
				 ...
				 */
				'previewDisabled' => apply_filters( 'kdnaform_shortcode_preview_disabled', true ),
				'strings'         => array(
					'pleaseSelectAForm'   => wp_strip_all_tags( __( 'Please select a form.', 'kdnaforms' ) ),
					'errorLoadingPreview' => wp_strip_all_tags( __( 'Failed to load the preview for this form.', 'kdnaforms' ) ),
				)
			) );
		}
		...
		if ( $page === 'form_editor' ) {
			...
			wp_localize_script( 'kdnaform_form_editor', 'kform_form_strings', $form_strings );
		...
		if ( self::has_members_plugin() && self::get_page_query_arg() === 'roles' ) {
		    wp_enqueue_style( 'kdnaform_dashicons' );
        }
		...
		KDNACommon::localize_legacy_check( 'kdnaform_layout_editor' );
	...
	public static function get_page() {
		...
		if ( $page == 'kdna_edit_forms' && ! rgget( 'id' ) ) {
			return 'form_list';
		}

		if ( $page == 'kdna_edit_forms' && count( explode(',', rgget( 'id' ) ) ) > 1 ) {
			return 'imported_forms_list';
		}

		if ( $page == 'kdna_edit_forms' && ! rgget( 'view' ) ) {
			return 'form_editor';
		}

		if ( $page == 'kdna_edit_forms' && rgget( 'view' ) == 'settings' && ( ! rgget( 'subview' ) || rgget( 'subview' ) == 'settings' ) ) {
			return 'form_settings';
		}

		if ( $page == 'kdna_edit_forms' && rgget( 'view' ) == 'settings' && rgget( 'subview' ) == 'personal-data' ) {
			return 'personal_data';
		}

		if ( $page == 'kdna_edit_forms' && rgget( 'view' ) == 'settings' && rgget( 'subview' ) == 'confirmation' ) {
			return 'confirmation';
		}

		if ( $page == 'kdna_edit_forms' && rgget( 'view' ) == 'settings' && rgget( 'subview' ) == 'notification' && rgget( 'nid' ) ) {
			return 'notification_edit';
		}

		if ( $page == 'kdna_edit_forms' && rgget( 'view' ) == 'settings' && rgget( 'subview' ) == 'notification' && isset( $_GET['nid'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			return 'notification_edit';
		}

		if ( $page == 'kdna_edit_forms' && rgget( 'view' ) == 'settings' && rgget( 'subview' ) == 'notification' ) {
			return 'notification_list';
		}

		if ( $page == 'kdna_edit_forms' && rgget( 'view' ) == 'settings' && rgget( 'subview' ) ) {
			return 'form_settings_' . rgget( 'subview' );
		}

		if ( $page == 'kdna_entries' && ( ! rgget( 'view' ) || rgget( 'view' ) == 'entries' ) ) {
			return 'entry_list';
		}

		if ( $page == 'kdna_entries' && rgget( 'view' ) == 'entry' && isset( $_POST['screen_mode'] ) && rgpost( 'screen_mode' ) == 'edit' ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			return 'entry_detail_edit';
		}

		if ( $page == 'kdna_entries' && rgget( 'view' ) == 'entry' ) {
			return 'entry_detail';
		}

		if ( $page == 'kdna_settings' ) {
			return 'settings';
		}
		...
		if ( $page == 'kdna_entries' && strpos( rgget( 'view' ), 'kdna_results' ) !== false ) {
			return 'results';
		}

		if ( $page == 'kdna_export' && ( rgget( 'subview' ) == 'export_entry' || ! isset( $_GET['subview'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			return 'export_entry';
		}

		if ( $page == 'kdna_export' && rgget( 'subview' ) == 'export_form' ) {
			return 'export_form';
		}

		if ( $page == 'kdna_export' && rgget( 'subview' ) == 'import_form' ) {
			return 'import_form';
		}

		if ( $page == 'kdna_system_status' ) {
			return rgget( 'subview' ) === 'updates' ? 'updates' : 'system_status';
		}

		if ( defined( 'DOING_AJAX' ) && DOING_AJAX && ( ( isset( $_POST['form_id'] ) && rgpost( 'action' ) === 'rg_select_export_form' ) || ( isset( $_POST['export_form'] ) && rgpost( 'action' ) === 'kdna_process_export' ) ) ) { //phpcs:ignore WordPress.Security.NonceVerification.Missing
	...
	public static function get_form( $form_id, $display_title = true, $display_description = true, $force_display = false, $field_values = null, $ajax = false, $tabindex = 0, $theme = null, $style_settings = null ) {
		...
		return KDNAFormDisplay::get_form( $form_id, $display_title, $display_description, $force_display, $field_values, $ajax, $tabindex, $theme, $style_settings );
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormDisplay::enqueue_scripts()
	 */
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormDisplay::print_form_scripts()
	 */
	public static function print_form_scripts( $form, $ajax ) {
		require_once( KDNACommon::get_base_path() . '/form_display.php' );
		KDNAFormDisplay::print_form_scripts( $form, $ajax );
	}

	/**
	 * Displays the Forms page
	 *
	 * Passes everything off to KDNAFormDetail::forms_page
	 *
	 * @since  Unknown
	 * @access public
	 *
	 * @uses   KDNAFormDetail::forms_page()
	 */
	public static function forms_page( $form_id ) {
		$styles = array( 'jquery-ui-styles', 'kdnaform_admin', 'kdnaform_settings', 'kdnaform_editor' );

		wp_print_styles( $styles );

		KDNAFormDetail::forms_page( $form_id );
	}

	/**
	 * Runs the KDNA Forms settings page.
	 *
	 ...
	 * @access public
	 *
	 * @uses   KDNASettings::settings_page()
	 */
	public static function settings_page() {
		...
		require_once( KDNACommon::get_base_path() . '/settings.php' );
		KDNASettings::settings_page();
	}

	/**
	 * Runs the KDNA Forms system status page.
	 *
	 * @since  2.2
	 * @access public
	 *
	...
	/**
	 * Adds pages to the KDNA Forms Settings page
	 *
	 * @since   Unknown
	 * @access  public
	 *
	 * @used-by KDNASettings::add_settings_page()
	 */
	public static function add_settings_page( $name, $handle = '', $icon_path = '' ) {
		require_once( KDNACommon::get_base_path() . '/settings.php' );
		KDNASettings::add_settings_page( $name, $handle, $icon_path );
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAHelp::help_page()
	 */
	public static function help_page() {
		require_once( KDNACommon::get_base_path() . '/help.php' );
		KDNAHelp::help_page();
	}

	/**
	 * Displays the KDNA Forms Export page
	 *
	 * @since  Unknown
	 * @access public
	 *
	 * @uses   KDNAForms::maybe_display_wizard()
	 * @uses   KDNAExport::export_page()
	 */
	public static function export_page() {
		...
		require_once( KDNACommon::get_base_path() . '/export.php' );
		KDNAExport::export_page();
	}

	/**
	 * Target for the wp_ajax_kdna_process_export ajax action requested from the export entries page.
	 *
	 * @since  2.0.0
	 * @access public
	 *
	 * @uses   KDNACommon::get_base_path()
	 * @uses   KDNAExport::ajax_process_export()
	 */
	public static function ajax_process_export() {

		require_once( KDNACommon::get_base_path() . '/export.php' );
		KDNAExport::ajax_process_export();
	}

	/**
	 * Target for the wp_ajax_kdna_download_export ajax action requested from the export entries page.
	 *
	 * @since  2.0.0
	 * @access public
	 *
	 * @uses   KDNACommon::get_base_path()
	 * @uses   KDNAExport::ajax_download_export()
	 */
	public static function ajax_download_export() {

		require_once( KDNACommon::get_base_path() . '/export.php' );
		KDNAExport::ajax_download_export();
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNACommon::dismiss_message()
	 */
	public static function ajax_dismiss_message() {
		...
		KDNACommon::dismiss_message( $key );
	}

	/**
	 * Target for the wp_ajax_kdna_disable_logging AJAX action requested from WordPress admin pages.
	 *
	 * @since  2.2.4.2
	 * @access public
	 *
	 * @uses   KDNACommon::get_base_path()
	 * @uses   KDNASettings::disable_logging()
	 */
	public static function ajax_disable_logging() {

		// Verify nonce.
		check_admin_referer( 'kdna_disable_logging_nonce', 'nonce' );
		...
		// Disable logging.
		$disabled = KDNASettings::disable_logging();

		if ( $disabled ) {
			wp_send_json_success( esc_html__( 'Logging disabled.', 'kdnaforms' ) );
		} else {
			wp_send_json_error( esc_html__( 'Unable to disable logging.', 'kdnaforms' ) );
		}

	}

	/**
	 * Target for the wp_ajax_kdna_force_upgrade ajax action requested from the System Status page.
	 *
	...
	public static function ajax_force_upgrade() {
		...
		check_ajax_referer( 'kdna_force_upgrade', 'nonce' );

		if ( ! KDNACommon::current_user_can_any( 'kdnaforms_uninstall' ) ) {
			wp_die( -1, 403 );
		}

		$status_label = get_option( 'kdnaform_upgrade_status' );

		if ( empty( $status_label ) ) {
			$status = 'complete';
			$status_label =  __( 'Finished', 'kdnaforms' );
	...
	public static function addons_page() {
		...
		wp_print_styles( array( 'thickbox', 'kdnaform_settings' ) );
		...
		$raw_response = KDNACommon::post_to_manager( 'api.php', "op=plugin_browser&{$_SERVER['QUERY_STRING']}", $options ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotValidatedNotSanitized

		if ( is_wp_error( $raw_response ) || $raw_response['response']['code'] != 200 ) {
			echo "<div class='error' style='margin-top:50px; padding:20px;'>" . esc_html__( 'Add-On browser is currently unavailable. Please try again later.', 'kdnaforms' ) . '</div>';
		} else {
	...
	public static function get_addon_info( $api, $action, $args ) {
		...
		if ( $action == 'plugin_information' && empty( $api ) && ( ! rgempty( 'rg', $_GET ) || $args->slug == 'kdnaforms' ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$key          = KDNACommon::get_key();
			$raw_response = KDNACommon::post_to_manager( 'api.php', "op=get_plugin&slug={$args->slug}&key={$key}", array() );
	...
	public static function get_addon_nonces() {
		...
		$raw_response = KDNACommon::post_to_manager( 'api.php', 'op=get_plugins', array() );
	 ...
	 * @access public
	 *
	 * @uses   KDNAExport::start_export()
	 */
	public static function start_export() {
		require_once( KDNACommon::get_base_path() . '/export.php' );
		KDNAExport::start_export();
	}
	...
	*/
	...
	public static function get_address_rule_values_select() {
		...
		$markup = sprintf( '<select id="%1$s" name="%1$s" class="kfield_rule_select kfield_rule_value_dropdown">%2$s</select>', esc_attr( $id ), $address_field->get_state_dropdown( $items, $value ) );
	 ...
	 * @access public
	 *
	 * @uses   KDNANotification::get_post_category_values()
	 */
	public static function get_notification_post_category_values() {
		require_once( KDNACommon::get_base_path() . '/notification.php' );
		KDNANotification::get_post_category_values();
	}
	 ...
	 *
	 * @uses   KDNAForms::maybe_display_wizard()
	 * @uses   KDNAEntryDetail::lead_detail_page()
	 * @uses   KDNAEntryList::all_entries_page()
	 */
	public static function all_leads_page() {
		...
		if ( $view == 'entry' && ( rgget( 'lid' ) || ! rgblank( rgget( 'pos' ) ) ) ) {
			require_once( KDNACommon::get_base_path() . '/entry_detail.php' );
			KDNAEntryDetail::lead_detail_page();
		} else if ( $view == 'entries' || empty( $view ) || ! has_action( 'kdnaform_entries_view' ) ) {
			require_once( KDNACommon::get_base_path() . '/entry_list.php' );
			KDNAEntryList::all_entries_page();
		} else {
			$form_id = rgget( 'id' );
			 ...
			 */
			do_action( 'kdnaform_entries_view', $view, $form_id, $lead_id );
		}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormList::form_list_page()
	 */
	public static function form_list_page() {
		require_once( KDNACommon::get_base_path() . '/form_list.php' );
		KDNAFormList::form_list_page();
	}
	 ...
	 *
	 * @uses   KDNAForms::maybe_display_wizard()
	 * @uses   KDNACommon::ensure_wp_version()
	 * @uses   KDNAEntryList::leads_page()
	 * @uses   KDNAEntryDetail::lead_detail_page()
	 * @uses   KDNAFormSettings::form_settings_page()
	 * @uses   KDNAForms::forms_page()
	 * @uses   KDNAForms::form_list_page()
	 */
	public static function forms() {
		...
		if ( $view == 'entries' ) {
			require_once( KDNACommon::get_base_path() . '/entry_list.php' );
			KDNAEntryList::leads_page( $id );
		} else if ( $view == 'entry' ) {
			require_once( KDNACommon::get_base_path() . '/entry_detail.php' );
			KDNAEntryDetail::lead_detail_page();
		} else if ( $view == 'notification' ) {
			require_once( KDNACommon::get_base_path() . '/notification.php' );
			//KDNANotification::notification_page($id);
		} else if ( $view == 'settings' ) {
			require_once( KDNACommon::get_base_path() . '/form_settings.php' );
			KDNAFormSettings::form_settings_page( $id );
		} else if ( empty( $view ) ) {
			if ( is_numeric( $id ) ) {
				self::forms_page( $id );
			} else {
		 ...
		 */
		do_action( 'kdnaform_view', $view, $id );
	 ...
	 * @access public
	 *
	 * @uses   KDNACommon::send_notification()
	 */
	public static function resend_notifications() {

		check_admin_referer( 'kdna_resend_notifications', 'kdna_resend_notifications' );
		...
		if ( 0 == $leads ) {
			...
			$leads = KDNAFormsModel::search_lead_ids( $form_id, $search_criteria );
		} else {
		 ...
		 */
		$form = kdna_apply_filters( array(
			'kdnaform_before_resend_notifications',
			$form_id
		), KDNAFormsModel::get_form_meta( $form_id ), $leads );

		if ( empty( $leads ) || empty( $form ) ) {
			esc_html_e( 'There was an error while resending the notifications.', 'kdnaforms' );
			die();
		};

		$notifications = json_decode( rgpost( 'notifications' ) );
		if ( ! is_array( $notifications ) ) {
			die( esc_html__( 'No notifications have been selected. Please select a notification to be sent.', 'kdnaforms' ) );
		}

		if ( ! rgempty( 'sendTo', $_POST ) && ! KDNACommon::is_valid_email_list( rgpost( 'sendTo' ) ) ) {
			die( sprintf( esc_html__( 'The %sSend To%s email address provided is not valid.', 'kdnaforms' ), '<strong>', '</strong>' ) );
		}

		foreach ( $leads as $lead_id ) {

			$lead = KDNAFormsModel::get_lead( $lead_id );
			foreach ( $notifications as $notification_id ) {
				 ...
				 */
				$abort_email = apply_filters( 'kdnaform_disable_resend_notification', false, $notification, $form, $lead );

				if ( ! $abort_email ) {
					KDNACommon::send_notification( $notification, $form, $lead );
				}
				 ...
				 * @param array $lead         The current entry object.
				 */
				do_action( 'kdnaform_post_resend_notification', $notification, $form, $lead );
			}
		}
		 ...
		 * @param array $lead The current entry object.
		 */
		do_action( 'kdnaform_post_resend_all_notifications', $form, $lead );
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormsModel::update_form_active()
	 */
	public static function update_form_active() {
		check_ajax_referer( 'rg_update_form_active', 'rg_update_form_active' );

		if ( KDNACommon::current_user_can_any( 'kdnaforms_edit_forms' ) ) {
			KDNAFormsModel::update_form_active( rgpost( 'form_id' ), rgpost( 'is_active' ) );
		} else {
			wp_die( -1, 403 );
		}
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormsModel::update_notification_active()
	 */
	public static function update_notification_active() {
		...
		if ( KDNACommon::current_user_can_any( 'kdnaforms_edit_forms' ) ) {
			KDNAFormsModel::update_notification_active( rgpost( 'form_id' ), rgpost( 'notification_id' ), rgpost( 'is_active' ) );
		} else {
			wp_die( -1, 403 );
		}
	}
	 ...
	 * @access public
	 *
	 * @since  KDNAFormsModel::update_confirmation_active()
	 */
	public static function update_confirmation_active() {
		...
		if ( KDNACommon::current_user_can_any( 'kdnaforms_edit_forms' ) ) {
			KDNAFormsModel::update_confirmation_active( rgpost( 'form_id' ), rgpost( 'confirmation_id' ), rgpost( 'is_active' ) );
		} else {
			wp_die( -1, 403 );
		}
	}
	 ...
	 * @access public
	 *
	 * @uses KDNAFormsModel::update_entry_property()
	 */
	public static function update_lead_property() {
		...
		if ( KDNACommon::current_user_can_any( 'kdnaforms_edit_entries' ) ) {
			KDNAFormsModel::update_entry_property( rgpost( 'lead_id' ), rgpost( 'name' ), rgpost( 'value' ) );
		} else {
			wp_die( -1, 403 );
		}
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormsModel::update_lead_property()
	 * @uses   KDNAFormsModel::delete_lead()
	 */
	public static function update_lead_status() {
		check_ajax_referer( 'kdna_delete_entry' );
		...
		$entry = KDNAAPI::get_entry( $lead_id );
		$form  = KDNAAPI::get_form( $entry['form_id'] );

		switch ( $status ) {
			case 'unspam':
				KDNAFormsModel::restore_entry_status( $lead_id );
				break;
			...
			case 'delete':
				if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
					KDNAFormsModel::delete_entry( $lead_id );
				}
				break;

			case 'trash':
				if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
					KDNAFormsModel::change_entry_status( $lead_id, 'trash' );
				}
				break;

			default :
				KDNAFormsModel::change_entry_status( $lead_id, $status );
				break;
		}
		require_once( 'entry_list.php' );


		$filter_links = KDNAEntryList::get_filter_links( $form );
	 ...
	 * @access public
	 *
	 * @uses   KDNASettings::upgrade_license()
	 */
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormDetail::save_form()
	 */
	public static function save_form() {
		require_once( KDNACommon::get_base_path() . '/form_detail.php' );
		KDNAFormDetail::save_form();
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormDetail::add_field()
	 */
	public static function add_field() {
		require_once( KDNACommon::get_base_path() . '/form_detail.php' );
		KDNAFormDetail::add_field();
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormDetail::duplicate_field()
	 */
	public static function duplicate_field() {
		require_once( KDNACommon::get_base_path() . '/form_detail.php' );
		KDNAFormDetail::duplicate_field();
	}
	 ...
	 * @access public
	 *
	 * @uses   \KDNAFormDetail::delete_field()
	 */
	public static function delete_field() {
		require_once( KDNACommon::get_base_path() . '/form_detail.php' );
		KDNAFormDetail::delete_field();
	}
	 ...
	 * @access public
	 *
	 * @uses   \KDNAFormDetail::ajax_get_form()
	 */
	public static function ajax_get_form() {
		require_once( KDNACommon::get_base_path() . '/form_detail.php' );
		KDNAFormDetail::ajax_get_form();
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormDetail::change_input_type()
	 */
	public static function change_input_type() {
		require_once( KDNACommon::get_base_path() . '/form_detail.php' );
		KDNAFormDetail::change_input_type();
	}
	 ...
	 * @access public
	 *
	 * @uses   \KDNAFormDetail::refresh_field_preview
	 */
	public static function refresh_field_preview() {
		require_once( KDNACommon::get_base_path() . '/form_detail.php' );
		KDNAFormDetail::refresh_field_preview();
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormDetail::delete_custom_choice()
	 */
	public static function delete_custom_choice() {
		require_once( KDNACommon::get_base_path() . '/form_detail.php' );
		KDNAFormDetail::delete_custom_choice();
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormDetail::save_custom_choice()
	 */
	public static function save_custom_choice() {
		require_once( KDNACommon::get_base_path() . '/form_detail.php' );
		KDNAFormDetail::save_custom_choice();
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormsModel::delete_file()
	 */
	public static function delete_file() {
		...
		KDNAFormsModel::delete_file( $lead_id, $field_id, $file_index );
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormsModel::get_form_meta()
	 */
	public static function select_export_form() {
		 ...
		 */
		$form = kdna_apply_filters( array( 'kdnaform_form_export_page', $form_id ), $form );

		$filter_settings      = KDNACommon::get_field_filter_settings( $form );
		$filter_settings_json = json_encode( $filter_settings );
		$fields               = array();

		$form = KDNAExport::add_default_export_fields( $form );
			...
			foreach ( $form['fields'] as $field ) {
				...
				if ( is_array( $inputs ) ) {
					foreach ( $inputs as $input ) {
						$fields[] = array( $input['id'], KDNACommon::get_label( $field, $input['id'] ) );
					}
				} else if ( ! $field->displayOnly ) {
					$fields[] = array( $field->id, KDNACommon::get_label( $field ) );
				}
			}
		}
		$field_json = KDNACommon::json_encode( $fields );
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormSettings::save_confirmation()
	 */
	//	public static function save_confirmation() {
	//		require_once( KDNACommon::get_base_path() . '/form_settings.php' );
	//		KDNAFormSettings::save_confirmation();
	//	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormSettings::save_form_title()
	 */
	public static function save_form_title() {
		require_once( KDNACommon::get_base_path() . '/form_settings.php' );
		KDNAFormSettings::save_form_title();
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormSettings::delete_confirmation()
	 */
	public static function delete_confirmation() {
		require_once( KDNACommon::get_base_path() . '/form_settings.php' );
		KDNAFormSettings::delete_confirmation();
	}
	 ...
	 * @access public
	 *
	 * @uses   KDNAFormList::save_new_form()
	 */
	public static function save_new_form() {
		require_once( KDNACommon::get_base_path() . '/form_list.php' );
		KDNAFormList::save_new_form();
	}
	...
	public static function check_background_tasks() {
		check_ajax_referer( 'kdna_check_background_tasks', 'nonce' );
		echo 'ok';
		die();
	}
	...
	public static function edit_form_title( $form ) {
		...
		<div id="edit-title-container" class="add_field_button_container">
			...
			<div class="add-buttons">
				...
				<div class="edit-form-footer">
					<input type="button" value="<?php esc_html_e( 'Update', 'kdnaforms' ); ?>" class="button-primary" onclick="KDNA_SaveTitle();" />
					<span id="kdnaform_settings_page_title_error"></span>
				</div>
			</div>
		...
		<script type="text/javascript">
			function KDNA_ShowEditTitle() {
				jQuery('#edit-title-container').css('visibility', 'visible')
					.find('#edit-title-input').focus();
			}

			function KDNA_CloseEditTitle() {
				jQuery('#edit-title-container').css('visibility', 'hidden');

				jQuery('#edit-title-input').val( jQuery('#kdnaform_settings_page_title').text() );
				jQuery('#kdnaform_settings_page_title_error').text('');
			}

			function KDNA_SaveTitle(){

				var title = jQuery( '#edit-title-input' ).val();

				jQuery.post(ajaxurl, {
					action       : "kdna_save_title",
					kdna_save_title: '<?php echo esc_js( wp_create_nonce( 'kdna_save_title' ) ); ?>',
					...
					.done(function (data) {
						...
						if ( !isValid ) {
							...
							var errorMessage = result ? result.message : '<?php esc_attr_e( 'Oops! There was an error saving the form title. Please refresh the page and try again.', 'kdnaforms' ); ?>' ;

							jQuery('#kdnaform_settings_page_title_error').text(errorMessage);
						}
						else {
							var title = jQuery('#edit-title-input').val();
							jQuery('#kdnaform_settings_page_title').text(title);
							jQuery('#form_title_input').val(title);
							<?php echo KDNACommon::is_form_editor() ? 'form.title = title;' : ''; ?>
					...
					.fail(function () {
						alert('<?php esc_attr_e( 'Oops! There was an error saving the form title. Please refresh the page and try again.', 'kdnaforms' ); ?>');
						KDNA_CloseEditTitle();
					});

			}

			function KDNA_IsOutsideTitleWindow(element) {
				var parents = jQuery(element).parents('#edit-title-container');
				return parents.length == 0;
			}

			jQuery(document).mousedown(function (event) {
				if (KDNA_IsOutsideTitleWindow(event.target)) {
					KDNA_CloseEditTitle();
				}
			});
	...
	public static function form_switcher( $title = '', $form_id = '' ) {
		...
		$recent_forms = KDNAFormsModel::get_recent_forms();
		$forms        = array();
		foreach( $recent_forms as $recent_form_id ) {
			$form = KDNAFormsModel::get_form( $recent_form_id );
			if ( $form ) {
				$forms[] = $form;
			}
		}
		...
		<article class="kform-dropdown" data-js="kform-form-switcher">
			<span
				class="kform-visually-hidden"
				id="kform-form-switcher-label"
			><?php esc_attr_e( 'Select a different form', 'kdnaforms' ); ?></span>
			<button
			    type="button"
				aria-expanded="false"
				aria-haspopup="listbox"
				aria-labelledby="kform-form-switcher-label kform-form-switcher-control"
				class="kform-dropdown__control"
				data-js="kform-dropdown-control"
				id="kform-form-switcher-control"
				data-value="<?php esc_attr_e( $form_id ); ?>"
				title="<?php echo esc_attr( $title ); ?>"
			>
				<span class="kform-dropdown__control-text" data-js="kform-dropdown-control-text">
				    <?php echo esc_html( $title ); ?>
				</span>
				<i class="kform-spinner kform-dropdown__spinner"></i>
				<i class="kform-icon kform-icon--chevron kform-dropdown__chevron" aria-hidden="true"></i>
			</button>
			<div
				aria-labelledby="kform-form-switcher-label"
			...
			>
				<div class="kform-dropdown__search">
					<label for="kform-form-switcher-search" class="kform-visually-hidden"><?php esc_attr_e( 'Search forms', 'kdnaforms' ); ?></label>
					<input
						id="kform-form-switcher-search"
						type="text" class="kform-input kform-dropdown__search-input"
						placeholder="<?php esc_attr_e( 'Search for form', 'kdnaforms' ); ?>"
						data-js="kform-dropdown-search"
					/>
					<i class="kform-icon kform-icon--search kform-dropdown__search-icon"></i>
				</div>
				<div class="kform-dropdown__list-container" data-simplebar<?php echo is_rtl() ? ' data-simplebar-direction="rtl"' : ''; ?>>
					<ul class="kform-dropdown__list" data-js="kform-dropdown-list">
					<?php
						foreach ( $forms as $form_info ) {
							printf(
								'
									<li class="kform-dropdown__item">
										<button type="button" class="kform-dropdown__trigger" data-js="kform-dropdown-trigger" data-value="%1$d" %2$s %3$s title="%4$s">
											<span class="kform-dropdown__trigger-text" data-value="%1$d">%4$s</span>
										</button>
									</li>
									',
								absint( $form_info->id ),
			...
			<input type="hidden" data-js="kdna-form-switcher-input" name="_kdnaform_form_switcher" value=""/>
		</article>
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function () {
				...
				jQuery('a.kdna_toolbar_disabled').click(function (event) {
	...
	protected static function get_form_switcher_results_page_slug( $form_id ) {
		...
		$cached_result =  KDNACache::get( 'has_results_page_' . $form_id );
		...
		if ( rgar( $form, 'id' ) && rgar( $form, 'fields' ) ) {
			...
			foreach ( KDNAAddOn::get_results_addon() as $results_addon  ) {

				$addon = rgars( $results_addon, 'callbacks/fields/0');

				if (
					$addon
					 && is_a( $addon, 'KDNAAddOn' )
		...
		KDNACache::set( 'has_results_page_' . rgar( $form, 'id' ), $results_addon_slug, true, HOUR_IN_SECONDS );
	...
	public static function get_form_switcher_subview_attr( $form_id ) {
		...
		$form_subviews = KDNAFormSettings::get_tabs( $form_id );
		$subview_list  = [];
		foreach( $form_subviews as $subview ) {
			$subview_list[] = $subview['name'];
		}
		$subview_list[] = 'kdna_theme_layers';
	...
	*/
	public static function admin_header( $tabs = array(), $toolbar = true ) {
		// Print admin styles.
		wp_print_styles( array( 'jquery-ui-styles', 'kdnaform_admin', 'kdnaform_settings', 'wp-pointer' ) );

		// Set class for display mode on entries list page.
		$view_class = null;
		if ( self::get_page_query_arg() === 'kdna_entries' && ! isset( $_GET['lid'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			if ( class_exists( 'KDNAEntryList' ) ) {
				$option_values = KDNAEntryList::get_screen_options_values();
				$view_class    = ( $option_values['display_mode'] === 'full_width' ) ? ' kdnaform_form_settings_wrap--full-width' : null;
			}
		}
		?>
		...
		<?php
			...
			$wrapper_classes = ! empty( $tabs ) ? 'kform-settings__wrapper' : 'kform-settings__wrapper kform-settings__wrapper--full';
		?>

			<?php echo KDNACommon::get_remote_message(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>

			<?php KDNACommon::notices_section(); ?>
			...
			?>
				...
				<nav class="kform-settings__navigation">
					<?php
						...
						foreach ( $tabs as $tab ) {
							...
							if ( rgar( $tab, 'capabilities' ) && ! KDNACommon::current_user_can_any( $tab['capabilities'] ) ) {
								continue;
							}
				...
				<div class="kform-settings__content" <?php echo isset( $current_tab ) ? 'id="tab_' . esc_attr( $current_tab ) . '"' : ''; ?>>
		<?php
	}
	...
	*/
	...
	public static function add_admin_body_class( $body_classes ) {
		...
		$classes = array_merge( $classes, array( 'kform-admin' ) );

		if ( KDNACommon::is_form_editor() && wp_style_is( 'jetpack-admin-menu' ) && ! is_rtl() ) {
			$classes[] = 'kform-jetpack-admin-menu';
		}

		if ( self::is_kdna_page() ) {
			$classes[] = 'kform-admin-screen';
		}
	...
	/**
	 * Displays the top toolbar within KDNA Forms pages.
	 *
	 * @since  Unknown
	 * @access public
	 *
	 * @uses   KDNAFormsModel::get_forms()
	 * @uses   KDNAForms::get_toolbar_menu_items()
	 * @uses   KDNAForms::format_toolbar_menu_items()
	 */
	public static function top_toolbar() {
		...
		<div id="kform-form-toolbar" class="kform-form-toolbar">
			...
			<div class="kform-form-toolbar__container">

				<div class="kform-form-toolbar__form-title">
				...
				<div id="kdna_toolbar_buttons_container" class="kform-form-toolbar__buttons kdna_toolbar_buttons_container">
					<?php
					$preview_args = array(
						'form_id' => $id,
					);
	...
	/**
	 * Formats the menu items for display in the KDNA Forms toolbar.
	 *
	 * @since   Unknown
	 * @access  public
	 *
	 * @used-by KDNAForms::top_toolbar()
	 * @uses    KDNAForms::toolbar_sub_menu_items()
	 *
	 * @param array $menu_items Contains the menu items to be displayed
	...
	public static function format_toolbar_menu_items( $menu_items, $compact = false ) {
		...
		foreach ( $menu_items as $key => $menu_item ) {
			if ( is_array( $menu_item ) ) {
				if ( KDNACommon::current_user_can_any( rgar( $menu_item, 'capabilities' ) ) ) {
					$sub_menu_str         = '';
					...
					if ( is_array( $sub_menu_items ) ) {
						foreach ( $sub_menu_items as $k => $val ) {
							if ( false === KDNACommon::current_user_can_any( rgar( $sub_menu_items[ $k ], 'capabilities' ) ) ) {
					...
					if ( $compact ) {
						if ( $key == 'delete' ) {
							 ...
							 */
							$link = apply_filters( 'kdnaform_form_delete_link', $link );
						}
	...
	public static function get_toolbar_menu_items( $form_id, $compact = false ) {
		...
		$edit_capabilities = array( 'kdnaforms_edit_forms' );
		...
		if ( $page != 'form_editor' ) {
			$menu_items['edit'] = array(
				'label'        => esc_html__( 'Edit', 'kdnaforms' ),
				'short_label'  => esc_html__( 'Editor', 'kdnaforms' ),
				'aria-label'   => esc_html__( 'Editor', 'kdnaforms' ),
				'icon'         => '<i class="fa fa-pencil-square-o fa-lg"></i>',
				'url'          => '?page=kdna_edit_forms&id=' . $form_id,
				'menu_class'   => 'kdna_form_toolbar_editor',
		...
		$menu_items['settings'] = array(
			'label'          => esc_html__( 'Settings', 'kdnaforms' ),
			'url'            => $is_mobile ? '#' : '?page=kdna_edit_forms&view=settings&id=' . $form_id,
			'menu_class'     => 'kdna_form_toolbar_settings',
		...
		$entries_capabilities = array(
			'kdnaforms_view_entries',
			'kdnaforms_edit_entries',
			'kdnaforms_delete_entries'
		);
		$menu_items['entries'] = array(
			'label'        => esc_html__( 'Entries', 'kdnaforms' ),
			'url'          => '?page=kdna_entries&id=' . $form_id,
			'menu_class'   => 'kdna_form_toolbar_entries',
		...
		if ( in_array( $page, array( 'form_list', 'imported_forms_list', 'new_form' ) ) ) {
			...
			$menu_items['preview'] = KDNACommon::get_preview_link_data( $preview_args );
		}
	...
	/**
	 * Builds the sub-menu items within the KDNA Forms toolbar.
	 *
	 * @since   Unknown
	 * @access  public
	 *
	 * @used-by KDNAForms::format_toolbar_menu_items()
	 *
	...
	public static function toolbar_sub_menu_items( $menu_items, $compact = false ) {
		...
		foreach ( $menu_items as $menu_item ) {
			if ( KDNACommon::current_user_can_any( rgar( $menu_item, 'capabilities' ) ) ) {
				...
				$icon       = rgar( $menu_item, 'icon' );
				$sub_menu_items_string .= "<li class='{$menu_class}'><a href='{$url}' class='{$link_class}' target='{$target}'><span class='kform-form-toolbar__icon'>{$icon}</span> {$label}</a></li>";
			}
		}

		$simplebar_rtl_attr = is_rtl() ? ' data-simplebar-direction="rtl"' : '';
		$sub_menu_items_string = sprintf(
			'<div class="kform-form-toolbar__submenu"><div data-simplebar%s><ul>' . $sub_menu_items_string . '</ul></div></div>',
			$simplebar_rtl_attr
		);
	 ...
	 * @access  public
	 *
	 * @used-by KDNAForms::get_toolbar_menu_items()
	 * @uses    KDNAFormSettings::get_tabs()
	 *
	 * @param string $form_id The form ID.
	 *
	...
	public static function get_form_settings_sub_menu_items( $form_id ) {
		...
		$tabs           = KDNAFormSettings::get_tabs( $form_id );
		...
		foreach ( $tabs as $tab ) {
            ...
            $url = admin_url( "admin.php?page=kdna_edit_forms&view=settings&subview={$tab['name']}&id={$form_id}" );
			...
			$sub_menu_items[] = array(
				'url'          => esc_url( $url ),
				'label'        => $tab['label'],
				'icon'         => KDNACommon::get_icon_markup( $tab ),
				'capabilities' => ( isset( $tab['capabilities'] ) ) ? $tab['capabilities'] : array( 'kdnaforms_edit_forms' ),
			);
	 ...
	 * @used-by KDNAForms::get_toolbar_menu_items()
	 *
	 * @param string $item The KDNA Forms view (current page).
	 *
	...
	private static function toolbar_class( $item ) {
		...
		switch ( $item ) {
			...
			case 'preview' :
				if ( self::get_page_query_arg() == 'kdna_new_form' ) {
					return 'kdna_toolbar_disabled';
				}
	...
	/**
	 * Modifies the top WordPress toolbar to add KDNA Forms menu items.
	 *
	 * @since   Unknown
	 * @access  public
	 * @global $wp_admin_bar
	 *
	 * @used-by KDNAForms::init()
	 */
	public static function admin_bar() {
		...
		if ( ! get_option( 'kdnaform_enable_toolbar_menu' ) ) {
			return;
		}

		if ( ! KDNACommon::current_user_can_any( array(
			'kdnaforms_edit_forms',
			'kdnaforms_create_form',
			'kdnaforms_preview_forms',
			'kdnaforms_view_entries'
		) )
		) {
		...
		$recent_form_ids = KDNAFormsModel::get_recent_forms();

		if ( $recent_form_ids ) {
			$forms = KDNAFormsModel::get_form_meta_by_id( $recent_form_ids );

			$wp_admin_bar->add_node(
				array(
					'id'     => 'kform-form-recent-forms',
					'parent' => 'kform-forms',
			...
			foreach ( $recent_form_ids as $recent_form_id ) {
				...
				foreach ( $forms as $form ) {
					if ( $form['id'] == $recent_form_id ) {
						$wp_admin_bar->add_node(
							array(
								'id'     => 'kform-form-' . $recent_form_id,
								'parent' => 'kform-form-recent-forms',
						...
						if ( KDNACommon::current_user_can_any( 'kdnaforms_edit_forms' ) ) {
							$wp_admin_bar->add_node(
								array(
									'id'     => 'kform-form-' . $recent_form_id . '-edit',
									'parent' => 'kform-form-' . $recent_form_id,
						...
						if ( KDNACommon::current_user_can_any( 'kdnaforms_view_entries' ) ) {
							$wp_admin_bar->add_node(
								array(
									'id'     => 'kform-form-' . $recent_form_id . '-entries',
									'parent' => 'kform-form-' . $recent_form_id,
						...
						if ( KDNACommon::current_user_can_any( 'kdnaforms_edit_forms' ) ) {
							$wp_admin_bar->add_node(
								array(
									'id'     => 'kform-form-' . $recent_form_id . '-settings',
									'parent' => 'kform-form-' . $recent_form_id,
						...
						if ( KDNACommon::current_user_can_any( array(
							...
							$wp_admin_bar->add_node(
								array(
									'id'     => 'kform-form-' . $recent_form_id . '-preview',
									'parent' => 'kform-form-' . $recent_form_id,
	 ...
	 * @uses    KDNAForms::is_auto_update_disabled()
	 *
	 * @param bool|null $update          Whether to update.
	 * @param object    $item            The update offer object.
	 * @param string    $slug            The plugin or add-on slug. Optional. Defaults to kdnaforms.
	 * @param string    $current_version The current version. Optional. Defaults to the KDNA Forms version.
	 *
	 * @return bool|null
	 */
	public static function maybe_auto_update( $update, $item, $slug = 'kdnaforms', $current_version = null ) {
		...
		KDNACommon::log_debug( __METHOD__ . sprintf( '(): Checking if auto-update available for %s.', $slug ) );

		if ( self::is_auto_update_disabled( $update, $slug ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): Aborting. Auto-update is disabled.' );
		...
		if ( is_null( $current_version ) ) {
			if ( $slug === 'kdnaforms' ) {
				$current_version = KDNAForms::$version;
			} else {
				// translators: %s: Add-on slug.
				_doing_it_wrong( __METHOD__, sprintf( esc_html__( 'The $current_version parameter is required when checking for auto-updates for %s.', 'kdnaforms' ), esc_html( $slug ) ), '2.9.24' );
				KDNACommon::log_debug( __METHOD__ . '(): Aborting. $current_version was not provided.' );
		...
		if ( version_compare( $current_version, $item->new_version, '>=' ) ) {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): Aborting. Newer %s version not available. Installed: %s; Available: %s.', $slug, $current_version, $item->new_version ) );

			return false;
		}

		if ( ! self::should_update_to_version( $item->new_version, $current_version ) ) {
			KDNACommon::log_debug( __METHOD__ . sprintf( '(): Aborting. Automatically updating %s from %s to %s is not supported.', $slug, $current_version, $item->new_version ) );

			return false;
		}

		KDNACommon::log_debug( __METHOD__ . sprintf( '(): Automatically updating %s from %s to %s is supported.', $slug, $current_version, $item->new_version ) );
	 ...
	 * @param null|string $current_version The current version number. If null, the installed KDNA Forms version number will be used.
	 *
	 * @return bool
	 */
	 ...
	 * @access  public
	 *
	 * @used-by KDNAForms::maybe_auto_update()
	 *
	 * @param bool|null $enabled Indicates if auto updates are enabled.
	 * @param string    $slug    The plugin or add-on slug. Optional. Default is 'kdnaforms'.
	 *
	 * @return bool True if auto update is disabled.  False otherwise.
	 */
	public static function is_auto_update_disabled( $enabled = null, $slug = 'kdnaforms' ) {
		if ( is_null( $enabled ) ) {
			// Check KDNA Forms Background Update Settings.
			$enabled = (bool) get_option( 'kdnaform_enable_background_updates' );
		}

		KDNACommon::log_debug( __METHOD__ . '(): Enabled by toggle? ' . ( $enabled ? 'Yes.' : 'No.' ) );

		/**
		 * Filter to disable KDNA Forms Automatic updates
		 *
		 * @since 1.9
		 ...
		 */
		$disabled = apply_filters( 'kdnaform_disable_auto_update', ! $enabled, $slug );
		KDNACommon::log_debug( __METHOD__ . '(): Disabled after kdnaform_disable_auto_update filter? ' . ( $disabled ? 'Yes.' : 'No.' ) );

		if ( ! $disabled ) {
			$disabled = defined( 'KDNAFORM_DISABLE_AUTO_UPDATE' ) && KDNAFORM_DISABLE_AUTO_UPDATE;
			KDNACommon::log_debug( __METHOD__ . '(): Disabled by constant KDNAFORM_DISABLE_AUTO_UPDATE? ' . ( $disabled ? 'Yes.' : 'No.' ) );
		}
	...
	public static function auto_update_message( $html, $plugin_file, $plugin_data ) {
		// Check if the plugin is KDNA Forms or an add-on.
		if ( ! self::is_kdna_or_addon( $plugin_data['PluginURI'] ) ) {
			return $html;
		}
		...
		return esc_html__( 'Auto-updates unavailable.', 'kdnaforms' );
	}
	...
	public static function auto_update_debug_message( $auto_updates_string, $plugin_path, $plugin, $enabled ) {
		// Check if the plugin is KDNA Forms or an add-on.
		if ( ! self::is_kdna_or_addon( $plugin['PluginURI'] ) ) {
		...
		return __( 'Please register your copy of KDNA Forms to enable automatic updates.', 'kdnaforms' );
	}

	/**
	 * Check if a plugin is KDNA Forms or an offical add-on.
	 *
	 * @since 2.4.20.2
	 *
	 ...
	 * @return bool
	 */
	public static function is_kdna_or_addon( $plugin_uri ) {
		if ( strpos( $plugin_uri, 'kdnaforms.com' ) ) {
			return true;
		} else {
			return false;
		}
	...
	public static function deprecate_add_on_methods() {
		...
		$deprecated = KDNAAddOn::get_all_deprecated_protected_methods();
		if ( ! empty( $deprecated ) ) {
	...
	public static function handle_ajax_do_shortcode() {
		...
		if ( ! current_user_can( 'edit_post', $post_id ) || ! wp_verify_nonce( rgpost( 'nonce' ), 'kdna-shortcode-ui-preview' ) ) {
			echo esc_html__( 'Error', 'kdnaforms' );
			exit;
		}
	 ...
	 *
	 * @used-by KDNAForms::init()
	 * @used    KDNAForms::get_view()
	 *
	 * @return void
	 */
	public static function action_print_media_templates() {

		echo KDNAForms::get_view( 'edit-shortcode-form' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	 ...
	 * @access  public
	 *
	 * @used-by KDNAForms::action_print_media_templates()
	 *
	...
	public static function get_view( $template ) {
		...
		if ( ! file_exists( $template ) ) {
			...
			$template_dir = KDNACommon::get_base_path() . '/includes/templates/';
	...
	public static function get_shortcodes() {
		...
		$forms             = KDNAAPI::get_forms( true, false, 'title' );
		$forms_options[''] = __( 'Select a Form', 'kdnaforms' );
		foreach ( $forms as $form ) {
		 ...
		 */
		$forms_options = apply_filters( 'kdnaform_shortcode_builder_forms', $forms_options );

		$default_attrs = array(
			array(
				'label'       => __( 'Select a form below to add it to your post or page.', 'kdnaforms' ),
				'tooltip'     => __( 'Select a form from the list to add it to your post or page.', 'kdnaforms' ),
				'attr'        => 'id',
				'type'        => 'select',
				'section'     => 'required',
				'description' => __( "Can't find your form? Make sure it is active.", 'kdnaforms' ),
				'options'     => $forms_options,
			),
			array(
				'label'   => __( 'Display form title', 'kdnaforms' ),
				'attr'    => 'title',
				'default' => 'true',
				'section' => 'standard',
				'type'    => 'checkbox',
				'tooltip' => __( 'Whether or not to display the form title.', 'kdnaforms' )
			),
			array(
				'label'   => __( 'Display form description', 'kdnaforms' ),
				'attr'    => 'description',
				'default' => 'true',
				'section' => 'standard',
				'type'    => 'checkbox',
				'tooltip' => __( 'Whether or not to display the form description.', 'kdnaforms' )
			),
			array(
				'label'   => __( 'Enable Ajax', 'kdnaforms' ),
				'attr'    => 'ajax',
				'section' => 'standard',
				'type'    => 'checkbox',
				'tooltip' => __( 'Specify whether or not to use Ajax to submit the form.', 'kdnaforms' )
			),
			array(
				'label'   => 'Tabindex',
				'attr'    => 'tabindex',
				'type'    => 'number',
				'tooltip' => __( 'Specify the starting tab index for the fields of this form.', 'kdnaforms' )
			),
		 ...
		 */
		$add_on_actions = apply_filters( 'kdnaform_shortcode_builder_actions', array() );

		if ( ! empty( $add_on_actions ) ) {
			$action_options = array( '' => __( 'Select an action', 'kdnaforms' ) );
			...
			$default_attrs[] = array(
				...
				'tooltip' => __( 'Select an action for this shortcode. Actions are added by some add-ons.', 'kdnaforms' )
			);
		}

		$shortcode = array(
			'shortcode_tag' => 'kdnaform',
			'action_tag'    => '',
			'label'         => 'KDNA Forms',
			'attrs'         => $default_attrs,
		);
	 ...
	 * @access public
	 *
	 * @used   KDNAFormDisplay::enqueue_form_scripts()
	 * @used   KDNAAddOn::get_registered_addons()
	 *
	...
	public static function enqueue_form_scripts( $form_id, $is_ajax = false ) {
		require_once( KDNACommon::get_base_path() . '/form_display.php' );
		$form = KDNAFormsModel::get_form_meta( $form_id );
		KDNAFormDisplay::enqueue_form_scripts( $form, $is_ajax );
		$addons = KDNAAddOn::get_registered_addons();
		foreach ( $addons as $addon ) {
	...
	public static function maybe_display_wizard() {
		...
		return kdna_upgrade()->maybe_display_wizard();
	}
	...
	public static function maybe_display_logging_notice() {
		...
		$notice_disabled = defined( 'KDNA_LOGGING_DISABLE_NOTICE' ) && KDNA_LOGGING_DISABLE_NOTICE;
		$logging_enabled = get_option( 'kdnaform_enable_logging', false ) || is_plugin_active( 'kdnaformslogging/logging.php' );

		// If logging is disabled, return.
		if ( $notice_disabled || ! $logging_enabled || ! KDNACommon::current_user_can_any( 'kdnaforms_edit_settings' ) ) {
			return;
		}
		...
		$message = sprintf(
			...
			esc_html__( 'KDNA Forms logging is currently enabled. ', 'kdnaforms' ),
			esc_html__( 'If you currently have a support ticket open, please do not disable logging until the Support Team has reviewed your logs. ', 'kdnaforms' ),
			esc_html__( 'Since logs may contain sensitive information, please ensure that you only leave it enabled for as long as it is needed for troubleshooting. ', 'kdnaforms' ),
			sprintf(
				esc_html__( 'Once troubleshooting is complete, %1$sclick here to disable logging and permanently delete your log files.%2$s ', 'kdnaforms' ),
				'<a href="' . esc_url( admin_url( 'admin.php?page=kdna_settings' ) ) . '">',
				'</a>'
			)
		);
		...
		$script = "<script type='text/javascript'>
			jQuery( document ).on( 'click', '#kdnaform_disable_logging_notice a', function( e ) {
				e.preventDefault();
				var container = jQuery( '#kdnaform_disable_logging_notice' );
				jQuery.ajax( {
					url: ajaxurl,
					dataType: 'json',
					data: {
						action: 'kdna_disable_logging',
						nonce:  container.data( 'nonce' )
					},
					success: function( response ) {
						if ( response.success ) {
							container.removeClass( 'notice-error' ).addClass( 'notice-success' );
							jQuery( '#_kdnaform_setting_enable_logging' ).prop( 'checked', false );
						}
		...
		printf( '<div class="notice notice-error kdna-notice" id="kdnaform_disable_logging_notice" data-nonce="%s">%s</div>%s', esc_attr( wp_create_nonce( 'kdna_disable_logging_nonce' ) ), $message, $script ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	 ...
	 * @used    KDNAEntryList::get_screen_options_markup()
	 *
	...
	public static function show_screen_options( $status, $args ) {
		...
		if ( self::get_page() == 'entry_list' ) {
			require_once( KDNACommon::get_base_path() . '/entry_list.php' );
			$return = KDNAEntryList::get_screen_options_markup( $status, $args );
		}

		if ( self::get_page() == 'form_list' ) {
			require_once( KDNACommon::get_base_path() . '/form_list.php' );
			$return = KDNAFormList::get_screen_options_markup( $status, $args );
		}
	 ...
	 * @access public
	 *
	 * @used   KDNAEntryDetail::add_meta_boxes()
	 */
	public static function load_screen_options() {
		...
		if ( in_array( $page, array( 'entry_detail', 'entry_detail_edit' ) ) ) {
			...
			KDNAEntryDetail::add_meta_boxes();
		}
	}

	/**
	 * Daily cron task. Target for the kdnaforms_cron action.
	 *
	 ...
	 * @access  public
	 *
	 * @used-by Action: kdnaforms_cron
	 * @used    KDNAForms::add_security_files()
	 * @used    KDNAForms::delete_old_export_files()
	 * @used    KDNAForms::delete_old_log_files()
	 * @used    KDNAForms::do_self_healing()
	 * @used    KDNAForms::delete_orphaned_entries()
	 */
	public static function cron() {

		KDNACommon::log_debug( __METHOD__ . '(): Starting cron.' );
		KDNACommon::record_cron_event( 'kdnaforms_cron' );
	...
	public static function delete_old_export_files() {
		KDNACommon::log_debug( __METHOD__ . '(): Starting.' );
		$uploads_folder = KDNAFormsModel::get_upload_root();
		if ( ! is_dir( $uploads_folder ) || is_link( $uploads_folder ) ) {
			KDNACommon::log_debug( __METHOD__ . '(): No upload root - bailing.' );

			return;
		}
		$export_folder = $uploads_folder . 'export';
		if ( ! is_dir( $export_folder ) || is_link( $export_folder ) ) {
			KDNACommon::log_debug( __METHOD__ . '():  No export root - bailing.' );

			return;
		}
		KDNACommon::log_debug( __METHOD__ . '(): Start deleting old export files' );
		foreach ( KDNACommon::glob( '*.csv', $export_folder . DIRECTORY_SEPARATOR ) as $filename ) {
			$timestamp = filemtime( $filename );
			if ( $timestamp < time() - DAY_IN_SECONDS ) {
				// Delete files over a day old
				KDNACommon::log_debug( __METHOD__ . '(): Proceeding to delete ' . $filename );
				$success = unlink( $filename );
				KDNACommon::log_debug( __METHOD__ . '(): Delete successful: ' . ( $success ? 'yes' : 'no' ) );
			}
		}
	}
	...
	public static function delete_old_log_files() {
		...
		if ( ! is_dir( $logs_folder ) || is_link( $logs_folder ) ) {
			KDNACommon::log_debug( __METHOD__ . '():  No logs folder - bailing.' );

			return;
		}
		KDNACommon::log_debug( __METHOD__ . '(): Start deleting old log files' );
		foreach ( KDNACommon::glob( '*.txt', $logs_folder . DIRECTORY_SEPARATOR ) as $filename ) {
			$timestamp = filemtime( $filename );
	...
	public static function delete_orphaned_entries() {
		...
		if ( version_compare( KDNAFormsModel::get_database_version(), '2.3-beta-1', '<' ) || KDNAFormsModel::has_batch_field_operations() ) {
			return;
		}

		KDNACommon::log_debug( __METHOD__ . '(): Starting to delete orphaned entries' );
		...
		KDNACommon::log_debug( __METHOD__ . '(): Delete result: ' . print_r( $result, true ) );
	}
	...
	public static function load_admin_bar_styles() {
		...
		<style>
			...
			@media screen and ( max-width: 782px ) {
				#wpadminbar #wp-admin-bar-kform-forms .ab-item {
					line-height: 53px;
				...
				#wpadminbar li#wp-admin-bar-kform-forms {
					display: block;
				}
	...
	public static function get_image_sizes() {
		/**
		 * Filters the KDNA Forms image sizes.
		 *
		 * @since 2.9
		 *
		 * @param array  $image_sizes The array of image sizes with their respective attributes.
		 */
		$image_sizes = apply_filters( 'kdnaform_image_sizes', array(
	...
	public static function register_image_sizes() {
		...
		foreach ( $image_sizes as $size => $attributes ) {
			add_image_size( 'kform-' . $size, $attributes['width'], $attributes['height'], $attributes['crop'] );
		}
	}
	...
	public static function remove_image_sizes( $sizes ) {
		$kdna_sizes = self::get_image_sizes();

		foreach( $kdna_sizes as $size => $attributes ) {
			unset( $sizes[ 'kform-' . $size ] );
		}
	...
	public static function preload_webfonts() {
		...
		foreach( $preloaded as $font_file ) {
			$url = KDNACommon::get_font_url( $font_file );
	 ...
	 *
	 * @return void
	 * @deprecated Use kdna_upgrade()->drop_index() instead
	 * @remove-in 3.0
	 */
	public static function drop_index( $table, $index ) {
		_deprecated_function( 'This function has been deprecated. Use kdna_upgrade()->drop_index() instead', '2.2', 'kdna_upgrade()->drop_index()' );

		kdna_upgrade()->drop_index( $table, $index );
	...
	public static function dbdelta_fix_case( $cqueries ) {
		_deprecated_function( 'dbdelta_fix_case', '2.2', 'kdna_upgrade()->dbdelta_fix_case()' );

		return kdna_upgrade()->dbdelta_fix_case( $cqueries );
	}
	...
	public static function setup_database() {
		_deprecated_function( 'This function has been deprecated. Use kdna_upgrade()->upgrade_schema()', '2.2', 'kdna_upgrade()->upgrade_schema()' );

		kdna_upgrade()->upgrade_schema();
	}

	/**
	 * Creates an instance of KDNA_Background_Upgrader and stores it in KDNAForms::$background_upgrader
	 *
	 * @since 2.3
	 */
	...
	public static function action_admin_notices() {
		KDNACommon::display_dismissible_message( false, 'site-wide' );
	}

	/**
	 * Registers the KDNA Forms data exporter.
	 *
	 * @since 2.4
	 *
	...
	public static function register_data_exporter( $exporters ) {
		$exporters['kdnaforms'] = array(
			'exporter_friendly_name' => __( 'KDNA Forms Exporter' ),
			'callback' => array( 'KDNAForms', 'data_exporter' ),
		);
		return $exporters;
	}

	/**
	 * Registers the KDNA Forms data eraser.
	 *
	 * @since 2.4
	 *
	 * @param array $erasers
	...
	public static function register_data_eraser( $erasers ) {
		$erasers['kdnaforms'] = array(
			'eraser_friendly_name' => __( 'KDNA Forms Eraser' ),
			'callback' => array( 'KDNAForms', 'data_eraser' ),
		);
		return $erasers;
	}
	...
	public static function init_buffer() {
		...
		require_once KDNACommon::get_base_path() . '/includes/libraries/class-dom-parser.php';
		...
		ob_start( array( 'KDNAForms', 'ensure_hook_js_output' ) );
	}
 ...
 *
 * Should be used to insert a KDNA Form from code.
 *
 ...
 * @param string $style_settings JSON-encoded style settings. Passing false will bypass the kdnaform_default_styles filter.
 *
 * @return string|void
 */
function kdna_form( $id, $display_title = true, $display_description = true, $display_inactive = false, $field_values = null, $ajax = false, $tabindex = 0, $echo = true, $form_theme = null, $style_settings = null ) {
	if ( ! $echo ) {
		return KDNAForms::get_form( $id, $display_title, $display_description, $display_inactive, $field_values, $ajax, $tabindex, $form_theme, $style_settings );
	}

	echo KDNAForms::get_form( $id, $display_title, $display_description, $display_inactive, $field_values, $ajax, $tabindex, $form_theme, $style_settings ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}

/**
 * @return KDNA_Upgrade
 */
function kdna_upgrade() {
	require_once( KDNACommon::get_base_path() . '/includes/class-kdna-upgrade.php' );

	return KDNA_Upgrade::get_instance();
}
 ...
 *
 * @uses KDNAForms::enqueue_form_scripts()
 *
 * @param string $form_id The form ID.
 * @param bool $is_ajax If the form is submitted via AJAX.  Defaults to false.
 */
function kdna_form_enqueue_scripts( $form_id, $is_ajax = false ) {
	KDNAForms::enqueue_form_scripts( $form_id, $is_ajax );
}
...
if ( ! function_exists( 'rgget' ) ) {
	 ...
	 *
	 * @since 2.9.1 Updated to use KDNAForms::get().
	 *
	...
	function rgget( $name, $array = null ) {
		return KDNAForms::get( $name, $array );
	}
}
...
if ( ! function_exists( 'rgpost' ) ) {
	 ...
	 *
	 * @since 2.9.1 Updated to use KDNAForms::post().
	 *
	 * @param string $name            The key
	...
	function rgpost( $name, $do_stripslashes = true ) {
		return KDNAForms::post( $name, $do_stripslashes );
	}
}
...
if ( ! function_exists( 'kdna_apply_filters' ) ) {
	//function kdna_apply_filters( $filter, $modifiers, $value ) {
	/**
	 * KDNA Forms pre-processing for apply_filters
	 *
	 ...
	 */
	function kdna_apply_filters( $filter, $value ) {
		...
		if ( is_array( $filter ) ) {
			...
			//_deprecated_argument( 'kdna_apply_filters', '1.9.14.20', "Modifiers should no longer be passed as a separate parameter. Combine the filter name and modifier(s) into an array and pass that array as the first parameter of the function. Example: kdna_apply_filters( array( 'action_name', 'mod1', 'mod2' ), \$value, \$arg1, \$arg2 );" );
...
if ( ! function_exists( 'kdna_do_action' ) ) {
	/**
	 * KDNA Forms pre-processing for do_action.
	 *
	 ...
	 */
	function kdna_do_action( $action ) {
		...
		if ( is_array( $action ) ) {
			...
			//_deprecated_argument( 'kdna_do_action', '1.9.14.20', "Modifiers should no longer be passed as a separate parameter. Combine the action name and modifier(s) into an array and pass that array as the first parameter of the function. Example: kdna_do_action( array( 'action_name', 'mod1', 'mod2' ), \$arg1, \$arg2 );" );
...
if ( ! function_exists( 'kdna_has_filters' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function kdna_has_filters( $filter, $function_to_check = false ) {
		$modifiers = array_splice( $filter, 1, count( $filter ) );
		$filter    = $filter[0];
...
if ( ! function_exists( 'kdna_has_filter' ) ) {
	/**
	 * Determines if a callback has been registered for the specified filter.
	 *
	 * @since 2.4.18
	 ...
	 * @return bool
	 */
	function kdna_has_filter( $filter, $function_to_check = false ) {
		return kdna_has_filters( $filter, $function_to_check );
	}
}

if ( ! function_exists( 'kdna_has_action' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function kdna_has_action( $action, $function_to_check = false ) {
		return kdna_has_filters( $action, $function_to_check );
	}
