...
class KDNAFormList {
	...
	public static function form_list_page() {
		...
		<div id="gf_new_form_modal" style="display:none;">
				<div class="kdnaform-settings__wrapper ">
					<div class="kdnaform-settings-panel__content">
						<form class="kdnaform_new_form_modal_container" onsubmit="saveNewForm();return false;">
                            <div id="gf_new_form_error_message" ></div>
							<div class="setting-row kdnaform-settings-field kdnaform-settings-field__text">
								<label class="kdnaform-settings-label" for="new_form_title"><?php esc_html_e( 'Form Title', 'kdnaforms' ); ?>
									<span class="kdnafield_required">*</span></label>
                                <div class="kdnaform-settings-input__container">
                                    <input type="text" class="regular-text" value="" id="new_form_title" tabindex="9000">                                         </div>
							</div>

							<div class="setting-row">
								<label class="kdnaform-settings-label" for="new_form_description"><?php esc_html_e( 'Form Description', 'kdnaforms' ); ?></label>
							...
							<div class="submit-row">
								 ...
								 */
								echo apply_filters( 'kdnaform_new_form_button', '<button type="submit" value="save" id="save_new_form" class="button large primary" tabindex="9002">' . esc_html__( 'Create Form', 'kdnaforms' ) . '</button>' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
							</div>
						</form>
					</div>
				</div>
		...
		<script text="text/javascript">
			...
			function ConfirmDeleteForm(form_id){
				if( confirm(<?php echo json_encode( __( 'WARNING: You are about to delete this form and ALL entries associated with it. ', 'kdnaforms' ) . esc_html__( 'Cancel to stop, OK to delete.', 'kdnaforms' ) ); ?>) ){
					DeleteForm(form_id);
				}
			}
			...
			function gfConfirmBulkAction(element_id) {
				...
				if (jQuery(element).val() == 'delete')
					...
					return confirm(<?php echo json_encode( __( 'Are you sure you would like to reset the Views for the selected forms? ', 'kdnaforms' ) . __( "'Cancel' to stop, 'OK' to reset.", 'kdnaforms' ) ); ?>);
				else if (jQuery(element).val() == 'delete_entries')
					return confirm(<?php echo json_encode( __( 'WARNING: You are about to delete ALL entries associated with the selected forms. ', 'kdnaforms' ) . __( "'Cancel' to stop, 'OK' to delete.", 'kdnaforms' ) ); ?>);
		...
		?>
                ...
                <div class="kdnaform-settings-panel__content form-list">
                    <div class="form-list-head">
                    <h2> <?php esc_html_e( 'Forms', 'kdnaforms' ); ?> </h2>
                        <?php if ( KDNACommon::current_user_can_any( 'kdnaforms_create_form' ) ) {
                            echo '<button class="button kdnaform-add-new-form primary add-new-h2" data-js="kdnaform-add-new-form">' . esc_html__( 'Add New', 'kdnaforms' ) . '</button>';
                        } ?>
                    </div>
                    ...
                    <?php
                        ...
                        $table->search_box( esc_html__( 'Search Forms', 'kdnaforms' ), 'form' );
                    ?>
                    </form>
                    </div>
		...
		<script type="text/javascript">
			jQuery( document ).ready( function( $ ) {
				...
				$( '.kdna_form_action_has_submenu' ).hover( function() {
					...
					var $subMenu = $this.find( '.kdnaform-form-toolbar__submenu' );
					...
					$( this ).find( '.kdnaform-form-toolbar__submenu' )
						.css( 'height', '' )
						.hide();
				} );
				...
				gfPageLoaded = true;
				$( '.kdnaform_active_icon' ).removeClass( 'gf_not_ready' );
			...
			function saveNewForm() {
				...
				var form = {
					...
					button: {
						type: 'text',
						text: <?php echo json_encode( esc_html__( 'Submit', 'kdnaforms' ) ); ?>,
						imageUrl : ''
					},
					fields:[]
				}
				...
				jQuery.post(ajaxurl, {
					...
					if(respData['error']) {
						...
						createButton.val(<?php echo json_encode( esc_html__( 'Saved! Redirecting...', 'kdnaforms' ) ); ?>);
					}
			...
			function resetNewFormModal() {
				...
				removeInputErrorIcons( '.kdnaform_new_form_modal_container' );
			}

			function addInputErrorIcon( elem ) {
				var elem = jQuery(elem);
				elem.after( '<span class="kdnaform-settings-field__feedback kdnaform-settings-field__feedback--invalid" aria-hidden="true"></span>' );
			}

			function removeInputErrorIcons( elem ) {
				var elem = jQuery(elem);
				elem.find('span.kdnaform-settings-field__feedback--invalid').remove();
			}
	...
	public static function get_screen_options_markup( $status, $args ) {
		...
		$return = $status;
		if ( ! KDNAForms::get_page() == 'form_list' ) {
			return $return;
		}
		...
		$per_page_label       = esc_html__( 'Forms per page:', 'kdnaforms' );
		$sort_options_title   = esc_html__( 'Sorting Options', 'kdnaforms' );
		...
		$return .= "
			...
			<legend>{$pagination_title}</legend>
				<label for='kdnaform_per_page'>{$per_page_label}</label>
				<input type='number' step='1' min='1' class='screen-per-page' name='kdnaform_per_page' id='kdnaform_per_page' value='{$per_page}' />
				<input type='hidden' name='wp_screen_options[option]' value='kdnaform_forms_screen_options' />
	...
	public static function get_screen_options_attributes() {
		return array(
			'order_by' => array(
				'label'   => __( 'Default Sort Column', 'kdnaforms' ),
				'choices' => array(
					'is_active'   => __( 'Status', 'kdnaforms' ),
					'title'       => __( 'Title', 'kdnaforms' ),
					'id'          => __( 'ID', 'kdnaforms' ),
					'entry_count' => __( 'Entries', 'kdnaforms' ),
					'view_count'  => __( 'Views', 'kdnaforms' ),
					'conversion'  => __( 'Conversion', 'kdnaforms' ),
				),
			),
			'sort_order' => array(
				'label'   => __( 'Default Sort Order', 'kdnaforms' ),
				'choices' => array(
					'ASC'  => __( 'Ascending', 'kdnaforms' ),
					'DESC' => __( 'Descending', 'kdnaforms' ),
				),
			),
		);
	}
	...
	public static function get_screen_options_values() {
		...
		$option_values = get_user_option( 'kdnaform_forms_screen_options' );

		// Prior to 2.9.2, the per_page value was stored in a separate option.
		$old_per_page = get_user_option( 'kdnaform_forms_per_page' );
...
class KDNA_Form_List_Table extends WP_List_Table {
	...
	function get_views() {
		$form_count = KDNAFormsModel::get_form_count();
		 ...
		 */
		$form_count = apply_filters( 'kdnaform_form_list_count', $form_count );
	...
	function prepare_items() {
		...
		$sort_options = KDNAFormList::get_screen_options_values();
		...
		if ( rgblank( $search_query ) ) {
			$forms = KDNAFormsModel::get_forms( $active, $sort_column, $sort_direction, $trash );
		} else {
			$forms = KDNAFormsModel::search_forms( $search_query, $active, $sort_column, $sort_direction, $trash );
		}
		 ...
		 */
		$forms = apply_filters( 'kdnaform_form_list_forms', $forms, $search_query, $active, $sort_column, $sort_direction, $trash );

		$per_page = $sort_options['per_page'];

		$per_page = apply_filters( 'kdnaform_page_size_form_list', $per_page );
	...
	function get_bulk_actions() {
		if ( $this->filter == 'trash' ) {
			$actions = array(
				'restore' => esc_html__( 'Restore', 'kdnaforms' ),
				'delete' => esc_html__( 'Delete permanently', 'kdnaforms' ),
			);
		} else {
			$actions = array(
				'activate' => esc_html__( 'Mark as Active', 'kdnaforms' ),
				'deactivate' => esc_html__( 'Mark as Inactive', 'kdnaforms' ),
				'reset_views' => esc_html__( 'Reset Views', 'kdnaforms' ),
				'delete_entries' => esc_html__( 'Permanently Delete Entries', 'kdnaforms' ),
				'trash' => esc_html__( 'Move to trash', 'kdnaforms' ),
			);
		}
		return $actions;
	}
	...
	function get_columns() {
		...
		$columns = array(
			...
			'is_active'   => esc_html__( 'Status', 'kdnaforms' ),
			'title'       => esc_html__( 'Title', 'kdnaforms' ),
			'id'          => esc_html__( 'ID', 'kdnaforms' ),
			'entry_count' => esc_html__( 'Entries', 'kdnaforms' ),
			'view_count'  => esc_html__( 'Views', 'kdnaforms' ),
			'conversion'  => esc_html__( 'Conversion', 'kdnaforms' ),
		);

		$columns = apply_filters( 'kdnaform_form_list_columns', $columns );
	...
	function single_row_columns( $item ) {
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			if ( 'cb' === $column_name ) {
				...
				echo '</th>';
			} elseif ( has_action( 'kdnaform_form_list_column_' . $column_name ) ) {
				echo "<td $attributes>"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				do_action( 'kdnaform_form_list_column_' . $column_name, $item );
		...
		<div class="row-actions">
			...
			if ( $this->filter == 'trash' ) {
				$form_actions['restore'] = array(
					'label'        => __( 'Restore', 'kdnaforms' ),
					'url'          => '#',
					'onclick'      => 'RestoreForm(' . absint( $form->id ) . ');',
					'onkeypress'   => 'RestoreForm(' . absint( $form->id ) . ');',
					'capabilities' => 'kdnaforms_delete_forms',
					'priority'     => 600,
				);
				$form_actions['delete']  = array(
					'label'        => __( 'Delete permanently', 'kdnaforms' ),
					'menu_class'   => 'delete',
					'url'          => '#',
				...
				$form_actions = KDNAForms::get_toolbar_menu_items( $form->id, true );

				$form_actions['duplicate'] = array(
					'label'        => __( 'Duplicate', 'kdnaforms' ),
					'url'          => '#',
					'onclick'      => 'DuplicateForm(' . absint( $form->id ) . ');return false;',
					'onkeypress'   => 'DuplicateForm(' . absint( $form->id ) . ');return false;',
					'capabilities' => 'kdnaforms_create_form',
					'priority'     => 600,
				);

				$form_actions['trash'] = array(
					'label'        => __( 'Trash', 'kdnaforms' ),
					'aria-label'        => __( 'Move this form to the trash', 'kdnaforms' ),
					'url'          => '#',
			...
			$form_actions = apply_filters( 'kdnaform_form_actions', $form_actions, $form->id );

			echo KDNAForms::format_toolbar_menu_items( $form_actions, true ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	...
	function no_items() {
		if ( rgget( 's' ) ) {
			printf(
				esc_html__( "No forms were found for your search query. %sView all forms%s.", 'kdnaforms' ),
	...
	function process_action() {
		...
		if ( $single_action ) {
			...
			switch ( $single_action ) {
				case 'trash' :
					if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_forms' ) ) {
						...
						$message       = is_wp_error( $trashed ) ? $trashed->get_error_message() : __( 'Form moved to the trash.', 'kdnaforms' );
						$message_class = is_wp_error( $trashed ) ? 'error' : 'success';
					} else {
						$message       = __( "You don't have adequate permission to trash forms.", 'kdnaforms' );
						$message_class = 'error';
					}
					break;
				case 'restore' :
					if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_forms' ) ) {
						$restored      = KDNAFormsModel::restore_form( $form_id );
						$message       = is_wp_error( $restored ) ? $restored->get_error_message() : __( 'Form restored.', 'kdnaforms' );
						$message_class = is_wp_error( $restored ) ? 'error' : 'success';
					} else {
						$message       = __( "You don't have adequate permission to restore forms.", 'kdnaforms' );
						$message_class = 'error';
					}
					break;
				case 'delete' :
					if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_forms' ) ) {
						$deleted = KDNAFormsModel::delete_form( $form_id );
					    $message = is_wp_error( $deleted ) ? $deleted->get_error_message() : __( 'Form deleted.', 'kdnaforms' );
					    $message_class = is_wp_error( $deleted ) ? 'error' : 'success';
					} else {
						$message = __( "You don't have adequate permission to delete forms.", 'kdnaforms' );
						$message_class = 'error';
					}
					break;
				case 'duplicate' :
					if ( KDNACommon::current_user_can_any( 'kdnaforms_create_form' ) ) {
						...
						$message       = is_wp_error( $duplicated ) ? $duplicated->get_error_message() : __( 'Form duplicated.', 'kdnaforms' );
						$message_class = is_wp_error( $duplicated ) ? 'error' : 'success';
					} else {
						$message       = __( "You don't have adequate permission to duplicate forms.", 'kdnaforms' );
						$message_class = 'error';
					}
					break;
			...
			switch ( $bulk_action ) {
				case 'trash':
					if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_forms' ) ) {
						KDNAFormsModel::trash_forms( $form_ids );
						$message = _n( '%s form moved to the trash.', '%s forms moved to the trash.', $form_count, 'kdnaforms' );
					} else {
						$message = __( "You don't have adequate permissions to trash forms.", 'kdnaforms' );
					}
					break;
				case 'restore':
					if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_forms' ) ) {
						KDNAFormsModel::restore_forms( $form_ids );
						$message = _n( '%s form restored.', '%s forms restored.', $form_count, 'kdnaforms' );
					} else {
						$message = __( "You don't have adequate permissions to restore forms.", 'kdnaforms' );
					}
					break;
				case 'delete':
					if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_forms' ) ) {
						KDNAFormsModel::delete_forms( $form_ids );
						$message = _n( '%s form deleted.', '%s forms deleted.', $form_count, 'kdnaforms' );
					} else {
						$message = __( "You don't have adequate permissions to delete forms.", 'kdnaforms' );
					}
					break;
				case 'reset_views':
					foreach ( $form_ids as $form_id ) {
						KDNAFormsModel::delete_views( $form_id );
					}
					KDNACache::delete( 'get_view_count_per_form' );
					$message = _n( 'Views for %s form have been reset.', 'Views for %s forms have been reset.', $form_count, 'kdnaforms' );
					break;
				case 'delete_entries':
					if ( KDNACommon::current_user_can_any( 'kdnaforms_delete_entries' ) ) {
						foreach ( $form_ids as $form_id ) {
							KDNAFormsModel::delete_leads_by_form( $form_id );
						}
						$message = _n( 'Entries for %s form have been deleted.', 'Entries for %s forms have been deleted.', $form_count, 'kdnaforms' );
					} else {
						$message = __( "You don't have adequate permission to delete entries.", 'kdnaforms' );
					}

					break;
				case 'activate':
					foreach ( $form_ids as $form_id ) {
						KDNAFormsModel::update_form_active( $form_id, 1 );
					}
					$message = _n( '%s form has been marked as active.', '%s forms have been marked as active.', $form_count, 'kdnaforms' );
					break;
				case 'deactivate':
					foreach ( $form_ids as $form_id ) {
						KDNAFormsModel::update_form_active( $form_id, 0 );
					}
					$message = _n( '%s form has been marked as inactive.', '%s forms have been marked as inactive.', $form_count, 'kdnaforms' );
					break;
			}
