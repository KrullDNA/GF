...
/**
 * Class KDNANotification
 * Handles notifications within KDNA Forms
 */
Class KDNANotification {
	 ...
	 * @access public
	 *
	 * @uses KDNANotification::notification_edit_page()
	 * @uses KDNANotification::notification_list_page()
	 *
	 * @return void
	 */
	 ...
	 * @access public
	 *
	 * @used-by KDNANotification::notification_page()
	 * @uses    KDNAFormsModel::get_form_meta()
	 * @uses    KDNANotification::get_notification()
	 * @uses    KDNANotification::validate_notification
	 * @uses    KDNAFormsModel::sanitize_conditional_logic()
	 * @uses    KDNAFormsModel::trim_conditional_logic_values_from_element()
	 * @uses    KDNAFormsModel::save_form_notifications()
	 * @uses    KDNACommon::add_message()
	 * @uses    KDNACommon::json_decode()
	 * @uses    KDNACommon::add_error_message()
	 * @uses    KDNAFormSettings::page_header()
	 * @uses    KDNANotification::get_notification_ui_settings()
	 * @uses    SCRIPT_DEBUG
	 * @uses    KDNAFormsModel::get_entry_meta()
	 * @uses    KDNAFormSettings::output_field_scripts()
	 * @uses    KDNAFormSettings::page_footer()
	 *
	...
	public static function notification_edit_page( $form_id, $notification_id ) {
		...
		KDNAFormSettings::page_header( esc_html__( 'Notifications', 'kdnaforms' ) );
	...
	private static function settings_fields( $notification, $form ) {
		...
		if ( 'hidden' === rgar( $notification, 'toType' ) ) {
			...
			$to_type = array(
				'name'          => 'toType',
				'label'         => esc_html__( 'Send To', 'kdnaforms' ),
				'tooltip'       => kdnaform_tooltip( 'notification_send_to_email', null, true ),
				'type'          => 'radio',
				'horizontal'    => true,
				'choices'       => array(
					array(
					...
					array(
						'label' => esc_html__( 'Select a Field', 'kdnaforms' ),
						'value' => 'field',
					),
					array(
						'label'   => esc_html__( 'Configure Routing', 'kdnaforms' ),
						'value'   => 'routing',
						'tooltip' => kdnaform_tooltip( 'notification_send_to_routing', null, true ),
					),
				),
				'default_value' => 'email',
			);
		...
		$fields = array(
			array(
				...
				'fields' => array(
					...
					array(
						'name'          => 'service',
						'label'         => esc_html__( 'Email Service', 'kdnaforms' ),
						'type'          => 'radio',
					...
					array(
						'name'    => 'event',
						'label'   => esc_html__( 'Event', 'kdnaforms' ),
						'tooltip' => kdnaform_tooltip( 'notification_event', null, true ),
						'type'    => 'select',
					...
					array(
						'name'                => 'toEmail',
						'label'               => esc_html__( 'Send To Email', 'kdnaforms' ),
						'type'                => 'text',
						...
						'validation_callback' => function( $field, $value ) {
							...
							// Determine if valid.
							$is_valid = KDNANotification::is_valid_notification_email( $value );

							// Get filter parameters.
							$to_type  = KDNANotification::get_settings_renderer()->get_value( 'toType' );
							$to_field = KDNANotification::get_settings_renderer()->get_value( 'toField' );
							 ...
							 * @param string $kdnaform_notification_to_type  The type of destination.
							 * @param string $kdnaform_notification_to_email The destination email address, if available.
							 * @param string $kdnaform_notification_to_field The field that is being used for the notification, if available.
							 */
							$is_valid = apply_filters( 'kdnaform_is_valid_notification_to', $is_valid, $to_type, $value, $to_field );

							if ( ! $is_valid ) {
								$field->set_error( __( 'Please enter a valid email address.', 'kdnaforms' ) );
							}
					...
					array(
						'name'                => 'toField',
						'label'               => esc_html__( 'Send To Field', 'kdnaforms' ),
						'type'                => 'field_select',
						'required'            => true,
						'args'                => array( 'input_types' => array( 'email' ) ),
						'no_choices'          => esc_html__( 'Your form does not have an email field. Add an email field to your form and try again.', 'kdnaforms' ),
						...
						'validation_callback' => function ( $field, $value ) {
							...
							// Get filter parameters.
							$to_type  = KDNANotification::get_settings_renderer()->get_value( 'toType' );
							$to_email = KDNANotification::get_settings_renderer()->get_value( 'toEmail' );
							 ...
							 * @param string $kdnaform_notification_to_field The field that is being used for the notification, if available.
							 */
							$is_valid = apply_filters( 'kdnaform_is_valid_notification_to', ! empty( $value ), $to_type, $to_email, $value );

							if ( ! $is_valid ) {
								$field->set_error( __( 'Please select an Email Address field.', 'kdnaforms' ) );
							}
					...
					array(
						'name'    => 'fromName',
						'label'   => esc_html__( 'From Name', 'kdnaforms' ),
						'tooltip' => kdnaform_tooltip( 'notification_from_name', null, true ),
						'type'    => 'text',
						'class'   => 'merge-tag-support mt-position-right mt-hide_all_fields',
					),
					array(
						'name'                => 'from',
						'label'               => esc_html__( 'From Email', 'kdnaforms' ),
						'tooltip'             => kdnaform_tooltip( 'notification_from_email', null, true ),
						'type'                => 'text',
						'class'               => 'merge-tag-support mt-position-right mt-hide_all_fields',
						'default_value'       => '{admin_email}',
						'after_input'         => $from_email_warning,
						'validation_callback' => function( $field, $value ) {
							if ( ! empty( $value ) && ! KDNANotification::is_valid_notification_email( $value ) ) {
								$field->set_error( __( 'Please enter a valid email address or merge tag in the From Email field.', 'kdnaforms' ) );
							}
						},
					),
					array(
						'name'                => 'replyTo',
						'label'               => esc_html__( 'Reply To', 'kdnaforms' ),
						'tooltip'             => kdnaform_tooltip( 'notification_reply_to', null, true ),
						'type'                => 'text',
						'class'               => 'merge-tag-support mt-position-right mt-hide_all_fields',
						'validation_callback' => function( $field, $value ) {
							if ( ! empty( $value ) && ! KDNANotification::is_valid_notification_email( $value ) ) {
								$field->set_error( __( 'Please enter a valid email address or merge tag in the Reply To field.', 'kdnaforms' ) );
							}
						},
					),
					array(
						'name'                => 'cc',
						'label'               => esc_html__( 'CC', 'kdnaforms' ),
						'tooltip'             => kdnaform_tooltip( 'notification_cc', null, true ),
						'type'                => 'text',
						'class'               => 'merge-tag-support mt-position-right mt-hide_all_fields',
						...
						'validation_callback' => function( $field, $value ) {
							if ( ! empty( $value ) && ! KDNANotification::is_valid_notification_email( $value ) ) {
								$field->set_error( __( 'Please enter a valid email address or merge tag in the CC field.', 'kdnaforms' ) );
							}
						},
					),
					array(
						'name'                => 'bcc',
						'label'               => esc_html__( 'BCC', 'kdnaforms' ),
						'tooltip'             => kdnaform_tooltip( 'notification_bcc', null, true ),
						'type'                => 'text',
						'class'               => 'merge-tag-support mt-position-right mt-hide_all_fields',
						'validation_callback' => function( $field, $value ) {
							if ( ! empty( $value ) && ! KDNANotification::is_valid_notification_email( $value ) ) {
								$field->set_error( __( 'Please enter a valid email address or merge tag in the BCC field.', 'kdnaforms' ) );
							}
						},
					),
					array(
					...
					array(
						'name'       => 'message',
						'label'      => esc_html__( 'Message', 'kdnaforms' ),
						'type'       => 'textarea',
						'use_editor' => true,
						'required'   => true,
					),
					array(
						...
						'label'   => esc_html__( 'Auto-formatting', 'kdnaforms' ),
						'tooltip' => kdnaform_tooltip( 'notification_autoformat', null, true ),
						'type'    => 'checkbox',
						'choices' => array(
							array(
								'name'  => 'disableAutoformat',
								'label' => esc_html__( 'Disable auto-formatting', 'kdnaforms' ),
							),
						),
					),
					array(
						...
						'tooltip' => kdnaform_tooltip( 'notification_attachments', null, true ),
						'type'    => 'checkbox',
						'choices' => array(
							array(
								'name'  => 'enableAttachments',
								'label' => esc_html__( 'Attach uploaded fields to notification', 'kdnaforms' ),
							),
						),
					...
					array(
						...
						'checkbox'    => array(
							'label'  => esc_html__( 'Enable conditional logic', 'kdnaforms' ),
							'hidden' => false,
						),
					),
					array(
						'type'  => 'save',
						'value' => esc_html__( 'Update Notification', 'kdnaforms' ),
					),
				),
			),
		);
	...
	/**
	 * Pass the field choices for the select field through the kdnaform_email_fields_notification_admin filter to allow
	...
	private static function append_legacy_settings_fields( $fields, $notification, $form ) {
		 ...
		 */
		$legacy_settings = apply_filters( 'kdnaform_notification_ui_settings', array(), $notification, $form, null );

		if ( has_filter( 'kdnaform_notification_ui_settings' ) ) {
			trigger_error( 'kdnaform_notification_ui_settings is deprecated and will be removed in version 3.0.', E_USER_DEPRECATED ); // phpcs:ignore QITStandard.PHP.DebugCode.DebugFunctionFound
		}
		...
		$renderer = new Settings(
			array(
				...
				'save_callback'  => function( $values ) use ( &$notification, &$form, &$notification_id ) {
					...
					$notification = KDNAFormSettings::save_changed_form_settings_fields( $notification, $values );
					...
					$notification['conditionalLogic'] = rgar( $values, 'notification_conditional_logic' ) && is_array( $conditionalLogicObject ) ? KDNAFormsModel::sanitize_conditional_logic( $conditionalLogicObject ) : null;

					if ( isset( $values['routing'] ) && ! empty( $values['routing'] ) ) {
						$routing_logic           = array( 'rules' => $values['routing'] );
						$routing_logic           = KDNAFormsModel::sanitize_conditional_logic( $routing_logic );
						$notification['routing'] = $routing_logic['rules'];
					}

					$notification = KDNACommon::fix_notification_routing( $notification );
					...
					// Save notification.
					$notification                              = KDNAFormsModel::trim_conditional_logic_values_from_element( $notification, $form );
					$form['notifications'][ $notification_id ] = $notification;
					KDNAFormsModel::save_form_notifications( $form['id'], $form['notifications'] );
				...
				'before_fields' => function() use ( &$form, $form_id, &$notification, $notification_id ) {
					...
					<script type="text/javascript">
						...
						function MaybeAddSaveLinkMergeTag( mergeTags, elementId, hideAllFields, excludeFieldTypes, isPrepop, option ) {
							...
							if ( event === 'form_saved' || event === 'form_save_email_requested' ) {
								mergeTags['other'].tags.push( {
									tag:  '{save_link}',
									label: <?php echo json_encode( esc_html__( 'Save & Continue Link', 'kdnaforms' ) ); ?>
								} );
								mergeTags['other'].tags.push( {
									tag:   '{save_token}',
									label: <?php echo json_encode( esc_html__( 'Save & Continue Token', 'kdnaforms' ) ); ?>
								} );
							}
							return mergeTags;
						}
						 ...
						 * @param int   $notification_id The notification ID
						 */
						$entry_meta = apply_filters( 'kdnaform_entry_meta_conditional_logic_notifications', $entry_meta, $form, $notification_id );

						?>

						var form = <?php echo json_encode( $form ) ?>;
						var current_notification = <?php echo KDNACommon::json_encode( $notification ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>;
	 ...
	 *
	 * @used-by KDNANotification::notification_page()
	 * @uses    KDNANotification::maybe_process_notification_list_action()
	 * @uses    KDNAFormsModel::get_form_meta()
	 * @uses    KDNAFormSettings::page_header()
	 * @uses    KDNANotificationTable::__construct()
	 * @uses    KDNANotificationTable::prepare_items()
	 * @uses    KDNANotificationTable::display()
	 * @uses    KDNAFormSettings::page_footer()
	 *
	...
	public static function notification_list_page( $form_id ) {
		...
		$notification_table = new KDNANotificationTable( $form );
		...
		<div class="gform-settings-panel">
			...
			<div class="gform-settings-panel__content">
				...
				<form id="notification_list_form" method="post">
					...
					wp_nonce_field( 'kdnaform_notification_list_action', 'kdnaform_notification_list_action' );
					?>
				...
				jQuery.ajax(
					{
						...
						error:    function() {
							...
							alert( '<?php echo esc_js( __( 'Ajax error while updating form', 'kdnaforms' ) ); ?>' );
						}
					}
				);
	 ...
	 * @uses    KDNANotification::delete_notification()
	 * @uses    KDNANotification::duplicate_notification()
	 * @uses    KDNACommon::add_message()
	 * @uses    KDNACommon::add_error_message()
	 *
	 * @return void
	...
	public static function maybe_process_notification_list_action() {
		...
		if ( empty( $_POST ) || ! check_admin_referer( 'kdnaform_notification_list_action', 'kdnaform_notification_list_action' ) ) {
			return;
		}
		...
		switch ( $action ) {
			case 'delete':
				$notification_deleted = KDNANotification::delete_notification( $object_id, rgget( 'id' ) );
				if ( $notification_deleted ) {
					KDNACommon::add_message( esc_html__( 'Notification deleted.', 'kdnaforms' ) );
				} else {
					KDNACommon::add_error_message( esc_html__( 'There was an issue deleting this notification.', 'kdnaforms' ) );
				}
				break;
			case 'duplicate':
				$notification_duplicated = KDNANotification::duplicate_notification( $object_id, rgget( 'id' ) );
				if ( $notification_duplicated ) {
					KDNACommon::add_message( esc_html__( 'Notification duplicated.', 'kdnaforms' ) );
				} else {
					KDNACommon::add_error_message( esc_html__( 'There was an issue duplicating this notification.', 'kdnaforms' ) );
				}
				break;
		}
	...
	public static function get_notification_services() {
		...
		$services = array(
			'wordpress' => array(
				'label' => esc_html__( 'WordPress', 'kdnaforms' ),
	...
	public static function get_notification_events( $form ) {
		$notification_events = array( 'form_submission' => esc_html__( 'Form is submitted', 'kdnaforms' ) );
		if ( rgars( $form, 'save/enabled' ) ) {
			$notification_events['form_saved']                = esc_html__( 'Form is saved', 'kdnaforms' );
			$notification_events['form_save_email_requested'] = esc_html__( 'Save and continue email is requested', 'kdnaforms' );
		}
		 ...
		 */
		return apply_filters( 'kdnaform_notification_events', $notification_events, $form );
	}
	 ...
	 * @access private
	 *
	 * @uses KDNACommon::is_invalid_or_empty_email()
	 *
	...
	public static function is_valid_notification_email( $text ) {
		...
		foreach ( $emails as $email ) {
			$email            = trim( $email );
			$invalid_email    = KDNACommon::is_invalid_or_empty_email( $email );
	...
	private static function is_site_domain_in_from( $from_email ) {
		...
		return KDNACommon::email_domain_matches( $from_email );
	 ...
	 * @access public
	 *
	 * @uses KDNANotification::$supported_fields()
	 *
	...
	public static function get_routing_field_types() {
		 ...
		 * @since 1.9.6
		 *
		 * @param array KDNANotification::$supported_fields Currently supported field types.
		 */
		$field_types = apply_filters( 'kdnaform_routing_field_types', self::$supported_fields );
		return $field_types;
	}
	 ...
	 *
	 * @uses KDNAFormsModel::get_form_meta()
	 * @uses KDNAFormsModel::flush_current_forms()
	 * @uses KDNAFormsModel::save_form_notifications()
	 *
	...
	public static function delete_notification( $notification_id, $form_id ) {
		...
		$form = ! is_array( $form_id ) ? KDNAFormsModel::get_form_meta( $form_id ) : $form_id;
		 ...
		 */
		do_action( 'kdnaform_pre_notification_deleted', $form['notifications'][ $notification_id ], $form );
		...
		return KDNAFormsModel::save_form_notifications( $form['id'], $form['notifications'] );
	}
	 ...
	 *
	 * @uses KDNAFormsModel::get_form_meta()
	 * @uses KDNANotification::is_unique_name()
	 * @uses KDNAFormsModel::flush_current_forms()
	 * @uses KDNAFormsModel::save_form_notifications()
	 *
	...
	public static function duplicate_notification( $notification_id, $form_id ) {
		...
		$new_notification = KDNACommon::fix_notification_routing( $new_notification );
...
/**
 * Class KDNANotificationTable.
 *
 * Extends WP_List_Table to display the notifications list.
 *
 * @uses WP_List_Table
 */
class KDNANotificationTable extends WP_List_Table {
	...
	/**
	 * KDNANotificationTable constructor.
	 *
	 ...
	 * @access public
	 *
	 * @uses KDNANotification::get_notification_events()
	 * @uses KDNANotification::get_notification_services()
	 * @uses KDNANotificationTable::$form
	 * @uses KDNANotificationTable::$notification_events
	 * @uses KDNANotificationTable::$notification_services
	...
	function __construct( $form ) {
		...
		$this->form                  = $form;
		$this->notification_events   = KDNANotification::get_notification_events( $form );
		$this->notification_services = KDNANotification::get_notification_services();

		$columns = array(
			'cb'      => '',
			'name'    => esc_html__( 'Name', 'kdnaforms' ),
			'subject' => esc_html__( 'Subject', 'kdnaforms' ),
		);

		if ( count( $this->notification_events ) > 1 ) {
			$columns['event'] = esc_html__( 'Event', 'kdnaforms' );
		}

		if ( count( $this->notification_services ) > 1 ) {
			$columns['service'] = esc_html__( 'Service', 'kdnaforms' );
		}
	...
	function column_name( $item ) {
		...
		$actions  = apply_filters(
			'kdnaform_notification_actions', array(
				'edit'      => '<a href="' . esc_url( $edit_url ) . '">' . esc_html__( 'Edit', 'kdnaforms' ) . '</a>',
				'duplicate' => '<a href="javascript:void(0);" onclick="javascript: DuplicateNotification(\'' . esc_js( $item['id'] ) . '\');" onkeypress="javascript: DuplicateNotification(\'' . esc_js( $item['id'] ) . '\');" style="cursor:pointer;">' . esc_html__( 'Duplicate', 'kdnaforms' ) . '</a>',
				'delete'    => '<a href="javascript:void(0);" class="submitdelete" onclick="javascript: if(confirm(\'' . esc_js( esc_html__( 'WARNING: You are about to delete this notification.', 'kdnaforms' ) ) . esc_js( esc_html__( "'Cancel' to stop, 'OK' to delete.", 'kdnaforms' ) ) . '\')){ DeleteNotification(\'' . esc_js( $item['id'] ) . '\'); }" onkeypress="javascript: if(confirm(\'' . esc_js( esc_html__( 'WARNING: You are about to delete this notification.', 'kdnaforms' ) ) . esc_js( esc_html__( "'Cancel' to stop, 'OK' to delete.", 'kdnaforms' ) ) . '\')){ DeleteNotification(\'' . esc_js( $item['id'] ) . '\'); }" style="cursor:pointer;">' . esc_html__( 'Delete', 'kdnaforms' ) . '</a>'
			)
		);
	...
	function column_service( $notification ) {
		...
		if ( ! rgar( $notification, 'service' ) ) {
			esc_html_e( 'WordPress', 'kdnaforms' );
			...
			esc_html_e( 'Undefined Service', 'kdnaforms' );
		}
	...
	function no_items() {
		...
		printf( esc_html__( "This form doesn't have any notifications. Let's go %screate one%s.", 'kdnaforms' ), "<a href='" . esc_url( $url ) . "'>", '</a>' );
	}
